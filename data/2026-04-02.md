...
Motivation: 现有机器人基础策略主要通过大规模模仿学习训练，但在长时程任务中因分布偏移和误差累积而表现不佳。虽然强化学习可以微调这些模型，但需要人工奖励工程才能在不同任务上良好工作。

Method: VLLR框架结合：(1) 使用LLM分解任务为可验证子任务，VLM估计进度作为外部奖励；(2) 基于策略自确定性的内部奖励。VLM初始化价值函数用于简短预热阶段，避免完整训练中的高昂推理成本；自确定性在整个PPO微调过程中提供每步内在指导。

Result: 在CHORES基准（移动操作和导航任务）上，VLLR相比预训练策略获得高达56%的绝对成功率提升，相比最先进的RL微调方法在分布内任务上提升高达5%，在分布外任务上提升高达10%，且无需人工奖励工程。

Conclusion: VLLR通过结合LLM/VLM的外部奖励和策略自确定性的内部奖励，有效解决了机器人基础策略微调中的奖励工程问题，在分布内和分布外任务上都取得了显著性能提升，展示了互补的益处。
...
TL;DR: MRReP：一种混合现实界面，允许用户通过手势在物理地面上绘制参考路径，并将其集成到机器人导航系统中，以提高路径指定精度和用户体验。


<details>
  <summary>Details</summary>
Motivation: 在人类共享的室内环境中，自主移动机器人需要反映人类空间意图的路径，但传统路径规划器主要优化几何成本，对人类操作者的显式路径指定支持有限。

Method: 提出MRReP混合现实界面，用户可通过手势在物理地面上直接绘制手绘参考路径（HRP），并通过定制的手绘参考路径规划器将用户指定的点序列转换为自主导航的全局路径。

Result: 在与传统2D基线界面的受试者内实验中，MRReP提高了路径指定精度、可用性和感知工作量，同时在物理环境中实现了更稳定的路径指定。

Conclusion: 混合现实中的直接路径指定是将人类空间意图融入移动机器人导航的有效方法。
...
### [3] [Advancing Multi-Robot Networks via MLLM-Driven Sensing, Communication, and Computation: A Comprehensive Survey](https://arxiv.org/abs/2604.00061)
...
TL;DR: 这篇综述探讨了在多模态大语言模型指导下，通过意图到资源的协同优化实现多机器人协调的R2X框架，展示了四个端到端演示并验证了系统级性能优势。


<details>
  <summary>Details</summary>
Motivation: 随着人形机器人在物流、制造、救援等领域的应用，多机器人协调任务需要处理大量传感器数据。现有系统面临通信带宽限制和计算资源约束，需要一种能够根据高层自然语言指令优化感知、通信和计算资源的智能协调框架。

Method: 提出R2X框架，将意图理解作为资源优化的过滤器，通过MLLM理解任务目标，选择性激活相关感知模态，动态分配带宽，确定计算部署位置。综述了最先进的感知模态、通信策略和计算方法，展示了四个端到端演示系统。

Result: 展示了四个具体应用案例：(1)具有预测链路上下文的数字孪生仓库导航；(2)移动驱动的主动MCS控制；(3)具有语义感知开关的FollowMe机器人；(4)通过边缘辅助MLLM实现的真实硬件开放词汇垃圾分类。系统级指标显示R2X在负载、延迟和成功率方面优于纯设备端基线。

Conclusion: R2X框架通过将意图理解与资源优化相结合，实现了多机器人在MLLM指导下的高效协调。这种集成设计方法能够根据任务需求动态调整感知、通信和计算资源，在资源约束下最大化任务成功率，为多机器人系统提供了系统级的优化解决方案。
...
TL;DR: 提出分层规划控制框架，结合MILP-MPC长时程规划和CBF安全滤波，实现多面体机器人在多面体障碍环境中的几何感知安全导航


<details>
  <summary>Details</summary>
Motivation: 复杂非凸环境中考虑机器人动力学、控制限制和精确几何形状的自主导航仍然具有挑战性，需要同时满足长时程引导和几何感知安全保证

Method: 分层框架：高层使用MILP-MPC生成名义轨迹（将机器人简化为质点）；低层使用基于闵可夫斯基差空间精确符号距离的CBF作为安全滤波器，并扩展为高阶CBF，显式执行机器人几何约束

Result: 在U形和迷宫环境中测试单积分器和双积分器动力学，结果表明该框架缓解了纯反应式CBF导航的拓扑诱导局部最小值问题，实现了安全、实时、几何感知的导航

Conclusion: 提出的分层规划控制框架成功结合了长时程轨迹规划和几何感知安全保证，为多面体机器人在复杂障碍环境中的导航提供了有效解决方案
...
TL;DR: 本文提出了一种改进的机器人运动控制框架，通过直接在仿人机器人运动空间训练引导扩散模型，整合人类和机器人数据集，实现更鲁棒的自主操作技能学习。


<details>
  <summary>Details</summary>
Motivation: 仿人机器人的鲁棒自主操作技能开发仍是一个开放问题。虽然强化学习在腿部运动控制上取得成功，但将其应用于复杂的交互式操作任务面临长期规划挑战。现有方法DreamControl使用人类运动扩散模型作为生成先验来指导强化学习策略，但存在局限性。

Method: 提出改进框架：直接在仿人机器人运动空间训练引导扩散模型，将多样化的人类和机器人数据集聚合到统一的体现空间中。该方法避免了手动筛选干预，建立了更自动化的流程，并通过扩展参考轨迹生成来获得更鲁棒的强化学习策略。

Result: 该方法能够捕捉更广泛的技能（得益于更大的训练数据混合），建立了更自动化的流程，并且通过扩展参考轨迹生成实现了更鲁棒的下游强化学习策略。在仿真和真实Unitree-G1机器人上进行了广泛实验验证。

Conclusion: 直接在机器人运动空间训练扩散模型，整合多源数据，能够更好地支持仿人机器人的鲁棒自主操作技能学习，为复杂交互任务提供了更有效的解决方案。
...
TL;DR: REMind是一个创新的教育机器人角色扮演游戏，旨在通过机器人中介的戏剧表演支持儿童反欺凌旁观者干预训练。


<details>
  <summary>Details</summary>
Motivation: 开发一种创新的教育工具来支持儿童反欺凌旁观者干预，通过机器人中介的戏剧表演促进社会情感学习。

Method: 设计REMind游戏系统，让儿童观察社交机器人表演的欺凌场景，反思角色视角，并通过操控机器人化身来排练防御策略。采用混合方法对18名9-10岁儿童进行游戏测试研究。

Result: 研究发现该体验支持了关键学习目标，包括自我效能感、观点采择、理解防御结果和干预策略。这些结果突显了机器人中介应用戏剧作为支持社会情感学习的新教学框架的潜力。

Conclusion: REMind展示了机器人中介应用戏剧作为支持儿童反欺凌旁观者干预的有效教学框架，为促进社会情感学习提供了创新方法。
...
TL;DR: 提出分层规划控制框架，在未知非线性动力学下进行在线运动规划，通过自适应状态空间分区和增量图构建实现探索与利用的平衡。


<details>
  <summary>Details</summary>
Motivation: 在未知非线性动力学下进行自主运动规划需要同时学习系统特性并导航到目标，传统方法通常需要大量先验知识或计算复杂度过高。

Method: 1) 将状态空间划分为多面体，使用分段仿射模型近似未知非线性系统；2) 引入非均匀自适应状态空间分区策略，仅在任务相关区域细化分区；3) 将PWA系统抽象为有向加权图，使用可达控制理论和预测可达条件增量验证边；4) 通过图搜索进行高层规划，合成低层仿射反馈控制器执行计划；5) 引入松弛可达条件扩展到欠驱动系统。

Result: 仿真结果表明该方法能够有效平衡探索与利用，同时提供形式化的可达性保证，特别适用于欠驱动系统。

Conclusion: 该框架能够在有限先验知识下实现在线运动合成，通过自适应分区和图抽象降低了计算复杂度，扩展了可达控制理论在欠驱动系统中的应用。
...
TL;DR: 该研究提出了一种让空中机器人能够使用机载传感器和嵌入式飞行硬件实时推理周围风场的方法，通过融合地形测量数据和稀疏风测量数据来预测周围流场，并应用于运动规划以提高能源效率和避障能力。


<details>
  <summary>Details</summary>
Motivation: 研究旨在解决两个基本问题：1）在密集城市环境中，地形数据是否足以准确预测风场；2）学习到的风模型如何用于运动规划，特别是关注能源效率和障碍物避免。通过让机器人能够快速评估局部风况而无需先验环境知识，加速空中机器人进入更具挑战性的环境。

Method: 结合深度学习、流体力学和最优控制工具，建立了一个使用导航LiDAR进行局部风预测的框架，然后将局部风模型先验整合到后退时域最优控制器中，研究局部风知识如何影响自主导航中的能源使用和鲁棒性。

Result: 通过在不同城市风场景中的模拟演示，评估了风预测器的预测能力，并量化了将局部风信息整合到运动规划中对自主城市导航的改进（碰撞率和能耗）。亚尺度自由飞行实验表明，这些算法可以在嵌入式飞行计算机上实时运行，具有足够的带宽来稳定控制小型空中机器人。

Conclusion: 该论文为未知风环境中的局部风推断和运动规划贡献了一个新范式。通过使机器人能够快速评估局部风况而无需先验环境知识，这项研究加速了空中机器人进入日益挑战性环境的进程。
...
TL;DR: 使用Webots机器人仿真平台模拟了Braitenberg机器人的群体捕食者回避和围攻行为，研究了围攻呼叫范围和群体规模对围攻成功率的影响。


<details>
  <summary>Details</summary>
Motivation: 模仿动物群体中的围攻行为（一种反捕食者适应机制），研究机器人群体如何通过围攻呼叫协调行动来应对威胁，为人工生命行为选择和自主智能体控制架构设计提供参考。

Method: 使用Webots机器人仿真平台，模拟Braitenberg机器人群体的围攻行为。机器人遇到光源（代表无生命捕食者）时会发出围攻呼叫召集同伴，如果召集到盟友就围攻，否则逃跑。研究了围攻呼叫范围（无限、中等、低）和群体规模（10个vs 3个机器人）对围攻成功率的影响。

Result: 研究结果表明，围攻呼叫范围和群体规模都对围攻成功率有显著影响。具体来说，更大的呼叫范围和更大的群体规模能够提高围攻的成功率。

Conclusion: 这项工作对人工生命中的行为选择仿真和自主智能体控制架构设计具有重要意义，展示了群体协调机制在机器人行为选择中的重要性。
...
TL;DR: 该论文提出了一种热红外-深度（TIR-D）跟踪架构，利用SLAM机器人的标准传感器套件（LiDAR和热红外相机）实现全天候鲁棒的人员跟踪，解决了RGB-D跟踪在极端光照条件下的性能退化问题。


<details>
  <summary>Details</summary>
Motivation: 自主移动机器人在多样化和不可预测环境中需要鲁棒的人员跟踪能力。虽然RGB-D跟踪精度高，但在完全黑暗或强烈背光等挑战性光照条件下性能严重下降。需要实现全天候鲁棒的跟踪解决方案。

Method: 提出TIR-D跟踪架构，利用LiDAR和热红外相机。采用序列知识转移策略，将大规模热红外训练模型的结构先验迁移到TIR-D域。使用"细粒度差分学习率策略"，在保持预训练特征提取能力的同时，快速适应几何深度线索。

Result: 实验结果显示，提出的TIR-D跟踪器实现了0.700的平均重叠率（AO）和58.7%的成功率（SR），显著优于传统的RGB迁移和单模态基线方法。

Conclusion: 该方法为全天候机器人应用中鲁棒的人员跟随提供了实用且资源高效的解决方案，通过热红外和深度传感器的融合克服了光照条件限制。
...
TL;DR: 本文提出了一种在不确定环境中集成任务和动力学运动规划的方法，使用部分可观测随机混合系统建模，结合决策技术和采样运动规划，证明算法正确性和渐进最优性。


<details>
  <summary>Details</summary>
Motivation: 解决在不确定环境中的集成任务和动力学运动规划问题，机器人具有非线性动力学，需要在部分可观测环境中执行有限轨迹线性时序逻辑规范，环境语义标签存在不确定性。

Method: 将问题建模为部分可观测随机混合系统，捕捉机器人动力学、LTLf任务和环境状态变量不确定性；提出一种利用混合系统结构的随时算法，结合决策技术和采样运动规划。

Result: 证明了算法的正确性和渐进最优性；实验结果显示算法在不确定环境中有效，且始终优于基线方法。

Conclusion: 提出的方法成功解决了不确定环境中的集成任务和动力学运动规划问题，通过混合系统建模和结合决策与运动规划的技术，实现了高效可靠的解决方案。
...
TL;DR: EgoNav系统仅用5小时人类行走数据训练，无需机器人数据或微调，即可让仿人机器人在未见环境中导航。系统通过扩散模型预测未来轨迹分布，结合360度视觉记忆和DINOv3特征，实现实时推理和避障。


<details>
  <summary>Details</summary>
Motivation: 现有机器人导航系统通常需要大量机器人数据或特定环境微调，限制了在多样化未见环境中的泛化能力。本文旨在开发仅从少量人类行走数据学习、无需机器人数据或微调的通用导航系统。

Method: 1) 使用扩散模型预测基于过去轨迹、360度视觉记忆（融合颜色、深度和语义）和DINOv3视频特征的未来轨迹分布；2) 混合采样方案实现10步去噪的实时推理；3) 滚动时域控制器从预测分布中选择路径。

Result: 离线评估显示EgoNav在避撞和多模态覆盖方面优于基线方法。在Unitree G1仿人机器人上的零样本部署中，系统能在未见室内外环境中自然涌现出等待开门、绕开人群、避开玻璃墙等行为。

Conclusion: EgoNav证明了仅从少量人类行走数据学习、无需机器人数据或微调的仿人机器人导航可行性。系统在多样化未见环境中表现出鲁棒的导航能力，并将发布数据集和训练模型。
...
### [13] [Certificate-Driven Closed-Loop Multi-Agent Path Finding with Inheritable Factorization](https://arxiv.org/abs/2604.00428)
*Jiarui Li,Runyu Zhang,Gioele Zardini*

Main category: cs.RO

TL;DR: CDCBS算法通过引入证书轨迹和舰队预算机制，改进了基于冲突搜索的闭环多智能体路径规划，在密集场景中提供更一致的解质量


<details>
  <summary>Details</summary>
Motivation: 现有闭环MAPF算法（如ACCBS）采用有限视野规划，在密集场景中可能产生短视行为，难以保持全局保证和利用组合结构，特别是在有限计算预算下可能提前终止

Method: 提出证书轨迹和舰队预算机制：证书提供无冲突回退计划和剩余成本单调上界；仅接受证书改进的更新确保完备性；预算信息诱导预算限制分解，实现跨时间步的全局可继承分解。将框架实例化到ACCBS得到CDCBS算法

Result: 在基准地图上的实验表明，CDCBS比ACCBS获得更一致的解质量，尤其在密集场景中；提出的分解机制减少了有效组大小

Conclusion: 证书轨迹和舰队预算机制为闭环MAPF算法提供了有效的过滤和分解框架，CDCBS在保持完备性的同时提高了密集场景下的规划质量
...
### [14] [Reachability-Aware Time Scaling for Path Tracking](https://arxiv.org/abs/2604.00439)
*Hossein Gholampour,Logan E. Beaver*

Main category: cs.RO

TL;DR: 提出一种基于可达性引导的二次规划跟踪器，用于跟踪离线规划器生成的无碰撞路径，通过速度缩放确保加速度约束下的可达性


<details>
  <summary>Details</summary>
Motivation: 采样式规划器生成的路径可能包含急转弯和高曲率区域，即使路径几何无碰撞，在加速度限制下的一步可达性也变得关键，需要确保跟踪过程中不违反加速度约束

Method: 基于纯追踪风格的可达性引导二次规划跟踪器，具有一步加速度裕度；离线阶段评估样条路径上的加速度裕度并更新标量速度缩放剖面；在线阶段使用相同的向前看跟踪结构跟踪缩放后的参考路径

Result: 该方法能够确保在加速度约束下跟踪包含急转弯和高曲率区域的路径，通过速度缩放保持一步可达性，避免违反加速度限制

Conclusion: 提出的可达性引导速度缩放方法有效解决了加速度约束下的路径跟踪问题，特别适用于采样式规划器生成的复杂几何路径，确保跟踪的安全性和可行性
...
### [15] [Multi-Camera View Scaling for Data-Efficient Robot Imitation Learning](https://arxiv.org/abs/2604.00557)
*Yichen Xie,Yixiao Wang,Shuqi Zhao,Cheng-En Wu,Masayoshi Tomizuka,Jianwen Xie,Hao-Shu Fang*

Main category: cs.RO

TL;DR: 通过多视角摄像头采集演示数据，无需额外人工标注即可提升模仿学习的泛化能力，利用视角多样性增强数据分布，提高视觉表示的不变性。
...
Method: 提出多视角框架：在演示收集时使用多个同步摄像头视角，从每个专家轨迹生成伪演示，丰富训练数据分布。分析不同动作空间与视角扩展的交互，使用相机空间表示增强多样性。引入多视角动作聚合方法，使单视角策略在部署时能从多摄像头受益。

Result: 在仿真和真实世界操作任务中的大量实验表明，相比单视角基线，该方法在数据效率和泛化能力方面取得显著提升。多视角扩展为模仿学习提供了实用且可扩展的解决方案。

Conclusion: 扩展摄像头视角为模仿学习提供了实用且可扩展的解决方案，只需最少的额外硬件设置，并能与现有模仿学习算法无缝集成，显著提升泛化能力。
...
### [16] [A Physical Imitation Learning Pipeline for Energy-Efficient Quadruped Locomotion Assisted by Parallel Elastic Joint](https://arxiv.org/abs/2604.00611)
*Huyue Ma,Yurui Jin,Helmut Hauser,Rui Wu*

Main category: cs.RO

TL;DR: 该论文提出物理模仿学习(PIL)方法，将强化学习控制策略转化为可物理实现的被动并联弹性关节响应，从而将部分控制任务卸载到机械身体上，实现节能的脑-体协同设计。


<details>
  <summary>Details</summary>
Motivation: 动物通过脑-体协同进化，利用内在身体动力学实现节能运动，而机器人设计通常采用集中式控制器抑制而非利用身体动力学。需要一种方法来让机器人身体模仿部分控制行为，实现类似动物的物理智能。

Method: 提出物理模仿学习(PIL)：将强化学习控制策略蒸馏为可在被动并联弹性关节(PEJs)上物理实现的身体响应，使身体能够模仿部分控制行为；剩余策略控制电机恢复RL策略性能。

Result: 在模拟四足机器人中，PIL方法在平坦地形上可将高达87%的机械功率卸载到PEJs，在崎岖地形上可卸载18%的功率，显著降低能耗。

Conclusion: PIL实现了脑-体协同设计，无需扩展搜索空间包含身体设计参数，为基于关节的机器人形态提供了计算高效的任务特定物理智能实现途径。
...
### [17] [StretchBot: A Neuro-Symbolic Framework for Adaptive Guidance with Assistive Robots](https://arxiv.org/abs/2604.00628)
...
TL;DR: StretchBot是一个结合多模态感知和知识图谱的大型语言模型机器人教练，用于自适应辅助拉伸指导，相比脚本化系统能更好地适应用户状态和环境。


<details>
  <summary>Details</summary>
Motivation: 现有辅助机器人系统大多采用脚本化方式，限制了其适应用户状态、环境背景和交互动态的能力，需要更自适应的辅助指导系统。

Method: 开发了StretchBot混合神经符号机器人教练，结合多模态感知与基于知识图谱的大型语言模型推理，支持在保持结构化常规的同时进行上下文感知调整。

Result: 初步实验表明自适应条件改善了感知适应性和上下文相关性，而脚本化指导在流畅性和可预测性方面仍具竞争力。

Conclusion: 结构化可操作知识可以帮助基于语言模型的适应在具身辅助交互中落地，但需要更大规模的纵向研究来评估鲁棒性、泛化能力和长期用户体验。
...
### [18] [LiPS: Lightweight Panoptic Segmentation for Resource-Constrained Robotics](https://arxiv.org/abs/2604.00634)
*Calvin Galagain,Martyna Poreba,François Goulette,Cyrill Stachniss*

Main category: cs.RO

TL;DR: LiPS是一种轻量级全景分割方法，在保持查询式解码的同时简化特征提取和融合路径，在标准基准测试中达到与重型基线相当的精度，但吞吐量提高4.5倍，计算量减少6.8倍。
...
Method: 提出LiPS方法，保留基于查询的解码，同时引入简化的特征提取和融合路径，采用轻量级设计。

Result: 在标准基准测试中，LiPS达到与重型基线相当的精度，同时吞吐量提高4.5倍（帧率），计算量减少6.8倍。
...
### [19] [SoftHand Model-W: A 3D-Printed, Anthropomorphic, Underactuated Robot Hand with Integrated Wrist and Carpal Tunnel](https://arxiv.org/abs/2604.00738)
*Dhillon B. Merritt,Christopher J. Ford,Haoran Li,Malia Smith,Zhixing Chen,Efi Psomopoulou,Nathan F. Lepora*

Main category: cs.RO

TL;DR: 本文介绍了SoftHand Model-W：一款基于Pisa/IIT SoftHand的3D打印欠驱动拟人机器人手，集成了拮抗肌腱机制和2自由度肌腱驱动腕部，通过四个驱动自由度实现手指主动屈伸和手掌腕部屈伸/桡尺偏转，同时保持软手的协同和自适应特性。


<details>
  <summary>Details</summary>
Motivation: 开发具有腕部驱动功能的机器人手，以减少手臂的补偿性运动，提高任务完成效率，并实现原本不可能完成的拾放操作，从而在现实世界机器人应用中实现更灵活、更类似人类的操作能力。

Method: 设计并制造了SoftHand Model-W，采用3D打印技术制造欠驱动拟人机器人手，集成拮抗肌腱机制和2自由度肌腱驱动腕部。通过腕管式肌腱布线将电机远程放置在手臂中，减少远端惯性。将软手安装在6轴机器人手臂上，通过立方体堆叠和平面圆盘旋转两个重定向任务测试手-臂协调性能。

Result: 实验结果显示，添加腕部驱动显著减少了手臂的补偿性和重新配置运动，缩短了任务完成时间。腕部功能使得原本不可能完成的拾放操作成为可能。比较有腕部驱动和无腕部驱动条件下的任务时间、手臂关节行程和配置变化，证实了腕部驱动对提升操作性能的重要性。

Conclusion: SoftHand Model-W证明了近端自由度对于在现实世界机器人应用中实现多功能、类人操作的关键作用。其紧凑设计使其适用于研究和辅助场景，展示了腕部驱动在提升机器人操作能力和效率方面的重要价值。
...
### [20] [How to Train your Tactile Model: Tactile Perception with Multi-fingered Robot Hands](https://arxiv.org/abs/2604.00744)
*Christopher J. Ford,Kaichen Shi,Laura Butcher,Nathan F. Lepora,Efi Psomopoulou*

Main category: cs.RO

TL;DR: TacViT：基于视觉Transformer的触觉感知模型，能够在未见过的传感器上实现接触属性推断，减少数据收集和重新训练需求，加速新传感器部署。


<details>
  <summary>Details</summary>
Motivation: 当前基于CNN的触觉感知方法需要大量传感器特定数据集，且对新传感器需要重新训练，限制了多指机器人手中视觉触觉传感器的快速部署和可扩展性。

Method: 提出TacViT模型，基于Vision Transformers架构，利用全局自注意力机制从触觉图像中提取鲁棒特征，实现对新传感器的泛化能力。

Result: 在五指机器人手的传感器上评估，TacViT相比CNN展现出优越的泛化性能，显著减少对新传感器的数据收集和重新训练需求。

Conclusion: TacViT有潜力使触觉感知更具可扩展性和实用性，加速新触觉传感器在现实世界机器人应用中的部署。
...
### [21] [A wearable haptic device for edge and surface simulation](https://arxiv.org/abs/2604.00752)
*Rui Chen,Xianlong Mai,Alireza Sanaei,Domenico Chiaradia,Antonio Frisoli,Daniele Leonardis*

Main category: cs.RO

TL;DR: 提出了一种紧凑轻量（24.3克）的指尖触觉设备，通过双电机机制提供可区分的表面和边缘接触反馈，在VR中增强物体操纵的真实感。


<details>
  <summary>Details</summary>
Motivation: 传统指尖触觉设备无法渲染某些对沉浸式和精确交互相关的触觉特征（如边缘检测），限制了VR应用中物体操纵的真实感。

Method: 设计了一种紧凑轻量的指尖触觉设备（24.3克），采用新颖的双电机机制来提供表面和边缘接触反馈。使用6×6柔性传感器阵列进行压力分布表征，并通过5名参与者的初步用户研究评估设备性能。

Result: 压力分布表征显示两种刺激模式具有不同的接触模式。用户研究在四种条件（边缘/表面接触×轻/重压力）下达到93%的平均分类准确率，平均响应时间为2.79秒。

Conclusion: 该设备能有效传递边缘和表面触觉线索，有望提高VR环境中物体操纵的保真度，为沉浸式VR交互提供更好的触觉反馈解决方案。
...
### [22] [A Dual-Action Fabric-Based Soft Robotic Glove for Ergonomic Hand Rehabilitation](https://arxiv.org/abs/2604.00768)
...
TL;DR: 研究人员开发了一种基于织物的双动作软体机器人手套，通过定制化执行器支持手指屈伸和拇指外展，旨在改善神经障碍患者的手部功能康复和辅助


<details>
  <summary>Details</summary>
Motivation: 神经障碍导致的手部功能障碍严重影响日常生活独立性，现有软体机器人手套在定制化、人体工学适配和屈伸驱动方面存在局限，限制了临床实用性

Method: 采用计算机数控热封技术制造具有对称腔室的执行器，设计五组独立控制的双动作执行器支持手指屈伸，以及专门的拇指外展执行器，执行器充气时形成凹形外表面以最大化手指接触面积

Result: 执行器产生足够的关节力矩和指尖力用于日常生活任务，手套系统能提供足够抓握力；健康受试者测试显示主动辅助显著减少前臂肌肉活动；脊髓损伤患者初步研究显示手套促进更自然的抓握模式并减少肌腱固定抓握依赖，但任务完成时间增加

Conclusion: 这种可定制、符合人体工学的设计代表了向个性化手部康复和辅助机器人技术的实际进展，尽管当前驱动接口仍需改进以减少任务完成时间
...
### [23] [Compact Keyframe-Optimized Multi-Agent Gaussian Splatting SLAM](https://arxiv.org/abs/2604.00804)
*Monica M. Q. Li,Pierre-Yves Lajoie,Jialiang Liu,Giovanni Beltrame*

Main category: cs.RO

TL;DR: 提出改进的多智能体RGB-D高斯泼溅SLAM框架，通过压缩冗余3D高斯和集中式闭环检测，在保持地图质量的同时大幅减少通信负载


<details>
  <summary>Details</summary>
Motivation: 多智能体3D建图在未知环境中至关重要，但密集表示阻碍了在有限通信链路上的实时交换。现有SLAM系统依赖中央服务器合并局部地图，但共享大型地图表示（特别是高斯泼溅方法生成的地图）在带宽受限场景中成为瓶颈

Method: 1) 在SLAM系统中加入压缩步骤，移除冗余3D高斯而不降低渲染质量；2) 采用无需初始猜测的集中式闭环检测，支持两种模式：纯渲染深度模式（仅需3D高斯）和相机深度模式（包含轻量级深度图像以提高配准精度和额外高斯修剪）

Result: 在合成和真实世界数据集上的评估显示，与最先进方法相比，两种模式下传输数据减少85-95%，使3D高斯多智能体SLAM更接近实际部署

Conclusion: 提出的改进框架显著降低了多智能体高斯泼溅SLAM的通信负载，同时保持地图保真度，为实际场景部署提供了可行方案
...
### [24] [PanoAir: A Panoramic Visual-Inertial SLAM with Cross-Time Real-World UAV Dataset](https://arxiv.org/abs/2604.00852)
*Yiyang Wu,Xiaohu Zhang,Yanjin Du,Tongsu Zhang,Chujun Li,Siyang Chen,Guoyi Zhang,Xiangpeng Xu*

Main category: cs.RO

TL;DR: 本文针对无人机应用中的姿态估计问题，提出了一个全景视觉惯性SLAM框架，并构建了首个真实世界的无人机全景视觉惯性数据集，以解决传统有限视野传感器在复杂场景下的漂移和失效问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉惯性SLAM方法主要依赖有限视野传感器，在复杂的无人机场景中容易导致漂移甚至失效。虽然全景相机提供了全向感知能力，但针对无人机的全景视觉惯性SLAM方法和相应的真实世界数据集仍然缺乏研究。

Method: 首先构建了一个覆盖多样化飞行条件的真实世界全景视觉惯性数据集。然后提出了一个全景视觉惯性SLAM框架，通过全景特征提取和全景闭环检测来利用全向视野，增强特征约束并确保全局一致性。

Result: 在提出的数据集和公共基准测试上的大量实验表明，该方法在准确性、鲁棒性和一致性方面优于现有方法。在嵌入式平台上的部署验证了其实际应用性，计算效率与PC实现相当。

Conclusion: 该研究填补了无人机全景视觉惯性SLAM领域的空白，提出的框架和数据集为无人机在复杂环境下的精确实时定位提供了有效解决方案，具有重要的理论和应用价值。
...
### [25] [An Integrated Soft Robotic System for Measuring Vital Signs in Search and Rescue Environments](https://arxiv.org/abs/2604.00971)
*Jorge Francisco García-Samartín,Christyan Cruz Ulloa,Andrés Sánchez-Silva,Jaime del Cerro,Antonio Barrientos*

Main category: cs.RO

TL;DR: 开发了一种用于灾后救援的软体抓取器，集成到移动机器人系统中，能够测量受害者的脉搏和血压，在灾后环境中实现准确的生理参数监测。


<details>
  <summary>Details</summary>
Motivation: 虽然机器人在搜救行动中广泛应用，近年来受害者评估领域也取得显著进展，但在心率测量方面仍存在未解决的问题，且没有研究评估灾后场景中的血压测量。需要开发能够在灾后环境中准确测量受害者脉搏和血压的设备。

Method: 设计了一种软体抓取器并集成到移动机器人系统中。抓取器能够包裹受害者手臂并像血压计一样充气，通过专门的便携系统实现。采用不同的信号处理算法来处理测量数据。

Result: 系统实现了脉搏偏差4bpm，收缩压和舒张压偏差约5mmHg。统计数据和误差项同方差性验证证明了系统能够准确确定心率和血压。通过灾后场景测试验证了系统功能，展示了其适应不同受害者位置的能力、测量速度和安全性。

Conclusion: 该系统能够准确测量灾后受害者的心率和血压，适合搜救行动使用，具有适应性强、测量快速且对受害者安全的特点。
...
TL;DR: 本文提出了一种基于核均值嵌入的无限时域遍历控制器，用于解决一般域上的长期覆盖任务，通过扩展的误差访问状态实现计算可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有基于核的遍历控制方法虽然能提供一般覆盖域的强覆盖保证，但由于计算复杂度随时域增长而急剧增加，实际应用仅限于有限时域，无法用于长期覆盖任务。

Method: 推导出基于核均值嵌入的无限时域遍历控制器，引入扩展的核均值嵌入误差访问状态来递归记录状态访问历史，将过去访问与未来控制解耦，并提出了基于扩展误差状态的滚动时域控制变体。

Result: 证明了所推导控制器的渐近收敛性，并在2D和3D覆盖问题中展示了保持遍历覆盖保证的能力，解决了长期覆盖任务的计算可扩展性问题。

Conclusion: 通过扩展误差状态方法，成功将遍历控制扩展到无限时域设置，为长期覆盖任务提供了计算可行的解决方案，同时保持了遍历覆盖的理论保证。
...
### [27] [Stein Variational Uncertainty-Adaptive Model Predictive Control](https://arxiv.org/abs/2604.01034)
*Hrishikesh Sathyanarayan,Ian Abraham*

Main category: cs.RO

TL;DR: 提出了一种基于Stein变分分布的鲁棒控制器，用于处理具有潜在参数不确定性的非线性动力系统，通过任务相关的粒子近似替代保守的最坏情况优化，在保持计算并行性的同时实现更好的性能-鲁棒性权衡。


<details>
  <summary>Details</summary>
Motivation: 传统分布鲁棒优化（DRO）方法通常基于最坏情况设计，可能牺牲名义性能。对于具有潜在参数不确定性的非线性动力系统，需要一种既能保持鲁棒性又不牺牲性能的方法，同时避免对不确定性模型做出限制性参数假设。

Method: 将最优控制与Stein变分推理相结合，使用确定性粒子基近似任务相关的不确定性分布，使控制器能够专注于对闭环性能影响最大的参数敏感性。该方法避免了不确定性模型的参数假设，同时保持了计算并行性。

Result: 在代表性控制问题上验证了该方法，实证表明相比名义控制器、集成方法和经典分布鲁棒基准，该方法实现了更好的性能-鲁棒性权衡。

Conclusion: 该框架将鲁棒控制与变分推理统一在单一决策理论公式中，适用于具有参数不确定性的广泛控制系统类别，通过围绕对任务目标最关键的相关不确定性来塑造控制律，实现了鲁棒性而不牺牲名义性能。
...
### [28] [BAT: Balancing Agility and Stability via Online Policy Switching for Long-Horizon Whole-Body Humanoid Control](https://arxiv.org/abs/2604.01064)
*Donghoon Baek,Sang-Hun Kim,Sehoon Ha*

Main category: cs.RO

TL;DR: BAT是一个在线策略切换框架，通过动态选择两个互补的全身RL控制器，在不同运动情境中平衡敏捷性和稳定性，实现长时程的全身运动控制。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常采用两种范式：耦合的全身策略用于全局协调，解耦的策略用于模块化精度。但缺乏系统方法来整合两者，导致敏捷性、鲁棒性和精度之间的权衡问题未解决。

Method: BAT框架包含两个互补模块：1) 通过分层强化学习学习的切换策略，带有滑动时域策略预评估的专家指导；2) 选项感知的VQ-VAE，从离散运动标记序列预测选项偏好以改进泛化。最终决策通过两个模块的置信度加权融合获得。

Result: 在Unitree G1人形机器人上的大量仿真和真实世界实验表明，BAT能够实现多功能的长时程移动操作任务，并在多样化任务中优于先前方法。

Conclusion: BAT通过动态策略切换框架成功解决了敏捷性、鲁棒性和精度之间的权衡问题，为长时程全身行为控制提供了有效的统一解决方案。
...
### [29] [VRUD: A Drone Dataset for Complex Vehicle-VRU Interactions within Mixed Traffic](https://arxiv.org/abs/2604.01134)
*Ziyu Wang,Hongrui Kou,Cheng Wang,Ruochen Li,Hubert P. H. Shum,Amir Atapour-Abarghouei,Yuxin Zhang*

Main category: cs.RO

TL;DR: 本文提出了一种高效、高精度的无人机数据集构建方法，并建立了车辆-弱势道路使用者交互数据集（VRUD），填补了复杂、非结构化城市环境数据的空白。


<details>
  <summary>Details</summary>
Motivation: 城市L4级自动驾驶面临的主要挑战来自弱势道路使用者（VRU）的高密度及其高度不确定的交互行为。现有开源数据集主要关注高速公路或规范交叉口等结构化场景，缺乏代表混乱、非结构化城市环境的数据。

Method: 提出高效、高精度的无人机数据集构建方法，在深圳典型"城中村"（交通监管松散、遮挡严重）收集数据，包含4小时4K/30Hz录像，涵盖11,479个VRU轨迹和1,939个车辆轨迹。基于新型向量碰撞时间阈值提取了4,002个多智能体交互场景，并提供标准OpenDRIVE高清地图支持。

Result: 建立了VRUD数据集，其中VRU占所有交通参与者的约87%，远超现有基准。数据集提供了罕见边缘案例资源，有助于提升自动驾驶系统在复杂非结构化城市环境中的安全性能。

Conclusion: VRUD数据集填补了复杂非结构化城市环境数据的空白，为提升自动驾驶系统在VRU密集环境中的安全性能提供了宝贵资源。数据集已开源供进一步研究使用。
...
### [30] [Deep Reinforcement Learning for Robotic Manipulation under Distribution Shift with Bounded Extremum Seeking](https://arxiv.org/abs/2604.01142)
...
<details>
  <summary>Details</summary>
Motivation: 强化学习在机器人操作中表现出色，但学习到的策略在测试条件与训练分布不同时性能会下降。这在接触丰富的任务（如推动和抓取放置）中尤为突出，因为目标、接触条件或机器人动力学的变化都可能在推理时将系统推出分布范围。

Method: 提出了一种混合控制器，将深度确定性策略梯度（DDPG）策略与有界极值搜索（bounded ES）相结合。DDPG策略在标准条件下训练机器人推动和抓取放置任务，然后在部署时与有界ES结合使用。RL策略提供快速操作行为，而有界ES确保当操作条件偏离训练条件时，整体控制器对时间变化的鲁棒性。

Result: 该控制器在多种分布外设置下进行评估，包括随时间变化的目标和空间变化的摩擦斑块。结果表明混合控制器在这些挑战性条件下提高了鲁棒性。

Conclusion: 结合强化学习和有界极值搜索的混合方法能够有效提高机器人操作任务在分布外条件下的鲁棒性，RL提供快速行为，有界ES确保对时间变化的适应性。
...
### [31] [SMASH: Mastering Scalable Whole-Body Skills for Humanoid Ping-Pong with Egocentric Vision](https://arxiv.org/abs/2604.01158)
...
<details>
  <summary>Details</summary>
Motivation: 现有的人形乒乓球系统存在两个核心限制：1）依赖外部传感系统；2）无法实现敏捷的全身协调以执行精确任务。这些限制源于两个挑战：在快速机器人运动下实现低延迟、鲁棒的机载自我中心感知，以及获取足够多样化的任务对齐击球动作来学习精确而自然的全身行为。

Method: 提出了一个模块化系统，统一了可扩展的全身技能学习和机载自我中心感知。系统包含三个关键创新：1）实现敏捷精确的球交互，通过紧密协调的全身控制而非分离的上下半身行为；2）使用生成模型增强和多样化击球动作，获得可扩展的运动先验；3）首次实现仅使用机载传感的人形乒乓球连续击球系统。

Result: 通过大量真实世界实验验证，系统能够在高速条件下实现稳定和精确的球交换，展示了可扩展的、感知驱动的全身技能学习在动态人形交互任务中的有效性。

Conclusion: 该工作展示了首个仅使用机载感知就能完成连续击球的人形乒乓球系统，验证了可扩展的、感知驱动的全身技能学习方法在动态人形交互任务中的潜力。
...
### [32] [Collaborative Task and Path Planning for Heterogeneous Robotic Teams using Multi-Agent PPO](https://arxiv.org/abs/2604.01213)
*Matthias Rubio,Julia Richter,Hendrik Kolvenbach,Marco Hutter*

Main category: cs.RO

TL;DR: 本文提出基于多智能体近端策略优化(MAPPO)的协作规划策略，用于协调异构机器人团队解决复杂的目标分配和调度问题，相比传统规划方法具有更好的可扩展性。


<details>
  <summary>Details</summary>
Motivation: 高效的外星探索需要多样化的机器人能力，但传统规划算法在问题规模增大时计算复杂度呈组合增长，导致规划周期长、推理成本高。学习型方法将扩展性担忧从运行时转移到训练时，是实现实时规划的关键一步。

Method: 采用多智能体近端策略优化(MAPPO)方法，协调异构机器人团队解决复杂的目标分配和调度问题。该方法将规划问题建模为多智能体协作任务。

Result: 通过穷举搜索获得的单目标最优解进行基准测试，并在行星探索场景中评估其在线重新规划能力。结果表明该方法在复杂协调任务中具有良好性能。

Conclusion: 基于MAPPO的协作规划策略为解决机器人团队协调问题提供了有效的学习型方法，相比传统规划算法具有更好的可扩展性，是实现实时规划的重要进展。
...
### [33] [Functional Force-Aware Retargeting from Virtual Human Demos to Soft Robot Policies](https://arxiv.org/abs/2604.01224)
...
TL;DR: SoftAct是一个通过显式推理接触力来教授软体机器人手执行类人操作技能的框架，使用VR捕捉人类演示数据，通过两阶段力感知重定向算法实现技能迁移。


<details>
  <summary>Details</summary>
Motivation: 传统方法仅重定向人类关节轨迹，无法有效处理软体机器人手与人类手之间的极端形态不匹配和非线性柔顺性，需要显式建模接触几何和力分布来实现有效的技能迁移。

Method: 1) 使用VR捕捉人类演示，包括手部运动学、物体运动、密集接触斑块和详细接触力信息；2) 两阶段力感知重定向算法：第一阶段将演示接触力分配给人类手指并按比例分配给机器人手指，建立力平衡映射；第二阶段结合基线末端执行器姿态跟踪与测地加权接触细化，实时调整机器人指尖目标。

Result: 在接触丰富的操作任务上评估，SoftAct控制器相比运动学和基于学习的基线方法，指尖轨迹跟踪RMSE降低高达55%，跟踪方差降低高达69%；在策略层面，在零样本真实世界部署和仿真中均获得更高成功率。

Conclusion: 显式建模接触几何和力分布对于软体机器人手的有效技能迁移至关重要，仅通过运动学模仿无法恢复这些信息。SoftAct框架能够使软体机器人手再现人类演示的功能意图，同时自然适应极端形态不匹配和非线性柔顺性。
