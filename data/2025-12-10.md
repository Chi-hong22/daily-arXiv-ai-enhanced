...
TL;DR: 使用Planet Labs四波段卫星影像和深度学习模型，可近乎实时监测苏丹武装冲突中的火灾损害，相比基线方法能更准确捕捉活跃火灾和烧焦区域。
...
Conclusion: 四波段卫星影像结合深度学习模型可有效监测武装冲突中的火灾损害，实现最小延迟的监测，为冲突分析提供实用工具。
...
Method: 采用关键帧作为条件信号注入目标身份，实现灵活可控的编辑。结合关键帧条件和视频参考指导，模型执行时序拼接以确保稳定的身份保持和长视频序列的高保真重建。为解决参考引导训练数据稀缺问题，构建了配对人脸交换数据集Face2Face，并通过反转数据对确保可靠的真实监督。

Result: 大量实验表明，该方法实现了最先进的结果，能够无缝地将目标身份与源视频的表情、光照和运动相结合，同时显著减少了制作流程中的手动工作量。

Conclusion: LivingSwap作为首个视频参考引导的人脸交换模型，通过创新的关键帧条件注入和时序拼接技术，成功解决了长视频序列中人脸交换的高保真度和时序一致性难题，为影视制作提供了高效解决方案。
...
TL;DR: Terrain Diffusion：一种基于扩散模型的无限地形生成系统，作为Perlin噪声的AI时代替代品，能够无缝生成无限、一致且可实时访问的逼真地形。


<details>
  <summary>Details</summary>
Motivation: 传统基于Perlin噪声的程序化地形生成方法虽然快速且无限，但在真实感和大规模一致性方面存在根本限制。需要一种能够结合扩散模型高保真度与传统程序化噪声优点的解决方案。
...
Result: 建立扩散模型作为程序化世界生成的实用基础，能够合成整个行星的地形，具有一致性、可控性且无限制，实现了无缝、实时的无限景观合成。

Conclusion: Terrain Diffusion成功将扩散模型的高保真度与传统程序化噪声的关键特性（无缝无限扩展、种子一致性、恒定时间随机访问）相结合，为AI时代的程序化地形生成提供了新的基础框架。
...
TL;DR: FRIEDA是一个用于评估大型视觉语言模型在复杂开放式地图推理能力的新基准，测试包括拓扑、度量和方向关系在内的空间关系理解，结果显示当前最先进模型表现远低于人类水平。


<details>
  <summary>Details</summary>
Motivation: 地图推理是解释地理关系的关键认知能力，对灾害响应和城市规划等任务至关重要，但目前缺乏专门的评估基准。现有地图视觉问答研究往往将地图视为图表的特例，但地图VQA需要理解分层符号系统和空间关系，这些在图表式评估中未被充分捕捉。

Method: FRIEDA基准从各种领域和地理区域的文档和报告中收集真实地图图像，基于GIS文献分类，针对所有三类空间关系：拓扑关系（边界、相等、相交、包含）、度量关系（距离）和方向关系（方位）。所有问题都需要多步推理，许多问题需要跨地图对齐和推理。

Result: 评估了11个最先进的LVLM，在两种设置下：直接设置（提供相关地图）和上下文设置（模型可能需要先识别相关地图）。最强的模型Gemini-2.5-Pro和GPT-5-Think分别仅达到38.20%和37.20%的准确率，远低于人类84.87%的表现。

Conclusion: FRIEDA揭示了LVLM在多步地图推理方面存在持续差距，为推进空间智能研究提供了严格的基准，表明当前模型在复杂地图理解方面仍有很大改进空间。
...
TL;DR: 该论文提出了一种基于频域掩码的通用深度伪造检测方法，通过频率掩码训练策略提升对未知生成模型的泛化能力，同时保持计算效率。
...
Method: 提出频率域掩码作为深度伪造检测器的训练策略。与传统依赖空间特征或大规模预训练模型的方法不同，该方法引入随机掩码和几何变换，特别关注频率掩码因其优越的泛化特性。频率掩码不仅提升检测精度，还能在显著模型剪枝下保持性能。

Result: 该方法在GAN和扩散模型生成的图像数据集上实现了最先进的泛化性能，并在结构化剪枝下表现出一致的鲁棒性。代码和模型已开源。

Conclusion: 频率掩码为可持续和可泛化的深度伪造检测提供了实用解决方案，在保持高性能的同时降低了计算资源需求。
...
### [6] [PAVAS: Physics-Aware Video-to-Audio Synthesis](https://arxiv.org/abs/2512.08282)
*Oh Hyun-Bin,Yuhta Takida,Toshimitsu Uesaka,Tae-Hyun Oh,Yuki Mitsufuji*

Main category: cs.CV

TL;DR: PAVAS是一种将物理推理融入视频到音频生成的方法，通过物理驱动的音频适配器结合物体级物理参数，生成更符合物理现实的声音。
...
Method: 1. 使用基于潜在扩散的视频到音频生成框架；2. 引入物理驱动音频适配器（Phy-Adapter）；3. 通过物理参数估计器（PPE）获取物体级物理参数：使用视觉语言模型推断移动物体质量，使用基于分割的动态3D重建模块恢复运动轨迹计算速度；4. 将这些物理线索融入音频生成过程。

Result: 1. 创建了VGG-Impact基准测试，专注于物体-物体交互；2. 引入了音频-物理相关系数（APCC）评估指标，衡量物理属性和听觉属性之间的一致性；3. 综合实验表明PAVAS产生物理上合理且感知上连贯的音频，在定量和定性评估中均优于现有视频到音频模型。

Conclusion: PAVAS成功将物理推理融入视频到音频生成，通过结合物体级物理参数，能够生成更符合物理现实的声音，在物理真实性和感知质量方面优于现有方法。
...
TL;DR: M2A提出了一种简单的持续测试时自适应方法，通过随机掩码生成多个视图，结合掩码一致性损失和熵最小化损失进行适配，在强损坏条件下表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有持续测试时自适应方法依赖校准的不确定性或稳定的注意力分数，并引入额外复杂性。本文探讨是否在强损坏条件下需要定制掩码设计，还是简单的随机掩码调度就足够有效。

Method: 提出M2A方法：生成短序列的掩码视图（空间或频率），使用掩码一致性损失对齐不同视图的预测，以及熵最小化损失鼓励自信输出。研究了两种掩码家族：空间掩码（补丁vs像素）和频率掩码（全频vs低频vs高频）。
...
TL;DR: CVP框架受人类中央和周边视觉启发，通过目标亲和性令牌和以自我为中心的网格增强多模态模型的空间推理能力，在3D场景理解任务上达到SOTA性能。
...
Method: 提出受中央和周边视觉启发的CVP框架：1）目标亲和性令牌（类似中央视觉）引导模型关注查询相关对象；2）以自我为中心的网格（类似周边视觉）捕捉全局场景上下文和空间布局。两者在大型多模态模型架构中协同工作。

Result: CVP在多个3D场景理解基准测试中实现了最先进的性能表现。

Conclusion: 受人类视觉系统启发的结构化表示方法能有效提升多模态模型的空间推理能力，为复杂3D环境理解提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的非均匀雾霾条件给图像去雾带来巨大挑战。虽然基于Transformer的方法擅长捕捉全局上下文，但其二次计算复杂度阻碍了实时部署。需要一种既能有效建模雾霾退化又具有线性复杂度的解决方案。
...
Conclusion: Fourier-RWKV通过创新的多状态感知范式成功解决了非均匀雾霾去雾问题，在保持线性计算复杂度的同时实现了优异的去雾性能，为实时部署提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 高质量几何图形生成既具有挑战性又有机遇：它要求严格的空间精度，同时提供了明确的约束来指导生成。受几何问题求解中形式化语言和符号求解器在增强正确性和可解释性方面的最新进展启发。

Method: GeoLoom包含两个核心组件：1）自动形式化模块，将自然语言翻译为专门设计的面向生成的形式语言GeoLinua；2）坐标求解器，使用高效的蒙特卡洛优化将形式约束映射到精确坐标。还引入了GeoNF数据集，对齐自然语言几何描述和形式化GeoLinua描述。

Result: 经验结果表明，GeoLoom在结构保真度方面显著优于最先进的基线方法，为可解释和可扩展的图形生成提供了原则性基础。

Conclusion: GeoLoom通过形式化语言和约束求解的方法，为几何图形生成提供了一个可解释、可扩展的框架，显著提高了生成图形的结构准确性。
...
TL;DR: 该论文提出了视觉知识库（Visual Knowledge Base）和VisKnow构建框架，通过图结构组织多模态对象知识，并以AnimalKB为案例展示了其在零样本识别、细粒度VQA等任务中的增强效果。


<details>
  <summary>Details</summary>
Motivation: 当前对象理解任务面临数据分散、缺乏系统组织的问题。现有数据多为任务导向，不足以支持对对象类别的全面理解，需要整合多模态知识（视觉标注和文本知识）来支持推理和问答等高级任务。

Method: 提出VisKnow构建框架，结合专家设计和大规模模型应用，从百科全书等文本源提取知识三元组，整合对齐的文本和图像源知识，并提供对象和部件级别的区域标注，构建结构化知识图谱。

Result: 构建了AnimalKB知识库，涵盖406个动物类别，包含22K文本知识三元组、420K图像及对应区域标注。实验表明该知识库能增强零样本识别和细粒度VQA等任务，并为知识图谱补全和部件分割提供基准。

Conclusion: 自动构建视觉知识库有潜力推动视觉理解及其实际应用，VisKnow框架和AnimalKB展示了结构化多模态知识在提升对象级视觉任务性能方面的价值。
...
TL;DR: 该论文探索了在3D目标检测领域中应用提示调优方法的有效性，研究基于大规模Waymo数据集训练的模型能否作为基础模型适应其他场景，并提出了面向场景的提示池方法。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型展现出强大的泛化能力，通过微调和提示调优等技术可以适应各种下游任务，这在自然语言处理领域已很常见。本文旨在探索提示调优方法在3D目标检测领域的有效性，研究基于大规模数据集训练的模型能否作为基础模型适应其他场景。

Method: 研究首先顺序考察提示标记和提示生成器的影响，进一步提出了面向场景的提示池（SOP²）。该方法旨在探索提示池在3D目标检测中的有效性。

Result: 论文展示了提示池在3D目标检测中的有效性，证明了基于大规模Waymo数据集训练的模型可以作为基础模型，并通过提示调优技术适应其他3D目标检测场景。

Conclusion: 该研究证明了提示调优方法在3D目标检测领域的潜力，提出的面向场景的提示池方法有效，旨在启发未来研究者深入探索提示在3D领域的应用潜力。
...
Result: 评估领先的视觉语言模型发现，即使最先进的系统也在MM-CoT上表现不佳，揭示了生成流畅性与真实推理保真度之间的显著差异。MM-CoT与现有基准测试相关性低，证实其测量的是视觉基础和逻辑推理的独特组合。
...
TL;DR: 提出一种基于法线引导的稀疏深度采样策略，通过PCA估计表面法线计算深度可靠性，改进深度补全模型的训练数据质量
...
Method: 提出法线引导的稀疏深度采样策略：1) 在RGB-D点云上使用PCA进行表面法线估计；2) 计算每个像素的深度可靠性度量；3) 根据可靠性分布采样稀疏深度点；4) 将该方法与Marigold-DC扩散模型集成

Result: 在NYU Depth v2数据集上的实验表明，几何感知的稀疏深度提高了准确性，减少了边缘和不连续处的伪影，并产生了更真实的训练条件

Conclusion: 几何感知的稀疏深度采样策略能更好地反映真实传感器行为，提高深度补全模型的性能和鲁棒性
...
TL;DR: FastBEV++是一个相机BEV感知框架，通过算法优化和可部署设计，在保持高性能的同时实现高效部署，在nuScenes基准上达到0.359 NDS和134 FPS的实时性能。


<details>
  <summary>Details</summary>
Motivation: 当前相机BEV感知面临性能与部署可行性之间的根本矛盾，主要源于计算密集的视角变换和平台特定的定制内核，阻碍了实际车载部署。

Method: 采用"算法快速"和"设计可部署"两大原则：1) 通过确定性预排序策略将视角变换分解为标准索引-收集-重塑管道，使用原生算子；2) 集成端到端深度感知融合机制，结合时间聚合和数据增强。

Result: 在nuScenes基准测试中达到0.359 NDS的新SOTA性能，在Tesla T4等车载硬件上实现超过134 FPS的实时性能，无需定制CUDA内核即可实现TensorRT原生移植。

Conclusion: FastBEV++通过消除对定制插件的依赖同时保持高精度，为生产级自动驾驶系统提供了成熟且可扩展的设计方案，成功解决了性能与部署可行性之间的矛盾。
...
TL;DR: HTC-VLM提出了一种混合视觉语言模型框架，通过双通道分离语义和外观信息，使用580:1的压缩比实现高效多模态推理，在多个基准测试中保持87.2%的性能。


<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型在处理视觉补丁标记时面临二次计算成本问题，现有方法在连续压缩和离散量化之间存在权衡：连续压缩会稀释高级语义（如物体身份），而离散量化会丢失细粒度细节（如纹理）。

Method: 提出HTC-VLM混合框架，通过双通道解耦语义和外观：1）连续通道使用ViT补丁保留细粒度细节；2）离散通道使用MGVQ量化生成符号锚点（投影为四个标记）。然后将两者融合成580个标记的混合序列，通过解耦注意力掩码和瓶颈压缩成单个voco标记。

Result: 在七个基准测试（GQA、VQAv2、MMBench、MME、POPE、SEED-Bench、ScienceQA-Image）上平均性能保持率达到87.2%，优于领先的连续基线方法（81.0%），压缩比为580:1。注意力分析显示压缩标记优先考虑离散锚点，验证了其语义指导作用。

Conclusion: 研究表明，极简的混合设计可以解决效率与保真度之间的困境，推动可扩展视觉语言模型的发展，证明了通过分离语义和外观的双通道方法能够实现高效且准确的多模态表示。
...
TL;DR: 提出了一种名为Residual-SwinCA-Net的新型深度混合分割框架，结合了残差CNN模块和Swin Transformer块，用于乳腺超声图像病灶分割，在BUSI数据集上取得了99.29%的平均准确率。


<details>
  <summary>Details</summary>
Motivation: 解决乳腺超声图像分割中的挑战，包括提取局部相关且鲁棒的特征、学习全局依赖关系、增强组织连续性、抑制超声噪声、保持恶性病灶轮廓的形态完整性等。

Method: 1. 结合残差CNN模块提取局部相关特征；2. 定制Swin Transformer块通过内部残差路径学习全局依赖；3. 使用拉普拉斯-高斯区域算子增强组织连续性和抑制噪声；4. 引入边界导向算子保持病灶轮廓完整性；5. 采用阶段式收缩策略捕获尺度不变性；6. 在解码器中集成多尺度通道注意力与压缩模块；7. 使用像素注意力模块自适应加权恶性病灶像素。

Result: 在公开的BUSI数据集上，Residual-SwinCA-Net框架实现了99.29%的平均准确率、98.74%的IoU和0.9041的Dice系数，优于现有的CNN和ViT技术。

Conclusion: 提出的Residual-SwinCA-Net框架显著提高了BUSI病灶诊断性能，增强了临床决策的及时性，为乳腺超声图像分割提供了一种有效的混合深度学习方法。
...
Result: 在两个高性能3D目标检测基准上，在nuScenes数据集上获得最高1.3 mAP和1.3 NDS提升，同时实现最准确的运动估计，且不增加推理成本

Conclusion: FTKD有效将未来帧知识从离线教师模型转移到在线学生模型，显著提升相机时序3D目标检测性能
...
TL;DR: 本文提出了一种查询感知的枢纽原型学习方法，通过构建查询-支持点二分图来生成与查询相关的原型，解决少样本3D点云分割中原型偏差问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基于度量的原型学习方法仅从支持集生成原型，不考虑与查询数据的相关性，导致原型偏差。当存在分布偏移时，原型过度拟合支持集特定特征，无法泛化到查询分布，从而降低分割性能。

Method: 提出查询感知枢纽原型学习方法，包含两个模块：1) 枢纽原型生成模块：构建查询和支持点的二分图，识别频繁连接的枢纽点，生成与查询相关的原型；2) 原型分布优化模块：使用纯度重加权对比损失，将坏枢纽和边界模糊原型拉近到对应类中心。

Result: 在S3DIS和ScanNet数据集上的大量实验表明，QHP方法相比最先进方法取得了显著的性能提升，有效缩小了原型与查询集之间的语义差距。

Conclusion: 提出的查询感知枢纽原型学习方法通过显式建模支持集和查询集之间的语义相关性，解决了少样本3D点云分割中的原型偏差问题，提高了分割性能。
...
TL;DR: RLCNet：一种用于LiDAR、RADAR和相机多模态传感器在线校准的端到端可训练深度学习框架
...
Method: 提出RLCNet，一种新颖的端到端可训练深度学习框架，用于多模态传感器的同时在线校准；引入包含加权移动平均和异常值拒绝的在线校准框架，支持实时操作

Result: 在真实世界数据集上验证，RLCNet在实际部署中表现出稳健性能；在线校准框架通过动态调整校准参数，减少预测噪声并提高对漂移的鲁棒性

Conclusion: 消融研究强调了架构选择的重要性，与现有方法的比较表明所提方法在准确性和鲁棒性方面具有优越性
...
TL;DR: EgoX框架通过轻量级LoRA适配和几何引导自注意力机制，将第三人称视频转换为第一人称视角视频，解决了视角差异大、重叠区域少的挑战。
...
Method: EgoX框架利用预训练的大规模视频扩散模型时空知识，通过轻量级LoRA适配；采用统一条件策略，通过宽度和通道级联结合外中心和自我中心先验；引入几何引导的自注意力机制，选择性关注空间相关区域。

Result: 该方法实现了连贯且逼真的自我中心视频生成，在未见过的和野外视频上展现出强大的可扩展性和鲁棒性。

Conclusion: EgoX框架成功解决了从单体外中心输入生成自我中心视频的挑战，通过创新的条件策略和几何引导机制，在保持几何一致性和视觉保真度方面表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 当前主体驱动图像生成模型存在两个主要问题：1）容易偏离参考身份；2）在包含多个主体的复杂场景中表现不佳。为了解决这些挑战，需要高质量、大规模的训练数据。

Method: 采用四阶段流水线构建OpenSubject数据集：1）视频筛选（分辨率与美学过滤）；2）跨帧主体挖掘与配对（基于VLM的类别共识、局部定位和多样性感知配对）；3）身份保持的参考图像合成（分割图引导的外绘和框引导的内绘，结合几何感知增强和边界侵蚀）；4）验证与标注（VLM验证样本，重新合成失败样本，构建短长标注）。同时建立了涵盖主体驱动生成和编辑的基准测试。

Result: 实验表明，使用OpenSubject数据集进行训练可以显著提升主体驱动生成和编辑的性能，特别是在复杂场景中。数据集包含250万样本和435万图像，通过VLM评估显示在身份保真度、提示遵循度、编辑一致性和背景一致性方面都有改善。

Conclusion: OpenSubject数据集通过系统化的四阶段构建流程，有效解决了主体驱动生成中的身份偏离和复杂场景处理问题，为相关任务提供了高质量的训练数据和评估基准，推动了该领域的发展。
...
TL;DR: GeoDM提出了一种几何感知的数据集蒸馏框架，通过在欧几里得、双曲和球面流形的笛卡尔积空间中操作，统一捕获平坦、层次和循环结构，优于现有方法。
...
Method: 提出GeoDM框架，在欧几里得、双曲和球面流形的笛卡尔积空间中操作；引入可学习的曲率和权重参数以适应底层数据几何；设计最优传输损失以增强分布保真度。

Result: 理论分析表明，在乘积空间中进行几何感知分布匹配比欧几里得对应方法具有更小的泛化误差界。在标准基准测试上的大量实验证明，该方法优于最先进的数据蒸馏方法，并在各种单几何分布匹配策略中保持有效。

Conclusion: 通过在多几何乘积空间中操作并引入可学习参数，GeoDM能够更好地捕获数据的固有几何结构，实现更有效的数据集蒸馏，为处理复杂数据几何提供了一种统一框架。
...
### [24] [SDT-6D: Fully Sparse Depth-Transformer for Staged End-to-End 6D Pose Estimation in Industrial Multi-View Bin Picking](https://arxiv.org/abs/2512.08430)
*Nico Leuze,Maximilian Hoh,Samed Doğan,Nicolas R. -Peña,Alfred Schoettl*

Main category: cs.CV

TL;DR: 提出一种基于深度图的6D姿态估计方法，通过多视角深度图融合为精细点云或稀疏TSDF，采用分阶段热图机制和密度感知稀疏Transformer，实现高分辨率稀疏体素表示，在密集工业分拣场景中表现优异。


<details>
  <summary>Details</summary>
Motivation: 工业分拣环境中密集堆叠物体的6D姿态估计面临严重挑战，包括遮挡、反射和无纹理部件。现有方法难以处理这些复杂情况，需要一种能够处理高分辨率表示同时保持计算效率的解决方案。

Method: 提出全深度6D姿态估计框架：1）将多视角深度图融合为精细3D点云或稀疏TSDF；2）采用分阶段热图机制生成场景自适应注意力先验，引导计算聚焦前景区域；3）设计密度感知稀疏Transformer块，动态处理遮挡和3D数据非均匀分布；4）使用基于体素的投票策略预测6D姿态，支持任意数量目标物体同时预测。

Result: 在IPD和MV-YCB多视角数据集上验证，在密集工业分拣和家庭分拣场景中表现出竞争力。方法能够处理高分辨率体素表示，捕捉精细几何细节，在遮挡和反射条件下仍能准确估计姿态。

Conclusion: 提出的全稀疏框架成功解决了密集工业分拣中的6D姿态估计挑战，通过高分辨率体素表示和智能计算分配，在保持内存效率的同时实现了精确的姿态恢复，为近距离机器人应用提供了有效解决方案。
...
TL;DR: 该论文介绍了3DTeethLand挑战赛，旨在通过深度学习技术解决牙齿标志点检测的临床挑战，并发布了首个公开的3D牙齿标志点检测数据集。
...
Method: 通过组织3DTeethLand挑战赛，与MICCAI 2024合作，征集专注于从口内3D扫描中检测牙齿标志点的算法，并发布了首个公开的3D牙齿标志点检测数据集。

Result: 该挑战赛引入了首个公开可用的3D牙齿标志点检测数据集，为评估该任务的最先进方法提供了宝贵资源，并鼓励社区提供具有重要临床意义的方法学贡献。
...
<details>
  <summary>Details</summary>
Motivation: 现有欧拉方法通过解耦表示学习（纹理、形状、频率方案）处理放大引起的噪声，但在运动位移非常小时难以分离光子噪声与真实微运动，需要更有效的噪声抑制和结构保持方法。

Method: 1. 无噪声光流增强策略：合成多样非刚性运动场作为监督，学习更准确的几何感知光流；2. 扩散运动放大器：以光流作为几何先验和可学习放大因子控制幅度，选择性放大与场景语义结构一致的运动；3. 基于光流的视频合成：将放大运动高保真映射回图像域。

Result: 在真实和合成数据集上的大量实验表明，GeoDiffMM优于现有最先进方法，显著改善了运动放大效果。

Conclusion: 提出的基于扩散的拉格朗日VMM框架通过几何条件化光流，实现了结构一致的运动放大，有效解决了小位移下光子噪声分离的挑战。
...
### [27] [Disturbance-Free Surgical Video Generation from Multi-Camera Shadowless Lamps for Open Surgery](https://arxiv.org/abs/2512.08577)
*Yuna Kato,Shohei Mori,Hideo Saito,Yoshifumi Takatsume,Hiroki Kajita,Mariko Isogawa*

Main category: cs.CV

TL;DR: 提出了一种自动对齐手术视频中多摄像头视角的方法，通过检测照明系统移动、重新对齐帧并选择遮挡最少的摄像头，生成固定视角的手术区域视频。


<details>
  <summary>Details</summary>
Motivation: 手术视频对教育和研究很重要，但拍摄时外科医生经常遮挡摄像头视野。现有方法使用多个摄像头但需要手动对齐，因为每次调整照明时摄像头配置都会变化。

Method: 识别照明系统移动的帧，重新对齐这些帧，选择遮挡最少的摄像头，生成固定视角的手术区域视频。还实现了多种合成选项供用户选择。

Result: 用户研究表明，该方法生成的视频在确认手术区域的便利性和观看舒适度方面优于传统方法。视频质量也有所提升，并评估了外科医生对不同合成选项的偏好。

Conclusion: 该方法能够完全自动化多摄像头手术视频的对齐任务，生成高质量、固定视角的手术视频，为外科教育和研究提供了更好的工具。
...
TL;DR: 提出了一种基于四元数的低秩支持四元数矩阵机（LSQMM）用于彩色图像分类，将RGB通道作为纯四元数处理以保持通道间的内在耦合关系，并通过四元数核范数正则化促进低秩结构。


<details>
  <summary>Details</summary>
Motivation: 传统彩色图像分类方法通常将特征表示为向量、矩阵或三阶张量，但未能有效保持RGB通道间的内在耦合关系。受四元数在图像恢复和去噪任务中成功应用的启发，希望利用四元数代数更好地建模彩色图像。

Method: 提出LSQMM模型：1）将RGB通道作为纯四元数处理；2）引入四元数核范数正则化项以促进低秩结构；3）设计基于ADMM的迭代算法求解四元数优化模型。

Result: 在多个彩色图像分类数据集上的实验表明，LSQMM在分类准确率、鲁棒性和计算效率方面优于支持向量机、支持矩阵机和支持张量机等现有方法。

Conclusion: LSQMM通过四元数表示有效保持了彩色图像通道间的耦合关系，结合低秩正则化提升了分类性能，为彩色图像分类提供了新的有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 尽管Glaze和Nightshade等图像保护机制在经验上有效，但其内部结构、可检测性和表征行为仍不清楚。本研究旨在通过可解释AI分析来理解这些保护机制的工作原理。

Method: 采用统一框架，结合白盒特征空间检查（潜在空间聚类、特征通道激活分析、基于遮挡的空间敏感性映射）和黑盒信号级探测（频域表征分析），系统分析保护机制。

Result: 保护机制作为结构化、低熵的扰动运行，与底层图像内容在表征、空间和频谱域紧密耦合。受保护图像保留了内容驱动的特征组织，但具有保护特定的子结构。可检测性受扰动熵、空间部署和频率对齐的交互效应控制。Glaze和Nightshade沿主导图像对齐频率轴重新分配能量，而非引入扩散噪声。

Conclusion: 当代图像保护通过结构化特征级变形而非语义错位操作，这解释了保护信号为何视觉上微妙但一致可检测。该工作推进了对抗性图像保护的可解释性，并为未来生成AI系统的防御和检测策略设计提供了信息。
...
<details>
  <summary>Details</summary>
Motivation: 无人机航拍图像中的目标具有任意方向和精细细节，需要旋转不变性进行精确分割。传统卷积架构如U-Net缺乏旋转不变性，导致不同视角下分割精度下降。虽然可以通过扩展多方向滤波器组实现旋转不变性，但这会显著增加计算成本和内存流量。

Method: 提出GPU优化的旋转不变卷积框架，消除了传统基于矩阵乘法的卷积所需的数据降维（im2col）步骤。通过利用对称旋转滤波器间的结构化数据共享，实现了多方向卷积，大幅减少了内存流量和计算冗余。该方法还可推广到加速任意（非对称）旋转角度的卷积。

Result: 在广泛基准测试中，提出的卷积比CUDNN快20-55%的训练速度，能耗降低15-45%，同时保持与最先进旋转不变方法相当的精度。在八方向设置下，256×256输入上实现45%加速和41%能耗节省，1024×1024输入上实现32%加速和23%能耗降低。集成到U-Net分割模型中，比非旋转感知基线精度提升达6%。
...
TL;DR: PointDico：一种结合扩散模型和对比学习的3D表示学习方法，通过知识蒸馏实现两种范式的优势互补，在3D点云表示学习上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有的自监督表示学习方法在3D数据上面临挑战，因为3D数据无序且密度不均匀。对比模型容易过拟合，而3D掩码自编码器难以处理无序点云。需要结合扩散模型和对比学习的优势来改进3D表示学习。
...
Conclusion: PointDico成功结合了扩散模型和对比学习的优势，通过知识蒸馏和创新的架构设计，有效解决了3D点云表示学习的挑战，取得了显著的性能提升。
...
TL;DR: LiDAS是一种闭环主动照明系统，通过动态优化照明分布来提升夜间感知性能，将白天的感知模型零样本应用于夜间环境。
...
Method: 结合现成的视觉感知模型和高清头灯，动态预测最优照明场，最大化下游感知性能，减少空区域照明以重新分配到物体区域。

Result: 在真实世界闭环驾驶场景中，相比标准近光灯，mAP50提升18.7%，mIoU提升5.0%，在保持性能的同时减少40%能耗。

Conclusion: LiDAS通过将现有头灯转变为主动视觉执行器，为鲁棒的夜间感知提供了经济有效的解决方案，无需重新训练即可增强模型鲁棒性。
...
Method: 提出双模态双自适应掩码感知卷积(Bi^2MAC)，设计轻量级模块生成软硬两种掩码：软掩码用于初步调制输入特征，硬掩码将不同类型区域引导到不同处理分支。冗余特征进入紧凑分支进行低成本全局处理，异质特征进入聚焦分支投入更多计算资源进行细粒度建模

Result: 在多个基准数据集上的实验表明，Bi^2MAC实现了最先进的性能，同时显著降低了训练时间和参数数量，在自适应卷积模型中计算成本最小

Conclusion: Bi^2MAC方法通过智能分配计算资源，有效利用不同类型区域的信息，在全色锐化任务中取得了计算效率和性能的平衡
...
<details>
  <summary>Details</summary>
Motivation: 当前使用3D高斯溅射渲染真实世界复杂反射场景虽然是有前景的解决方案，但在渲染速度和内存存储方面仍面临瓶颈，需要更高效的渲染方法。

Method: 提出混合溅射机制：1) 反射烘焙高斯追踪，在每个高斯基元内烘焙视角相关反射；2) 基于瓦片的高斯溅射渲染反射；3) 将反射高斯基元与基础高斯基元通过统一混合溅射框架集成；4) 引入流水线级加速和反射敏感高斯剪枝以减少模型大小。

Result: 在Ref-NeRF和NeRF-Casting的复杂反射场景上，HybridSplat实现了约7倍的渲染加速，使用的高斯基元数量比类似基于光线追踪的高斯溅射基线少4倍，成为复杂反射场景的新SOTA方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有单目3D跟踪方法存在两个主要局限：1) 无法有效分离相机运动与前景动态运动；2) 无法密集跟踪视频中新出现的动态目标。需要一种能在世界坐标系下跟踪几乎所有像素的方法。

Method: 提出三阶段管道：1) 跟踪上采样器将稀疏2D跟踪提升为密集2D跟踪；2) 将上采样器应用于所有帧，通过消除重叠区域跟踪来减少冗余；3) 基于优化的框架将密集2D跟踪反投影到世界中心的3D轨迹，同时估计相机姿态和3D坐标。

Result: 在合成和真实世界数据集上的广泛评估表明，该系统在世界坐标系下实现了准确且密集的3D跟踪。

Conclusion: TrackingWorld方法有效解决了现有单目3D跟踪的局限性，能够在世界坐标系中实现密集的3D像素跟踪，分离相机运动与动态目标运动，并能处理新出现的动态主体。
...
Method: 提出SCU-CGAN模型，集成U-Net、CBAM（卷积注意力模块）和额外判别器，从非火灾图像生成逼真的火灾图像。通过评估生成图像质量，验证模型有效性。

Result: SCU-CGAN在KID评分上比CycleGAN提升41.5%，生成图像质量更优。使用增强数据集后，火灾检测模型性能显著提升：YOLOv5 nano模型的mAP@0.5:0.95指标提升56.5%，且无需改变模型结构。
...
TL;DR: MLLMs中的Pre-Norm架构导致视觉token和文本token存在严重的范数差异，这引发了不对称更新动态，阻碍了跨模态特征融合。通过简单的LayerNorm层插入即可显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）虽然表现出色，但其普遍采用的Pre-Norm架构存在一个关键缺陷：视觉token的高范数与文本token的低范数之间存在严重不平衡。这种不平衡不是静态问题，而是会导致"不对称更新动态"，即高范数的视觉token表现出"表征惯性"，其语义转换速度远慢于文本token，从而损害跨模态特征融合的有效性。

Method: 首先对范数不平衡问题进行形式化理论分析，证明其导致的不对称更新动态现象。然后提出一个简单而有效的解决方案：在视觉投影器后插入一个精心初始化的LayerNorm层，以强制实现范数对齐。该方法在LLaVA-1.5架构上进行实验验证。

Result: 实验验证表明，范数差异和由此产生的不对称更新速率是主流MLLMs中的普遍现象。提出的LayerNorm干预方法在LLaVA-1.5架构上取得了显著性能提升，不仅在一系列多模态基准测试中表现更好，而且在纯文本评估（如MMLU）上也显示出改进，表明解决架构不平衡可以带来更全面的模型能力。

Conclusion: MLLMs中的范数不平衡问题是一个关键但被忽视的架构缺陷，它通过不对称更新动态阻碍了有效的跨模态融合。简单的LayerNorm层插入可以有效解决这一问题，显著提升模型在多模态和纯文本任务上的性能，证明了架构平衡对模型整体能力的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 在挑战性光照条件下，捕获的图像常出现各种退化，导致基于视觉的应用性能下降。现有方法要么显著放大固有噪声，要么只在特定光照条件下有效，需要一种能在复杂光照条件下同时增强图像和抑制噪声的解决方案。
...
Conclusion: 提出的框架能够有效处理复杂光照条件下的图像退化问题，同时实现图像增强和噪声抑制，为实际应用中的图像质量改善提供了有效解决方案。
...
TL;DR: 该研究针对面部美化处理对生物识别系统的影响，提出通过分析美化算法变化和改进特征提取方法来检测面部美化，并在未知攻击算法情况下实现了1.1%的D-EER检测率。


<details>
  <summary>Details</summary>
Motivation: 面部美化处理在社交媒体和广告中广泛使用，但当这些美化后的图像作为生物识别样本时，会对人脸识别系统造成挑战。因此需要有效检测面部美化处理。

Method: 研究分析美化评估算法的变化，评估基于人工智能的不同特征提取方法，探索是否可以利用面部美感评估来提升检测率。

Result: 在攻击美化算法未知的场景下，单图像检测达到了1.1%的D-EER（检测等错误率）。

Conclusion: 面部美化检测对于生物识别系统的安全性至关重要，通过分析美化算法特征和改进检测方法可以有效识别美化处理，提升系统鲁棒性。
...
TL;DR: 本文针对渔业电子监控系统产生的大量视频数据，开发了优化的深度学习管道用于鱼类重识别，使用AutoFish数据集，通过硬三元组挖掘和定制图像变换显著提升了重识别性能。


<details>
  <summary>Details</summary>
Motivation: 随着电子监控系统在渔业中的广泛应用，产生了大量视频数据，手动审查不可行。需要自动化解决方案来有效识别鱼类，以支持可持续的海洋资源管理。

Method: 开发了优化的深度学习管道，使用AutoFish数据集（模拟EM系统的传送带场景，包含6种相似鱼类）。采用硬三元组挖掘策略和定制图像变换管道（包括数据集特定归一化），比较了Vision Transformer的Swin-T架构和CNN的ResNet-50性能。

Result: Swin-T架构在硬三元组挖掘和定制变换下表现最佳，达到41.65% mAP@k和90.43% Rank-1准确率。分析显示主要挑战是区分同一物种的视觉相似个体（种内错误），其中视角不一致比部分遮挡更具破坏性。

Conclusion: 优化的深度学习管道显著提升了鱼类重识别性能，为渔业电子监控系统的自动化分析提供了有效解决方案。Vision Transformer架构在区分相似鱼类方面优于传统CNN，但种内识别仍是主要挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于图像的HMR方法在处理视频时存在时间不一致性和遮挡下性能下降的问题，需要一种无需重新训练就能解决这些挑战的方案

Method: 提出训练免费的SAM-Body4D框架：1) 使用可提示视频分割模型生成身份一致的掩码片段；2) 遮挡感知模块恢复缺失区域；3) 精炼的掩码片段指导SAM 3D Body生成一致的全身体网格轨迹；4) 基于填充的并行策略实现高效多人推理

Result: 实验结果表明，SAM-Body4D在具有挑战性的野外视频中实现了改进的时间稳定性和鲁棒性，且无需任何重新训练

Conclusion: SAM-Body4D通过利用视频中固有的人类连续性，有效解决了现有HMR方法在视频处理中的时间不一致和遮挡问题，为视频中的人体网格恢复提供了实用解决方案
...
TL;DR: OneClip-RAG：一种基于单次视频片段检索增强的高效多模态大语言模型范式，通过查询引导的视频分块算法和渐进式训练，显著提升长视频理解能力


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs因内存开销过大只能处理有限帧数的视频，需要一种有效且高效的方法来增强长视频理解能力

Method: 提出OneClip-RAG范式，包含查询引导的视频分块算法（统一分块和跨模态检索），以及SynLongVideo数据集和渐进式训练策略

Result: 在多个MLLMs上验证，显著提升长视频理解性能（如将InternLV2 8B和Qwen2-VL 7B提升到GPT-4o水平），效率极高（LLaVA-Video可处理1小时视频仅需2.2分钟）

Conclusion: OneClip-RAG是一种有效且高效的视频检索增强范式，能显著提升MLLMs的长视频理解能力，同时保持高处理效率
...
<details>
  <summary>Details</summary>
Motivation: 当前手术分割面临标注稀缺和语义不一致的挑战，现有方法通常只是微调自然基础模型（如SAM），仅作为领域适配器而非真正的手术基础模型，难以泛化到多样化的手术目标。
...
Conclusion: LapFM是一个专门为腹腔镜手术设计的基础模型，通过创新的分层概念演化预训练方法，解决了手术分割中的标注稀缺和语义不一致问题，实现了更好的泛化性能。
...
TL;DR: 提出一个统一的端到端学习框架，联合利用高分辨率RGB传感器和辅助低分辨率多光谱传感器进行颜色校正，相比现有方法显著提升颜色准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的快照多光谱成像系统虽然能捕获比传统RGB传感器更丰富的频谱信息，但大多数现有方法将颜色校正流程分为多个独立阶段，往往在早期就丢弃了多光谱数据，导致信息利用不充分。

Method: 提出一个统一的端到端学习框架，将整个颜色校正流程集成在单一模型中，联合利用高分辨率RGB传感器和辅助低分辨率多光谱传感器的数据。通过重构两种不同的最先进的图像到图像架构来展示框架的灵活性和通用性，并构建专门的数据集进行训练和评估。

Result: 实验表明该方法显著提高了颜色准确性和稳定性，与仅使用RGB和多光谱驱动的基线方法相比，误差减少了高达50%。

Conclusion: 提出的端到端学习框架能够有效联合利用RGB和多光谱数据，实现更准确和稳定的颜色校正，为消费级和移动设备的多光谱成像应用提供了有效的解决方案。
...
TL;DR: 该研究分析了基于子集选择的方法在OOD条件下的局限性，并提出结合子模优化和不确定性估计的新框架来提升解释的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有基于子集选择的可解释性方法在分布内（ID）表现良好，但在分布外（OOD）条件下的可靠性和稳定性显著下降，产生冗余、不稳定且对不确定性敏感的解释，这限制了这些方法在真实世界视觉应用中的可信度。

Method: 提出一个结合子模子集选择和分层梯度不确定性估计的框架。通过自适应权重扰动估计不确定性，并利用这些估计指导子模优化，确保多样化和信息丰富的子集选择，无需额外训练或辅助模型。

Result: 实验评估表明，该框架不仅缓解了现有方法在OOD场景下的弱点，还在ID设置中带来改进。新方法产生的解释更加稳定、多样且对不确定性不敏感。

Conclusion: 研究揭示了当前基于子集方法的局限性，展示了不确定性驱动优化如何增强归因和对象级可解释性，为现实世界视觉应用中更透明、可信的AI铺平了道路。
...
<details>
  <summary>Details</summary>
Motivation: 足球运动员跟踪面临频繁遮挡、相似外观和拥挤场景中快速运动的挑战。现有方法在资源受限环境下难以平衡跟踪精度和计算效率，需要一种轻量级且鲁棒的解决方案。

Method: 提出团队感知跟踪系统：1) 使用SAM进行精确初始化；2) 结合CSRT跟踪器进行连续跟踪；3) 基于HSV直方图的球衣颜色外观模型进行重识别，以改善遮挡恢复。

Result: 在足球视频序列上评估：处理速度7.6-7.7 FPS，内存稳定约1880 MB；轻度遮挡下100%跟踪成功率，拥挤禁区场景（5名以上球员）90%成功率；外观重识别能恢复50%重度遮挡。但长期遮挡（球员离开画面）重获取成功率仅8.66%。

Conclusion: SAM+CSRT组合在不同人群密度下提供一致性能，但在长期遮挡场景表现不足。经典跟踪器方法在连续可见性下表现良好，但需要更强的重识别机制处理长时间缺席。结果为资源受限条件下部署足球跟踪系统提供了实用指南。
...
TL;DR: ContextDrag是一种新的基于拖拽的图像编辑范式，通过利用编辑模型的强大上下文建模能力，结合参考图像的VAE编码特征，实现了更连贯、保真度更高的图像编辑效果。
...
TL;DR: 本文提出PCI框架，通过分析概念插入成功率来研究扩散模型中概念形成的动态过程，揭示了不同概念在不同时间步的锁定时机，为图像编辑提供实用指导。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注扩散模型的最终输出，但生成过程是动态的轨迹。理解这一动态过程对于掌握模型的可控性、可靠性和可预测性至关重要。本文旨在探究噪声何时转化为特定概念并锁定去噪轨迹。

Method: 提出PCI（提示条件干预）框架，这是一个无需训练、模型无关的方法。核心是分析概念插入成功率（CIS），即在给定时间步插入的概念在最终图像中被保留的概率。通过这种方法可以刻画概念形成的时间动态。

Result: 应用于多个先进的文本到图像扩散模型和广泛的概念分类，PCI揭示了不同扩散模型中多样的时间行为。研究发现轨迹的某些阶段对特定概念更有利，即使在同一概念类型内也存在差异。这些发现为文本驱动的图像编辑提供了可操作的见解。

Conclusion: PCI框架能够分析扩散模型中概念形成的动态过程，揭示了概念锁定的时间特性。该方法无需访问模型内部或进行训练，为图像编辑提供了量化指导，在语义准确性和内容保留之间取得了比基线方法更好的平衡。
...
TL;DR: 首个面向多相机阵列的在线3D高斯泼溅重建框架，通过多相机RGB流融合实现无漂移轨迹估计和高效在线重建，在2分钟内重建数百米3D场景。


<details>
  <summary>Details</summary>
Motivation: 现有单目3DGS在线重建方法因视野有限难以实现完整3D覆盖，多相机阵列能从根本上解决此限制，但缺乏相应的在线重建框架。

Method: 提出分层相机初始化方案实现无标定粗对齐，轻量级多相机束调整稳定轨迹，冗余消除高斯采样策略减少基元数量，频率感知优化调度器减少迭代次数。

Result: 仅使用原始多相机视频流，在2分钟内重建数百米3D场景，展现出前所未有的速度、鲁棒性和重建保真度。

Conclusion: 该方法首次实现了多相机阵列的在线3D高斯泼溅重建，通过多相机融合、高效优化策略和冗余消除技术，在保持实时性能的同时显著提升了重建覆盖范围和保真度。
...
TL;DR: ReasonBreak是一种针对多模态大推理模型的对抗性隐私保护框架，通过概念感知扰动破坏地理定位推理，相比现有方法显著提升了隐私保护效果。
...
Method: 提出ReasonBreak对抗性框架，基于概念层次而非均匀噪声的扰动策略，针对推理链中的关键概念依赖关系生成扰动，使特定推理步骤失效并级联影响后续推理阶段。同时贡献了包含6,341张超高分辨率图像的GeoPrivacy-6K数据集。

Result: 在7个最先进的MLRMs（包括GPT-o3、GPT-5、Gemini 2.5 Pro）上评估，ReasonBreak在区域级保护上提升14.4%（33.8% vs 19.4%），在街区级保护上几乎翻倍（33.5% vs 16.8%）。

Conclusion: ReasonBreak为对抗基于推理的隐私威胁建立了新范式，通过概念感知扰动有效破坏MLRMs的分层推理过程，显著提升了地理隐私保护效果。
...
<details>
  <summary>Details</summary>
Motivation: 条件扩散模型虽然依赖语言-图像对齐方法来引导生成，但错位和幻觉问题仍然常见。现有方法需要在完整生成后才能评估对齐性，计算成本高昂。本文假设文本/图像错位可以在去噪过程中早期检测，实现实时对齐评估。

Method: 提出NoisyCLIP方法，在噪声潜在空间中测量语义对齐。这是首个在反向扩散过程中使用双编码器探索和基准测试提示-潜在错位检测的工作，能够在生成过程中实时评估对齐性。

Result: NoisyCLIP在BoN设置中将计算成本降低50%，同时达到98%的CLIP对齐性能。该方法实现了生成过程中的实时对齐评估，在不牺牲语义保真度的前提下降低成本。

Conclusion: 文本/图像错位可以在去噪过程中早期检测，NoisyCLIP方法通过测量噪声潜在空间中的语义对齐，实现了高效、实时的对齐评估，为扩散模型的质量控制提供了新思路。
...
TL;DR: 提出OCCDiff方法，在占用函数空间应用潜在扩散模型，从LiDAR点云中灵活重建高质量建筑表面，处理不同点密度和噪声干扰。


<details>
  <summary>Details</summary>
Motivation: 从LiDAR点云重建建筑面临的主要挑战是在不同点密度和噪声干扰下准确捕捉建筑表面。需要一种能灵活获取高质量3D轮廓的方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有GUI坐标预测模型存在严重的坐标预测不稳定性问题，轻微视觉扰动（如裁剪几个像素）就会导致预测结果在正确与错误之间翻转，严重影响模型性能，特别是在高分辨率和小UI元素场景下

Method: 提出MVP（多视角预测）训练免费框架，包含两个组件：1）注意力引导的视角提议，基于指令到图像的注意力分数生成多样化的裁剪视图；2）多坐标聚类，通过选择最密集空间簇的质心来集成预测结果
...
<details>
  <summary>Details</summary>
Motivation: 油画作为融合人类抽象思维与艺术表达的高级媒介，其复杂的笔触动态和风格化特征给数字生成和编辑带来巨大挑战。现有技术受限于训练数据分布，主要关注修改真实照片，缺乏对油画风格生成和编辑的统一解决方案。

Method: 1. 训练阶段增强：采用空间对齐和语义增强条件策略，将掩码和草图映射为空间约束，从参考图像和文本编码上下文嵌入作为特征约束，实现对象级语义对齐。2. 自监督风格迁移：基于笔触渲染(SBR)构建自监督风格迁移管道，模拟油画修复的修复动态，将真实图像转换为保留笔触纹理的风格化油画，构建大规模配对训练数据集。3. 推理阶段：使用AdaIN算子集成特征以确保风格一致性。

Result: 广泛的实验表明，该交互式系统能够在保持油画艺术品质的同时实现细粒度编辑，在风格化油画生成和编辑方面达到了前所未有的想象力实现水平。

Conclusion: 提出的统一多模态框架通过技术创新解决了油画数字生成和编辑的挑战，实现了交互式油画创作，为艺术创作提供了新的可能性。
...
TL;DR: Photo3D是一个通过GPT-4o-Image生成的图像数据驱动，提升3D生成真实感的框架，通过结构对齐的多视角合成和细节增强技术，在保持几何结构一致性的同时实现逼真外观。


<details>
  <summary>Details</summary>
Motivation: 当前3D原生生成器在几何结构方面取得了进展，但在实现逼真外观方面仍然不足。主要障碍在于缺乏多样化和高质量的真实世界3D资产，这些资产需要丰富的纹理细节，而由于场景尺度多样、物体非刚性运动和3D扫描精度有限，获取这样的数据本质上是困难的。

Method: 1. 使用GPT-4o-Image模型生成的图像数据驱动框架；2. 设计结构对齐的多视角合成管道，构建细节增强的多视角数据集与3D几何配对；3. 提出逼真细节增强方案，利用感知特征适应和语义结构匹配，在保持与3D原生几何结构一致性的同时实现逼真细节的外观一致性；4. 为几何-纹理耦合和解耦的3D原生生成范式提供专门的训练策略。

Result: 实验表明，Photo3D能够很好地泛化到不同的3D原生生成范式，并实现了最先进的逼真3D生成性能。

Conclusion: Photo3D框架通过利用生成图像数据、结构对齐的多视角合成和细节增强技术，有效解决了3D生成中逼真外观的挑战，为不同的3D原生生成器提供了通用的逼真细节增强方案。
...
TL;DR: Fast-ARDiff提出了一种统一的AR-扩散混合框架，通过熵感知推测解码和动态调度器联合优化AR和扩散组件，显著加速生成过程，在图像和文本生成任务上实现3-4.3倍无损加速。


<details>
  <summary>Details</summary>
Motivation: 现有的AR-扩散混合范式结合了AR的结构化建模能力和扩散模型的光写实合成能力，但由于顺序AR生成和迭代去噪过程，存在高延迟问题。需要解决这一瓶颈，实现更高效的生成。

Method: 1) 熵感知推测策略：鼓励草稿模型产生与目标模型熵特征对齐的高熵表示，缓解熵不匹配和因草稿过度自信导致的高拒绝率。2) 动态调度器：将扩散解码集成到端到端框架中，优先优化AR部分来指导后续扩散步骤。3) 联合蒸馏框架：结合轨迹匹配和分布匹配优化扩散部分，确保稳定训练和极少数步骤下的高质量合成。4) 推理时使用AR模块的浅层特征熵预过滤低熵草稿，避免冗余计算。

Result: 在ImageNet 256×256上，TransDiff实现了4.3倍无损加速；在文本条件生成任务上，NextStep-1实现了3倍加速。该方法在多种模型上均达到最先进的加速效果。
...
TL;DR: 提出了一种无需迭代的固定点估计器用于扩散模型反演，通过误差近似方法避免固定点迭代的高计算成本和超参数选择问题
...
Method: 首先推导理想反演步骤中固定点的显式表达式，然后引入误差近似方法，使用前一步的可计算误差来近似当前步骤的未知误差，从而得到可计算的近似固定点表达式

Result: 在NOCAPS和MS-COCO数据集上的评估显示，相比DDIM反演和其他基于固定点迭代的方法，该方法在重建任务中取得了更优且一致的性能，无需额外迭代或训练
...
### [58] [SSCATeR: Sparse Scatter-Based Convolution Algorithm with Temporal Data Recycling for Real-Time 3D Object Detection in LiDAR Point Clouds](https://arxiv.org/abs/2512.08557)
...
<details>
  <summary>Details</summary>
Motivation: 传统LiDAR目标检测方法对每帧完整点云进行处理，计算量大。LiDAR连续扫描中大部分区域变化缓慢，存在大量冗余计算，需要更高效的处理方法。

Method: 1. 使用滑动时间窗口和短步长处理连续LiDAR数据流；2. 存储卷积结果在帧间复用；3. 仅处理变化的点云区域；4. 扩展散射卷积支持数据复用，提出SSCATeR算法。

Result: 1. 处理时间最多减少6.61倍；2. 输出特征图与传统稀疏卷积技术完全相同；3. 计算效率大幅提升；4. 在保持精度的同时显著减少卷积操作。

Conclusion: SSCATeR算法通过利用LiDAR数据的时间连续性和空间稀疏性，实现了高效的目标检测，为实时LiDAR处理提供了有效的解决方案。
...
### [59] [BrainExplore: Large-Scale Discovery of Interpretable Visual Representations in the Human Brain](https://arxiv.org/abs/2512.08560)
*Navve Wasserman,Matias Cosarinsky,Yuval Golbari,Aude Oliva,Antonio Torralba,Tamar Rott Shaham,Michal Irani*

Main category: cs.CV

TL;DR: 该论文提出了一个大规模自动化框架，用于发现和解释人类大脑皮层中的视觉表征，通过无监督分解方法发现可解释模式，并自动生成自然语言描述来解释这些模式。


<details>
  <summary>Details</summary>
Motivation: 理解人类大脑如何表示视觉概念以及这些表征在哪些脑区编码是一个长期挑战。现有研究通常规模较小，依赖人工检查，关注特定区域和属性，缺乏系统验证。大脑信号复杂，视觉概念空间巨大，需要更系统的方法。

Method: 方法分为两个主要阶段：首先通过无监督数据驱动分解方法发现fMRI活动中的候选可解释模式；然后通过识别最能激发这些模式的自然图像集，并生成描述其共享视觉意义的自然语言描述来解释每个模式。引入自动化流程测试多个候选解释，分配定量可靠性分数，并为每个体素模式选择最一致的描述。

Result: 该框架揭示了数千个可解释模式，涵盖许多不同的视觉概念，包括以前未报告的细粒度表征。

Conclusion: 该研究提供了一个大规模自动化框架，能够系统发现和解释人类大脑皮层中的视觉表征，克服了传统小规模研究的局限性，为理解大脑视觉处理提供了新工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有连续测试时适应方法通常依赖固定或批量级阈值，无法考虑不同类别和实例间的难度差异，这在需要密集多类预测的语义分割任务中尤为严重，导致监督不可靠和错误累积。

Method: 提出自适应调整伪标签的方法：1）根据每张图像内的置信度分布调整伪标签，反映实例级难度；2）动态平衡学习，重点关注受域偏移影响最大的类别；3）实现细粒度、类别和实例感知的适应。

Result: 在八个CTTA和TTA场景（包括合成到真实和长期偏移）的广泛实验中，该方法在语义分割任务上一致优于现有最先进技术，为演化条件下的语义分割设定了新标准。

Conclusion: 通过自适应调整伪标签和动态平衡类别学习，该方法能够产生更可靠的监督信号，减轻连续适应过程中的错误累积，在持续演化的域条件下实现了更鲁棒的语义分割性能。
...
TL;DR: HiGINE是一种基于分层图的方法，利用肿瘤微环境的多重免疫荧光图像预测肺癌患者生存期，通过融合临床分期信息提升风险分层性能。
...
Method: 提出HiGINE分层图模型，编码细胞邻域中的局部和全局相互关系，整合细胞类型和形态学信息；通过多模态融合将癌症分期与mIF衍生特征相结合。

Result: 在两个公共数据集上验证，显示HiGINE在风险分层、鲁棒性和泛化性方面均有改进，多模态融合进一步提升了性能。

Conclusion: HiGINE能够有效利用肿瘤微环境的复杂相互作用信息，结合临床数据显著改善肺癌患者的生存预测和风险分层，具有临床应用潜力。
...
TL;DR: 本研究探索了深度学习在光学和全息显微镜图像中自动检测和分类花粉粒的应用，特别关注兽医细胞学用例。使用YOLOv8s进行目标检测，MobileNetV3L进行分类，在光学图像上取得了91.3% mAP50检测精度和97%分类准确率，但在灰度全息图像上性能显著较低。通过自动标注和边界框区域扩大的数据集扩展技术，将全息图像的检测性能从2.49%提升到13.3% mAP50，分类性能从42%提升到54%。


<details>
  <summary>Details</summary>
Motivation: 研究旨在解决花粉粒在光学和全息显微镜图像中的自动检测和分类问题，特别关注兽医细胞学应用场景。传统方法可能效率低下或成本高昂，需要探索深度学习技术来改善这一过程，并验证低成本无透镜数字全息显微镜设备与深度学习技术结合的可能性。

Method: 采用YOLOv8s模型进行目标检测任务，MobileNetV3L模型进行分类任务。针对全息图像性能较差的问题，实施了数据集扩展策略，包括自动标注技术和边界框区域扩大技术。在两种成像模态（光学和全息）上评估模型性能。

Result: 在光学图像上，模型取得了91.3% mAP50的检测精度和97%的整体分类准确率。在全息图像上，初始性能较低（2.49% mAP50检测精度和42%分类准确率）。通过数据集扩展技术，全息图像的检测性能提升到13.3% mAP50，分类性能提升到54%。

Conclusion: 研究表明，至少在图像分类任务中，可以将深度学习技术与低成本的无透镜数字全息显微镜设备相结合。虽然全息图像的性能仍低于光学图像，但通过数据集扩展技术可以显著改善性能。这为在资源有限的环境中使用经济高效的成像设备进行自动化细胞学分析提供了可能性。
...
TL;DR: CLIP模型在少样本学习中存在模板-样本相似性偏差，导致模型依赖模板而非真实类别对齐。本文提出使用空提示捕获无偏模板特征来抵消这种偏差，通过预训练和微调两阶段框架提升分类准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 研究发现CLIP模型中的模板-样本相似性(TSS)会引入偏差，使模型过度依赖文本模板与图像样本的相似性，而非真正的样本-类别对齐，这降低了分类的准确性和鲁棒性。

Method: 提出两阶段框架：1) 预训练阶段使用空提示（不含类别信息的文本输入）来揭示和减少CLIP编码器中的模板诱导偏差；2) 少样本微调阶段使用偏差校准损失强制图像与其类别正确对齐，确保模型关注相关视觉线索。

Result: 在多个基准测试上的实验表明，模板校正方法显著减少了由TSS引起的性能波动，获得了更高的分类准确性和更强的鲁棒性。

Conclusion: 通过使用空提示来解耦CLIP中的模板偏差，可以有效提高少样本学习的性能，使模型更加关注视觉内容与类别的真实对齐关系。
...
Method: 提出OpenMonoGS-SLAM框架，统一3D高斯溅射与开放集语义理解。利用视觉基础模型：MASt3R用于视觉几何，SAM和CLIP用于开放词汇语义。采用自监督学习目标，无需深度输入或3D语义标注。设计了专门的内存机制管理高维语义特征，构建高斯语义特征图

Result: 实验结果表明，该方法在封闭集和开放集分割任务中达到或超越现有基线性能，且不依赖深度图或语义标注等辅助传感器

Conclusion: OpenMonoGS-SLAM成功实现了首个结合3D高斯溅射与开放集语义理解的单目SLAM框架，通过视觉基础模型和专门的内存机制，在无需额外传感器的情况下实现了开放世界环境下的准确感知和理解
...
TL;DR: 提出一种通过分析视频流模糊模式来估计度量深度和密集轨迹的新方法，结合光学设计算法，在手持拍摄场景中实现高精度深度估计和轨迹重建。


<details>
  <summary>Details</summary>
Motivation: 在缺乏机械稳定器的情况下，相机在拍摄过程中不可避免地会发生旋转运动，特别是在长时间曝光场景下会导致透视模糊。从光学角度看，这种模糊是深度位置相关的：不同空间位置的物体即使在相同成像设置下也会产生不同的模糊程度。

Method: 提出一种新颖方法，通过分析视频流的模糊模式来估计度量深度和密集轨迹，结合联合光学设计算法。具体包括：使用现成的视觉编码器和点跟踪器提取视频信息；通过窗口嵌入和多窗口聚合估计深度图；使用视觉语言模型对光学算法产生的稀疏轨迹进行稠密化。

Result: 在多个深度数据集上的评估表明，该方法在大深度范围内表现出色，同时保持良好的泛化能力。相对于手持拍摄场景中的真实轨迹，光学算法实现了更高的精度，稠密重建保持了很强的准确性。

Conclusion: 该方法通过分析视频模糊模式成功实现了度量深度估计和密集轨迹重建，在手持拍摄场景中表现出优越的性能，为无稳定器条件下的深度感知提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有空中视觉语言导航方法通常依赖全景图像、深度输入或里程计来支持空间推理和动作规划，这些要求增加了系统成本和集成复杂度，阻碍了轻量化无人机的实际部署。因此需要开发仅使用单目RGB视觉和自然语言指令的轻量级解决方案。

Method: 1. 将导航建模为下一个令牌预测问题；2. 通过提示引导的多任务学习联合优化空间感知、轨迹推理和动作预测；3. 提出关键帧选择策略以减少视觉冗余；4. 引入动作合并和标签重加权机制来缓解长尾监督不平衡并促进稳定的多任务协同训练。

Result: 在Aerial VLN基准测试中，该方法在具有挑战性的单目RGB-only设置下，在可见和未见环境中均取得了强劲结果。显著优于现有的RGB-only基线方法，并缩小了与最先进的全景RGB-D对应方法的性能差距。

Conclusion: 该研究提出的统一框架证明了仅使用单目RGB视觉和自然语言指令实现高效空中视觉语言导航的可行性，为轻量化无人机的实际部署提供了有前景的解决方案，并通过综合消融研究验证了任务设计和架构选择的有效性。
...
TL;DR: CoIG框架将图像生成重构为类似人类艺术创作的顺序语义过程，通过LLM分解复杂提示为逐步指令，增强生成过程的可监控性和可控性。


<details>
  <summary>Details</summary>
Motivation: 当前图像生成模型内部过程不透明，限制了人类观察和干预能力，影响模型可靠性、安全性和控制性，且其非人类化工作流程难以解释。

Method: CoIG框架使用LLM将复杂提示分解为简单逐步指令，图像生成模型按计划逐步生成和编辑图像，每个步骤专注于单一语义实体。

Result: CoIG显著增强了定量可监控性，在组合鲁棒性方面与基线模型竞争，并提出了CoIG可读性和因果相关性两个新评估指标。

Conclusion: CoIG框架通过将图像生成重构为顺序语义过程，实现了类似CoT在LLM中的优势，提高了生成过程的可观察性和可控性，且模型无关可集成到任何图像生成模型中。
...
<details>
  <summary>Details</summary>
Motivation: 车载摄像头需要实时边缘推理，但轻量级模型难以捕捉细粒度行为特征，ROI方法计算成本高，需要在效率和准确性之间找到平衡。

Method: 提出C-DIRA框架，结合显著性驱动的Top-K ROI池化和融合分类进行局部特征提取，动态ROI路由仅对高难度样本应用ROI推理，使用伪域标注和对抗学习学习域不变特征。

Result: 在State Farm分心驾驶员检测数据集上，C-DIRA在显著减少FLOPs和降低延迟的同时保持高精度，在模糊和低光等视觉退化条件下表现稳健，在未见域上性能稳定。
...
TL;DR: 提出Mname框架，通过内存银行机制提供大量负样本，实现去噪生成模型的自监督表示学习，无需外部编码器，在ImageNet-256上达到SOTA的FID 2.40。
...
Result: 在ImageNet-256上，Mname在40万步内达到了2.40的FID分数，显著优于可比方法，实现了更快的收敛速度和更优的生成质量。

Conclusion: Mname框架通过自包含的内存银行机制解决了去噪生成模型对预训练编码器的依赖问题，实现了高效的自监督表示学习，在推理时不增加额外参数或计算成本，显著提升了训练效率和生成质量。
...
Method: 1. 实施掩码融合策略，结合SAM 3的语义分割头和Transformer解码器（实例头）输出，利用两个头的优势改善土地覆盖识别；2. 使用存在头中的存在分数过滤场景中不存在的类别，减少地理空间场景中因词汇量大和补丁级处理导致的误报。

Result: 在广泛的遥感数据集上进行评估，实验表明这种简单适配取得了有前景的性能，证明了SAM 3在遥感开放词汇语义分割任务中的潜力。

Conclusion: SAM 3在无需训练的情况下，通过掩码融合和存在分数过滤的简单适配，在遥感开放词汇语义分割任务中展现出良好潜力，为遥感场景中的密集小目标分割提供了新思路。
...
TL;DR: 该研究提出了一个基于分布的个体公平性评估框架，将肤色作为连续属性而非分类标签，使用核密度估计建模肤色分布，比较12种统计距离度量，并提出基于距离的重新加权损失函数来纠正少数肤色的代表性不足。


<details>
  <summary>Details</summary>
Motivation: 肤色历来是歧视的焦点，但医学影像中的机器学习公平性研究通常依赖于粗粒度的子组分类，忽视了个体层面的差异。这种基于组的方法可能掩盖了子组内异常值面临的偏见。

Method: 将肤色视为连续属性而非分类标签，使用核密度估计(KDE)建模肤色分布，比较12种统计距离度量来量化肤色分布间的差异，并提出基于距离的重新加权(DRW)损失函数来纠正少数肤色的代表性不足。

Result: 在CNN和Transformer模型上的实验表明：(1)分类重新加权在捕捉个体层面差异方面存在局限性；(2)基于分布的重新加权方法表现更优，特别是使用Fidelity Similarity(FS)、Wasserstein Distance(WD)、Hellinger Metric(HM)和Harmonic Mean Similarity(HS)时。
...
TL;DR: 提出了一种结合3D渲染和约束扩散模型的工作流，用于自动生成高质量、逼真的蘑菇合成图像，无需大量真实标注数据即可训练出优秀的检测分割模型。


<details>
  <summary>Details</summary>
Motivation: 工业蘑菇种植依赖计算机视觉进行监控和自动化收获，但开发准确的检测和分割模型需要大量精确标注的数据集，成本高昂。合成数据虽然可扩展，但往往缺乏足够的真实感，难以泛化到真实场景。

Method: 提出了一种新颖的工作流，将Blender中的3D渲染与约束扩散模型相结合，自动生成高质量、逼真的双孢蘑菇合成图像。该方法保持对3D场景配置和标注的完全控制，同时实现逼真效果，无需专门的计算机图形学专业知识。

Result: 发布了两个合成数据集（每个包含6,000张图像，描绘超过25万个蘑菇实例），并在零样本设置下评估了基于这些数据集训练的Mask R-CNN模型。在两个独立的真实世界数据集（包括一个新收集的基准数据集）上测试时，该方法实现了最先进的分割性能（在M18K数据集上F1=0.859），尽管仅使用合成训练数据。
...
<details>
  <summary>Details</summary>
Motivation: 高性能计算机视觉模型在医学影像中表现出色，但计算密集、模型庞大，不适合边缘设备部署；隐私限制阻碍集中式数据管理，需要联邦学习解决方案

Method: 提出偏度指导的剪枝方法，基于输出分布的统计偏度，选择性地剪枝多模态Swin Transformer的多头自注意力和多层感知机层

Result: 在水平联邦学习环境中验证，紧凑型Swin Transformer实现约36%模型大小缩减，且准确率无损失
...
TL;DR: Wan-Move是一个简单可扩展的视频生成运动控制框架，通过密集点轨迹实现精细运动控制，无需修改基础模型架构，在5秒480p视频生成中达到商业级运动控制质量。
...
Method: 使用密集点轨迹表示物体运动，实现场景的细粒度控制；将轨迹投影到潜在空间，沿轨迹传播第一帧特征，生成对齐的时空特征图作为更新的潜在条件；该特征图作为运动指导自然集成到现成的图像到视频模型中，无需架构修改。

Result: 通过规模化训练，Wan-Move生成5秒480p视频，其运动可控性在用户研究中与Kling 1.5 Pro的商业Motion Brush相当；在MoveBench基准测试和公开数据集上的实验一致显示其优越的运动质量。

Conclusion: Wan-Move框架实现了精确高质量的运动控制，无需辅助运动编码器且易于扩展基础模型，为视频生成提供了实用的运动控制解决方案，并建立了全面的评估基准MoveBench。
...
TL;DR: 提出自优化扩散模型框架，利用可解释AI技术检测图像缺陷，通过缺陷激活图在扩散过程中针对性优化，显著提升图像生成质量


<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像合成方面取得了显著成功，但处理伪影和不真实区域仍然是关键挑战。现有方法难以有效识别和修复这些缺陷，需要一种能够主动检测并改进图像质量的新框架。

Method: 提出自优化扩散框架，采用基于可解释AI的缺陷高亮器生成缺陷激活图(FAMs)。这些FAMs在正向过程中放大缺陷区域的噪声，在反向过程中聚焦这些区域进行优化，从而提升重建质量。

Result: 在各种基于扩散的模型上实现了高达27.3%的Fréchet起始距离改进，在多样化数据集上表现一致强劲。在不同任务（图像生成、文本到图像生成、修复）中都显示出稳健的有效性。
...
TL;DR: 本文提出MatteViT框架，结合空间和频域信息进行文档阴影去除，通过高频放大模块和连续亮度阴影蒙版来保护文本边缘等高频细节，在公开数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 文档阴影去除对数字化文档清晰度至关重要，阴影常常会模糊或扭曲文本边缘和线条等高频细节结构，需要有效去除阴影的同时保护这些精细结构。

Method: 提出MatteViT框架，采用两种保护策略：1) 轻量级高频放大模块(HFAM)分解并自适应放大高频分量；2) 基于连续亮度的阴影蒙版，使用定制数据集和蒙版生成器提供精确空间引导。

Result: 在RDD和Kligler等公开基准测试中达到最先进性能，为实际文档阴影去除提供稳健实用解决方案，在OCR等下游任务中更好地保护文本级细节，提升识别性能。

Conclusion: MatteViT通过空间和频域信息的结合，有效去除文档阴影同时保护高频细节，在文档阴影去除任务中表现出色，对实际应用具有重要价值。
...
TL;DR: 提出了一种无需训练的双曲适配器方法T-DHA，用于提升视觉语言模型在领域变化下的性能，通过双曲空间建模层次结构关系，在少样本识别和领域泛化任务中表现优异。
...
Method: 提出训练免费的双曲适配器(T-DHA)，在双曲空间而非欧氏空间中建模视觉-语言关系的层次树结构。利用庞加莱球模型嵌入层次数据结构，结合负学习，以更少的特征维度实现更准确和鲁棒的分类。

Result: 在多个数据集上的实验结果表明，T-DHA方法在少样本图像识别和领域泛化任务中显著优于现有的最先进方法。

Conclusion: 双曲空间建模视觉-语言层次关系是一种有效的适应策略，T-DHA方法无需训练即可提升模型在跨域任务中的性能，为视觉语言模型的领域适应提供了新思路。
...
TL;DR: InfiniteVL是一种线性复杂度的视觉语言模型架构，结合滑动窗口注意力与Gated DeltaNet，在有限资源下实现高性能多模态处理，推理速度比传统Transformer快3.6倍以上。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型面临两个主要问题：基于窗口的VLM在序列长度超过窗口大小时性能下降，而线性注意力在信息密集型任务（如OCR和文档理解）上表现不佳。需要一种既能保持线性复杂度又能处理长序列的解决方案。

Method: 提出InfiniteVL架构，将滑动窗口注意力与Gated DeltaNet相结合，采用三阶段训练策略：蒸馏预训练、指令微调和长序列监督微调。

Result: 使用不到领先VLM所需训练数据的2%，InfiniteVL不仅显著优于之前的线性复杂度VLM，还能匹配领先的基于Transformer的VLM性能，同时保持有效的长期记忆。相比使用FlashAttention-2加速的类似大小Transformer VLM，推理速度提升超过3.6倍，延迟和内存占用保持恒定。在流式视频理解场景中，能维持稳定的24 FPS实时预填充速度。

Conclusion: InfiniteVL成功解决了现有VLM架构在长序列处理和信息密集型任务上的局限性，提供了一种高效、可扩展的视觉语言模型解决方案，在保持线性复杂度的同时实现了与Transformer相当的性能。
...
TL;DR: 该研究探讨生成式方法与非生成式方法在视觉感知中的组合泛化能力，理论证明生成式方法通过约束解码器并求逆能实现组合泛化，而非生成式方法难以实现。


<details>
  <summary>Details</summary>
Motivation: 人类视觉感知被认为需要生成式方法，但当前最成功的视觉模型都是非生成式的编码器模型。这引发了一个问题：生成是否真的是机器实现人类水平视觉感知的必要条件？研究旨在探讨生成式和非生成式方法能否实现组合泛化这一人类感知的标志性能力。

Method: 在组合数据生成过程下，形式化分析了基于解码器（生成式）和基于编码器（非生成式）方法实现组合泛化所需的归纳偏置。理论上证明了对编码器施加这些归纳偏置通常不可行，而生成式方法可以通过约束解码器并求逆来直接实施这些偏置。研究还展示了如何通过梯度搜索（在线）或生成回放（离线）高效执行这种求逆操作。

Result: 理论分析表明，没有必要的归纳偏置时，非生成式方法通常无法实现组合泛化，需要大规模预训练或额外监督来改善泛化能力。相比之下，生成式方法通过利用解码器的合适归纳偏置以及搜索和回放技术，能在不需要额外数据的情况下显著提升组合泛化能力。实证研究在真实感图像数据集上训练多种方法验证了这一理论。

Conclusion: 生成式方法在实现组合泛化方面具有理论优势，能够通过约束解码器并求逆来实施必要的归纳偏置，而非生成式方法在这方面存在根本性困难。这支持了生成式方法在实现人类水平视觉感知中的重要性。
...
Method: 通过微调预训练的视觉语言模型，结合专用定位模块和特殊控制令牌，实现语言和空间信息的联合推理，提升复杂卫星场景中的物体定位能力。
...
TL;DR: 提出VALOR框架，通过AI验证器（LLM和VLM）无需标注训练，提升视觉推理和视觉定位能力，超越开源和专有模型


<details>
  <summary>Details</summary>
Motivation: 现有视觉推理方法存在局限：语言链式思维方法需要大规模监督数据，程序合成方法存在逻辑缺陷和定位错误。需要无需标注的训练框架来同时改进推理和定位能力

Method: 使用AI验证器框架：LLM验证器通过强化学习优化推理，VLM验证器通过自动硬负例挖掘增强视觉定位，无需真实标签。结合语言模型分解空间查询和视觉专家模型改进

Result: 在多种空间推理任务上评估，方法改进视觉推理并超越开源和专有模型，改进的视觉定位模型进一步优于近期纯文本视觉推理方法

Conclusion: VALOR框架通过AI验证器实现无需标注训练，有效提升视觉推理和视觉定位性能，展示了现代AI系统优势结合的实际应用价值
...
TL;DR: UniLayDiff：首个统一的扩散Transformer模型，能够用单一端到端可训练模型处理各种内容感知布局生成任务，包括无条件生成和多种条件约束生成。


<details>
  <summary>Details</summary>
Motivation: 内容感知布局生成是图形设计自动化的关键任务，但现实应用的多样性使得开发单一模型统一处理各种输入约束生成子任务（如元素类型、大小或关系约束）极具挑战。现有方法要么只能处理部分任务，要么需要为不同条件使用单独的模型参数，缺乏真正的统一解决方案。

Method: 提出UniLayDiff：统一扩散Transformer模型。将布局约束视为独立模态，采用多模态扩散Transformer框架捕捉背景图像、布局元素和多样化约束之间的复杂交互。通过LoRA在预训练后对模型进行微调来整合关系约束。

Result: 大量实验表明，UniLayDiff在从无条件到各种条件生成任务中均达到最先进性能，并且据我们所知，是首个能够统一完整范围内容感知布局生成任务的模型。

Conclusion: UniLayDiff首次实现了用单一端到端可训练模型统一处理各种内容感知布局生成任务，不仅实现了统一的条件生成，还提升了整体布局质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像到3D生成器虽然能从单视图恢复合理的几何结构，但由于其面向对象的训练方式，难以泛化到复杂的大规模场景，无法保证准确的结构和纹理重建。

Method: 通过三个迭代阶段：空间先验初始化、视觉引导的3D场景网格生成、空间引导的新视图生成，在2D和3D域之间交替，逐步改进结构和外观。

Result: 在多样化场景上的实验表明，EvoScene相比强基线方法，在几何稳定性、视图一致的纹理和未见过区域补全方面表现更优，能够生成可直接用于实际应用的3D网格。
...
TL;DR: D4RT是一个简单而强大的前馈模型，使用统一的Transformer架构从单视频联合推断深度、时空对应关系和完整相机参数，通过新颖的查询机制实现轻量高效训练和推理。
...
Result: 该方法在广泛的4D重建任务中超越了先前方法，达到了新的state-of-the-art水平，实现了轻量化和高度可扩展的训练与推理。

Conclusion: D4RT通过创新的查询机制和统一架构，为动态场景的4D重建提供了一种高效、轻量且可扩展的解决方案，在多个任务上表现出色。
...
### [86] [A Comparative Study of EMG- and IMU-based Gesture Recognition at the Wrist and Forearm](https://arxiv.org/abs/2512.07997)
*Soroush Baghernezhad,Elaheh Mohammadreza,Vinicius Prado da Fonseca,Ting Zou,Xianta Jiang*

Main category: cs.HC

TL;DR: 该研究探索使用惯性测量单元（IMU）信号从不同肌肉群捕获用户意图，发现IMU信号包含足够信息用于静态手势识别，肌腱诱导的微运动是主要贡献因素。


<details>
  <summary>Details</summary>
Motivation: 手势识别通常使用表面肌电图（sEMG），但较少探索的惯性测量单元（IMU）可以提供关于细微肌肉运动的补充信息，这对于手势识别很有价值。研究旨在探索使用不同肌肉群的IMU信号来捕获用户意图。

Method: 研究使用IMU信号作为输入传感器，从不同肌肉群采集数据，分析肌腱诱导的微运动对静态手势识别的贡献，并比较不同肌肉群的模式识别质量。

Result: IMU信号包含足够信息作为静态手势识别的唯一输入传感器，肌腱诱导的微运动是静态手势识别的主要贡献因素，不同肌肉群在模式识别质量上存在差异。

Conclusion: 利用肌肉微运动信息可以增强假肢手臂对截肢者的可用性，并为机器人、远程操作、手语解释等领域的手势识别提供新的可能性。
...
### [87] ["Your Privacy is Your Responsibility": Understanding How Users Collectively Navigate the Complexity of Privacy on Quora](https://arxiv.org/abs/2512.08025)
*Varun Shiri,Maggie Xiong,Jin L. C. Guo,Jinghui Cheng*

Main category: cs.HC

TL;DR: 该研究通过分析Quora上用户讨论的隐私话题，探索自下而上的集体隐私保护方法，识别了用户关注的隐私风险、保护策略、组织实践和法律法规等相互关联的话题。


<details>
  <summary>Details</summary>
Motivation: 当前技术环境中，用户在隐私保护方面处于弱势地位。以往隐私保护工作主要关注自上而下的监管和技术设计方法，缺乏对如何通过自下而上的集体方法赋权用户的理解。

Method: 通过分析Quora平台上用户讨论的隐私相关话题，识别用户自发提出的隐私关注点和讨论模式。

Result: 识别了用户讨论的广泛且相互关联的隐私话题，包括隐私风险与危险、保护策略、组织实践以及现有法律法规。结果突出了影响用户隐私态度的个人、技术、组织和社会因素之间的相互作用。

Conclusion: 研究为设计社区工具提供了启示，以更好地支持用户的集体隐私保护努力。这些工具应整合用户多样化的隐私相关行为和偏好，简化信息访问和共享，并将设计者、开发者与用户社区连接起来。
...
### [88] [Joint Activity Design Heuristics for Enhancing Human-Machine Collaboration](https://arxiv.org/abs/2512.08036)
*Mohammadreza Jalaeian,Dane A. Morey,Michael F. Rayo*

Main category: cs.HC

TL;DR: 论文提出了支持人机联合活动的设计框架，重点关注五个宏观认知功能和14条启发式设计原则，以促进人机协作的有效协调。


<details>
  <summary>Details</summary>
Motivation: 当前技术设计主要关注可用性，但缺乏对人机联合活动中协调互依性的专门支持。需要从单纯设计可用技术转向设计能够作为有效团队成员的技术，以支持复杂的人机协作任务。

Method: 从显示设计、人因工程、认知系统工程、认知心理学和计算机科学等相关文献中综合提炼出14条启发式设计原则，用于指导支持人机联合活动的技术设计、开发和评估。

Result: 提出了一个包含五个核心宏观认知功能（事件检测、意义建构、适应性、视角转换和协调）的设计框架，并提供了14条具体的启发式设计原则来支持这些功能。

Conclusion: 支持联合活动的设计需要超越传统的可用性考虑，重点关注团队层面的宏观认知功能。提出的启发式原则为设计能够有效支持人机协作的技术提供了实用指导，这对于复杂的人机系统设计至关重要。
...
### [89] [Mediating Personal Relationships with Robotic Pets for Fostering Human-Human Interaction of Older Adults](https://arxiv.org/abs/2512.08426)
*Delong Du,Sara Gilda Amirhajlou,Akwasi Gyabaah,Richard Paluch,Claudia Müller*

Main category: cs.HC

TL;DR: 研究探索如何设计机器人宠物来促进老年人的人际关系，而非仅关注人机关系


<details>
  <summary>Details</summary>
Motivation: 良好人际关系对幸福生活和心理健康至关重要，现有研究多关注老年人与机器人宠物的直接关系，而忽视了机器人宠物如何促进人与人之间的关系

Method: 采用民族志研究方法，通过对六位老年人进行半结构化访谈，并进行主题分析

Result: 研究发现机器人宠物可以设计为远程机器人，帮助老年人与他人远程连接，为促进人际关系提供新途径
...
### [90] [Time and Money Matters for Sustainability: Insights on User Preferences on Renewable Energy for Electric Vehicle Charging Stations](https://arxiv.org/abs/2512.08437)
*Delong Du,Apostolos Vavouris,Omid Veisi,Lu Jin,Gunnar Stevens,Lina Stankovic,Vladimir Stankovic,Alexander Boden*

Main category: cs.HC

TL;DR: 研究探索将动态可再生能源可用性整合到充电站导航中，如何影响电动车用户选择可再生能源充电站的行为模式。


<details>
  <summary>Details</summary>
Motivation: 电动车使用可再生能源充电可减少环境影响，但可再生能源的波动性影响公共充电站的可持续性。不同充电站因微电网连接而使用不同能源组合（从纯可再生能源到非可再生能源），短距离内能源供应类型存在显著差异。

Method: 采用组内设计调查了50名汽车用户，并对来自农村、郊区和城市地区的10名电动车用户进行了半结构化访谈。

Result: 结果显示：选择充电站时，驾驶员通常根据驾驶场景影响其消费价值，优先考虑节省时间或节省金钱。电动车用户倾向于选择可再生能源充电站，当这些站点与其主要优先事项（省钱或省时）一致时。

Conclusion: 研究为导航推荐系统的前端图形用户界面和后端排名算法开发提供了终端用户洞察，这些系统整合了动态可再生能源可用性，以实现电动车的可持续使用。
...
### [91] [Exploring the Grassroots Understanding and Practices of Collective Memory Co-Contribution in a University Community](https://arxiv.org/abs/2512.08787)
*Zeyu Huang,Xinyi Cao,Yue Deng,Junze Li,Kangyu Yuan,Xiaojuan Ma*

Main category: cs.HC

TL;DR: 该研究从成员层面探讨集体记忆的构建，通过对比定位叙事系统和在线论坛，分析集体记忆形成中的解释与互动模式，识别出"回顾性思考"与"未来性记录"的核心辩论，并提取相关设计考虑。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要从整体社会学视角分析成熟的集体记忆，较少关注成员层面的集体记忆概念化以及共同贡献实践的具体模式。本研究旨在填补这一空白，从成员视角探究集体记忆的解释和互动模式。

Method: 采用混合方法田野研究，设计两个移动系统作为观察锚点：1)基于文献设计框架的定位叙事探索系统；2)代表当前实践的常规在线论坛。在校园环境中进行为期两周的研究（n=38）。

Result: 识别出核心辩论：是回顾性思考当下作为历史，还是为未来记录当下。这一辩论影响了叙事焦点、对集体记忆构成的期望，以及参与者从群体中寻求灵感的方式。提取了能更好包容集体记忆多样化概念化并连接不同社区成员的设计考虑。

Conclusion: 通过反思设计，为社区驱动的用户生成内容平台提供了专门定位叙事体验的额外见解。研究强调了在集体记忆构建中需要平衡回顾性与前瞻性视角，并提出了连接不同社区成员的设计策略。
...
### [92] ["Nothing about us without us": Perspectives of Global Deaf and Hard-of-hearing Community Members on Sign Language Technologies](https://arxiv.org/abs/2512.08839)
*Katherine Atwell,Saki Imai,Danielle Bragg,Malihe Alikhani*

Main category: cs.HC

TL;DR: 全球多语言调查显示，虽然聋人和重听人士认可手语技术的潜力，但对其文化侵蚀、翻译不准确和听力主导的研究流程表示担忧，强调聋人主导设计的重要性。


<details>
  <summary>Details</summary>
Motivation: 手语技术发展迅速，但聋人和重听人士的视角在技术开发中被边缘化，特别是在西方以外和全球南方地区。本研究旨在通过全球调查了解社区对手语技术的真实看法。

Method: 采用全球多语言调查方法，覆盖多个国家、手语和文化背景，收集社区对手语技术的观点和意见。

Result: 参与者认可手语技术在支持无障碍和独立性方面的潜力，但普遍担忧文化侵蚀、翻译不准确、听力主导的研究流程等问题。手语熟练度、政策接触度和聋人身份认同等因素影响对技术的看法。各地区参与者都强调聋人主导设计的重要性。

Conclusion: 研究提供了跨大洲、社区知情的手语技术分析，并为研究人员、技术开发者和政策制定者提出了可操作的建议，强调将聋人社区纳入技术决策过程的必要性。
...
### [93] [Sparse Variable Projection in Robotic Perception: Exploiting Separable Structure for Efficient Nonlinear Optimization](https://arxiv.org/abs/2512.07969)
*Alan Papalia,Nikolas Sanderson,Haoyu Han,Heng Yang,Hanumant Singh,Michael Everett*

Main category: cs.RO

TL;DR: 本文提出了一种针对具有规范对称性的机器人感知问题的变量投影方法，通过联合利用可分离性和稀疏性，构建矩阵无关的Schur补算子，在SLAM、SNL和SfM等任务中实现了2-35倍的加速。


<details>
  <summary>Details</summary>
Motivation: 机器人感知中的大规模非线性最小二乘问题通常具有可分离性结构（某些变量线性出现），但现有的变量投影方法在处理规范对称性（如全局平移和旋转不变性）时面临计算挑战，限制了其在机器人感知中的应用。

Method: 提出了一种专门针对具有规范对称性问题的变量投影方案，通过一次性预处理构建矩阵无关的Schur补算子，该算子能够高效计算简化问题的成本、梯度和Hessian-向量积，并与标准迭代NLS求解器集成。

Result: 在SLAM、SNL和SfM的合成和真实基准测试中，该方法相比最先进方法实现了2-35倍的运行时间加速，同时保持了准确性。

Conclusion: 该方法成功解决了变量投影在机器人感知中应用的主要障碍，通过联合利用可分离性和稀疏性，为具有规范对称性的大规模非线性最小二乘问题提供了高效解决方案，并发布了开源C++实现和实验数据集。
...
### [94] [VLD: Visual Language Goal Distance for Reinforcement Learning Navigation](https://arxiv.org/abs/2512.07976)
*Lazar Milikic,Manthan Patel,Jonas Frey*

Main category: cs.RO

TL;DR: 提出Vision-Language Distance (VLD)学习框架，通过解耦感知学习和策略学习来解决机器人导航中的sim-to-real差距和数据不足问题。先在互联网规模视频数据上训练自监督的距离预测器，然后在仿真中用强化学习训练策略，最后部署时策略使用VLD预测的距离信号。


<details>
  <summary>Details</summary>
Motivation: 现有从图像数据端到端训练机器人导航策略的方法面临两个主要问题：1) 策略迁移时的sim-to-real差距；2) 带动作标签的训练数据有限。需要一种更可扩展的方法来学习可靠的多模态导航策略。

Method: 提出VLD学习框架，分两个阶段：1) 在互联网规模视频数据上训练自监督的距离到目标预测器，支持图像和文本目标；2) 在仿真环境中使用特权几何距离信号训练强化学习策略，注入噪声模拟距离预测器的不确定性。部署时策略使用VLD预测的距离信号。

Result: VLD在距离函数评估上优于之前的时序距离方法（如ViNT和VIP）。实验表明解耦设计在仿真中实现了有竞争力的导航性能，同时支持灵活的目标模态，为可靠的多模态导航策略提供了可扩展的路径。

Conclusion: VLD框架通过解耦感知学习和策略学习，结合大规模视觉训练和仿真中的鲁棒导航行为学习，为解决机器人导航中的sim-to-real差距和数据不足问题提供了一种可扩展的替代方案。
...
### [95] [DIJIT: A Robotic Head for an Active Observer](https://arxiv.org/abs/2512.07998)
*Mostafa Kamali Tabrizi,Mingshi Chi,Bir Bikram Dey,Yu Qing Yuan,Markus D. Solbach,Yiqian Liu,Michael Jenkin,John K. Tsotsos*

Main category: cs.RO

TL;DR: DIJIT是一个专为移动智能体设计的双目机器人头部系统，具有9个机械自由度和4个光学自由度，能够模拟人类眼部和头颈运动，用于主动视觉研究和人机视觉对比研究。


<details>
  <summary>Details</summary>
Motivation: 设计一个能够模拟人类眼部和头颈运动的机器人头部系统，用于研究主动视觉、人类视觉机制以及人机视觉方法的差异，促进主动视觉研究的发展。

Method: 设计了具有9个机械自由度和4个光学自由度的双目机器人头部系统，实现了人类水平的运动范围和速度，支持会聚立体视觉所需的运动范围（会聚、共轭、旋转）。开发了新的扫视相机运动方法，建立了相机方向与电机值之间的直接关系。

Result: DIJIT系统能够实现接近人类运动准确度的扫视相机运动，其机械设计参数与人类性能相当，能够有效模拟人类眼部和头颈的协调运动。

Conclusion: DIJIT是一个功能全面的机器人头部平台，能够支持主动视觉研究、人类视觉机制探索以及人机视觉对比研究，为理解人类视觉系统和改进计算机视觉方法提供了重要工具。
...
### [96] [Optimized Area Coverage in Disaster Response Utilizing Autonomous UAV Swarm Formations](https://arxiv.org/abs/2512.08028)
...
TL;DR: 本文提出了一种用于灾害救援的无人机集群系统，通过分布式传感器延长飞行时间，采用局部ESDF地图进行避障，结合TSP变体优化区域覆盖，优先扫描重要兴趣点。


<details>
  <summary>Details</summary>
Motivation: 在野火等灾害场景中，第一响应者需要高效、安全的无人机集群系统来收集关键信息。传统单无人机系统存在飞行时间有限、碰撞风险高、数据可用性不足等问题，需要开发能够延长任务时间、确保安全并优化覆盖的集群解决方案。

Method: 1. 采用分布式传感器架构，将传感器分布在多个无人机上以延长整体飞行时间；2. 开发自主导航框架，使用局部欧几里得符号距离场（ESDF）地图进行障碍物避让；3. 设计集群编队保持机制，最小化路径偏差；4. 引入旅行商问题（TSP）变体优化区域覆盖，基于环境行为和关键基础设施预分配值优先扫描兴趣点（POIs）。

Result: 通过不同集群规模的模拟验证，系统能够最大化区域覆盖，同时确保无人机之间以及无人机与障碍物之间的碰撞避免。系统在延长飞行时间、提高数据可用性和降低任务失败风险方面表现出色。

Conclusion: 提出的无人机集群系统为灾害救援场景提供了有效的解决方案，通过分布式传感器、ESDF避障和TSP优化覆盖等技术，实现了安全、高效的区域监测，有望显著提升第一响应者的灾害应对能力。
...
### [97] [An Introduction to Deep Reinforcement and Imitation Learning](https://arxiv.org/abs/2512.08052)
*Pedro Santana*

Main category: cs.RO

TL;DR: 本文介绍了深度强化学习(DRL)和深度模仿学习(DIL)在具身智能体控制中的应用，采用深度优先的方法讲解基础算法，涵盖从马尔可夫决策过程到PPO和GAIL等关键技术。
...
Method: 采用深度优先的文献分析方法，聚焦于一小套基础算法和技术，包括：DRL方面的马尔可夫决策过程、REINFORCE、近端策略优化(PPO)；DIL方面的行为克隆、数据集聚合(DAgger)、生成对抗模仿学习(GAIL)。

Result: 本文提供了自包含的数学和机器学习概念介绍，不是该领域的全面综述，而是专注于深入理解核心算法。

Conclusion: DRL利用奖励信号优化行为，DIL使用专家演示指导学习，这两种方法为具身智能体的控制器设计提供了有效的学习框架，本文通过深度优先的方式系统介绍了这些基础技术。
...
### [98] [Chat with UAV -- Human-UAV Interaction Based on Large Language Models](https://arxiv.org/abs/2512.08145)
*Haoran Wang,Zhuohang Chen,Guang Li,Bo Ma,Chuanghuang Li*

Main category: cs.RO

TL;DR: 本文提出了一种基于大语言模型的双智能体人机交互框架，通过任务规划智能体和执行智能体的分工协作，解决了传统无人机交互系统中混合任务规划与执行的困难，提升了交互流畅性和任务执行灵活性。


<details>
  <summary>Details</summary>
Motivation: 无人机交互系统正从工程师驱动转向用户驱动，但缺乏用户与无人机之间的共同语言阻碍了这种转变。虽然大语言模型为个性化人机交互提供了可能，但现有基于LLM的框架在混合任务规划与执行方面存在困难，难以适应复杂场景。

Method: 提出双智能体HUI框架，构建两个独立的LLM智能体（任务规划智能体和执行智能体），应用不同的提示工程技术分别处理任务的理解、规划和执行。建立了覆盖四个典型无人机应用场景的任务数据库，并使用三种独立指标量化框架性能。

Result: 用户研究实验结果表明，该框架在设定的任务场景中提高了HUI的流畅性和任务执行的灵活性，有效满足了用户的个性化需求。同时比较了不同LLM模型控制无人机的性能表现。

Conclusion: 提出的双智能体框架通过智能体分工和专门的提示工程，解决了现有LLM-based HUI框架在混合任务规划与执行方面的困难，为个性化人机交互提供了有效解决方案，在农业、航拍、物流、环境监测等领域具有应用潜力。
...
### [99] [RAVES-Calib: Robust, Accurate and Versatile Extrinsic Self Calibration Using Optimal Geometric Features](https://arxiv.org/abs/2512.08170)
*Haoxin Zhang,Shuaixin Li,Xiaozhou Zhu,Hongbo Chen,Wen Yao*

Main category: cs.RO

TL;DR: 提出一个无需标定板、兼容多种LiDAR和相机传感器的标定工具包，只需单对激光点和相机图像即可实现鲁棒的标定，通过特征分布分析和自适应加权提升精度。


<details>
  <summary>Details</summary>
Motivation: 现有LiDAR-相机标定方法通常需要标定板或初始变换参数，在无目标环境和大位姿差异下鲁棒性不足，需要开发更通用、鲁棒的标定工具。

Method: 使用Gluestick流水线建立2D-3D点和线特征对应关系获取初始猜测；定量分析特征分布对标定结果的影响；基于特征质量指标自适应加权每个特征的代价；通过过滤劣质特征优化外参。

Result: 在室内外多种LiDAR-相机传感器上的实验表明，该方法相比SOTA技术具有更优的鲁棒性和精度，且代码已在GitHub开源。

Conclusion: 提出的无目标LiDAR-相机标定工具包兼容多种传感器，无需初始变换，在大位姿差异下仍保持鲁棒，通过特征分布分析和自适应加权实现了高精度标定。
...
### [100] [Ground Slow, Move Fast: A Dual-System Foundation Model for Generalizable Vision-and-Language Navigation](https://arxiv.org/abs/2512.08186)
...
TL;DR: DualVLN：首个双系统视觉语言导航基础模型，通过分离高层推理与低层执行，实现实时鲁棒导航


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言导航方法通常采用端到端管道，直接将视觉语言输入映射为短视距离散动作。这种设计导致运动碎片化、延迟高，且难以应对动态障碍物避让等现实挑战。

Method: 提出DualVLN双系统架构：System 2（基于VLM的全局规划器）通过图像基础推理预测中期航点目标；System 1（轻量级多模态条件扩散Transformer策略）利用System 2提供的显式像素目标和潜在特征生成平滑准确轨迹。通过解耦训练保持VLM泛化能力。

Result: DualVLN在所有VLN基准测试中超越先前方法，真实世界实验展示了在动态环境中鲁棒的长视距规划和实时适应能力。

Conclusion: 双系统设计实现了稳健的实时控制和复杂动态环境中的自适应局部决策，System 1实现了可解释且有效的局部导航。
...
### [101] [Multi-Task Bayesian Optimization for Tuning Decentralized Trajectory Generation in Multi-UAV Systems](https://arxiv.org/abs/2512.08630)
*Marta Manzoni,Alessandro Nazzari,Roberto Rubinacci,Marco Lovera*

Main category: cs.RO

TL;DR: 该研究使用多任务贝叶斯优化来调优多无人机系统中的分散式轨迹生成算法，将不同无人机交互场景视为独立任务，通过多任务高斯过程建模任务间关系，比较了优化平均任务时间和单独优化每个任务两种策略。


<details>
  <summary>Details</summary>
Motivation: 在多无人机系统中，分散式轨迹生成算法的参数调优是一个复杂问题，特别是当无人机数量和交互场景变化时。传统单任务优化方法需要为每个场景单独优化，计算成本高。研究旨在探索多任务贝叶斯优化方法，通过利用不同任务间的共享结构来提高优化效率。

Method: 采用多任务贝叶斯优化框架，将每个无人机交互场景视为一个独立任务。使用多任务高斯过程建模不同任务之间的关系，捕捉共享结构并实现优化过程中的信息传递。比较了两种优化策略：1）优化所有任务的平均任务时间；2）单独优化每个任务。通过全面的仿真实验验证方法有效性。

Result: 仿真结果表明：单任务优化在无人机群规模增大时能获得逐渐缩短的任务时间，但需要显著更多的优化时间；而平均任务优化方法在保持合理性能的同时大幅减少了优化时间。多任务高斯过程成功捕捉了任务间的共享结构，实现了高效的信息传递。

Conclusion: 多任务贝叶斯优化是调优多无人机系统分散式轨迹生成算法的有效方法。平均任务优化策略在优化效率和性能之间提供了良好平衡，特别适用于需要处理多种交互场景的实际应用。该方法为复杂多智能体系统的参数优化问题提供了新思路。
...
### [102] [Embodied Tree of Thoughts: Deliberate Manipulation Planning with Embodied World Model](https://arxiv.org/abs/2512.08188)
*Wenjiang Xu,Cindy Wang,Rui Fang,Mingkang Zhang,Lusong Li,Jing Xu,Jiayuan Gu,Zecui Zeng,Rui Chen*

Main category: cs.RO

TL;DR: EToT提出了一种基于物理模拟器的Real2Sim2Real规划框架，通过先验分支和反思分支机制在数字孪生中生成和优化机器人操作计划，解决了传统视频生成模型缺乏物理基础的问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于视频生成的世界模型在机器人操作规划中缺乏严格的物理基础，容易产生幻觉且无法保持长期物理约束的一致性，需要更可靠的物理基础规划方法。

Method: 提出Embodied Tree of Thoughts (EToT)框架，使用基于物理的交互式数字孪生作为具身世界模型，通过先验分支（基于语义和空间分析生成候选执行路径）和反思分支（利用视觉语言模型诊断模拟器中的执行失败并迭代优化规划树）两种机制进行树搜索规划。

Result: 在短期和长期操作任务套件上验证，EToT始终优于基线方法，能有效预测物理动态并适应潜在失败。

Conclusion: EToT通过将高级推理基于物理模拟器，确保生成的计划符合刚体动力学和碰撞约束，为机器人操作规划提供了物理可靠的解决方案。
...
### [103] [High-Performance Dual-Arm Task and Motion Planning for Tabletop Rearrangement](https://arxiv.org/abs/2512.08206)
*Duo Zhang,Junshan Huang,Jingjin Yu*

Main category: cs.RO

TL;DR: SDAR是一个用于桌面重排任务的TAMP框架，通过紧密集成的任务规划和运动规划模块，使双机械臂能够协作解决复杂、非单调的长时程重排任务，实现了100%的成功率。


<details>
  <summary>Details</summary>
Motivation: 解决双机械臂在近距离协作时面临的复杂桌面重排问题，特别是当起始和目标配置高度纠缠时，需要智能地筛选大量可能的任务和运动规划方案。

Method: SDAR包含两个核心组件：SDAR-T（依赖驱动的任务规划器）通过分解全局对象依赖图来生成优化的双臂任务规划；SDAR-M（同步双臂运动规划器）采用分层运动规划策略，利用GPU SIMD工具筛选最佳同步双臂运动规划。

Result: SDAR在解决复杂、非单调、长时程桌面重排任务中实现了100%的成功率，解决方案质量远超先前最先进方法，并在UR-5e机械臂上验证了直接可靠的硬件转移能力。

Conclusion: SDAR框架通过紧密集成的任务和运动规划，成功解决了复杂双机械臂桌面重排问题，展示了在硬件上的实际应用潜力，为机器人协作任务规划提供了有效解决方案。
...
### [104] [Semantic-Metric Bayesian Risk Fields: Learning Robot Safety from Human Videos with a VLM Prior](https://arxiv.org/abs/2512.08233)
*Timothy Chen,Marcus Dominguez-Kuhne,Aiden Swann,Xu Liu,Mac Schwager*

Main category: cs.RO

TL;DR: 提出基于贝叶斯框架的隐式人类风险模型提取方法，通过语义条件和空间变化的参数化风险表示，从人类安全演示视频和视觉语言模型常识中学习，生成像素级风险图像用于机器人规划


<details>
  <summary>Details</summary>
Motivation: 人类将安全视为连续、上下文相关和空间依赖的风险概念，而非二元信号。为了让自主系统能够内化类似人类的风险认知，需要提取人类隐式的风险模型

Method: 采用贝叶斯框架：先验由预训练视觉语言模型提供，似然函数通过学习ViT将预训练特征映射到像素对齐的风险值来调制先验。输入RGB图像和查询对象字符串，输出像素密集的风险图像

Result: 框架能够生成与人类偏好一致的情境化风险评估，可泛化到新对象和上下文，并支持快速适应额外观察或常识规则。在机器人规划任务中作为价值预测器或与经典轨迹优化算法结合应用

Conclusion: 提出的贝叶斯风险框架是使自主系统内化人类风险认知的重要一步，具有扩展到更大训练数据集的潜力，为机器人产生类人运动提供了有效方法
...
### [105] [SensHRPS: Sensing Comfortable Human-Robot Proxemics and Personal Space With Eye-Tracking](https://arxiv.org/abs/2512.08518)
*Nadezhda Kushina,Ko Watanabe,Aarthi Kannan,Ashita Ashok,Andreas Dengel,Karsten Berns*

Main category: cs.RO

TL;DR: 研究使用眼动追踪技术评估人与机器人互动中的舒适度，发现决策树模型在预测舒适度方面表现最佳，瞳孔直径是最关键预测因子，表明人机交互的舒适度阈值与人人交互不同。


<details>
  <summary>Details</summary>
Motivation: 社交机器人需要适应人类的空间距离规范以确保用户舒适度和参与度。虽然先前研究表明眼动追踪特征能可靠估计人人互动中的舒适度，但这些方法在与人形机器人互动中的适用性尚未探索。

Method: 使用机器人"Ameca"在四个实验控制距离（0.5米到2.0米）下研究用户舒适度，采用移动眼动追踪和主观报告（N=19）。评估多种机器学习和深度学习模型，基于凝视特征估计舒适度。

Result: 与先前人人互动研究中Transformer模型表现优异不同，决策树分类器取得了最高性能（F1分数=0.73），最小瞳孔直径被确定为最关键预测因子。

Conclusion: 这些发现表明人机交互中的生理舒适度阈值与人人互动动态不同，可以使用可解释的逻辑模型有效建模。
...
### [106] [Zero-Splat TeleAssist: A Zero-Shot Pose Estimation Framework for Semantic Teleoperation](https://arxiv.org/abs/2512.08271)
...
<details>
  <summary>Details</summary>
Motivation: 解决多机器人遥操作中需要实时获取全局位置和姿态信息的问题，避免使用基准标记或深度传感器

Method: 集成视觉语言分割、单目深度估计、加权主成分分析姿态提取和3D高斯泼溅技术

Result: 为每个操作员提供多个机器人的实时全局位置和姿态信息，支持交互为中心的遥操作设置

Conclusion: 提出了一种无需基准标记或深度传感器的零样本传感器融合方法，实现了基于普通CCTV流的多边遥操作
...
### [107] [Model-Based Diffusion Sampling for Predictive Control in Offline Decision Making](https://arxiv.org/abs/2512.08280)
...
<details>
  <summary>Details</summary>
Motivation: 现有生成方法在离线决策中经常产生动力学不可行的轨迹，需要一种能够同时保证任务对齐和动力学可行性的框架。

Method: 提出模型预测扩散器（MPDiffuser），包含三个模块：规划器生成多样化任务对齐轨迹；动力学模型确保与底层系统动力学一致；排序器选择符合任务目标的行为。采用交替扩散采样方案，在采样过程中逐步优化轨迹。

Result: 在D4RL和DSRL离线决策基准测试中表现优于现有方法，组合设计提高了样本效率，能够适应新动力学，并在真实四足机器人上验证了实用性。

Conclusion: MPDiffuser通过组合模型设计有效解决了离线决策中轨迹可行性问题，在基准测试和真实机器人控制中都展现了优越性能，并具有扩展到高维视觉控制任务的潜力。
...
### [108] [Robust Finetuning of Vision-Language-Action Robot Policies via Parameter Merging](https://arxiv.org/abs/2512.08333)
*Yajat Yadav,Zhiyuan Zhou,Andrew Wagenmaker,Karl Pertsch,Sergey Levine*

Main category: cs.RO

TL;DR: 提出一种简单的模型权重插值方法，在微调新任务时保留通用策略的泛化能力，实现单策略同时掌握新技能和保持原有广泛能力。
...
Method: 采用模型权重插值策略：将微调后的模型权重与预训练模型权重进行插值，产生一个单一模型继承基础模型的通用能力并稳健学习新任务。

Result: 模型合并方法在模拟和真实世界实验中表现优异，在新任务的分布外变体上优于预训练和微调模型，并能实现持续学习新技能而不牺牲原有能力。

Conclusion: 简单的模型权重插值策略能有效保留通用策略的泛化能力，使单策略既能学习新任务的泛化，又能保持预训练获得的广泛能力，支持持续学习。
...
### [109] [Learning Robot Manipulation from Audio World Models](https://arxiv.org/abs/2512.08405)
...
Result: 在两个需要感知真实世界音频或音乐信号的操作任务中，该系统表现出优于无前瞻方法的性能。
...
### [110] [A Multi-Agent LLM Framework for Design Space Exploration in Autonomous Driving Systems](https://arxiv.org/abs/2512.08476)
*Po-An Shih,Shao-Hua Wang,Yung-Che Li,Chia-Heng Tu,Chih-Han Chang*

Main category: cs.RO

TL;DR: 本文提出了一种基于多智能体大语言模型的设计空间探索框架，用于自动驾驶系统的自动化设计优化，通过集成多模态推理与3D仿真工具，相比传统遗传算法能发现更多帕累托最优解。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统设计需要在多样化环境条件下探索庞大的硬件/软件配置空间，传统设计空间探索方法难以处理多模态执行输出和复杂性能权衡，且需要人工干预评估正确性。

Method: 采用多智能体大语言模型框架，集成多模态推理与3D仿真和性能分析工具，利用专门的LLM智能体处理用户输入解释、设计点生成、执行编排以及视觉和文本执行输出分析。

Result: 在机器人出租车案例研究中，相比遗传算法基准，该框架在相同探索预算下识别出更多帕累托最优、成本效益更高的解决方案，并减少了导航时间，证明了LLM方法在设计空间探索中的效率。

Conclusion: 该框架为自动驾驶系统的设计自动化铺平了道路，通过自动化解释执行输出和指导系统设计探索，减少了人工干预需求。
...
### [111] [Prospect Theory in Physical Human-Robot Interaction: A Pilot Study of Probability Perception](https://arxiv.org/abs/2512.08481)
*Yixiang Lin,Tiancheng Yang,Jonathan Eden,Ying Tan*

Main category: cs.RO

TL;DR: 该研究通过物理耦合目标到达任务，探索人类在不确定性下的决策行为，发现存在两种不同的行为模式群体，为pHRI控制器设计提供实证依据。


<details>
  <summary>Details</summary>
Motivation: 传统pHRI控制框架基于最优控制理论，假设人类行为最小化成本函数，但人类在不确定性下的行为往往偏离这种最优模式。需要更多理解人类在不确定性下的行为，以设计安全有效的物理人机交互系统。

Method: 实施物理耦合目标到达任务，机器人以系统变化的概率（10%到90%）提供协助或干扰。分析参与者的力输入和决策策略。

Result: 发现两种不同的行为集群：1）"权衡"组根据干扰可能性调节物理响应；2）"总是补偿"组表现出强烈的风险规避，不考虑概率。这表明人类在pHRI中的决策高度个性化，概率感知可能与真实值不同。

Conclusion: 研究强调需要更可解释的行为模型（如累积前景理论）来准确捕捉这些行为，为未来自适应机器人控制器的设计提供信息。
...
### [112] [vEDGAR - Can CARLA Do HiL?](https://arxiv.org/abs/2512.08541)
*Nils Gehrke,David Brecht,Dominik Kulmer,Dheer Patel,Frank Diermeyer*

Main category: cs.RO

TL;DR: 本文开发了vEDGAR仿真框架，将CARLA模拟器扩展到硬件在环测试，实现了自动驾驶车辆完整传感器和执行器堆栈的实时仿真。


<details>
  <summary>Details</summary>
Motivation: CARLA等开源模拟器在自动驾驶算法训练和评估中广泛应用，但缺乏对完整传感器和执行器堆栈的实时硬件在环测试能力。为了在整个开发过程中提供一致的评估工具，需要扩展CARLA的测试能力。

Method: 首先推导需求并指定仿真架构，然后实现vEDGAR软件框架。该框架将CARLA模拟器与专用硬件集成，支持自动驾驶软件的硬件在环测试。

Result: 成功开发了vEDGAR仿真框架，并开源了修改的CARLA分支和vEDGAR框架。该工具能够测试自动驾驶软件在专用硬件上的性能并识别其限制。

Conclusion: vEDGAR框架扩展了CARLA的测试能力，使其适用于自动驾驶车辆的硬件在环测试，为整个开发过程提供了统一的评估工具，促进了开源自动驾驶功能开发工作流程。
...
### [113] [Bridging Scale Discrepancies in Robotic Control via Language-Based Action Representations](https://arxiv.org/abs/2512.08548)
*Yuchi Zhang,Churui Sun,Shiqi Liang,Diyuan Liu,Chao Ji,Wei-Nan Zhang,Ting Liu*

Main category: cs.RO

TL;DR: 提出一种基于语义的语言表示来归一化机器人动作，通过忽略数值尺度效应、强调方向性来缓解分布偏移，提升预训练表示的可泛化性


<details>
  <summary>Details</summary>
Motivation: 当前端到端机器人操作研究采用类似大语言模型的架构，但由于不同机器人平台和任务间动作命令的数值变化导致严重分布偏移，阻碍了预训练知识的有效迁移

Method: 提出语义基础的语言表示来归一化动作，该运动表示忽略数值尺度效应，强调方向性，从而缓解分布偏移，缩小动作标记与标准词汇标记之间的特征距离

Result: 在两个基准测试上的多任务实验表明，该方法显著提高了机器人操作任务的泛化性能和可迁移性

Conclusion: 通过语义基础的运动表示来归一化机器人动作，能够有效缓解分布偏移问题，提升预训练模型在机器人操作任务中的泛化能力和可迁移性
...
### [114] [RVC-NMPC: Nonlinear Model Predictive Control with Reciprocal Velocity Constraints for Mutual Collision Avoidance in Agile UAV Flight](https://arxiv.org/abs/2512.08574)
*Vit Kratky,Robert Penicka,Parakh M. Gupta,Ondrej Prochazka,Martin Saska*

Main category: cs.RO

TL;DR: 本文提出了一种基于非线性模型预测控制(NMPC)和时间相关互惠速度约束(RVCs)的无人机互避碰撞方法，无需过多通信，计算效率高，可实现100Hz实时控制，在挑战性场景中飞行时间减少31%。


<details>
  <summary>Details</summary>
Motivation: 现有无人机互避碰撞方法通常依赖大量通信，而本文旨在开发一种仅基于可观测信息的解决方案，减少通信需求，同时实现高频率实时控制，适用于敏捷无人机飞行。

Method: 采用非线性模型预测控制(NMPC)框架，结合时间相关互惠速度约束(RVCs)，设计高效算法计算RVCs并将其直接集成到控制器层面的NMPC问题中，支持100Hz实时处理。

Result: 在模拟10架无人机、速度达25m/s的真实场景中，以及加速度达30m/s²的实际实验中，所有试验均实现无碰撞导航，与现有技术相比，在挑战性场景中飞行时间减少31%。

Conclusion: 该方法通过NMPC与RVCs的结合，实现了仅基于可观测信息的高效无人机互避碰撞，无需过多通信，计算效率高，适用于敏捷无人机飞行，显著提升了飞行效率。
...
### [115] [Mind to Hand: Purposeful Robotic Control via Embodied Reasoning](https://arxiv.org/abs/2512.08580)
*Peijun Tang,Shangjin Xie,Binyan Sun,Baifu Huang,Kuncheng Luo,Haotian Yang,Weiqi Jin,Jianan Wang*

Main category: cs.RO

TL;DR: Lumo-1是一个统一的视觉-语言-动作模型，通过三阶段预训练将推理能力与物理动作结合，在机器人任务中表现出色


<details>
  <summary>Details</summary>
Motivation: 虽然互联网规模的数据使AI系统具备了广泛的推理能力，但将这些能力落实到物理动作中仍然是一个重大挑战。人类行动具有上下文和意图，推理在其中起着核心作用，因此需要将机器人的推理（"思维"）与动作（"手"）统一起来。

Method: 采用三阶段预训练流程：1）在精选的视觉-语言数据上继续预训练VLM，增强具身推理技能；2）在跨具身机器人数据和视觉-语言数据上进行协同训练；3）在Astribot S1双手机器人收集的轨迹上进行带推理过程的动作训练。最后整合强化学习来优化推理-动作一致性。

Result: Lumo-1在具身视觉-语言推理方面取得了显著性能提升，这是通用机器人控制的关键组成部分。真实世界评估显示，Lumo-1在广泛的挑战性机器人任务中超越了强基线，对新物体和环境具有强大的泛化能力，尤其在长时程任务和需要策略、概念和空间推理的人类自然指令响应方面表现出色。
...
### [116] [A Sensor-Aware Phenomenological Framework for Lidar Degradation Simulation and SLAM Robustness Evaluation](https://arxiv.org/abs/2512.08653)
*Doumegna Mawuto Koudjo Felix,Xianjia Yu,Zhuo Zou,Tomi Westerlund*

Main category: cs.RO

TL;DR: 该论文提出了一个传感器感知的激光雷达退化模拟框架，用于在真实点云上直接模拟可解释的激光雷达退化，实现对SLAM系统的可控、可重复的压力测试。


<details>
  <summary>Details</summary>
Motivation: 现有的激光雷达SLAM系统对遮挡、噪声和视场退化等恶劣条件高度敏感，但现有的鲁棒性评估方法要么缺乏物理基础，要么无法捕捉传感器特定的行为。

Method: 提出了一个传感器感知的现象学框架，在真实点云上直接模拟可解释的激光雷达退化，包括结构化丢失、视场减少、高斯噪声、遮挡掩码、稀疏化和运动失真等退化类型。该框架具有自主主题和传感器检测、模块化配置（四个严重级别）和实时性能（每帧小于20毫秒），兼容ROS工作流程。

Result: 在三种激光雷达架构和五种最先进的SLAM系统上进行实验验证，揭示了由传感器设计和环境上下文塑造的独特鲁棒性模式。开源实现为在物理意义退化场景下基准测试激光雷达SLAM提供了实用基础。

Conclusion: 该传感器感知的激光雷达退化模拟框架能够对SLAM系统进行可控、可重复的压力测试，揭示了不同传感器设计和环境条件下的鲁棒性差异，为激光雷达SLAM的鲁棒性评估提供了实用的基准测试工具。
...
### [117] [Sim2Swim: Zero-Shot Velocity Control for Agile AUV Maneuvering in 3 Minutes](https://arxiv.org/abs/2512.08656)
*Lauritz Rismark Fosso,Herman Biørn Amundsen,Marios Xanthidis,Sveinung Johan Ohrem*

Main category: cs.RO

TL;DR: 本文提出了Sim2Swim方法，首个基于零样本sim2real深度强化学习的AUV速度控制器，仅需3分钟训练即可实现6DOF敏捷机动和路径跟踪。


<details>
  <summary>Details</summary>
Motivation: 全向AUV具有硬件能力实现平移和旋转自由度的敏捷机动，但由于水下环境的复杂性（复杂流体静力学和动力学、参数不确定性、载荷变化导致的动态特性频繁变化），控制具有挑战性。现有方法通常需要针对特定平台配置精心调优控制器，且在不同载荷和流体条件下需要重新调优，导致6DOF敏捷机动在实践中很少使用。

Method: 提出Sim2Swim方法，受最先进的DRL位置控制启发，利用领域随机化和大规模并行训练，为不同特性的AUV收敛到可直接现场部署的控制策略，无需后处理或调优。该方法基于零样本sim2real深度强化学习，训练时间仅需3分钟。

Result: 在多种配置的池试验中进行了广泛验证，展示了高度敏捷运动的鲁棒控制能力。该方法首次实现了通用的零样本sim2real DRL速度控制器，能够同时跟踪平移和旋转自由度的时变参考信号。

Conclusion: Sim2Swim方法通过领域随机化和大规模并行训练，成功开发了能够适应不同AUV特性和水下条件的通用控制策略，解决了传统方法需要频繁重新调优的问题，为AUV的6DOF敏捷机动提供了实用解决方案。
...
### [118] [Ergodic Trajectory Planning with Dynamic Sensor Footprints](https://arxiv.org/abs/2512.08661)
*Ziyue Zheng,Yongce Liu,Hesheng Wang,Zhongqiang Ren*

Main category: cs.RO

TL;DR: 本文提出了一种考虑动态传感器足迹的遍历规划新方法，用于信息收集任务，相比传统方法能提升一个数量级的遍历性能。


<details>
  <summary>Details</summary>
Motivation: 现有遍历规划方法通常假设点传感器或固定形状分辨率的传感器足迹，但实际应用中传感器足迹会随机器人运动而动态变化（如无人机搭载的俯视相机，其视野随姿态和高度变化）。这种简化限制了实际应用效果。

Method: 提出了一种考虑动态传感器足迹的新度量标准，分析了理论局部最优条件，并开发了数值轨迹优化算法，能够同时优化轨迹和传感器足迹。

Result: 实验结果表明，所提方法能同时优化轨迹和传感器足迹，相比传统方法提升了一个数量级的遍历性能。该方法还成功应用于多无人机系统对3D空间物体的遍历覆盖。

Conclusion: 通过考虑动态传感器足迹，本文提出的遍历规划方法显著提升了信息收集任务的性能，为实际机器人系统中的传感器优化提供了有效解决方案。
...
### [119] [Non Normalized Shared-Constraint Dynamic Games for Human-Robot Collaboration with Asymmetric Responsibility](https://arxiv.org/abs/2512.08688)
*Mark Pustilnik,Francesco Borrelli*

Main category: cs.RO

TL;DR: 提出了一种用于共享工作空间中协作人机导航的动态博弈框架，引入非归一化均衡结构处理共享安全约束


<details>
  <summary>Details</summary>
Motivation: 在存在障碍物的共享工作空间中，人类和机器人需要协作完成共同任务，同时满足共享的安全约束（如碰撞避免和间距保持）。传统方法难以处理两个智能体对安全约束的不同贡献程度。

Method: 1. 提出动态博弈框架处理协作人机导航问题；2. 引入非归一化均衡结构处理共享约束，允许两个智能体以不同努力程度执行安全要求；3. 将该均衡结构嵌入到滚动时域最优控制方案中。

Result: 通过非归一化均衡结构，人类和机器人可以灵活地以不同努力程度共同满足安全约束，同时协作完成共同任务。

Conclusion: 提出的动态博弈框架和非归一化均衡结构为共享工作空间中的协作人机导航提供了一种有效方法，能够处理智能体间不同的安全约束执行能力。
...
### [120] [A Multi-Robot Platform for Robotic Triage Combining Onboard Sensing and Foundation Models](https://arxiv.org/abs/2512.08754)
...
TL;DR: 该研究开发了一个用于大规模伤亡事件远程初级分诊的异构机器人系统，由无人机和地面机器人协同工作，定位伤员、评估伤情并优先安排医疗救助，以保护急救人员安全。


<details>
  <summary>Details</summary>
Motivation: 大规模伤亡事件中，急救人员面临高风险环境，需要安全有效的伤员分诊方法。现有机器人系统多专注于探索或有限的医疗评估，缺乏完整的远程分诊能力。

Method: 采用无人机和地面机器人协同工作的异构系统：无人机负责识别伤员并提供俯视视角；地面机器人配备专业传感器测量生命体征、检测和定位物理损伤，实现完整的伤员分诊流程。

Result: 该系统能够完成完整的远程分诊流程，包括伤员定位、生命体征测量、伤情严重程度分类、精神状态评估和数据整合，为急救人员提供决策支持。

Conclusion: 多机器人系统能够增强人类在灾难响应场景中的能力，最大限度地挽救生命，为DARPA分诊挑战提供了有效的技术解决方案。
...
### [121] [Cascaded Tightly-Coupled Observer Design for Single-Range-Aided Inertial Navigation](https://arxiv.org/abs/2512.06198)
*Oussama Sifour,Soulaimane Berkane,Abdelhamid Tayebi*

Main category: cs.RO

TL;DR: 提出一种仅使用IMU、体坐标系矢量测量和固定锚点距离测量的单距离辅助导航观测器，能够重构刚体的完整状态


<details>
  <summary>Details</summary>
Motivation: 开发一种轻量级的自主导航解决方案，仅需最小传感器配置（IMU、磁力计和单距离测量）即可实现完整状态估计，降低系统复杂性和成本

Method: 采用级联观测器架构：首先构建扩展线性时变系统估计体坐标系位置、速度和重力方向；然后结合体坐标系矢量测量重构完整姿态；建立几乎全局渐近稳定性条件

Result: 在满足一致可观性条件下，系统具有几乎全局渐近稳定性；三维轨迹仿真显示能够准确估计位置、速度和姿态

Conclusion: 单距离辅助是一种轻量且有效的自主导航模态，仅需最小传感器配置即可实现完整的刚体状态重构，具有实际应用价值
...
### [122] [Heterogeneity in Multi-Robot Environmental Monitoring for Resolving Time-Conflicting Tasks](https://arxiv.org/abs/2512.08813)
*Connor York,Zachary R Madin,Paul O'Dowd,Edmund R Hunt*

Main category: cs.RO

TL;DR: 多机器人系统在执行连续任务时面临性能权衡，当被紧急、时间关键的子任务打断时。本文研究了巡逻区域与定位异常无线电信号之间的权衡，通过行为异质性（角色专业化）和感知异质性来优化系统性能。


<details>
  <summary>Details</summary>
Motivation: 多机器人系统在执行连续任务（如区域巡逻）时，经常会被紧急、时间关键的子任务（如定位异常信号）打断。这种中断导致系统需要在两个任务之间进行性能权衡，而如何通过机器人团队的异质性设计来优化这种权衡是一个重要研究问题。

Method: 通过模拟实验评估两种异质性策略：1）行为异质性 - 将机器人分为"巡逻者"和"搜索者"两种角色；2）感知异质性 - 只有搜索者能够感知无线电信号。研究不同团队组成下的帕累托最优权衡，分析团队规模、角色分配和感知能力限制对性能的影响。

Result: 行为异质性团队在大多数情况下表现出最平衡的权衡效果。当感知能力受限时，只有一半机器人具备感知能力的异质性团队与同质性团队表现相当，这为限制传感器部署提供了成本节约的依据。通过调整行为异质性程度，可以调节系统向任一任务的性能倾斜。

Conclusion: 预部署的角色和感知专业化是多机器人系统面对时间冲突任务时的强大设计考虑因素。行为异质性程度的变化可以调节系统性能向任一任务倾斜，为实际系统设计提供了灵活性和成本效益的优化策略。
...
### [123] [IPPO Learns the Game, Not the Team: A Study on Generalization in Heterogeneous Agent Teams](https://arxiv.org/abs/2512.08877)
*Ryan LeRoy,Jack Kolb*

Main category: cs.RO

TL;DR: 研究探讨了多智能体强化学习中自训练智能体是否真正学会了通用协调策略，还是仅仅过拟合了训练伙伴的行为。通过在异构多智能体环境中测试，发现简单的IPPO基线方法对未见过的队友算法具有良好泛化能力。


<details>
  <summary>Details</summary>
Motivation: 在多智能体强化学习中，智能体通常通过自训练与同质队友一起训练，这引发了一个问题：自训练的PPO智能体究竟学会了基于游戏本质的通用协调策略，还是仅仅过拟合了训练伙伴的行为？本研究旨在探究这个问题。

Method: 使用异构多智能体挑战环境，引入旋转策略训练方法，在训练过程中轮换不同学习算法的异构队友策略，让智能体接触更广泛的伙伴策略。同时与标准的自训练基线IPPO进行比较。

Result: 当与未参与训练的队友策略一起游戏时，RPT方法取得了与标准自训练基线IPPO相似的性能。这表明在异构多智能体设置中，IPPO基线能够泛化到未见过的队友算法，尽管在训练过程中没有经历队友多样性。

Conclusion: 简单的IPPO基线可能已经具备了多样训练方案所期望达到的对新队友的泛化能力。这表明在某些情况下，复杂的多样化训练可能并不比简单的自训练基线带来显著优势。
...
### [124] [Conditional Morphogenesis: Emergent Generation of Structural Digits via Neural Cellular Automata](https://arxiv.org/abs/2512.08360)
*Ali Sakour*

Main category: cs.NE

TL;DR: 提出条件神经细胞自动机（c-NCA），可从单一通用种子生长出不同拓扑结构（MNIST数字），仅通过空间广播类别向量引导，实现类别条件结构生成。


<details>
  <summary>Details</summary>
Motivation: 现有NCA研究主要关注连续纹理合成或单目标对象恢复，类别条件结构生成这一挑战尚未充分探索。生物系统具有显著的形态可塑性，单个基因组可在局部化学信号触发下编码多种特化细胞结构，这启发了在深度学习领域开发类似的自组织能力。

Method: 提出条件神经细胞自动机（c-NCA）架构，通过将one-hot条件向量注入细胞感知场，使单一局部规则集能够学习打破对称性并自组装成十个不同的几何吸引子。模型强制执行严格的局部性和平移等变性，不同于依赖全局感受野的传统生成模型。

Result: c-NCA实现了稳定收敛，能够从单个像素正确形成数字拓扑结构，并展现出类似生物系统的鲁棒性。模型成功从单一通用种子生长出不同的MNIST数字结构。

Conclusion: 这项工作弥合了基于纹理的NCA与结构模式形成之间的差距，为条件生成提供了一种轻量级、生物学上合理的替代方案，展示了局部规则如何通过条件引导实现复杂结构自组织。
...
### [125] [Sensor Attack Detection Method for Encrypted State Observers](https://arxiv.org/abs/2512.08010)
*Yeongjun Jang,Sangwon Lee,Junsoo Kim*

Main category: eess.SY

TL;DR: 提出一种无需解密的加密状态观测器，能够在加密状态下检测传感器攻击，适用于多输入多输出系统


<details>
  <summary>Details</summary>
Motivation: 传统状态观测器在传感器攻击检测时需要解密数据，存在安全风险。先前工作仅限于单输入单输出系统，需要扩展到更通用的多输入多输出系统

Method: 首先设计基于整数有限域和模运算的状态观测器，生成指示攻击存在的残差信号；然后开发同态加密方案，使观测器能在加密数据上运行并自动披露残差信号

Result: 提出的加密状态观测器能够在不解密的情况下检测传感器攻击，当披露的残差信号低于预设阈值时，可以恢复完整的加密状态

Conclusion: 该方法成功将加密状态观测器扩展到多输入多输出系统，在稀疏攻击和传感冗余条件下，能够在保持数据加密的同时有效检测传感器攻击
...
### [126] [Learning Dynamics from Infrequent Output Measurements for Uncertainty-Aware Optimal Control](https://arxiv.org/abs/2512.08013)
*Robert Lefringhausen,Theodor Springer,Sandra Hirche*

Main category: eess.SY

TL;DR: 提出一种贝叶斯方法处理非线性系统未知动力学和稀疏噪声测量下的最优控制问题，通过状态空间建模和MCMC采样处理不确定性，在糖尿病血糖调节案例中验证


<details>
  <summary>Details</summary>
Motivation: 当非线性系统动力学未知且只有稀疏、噪声输出测量可用时，可靠的最优控制具有挑战性。需要处理模型不确定性和测量不确定性

Method: 1) 在状态空间形式下建立连续时间动力学和潜在状态轨迹的贝叶斯先验；2) 使用配备数值ODE积分器的目标边缘Metropolis-Hastings采样器更新先验；3) 利用后验样本构建考虑模型和测量不确定性的场景最优控制问题；4) 使用标准非线性规划方法求解

Result: 该方法在1型糖尿病模型的血糖调节数值案例研究中得到验证，能够处理有限传感条件下的最优控制问题

Conclusion: 提出的贝叶斯框架能够有效处理非线性系统在未知动力学和稀疏噪声测量条件下的最优控制问题，通过后验采样和场景优化方法同时考虑模型和测量不确定性
...
### [127] [Cabin Layout, Seat Density, and Passenger Segmentation in Air Transport: Implications for Prices, Ancillary Revenues, and Efficiency](https://arxiv.org/abs/2512.08066)
*Alessandro V. M. Oliveira,Moises D. Vassallo*

Main category: eess.SY

TL;DR: 研究飞机客舱座位布局和密度如何影响国内航班机票定价，发现座位排数密度越高价格越低，且无选座费时高价票乘客常被分配到中间座位


<details>
  <summary>Details</summary>
Motivation: 探究航空公司客舱座位配置与机票定价之间的关系，理解座位密度、布局等物理因素如何影响票价策略和航空公司的经济逻辑

Method: 基于登机牌微观数据与乘客面对面访谈数据，使用后双选LASSO（PDS-LASSO）计量经济模型，控制商业和运营方面的不可观测因素，分析座位位置、市场特征和航班运营对价格的影响

Result: 座位排数密度越高，机票价格越低，反映了飞机规模扩大带来的规模经济和运营效率提升；无选座费情况下，高价票乘客常被分配到中间座位，因为购买时间较晚时靠窗和靠过道座位已售罄

Conclusion: 客舱座位密度与票价存在负相关关系，揭示了航空公司通过座位配置优化运营效率的经济逻辑；高价票乘客被分配中间座位的现象解释了航空公司选座费这一重要辅助收入的合理性
...
### [128] [Mitigation of Datacenter Demand Ramping and Fluctuation using Hybrid ESS and Supercapacitor](https://arxiv.org/abs/2512.08076)
*Min-Seung Ko,Jae Woong Shim,Hao Zhu*

Main category: eess.SY

TL;DR: 本文提出了一种基于混合储能系统的控制框架，用于超大规模AI数据中心的大负载变化功率平滑，通过电池和超级电容器的协调控制有效抑制电网频率和电压波动。


<details>
  <summary>Details</summary>
Motivation: AI数据中心的大负载变化会给电网频率和电压稳定性带来严重的爬坡和波动压力，需要有效的功率平滑解决方案来减轻这些干扰。

Method: 采用混合储能系统（电池+超级电容器），通过高通滤波器分离慢速和快速功率分量，分别由漏积分控制器和相位超前比例微分控制器处理，并加入前馈补偿、斜坡跟踪、自适应权重和重复控制机制。

Result: 该方法能有效抑制爬坡和波动，稳定系统频率，并在长时间随机训练周期下维持电池和超级电容器的可持续荷电状态轨迹。

Conclusion: 提出的混合储能系统控制框架为超大规模AI数据中心提供了有效的功率平滑解决方案，能显著改善电网稳定性并确保储能系统的可持续运行。
...
### [129] [Integrating Delay-Absorption Capability into Flight Departure Delay Prediction](https://arxiv.org/abs/2512.08197)
*Jianyang Zhou*

Main category: eess.SY

TL;DR: 该研究提出一个两阶段机器学习框架，通过建模航班延误吸收行为来改进航班延误预测，相比传统方法显著提升了预测性能。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习方法将上游延误视为静态变量，忽略了决定延误是否被吸收或传播到后续航班的动态恢复过程。准确预测航班起飞延误对于提高运营效率和缓解紧密耦合的飞机轮转中的连锁中断至关重要。
...
Conclusion: 将延误吸收建模为中间机制显著提高了预测性能，并提供了对机场恢复动态的可解释性见解，为数据驱动的延误管理和主动运营规划提供了实用基础。
...
### [130] [Bounding the Minimal Current Harmonic Distortion in Optimal Modulation of Single-Phase Power Converters](https://arxiv.org/abs/2512.08201)
*Jared Miller,Petros Karamanakos,Tobias Geyer*

Main category: eess.SY

TL;DR: 该论文提出了一种将最优脉冲模式设计重新解释为混合系统周期模式选择最优控制问题的方法，通过凸松弛技术构建半定规划层次结构，有效解决了包含整数电压电平和三角非线性的混合整数非凸优化问题。


<details>
  <summary>Details</summary>
Motivation: 最优脉冲模式设计是一个具有挑战性的混合整数非凸优化问题，涉及整数电压电平和目标函数及约束中的三角非线性。传统方法难以有效解决这类问题，需要新的优化框架来处理这种复杂结构。

Method: 将最优脉冲模式设计重新解释为混合系统的周期模式选择最优控制问题，其中选择开关角度和电压电平对应于在转移图中选择跳变时间。这种时域表述使得能够直接应用最优控制中的凸松弛技术，构建一系列半定规划层次结构，这些结构随转换器电平和开关角度数量呈亚二次方增长。

Result: 提出的方法能够为可实现的最小谐波失真提供下界，数值结果证明了该方法的有效性。半定规划层次结构能够有效处理混合整数非凸优化问题，并具有良好的可扩展性。

Conclusion: 通过将最优脉冲模式设计重新表述为混合系统的最优控制问题，并应用凸松弛技术，成功解决了这一具有挑战性的混合整数非凸优化问题，为电力电子调制技术提供了有效的优化框架。
...
### [131] [Theoretical Studies of Sub-THz Active Split-Ring Resonators for Near-Field Imaging](https://arxiv.org/abs/2512.08265)
*Ali Ameri,Jun-Chau Chien,Ali M. Niknejad*

Main category: eess.SY

TL;DR: 该论文开发了主动开口谐振环的理论框架，通过可调负电阻实现开关特性和品质因数提升，适用于硅芯片上的密集阵列集成，用于人体组织的高分辨率二维成像。


<details>
  <summary>Details</summary>
Motivation: 开发适用于硅芯片集成的高分辨率成像技术，实现实时、非侵入、低成本的人体组织成像。传统SRR在密集阵列应用中存在限制，需要增强开关性和品质因数控制能力。

Method: 提出主动开口谐振环设计框架，在传统SRR基础上集成可调负电阻。研究ASRR与传输线的耦合、非线性效应、信号流以及各种噪声源对检测性能的影响，通过仿真验证设计。

Result: 建立了ASRR的理论框架，提供了优化信噪比和功耗的设计指南，同时满足可扩展阵列的约束条件。验证了ASRR在密集阵列中生成像素化近场用于高分辨率成像的可行性。

Conclusion: ASRR技术为硅芯片上的密集阵列集成提供了有效的解决方案，能够实现高分辨率、实时、非侵入的人体组织成像，为低成本医疗成像设备的发展铺平了道路。
...
### [132] [Formation and Investigation of Cooperative Platooning at the Early Stage of Connected and Automated Vehicles Deployment](https://arxiv.org/abs/2512.08298)
*Zeyu Mu,Sergei S. Avedisov,Ahmadreza Moradipari,B. Brian Park*

Main category: eess.SY

TL;DR: 该论文提出了一种创新的控制框架，通过混合协同编队策略和战略换道决策模型，在CAV渗透率较低的情况下提升协同编队效益，显著改善安全、舒适和交通效率。


<details>
  <summary>Details</summary>
Motivation: 在网联自动驾驶车辆（CAV）初始部署阶段，由于道路上CAV分布稀疏，协同编队技术带来的效益有限。需要解决混合交通环境下如何最大化协同编队效益的问题。

Method: 提出创新控制框架：1）混合协同编队策略（CACCu），将CACC与非网联车辆集成；2）战略换道决策模型，促进安全高效的编队形成；3）嵌入周围车辆识别系统，帮助CAV识别和选择潜在的编队领导者。

Result: 模拟研究表明，即使在CV渗透率低至10%的情况下，CACCu系统相比仅使用CACC和ACC的系统，在安全、舒适和交通效率方面均有显著改善。最大编队形成增加达24%，加速度减少11%，燃油消耗降低7%。战略换道模型在6%-60%CV渗透率范围内提升了CAV性能，且不影响整体交通流。

Conclusion: 该控制框架有效解决了CAV初始部署阶段协同编队效益有限的问题，通过混合协同编队策略和战略换道决策，在低渗透率条件下显著提升了协同编队的效果，为CAV的逐步推广提供了实用解决方案。
...
### [133] [Integration of AI-Driven CAD Systems in Designing Water and Power Transportation Infrastructure for Industrial and Remote Landscape Applications](https://arxiv.org/abs/2512.08415)
*Sunggyu Park*

Main category: eess.SY

TL;DR: AI驱动的CAD系统通过自动化、预测建模和实时数据分析，提升基础设施设计的效率、效果和可持续性，特别是在欠发达地区的水电运输项目中。


<details>
  <summary>Details</summary>
Motivation: 随着全球对可持续基础设施需求的增长，传统CAD系统在应对复杂的水电运输项目（特别是在工业和偏远地区）时面临效率、精度和适应性方面的挑战。需要将AI技术集成到CAD系统中，以提升设计工作流程、减少人为错误并优化资源配置。

Method: 研究探讨了AI驱动的CAD系统如何通过嵌入自动化、预测建模和实时数据分析来改进基础设施设计。同时分析了AI支持的工具集如何增强设计工作流程，并研究了AI采用面临的技术和组织挑战，包括数据孤岛、互操作性问题和人员适应问题。

Result: 研究发现AI驱动的CAD系统能够实现更快的项目交付、更高的设计精度，并增强对环境与物流约束的韧性。AI技术有助于连接CAD、GIS和IoT技术，开发出自学习和自适应设计系统。

Conclusion: AI与CAD系统的集成对于满足全球可持续基础设施日益增长的需求至关重要。AI驱动的CAD系统通过自动化、预测分析和实时数据整合，能够显著提升基础设施设计的效率、精度和适应性，尽管在实施过程中需要克服数据孤岛、互操作性和人员适应等挑战。
...
### [134] [Beyond Wave Variables: A Data-Driven Ensemble Approach for Enhanced Teleoperation Transparency and Stability](https://arxiv.org/abs/2512.08436)
*Nour Mitiche,Farid Ferguene,Mourad Oussalah*

Main category: eess.SY

TL;DR: 提出了一种数据驱动的混合框架，用三个先进序列模型的集成替代传统波变量变换，在时变延迟和噪声下保持透明度同时确保稳定性。


<details>
  <summary>Details</summary>
Motivation: 通信信道中的时间延迟对双边遥操作系统造成挑战，影响透明度和稳定性。传统基于波变量的四通道架构方法虽然通过无源性确保稳定性，但仍易受波反射、可变延迟和环境噪声等干扰影响。
...
Result: 实验验证在Python中使用MATLAB/Simulink生成的基线系统数据进行。结果表明，优化后的集成方法在可变延迟和噪声下实现了与基线波变量系统相当的透明度，同时通过无源性约束确保稳定性。

Conclusion: 提出的数据驱动混合框架有效解决了双边遥操作系统中时间延迟带来的挑战，在保持透明度的同时确保系统稳定性，优于传统波变量方法。
...
### [135] [MPC for tracking for anesthesia dynamics](https://arxiv.org/abs/2512.08452)
*Maxim Raymond,Kaouther Moussa,Mirko Fiacchini,Jimmy Lauber*

Main category: eess.SY

TL;DR: 提出了一种用于麻醉动力学控制的MPC跟踪方法，能够优化由于MISO模型特性导致的不唯一稳态对，通过将慢动态状态视为扰动并引入补偿项，确保递归可行性和渐近稳定性。


<details>
  <summary>Details</summary>
Motivation: 麻醉动力学是一个多时间尺度系统，具有快慢两种动态特性。在麻醉控制中，输出方程仅依赖于快动态，而慢动态状态可以作为扰动处理。由于模型的MISO（多输入单输出）特性，稳态对不唯一，需要一种能够优化稳态选择的控制策略。

Method: 提出MPC跟踪方法，将慢动态状态视为扰动并引入补偿项，将系统重新表述为名义系统。设计适当的终端条件，确保递归可行性和渐近稳定性。在MPC跟踪框架中优化稳态对选择。

Result: 在仿真环境中对患者进行控制器性能评估，验证了所提框架的有效性。通过适当的终端条件设计，确保了递归可行性和渐近稳定性。

Conclusion: 提出的MPC跟踪方法能够有效处理麻醉动力学中的多时间尺度特性和稳态对不唯一问题，为麻醉控制提供了一种鲁棒且稳定的解决方案。
...
### [136] [Quantization and Security Parameter Design for Overflow-Free Confidential FRIT](https://arxiv.org/abs/2512.08464)
*Jungjin Park,Osamu Kaneko,Kiminao Kogiso*

Main category: eess.SY

TL;DR: 提出了一种系统化设计方法，用于确定保密虚构参考迭代调谐（CFRIT）中的量化增益和安全参数，实现无溢出且精度保证的加密控制器调谐。


<details>
  <summary>Details</summary>
Motivation: 在加密数据驱动的增益调谐中，需要在保证计算安全的同时防止加密过程中的溢出问题，并确保调谐精度。现有方法缺乏对量化误差和安全参数选择的系统化指导。

Method: 基于操作数据估计编码（加密）过程中量化误差的范围，推导出量化增益和安全参数的显式解析条件以防止加密数据计算溢出。分析量化引起的误差与CFRIT和非保密FRIT获得增益之间的偏差关系。

Result: 通过数值示例验证了所提设计方法，设计的参数在规定的容差范围内实现了准确的加密调谐同时防止了溢出。可视化参数组合的允许区域，为加密数据驱动控制的参数设计提供实用见解。

Conclusion: 该研究为CFRIT加密控制器调谐提供了系统化的参数设计程序，能够在保证安全性的同时防止计算溢出并控制调谐精度，为实际加密控制系统的实现提供了理论指导。
...
### [137] [High-performance computing enabled contingency analysis for modern power networks](https://arxiv.org/abs/2512.08465)
*Alexandre Gracia-Calvo,Francesca Rossi,Eduardo Iraola,Juan Carlos Olives-Camps,Eduardo Prieto-Araujo*

Main category: eess.SY

TL;DR: 该论文提出了一种结合N-2事故分析与小信号稳定性评估的可扩展方法，通过风险指数优先处理关键组件，并利用高性能计算技术实现大规模事故集的全面探索。


<details>
  <summary>Details</summary>
Motivation: 现代电网因高复杂性和间歇性资源渗透增加而面临级联故障风险增加，需要超越传统N-1准则的严格安全评估。现有方法在计算可处理性和确定性筛选方面存在局限性。

Method: 提出基于风险指数的方法，将事故严重性（包括最优潮流发散、孤岛和振荡不稳定）与基于可靠性数据的元件故障频率相结合。使用PyCOMPSs并行编程库实现高性能计算，协调VeraGrid最优潮流仿真和STAMP小信号分析。

Result: 在IEEE 118节点测试系统上验证，处理超过57,000个场景，识别出易触发级联故障的组件。结果表明基于风险的方法能有效识别确定性N-1准则常忽略的关键资产。

Conclusion: 建立了一个可复制且高效的概率安全评估工作流程，适用于大规模电网，能够在近实时环境中支持操作员决策。
...
### [138] [Optimal Control of Behavioral-Feedback SIR Epidemic Model](https://arxiv.org/abs/2512.08544)
*Martina Alutto,Leonardo Cianfanelli,Giacomo Como,Fabio Fagnani,Francesca Parise*

Main category: eess.SY

TL;DR: 该研究提出了一个包含行为反馈的SIR传染病模型，其中感染率取决于易感者和感染者的比例。研究者为社交规划者制定了一个最优控制问题，目标是在无限时间范围内将感染曲线控制在阈值以下，同时最小化干预成本。研究证明，在相当一般的条件下，"填满盒子"策略是最优控制策略。


<details>
  <summary>Details</summary>
Motivation: 传统SIR模型假设感染率恒定，无法反映个体在疫情中的适应性行为（如自愿社交距离、戴口罩等）。本研究旨在建立一个更现实的模型，考虑内生的行为反馈机制，为疫情管理提供更有效的控制策略。

Method: 构建了一个行为反馈SIR流行病模型，其中感染率是易感者和感染者比例的函数。为社交规划者制定了一个最优控制问题，目标是最小化干预成本同时将感染人数控制在阈值以下。通过分析模型的动态特性，在相当一般的感染率条件下，证明了"填满盒子"策略的最优性。

Result: 研究证明，在相当一般的感染率条件下，"填满盒子"策略是最优控制策略。该策略包括：在感染人数达到阈值前不干预，让疫情自然传播；一旦达到阈值，施加最小控制使感染人数保持在阈值水平，直到基本再生数小于1，疫情自然消退。该结果推广了经典SIR模型中的现有结果。

Conclusion: 该研究通过引入行为反馈机制，扩展了传统SIR模型，为考虑人类适应性行为的疫情管理提供了理论框架。"填满盒子"策略的最优性证明为制定稳健的疫情控制策略提供了重要见解，有助于平衡公共卫生目标和经济成本。
...
### [139] [Decoupled Design of Time-Varying Control Barrier Functions via Equivariances](https://arxiv.org/abs/2512.08607)
*Adrian Wiltz,Dimos V. Dimarogonas*

Main category: eess.SY

TL;DR: 提出了一种系统化设计时变控制屏障函数的方法，将时不变分量与多个时变分量结合，利用系统动力学的结构特性，能够处理更广泛的时间变化约束。


<details>
  <summary>Details</summary>
Motivation: 现有控制屏障函数方法在处理时变约束时存在局限性，需要开发一种能够有效处理时间变化约束的系统化设计方法，特别是在不确定环境中应用时。

Method: 构建特定类别的时不变控制屏障函数来编码系统相对于给定约束的动态能力，然后通过适当设计的时变变换进行增强。利用系统动力学的结构特性（特别是等变性）来处理更广泛的时变约束，将时变设计与底层控制屏障函数的计算密集型构造解耦。

Result: 该方法能够处理输入约束和欠驱动系统，仅需要关于约束时间变化的定性知识，适用于不确定环境中的应用，提供了一种计算上具有吸引力的时变控制屏障函数设计方法。

Conclusion: 提出的系统化方法成功地将时变设计与控制屏障函数构造分离，利用系统结构特性处理时变约束，为不确定环境中的安全控制提供了有效的计算框架。
...
### [140] [NLoS Localization with Single Base Station Based on Radio Map](https://arxiv.org/abs/2512.08608)
*Jiajie Xu,Yifan Guo,Xiucheng Wang,Nan Cheng,Tingting Yang*

Main category: eess.SY

TL;DR: 提出了一种在NLoS环境下基于单个基站和无线电地图的定位框架，通过整合时序信号测量和先验无线电信息，在典型NLoS环境中实现亚米级定位精度。


<details>
  <summary>Details</summary>
Motivation: 现有GNSS和三基站定位方法在NLoS环境下性能不可靠，特别是在多径效应强烈的密集城区。需要解决NLoS环境下单基站定位的挑战。

Method: 提出单基站定位框架，整合时序信号测量和无线电地图中的先验无线电信息。利用时间测量特征并与无线电地图匹配，减轻多径传播的负面影响，减少对LoS路径的依赖。

Result: 仿真实验评估了不同无线电地图构建策略和测量序列长度对定位精度的影响。结果表明，该方案在典型NLoS环境中实现了亚米级定位精度。

Conclusion: 该方法为单基站部署提供了实用且鲁棒的解决方案，在NLoS环境下具有显著优势，特别是在密集城区等复杂环境中。
...
### [141] [Direct transfer of optimized controllers to similar systems using dimensionless MPC](https://arxiv.org/abs/2512.08667)
*Josip Kir Hromatko,Shambhuraj Sawant,Šandor Ileš,Sébastien Gros*

Main category: eess.SY

TL;DR: 提出基于无量纲模型预测控制的控制器直接迁移方法，无需额外调参即可将优化控制器直接应用于动态相似系统


<details>
  <summary>Details</summary>
Motivation: 传统缩比模型实验中，控制器迁移到全尺寸系统时通常需要额外调参，增加了实验成本和复杂性

Method: 采用无量纲模型预测控制方法，通过无量纲化处理实现控制器的直接迁移，并利用强化学习或贝叶斯优化自动调参

Result: 在倒立摆起摆和赛车控制问题上验证了方法的有效性，优化后的控制器可直接迁移到动态相似系统

Conclusion: 无量纲模型预测控制方法实现了控制器的直接迁移，减少了实验成本，并允许使用不同尺度系统的数据进行参数优化
...
### [142] [Gradient-Informed Monte Carlo Fine-Tuning of Diffusion Models for Low-Thrust Trajectory Design](https://arxiv.org/abs/2512.08705)
*Jannik Graebner,Ryne Beeson*

Main category: eess.SY

TL;DR: 本文提出了一种结合梯度信息马尔可夫链蒙特卡洛方法与扩散模型的低推力航天器轨迹初步设计方法，在土星-泰坦系统中实现了更高效的帕累托前沿探索。


<details>
  <summary>Details</summary>
Motivation: 圆形限制性三体问题中的低推力航天器轨迹初步设计是一个全局搜索问题，具有复杂的目标函数景观和众多局部极小值。传统方法在探索帕累托最优解方面效率较低，需要更有效的方法来加速收敛并提高可行性。

Method: 将问题建模为从局部最优解邻域支持的未归一化分布中采样，采用梯度信息马尔可夫链蒙特卡洛方法（MALA和HMC），结合扩散模型生成初始分布。使用状态转移矩阵解析计算平衡燃料消耗、飞行时间和约束违反的目标函数导数。

Result: 在土星-泰坦系统的多圈转移任务中，梯度漂移项加速了马尔可夫链的混合和收敛。MALA在性能和计算成本之间提供了最佳权衡，将可行性率从17.34%提高到63.01%，并产生了更密集、更多样化的帕累托前沿覆盖。通过奖励加权似然最大化对扩散模型进行微调，学习了问题的全局解结构。

Conclusion: 梯度信息马尔可夫链蒙特卡洛方法显著提高了低推力轨迹设计的效率和效果，特别是MALA算法在探索帕累托最优解方面表现优异。结合扩散模型的微调框架消除了繁琐的单独数据生成阶段，为复杂航天任务设计提供了有效解决方案。
...
### [143] [LaMoSys3.5D: Enabling 3.5D-IC-Based Large Language Model Inference Serving Systems via Hardware/Software Co-Design](https://arxiv.org/abs/2512.08731)
*Qipan Wang,Zhe Zhang,Shuangchen Li,Hongzhong Zheng,Zheng Liang,Yibo Lin,Runsheng Wang,Ru Huang*

Main category: eess.SY

TL;DR: LaMoSys3.5D：首个用于大语言模型推理服务的可扩展3.5DIC架构，通过异构3D-DRAM芯片和软硬件协同设计，在吞吐率/功耗和端到端延迟方面显著优于现有系统。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的成功，需要高吞吐、高能效的规模化推理。现有3D-DRAM加速器虽然提供高内存带宽，但如何平衡预填充阶段的计算密度和解码阶段的带宽容量仍是未解问题，且大多数设计未针对端到端服务进行优化。

Method: 提出LaMoSys3.5D架构，在2.5D中介层上集成异构3D-DRAM芯片：计算密集型芯片用于预填充，带宽容量密集型芯片用于解码。采用软硬件协同设计方法，涵盖数据流、并行映射，并引入热感知建模和分层设计空间探索框架。

Result: 在不同LLM和工作负载下，LaMoSys3.5D相比DGX-A100系统在吞吐率/功耗方面提升62%，相比现有3D设计在端到端延迟方面获得4.87倍的几何平均改进。

Conclusion: LaMoSys3.5D是首个用于LLM服务的可扩展3.5DIC架构，通过异构芯片集成和系统级优化，显著提升了推理效率和性能，并为3.5DIC架构和端到端推理服务提供了重要设计指导。
...
### [144] [IoT-based Cost-Effective Fruit Quality Monitoring System using Electronic Nose](https://arxiv.org/abs/2512.08753)
*Anindya Bhattacharjee,Nittya Ananda Biswas,Khondakar Ashik Shahriar,Kawsain Bin Salim*

Main category: eess.SY

TL;DR: 基于MQ气体传感器阵列的低成本物联网香蕉质量监测系统，通过检测乙醇、甲烷和氨气等挥发性气体，建立数学模型评估水果质量，减少主观判断导致的经济损失


<details>
  <summary>Details</summary>
Motivation: 收获后因主观质量评估造成的损失对经济和食品安全造成重大损害，特别是在孟加拉国等国家。需要基于科学方法的客观决策来减轻这种损害

Method: 提出低功耗、成本效益高的水果质量监测系统，使用MQ气体传感器阵列作为电子鼻，追踪香蕉成熟和分解过程中挥发性气体（乙醇、甲烷、氨气）的排放，基于气体浓度阈值建立数学模型评估水果质量，并将信息集成到仪表板中供农民决策

Result: 开发了能够准确评估水果质量的数学模型，并创建了用于及时决策和监测的仪表板系统

Conclusion: 该方法有潜力减少经济损失、增强食品安全，并为供应链提供可扩展的解决方案，超越主观质量监测和决策实践
...
### [145] [ThreadWeaver: Adaptive Threading for Efficient Parallel Reasoning in Language Models](https://arxiv.org/abs/2512.07843)
...
TL;DR: ThreadWeaver是一个自适应并行推理框架，在保持与流行顺序推理模型相当准确度的同时，显著降低推理延迟，在数学推理基准上实现高达1.53倍的平均加速。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型通过扩展推理时间计算实现了强大的推理性能，但固有的顺序解码导致高延迟。现有并行推理方法要么局限于监督行为克隆，要么相比顺序CoT基线准确度显著下降，且需要定制推理引擎，部署复杂。

Method: 1) 两阶段并行轨迹生成器，产生大规模高质量带并行标注的CoT数据用于监督微调；2) 基于trie的训练-推理协同设计，可在任何现成自回归推理引擎上实现并行推理，无需修改位置嵌入或KV缓存；3) 并行化感知的强化学习框架，教导模型平衡准确度与有效并行化。

Result: 在六个具有挑战性的数学推理基准上，基于Qwen3-8B训练的ThreadWeaver实现了与前沿顺序推理模型相当的准确度（平均71.9%，AIME24上79.9%），同时提供高达1.53倍的平均token延迟加速，建立了准确度与效率之间的新帕累托前沿。

Conclusion: ThreadWeaver框架成功解决了并行推理中的准确度与效率权衡问题，通过创新的数据生成、训练-推理协同设计和强化学习框架，在保持准确度的同时显著提升推理速度，且兼容现有推理引擎，易于部署。
...
### [146] [Softly Symbolifying Kolmogorov-Arnold Networks](https://arxiv.org/abs/2512.07875)
*James Bagrow,Josh Bongard*

Main category: cs.LG

TL;DR: S2KAN在KAN基础上引入符号原语字典和可学习门控，通过可微分稀疏化和最小描述长度目标，实现符号化与稠密样条的灵活切换，在保持精度的同时提升可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统KAN虽然具有可解释性潜力，但训练后的激活函数往往缺乏符号保真度，学习到的分解没有有意义的对应关系，无法真正实现可解释的机器学习。

Method: 提出S2KAN，将符号原语直接集成到训练中：每个激活函数从符号项和稠密项字典中提取，使用可学习门控稀疏化表示；通过可微分稀疏化和最小描述长度目标进行端到端优化。

Result: 在符号基准测试、动力学系统预测和真实世界预测任务中，S2KAN以更小的模型实现了竞争性或更优的精度；即使没有正则化压力，也观察到自稀疏化的出现。

Conclusion: S2KAN能够在符号项足够时发现可解释形式，不足时优雅地退化为稠密样条，为可解释机器学习提供了灵活且有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 医疗决策经常使用结合风险方程和规则的算法，但传统符号回归难以建模这种决策过程。由于符号回归能够推导数据驱动的可解释模型，在开发数据驱动的临床风险评分方面具有潜力。
...
Conclusion: Brush算法能够有效开发数据驱动的临床风险评分系统，在保持高预测性能的同时提供可解释的模型，为医疗决策支持提供了有前景的工具。
...
### [148] [RaX-Crash: A Resource Efficient and Explainable Small Model Pipeline with an Application to City Scale Injury Severity Prediction](https://arxiv.org/abs/2512.07848)
...
<details>
  <summary>Details</summary>
Motivation: 纽约市每年发生超过10万起机动车碰撞事故，造成严重的伤害和公共卫生负担，需要高效、可解释的预测模型来支持城市规模的伤害分析。

Method: 整合三个关联表（数千万条记录），构建统一特征模式，在分区存储上训练紧凑的树集成模型（随机森林和XGBoost），并与基于文本摘要的小型语言模型进行比较。

Result: XGBoost和随机森林在时间保留测试集上的准确率分别为0.7828和0.7794，明显优于小型语言模型（0.594和0.496）；类别不平衡分析显示简单类别加权可提高致命事故召回率；SHAP归因分析突出人类脆弱性因素、时间和位置是严重程度的主要驱动因素。
...
### [149] [Long-Sequence LSTM Modeling for NBA Game Outcome Prediction Using a Novel Multi-Season Dataset](https://arxiv.org/abs/2512.08591)
*Charles Rios,Longzhen Han,Almas Baimagambetov,Nikolaos Polatidis*

Main category: cs.LG

TL;DR: 该研究构建了覆盖2004-2024赛季的纵向NBA数据集，并开发了LSTM深度学习框架来预测篮球比赛结果，通过长达9840场比赛的序列长度捕捉长期趋势，在多项指标上优于传统机器学习方法。


<details>
  <summary>Details</summary>
Motivation: 预测NBA比赛结果对教练策略、球迷参与和体育博彩都很重要，但现有模型存在概念漂移、时间上下文有限和跨赛季不稳定的问题。

Method: 构建了覆盖2004-05至2024-25赛季的纵向NBA数据集，设计了LSTM深度学习框架，使用长达9840场比赛（相当于8个完整赛季）的序列长度来捕捉球队动态演变和赛季间依赖关系。

Result: LSTM模型在所有指标上表现最佳：准确率72.35%，精确率73.15%，AUC-ROC 76.13%，优于逻辑回归、随机森林、MLP和CNN等基线模型。

Conclusion: 长序列时间建模对篮球结果预测至关重要，新构建的多赛季数据集有助于开发稳健、可泛化的NBA预测系统。
...
### [150] [GPU Memory Prediction for Multimodal Model Training](https://arxiv.org/abs/2512.07853)
*Jinwoo Jeong,Minchul Kang,Younghun Go,Changyong Shin,Hyunho Lee,Junho Yoon,Gyeongsik Yang,Chuck Yoo*

Main category: cs.LG

TL;DR: 提出一个预测多模态模型GPU峰值内存使用的框架，通过分解模型架构和分析训练行为来准确预测内存使用，避免内存溢出错误。
...
Method: 提出一个框架，通过分析多模态模型的架构和训练行为来预测GPU峰值内存使用。具体方法是将多模态模型分解为其组成层，并应用因子化方法来估计每层的内存使用。

Result: 评估显示该框架实现了约8.7%的平均MAPE（平均绝对百分比误差）的高预测精度。

Conclusion: 该框架能够准确预测多模态模型的GPU内存使用，有助于防止内存溢出错误，提高训练过程的稳定性和计算资源利用率。
...
### [151] [LAPA: Log-Domain Prediction-Driven Dynamic Sparsity Accelerator for Transformer Model](https://arxiv.org/abs/2512.07855)
...
<details>
  <summary>Details</summary>
Motivation: Transformer模型在NLP和CV任务中表现出色，但随着输入序列变化，计算瓶颈在不同阶段呈现动态特性，需要跨阶段稀疏加速策略。现有稀疏Transformer方法多为单阶段设计，其稀疏性预测机制在多阶段应用时会产生显著功耗开销。
...
Result: 实验结果显示，LAPA相比最先进的Spatten、Sanger和FACT方法，能效分别提升3.52倍、3.24倍和2.79倍。

Conclusion: LAPA通过算法-架构协同设计有效解决了Transformer跨阶段稀疏加速问题，显著提升了能效，为动态计算瓶颈提供了有效的硬件加速方案。
...
### [152] [SA^2GFM: Enhancing Robust Graph Foundation Models with Structure-Aware Semantic Augmentation](https://arxiv.org/abs/2512.07857)
*Junhua Shi,Qingyun Sun,Haonan Yuan,Xingcheng Fu*

Main category: cs.LG

TL;DR: SA^2GFM是一个鲁棒的图基础模型框架，通过结构感知语义增强来提升领域自适应表示能力，在节点和图分类任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 当前图基础模型在领域噪声、结构扰动和对抗攻击下的鲁棒性研究不足，关键限制在于对层次结构语义建模不够充分，而这对泛化能力至关重要。

Method: 1. 通过将基于熵的编码树转化为结构感知文本提示来进行特征增强；2. 使用自监督信息瓶颈机制通过结构引导压缩来提取鲁棒、可迁移表示；3. 引入专家自适应路由机制（混合专家架构+空专家设计）解决跨领域适应的负迁移问题；4. 提出微调模块通过联合社区内和社区间结构学习来优化层次结构。

Result: 在广泛的实验中，SA^2GFM在节点和图分类任务中，在对抗随机噪声和对抗扰动的有效性和鲁棒性方面优于9个最先进的基线方法。

Conclusion: SA^2GFM通过结构感知语义增强和信息瓶颈机制，显著提升了图基础模型的鲁棒性和领域自适应能力，为解决图模型在复杂环境中的泛化问题提供了有效方案。
...
### [153] [FAIM: Frequency-Aware Interactive Mamba for Time Series Classification](https://arxiv.org/abs/2512.07858)
...
<details>
  <summary>Details</summary>
Motivation: 时间序列分类在环境监测、医疗诊断等应用中至关重要，但现有深度学习模型存在计算成本高、对噪声敏感、在小规模数据集上容易过拟合等问题，需要更轻量且鲁棒的解决方案。

Method: 提出FAIM模型，包含自适应滤波块（AFB）利用傅里叶变换提取频域特征，通过可学习自适应阈值动态抑制噪声；设计交互式Mamba块（IMB）实现高效多粒度信息交互；结合自监督预训练机制增强对复杂时序模式的理解。

Result: 在多个基准测试上的广泛实验表明，FAIM持续优于现有最先进方法，在准确性和效率之间实现了优越的平衡，并在各种领域和高噪声场景中表现出色。

Conclusion: FAIM通过频率感知设计和交互式Mamba架构，有效解决了时间序列分类中的计算效率、噪声鲁棒性和过拟合问题，为实际应用提供了轻量且高性能的解决方案。
...
### [154] [SetAD: Semi-Supervised Anomaly Learning in Contextual Sets](https://arxiv.org/abs/2512.07863)
*Jianling Gao,Chongyang Tao,Xuelian Lin,Junfeng Liu,Shuai Ma*

Main category: cs.LG

TL;DR: SetAD将半监督异常检测重构为集合级任务，通过注意力编码器和分级学习目标直接建模定义异常的复杂群体级交互，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有半监督异常检测方法主要关注单个点或简单点对的评分，这种点中心或对中心的视角忽略了异常的上下文本质（异常是由其与群体的偏离定义的），也未能利用集合组合生成的丰富监督信号。这些模型难以利用数据中的高阶交互，而这对学习判别性表示至关重要。

Method: 提出SetAD框架，将半监督异常检测重构为集合级异常检测任务。使用基于注意力的集合编码器，通过分级学习目标训练模型学习量化整个集合的异常程度。同时提出上下文校准的异常评分机制，通过聚合点在多个不同上下文集合中相对于同伴行为的归一化偏离来评估点的异常分数。

Result: 在10个真实世界数据集上的广泛实验表明，SetAD显著优于最先进的模型。特别值得注意的是，模型性能随着集合大小的增加而持续提升，为基于集合的异常检测公式提供了强有力的实证支持。

Conclusion: SetAD通过将异常检测重构为集合级任务，直接建模定义异常的复杂群体级交互，解决了现有方法忽略上下文本质和未能利用高阶交互的局限性。提出的框架在多个数据集上表现出优越性能，且性能随集合大小增加而提升，验证了集合视角的有效性。
...
### [155] [Pattern Recognition of Ozone-Depleting Substance Exports in Global Trade Data](https://arxiv.org/abs/2512.07864)
*Muhammad Sukri Bin Ramli*

Main category: cs.LG

TL;DR: 本文提出了一种基于无监督机器学习的框架，用于分析海关数据以监测环境条约（如《蒙特利尔议定书》）的执行情况，通过聚类、异常检测和启发式标记识别可疑贸易模式，并生成优先级评分供监管部门审查。


<details>
  <summary>Details</summary>
Motivation: 需要新的方法来监测环境条约（如《蒙特利尔议定书》）的执行情况，通过审查大型复杂海关数据集来发现违规贸易行为。

Method: 使用无监督机器学习框架：1) K-Means聚类基于货物价值和重量发现自然贸易原型；2) 隔离森林和IQR异常检测识别"超大贸易"和价格异常；3) 启发式标记发现模糊描述等规避策略；4) 结合多层结果生成优先级评分；5) 使用SHAP可解释AI验证关键风险因素。

Result: 应用于10万条贸易记录，成功识别出1,351个价格异常值和1,288个高优先级货物供海关审查。发现高优先级商品的价值重量比与普通商品显著不同。模型成功检测到2021年初"超大贸易"激增，这与美国AIM法案的实际监管影响直接相关。

Conclusion: 该工作提供了一个可重复的无监督学习流程，将原始贸易数据转化为可供监管机构使用的优先级情报，为环境条约监测提供了有效的技术工具。
...
### [156] [Using Text-Based Life Trajectories from Swedish Register Data to Predict Residential Mobility with Pretrained Transformers](https://arxiv.org/abs/2512.07865)
*Philipp Stark,Alexandros Sopasakis,Ola Hall,Markus Grillitsch*

Main category: cs.LG

TL;DR: 将瑞典大规模登记数据转化为文本化生命轨迹，解决分类变量高基数和编码不一致问题，预测居民流动性，验证序列模型效果


<details>
  <summary>Details</summary>
Motivation: 解决数据分析中长期存在的两个挑战：分类变量的高基数和随时间变化的编码方案不一致性，同时利用瑞典独特的全面人口登记数据进行纵向预测研究

Method: 将690万个体（2001-2013）的登记数据转化为语义丰富的文本生命轨迹，结合人口统计信息和年度居住、工作、教育、收入、家庭状况变化，使用多种NLP架构（LSTM、DistilBERT、BERT、Qwen）预测2013-2017年的居民流动性
...
### [157] [Advancing physiological time series reconstruction and imputation via mixture of receptive fields and experts fusion](https://arxiv.org/abs/2512.07873)
*Ci Zhang,Huayu Li,Changdi Yang,Jiangnan Xia,Yanzhi Wang,Xiaolong Ma,Jin Lu,Geng Yuan*

Main category: cs.LG

TL;DR: 提出基于混合专家(MoE)的噪声估计器在分数扩散框架中，用于医学时间序列信号重建，通过RFAMoE模块自适应选择感受野，通过Fusion MoE模块并行生成多个噪声信号并融合，单次推理完成重建，显著提升性能并降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 医学时间序列信号具有多变量、高时间变异性、高噪声和易受伪影影响等独特特性，使得基于深度学习的方法在插补等任务中仍然具有挑战性。扩散模型在时间序列信号重建中显示出巨大潜力，但在医学时间序列领域尚未充分探索。

Method: 提出基于混合专家(MoE)的噪声估计器在分数扩散框架中。设计了感受野自适应MoE(RFAMoE)模块，使每个通道在扩散过程中自适应选择所需感受野。设计了Fusion MoE模块，利用MoE特性并行生成K个噪声信号，通过路由机制融合，在单次推理步骤中完成信号重建。

Result: 广泛实验结果表明，提出的框架在不同任务和数据集上始终优于基于扩散的SOTA工作。不仅提高了性能，还消除了多次推理过程带来的显著计算成本和延迟。

Conclusion: 提出的基于MoE的扩散框架为医学时间序列信号重建提供了一种高效且性能优越的解决方案，通过自适应感受野选择和并行噪声生成融合机制，在保持高性能的同时显著降低了计算开销。
...
### [158] [Controllable risk scenario generation from human crash data for autonomous vehicle testing](https://arxiv.org/abs/2512.07874)
*Qiujing Lu,Xuanhan Wang,Runze Yuan,Wei Lu,Xinyi Gong,Shuo Feng*

Main category: cs.LG

TL;DR: CRAG框架统一建模自动驾驶车辆测试中的正常驾驶行为和罕见风险行为，通过结构化潜在空间分离正常与风险行为，结合优化模式转换机制，实现从安全状态到风险状态的平滑过渡，提升场景多样性和可控性。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆安全测试需要同时模拟日常驾驶和罕见安全关键场景，现有方法难以统一建模背景车辆和弱势道路使用者的正常行为与风险行为，特别是如何有效利用有限的碰撞数据生成多样化的风险场景。

Method: CRAG框架构建结构化潜在空间，分离正常行为和风险相关行为；结合风险感知的潜在表示与基于优化的模式转换机制，使智能体能够在长时间范围内从安全状态平滑、合理地过渡到风险状态，同时保持两种状态的高保真度。

Result: 实验表明CRAG相比现有基线方法提高了场景多样性，同时能够可控地生成风险场景，实现对自动驾驶系统鲁棒性的有针对性和高效评估。

Conclusion: CRAG框架成功统一了自动驾驶测试中正常行为和风险行为的建模，通过结构化潜在空间和优化转换机制，有效利用有限碰撞数据，生成多样化且可控的风险场景，为自动驾驶系统的鲁棒性评估提供了有效工具。
...
### [159] [Graph Contrastive Learning via Spectral Graph Alignment](https://arxiv.org/abs/2512.07878)
...
Method: 提出SpecMatch-CL损失函数，通过最小化视图特定图-图的归一化拉普拉斯矩阵差异来对齐图嵌入，理论证明该差异为理想完美对齐对比损失提供上界

Result: 在8个TU基准测试的无监督学习和低标签率半监督学习中建立新的SOTA，在PPI-306K和ZINC 2M数据集上实现一致的迁移学习增益

Conclusion: SpecMatch-CL通过谱匹配方法有效对齐图嵌入的全局结构，显著提升图表示学习性能，为图对比学习提供了新的理论框架和实践方法
...
### [160] [Nonnegative Matrix Factorization through Cone Collapse](https://arxiv.org/abs/2512.07879)
*Manh Nguyen,Daniel Pimentel-Alarcón*

Main category: cs.LG

TL;DR: 本文提出了一种基于几何视角的NMF新方法Cone Collapse，通过从完整非负象限开始迭代收缩到数据生成的最小锥体，然后在此基础上构建锥体感知的正交NMF模型CC-NMF，在多个数据集上取得了优于传统NMF方法的聚类效果。


<details>
  <summary>Details</summary>
Motivation: 现有NMF算法主要从优化角度出发，未能充分利用NMF诱导的锥体几何特性。数据点位于凸锥中，其极射线编码了基本方向或"主题"。本文旨在从几何视角重新审视NMF，通过显式恢复数据锥体来改进聚类性能。

Method: 提出Cone Collapse算法：从完整非负象限开始，迭代收缩到数据生成的最小锥体；在温和假设下证明算法在有限步内终止并恢复X^⊤的最小生成锥体；在此基础上，通过对恢复的极射线应用单正交NMF，推导出锥体感知的正交NMF模型CC-NMF。

Result: 在16个基准基因表达、文本和图像数据集上，CC-NMF在聚类纯度方面一致匹配或优于强基线NMF方法，包括乘法更新、ANLS、投影NMF、ONMF和稀疏NMF。

Conclusion: 显式恢复数据锥体能够产生既有理论依据又在实证上强大的基于NMF的聚类方法，验证了几何视角在NMF研究中的重要性。
...
### [161] [Semi-Supervised Contrastive Learning with Orthonormal Prototypes](https://arxiv.org/abs/2512.07880)
*Huanran Li,Manh Nguyen,Daniel Pimentel-Alarcón*

Main category: cs.LG

TL;DR: 本文提出CLOP损失函数，通过促进类别嵌入的正交线性子空间形成来防止对比学习中的维度坍缩问题，在图像分类和目标检测任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 对比学习在深度学习中表现优异，但维度坍缩问题（嵌入收敛到低维空间）在半监督和自监督设置中尤为严重，限制了表示学习的效果。

Method: 首先识别了标准对比损失收敛到坍缩解的学习率阈值，然后提出CLOP损失函数，通过促进类别嵌入的正交线性子空间形成来防止维度坍缩。

Result: 在真实和合成数据集上的实验表明，CLOP在图像分类和目标检测任务中提高了性能，同时在不同的学习率和批量大小下表现出更好的稳定性。

Conclusion: CLOP是一种有效的半监督损失函数，能够防止对比学习中的维度坍缩问题，提高模型性能并增强训练稳定性。
...
### [162] [GSPN-2: Efficient Parallel Sequence Modeling](https://arxiv.org/abs/2512.07884)
...
TL;DR: GSPN-2是对GSPN的算法-系统联合重设计，通过单次2D内核启动、共享内存优化和紧凑通道传播策略，显著提升了视觉Transformer在高分辨率图像和长视频应用中的计算效率。


<details>
  <summary>Details</summary>
Motivation: 尽管GSPN通过行扫描传播方案将自注意力计算成本降低到接近线性，但现有实现仍存在GPU内核重复启动开销大、全局内存数据传输过多以及每通道传播权重计算冗余的问题。

Method: 1. 系统优化：将数千次微启动合并为单个2D内核，为每个通道切片固定一个warp，在共享内存中暂存前一列激活值；2. 算法优化：引入紧凑通道传播策略，替代每通道矩阵，减少参数并自然对齐Transformer注意力中的亲和力映射。

Result: GSPN-2在图像分类和文本到图像合成任务中均表现出色，在显著降低计算成本的同时达到了Transformer级别的准确率，为视觉应用中的全局空间上下文建模建立了新的效率边界。

Conclusion: GSPN-2通过结构化矩阵变换和GPU优化实现的独特组合，为高分辨率图像和长视频相关应用提供了高效的视觉Transformer解决方案，在保持准确性的同时大幅提升了计算效率。
...
### [163] [Bridging the Clinical Expertise Gap: Development of a Web-Based Platform for Accessible Time Series Forecasting and Analysis](https://arxiv.org/abs/2512.07992)
*Aaron D. Mullen,Daniel R. Harris,Svetla Slavova,V. K. Cody Bumgardner*

Main category: cs.LG

TL;DR: 开发了一个用于时间序列预测的Web平台，使研究人员和临床医生能够轻松分析数据、训练模型和解释结果，无需深厚技术背景


<details>
  <summary>Details</summary>
Motivation: 时间序列预测在医疗健康等领域有广泛应用，但技术门槛高，需要专业知识分析数据、构建模型和解释结果，这成为使用这些技术的障碍

Method: 开发了一个Web平台，支持用户上传数据、生成可视化图表，提供多种可高度定制的预测模型和训练技术，并集成大语言模型提供参数建议和结果解释

Result: 平台使研究人员和临床医生能够轻松进行时间序列预测分析，通过可视化展示变量关系，支持多种模型选择，并获得智能化的参数建议和结果解释

Conclusion: 该平台旨在降低时间序列预测的技术门槛，计划整合到学习型医疗系统中，实现从临床管道中持续收集数据和进行推理
...
### [164] [Benchmarking Offline Multi-Objective Reinforcement Learning in Critical Care](https://arxiv.org/abs/2512.08012)
*Aryaman Bansal,Divya Sharma*

Main category: cs.LG

TL;DR: 本文在ICU临床决策中比较了多目标强化学习与单目标方法，发现基于决策变换器的PEDA DT算法在离线学习环境下能提供更灵活的策略选择，适应不同临床优先级。


<details>
  <summary>Details</summary>
Motivation: ICU临床决策需要平衡患者生存率与资源利用（如住院时长）的冲突目标。传统单目标强化学习方法使用固定标量化奖励函数，产生僵化策略，无法适应变化的临床优先级。多目标强化学习能学习帕累托前沿上的最优策略集，允许在测试时动态选择偏好，但医疗领域需要严格的离线学习。

Method: 在MIMIC-IV数据集上，对三种离线多目标强化学习算法（CPQL、Adaptive CPQL、PEDA DT）与三种标量化单目标基线方法（BC、CQL、DDQN）进行基准测试。使用离线策略评估指标比较性能。

Result: PEDA DT算法相比静态标量化基线展现出更优的灵活性。结果扩展了先前关于医疗领域单目标决策变换器的发现，证实序列建模架构在扩展到多目标条件生成时仍保持鲁棒性和有效性。

Conclusion: 离线多目标强化学习是一个有前景的框架，能够在无需重新训练的情况下实现重症监护中个性化、可调整的决策制定。
...
### [165] [CLARITY: Medical World Model for Guiding Treatment Decisions by Modeling Context-Aware Disease Trajectories in Latent Space](https://arxiv.org/abs/2512.08029)
...
Method: CLARITY是一个医学世界模型，直接在结构化潜在空间中预测疾病演化。它明确整合时间间隔（时间背景）和患者特定数据（临床背景），将治疗条件下的进展建模为平滑、可解释的轨迹，从而生成生理上可信的个体化治疗计划。模型还引入了新颖的预测到决策框架，将潜在推演转化为透明、可操作的建议。

Result: CLARITY在治疗规划方面展示了最先进的性能。在MU-Glioma-Post数据集上，该方法比最近的MeWM模型表现优异12%，并显著超越所有其他医学特定的大型语言模型。

Conclusion: CLARITY通过整合时间和临床背景，在结构化潜在空间中预测疾病演化，为肿瘤学临床决策提供了有效的个体化治疗规划方法，显著优于现有方法。
...
### [166] [LUNA: Linear Universal Neural Attention with Generalization Guarantees](https://arxiv.org/abs/2512.08061)
*Ashkan Shahbazi,Ping He,Ali Abbasi,Yikun Bai,Xinran Liu,Elaheh Akbari,Darian Salehi,Navid NaderiAlizadeh,Soheil Kolouri*

Main category: cs.LG

TL;DR: LUNA是一种可学习的核化线性注意力机制，在保持线性计算成本的同时，匹配甚至超越传统二次注意力机制的准确性。


<details>
  <summary>Details</summary>
Motivation: 传统softmax注意力存在O(n²)的二次计算成本，限制了其在长序列领域的应用。现有的线性注意力机制虽然将成本降至O(n)，但依赖于固定的随机特征映射，导致模型准确性和计算效率之间存在根本性权衡。

Method: LUNA提出可学习的核特征映射，而不是固定的先验特征。通过参数化核函数，LUNA学习针对特定数据和任务的特征基，克服固定特征方法的表达能力限制。该方法实现为正定核，并支持流式形式，在序列长度上具有线性的时间和内存扩展性。

Result: 在Long Range Arena (LRA)上，LUNA在计算对等条件下（相同参数量、训练步数和近似FLOPs）实现了高效Transformer中的最先进平均准确率。在BERT和ViT-B/16检查点的后验转换中，LUNA替换softmax并进行简短微调后，能恢复大部分原始性能，显著优于固定线性化方法。

Conclusion: LUNA通过可学习的核特征映射消除了线性注意力机制中准确性和效率之间的权衡，在保持线性计算成本的同时，能够匹配甚至超越传统二次注意力机制的性能，为长序列处理提供了有效的解决方案。
...
### [167] [Deep Kernel Aalen-Johansen Estimator: An Interpretable and Flexible Neural Net Framework for Competing Risks](https://arxiv.org/abs/2512.08063)
*Xiaobin Shen,George H. Chen*

Main category: cs.LG

TL;DR: 提出了一种可解释的深度竞争风险模型DKAJ，通过自动学习的核函数将数据点表示为聚类的加权组合，在保持与经典Aalen-Johansen估计器一致性的同时提供可视化解释。


<details>
  <summary>Details</summary>
Motivation: 现有竞争风险模型缺乏可解释性，难以理解模型决策过程。需要一种既能保持经典Aalen-Johansen估计器统计特性，又能提供可视化解释的深度学习方法。

Method: 提出Deep Kernel Aalen-Johansen (DKAJ)估计器，将每个数据点表示为聚类的加权组合。通过自动学习的核函数度量数据点相似性，当数据点仅对一个聚类有非零权重时，其预测CIFs对应经典Aalen-Johansen估计器在该聚类上的结果。

Result: 在四个标准竞争风险数据集上，DKAJ与最先进的基线方法竞争性相当，同时能够提供可视化辅助模型解释。

Conclusion: DKAJ成功地将经典非参数估计器推广到深度学习框架，在保持预测性能的同时提供了模型可解释性，为竞争风险分析提供了新的工具。
...
### [168] [CAMO: Causality-Guided Adversarial Multimodal Domain Generalization for Crisis Classification](https://arxiv.org/abs/2512.08071)
*Pingchuan Ma,Chengshuai Zhao,Bohan Jiang,Saketh Vishnubhatla,Ujun Jeong,Alimohammad Beigi,Adrienne Raglin,Huan Liu*

Main category: cs.LG

TL;DR: 本文提出了一种因果引导的多模态领域泛化框架，通过对抗解缠和统一表示学习，提升社交媒体危机分类在未见灾难类型上的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 社交媒体危机分类需要从多模态帖子中提取可操作的灾害相关信息，这对增强态势感知和促进及时应急响应至关重要。然而，现有方法在未见危机类型上泛化能力差，主要因为：1. 未能解缠虚假特征和因果特征，导致领域偏移时性能下降；2. 未能在共享空间中对齐异构模态表示，阻碍了单模态领域泛化技术向多模态设置的直接迁移。

Method: 提出因果引导的多模态领域泛化框架，结合对抗解缠与统一表示学习。对抗目标鼓励模型解缠并关注领域不变的因果特征，实现基于稳定因果机制的更泛化分类。统一表示将不同模态的特征在共享潜在空间中对齐，使单模态领域泛化策略能够无缝扩展到多模态学习。

Result: 在不同数据集上的实验表明，该方法在未见灾难场景中取得了最佳性能。

Conclusion: 通过因果引导的多模态领域泛化框架，有效解决了社交媒体危机分类在未见灾难类型上的泛化挑战，结合对抗解缠和统一表示学习，显著提升了模型的跨领域泛化能力。
...
### [169] [Scalable Offline Model-Based RL with Action Chunks](https://arxiv.org/abs/2512.08108)
*Kwanyoung Park,Seohong Park,Youngwoon Lee,Sergey Levine*

Main category: cs.LG

TL;DR: 本文提出MAC方法，通过动作块模型和拒绝采样解决离线模型强化学习中长时域任务的挑战，在包含1亿条转移的大规模数据集上取得最佳性能。


<details>
  <summary>Details</summary>
Motivation: 研究基于模型的强化学习（特别是基于模型的价值扩展）能否为离线RL中的复杂长时域任务提供可扩展解决方案。传统方法中，更大的n值减少价值自举偏差，但会放大长时域累积模型误差。

Method: 提出MAC方法：1）使用动作块模型预测动作序列（而非单个动作）的未来状态，减少复合误差；2）采用拒绝采样从表达性行为动作块策略中采样，防止模型因分布外动作而被利用。

Result: 在包含高达1亿条转移的大规模数据集上的高挑战性任务实验中，MAC在离线基于模型的RL算法中表现最佳，特别是在具有挑战性的长时域任务上。

Conclusion: MAC通过动作块建模和拒绝采样有效解决了离线模型强化学习中长时域任务的偏差-误差权衡问题，为复杂长时域任务提供了可扩展的解决方案。
...
### [170] [Balanced Accuracy: The Right Metric for Evaluating LLM Judges - Explained through Youden's J statistic](https://arxiv.org/abs/2512.08121)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型评估依赖于分类器（如LLM-as-a-judge或人工标注）来估计期望/非期望行为的流行率。传统指标如准确率、精确率、F1对类别不平衡和正类选择敏感，可能导致选择扭曲流行率估计的分类器，影响模型比较的可信度。

Method: 提出使用Youden's J统计量作为选择最佳分类器的理论依据，证明平衡准确率是J的线性变换。通过理论分析和实证模拟，展示基于平衡准确率选择分类器的优势。

Result: Youden's J统计量与选择最佳分类器进行模型比较的理论目标一致，平衡准确率作为其等价线性变换，能更稳健地选择分类器，减少流行率估计的偏差。

Conclusion: 在LLM评估中，应使用平衡准确率（或Youden's J）而非传统指标来选择分类器，以确保模型比较的可靠性和流行率估计的准确性。
...
### [171] [Improving the Sensitivity of Backdoor Detectors via Class Subspace Orthogonalization](https://arxiv.org/abs/2512.08129)
*Guangmingmei Yang,David J. Miller,George Kesidis*

Main category: cs.LG

TL;DR: 提出一种新的后训练后门检测方法CSO，通过抑制内在特征来增强对隐蔽后门的检测能力


<details>
  <summary>Details</summary>
Motivation: 现有后门检测方法依赖目标类表现出极端异常统计值，但在两类情况下会失效：1）某些非目标类本身容易区分，自然获得极端统计值；2）后门特征相对内在特征过于隐蔽。需要更敏感的检测器来应对这些挑战。

Method: 提出类子空间正交化（CSO）方法，利用少量干净样本，通过约束优化问题在优化检测统计量的同时与类的内在特征正交化，从而抑制内在特征贡献，突出后门触发器的贡献。

Result: CSO方法能够有效检测具有挑战性的混合标签攻击和自适应攻击，相比现有方法具有更高的检测灵敏度。

Conclusion: 通过抑制内在特征贡献，CSO方法能够更敏感地检测隐蔽后门，为后训练后门检测提供了新的有效解决方案。
...
### [172] [Biothreat Benchmark Generation Framework for Evaluating Frontier AI Models I: The Task-Query Architecture](https://arxiv.org/abs/2512.08130)
...
<details>
  <summary>Details</summary>
Motivation: 随着前沿AI模型（尤其是大语言模型）的快速发展，模型开发者和政策制定者需要量化并减轻这些模型可能被用于生物恐怖主义或获取生物武器的风险。现有基准测试往往忽略了威胁的关键方面，如不同行为者的能力水平和操作风险因素。

Method: 开发了生物威胁基准生成框架，采用分层结构的生物威胁类别、要素和任务，构建了细菌生物威胁模式。该框架基于任务对齐的查询设计，为后续将查询转化为模型提示和模型评估奠定了基础。

Result: 提出了细菌生物威胁模式作为BBG框架的第一个组成部分，该模式提供了一个可重复使用的结构，用于评估LLM在细菌生物风险方面的潜在危害，涵盖了技术性和操作性要求，并考虑了广泛的生物对手能力谱系。

Conclusion: BBG框架（包括细菌生物威胁模式）旨在为评估LLM在细菌生物风险方面提供稳健、可重复使用的评估结构，全面捕捉生物对手的技术和操作要求，并考虑不同能力水平的对手，从而更准确地评估AI模型的生物安全风险提升和一般危害潜力。
...
### [173] [Robust Agents in Open-Ended Worlds](https://arxiv.org/abs/2512.08139)
*Mikayel Samvelyan*

Main category: cs.LG

TL;DR: 该论文通过开放性和多智能体学习方法，训练和评估能够泛化到新环境、分布外输入以及与其他智能体交互的鲁棒AI智能体，涉及MiniHack框架、Maestro对抗课程生成、足球游戏中的质量多样性方法，以及针对LLM的对抗提示鲁棒性研究。


<details>
  <summary>Details</summary>
Motivation: 随着AI在各种应用中的普及，需要能够成功导航和适应不断变化的开放世界的智能体。关键挑战是确保这些AI智能体具有鲁棒性，不仅在训练期间熟悉的设置中表现出色，还能有效地泛化到以前未见过的多样化场景。

Method: 1. 引入MiniHack：基于NetHack游戏的沙盒框架，通过程序内容生成创建多样化环境；2. 提出Maestro：生成对抗课程的方法，逐步增强RL智能体在双人零和游戏中的鲁棒性和泛化能力；3. 使用质量多样性方法在足球游戏领域系统识别预训练RL策略的漏洞；4. 使用进化搜索生成多样化有效输入，诊断和增强LLM对抗对抗提示的鲁棒性。

Result: 开发了能够创建多样化任务的MiniHack框架；提出了增强RL智能体鲁棒性的Maestro对抗课程生成方法；在复杂足球游戏中识别了最先进RL策略的漏洞；建立了评估和增强LLM对抗对抗提示鲁棒性的方法。

Conclusion: 该工作为AI鲁棒性的未来发展铺平了道路，使智能体不仅能够适应不断变化的世界，还能在面对不可预见的挑战和交互时茁壮成长。通过开放性和多智能体学习方法的结合，为构建更鲁棒、更通用的AI系统提供了系统性的解决方案。
...
### [174] [PolyLingua: Margin-based Inter-class Transformer for Robust Cross-domain Language Detection](https://arxiv.org/abs/2512.08143)
...
<details>
  <summary>Details</summary>
Motivation: 语言识别是多语言系统的关键第一步，现有工具在特定场景（如音乐请求）中表现不佳，开源工具准确率低，大语言模型成本高，不适合低延迟或低资源环境

Method: 采用轻量级Transformer架构，结合实例级分离和类级对齐的两级对比学习框架，使用自适应边距，为相近语言生成紧凑且分离良好的嵌入表示

Result: 在Amazon Massive数据集上达到99.25% F1分数，在Song数据集上达到98.15% F1分数，性能超过Sonnet 3.5，参数量减少10倍

Conclusion: PolyLingua在保持高精度的同时显著降低计算成本，特别适合计算资源和延迟受限的环境，解决了现有语言识别工具在特定场景下的不足
...
### [175] [TreeGRPO: Tree-Advantage GRPO for Online RL Post-Training of Diffusion Models](https://arxiv.org/abs/2512.08153)
...
Method: 将去噪过程重构为搜索树，从共享的初始噪声样本出发，策略性地分支生成多个候选轨迹，同时高效重用它们的共同前缀。采用树结构实现多子分支，每前向传递可进行多次策略更新。

Result: 在扩散模型和流模型上的实验表明，TreeGRPO实现2.4倍训练加速，在效率-奖励权衡空间中建立更优的帕累托前沿，在多个基准测试和奖励模型上持续优于GRPO基线。

Conclusion: TreeGRPO通过树结构方法提供高样本效率、细粒度信用分配和摊销计算，为基于强化学习的视觉生成模型对齐提供了可扩展且有效的途径。
...
### [176] [MobileFineTuner: A Unified End-to-End Framework for Fine-Tuning LLMs on Mobile Phones](https://arxiv.org/abs/2512.08211)
...
Result: 在真实手机上成功微调了GPT-2、Gemma 3和Qwen 2.5模型，实验验证了优化策略的有效性，证明MobileFineTuner是设备端LLM训练研究的可行基础。

Conclusion: MobileFineTuner填补了在普通移动设备上进行实际LLM微调的空白，为利用私有数据同时保护隐私的设备端训练提供了实用解决方案。
...
### [177] [Correction of Decoupled Weight Decay](https://arxiv.org/abs/2512.08217)
*Jason Chuan-Chih Chou*

Main category: cs.LG

TL;DR: 本文挑战了传统AdamW中权重衰减与学习率γ成比例设置的假设，提出权重衰减应与γ²成比例，基于稳态时权重范数稳定的理论推导，并在Scion优化器中验证了该设置能改善训练动态和模型性能。


<details>
  <summary>Details</summary>
Motivation: 传统AdamW优化器中，解耦权重衰减（decoupled weight decay）一直默认与学习率γ成比例设置，但缺乏理论依据。最近有研究者基于正交性论证提出权重衰减应与γ²成比例，本文旨在深入探讨这一假设的正确性及其对训练动态的影响。

Method: 通过理论分析，推导出在稳态假设下（更新与权重无关），权重衰减应与γ²成比例才能保持权重范数稳定。在Scion优化器中进行实证验证，分析总更新贡献（TUC）与动量相关的有效学习率的关系，比较不同权重衰减设置对权重和梯度范数稳定性的影响。

Result: 研究发现：1）消除更新垂直分量对权重范数的贡献对训练动态影响很小；2）权重衰减∝γ²能实现稳定的权重范数；3）在Scion优化器中，TUC更好地由动量相关的有效学习率表征；4）权重衰减∝γ²设置能稳定权重和梯度范数，改善训练动态控制并提升模型性能。

Conclusion: 解耦权重衰减应与学习率的平方（γ²）而非线性比例（γ）设置，这一设置基于稳态时权重范数稳定的理论推导，能更好地控制训练动态，稳定权重和梯度范数，最终提升模型性能，为优化器设计提供了新的理论指导。
...
### [178] [SPROCKET: Extending ROCKET to Distance-Based Time-Series Transformations With Prototypes](https://arxiv.org/abs/2512.08246)
*Nicholas Harner*

Main category: cs.LG

TL;DR: SPROCKET是一种基于原型的时间序列分类特征工程方法，通过原型选择实现随机卷积核变换，在UCR和UEA数据集上表现优异，与现有卷积算法性能相当，其集成模型MR-HY-SP超越了之前的最佳卷积集成模型HYDRA-MR。
...
Method: 提出了SPROCKET（Selected Prototype Random Convolutional Kernel Transform）方法，这是一种基于原型的特征工程策略。该方法通过原型选择实现随机卷积核变换，并构建了MR-HY-SP集成模型（MultiROCKET-HYDRA-SPROCKET）。

Result: 在大多数UCR和UEA时间序列分类数据集上，SPROCKET达到了与现有卷积算法相当的性能。MR-HY-SP集成模型的平均准确率排名超过了之前的最佳卷积集成模型HYDRA-MR。
...
<details>
  <summary>Details</summary>
Motivation: 癫痫猝死（SUDEP）和急性缺血性中风是危及生命的疾病，涉及皮层、脑干和自主神经系统的复杂相互作用。需要一种统一的方法来整合多模态神经生理信号，实现早期检测和风险分层。
...
### [180] [gHAWK: Local and Global Structure Encoding for Scalable Training of Graph Neural Networks on Knowledge Graphs](https://arxiv.org/abs/2512.08274)
*Humera Sabir,Fatima Farooq,Ashraf Aboulnaga*

Main category: cs.LG

TL;DR: gHAWK是一个用于大规模知识图谱的GNN训练框架，通过预计算结构特征来提升训练效率和模型性能


<details>
  <summary>Details</summary>
Motivation: 现有消息传递GNN在处理大规模知识图谱时存在扩展性问题，因为迭代的消息传递过程效率低下，特别是在小批量训练中，节点只能看到其邻域的局部视图

Method: gHAWK引入预处理步骤：1) 使用Bloom过滤器紧凑编码局部邻域结构；2) 使用TransE嵌入表示节点在图中全局位置；3) 将这些结构特征与领域特定特征融合，生成节点特征向量，可集成到任何GNN技术中

Result: 在Open Graph Benchmark的大规模数据集上，gHAWK在节点属性预测和链接预测任务上实现了最先进的准确性和更低的训练时间，在三个图谱上位居OGB排行榜首位

Conclusion: 通过预计算结构特征并与消息传递训练结合，gHAWK显著减少了内存使用，加速了收敛，并提高了模型准确性，为大规模知识图谱的GNN训练提供了可扩展的解决方案
...
### [181] [Jacobian Aligned Random Forests](https://arxiv.org/abs/2512.08306)
*Sarwesh Rauniyar*
...
<details>
  <summary>Details</summary>
Motivation: 轴对齐决策树在处理具有旋转或特征交互依赖的决策边界时表现不佳，而倾斜森林虽然能解决这个问题但计算成本高且实现复杂。需要一种既能处理复杂边界又能保持轴对齐决策树简单性的方法。

Method: JARF（Jacobian-Aligned Random Forests）方法：1）先训练轴对齐森林估计类别概率或回归输出；2）计算预测相对于每个特征的有限差分梯度；3）聚合梯度为期望雅可比外积（EGOP的推广）；4）使用该全局线性预处理器旋转特征空间；5）在变换后的数据上重新训练标准轴对齐森林。

Result: 在表格分类和回归基准测试中，这种预处理器方法持续改进了轴对齐森林的性能，经常匹配甚至超过倾斜森林基线，同时提高了训练时间。实验表明监督预处理器能恢复倾斜森林的大部分准确性，同时保持轴对齐树的简单性和鲁棒性。

Conclusion: 监督预处理器方法能有效处理复杂决策边界，在保持轴对齐决策树简单性和训练效率的同时，达到与倾斜森林相当的性能，为处理表格数据中的旋转和交互依赖边界提供了实用解决方案。
...
### [182] [Minimizing Layerwise Activation Norm Improves Generalization in Federated Learning](https://arxiv.org/abs/2512.08314)
...
<details>
  <summary>Details</summary>
Motivation: 现有联邦学习方法可能导致全局模型收敛到"尖锐最小值"，从而损害模型的泛化能力。需要改进联邦学习框架以获得更好的泛化性能。

Method: 引入平坦性约束的联邦学习优化问题，通过约束训练损失Hessian矩阵的最大特征值来实现。提出MAN正则化技术，在客户端侧最小化每层激活范数，理论上证明这能降低客户端损失函数层间Hessian的最大特征值。

Result: 将提出的平坦性约束优化应用于现有联邦学习技术，获得了显著改进，建立了新的最先进性能。

Conclusion: MAN正则化技术能有效提高联邦学习模型的泛化性能，通过约束优化问题的平坦性，确保收敛到平坦最小值，从而改善模型在实际应用中的表现。
...
### [183] [A Multivariate Bernoulli-Based Sampling Method for Multi-Label Data with Application to Meta-Research](https://arxiv.org/abs/2512.08371)
*Simon Chung,Colby J. Vorland,Donna L. Maney,Andrew W. Brown*

Main category: cs.LG

TL;DR: 提出一种考虑标签依赖关系的多标签数据集采样算法，通过估计多元伯努利分布参数和计算标签组合权重，在保持目标分布特性的同时增强少数类别的代表性。


<details>
  <summary>Details</summary>
Motivation: 多标签数据集中标签通常非互斥且频率差异大，传统采样方法难以在保持已知分布特性的同时获得足够稀有标签的样本进行推断，特别是当需要考虑标签间依赖关系时。

Method: 使用多元伯努利分布作为多标签问题的底层分布模型，利用观测到的标签频率估计分布参数，计算每个标签组合的权重，开发考虑标签依赖关系的新型采样算法。

Result: 将方法应用于Web of Science的64个生物医学主题类别的研究文章样本，成功保持了类别频率顺序，减少了最频繁和最稀有类别间的频率差异，考虑了类别依赖关系，产生了更平衡的子样本。

Conclusion: 提出的采样方法能够有效增强少数类别的代表性，同时考虑标签依赖关系，为多标签数据集中的不平衡问题提供了有效的解决方案。
...
### [184] [Biothreat Benchmark Generation Framework for Evaluating Frontier AI Models III: Implementing the Bacterial Biothreat Benchmark (B3) Dataset](https://arxiv.org/abs/2512.08459)
...
TL;DR: B3数据集作为生物威胁基准测试框架的试点实施，展示了其能够快速评估大型语言模型的生物安全风险，识别风险来源并为缓解措施提供指导。


<details>
  <summary>Details</summary>
Motivation: 前沿AI模型特别是大语言模型可能被用于生物恐怖主义或获取生物武器，这引发了政策、学术和公众的广泛担忧。模型开发者和政策制定者需要量化并缓解这些风险，因此需要开发能够评估特定模型生物安全风险的基准测试。

Method: 实施了B3（细菌生物威胁基准）数据集的试点应用，这是生物威胁基准生成框架系列论文的第三篇。试点包括：1）在样本前沿AI模型上运行基准测试；2）人工评估模型响应；3）从多个维度对结果进行应用风险分析。

Result: 试点表明B3数据集提供了一种可行且细致的方法，能够快速评估LLM的生物安全风险，识别关键风险来源，并为缓解措施的优先领域提供指导。

Conclusion: B3基准测试框架为评估AI模型的生物安全风险提供了有效的工具，能够帮助识别风险并指导缓解措施的优先级设置，有助于应对AI模型可能带来的生物安全威胁。
...
### [185] [Transformers for Multimodal Brain State Decoding: Integrating Functional Magnetic Resonance Imaging Data and Medical Metadata](https://arxiv.org/abs/2512.08462)
*Danial Jafarzadeh Jazi,Maryam Hajiesmaeili*

Main category: cs.LG

TL;DR: 提出了一种结合fMRI数据和DICOM元数据的Transformer多模态框架，用于解码脑状态，提升准确性和可解释性


<details>
  <summary>Details</summary>
Motivation: 传统机器学习和深度学习方法在处理fMRI数据时未能充分利用DICOM元数据提供的丰富上下文信息，限制了脑状态解码的性能

Method: 采用基于Transformer的架构，整合fMRI数据和DICOM元数据作为多模态输入，利用注意力机制捕捉复杂的时空模式和上下文关系

Result: 该方法提高了模型的准确性、可解释性和鲁棒性，在临床诊断、认知神经科学和个性化医疗中具有应用潜力

Conclusion: 提出的多模态Transformer框架有效利用了DICOM元数据，解决了传统方法的局限性，未来需要优化可扩展性和泛化性以应对元数据变异性和计算需求
...
### [186] [Optimal Perturbation Budget Allocation for Data Poisoning in Offline Reinforcement Learning](https://arxiv.org/abs/2512.08485)
*Junnan Qiu,Jie Li*

Main category: cs.LG

TL;DR: 提出了一种针对离线强化学习的全局预算分配攻击策略，通过基于TD误差的样本重要性分配扰动预算，相比传统均匀扰动更高效且隐蔽
...
Method: 提出全局预算分配攻击策略：基于理论洞察（样本对价值函数收敛的影响与其TD误差成正比），将攻击建模为全局资源分配问题，推导出闭式解——在全局L2约束下，扰动幅度按TD误差敏感度比例分配

Result: 在D4RL基准测试中，该方法显著优于基线策略，仅用最小扰动即可实现高达80%的性能下降，并能逃避最先进的统计和频谱防御检测

Conclusion: 全局预算分配攻击策略通过智能分配扰动预算到高TD误差样本，实现了更高效和隐蔽的离线RL数据投毒攻击，揭示了现有防御机制的脆弱性
...
### [187] [DS FedProxGrad: Asymptotic Stationarity Without Noise Floor in Fair Federated Learning](https://arxiv.org/abs/2512.08671)
*Huzaifa Arif*

Main category: cs.LG

TL;DR: 本文改进了FedProxGrad算法的收敛性分析，提出了DS FedProxGrad框架，在Robbins-Monro步长调度下证明了算法能达到渐近平稳状态，消除了方差诱导的噪声下限依赖。


<details>
  <summary>Details</summary>
Motivation: 现有FedProxGrad方法在非凸复合优化问题的群公平联邦学习中，其收敛分析仅能证明收敛到噪声主导的平稳邻域，且显式依赖于方差诱导的噪声下限。本文旨在改进这一收敛性分析。

Method: 提出了DS FedProxGrad（衰减步长FedProxGrad）分析框架，该框架包含不精确的局部近端解和显式公平正则化。在Robbins-Monro步长调度和局部不精确性的温和衰减条件下进行分析。

Result: 证明了liminf_{r→∞} E[‖∇F(x^r)‖^2] = 0，即算法是渐近平稳的，且收敛速率不依赖于方差诱导的噪声下限。

Conclusion: DS FedProxGrad框架在群公平联邦学习的非凸复合优化问题中，通过改进的收敛分析消除了噪声下限依赖，实现了渐近平稳收敛。
...
### [188] [Exposing Hidden Biases in Text-to-Image Models via Automated Prompt Search](https://arxiv.org/abs/2512.08724)
*Manos Plitsis,Giorgos Bouritsas,Vassilis Katsouros,Yannis Panagakis*

Main category: cs.LG

TL;DR: 本文提出了Bias-Guided Prompt Search (BGPS)框架，用于自动生成能最大化文本到图像模型偏见的提示词，发现了现有去偏方法未能检测到的微妙偏见。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像扩散模型存在社会偏见，但现有去偏方法依赖人工或LLM生成的提示数据集，成本高且可能遗漏未预期的偏见触发提示。

Method: 提出BGPS框架，包含：(1) 生成属性中性提示的LLM；(2) 基于TTI内部表示的属性分类器，引导LLM解码过程向放大目标图像属性的提示空间区域发展。

Result: 在Stable Diffusion 1.5和先进去偏模型上的实验发现了一系列微妙且先前未记录的偏见，严重降低了公平性指标。发现的提示词具有可解释性，在困惑度指标上优于硬提示优化方法。
...
### [189] [Neural Ordinary Differential Equations for Simulating Metabolic Pathway Dynamics from Time-Series Multiomics Data](https://arxiv.org/abs/2512.08732)
*Udesh Habaraduwa,Andrei Lixandru*

Main category: cs.LG

TL;DR: NODE模型在微生物代谢工程中显著优于传统机器学习方法，预测精度提升90%以上，推理速度加快1000倍。


<details>
  <summary>Details</summary>
Motivation: 生物系统行为预测对人类健康寿命和生物工程至关重要，但将高通量多组学数据转化为可操作的预测模型仍然存在瓶颈。需要能够从观测数据直接推断潜在相互作用的高容量数据驱动模拟系统。

Method: 引入神经常微分方程（NODE）作为动态框架，学习蛋白质组和代谢组之间的复杂相互作用。将该框架应用于工程化大肠杆菌菌株的时间序列数据，模拟代谢途径的连续动态。

Result: NODE架构在捕捉系统动态方面表现出优越性能，在柠檬烯和异戊烯醇途径数据集上，均方根误差比基线改善超过90%（柠檬烯最高94.38%，异戊烯醇最高97.65%）。NODE模型的推理时间加速了1000倍。

Conclusion: NODE模型为下一代代谢工程和生物发现提供了一个可扩展、高保真的工具，能够模拟时间轨迹并预测个性化医学和合成生物学中的下游干预效果。
...
### [190] [Can TabPFN Compete with GNNs for Node Classification via Graph Tabularization?](https://arxiv.org/abs/2512.08798)
*Jeongwhan Choi,Woosung Kang,Minseo Kim,Jongwoo Kim,Noseong Park*

Main category: cs.LG

TL;DR: TabPFN-GN将图节点分类重新表述为表格学习问题，通过特征工程将图数据转换为表格特征，在异配图上表现优于GNNs


<details>
  <summary>Details</summary>
Motivation: 基于TabPFN在表格数据和时间序列上的成功，研究者探索是否可以将图节点分类重新表述为表格学习问题，以利用基础模型的零样本泛化能力

Method: TabPFN-GN通过提取节点属性、结构特征、位置编码和可选的平滑邻域特征，将图数据转换为表格特征，使TabPFN能够直接进行节点分类，无需图特定训练或语言模型依赖

Result: 在12个基准数据集上的实验表明，TabPFN-GN在同配图上与GNNs表现相当，在异配图上始终优于GNNs

Conclusion: 有原则的特征工程可以弥合表格和图领域之间的差距，为任务特定的GNN训练和LLM依赖的图基础模型提供了实用的替代方案
...
TL;DR: 提出一种基于提升模型预测得分的反事实估计方法，使用双变量Beta分布拟合提升分数，获得反事实结果的后验分布


<details>
  <summary>Details</summary>
Motivation: 提升建模只能估计干预的因果效应（处理组与对照组的潜在结果差异），而反事实识别旨在恢复这些潜在结果的联合分布（如"如果给客户提供营销优惠，他们是否仍会流失？"）。反事实分布提供比提升更丰富的信息但更难估计，但两种方法具有协同作用：可以利用提升模型进行反事实估计

Method: 提出一种反事实估计器，将双变量Beta分布拟合到预测的提升分数上，从而获得反事实结果的后验分布。该方法除了提升建模所需的因果假设外，不需要额外的因果假设

Result: 模拟实验显示了该方法的有效性，可以应用于电信客户流失等问题，揭示标准机器学习或单独提升模型无法获得的洞察

Conclusion: 通过利用提升模型进行反事实估计，可以在不增加额外因果假设的情况下获得更丰富的反事实分布信息，为实际应用如客户流失分析提供更深入的洞察
...
Method: 提出Weather Adaptive Adversarial Perturbation Optimization (WAAPO)框架，通过通道稀疏性、空间定位和平滑性约束生成目标对抗性扰动，确保扰动在物理上真实且难以察觉。

Result: 使用ERA5数据集和FourCastNet模型，WAAPO能够生成与预定目标紧密对齐的对抗性轨迹，即使是在约束条件下。实验显示初始条件的小扰动可导致预测天气模式的显著偏差。

Conclusion: AI驱动的天气预报模型存在关键脆弱性，需要建立强大的安全防护措施来防止对抗性攻击在操作预报系统中的利用。
...
TL;DR: STD(λ)是一种改进的TD(λ)算法，专注于状态值的相对排序而非绝对误差，在二元决策问题中表现更好，能避免TD(λ)在某些情况下收敛到次优策略的问题。


<details>
  <summary>Details</summary>
Motivation: 传统TD(λ)算法使用函数逼近时，最小化的是状态值近似值与真实值之间的平方误差。但对于策略而言，状态值的相对排序比绝对值的准确性更为关键。作者发现TD(λ)在某些情况下（从最优策略开始）会收敛到次优策略，这促使他们开发一种更关注状态值相对关系的算法。

Method: 提出了STD(λ)算法，这是TD(λ)的改进版本。STD(λ)在二元决策问题中训练函数逼近器时，专注于状态值的相对关系而非绝对误差。作者进行了理论分析，包括在二状态系统中证明了STD(λ)的策略单调改进特性，并与Bertsekas的差分训练方法进行了比较。

Result: 在简单的二状态和三状态系统中，TD(λ)从最优策略开始会收敛到次优策略，在西洋双陆棋中也有类似问题。而STD(λ)在二状态系统和著名的acrobot问题的变体上取得了成功演示，证明了其有效性。

Conclusion: STD(λ)算法通过专注于状态值的相对排序而非绝对误差，在强化学习中能够提供更好的策略性能。该方法在理论上具有策略单调改进的特性，并在实验中展示了优于传统TD(λ)算法的表现。
...
<details>
  <summary>Details</summary>
Motivation: 现有的文本到运动生成模型通常不专门针对IMU传感器数据，需要一种方法来使生成的虚拟IMU数据更接近真实IMU记录，以提高下游应用（如人类活动识别）的性能

Method: 在现有扩散模型基础上，引入加速度一致性损失L_acc进行微调，该损失强制生成运动在离散二阶时间差分上的一致性，使扩散先验与IMU特定的加速度模式对齐。然后将微调后的模型集成到包含表面建模和虚拟传感器模拟的文本到IMU框架中

Result: L_acc损失相比原始模型降低了12.7%，在高动态活动（如跑步、跳跃）中改进更显著。合成IMU数据在低维嵌入中更接近真实IMU记录的分布。使用改进模型生成的合成数据进行人类活动识别，分类性能比原始扩散模型提高了8.7%，比最佳对比扩散模型提高了7.6%

Conclusion: 加速度感知的扩散细化提供了一种有效的方法来对齐运动生成和IMU合成，展示了深度学习管道在将通用文本到运动先验专门化为传感器特定任务方面的灵活性
...
### [195] [Fed-SE: Federated Self-Evolution for Privacy-Constrained Multi-Environment LLM Agents](https://arxiv.org/abs/2512.08870)
*Xiang Chen,Yuling Shi,Qizhen Lan,Yuchao Qiu,Xiaodong Gu*

Main category: cs.LG

TL;DR: Fed-SE：一个用于LLM智能体的联邦自进化框架，通过局部进化-全局聚合范式解决隐私约束下异构任务中的梯度冲突问题，提升跨环境知识迁移效果。


<details>
  <summary>Details</summary>
Motivation: LLM智能体在复杂交互任务中广泛部署，但隐私约束阻碍了集中式优化和跨动态环境的协同进化。虽然联邦学习在静态数据集上有效，但将其扩展到智能体的开放式自进化仍待探索。直接应用标准联邦学习面临挑战：异构任务和稀疏的轨迹级奖励会引入严重的梯度冲突，破坏全局优化过程的稳定性。

Method: 提出Fed-SE框架，采用局部进化-全局聚合范式。局部层面：智能体在过滤后的高回报轨迹上采用参数高效微调，实现稳定的梯度更新。全局层面：Fed-SE在低秩子空间内聚合更新，解耦环境特定的动态特性，有效减少客户端间的负迁移。

Result: 在五个异构环境中的实验表明，Fed-SE相比联邦学习基线将平均任务成功率提高了约18%，验证了其在隐私约束部署中实现鲁棒跨环境知识迁移的有效性。

Conclusion: Fed-SE框架成功解决了LLM智能体在隐私约束下的联邦自进化问题，通过局部稳定优化和全局解耦聚合，有效缓解了异构任务中的梯度冲突，实现了更好的跨环境知识迁移效果。
...
### [196] [When Tables Leak: Attacking String Memorization in LLM-Based Tabular Data Generation](https://arxiv.org/abs/2512.08875)
*Joshua Ward,Bochao Gu,Chi-Hua Wang,Guang Cheng*

Main category: cs.LG

TL;DR: LLM生成的表格数据存在隐私泄露风险，数字序列易被记忆并重现，提出LevAtt攻击方法和防御策略
...
Result: 攻击暴露了广泛的隐私泄露，在某些情况下对最先进模型实现完美成员分类；提出的防御方法能以最小的保真度和效用损失抵御这些攻击

Conclusion: LLM合成数据生成存在独特的隐私漏洞，需要有效防御；提出的防御策略能平衡隐私保护和数据质量
...
### [197] [DAO-GP Drift Aware Online Non-Linear Regression Gaussian-Process](https://arxiv.org/abs/2512.08879)
*Mohammad Abu-Shaira,Ajita Rattani,Weishi Shi*

Main category: cs.LG

TL;DR: DAO-GP是一种新型的漂移感知在线高斯过程模型，能够自动检测和适应数据分布变化，无需手动调整超参数，在非平稳环境中保持稳健的回归性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界数据集常呈现随时间演化的数据分布（概念漂移），忽视这一现象会显著降低模型预测精度。传统在线高斯过程方法存在多个关键限制：缺乏漂移感知能力、依赖固定超参数、易受数据窥探影响、缺乏原则性衰减机制以及内存效率低下。

Method: 提出DAO-GP（漂移感知在线高斯过程），这是一种完全自适应、无超参数、具有衰减机制和稀疏性的非线性回归模型。DAO-GP内置漂移检测和适应机制，能根据漂移严重程度动态调整模型行为。

Result: 广泛的实证评估证实了DAO-GP在平稳条件、多种漂移类型（突变、增量、渐进）和不同数据特征下的稳健性。分析展示了其动态适应能力、高效的内存和衰减管理以及演化诱导点。与最先进的参数和非参数模型相比，DAO-GP始终实现优越或竞争性性能。

Conclusion: DAO-GP被确立为在线非线性回归的漂移弹性解决方案，能够有效应对现实世界数据中的概念漂移问题，提供自适应、高效且性能优越的建模框架。
...
TL;DR: 提出一种基于拓扑数据分析和拓扑损失函数的无监督核密度估计带宽选择方法


<details>
  <summary>Details</summary>
Motivation: 核密度估计是机器学习、贝叶斯推断、随机动力学和信号处理中的关键技术，但其关键超参数带宽需要调整。带宽选择对控制偏差-方差权衡至关重要，影响拓扑特征的平滑程度。拓扑数据分析可以量化高维数据中的拓扑特征，但传统方法需要人工干预。

Method: 提出一种无监督学习方法，使用基于拓扑的损失函数来自动选择最优带宽。该方法利用拓扑数据分析来量化密度估计的拓扑特征，通过优化拓扑损失函数实现带宽的自动化选择。

Result: 该方法在不同维度下进行了基准测试，与传统技术相比表现出潜力。拓扑损失函数能够有效指导带宽选择，避免过平滑或欠平滑问题。

Conclusion: 基于拓扑的损失函数为核密度估计的带宽选择提供了一种有效的无监督自动化方法，在高维数据中特别有价值，因为传统可视化方法在此时不可行。
...
TL;DR: Open Polymer Challenge (OPC) 发布了首个聚合物信息学社区基准数据集，包含10K聚合物和5种性质，通过多任务预测竞赛促进可持续聚合物材料的机器学习发现。


<details>
  <summary>Details</summary>
Motivation: 机器学习在发现可持续聚合物材料方面具有巨大潜力，但进展受到缺乏大规模、高质量、开放访问的聚合物数据集的限制。
...
### [200] [The Theory of Strategic Evolution: Games with Endogenous Players and Strategic Replicators](https://arxiv.org/abs/2512.07901)
*Kevin Vallier*

Main category: cs.GT

TL;DR: 该论文提出了战略演化理论，这是一个用于描述玩家群体、策略和制度规则共同演化的通用模型。该理论将复制者动态扩展到具有内生玩家、多层次选择、创新、宪法变革和元治理的环境中。


<details>
  <summary>Details</summary>
Motivation: 现有演化博弈理论主要关注固定玩家群体和策略的演化，但现实世界中玩家、策略和制度规则是共同演化的。需要建立一个统一的理论框架来研究这种多层次、内生性的战略演化系统。

Method: 提出Poiesis堆栈作为核心数学对象，这是一个通过跨层级增益矩阵连接的战略层级层次结构。在小的增益条件下，系统具有全局Lyapunov函数，并在每个有限深度满足选择、跟踪和随机稳定性结果。

Result: 证明了该类在块扩展、创新事件、异质效用、连续策略空间和宪法演化下是封闭的。封闭定理表明在更高层级不会出现新的动态，无限制的自我修改无法保持Lyapunov结构。

Conclusion: 该理论统一了演化博弈理论、制度设计、创新动态和宪法政治经济学的结果，为长期战略适应提供了一个通用的数学模型。
...
### [201] [Selling Privacy in Blockchain Transactions](https://arxiv.org/abs/2512.08096)
*Georgios Chionas,Olga Gorelkina,Piotr Krysta,Rida Laraki*

Main category: cs.GT

TL;DR: 论文从经济学角度研究区块链交易隐私增强方法，分析隐私意识用户在拍卖机制中的行为，提出最优密封投标拍卖和荷兰拍卖变体，并设计隐私市场双边匹配机制。


<details>
  <summary>Details</summary>
Motivation: 随着区块链交易隐私需求增长，用户不仅关注交易结果，还关注其经济偏好的暴露程度。现有经济机制在隐私保护方面存在不足，需要结合密码学原语来提升机制性能。

Method: 1) 分析订单流拍卖中隐私程度对拍卖收益的影响，提出最优密封投标拍卖；2) 研究荷兰拍卖变体，用户逐步降低价格和隐私程度直到交易成交；3) 设计隐私市场双边匹配机制，采用定价机制保证激励兼容和预算平衡。

Result: 1) 在订单流拍卖中，隐私程度影响拍卖收益和用户净效用，密封投标拍卖是最优机制；2) 荷兰拍卖变体的收益随通信轮数变化；3) 双边隐私市场机制能实现最优社会福利的常数近似，同时保证激励兼容和预算平衡。

Conclusion: 通过将密码学原语与经济机制结合，可以有效提升区块链交易隐私保护，为隐私意识用户提供更好的交易体验，同时维持市场效率和激励兼容性。
...
### [202] [Beyond Revenue and Welfare: Counterfactual Analysis of Spectrum Auctions with Application to Canada's 3800MHz Allocation](https://arxiv.org/abs/2512.08106)
*Sara Jalili Shani,Kris Joseph,Michael B. McNally,James R. Wright*

Main category: cs.GT

TL;DR: 该研究使用简化的短视竞价模型成功预测了加拿大2023年3800MHz频谱拍卖结果，并通过估计的估值模拟了替代拍卖机制，证明能显著改善农村偏远地区的人口覆盖。


<details>
  <summary>Details</summary>
Motivation: 传统频谱拍卖模型依赖强均衡假设，本研究旨在开发一个更简约的行为模型，仅假设竞标者是短视和直接的，以更实用地预测拍卖结果并为政策制定者提供评估替代拍卖设计的工具。

Method: 采用线性规划框架，基于加拿大2023年3800MHz频谱拍卖的逐轮竞价数据，估计竞标者的估值；使用短视竞价模型（每轮竞标者选择在当前价格下最大化效用的组合）来预测拍卖结果；然后利用估计的估值模拟替代拍卖机制。

Result: 简约的行为模型能够有效预测实际拍卖的分配结果和价格演变；模拟的替代拍卖机制（激励农村和偏远地区部署）显著改善了服务不足地区的人口覆盖；证明该方法可用于反事实机制设计，评估不同拍卖规则对政策目标的影响。

Conclusion: 基于最小假设的行为模型足以生成可靠的反事实预测，为政策制定者提供了评估替代拍卖设计如何影响未来结果的实用工具，特别是为促进加拿大公平部署等政策目标的反事实机制设计提供了框架。
...
### [203] [Multi-agent learning under uncertainty: Recurrence vs. concentration](https://arxiv.org/abs/2512.08132)
*Kyriakos Lotidis,Panayotis Mertikopoulos,Nicholas Bambos,Jose Blanchet*

Main category: cs.GT

TL;DR: 随机正则化学习在多智能体博弈中的收敛性分析：确定性模型收敛，但随机模型不收敛，而是围绕均衡点振荡，其长期分布集中在均衡邻域内


<details>
  <summary>Details</summary>
Motivation: 研究不确定性下多智能体学习的收敛性，特别是分析随机正则化学习在连续博弈中的长期行为，与确定性、完全信息模型形成对比

Method: 分析两种随机正则化学习模型（连续时间和离散时间），研究其在连续博弈中的长期行为，重点关注强单调博弈中的动态特性

Result: 随机正则化学习动态一般不收敛，但在强单调博弈中，动态会无限次远离均衡但总是在有限时间内返回其邻域，长期分布集中在均衡邻域内；非强单调博弈中这些良好性质可能失效

Conclusion: 随机正则化学习在持续随机性和不确定性存在下有其局限性，但在强单调博弈中表现出围绕均衡的集中分布特性，这为理解多智能体学习在不确定性下的行为提供了新视角
...
### [204] [Robust equilibria in continuous games: From strategic to dynamic robustness](https://arxiv.org/abs/2512.08138)
*Kyriakos Lotidis,Panayotis Mertikopoulos,Nicholas Bambos,Jose Blanchet*

Main category: cs.GT

TL;DR: 该论文研究了连续博弈中纳什均衡的鲁棒性，包括策略不确定性和动态不确定性下的鲁棒均衡概念及其相互关系。


<details>
  <summary>Details</summary>
Motivation: 研究纳什均衡在现实博弈中的实际稳定性，考虑博弈参与者在面对策略不确定性（支付结构扰动）和动态不确定性（随机性和不确定性下的学习动态）时的均衡鲁棒性。

Method: 1. 引入策略鲁棒均衡概念，定义为对博弈支付结构的小扰动保持不变的均衡；2. 研究动态鲁棒性，分析"跟随正则化领导者"（FTRL）动态在随机性和不确定性下的稳定极限点；3. 建立两种鲁棒性概念之间的结构对应关系；4. 分析收敛到鲁棒均衡的速率与正则化器的关系。

Result: 1. 提供了策略鲁棒均衡的几何特征刻画；2. 证明了策略鲁棒性蕴含动态鲁棒性，反之，要维持动态鲁棒性就不能放松策略鲁棒性要求；3. 在仿射约束行动空间的博弈中，熵正则化学习以几何速率收敛到鲁棒均衡。

Conclusion: 该研究建立了连续博弈中两种不同鲁棒性概念之间的深刻联系，表明策略鲁棒性是动态鲁棒性的必要条件，并为熵正则化学习在特定博弈结构中的快速收敛提供了理论保证。
...
### [205] [Learning to Control Physically-simulated 3D Characters via Generating and Mimicking 2D Motions](https://arxiv.org/abs/2512.08500)
*Jianan Li,Xiao Chen,Tao Huang,Tien-Tsin Wong*

Main category: cs.GR

TL;DR: Mimic2DM：直接从2D视频关键点轨迹学习3D角色运动控制器的框架，无需3D运动数据


<details>
  <summary>Details</summary>
Motivation: 视频数据比动作捕捉数据更经济，但直接从视频合成真实多样的3D角色运动仍具挑战。现有方法依赖运动重建技术获取3D轨迹进行物理模仿，但这些方法泛化性差，要么需要3D训练数据，要么无法产生物理合理的姿态，难以应用于人机交互或非人类角色等复杂场景。

Method: 提出Mimic2DM框架：1）通过最小化重投影误差，训练通用单视图2D运动跟踪策略，仅使用2D运动数据在物理模拟中跟随任意2D参考运动；2）在不同或略微不同视角采集的多样化2D运动上训练策略，通过聚合多视图获得3D运动跟踪能力；3）开发基于Transformer的自回归2D运动生成器，集成到分层控制框架中，生成高质量2D参考轨迹指导跟踪策略。

Result: 该方法具有通用性，能够有效学习合成物理合理且多样化的运动，涵盖舞蹈、足球运球和动物运动等多个领域，完全不依赖显式3D运动数据。

Conclusion: Mimic2DM展示了直接从广泛可用的2D视频关键点轨迹学习3D角色运动控制器的可行性，为视频驱动的运动合成提供了新途径，特别适用于缺乏3D训练数据的复杂场景。
...
### [206] [AudioScene: Integrating Object-Event Audio into 3D Scenes](https://arxiv.org/abs/2512.07845)
*Shuaihang Yuan,Congcong Wen,Muhammad Shafique,Anthony Tzes,Yi Fang*

Main category: cs.SD

TL;DR: 该研究提出了两个新颖的音频空间场景数据集AudioScanNet和AudioRoboTHOR，将音频片段与空间对齐的3D场景结合，用于研究音频信号与空间环境的交互，并通过LLM推理和人工验证实现高效标注。


<details>
  <summary>Details</summary>
Motivation: 音频分析在人类计算机交互、环境监测和公共安全方面具有巨大潜力，但现有的纯音频数据集缺乏空间上下文信息，限制了音频条件任务在3D环境中的研究。

Method: 通过整合音频片段与空间对齐的3D场景创建数据集，利用大语言模型的常识推理能力关联音频事件与空间信息，并辅以严格的人工验证，实现比纯人工标注更高的可扩展性。

Result: 数据集在标注准确性、完整性和多样性方面表现优异（通过标注者间一致性评估），并在两个基准任务（音频基础的3D视觉定位和音频基础的机器人零样本导航）上验证了其有效性，同时揭示了当前音频中心方法的局限性。

Conclusion: 该研究提出的数据集为音频引导的空间学习提供了重要资源，强调了音频与空间上下文结合的实际挑战和重要性，推动了音频条件任务在3D环境中的研究进展。
...
### [207] [LocaGen: Sub-Sample Time-Delay Learning for Beam Localization](https://arxiv.org/abs/2512.07872)
*Ishaan Kunwar,Henry Cantor,Tyler Rizzo,Ayaan Qayyum*

Main category: cs.SD

TL;DR: LocaGen通过机器学习模型减少采样量化误差，提高三麦克风阵列的2D音频波束定位性能，在低功耗嵌入式系统上实现DOA误差减少约67%


<details>
  <summary>Details</summary>
Motivation: 改进2D波束定位问题中音频信号的定位性能，特别是在低采样率（仅10kHz）麦克风阵列条件下，减少采样量化误差对定位精度的影响

Method: 使用仿真生成的真实合成数据训练机器学习模型，通过模型减少采样量化误差，提高三麦克风阵列的方向到达角（DOA）和精确位置估计精度

Result: 在低功耗嵌入式系统上，LocaGen将DOA误差减少约67%，同时实时资源使用量仅最小幅度增加，即使在仅10kHz音频处理条件下也能显著提升定位精度

Conclusion: LocaGen通过机器学习方法有效解决了低采样率麦克风阵列的定位精度问题，在资源受限的嵌入式系统中实现了显著的性能提升，为实际应用提供了可行的解决方案
...
### [208] [Beyond Unified Models: A Service-Oriented Approach to Low Latency, Context Aware Phonemization for Real Time TTS](https://arxiv.org/abs/2512.08006)
*Mahta Fetrat,Donya Navabi,Zahra Dehghanian,Morteza Abolghasemi,Hamid R. Rabiee*

Main category: cs.SD

TL;DR: 本文提出了一种轻量级实时文本转语音系统框架，通过服务化架构将上下文感知音素化模块与核心TTS引擎解耦，解决了高质量音素化与实时性能之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 当前高效的TTS系统通常使用轻量级音素化器，但难以处理上下文相关的挑战；而具有更深层语言理解的高级音素化器计算成本高，无法实现实时性能。需要在音素化质量和推理速度之间找到平衡。

Method: 提出轻量级上下文感知音素化策略，并采用服务化TTS架构，将这些模块作为独立服务执行，将重型的上下文感知组件与核心TTS引擎解耦。

Result: 实验结果表明，所提系统在保持实时响应性的同时，提高了发音正确性和语言准确性，适合离线和终端设备TTS应用。

Conclusion: 通过服务化架构解耦音素化模块与TTS引擎，成功打破了延迟障碍，使高质量音素化模型能够在实时TTS系统中使用，为离线设备和终端应用提供了实用解决方案。
...
### [209] [SpeechQualityLLM: LLM-Based Multimodal Assessment of Speech Quality](https://arxiv.org/abs/2512.08238)
*Mahathir Monjur,Shahriar Nirjon*

Main category: cs.SD

TL;DR: SpeechQualityLLM：基于多模态语言模型的语音质量问答系统，通过自然语言交互评估语音质量，替代传统客观评估方法


<details>
  <summary>Details</summary>
Motivation: 传统语音质量评估方法（如PESQ、POLQA）需要严格控制条件和昂贵的听力测试，而学习模型（如NISQA）虽然能回归MOS分数但缺乏灵活性：不支持交互式自然语言查询，也不提供文本解释

Method: 结合音频编码器和语言模型的多模态语音质量问答系统，在NISQA语料库上训练，使用基于模板的问题-答案对，覆盖整体MOS和四个感知维度（噪声、染色、不连续性和响度），支持单端和双端设置

Result: 在NISQA测试集上，双端模型达到MOS平均绝对误差0.41，皮尔逊相关系数0.86，在维度任务上表现有竞争力；系统提供灵活的自然语言界面，可模拟不同听众特征，产生多样化但合理的判断

Conclusion: SpeechQualityLLM不仅提供定量评估，还提供灵活的自然语言交互能力，可作为音频质量专家，减少对大规模众包测试的依赖和成本，为语音质量评估带来新的可能性
...
### [210] [DFALLM: Achieving Generalizable Multitask Deepfake Detection by Optimizing Audio LLM Components](https://arxiv.org/abs/2512.08403)
*Yupei Li,Li Wang,Yuxiang Wang,Lei Wang,Rizhao Cai,Jie Shi,Björn W. Schuller,Zhizheng Wu*

Main category: cs.SD

TL;DR: 本文研究音频大语言模型在音频深度伪造检测中的泛化能力，通过优化音频编码器和文本LLM的组合，提出了一种能够跨域检测伪造音频并执行多种深度伪造任务的ALLM架构，在多个数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习方法在音频深度伪造检测中缺乏泛化能力，难以应对新兴伪造技术和多任务需求。虽然大语言模型具备泛化潜力，但现有音频LLM在深度伪造检测中存在泛化瓶颈，需要研究其架构组件的影响。

Method: 研究音频LLM的主要组件（音频编码器和文本LLM）对深度伪造检测性能的影响，通过精心选择和组合这些组件，提出一种能够泛化到域外伪造测试并执行多种深度伪造任务（如伪造定位和属性识别）的ALLM架构。

Result: 提出的模型架构在ASVSpoof2019、InTheWild和Demopage等多个数据集上达到最先进性能，平均准确率高达95.76%，在伪造属性识别和定位等任务上也表现出与SOTA音频理解模型竞争的能力。

Conclusion: 音频编码器和文本LLM的精心选择和组合对于释放音频LLM的深度伪造检测潜力至关重要。提出的ALLM架构能够有效泛化到域外伪造测试并执行多种深度伪造任务，为音频深度伪造检测提供了新的解决方案。
...
### [211] [Emovectors: assessing emotional content in jazz improvisations for creativity evaluation](https://arxiv.org/abs/2512.08812)
*Anna Jordanous*

Main category: cs.SD

TL;DR: 该研究提出了一种基于嵌入的方法来量化音乐即兴演奏中的情感内容，以评估爵士即兴创作的创造力，并探索将其作为自动化创造力评估指标的可能性。


<details>
  <summary>Details</summary>
Motivation: 音乐即兴创作是创造性过程的现场展示，但如何评估爵士即兴演奏的创造力？当前基于LLM的生成系统需要自动化的创造力评估指标。情感参与与即兴创作中的创造力密切相关，因此需要量化分析音乐中的情感内容。

Method: 提出基于嵌入的方法来捕捉音乐即兴演奏中的情感内容，使用基于心理学基础的音乐特征分类（与情感相关的特征），生成"情感向量"（emovectors）来分析情感内容。

Result: 通过分析多个即兴演奏的情感向量来验证假设：如果即兴演奏包含更多情感内容证据，就更可能被认为是创造性的。这种量化方法能够为创造力评估提供新指标。

Conclusion: 通过量化捕捉音乐即兴演奏中的情感内容，可以为大规模应用的创造力评估开发新的自动化指标，有助于评估基于LLM的生成系统的创造力。
...
### [212] [CrowdLLM: Building LLM-Based Digital Populations Augmented with Generative Models](https://arxiv.org/abs/2512.07890)
*Ryan Feng Lin,Keyu Tian,Hanming Zheng,Congjing Zhang,Li Zeng,Shuai Huang*

Main category: cs.MA

TL;DR: CrowdLLM提出了一种结合预训练大语言模型和生成模型的方法，用于创建更准确、更多样化的数字人群，以解决现有LLM数字人口在准确性和多样性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的数字人群研究主要依赖单一LLM，无法充分捕捉真实人群的准确性和多样性。数字人群虽然能降低招募人类参与者的成本并解决人类研究的相关问题，但现有方法在代表性方面存在局限。

Method: 提出CrowdLLM框架，整合预训练大语言模型和生成模型，通过这种混合方法增强数字人群的多样性和保真度。进行了理论分析，证明该方法在创建成本效益高、代表性充分、可扩展的数字人群方面的潜力。

Result: 在众包、投票、用户评分等多个领域和模拟研究中进行了全面实验，结果表明CrowdLLM在准确性和对人类数据的分布保真度方面都取得了有前景的性能。

Conclusion: CrowdLLM能够创建与真实人群质量相匹配的数字人群，为社交模拟、众包、营销和推荐系统等应用提供了更有效的解决方案。
...
### [213] [MARINE: Theoretical Optimization and Design for Multi-Agent Recursive IN-context Enhancement](https://arxiv.org/abs/2512.07898)
*Hongwei Zhang,Ji Lu,Yongsheng Du,Yanqin Gao,Lingjun Huang,Baoli Wang,Fang Tan,Peng Zou*

Main category: cs.MA

TL;DR: MARINE框架通过多智能体递归上下文增强，将基础模型的pass@N能力转化为接近最优的pass@1性能，显著提升LLM推理效率并减少参数需求


<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的智能体虽然具备高级推理能力，但实际应用中通常只能输出单次响应，导致大量性能潜力未能充分发挥。传统的一次性或多样本范式存在效率瓶颈，需要新的方法来提升推理性能。

Method: MARINE框架将测试时推理重新概念化为对持久参考轨迹的迭代精炼。通过多智能体递归上下文增强，系统性地将基础模型的pass@N能力转化为pass@1性能。采用最小可行批次最大化固定调用预算下的期望性能增益，以及对数增长的批次调度确保持续改进。

Result: 在BrowserComp-ZH基准测试中取得最先进结果：685B参数实现46.0% pass@1准确率。参数效率方面，80B参数模型结合MARINE可匹配独立1000B参数智能体性能，参数需求降低一个数量级以上。在固定计算预算下，MARINE能为对齐和优化过程提供比传统采样-排序策略更高质量的样本。

Conclusion: MARINE建立了参数高效推理的新范式，通过迭代精炼机制显著提升大语言模型推理性能，具有极大潜力提升后训练效率，为实际应用中的推理优化提供了理论指导和实用框架。
...
### [214] [Probabilistic Multi-Agent Aircraft Landing Time Prediction](https://arxiv.org/abs/2512.08281)
*Kyungmin Kim,Seokbin Yoon,Keumjin Lee*

Main category: cs.MA

TL;DR: 提出一个概率性多智能体飞机着陆时间预测框架，为多个飞机的着陆时间提供分布预测，提高预测准确性和不确定性量化能力。


<details>
  <summary>Details</summary>
Motivation: 飞机着陆时间预测对空中交通管理资源分配至关重要，但飞机轨迹和交通流的不确定性给预测准确性和可信度带来挑战。预测模型不仅需要提供点估计，还需要提供相关的不确定性。此外，飞机轨迹经常受到附近飞机通过空中交通管制干预（如雷达引导）的影响，因此着陆时间预测模型必须考虑空域中的多智能体交互。

Method: 提出一个概率性多智能体飞机着陆时间预测框架，该框架将多个飞机的着陆时间作为分布提供。模型考虑了飞机之间的交互影响，特别是空中交通管制干预对轨迹的影响。

Result: 使用韩国仁川国际机场终端空域的空中交通监视数据集进行评估，结果显示该模型比基线方法具有更高的预测准确性，并能量化其结果的相关不确定性。此外，模型通过其注意力分数揭示了空中交通管制的潜在模式，从而增强了可解释性。

Conclusion: 提出的概率性多智能体框架能够有效预测飞机着陆时间并提供不确定性量化，同时通过注意力机制增强了对空中交通管制模式的理解，为空中交通管理提供了更可靠和可解释的预测工具。
...
### [215] [Multi-Agent Intelligence for Multidisciplinary Decision-Making in Gastrointestinal Oncology](https://arxiv.org/abs/2512.08674)
...
Method: 提出分层多智能体框架，模拟人类多学科团队的协作工作流程，通过智能体间的协作来解决复杂的医疗推理任务。

Result: 系统获得4.60/5.00的专家评估分数，相比单一基线模型有显著提升，在推理逻辑和医学准确性方面改进最为明显。

Conclusion: 模仿式的智能体协作为肿瘤学自动决策支持提供了可扩展、可解释且临床稳健的范式。
...
### [216] [Towards Foundation Models with Native Multi-Agent Intelligence](https://arxiv.org/abs/2512.08743)
*Shuyue Hu,Haoyang Yan,Yiqun Zhang,Yang Chen,Dongzhan Zhou,Lei Bai*

Main category: cs.AI

TL;DR: 基础模型在单智能体任务上表现出色，但在多智能体环境中缺乏原生智能，需要专门的研究来填补这一能力差距。


<details>
  <summary>Details</summary>
Motivation: 虽然基础模型正成为AI智能体的"大脑"，但目前主要关注单智能体能力（如GUI交互、工具使用），而多智能体智能是下一个前沿领域。研究发现单智能体性能强并不自动转化为多智能体智能。

Method: 通过41个大语言模型的广泛实证研究，识别基础模型在多智能体环境中的四个核心能力：理解、规划、高效通信和适应，并分析单智能体与多智能体性能的关系。

Result: 实证证据表明，强大的单智能体性能并不能自动产生稳健的多智能体智能，两者之间存在显著的能力差距。
...
### [217] [See-Control: A Multimodal Agent Framework for Smartphone Interaction with a Robotic Arm](https://arxiv.org/abs/2512.08629)
...
Method: 提出ESO任务和See-Control框架，包含三个关键组件：1) 包含155个任务和评估指标的ESO基准；2) 基于MLLM的具身智能体，无需ADB或系统后端访问即可生成机器人控制命令；3) 丰富标注的操作片段数据集。

Result: See-Control通过低自由度机械臂实现了智能手机的直接物理操作，提供了平台无关的解决方案，为未来研究提供了有价值的资源。

Conclusion: See-Control通过弥合数字智能体与物理世界之间的差距，为实现家庭机器人在真实环境中执行依赖智能手机的任务提供了具体步骤。
...
### [218] [Impact of Data-Oriented and Object-Oriented Design on Performance and Cache Utilization with Artificial Intelligence Algorithms in Multi-Threaded CPUs](https://arxiv.org/abs/2512.07841)
...
TL;DR: 该研究对比了数据导向设计(DOD)与传统面向对象设计(OOD)在多线程环境下的性能表现，通过A*搜索算法的四种实现版本发现，在多线程测试中DOD在执行时间、系统调用和缓存未命中方面表现更优，但细粒度任务中单线程版本反而优于多线程版本。
...
Result: 在多线程测试中，DOD实现显示出显著的性能优势，具有更快的执行时间、更少的原始系统调用和缓存未命中。对于细粒度任务如A*算法，线程管理开销导致单线程版本在两个范式中都显著优于多线程版本。

Conclusion: 即使在简单算法中性能差异看似细微，但DOD在关键指标上的持续优势突显了其基础架构优越性，表明它是复杂大规模AI和并行计算任务中最大化硬件效率的更有效方法。
...
### [219] [SkipKV: Selective Skipping of KV Generation and Storage for Efficient Inference with Large Reasoning Models](https://arxiv.org/abs/2512.07993)
...
TL;DR: SkipKV是一种无需训练、基于句子级别的KV缓存压缩方法，通过识别和移除高度相似的句子来减少推理过程中的KV缓存开销，同时保持语义连贯性，提高推理效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在推理过程中会产生大量KV缓存，随着思维链推理过程的线性增长，导致内存和吞吐量瓶颈。现有的KV缓存驱逐方法在多批次设置中无法保持准确性，且会生成更长的序列。

Method: 提出SkipKV方法：1）使用句子评分指标识别和移除高度相似的句子；2）动态调整引导向量来更新隐藏激活状态，抑制冗余生成；3）在句子级别进行序列移除，保持语义连贯性。

Result: 在多个推理基准测试中，SkipKV在相似压缩预算下比替代方法保持高达26.7%的准确率提升，生成长度减少1.6倍，吞吐量提高1.7倍。

Conclusion: SkipKV通过句子级别的KV缓存压缩，有效解决了大型推理模型中的KV缓存开销问题，在保持准确性的同时显著提高了推理效率。
...
### [220] [Toward an AI Reasoning-Enabled System for Patient-Clinical Trial Matching](https://arxiv.org/abs/2512.08026)
...
TL;DR: 开发了一个基于LLM的AI增强患者-临床试验匹配系统，解决传统人工筛选效率低的问题，提供结构化、可解释的资格评估，支持人机协同审查。


<details>
  <summary>Details</summary>
Motivation: 临床试验患者筛选目前主要依赖人工，过程耗时、资源密集且效率低下，需要自动化解决方案来减轻协调员负担并提高匹配效率。

Method: 利用开源、具备推理能力的大型语言模型，整合异构电子健康记录数据，生成结构化资格评估和可解释的推理链，支持人机协同审查，并确保严格的安全标准。

Result: 系统将资格表示为动态状态而非固定判断，能够识别现有匹配并提供使患者未来符合资格的可操作建议，旨在减轻协调员负担、智能扩大患者可考虑的试验范围，并保证所有AI生成输出的全面可审计性。

Conclusion: 该系统为AI增强的患者-试验匹配提供了安全、可扩展的概念验证，通过可解释的推理链支持专家审查，有望显著提高临床试验筛选的效率和准确性。
...
### [221] [Large Language Models for Education and Research: An Empirical and User Survey-based Analysis](https://arxiv.org/abs/2512.08057)
*Md Mostafizer Rahman,Ariful Islam Shiplu,Md Faizul Ibne Amin,Yutaka Watanobe,Lu Peng*

Main category: cs.AI

TL;DR: 本文对ChatGPT和DeepSeek两大LLM在教育研究领域进行综合评估，包括技术分析、实证实验和用户调查，比较它们在文本生成、编程和专业问题解决方面的表现。


<details>
  <summary>Details</summary>
Motivation: 随着预训练大语言模型在教育研究领域的广泛应用，需要系统评估当前最先进的ChatGPT和DeepSeek模型在实际应用中的表现，了解它们在准确性、计算效率和用户体验方面的权衡，为教育研究领域选择合适的AI工具提供依据。

Method: 采用三种评估方法：1）背景技术分析；2）实证实验（包括文本生成、编程和专业问题解决等任务）；3）针对学生、教育工作者和研究人员的真实用户调查，全面评估模型在教育和研究事务中的表现。

Result: 实验结果显示：ChatGPT在通用语言理解和文本生成方面表现优异；DeepSeek凭借其效率导向的设计在编程任务中表现更佳；两个模型都能提供医学上准确的诊断输出并有效解决复杂数学问题；用户调查揭示了这些模型在实际应用中的具体优势和局限。

Conclusion: ChatGPT和DeepSeek在教育研究领域各有优势，ChatGPT更适合通用语言任务，DeepSeek在编程任务中表现更优。两个模型都能有效支持教育和研究工作，用户调查提供了对实际应用价值的深入洞察，为领域发展提供了重要参考。
...
### [222] [Scalable Back-End for an AI-Based Diabetes Prediction Application](https://arxiv.org/abs/2512.08147)
*Henry Anand Septian Radityo,Bernardus Willson,Reynard Tanadi,Latifa Dwiyanti,Saiful Akbar*

Main category: cs.AI

TL;DR: 开发了一个用于移动糖尿病预测应用的可扩展后端系统，采用水平扩展、数据库分片和消息队列等技术，83%的功能满足性能目标，能处理1万并发用户。
...
Conclusion: 该系统验证了可扩展后端架构的有效性，关键功能如用户管理、活动跟踪和预测操作都达到了性能要求，RabbitMQ的异步通信确保了系统在高负载下的可靠性。
...
### [223] [Beyond Traditional Diagnostics: Transforming Patient-Side Information into Predictive Insights with Knowledge Graphs and Prototypes](https://arxiv.org/abs/2512.08261)
*Yibowen Zhao,Yinan Zhang,Zhixiang Su,Lizhen Cui,Chunyan Miao*

Main category: cs.AI

TL;DR: KPI框架通过整合医学知识图谱、构建疾病原型和对比学习，解决了仅基于患者信息预测疾病时的数据不平衡和可解释性问题，同时利用大语言模型生成个性化医学解释。


<details>
  <summary>Details</summary>
Motivation: 仅基于患者信息（人口统计学和自述症状）预测疾病具有重要价值，但现有方法面临疾病分布不平衡和缺乏可解释性的挑战，导致预测结果存在偏见或不可靠。

Method: 提出KPI框架：1）整合结构化可信医学知识构建统一疾病知识图谱；2）构建具有临床意义的疾病原型；3）采用对比学习提高预测准确性；4）利用大语言模型生成患者特定的医学相关解释。

Result: 在真实世界数据集上的实验表明，KPI在预测准确性方面优于现有最先进方法，并能提供与患者叙述高度一致的临床有效解释。

Conclusion: KPI框架通过整合医学知识、提高预测准确性和增强可解释性，为以患者为中心的医疗保健服务提供了实用价值，特别适用于长尾疾病的预测。
...
### [224] [Reasoning Models Ace the CFA Exams](https://arxiv.org/abs/2512.08270)
*Jaisal Patel,Yunzhe Chen,Kaiwen He,Keyi Wang,David Li,Kairong Xiao,Xiao-Yang Liu*

Main category: cs.AI

TL;DR: 最新推理模型在CFA各级考试中表现优异，多数模型通过全部三级考试，其中Gemini 3.0 Pro在Level I创下97.6%最高分记录。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明大型语言模型在CFA考试中表现不佳，但近期推理模型在各种学科的研究生级别和专业考试中取得了优异成绩，需要评估这些最新模型在CFA考试中的实际表现。

Method: 使用980道题目组成的模拟CFA考试集进行评估，包括三级Level I考试、两级Level II考试和三级Level III考试，采用与先前研究相同的通过/未通过标准。

Result: 大多数模型通过了全部三级考试，按总体表现排序为：Gemini 3.0 Pro、Gemini 2.5 Pro、GPT-5、Grok 4、Claude Opus 4.1和DeepSeek-V3.1。Gemini 3.0 Pro在Level I创下97.6%最高分记录，GPT-5在Level II以94.3%领先，Gemini 2.5 Pro在Level III选择题中获得86.4%最高分，而Gemini 3.0 Pro在建构回答题中获得92.0%最高分。

Conclusion: 最新推理模型在CFA考试中表现出色，多数模型能够通过全部三级考试，表明这些模型在金融专业知识和推理能力方面取得了显著进步。
...
### [225] [Towards a Science of Scaling Agent Systems](https://arxiv.org/abs/2512.08296)
...
TL;DR: 该研究通过系统实验建立了智能体系统的定量扩展原则，发现工具协调权衡、能力饱和和拓扑依赖错误放大是影响多智能体性能的三大主导效应。
...
Method: 在四个多样化基准测试上评估五种典型架构（单智能体、独立、集中式、分散式、混合式），使用三个LLM家族实例化，进行包含180个配置的受控评估，使用标准化工具和令牌预算，通过经验协调指标建立预测模型。

Result: 预测模型达到交叉验证R^2=0.513；识别出三大主导效应：工具协调权衡、能力饱和（β=-0.408, p<0.001）和拓扑依赖错误放大；集中式协调在并行任务上提升80.9%性能，分散式协调在动态网页导航上表现更佳（+9.2% vs +0.2%），但所有多智能体变体在顺序推理任务上性能下降39-70%；框架对87%的保留配置预测了最优协调策略。

Conclusion: 该研究提供了一个基于可测量任务属性的智能体扩展预测原则，能够预测最优协调策略，为智能体系统设计提供了定量指导框架。
...
### [226] [rSIM: Incentivizing Reasoning Capabilities of LLMs via Reinforced Strategy Injection](https://arxiv.org/abs/2512.08300)
*Sijia Chen,Baochun Li,Di Niu*

Main category: cs.AI

TL;DR: 该论文提出了一种强化策略注入机制(rSIM)，通过小型规划器引导大语言模型的思维链，使其进化为推理语言模型(RLM)，显著提升推理能力。


<details>
  <summary>Details</summary>
Motivation: 大语言模型通过强化学习后训练可以进化为推理语言模型，展现出"顿悟"时刻，在思维链中执行自我反思和深度思考等策略。受此启发，需要一种机制来增强LLM的推理能力。

Method: 提出强化策略注入机制(rSIM)，使用小型规划器引导LLM的思维链，通过自适应注入推理策略。采用领导者-跟随者框架的多智能体强化学习，规划器作为领导者智能体，LLM作为跟随者智能体，基于简单规则奖励进行联合训练。

Result: 实验显示，rSIM使Qwen2.5-0.5B进化为RLM，性能显著超越Qwen2.5-14B。规划器具有通用性：只需训练一次即可作为插件大幅提升现有LLM的推理能力。规划器支持跨任务持续学习，规划能力逐步提升并能泛化到更广泛问题。

Conclusion: rSIM机制有效将LLM转化为RLM，通过小型规划器的策略注入显著提升推理性能，具有通用性、可插拔性和持续学习能力，为增强语言模型推理能力提供了新途径。
...
### [227] [Predicting California Bearing Ratio with Ensemble and Neural Network Models: A Case Study from Türkiye](https://arxiv.org/abs/2512.08340)
...
<details>
  <summary>Details</summary>
Motivation: 传统CBR实验室测试方法耗时、昂贵且不适用于大规模或多样化土壤剖面，需要开发更快速、精确的预测方法。人工智能和机器学习的发展为复杂土壤行为的建模提供了数据驱动的新途径。

Method: 收集土耳其不同地理气候区域的382个土壤样本，包含与承载能力相关的物理化学特性。测试了12种机器学习算法，包括决策树、随机森林、极端随机树、梯度提升、XGBoost、K近邻、支持向量回归、多层感知器、AdaBoost、Bagging、投票和堆叠回归器。对每个模型进行训练、验证和评估，以评估其泛化能力和鲁棒性。

Result: 随机森林回归器表现最佳，获得了0.95（训练）、0.76（验证）和0.83（测试）的强R2分数。这些结果突显了模型强大的非线性映射能力，使其成为预测性岩土工程任务的有前途工具。

Conclusion: 该研究支持将智能、数据为中心的模型集成到岩土工程中，为传统方法提供了有效的替代方案，并促进了基础设施分析和设计的数字化转型。
...
### [228] [Soil Compaction Parameters Prediction Based on Automated Machine Learning Approach](https://arxiv.org/abs/2512.08343)
*Caner Erden,Alparslan Serhat Demir,Abdullah Hulusi Kokcam,Talas Fikret Kurnaz,Ugur Dagdeviren*

Main category: cs.AI

TL;DR: 该研究提出使用自动机器学习（AutoML）方法预测土壤压实参数（最优含水率和最大干密度），通过算法自动选择和超参数优化，在异质土壤数据集上取得了比传统方法更好的预测性能。


<details>
  <summary>Details</summary>
Motivation: 传统确定土壤压实参数的方法（实验室试验和回归模型）存在劳动强度大、适用范围有限、精度不高等问题。现有机器学习模型在异质土壤数据集上预测精度和泛化能力不足，需要更有效的自动化预测方法。

Method: 采用自动机器学习（AutoML）方法，自动进行算法选择和超参数优化，以预测最优含水率（OMC）和最大干密度（MDD）。通过大量实验验证不同算法的性能。

Result: 极端梯度提升（XGBoost）算法表现最佳，在独立数据集上对MDD的R²达到80.4%，对OMC的R²达到89.1%。证明了AutoML在预测不同土壤类型压实参数方面的有效性。
...
### [229] [DeepFeature: Iterative Context-aware Feature Generation for Wearable Biosignals](https://arxiv.org/abs/2512.08379)
*Kaiwei Liu,Yuting He,Bufang Yang,Mu Yuan,Chun Man Victor Wong,Ho Pong Andrew Sze,Zhenyu Yan,Hongkai Chen*

Main category: cs.AI

TL;DR: DeepFeature：首个基于大语言模型、上下文感知的可穿戴生物信号特征生成框架，通过多源特征生成、迭代特征精炼和多层过滤验证机制，显著提升医疗应用性能


<details>
  <summary>Details</summary>
Motivation: 当前可穿戴设备生物信号特征提取方法存在三个主要问题：缺乏任务特定的上下文知识、难以在高维特征空间中找到最优设置、容易产生代码生成和自动化错误，限制了医疗应用的性能

Method: 提出DeepFeature框架，包含三个核心组件：1）多源特征生成机制，整合专家知识和任务设置；2）迭代特征精炼过程，基于特征评估反馈重新选择特征；3）多层过滤验证方法，确保特征到代码的稳健转换，避免运行崩溃

Result: 在八个不同任务上的实验评估显示，DeepFeature相比基线方法平均AUROC提升4.21-9.67%，在五个任务上优于最先进方法，其余任务上保持可比性能

Conclusion: DeepFeature作为首个LLM赋能的上下文感知特征生成框架，通过整合专家知识、迭代优化和稳健代码转换，显著提升了可穿戴生物信号特征提取的效果和可靠性
...
### [230] [Autonomous Issue Resolver: Towards Zero-Touch Code Maintenance](https://arxiv.org/abs/2512.08492)
*Aliaksei Kaliutau*

Main category: cs.AI

TL;DR: 该论文提出了一种从控制流图转向数据转换图的新范式，通过多智能体框架解决仓库级程序自动修复中的语义陷阱问题，实现了87.1%的修复成功率。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在函数级代码生成方面取得进展，但仓库级自动程序修复仍面临挑战。现有方法采用控制中心范式，迫使智能体处理复杂的目录结构和无关的控制逻辑，存在"语义陷阱"问题。
...
Conclusion: 该研究通过数据转换图和多智能体框架，为仓库级自动程序修复提供了更稳健的基础，解决了现代编码智能体中的语义陷阱问题，满足了日益依赖软件的世界对更强大基础的需求。
...
### [231] [Principles2Plan: LLM-Guided System for Operationalising Ethical Principles into Plans](https://arxiv.org/abs/2512.08536)
*Tammy Zhong,Yang Song,Maurice Pagnucco*

Main category: cs.AI

TL;DR: Principles2Plan是一个交互式研究原型，展示人类与大型语言模型如何协作生成情境敏感的伦理规则，指导自动化规划，使伦理自动化规划更实用可行。
...
Method: 开发Principles2Plan交互式原型系统：领域专家提供规划领域、问题细节和相关高级伦理原则（如善行、隐私），系统生成与这些原则一致的可操作伦理规则，用户可审查、优先排序并提供给规划器生成伦理知情计划。

Result: 创建了首个支持用户在经典规划情境中生成基于原则的伦理规则的系统，展示了人类-LLM协作在伦理自动化规划中的潜力。

Conclusion: Principles2Plan展示了人类与大型语言模型协作的潜力，使伦理自动化规划变得更加实用可行，填补了现有规划工具在伦理支持方面的空白。
...
### [232] [CogMCTS: A Novel Cognitive-Guided Monte Carlo Tree Search Framework for Iterative Heuristic Evolution with Large Language Models](https://arxiv.org/abs/2512.08609)
*Hui Wang,Yang Liu,Xiaoyu Zhang,Chaoxu Mu*

Main category: cs.AI

TL;DR: CogMCTS框架通过将LLM的认知引导机制与MCTS紧密结合，利用多轮认知反馈、双轨节点扩展和策略性突变，实现了高效的自动启发式优化，在稳定性、效率和解决方案质量上优于现有方法。
...
Method: 提出CogMCTS框架：1）多轮认知反馈整合历史经验、节点信息和负面结果；2）双轨节点扩展结合精英启发式管理平衡探索与利用；3）策略性突变修改启发式形式和参数增强多样性。
...
### [233] [Protein Secondary Structure Prediction Using Transformers](https://arxiv.org/abs/2512.08613)
*Manzi Kevin Maxime*

Main category: cs.AI

TL;DR: 本文提出了一种基于Transformer的蛋白质二级结构预测模型，通过注意力机制处理蛋白质序列数据，使用滑动窗口数据增强技术，在CB513数据集上表现出色。


<details>
  <summary>Details</summary>
Motivation: 蛋白质二级结构（如α螺旋、β折叠、无规卷曲）的预测对于理解蛋白质功能至关重要，传统方法在捕捉长程残基相互作用方面存在局限。

Method: 采用基于注意力机制的Transformer模型处理蛋白质序列数据，使用滑动窗口数据增强技术在CB513数据集上扩展训练样本，模型能够处理可变长度序列。

Result: Transformer模型展现出强大的泛化能力，能够有效捕捉局部和长程残基相互作用，在蛋白质二级结构预测任务中表现优异。

Conclusion: 基于Transformer的模型为蛋白质二级结构预测提供了一种有效方法，能够同时处理局部和长程相互作用，在序列分析任务中具有良好应用前景。
...
### [234] [A Practical Guide for Designing, Developing, and Deploying Production-Grade Agentic AI Workflows](https://arxiv.org/abs/2512.08769)
...
TL;DR: 本文提供了构建生产级智能体AI工作流的端到端工程指南，涵盖设计模式、工具集成、部署策略和九个核心最佳实践，并通过多模态新闻分析案例进行验证。
...
Method: 提出结构化工程生命周期，包括工作流分解、多智能体设计模式、模型上下文协议(MCP)、工具集成、确定性编排、负责任AI考虑和环境感知部署策略，并介绍九个核心最佳实践。

Result: 通过多模态新闻分析和媒体生成工作流的案例研究，展示了所提原则的实际应用，为构建健壮、可扩展、生产就绪的智能体AI工作流提供了基础参考。

Conclusion: 本文提供了构建生产级智能体AI系统的实用指南，结合架构指导、操作模式和实践实施见解，为开发可靠、可扩展的智能体AI工作流奠定了坚实基础。
...
### [235] [CARLoS: Retrieval via Concise Assessment Representation of LoRAs at Scale](https://arxiv.org/abs/2512.08826)
*Shahar Sarfaty,Adi Haviv,Uri Hacohen,Niva Elkin-Koren,Roi Livni,Amit H. Bermano*

Main category: cs.AI

TL;DR: CARLoS是一个大规模LoRA表征框架，通过分析650多个LoRA组件，使用CLIP嵌入和与基础模型的差异，定义方向、强度和一致性三个维度的表示，实现高效的语义检索，并在自动和人工评估中优于文本基线。


<details>
  <summary>Details</summary>
Motivation: 生成式组件（如LoRA）的快速扩散创建了一个庞大但非结构化的生态系统。现有的发现方法依赖于不可靠的用户描述或有偏见的流行度指标，影响了可用性。需要一种无需额外元数据就能表征LoRA的方法。

Method: 分析650多个LoRA组件，在各种提示和种子下进行图像生成以评估其行为。使用CLIP嵌入及其与基础模型生成的差异，定义三部分表示：方向（定义语义偏移）、强度（量化效果显著性）和一致性（量化效果稳定性）。基于这些表示开发高效的检索框架。

Result: CARLoS在自动和人工评估中优于文本基线，能够语义匹配文本查询到相关LoRA，同时过滤过于强烈或不稳定的组件。该表示还支持分析强度、一致性与版权法中实质性、意志性等法律概念的关联。

Conclusion: CARLoS提供了一个实用的LoRA分析系统，不仅改进了检索性能，还具有更广泛的相关性，能够连接技术表征与法律考量，为LoRA生态系统提供结构化分析框架。
...
### [236] [Interpolation in Knowledge Representation](https://arxiv.org/abs/2512.08833)
...
<details>
  <summary>Details</summary>
Motivation: Craig插值和一致插值在知识表示中有广泛应用，包括可解释性、遗忘、模块化和重用等，但许多知识表示形式化方法通常缺乏这些插值特性，且实际计算插值具有挑战性

Method: 深入研究了两种主要的知识表示形式化方法：描述逻辑和逻辑编程，讨论了计算插值的理论结果和实用方法

Result: 论文分析了描述逻辑和逻辑编程中插值问题的理论性质，并探讨了实际计算插值的方法

Conclusion: Craig插值和一致插值在知识表示中具有重要应用价值，需要进一步研究这两种形式化方法中的插值计算问题
...
### [237] [Same Content, Different Answers: Cross-Modal Inconsistency in MLLMs](https://arxiv.org/abs/2512.08923)
*Angela van Sprang,Laurens Samson,Ana Lucic,Erman Acar,Sennay Ghebreab,Yuki M. Asano*

Main category: cs.AI

TL;DR: 该论文提出了REST和REST+两个新基准，用于系统评估多模态大语言模型中的跨模态不一致性问题，发现当前最先进的MLLMs在不同模态间无法保持一致的推理能力。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型被训练在相同的嵌入空间中表示视觉和语言信息，但它们无法在两种模态中执行相同的任务。现有的评估方法未能系统性地评估这种跨模态不一致性问题，因此需要新的基准来量化这种不一致性。

Method: 研究者创建了REST和REST+两个基准，包含具有相同语义信息的三种模态样本（图像、文本、混合）。他们评估了15个MLLMs，分析了OCR准确性、视觉特征（文本颜色、分辨率、字体）和视觉token数量对模型性能的影响，并计算了一致性得分。

Result: 研究发现：1）最先进的MLLMs在不同模态间无法保持一致的推理能力；2）即使OCR正确，视觉特征（文本颜色和分辨率）和视觉token数量仍影响模型性能；3）一致性得分与文本和图像之间的模态差距相关，为跨模态不一致性提供了机制性解释。

Conclusion: 该研究揭示了MLLMs中存在的系统性跨模态不一致问题，提出了量化评估方法，并发现简单的模态转换（文本渲染为图像或图像渲染为文本）无法解决不一致性问题，为未来改进多模态模型的一致性提供了重要见解。
