...
Motivation: 传统方法（如下采样或频域转换）在UHD图像恢复中存在信息丢失或局部退化问题，RetinexDual旨在解决这些问题。

Method: RetinexDual包含两个子网络：SAMBA（用于反射分量校正）和FIA（用于频域中的颜色和光照校正）。

Result: 在四种UHD IR任务（去雨、去模糊、去雾和低光增强）中，RetinexDual在质量和定量上均优于现有方法。

Conclusion: RetinexDual通过独特设计和组件有效性，为UHD图像恢复提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: ENT内窥镜图像分析因设备、操作者差异及细微病变而具有挑战性，现有公共基准缺乏分类和检索支持。

Method: 提出ENTRep挑战赛，整合精细解剖分类、图像-图像和文本-图像检索，并提供双语标注数据集。

Result: 定义了三个基准任务，标准化提交协议，并在公开和私有测试集上评估性能，报告了顶级团队结果。

Conclusion: ENTRep为ENT内窥镜分析提供了多模态数据集和标准化框架，推动了该领域的研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有自监督学习方法通常独立预训练，忽略了互补性，导致模型庞大且不适用于资源受限场景。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统眼动追踪技术依赖昂贵高速相机的问题，为VR/AR设备提供更舒适的用户体验。

Method: 采用CNN_LSTM深度学习模型，结合事件相机输入数据，定位眼球中心位置。

Result: CNN_LSTM模型在眼动追踪任务中达到约81%的准确率。

Conclusion: 研究成功开发了一种低成本、高效的眼动追踪算法，未来将通过LRP进一步提升模型性能。
...
Method: 使用离散小波变换编码时间信息，空间投影层捕捉关节依赖关系，KAN层利用Lucas多项式进行高效函数逼近，最后通过逆小波变换重建运动序列。

Result: 在三个基准数据集上表现出竞争力，计算效率高。
...
<details>
  <summary>Details</summary>
Motivation: 当前基准测试在评估MLLMs的视觉能力时存在不足，无法全面测试对细微局部细节的理解和复杂推理能力。
...
<details>
  <summary>Details</summary>
Motivation: Transformer-based检测器在遮挡、细粒度定位和计算效率方面存在不足，DAMM旨在通过多模态查询和双流注意力解决这些问题。

Method: DAMM引入三种查询：基于视觉语言模型的外观查询、多边形嵌入的位置查询和随机学习查询，并通过双流交叉注意力模块分别优化语义和空间特征。

Result: 在四个挑战性基准测试中，DAMM在平均精度（AP）和召回率上达到最先进性能。

Conclusion: DAMM通过多模态查询适应和双流注意力有效提升了检测器的精度和效率。
...
<details>
  <summary>Details</summary>
Motivation: 在线多媒体内容的快速增长加剧了仇恨言论的传播，现有方法依赖视频级标注，忽略了仇恨内容的时间粒度，导致标签噪声。
...
<details>
  <summary>Details</summary>
Motivation: 生物识别系统（如基于DNN的人脸识别）易受后门攻击，现有防御机制在识别和修正投毒图像时面临挑战。

Method: 采用多数投票机制结合大型视觉语言模型检测投毒图像，并通过校准噪声修正。

Result: TrueBiometric能100%准确检测并修正投毒图像，且不影响干净图像的识别精度。

Conclusion: TrueBiometric为后门攻击提供了一种更实用、准确且有效的解决方案。
...
Method: 提出Highlight-TTA框架，结合跨模态幻觉辅助任务和主任务联合优化，采用元辅助训练方案，测试时通过辅助任务自适应调整模型。
...
Method: 构建查询数据的类原型，利用稀疏人类标注指导，引入层次原型消歧机制和密集条件随机场优化标签分配。

Result: 在稀疏标注下性能媲美或超越现有方法，使用高级骨干网络和密集标注时显著优于替代方案。

Conclusion: HOW-Seg通过人类反馈动态优化，实现了对基础和新类的高质量分割。
...
Method: 引入校准令牌作为轻量适配机制，调整潜在嵌入以对齐分布，自监督训练利用透视图像数据集。

Result: 在室内外场景中，使用单一令牌集显著优于现有方法。

Conclusion: 通过潜在嵌入调制，有效解决了鱼眼图像的深度估计问题，无需额外数据或复杂操作。
...
Method: 使用[5]提出的新方法训练前馈人工神经网络，模型包含322,430,160参数，精度为4位小数。

Result: 模型在ImageNet 2012数据集上达到98.3%准确率和99.69 Top-1率，平均每批次完美分类285.9个标签。

Conclusion: 未达到100%准确率可能是由于数据集中的双标签问题（重复图像不同标签）。
...
<details>
  <summary>Details</summary>
Motivation: 现有提示学习方法（如CoOp和CoCoOp）容易过拟合已知类别，限制了模型对新类别的泛化能力。

Method: ProMIM通过掩码可见图像块生成实例条件提示，增强特征鲁棒性，无需修改现有模型架构。
...
Method: TRKT包括关系感知知识挖掘（生成类别特定注意力图并利用光流增强）和双流融合模块（整合注意力图以优化目标定位和置信度）。

Result: 在Action Genome数据集上，TRKT实现了最先进的性能。

Conclusion: TRKT通过关系感知和运动感知的知识挖掘，显著提升了WS-DSGG的效果，解决了外部检测器的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 自监督学习在无标注数据中表现优异，但领域偏移问题（如生物医学图像中的批次效应）限制了其鲁棒性。
...
Method: 引入学习到的不确定性，指导高斯不透明度的可微更新，并通过软可微丢弃正则化策略优化渲染过程。

Result: 在多个数据集上验证了方法的优越性，例如在MipNeRF 360数据集上PSNR提升了3.27%。

Conclusion: 该方法通过自适应权重和不确定性学习，有效提升了稀疏视角下的3D合成质量，同时减少了所需的高斯分布数量。
...
<details>
  <summary>Details</summary>
Motivation: 解决边缘平台上高分辨率目标检测在有限计算资源和严格延迟要求下的挑战。

Method: 扩展了画布注意力调度方法，引入可变画布大小和可选帧率，并在NVIDIA Jetson Orin Nano上使用YOLOv11评估。

Result: 实验表明，改进方法在Waymo Open Dataset上实现了更高的mAP和召回率，优化了质量与成本的权衡。

Conclusion: 可变画布和可选帧率的引入显著提升了实时感知系统的性能，优于现有技术。
...
<details>
  <summary>Details</summary>
Motivation: 传统扩散图像压缩方法存在解码延迟高和保真度低的缺点，SODEC旨在通过单步解码和保真度改进来解决这些问题。

Method: 利用预训练的VAE模型生成信息丰富的潜在表示，用单步解码替代迭代去噪过程，并引入保真度引导模块和速率退火训练策略。

Result: SODEC在率失真感知性能上显著优于现有方法，解码速度提升超过20倍。

Conclusion: SODEC通过单步解码和保真度改进，实现了高效的图像压缩，为扩散模型在压缩领域的应用提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于学习的深度补全模型依赖有限数据，导致在分布外场景中性能显著下降，而基础模型在大规模训练中表现出色，可用于提升深度补全的鲁棒性。

Method: 利用深度基础模型从RGB图像中提取环境线索（结构和语义上下文），设计无学习参数的双空间传播方法（3D和2D空间），并引入可学习的校正模块逐步调整深度预测。

Result: 在NYUv2和KITTI数据集上训练，并在16个其他数据集上评估，模型在分布外场景中表现优异，超越现有最先进方法。

Conclusion: 提出的框架显著提升了深度补全的鲁棒性，无需大规模训练，适用于广泛的实际应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在无监督域适应中存在模态间隙问题，导致仅能传递模态不变知识，性能受限。

Method: 提出模态分离框架，分离模态组件并分别处理，设计模态差异度量分类样本，结合提示调优技术。

Result: 方法在多种设置下表现优异，性能提升达9%，计算效率提高9倍。

Conclusion: 框架有效解决了模态间隙问题，显著提升了跨模态对齐和目标域性能。
...
<details>
  <summary>Details</summary>
Motivation: 探索快速学习如何通过局部循环交互和快速权重机制在深度神经网络中实现全局上下文敏感性。

Method: 采用Vision Transformer (ViT)自编码器，结合Low-Rank Adaptation (LoRA)实现快速权重，研究熟悉性训练对全局上下文的影响。

Result: ViT的自注意力机制模拟了神经回路的流形变换，熟悉性训练使早期层与顶层表征对齐，并扩展了自注意力范围，LoRA显著增强了这些效果。

Conclusion: 熟悉性训练在分层网络中引入全局敏感性，混合快慢权重架构为研究大脑快速学习提供了计算模型。
...
Result: 在多个Re-ID数据集上取得最优结果，显著提升遮挡和细微差异处理能力，同时在标准场景保持竞争力。
...
<details>
  <summary>Details</summary>
Motivation: 视频持续学习面临高内存需求挑战，尤其是长视频和连续流数据，传统方法难以满足内存限制。

Method: 使用压缩视觉技术存储视频编码，提出CRAM方法，通过刷新视频编码解决压缩器的灾难性遗忘问题。

Result: 在EpicKitchens-100和Kinetics-700等大规模数据集上，CRAM方法显著减少内存占用（2GB内存储数千视频），性能优于现有方法。

Conclusion: CRAM方法有效解决了视频持续学习中的内存和遗忘问题，为实际应用提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像的高变异性和复杂性导致现有视觉语言模型（如CLIP）在医学领域的泛化能力不足，主要受设备差异、成像模式等混杂因素影响。

Method: MCDRL分两步：1）利用CLIP的跨模态能力识别候选病变区域并构建混杂因素字典；2）训练因果干预网络消除混杂因素影响，保留关键解剖结构信息。

Result: 实验表明MCDRL在分割准确性和泛化能力上均优于其他方法。

Conclusion: MCDRL通过因果驱动的方法有效解决了医学图像分割中的领域泛化问题，具有广泛的应用潜力。
...
Result: 提供了对当前方法在AI-UGC质量评估中表现的全面分析。
...
Result: 方法在临床相关性指标上与GPT-4o、Claude和DeepSeek Janus Pro表现相当，并引入了MedConceptEval和CCS两项新评估指标。

Conclusion: skin-SOAP框架能有效生成临床相关SOAP笔记，减轻医生负担，减少对人工标注的依赖。
...
Method: SCSSIM是一种无需训练的度量方法，通过图像的立方体层次分区统计量来量化SCS的保留情况。

Result: 实验表明SCSSIM对非构图失真具有高不变性，同时对构图失真表现出强烈的单调递减特性。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频行人重识别方法未能有效识别和选择查询-图库对中最具区分性的特征，限制了匹配效果。

Method: HAMoBE框架利用预训练大模型的多层特征，分两层级提取特征：第一层提取低层特征，第二层由专注于长时、短时和时序特征的专家组成，并通过双输入决策门控网络动态调整专家贡献。

Result: 在MEVID等基准测试中，性能显著提升（如Rank-1准确率提升13.0%）。

Conclusion: HAMoBE通过层次化和自适应特征整合，显著提升了视频行人重识别的性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前MLLMs在文档理解任务中表现优异，但在处理复杂文档中的细粒度细节（如菜单中的营养信息或长文中的免责声明）时能力不足，类似于‘在图像中找针’（NiM）。

Method: 引入NiM基准，涵盖多样化的真实文档（如报纸、菜单等），并提出Spot-IT方法，通过智能补丁选择和高斯注意力增强MLLMs的能力。

Result: 实验表明Spot-IT显著优于基线方法，尤其在需要从复杂布局中精确提取细节的场景中。
...
Method: DualMat采用双潜在空间路径（RGB和材料专用），结合特征蒸馏和整流流技术，支持高分辨率、多视图输入。

Result: 在Objaverse和真实数据上，DualMat在反照率估计和金属-粗糙度预测误差上分别提升28%和减少39%。

Conclusion: DualMat通过双路径设计和高效技术实现了PBR材料估计的显著改进，适用于图像到3D的流程。
...
Method: DecoupleCSS分为两阶段：第一阶段使用预训练的文本和图像编码器（通过LoRA适配）生成位置感知提示；第二阶段利用Segment Anything Model（SAM）生成精确分割掩码。

Result: 该方法在多种挑战性任务中实现了最先进的性能。

Conclusion: DecoupleCSS通过解耦检测与分割，有效解决了持续语义分割中的灾难性遗忘问题，提升了性能。
...
<details>
  <summary>Details</summary>
Motivation: 图像着色任务在计算机视觉中具有重要应用，但由于其高度不适定性，传统回归方法难以捕捉多模态颜色预测。
...
Method: 提出AdaFusion框架，通过压缩和对齐不同模型的瓦片级特征，并利用轻量级注意力机制根据组织表型上下文自适应融合。

Result: 在三个真实世界基准测试中，AdaFusion在分类和回归任务上均优于单个PFM，并提供模型生物语义特化的可解释性洞察。
...
<details>
  <summary>Details</summary>
Motivation: 当前扩散模型在生成长视频时存在身份漂移和视频长度限制的问题，PoseGen旨在解决这些问题。

Method: PoseGen采用上下文LoRA微调策略，在令牌级别注入主题外观以保持身份，同时在通道级别调节姿态信息以控制运动。采用分段生成方法，通过共享KV缓存和过渡过程确保背景一致性和时间平滑性。
...
Method: SMP结合边缘惩罚，提出MIAM机制训练两组低秩适配器，分别增强基础类区分性和新类泛化性；增量任务中使用MPCC策略校准分类器。

Result: 在CIFAR100、ImageNet-R和CUB200上，SMP实现了FSCIL的最先进性能，平衡了基础类和新类。

Conclusion: SMP通过边缘惩罚和适配器合并，有效解决了FSCIL中的性能平衡问题。
...
<details>
  <summary>Details</summary>
Motivation: 解决多实例学习在处理高分辨率病理图像时的高推理成本问题。

Method: 提出AHDMIL框架，包含动态多实例网络（DMIN）和双分支轻量级实例预筛选网络（DB-LIPN），通过自蒸馏和非对称蒸馏两步训练过程优化分类。

Result: 在四个公开数据集上，AHDMIL在分类性能和推理速度上均优于现有方法，例如在Camelyon16数据集上准确率提升5.3%，推理速度加快1.2倍。

Conclusion: AHDMIL通过创新的蒸馏框架和CKA分类器，显著提升了病理图像分类的效率和准确性。
...
TL;DR: FedGIN是一个联邦学习框架，通过全局强度非线性增强模块实现多模态医学图像分割，解决了数据稀缺和隐私问题，并在不同数据集场景下显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割在AI辅助诊断中至关重要，但多模态数据共享和域偏移问题限制了模型的通用性和隐私保护。

Method: 提出FedGIN框架，结合GIN模块在联邦学习中协调多模态强度分布，避免原始数据共享。

Result: 在有限数据场景下，FedGIN比无GIN的联邦学习性能提升12-18%；在完整数据集下接近集中式性能，跨模态泛化能力显著。

Conclusion: FedGIN在隐私保护下实现了高效的多模态医学图像分割，具有临床实用潜力。
...
Result: 在15类疼痛分类任务中准确率达48.41%，显著优于人类专家（21.33%）和B-SOiD（30.52%）；简化至3类时准确率达73.1%。
...
<details>
  <summary>Details</summary>
Motivation: 解决低分辨率图像中几何模式（如重复纹理、边缘或形状）的扭曲问题，避免高分辨率恢复中的伪影。

Method: 重新设计INR和编码器模块，嵌入旋转等变性能力，并提供了理论分析评估其内在等变误差。

Result: 在模拟和真实数据集上验证了方法的优越性，并可轻松集成到现有ASISR方法中。

Conclusion: 提出的方法显著提升了ASISR网络的性能，特别是在保持几何模式结构完整性方面。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常将人类和动物运动分开建模，而联合跨物种方法能提供统一表示和更好的泛化能力，但形态差异仍是挑战。

Method: X-MoGen采用两阶段架构：1）条件图变分自编码器学习规范T姿势先验；2）通过掩码运动建模生成基于文本的运动嵌入，并引入形态一致性模块。
...
Method: PhysPatch结合语义掩码初始化、SVD局部对齐损失和势场掩码细化方法，优化补丁设计。

Result: 实验表明，PhysPatch显著优于现有方法，能有效引导MLLM系统产生目标一致的感知和规划输出。

Conclusion: PhysPatch在物理可行区域部署对抗补丁，具有强实际适用性和可部署性。
...
<details>
  <summary>Details</summary>
Motivation: 现实场景中，未知类别目标常因低置信度检测、弱运动与外观约束及长期遮挡等问题挑战现有方法。

Method: MTT框架通过短时空相关性自适应聚类检测结果生成鲁棒轨迹片段，并利用位置和外观等多线索估计最佳轨迹片段分区以减少长期关联中的错误传播。

Result: 在通用多目标跟踪基准测试中表现优异。

Conclusion: MTT框架通过轨迹片段增强和灵活关联，有效解决了未知类别目标的跟踪挑战。
...
Method: 构建SPIE数据集，提出SPEX模型，采用多尺度特征聚合、令牌上下文压缩和多光谱视觉预训练等技术。

Result: 在五个公共多光谱数据集上，SPEX在提取植被、建筑和水体等典型土地覆盖类别上表现优于现有方法。
...
TL;DR: EndoMatcher是一种通用的内窥镜图像匹配方法，通过大规模多域数据预训练解决视觉条件困难和数据稀缺问题，显著提升了匹配性能。
...
<details>
  <summary>Details</summary>
Motivation: 大型多模态模型（LMMs）在视觉-语言任务中表现优异，但过多的视觉令牌导致高计算成本，现有剪枝方法简单且性能下降明显。

Method: 提出VFlowOpt框架，包括重要性图生成、渐进式剪枝模块和回收机制，并通过视觉信息流优化剪枝策略。

Result: 实验表明，VFlowOpt能剪枝90%的视觉令牌，保持性能，减少89%的KV-Cache内存，推理速度提升3.8倍。

Conclusion: VFlowOpt提供了一种高效的令牌剪枝方法，显著降低了计算成本，同时保持了多模态任务的性能。
...
Conclusion: 该方法在跨域少样本分割任务中表现优异，且无需源域数据，具有实用性和高效性。
...
### [53] [ReasoningTrack: Chain-of-Thought Reasoning for Long-term Vision-Language Tracking](https://arxiv.org/abs/2508.05221)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在融合语言与视觉特征时表现有限，且未能充分利用大模型的优势，缺乏对推理过程的解释。

Method: 结合预训练模型Qwen2.5-VL，使用SFT和GRPO优化推理与语言生成，嵌入更新的语言描述并与视觉特征融合，通过跟踪头预测目标位置。

Result: 在多个基准数据集上验证了基于推理的自然语言生成策略的有效性，并构建了TNLLT数据集。
...
### [54] [Segmenting the Complex and Irregular in Two-Phase Flows: A Real-World Empirical Study with SAM2](https://arxiv.org/abs/2508.05227)
*Semanur Küçük,Cosimo Della Santina,Angeliki Laskari*

Main category: cs.CV

TL;DR: 利用微调的Segment Anything Model (SAM v2.1)成功解决了多相流中非球形气泡的精确分割问题，仅需100张标注图像。


<details>
  <summary>Details</summary>
Motivation: 传统方法和大多数基于学习的方法假设气泡为近球形，限制了其在气泡变形、合并或破裂场景中的有效性，特别是在空气润滑系统中。

Method: 将任务视为迁移学习问题，利用现代视觉基础模型SAM v2.1进行微调。

Result: 首次证明微调的SAM v2.1能够准确分割高度非凸、不规则的气泡结构。

Conclusion: 该方法为多相流中复杂气泡分割提供了高效解决方案。
...
### [55] [ArbiViewGen: Controllable Arbitrary Viewpoint Camera Data Generation for Autonomous Driving via Stable Diffusion Models](https://arxiv.org/abs/2508.05236)
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶中任意视角图像生成潜力大，但缺乏真实数据限制了高保真生成模型的训练。

Method: 采用FAVS进行层次化特征匹配和CVC-SSL自监督学习，确保跨视角一致性。

Result: 无需额外传感器或深度图，仅需多相机图像及其位姿即可训练。

Conclusion: Arbiviewgen是首个支持多车辆配置下可控任意视角图像生成的方法。
...
### [56] [Navigating the Trade-off: A Synthesis of Defensive Strategies for Zero-Shot Adversarial Robustness in Vision-Language Models](https://arxiv.org/abs/2508.05237)
*Zane Xu,Jason Sun*

Main category: cs.CV

TL;DR: 报告综合了八篇关于视觉语言模型（如CLIP）零样本对抗鲁棒性的论文，探讨了增强对抗鲁棒性与保持零样本泛化能力之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 研究视觉语言模型在零样本对抗攻击下的鲁棒性，并探索如何在不牺牲泛化能力的情况下提升其对抗鲁棒性。
...
Method: 提出区域感知的多模态对比学习框架RegionMed-CLIP，包含自适应ROI处理器和渐进式训练策略，并构建了MedRegion-500k数据集。
...
### [58] [A Study of Gender Classification Techniques Based on Iris Images: A Deep Survey and Analysis](https://arxiv.org/abs/2508.05246)
...
### [59] [CF3: Compact and Fast 3D Feature Fields](https://arxiv.org/abs/2508.05254)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖自下而上的优化，计算成本高，且将2D特征视为绝对真值。

Method: 采用多视角2D特征的快速加权融合，训练高斯级自编码器，并引入自适应稀疏化方法优化高斯属性。

Result: 实现了仅需5%高斯数量的高效3D特征场，同时保留几何细节。

Conclusion: CF3方法在计算效率和特征表示上具有竞争力。
...
Result: 方法在117个目标上以25.4 fps的速度运行，跟踪误差为5.00 +/- 0.22 px，优于深度学习和传统方法。

Conclusion: 该方法在心脏手术中实现了高精度和实时性的荧光成像跟踪。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法因缺乏深度语义理解，常丢失关键目标并引入伪影，影响图像质量和任务性能。

Method: 两阶段框架：先初步融合多模态特征，再用SAM语义掩码指导扩散模型进行粗到细去噪生成。

Result: SGDFuse在主观和客观评估中均达到最优性能，且适应下游任务。

Conclusion: SGDFuse为图像融合核心挑战提供了强大解决方案。
...
TL;DR: 论文提出了B4DL基准和数据处理流水线，首次实现MLLM直接处理4D LiDAR数据，用于动态室外环境的时空推理。


<details>
  <summary>Details</summary>
Motivation: 解决4D LiDAR在MLLM中应用不足的问题，包括缺乏高质量标注和适合的模型架构。

Method: 引入B4DL基准、数据生成流水线及直接处理4D LiDAR的MLLM模型。

Result: 提供统一解决方案，包括数据集、基准和模型，支持动态环境的时空推理。
...
TL;DR: 提出了一种基于小波变换的双频编码方法（WGDF），通过频域特征建模提升遥感图像变化检测的精度，特别是在边缘细节和全局结构的捕捉上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法主要依赖空间域建模，特征表示多样性有限，难以捕捉细微变化区域。频域特征建模（如小波域）可以放大频率分量的细粒度差异，增强边缘变化的感知。

Method: 1. 使用离散小波变换（DWT）分解图像为高频和低频成分；2. 高频分支设计双频特征增强模块（DFFE）和频域交互差异模块（FDID）；3. 低频分支利用Transformer捕捉全局语义关系，并采用渐进上下文差异模块（PCDM）细化变化区域；4. 高低频特征协同融合。

Result: 在多个遥感数据集上的实验表明，WGDF显著减轻了边缘模糊，检测精度和鲁棒性优于现有方法。

Conclusion: WGDF通过频域特征建模和协同融合，有效提升了变化检测的性能，特别是在边缘细节和全局结构的表征上。
...
<details>
  <summary>Details</summary>
Motivation: 传统PPAT草图分析依赖心理学家经验且耗时，需要自动化方法提高效率和准确性。

Method: 提出VS-LLM方法，结合视觉和语义信息分析PPAT草图，并建立实验环境进行自动化抑郁评估。

Result: 实验结果显示，VS-LLM方法比心理学家评估方法提高了17.6%的准确性。

Conclusion: 该方法为基于PPAT草图的心理状态评估研究提供了有效工具，代码和数据集已开源。
...
### [65] [CoCAViT: Compact Vision Transformer with Robust Global Coordination](https://arxiv.org/abs/2508.05307)
*Xuyang Wang,Lingjuan Miao,Zhiqiang Zhou*

Main category: cs.CV

TL;DR: 论文提出了一种名为CoCAViT的新型视觉主干网络，通过引入Coordinator-patch Cross Attention (CoCA)机制，解决了小型模型在分布外数据上泛化性能不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有高效模型在分布外数据上表现不佳，尤其是小型模型的性能下降更为显著，表明其泛化能力不足。

Method: 通过识别架构瓶颈和不合理设计，引入CoCA机制，动态生成全局感知的token，增强局部-全局特征建模。

Result: CoCAViT-28M在ImageNet-1K上达到84.0%的top-1准确率，在多个OOD基准测试中表现优异，同时在COCO和ADE20K任务中保持低延迟。
...
Method: 利用多模态大语言模型提取关键词和视觉-文本匹配，构建高质量多模态知识图谱；采用双阶段检索策略和问题感知的多模态检索器提升检索效率与精度。

Result: 实验表明，该方法显著优于现有方法，为知识密集型VQA任务设定了新的最佳性能。

Conclusion: 通过引入结构化多模态知识图谱，mKG-RAG框架有效提升了知识密集型VQA任务的生成质量和可靠性。
...
### [67] [Textual Inversion for Efficient Adaptation of Open-Vocabulary Object Detectors Without Forgetting](https://arxiv.org/abs/2508.05323)
...
Method: 采用类似文本反转（TI）的方法，通过学习新词或改进现有词，仅需少量样本即可扩展VLM的词汇表，同时保持原始模型权重不变。

Result: 该方法在多种定量和定性实验中表现优异，避免了传统微调导致的遗忘问题，且计算开销显著低于全模型微调。

Conclusion: 提出的方法在扩展VLM词汇表的同时，保留了其原始性能，为开放词汇目标检测提供了一种高效且兼容的解决方案。
...
### [68] [3DGabSplat: 3D Gabor Splatting for Frequency-adaptive Radiance Field Rendering](https://arxiv.org/abs/2508.05343)
*Junyu Zhou,Yuyang Huang,Wenrui Dai,Junni Zou,Ziyang Zheng,Nuowen Kan,Chenglin Li,Hongkai Xiong*

Main category: cs.CV

TL;DR: 3DGabSplat提出了一种基于3D Gabor的基元，通过多方向3D频率响应提升3D场景高频细节的表示能力，同时减少冗余基元和内存开销。


<details>
  <summary>Details</summary>
Motivation: 3DGS因使用高斯函数而无法有效表示高频细节，且存在冗余基元和内存问题。

Method: 利用3D Gabor基元构建滤波器组，结合CUDA光栅化器和频率自适应机制进行优化。

Result: 实验显示3DGabSplat在渲染质量和效率上优于3DGS，PSNR提升1.35 dB，同时减少基元数量和内存消耗。

Conclusion: 3DGabSplat是一种高效且高质量的3D渲染方法，可作为插件集成到现有3DGS框架中。
...
Method: 两阶段训练：1）先验引导的对比预训练，利用临床背景指导特征提取；2）先感知的粗到细解码，逐步整合先验知识与视觉编码器隐藏状态。
...
### [70] [Cross-View Localization via Redundant Sliced Observations and A-Contrario Validation](https://arxiv.org/abs/2508.05369)
*Yongjun Zhang,Mingtao Xiong,Yi Wan,Gui-Song Xia*

Main category: cs.CV

TL;DR: Slice-Loc提出了一种两阶段的跨视角定位方法，通过切片图像生成冗余观测，并结合几何刚性和NFA模型验证可靠性，显著提升了定位精度和失败检测能力。
...
Method: Slice-Loc将查询图像切片，为每个子图像估计3-DoF位姿，通过几何刚性过滤错误位姿，并利用NFA模型量化定位可靠性。
...
### [71] [CT-GRAPH: Hierarchical Graph Attention Network for Anatomy-Guided CT Report Generation](https://arxiv.org/abs/2508.05375)
*Hamza Kalisch,Fabian Hörst,Jens Kleesiek,Ken Herrmann,Constantin Seibold*

Main category: cs.CV

TL;DR: CT-GRAPH是一种分层图注意力网络，通过将解剖区域结构化建模为图，结合全局和器官级特征，显著提升了CT报告生成的准确性。
...
### [72] [Deformable Attention Graph Representation Learning for Histopathology Whole Slide Image Analysis](https://arxiv.org/abs/2508.05382)
*Mingxi Fu,Xitong Ling,Yuxuan Chen,Jiawen Li,fanglei fu,Huaitian Yuan,Tian Guan,Yonghong He,Lianghui Zhu*

Main category: cs.CV

TL;DR: 提出了一种基于可变形注意力的图神经网络框架，用于病理图像分析，显著提升了空间依赖性和分类性能。
...
### [73] [UNCAGE: Contrastive Attention Guidance for Masked Generative Transformers in Text-to-Image Generation](https://arxiv.org/abs/2508.05399)
*Wonjun Kang,Byeongkeun Ahn,Minjae Lee,Kevin Galim,Seunghyuk Oh,Hyung Il Koo,Nam Ik Cho*

Main category: cs.CV

TL;DR: 论文提出了一种名为UNCAGE的无训练方法，通过利用注意力图优先解掩码明确表示单个对象的标记，提高了文本到图像生成的组合保真度。
...
Method: 提出UNCAGE方法，利用注意力图优先解掩码明确表示单个对象的标记，无需额外训练。

Result: UNCAGE在多个基准和指标上显著提升了组合保真度，且推理开销可忽略不计。
...
### [74] [Physical Adversarial Camouflage through Gradient Calibration and Regularization](https://arxiv.org/abs/2508.05414)
*Jiawei Liang,Siyuan Liang,Jianjie Huang,Chenxi Si,Ming Zhang,Xiaochun Cao*

Main category: cs.CV

TL;DR: 提出了一种基于梯度优化的对抗性伪装框架，通过梯度校准和去相关方法解决现有技术在多角度和距离下的优化问题，显著提高了攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 物理对抗伪装对安全关键领域（如自动驾驶）构成威胁，现有技术在多变物理环境中面临梯度优化不稳定和冲突问题。

Method: 引入梯度校准策略确保梯度更新的连续性，开发梯度去相关方法优化多角度梯度更新。

Result: 实验表明，该方法在攻击成功率上显著优于现有技术，平均提升13.46%（距离）和11.03%（角度）。

Conclusion: 该方法有效解决了物理对抗伪装中的优化问题，同时强调了更鲁棒系统设计的必要性。
...
### [75] [Smoothing Slot Attention Iterations and Recurrences](https://arxiv.org/abs/2508.05417)
...
<details>
  <summary>Details</summary>
Motivation: 冷启动查询缺乏样本特定信息，影响图像或视频第一帧的精确聚合；非第一帧的查询已具备样本特定性，需要不同的变换。

Method: 1. 预热冷启动查询，通过自蒸馏模块丰富输入信息；2. 区分第一帧和非第一帧的变换，分别使用完整和单次迭代。

Result: 在对象发现、识别和下游任务中验证了方法的有效性，直观分析了如何平滑SA的迭代和递归。

Conclusion: SmoothSA通过改进查询初始化和帧间变换，显著提升了OCL的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有显著性方法仅捕捉一阶归因，忽略了跨模态交互的复杂性。

Method: 基于加权Banzhaf交互指数，提出FIxLIP方法，并扩展解释评估指标。

Result: 在MS COCO和ImageNet-1k上的实验表明，FIxLIP优于一阶方法，并可用于模型比较。
...
TL;DR: 提出了一种基于Flow Matching（FM）的无监督异常检测与定位新范式，解决了传统流方法的表达能力限制。通过时间反转FM（rFM）将未知数据分布转换为标准高斯分布，并提出了Worst Transport（WT）位移插值方法，增强了样本轨迹的动态控制。


<details>
  <summary>Details</summary>
Motivation: 传统流方法在模型表达能力上存在局限，FM通过向量场回归提供了一种新的解决思路。

Method: 提出时间反转FM（rFM）和Worst Transport（WT）位移插值方法，构建非概率演化路径以增强动态控制。
...
TL;DR: 预处理图像通过有损压缩可以防御对抗性扰动，但缺乏全面攻击评估。本文构建强白盒和自适应攻击，发现高真实感重建图像显著增加攻击难度。


<details>
  <summary>Details</summary>
Motivation: 探讨有损压缩作为对抗性防御的有效性，并评估其在不同攻击场景下的表现。

Method: 构建白盒和自适应攻击，测试多种压缩模型，分析攻击难度与重建图像真实感的关系。

Result: 高真实感重建的压缩模型对攻击更具抵抗力，而低真实感模型易被攻破。

Conclusion: 高真实感重建图像提供内在鲁棒性，未来对抗攻击需克服真实感挑战。
...
TL;DR: 论文提出了一种基于显式提示的方法（OND），用于改进预训练基础模型在历史地图语义分割中的表现，并展示了其优于UNet基线的性能。


<details>
  <summary>Details</summary>
Motivation: 历史地图缺乏现代图像的语义一致性，导致预训练基础模型难以直接应用。

Method: 通过替换SAM模型的提示编码器为OND提示机制，并在历史地图上微调，构建SMOL-MapSeg模型。
...
TL;DR: 该研究提出了一种针对低资源语言的多模态大语言模型（MLLM）增强方法，强调语言能力和文化基础的双重目标，并通过MELLA数据集实现了性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有方法在低资源语言中表现不佳，忽视了多模态信息和文化基础的重要性。

Method: 提出双源策略，分别收集文化相关的原生网络替代文本和语言相关的MLLM生成标题，构建MELLA数据集。

Result: 在MELLA上微调后，八种语言的MLLM性能普遍提升，生成更丰富的描述。

Conclusion: 研究验证了文化知识和语言能力的双重提升，MELLA数据集为低资源语言的多模态任务提供了有效支持。
...
<details>
  <summary>Details</summary>
Motivation: 工业异常检测（IAD）通常需要大量手动工作，AutoIAD旨在通过自动化框架减少人工干预。

Method: AutoIAD采用Manager-Driven中央智能体协调多个子智能体（如数据准备、模型设计等），并结合领域知识库，实现从原始图像数据到训练模型的完整流程。

Result: 实验表明，AutoIAD在任务完成率和模型性能（AUROC）上显著优于现有框架，并通过迭代优化减少幻觉问题。

Conclusion: Manager中央智能体和领域知识库模块对生成高质量IAD解决方案至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 手性信息在形状分析中普遍存在，但现有形状描述符缺乏手性感知能力，因此开发手性特征提取器是必要的。

Method: 基于Diff3F框架，提出了一种无监督的手性特征提取流程，利用2D基础模型为形状顶点添加手性感知信息。

Result: 通过定量和定性实验验证了手性特征的有效性，在下游任务（如左右解耦、形状匹配和部分分割）中表现出实用性。

Conclusion: 该方法为形状分析中的手性问题提供了有效的解决方案，具有实际应用价值。
...
TL;DR: MagicHOI提出了一种从单目视频中重建手和物体的方法，通过利用新视角合成扩散模型的先验知识，解决了物体部分不可见的问题。
...
Method: MagicHOI整合了新视角合成扩散模型作为先验，结合可见接触约束对齐手和物体。

Result: 实验表明，MagicHOI显著优于现有方法，新视角合成先验有效规范了不可见区域的重建。

Conclusion: MagicHOI通过结合新视角合成先验和接触约束，提升了手和物体的3D重建质量。
...
TL;DR: 提出了一种自监督预训练框架，用于从稀疏且嘈杂的事件数据中提取边缘信息和纹理线索，通过三个阶段增强特征提取能力。


<details>
  <summary>Details</summary>
Motivation: 事件相机数据稀疏且嘈杂，难以有效提取特征，需要一种方法充分挖掘其潜在信息。

Method: 框架包括三个阶段：差异引导掩模建模、主干固定特征转换和聚焦对比学习，分别用于重建时间强度差异图、对比事件与图像特征以及提升语义区分能力。

Result: 实验表明，该框架在目标识别、语义分割和光流估计等任务中优于现有方法。

Conclusion: 提出的自监督预训练框架能有效提取事件数据中的潜在信息，并在多个下游任务中表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 现实应用中，行人交互或重叠时的遮挡问题导致目标特征丢失，传统跟踪方法在严重遮挡场景下表现不佳。

Method: 结合目标检测器的回归和分类分支特征，引入头部关键点检测模型以减少遮挡影响，并提出迭代卡尔曼滤波方法进行运动建模。

Result: 提出的方法在拥挤环境中表现出更强的鲁棒性，能够更好地完成复杂场景下的运动轨迹跟踪。

Conclusion: 通过结合外观和运动建模的改进，该方法为遮挡普遍存在的多目标跟踪提供了更鲁棒的解决方案。
...
Method: 分析主干网络Jacobian矩阵的最大奇异值，将特征空间噪声水平与输入空间扰动关联，适用于全参考和无参考IQA模型，无需架构修改。
...
<details>
  <summary>Details</summary>
Motivation: 解决临床试验数据清理中手动流程效率低下、错误率高的问题。
...
TL;DR: 本文提出了一种名为Optimal Brain Connection的结构剪枝框架，通过Jacobian Criterion评估参数重要性，并利用Equivalent Pruning机制保留原始连接贡献，有效减少性能损失。
...
Method: 提出Jacobian Criterion评估参数重要性，并设计Equivalent Pruning机制通过自动编码器保留原始连接贡献。

Result: Jacobian Criterion在保持模型性能上优于其他方法，Equivalent Pruning有效减少微调后的性能损失。

Conclusion: Optimal Brain Connection框架在结构剪枝中表现出色，兼顾参数关联性和性能保留。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频检测方法难以泛化到多种篡改类型，且依赖大型模型。

Method: SSTGNN通过结构化图表示视频，结合空间不一致性、时序伪影和频谱失真进行联合推理，并引入可学习频谱滤波器和时序差分建模。

Result: 在多种基准数据集上表现优异，参数减少42.4倍，具有强泛化能力和鲁棒性。
...
Method: 引入多模态多语言数据集，评估Gemini、GPT和Llama等MLLMs在品牌安全分类中的表现，并与人工审核对比。

Result: MLLMs在品牌安全分类中表现出色，但存在局限性，论文公开数据集以促进未来研究。

Conclusion: MLLMs在内容审核中具有潜力，但仍需改进，公开数据集有助于推动负责任的内容审核研究。
...
TL;DR: 论文提出了一种名为Action Discovery的新方法，用于解决时间动作分割中模糊动作和部分标注数据的挑战。通过已知动作引导未知动作的分割和语义分类，显著优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 在部分标注数据集中，模糊动作和缺失标注是常见问题，尤其在神经科学等领域。本文旨在解决这一问题。

Method: 采用两步法：1) Granularity-Guided Segmentation Module (GGSM) 识别已知和未知动作的时间区间；2) Unknown Action Segment Assignment (UASA) 基于嵌入相似性对未知动作进行语义分类。

Result: 在Breakfast、50Salads和Desktop Assembly数据集上验证，方法显著优于现有基线。

Conclusion: Action Discovery方法有效解决了部分标注数据集中的动作分割问题，为模糊动作和未知动作的处理提供了新思路。
...
Method: 采用分组无查找量化（GQ）和生成解码（GD）技术，前者优化内存和计算效率，后者通过概率建模提升重建细节。

Result: 在ImageNet 50k验证集上，WeTok实现了最低的零样本rFID（0.12），并在高压缩比下表现优异。

Conclusion: WeTok在性能和效率上均超越现有方法，为视觉生成任务提供了更优的分词解决方案。
...
Method: 采用LLaVA架构，结合任务指令和多模态上下文样本，构建LLaVA-RE框架，并提出新的二进制相关性数据集。
...
TL;DR: Uni-CoT是一个统一的Chain-of-Thought框架，通过宏微观两级推理范式，实现了高效的多模态推理，并在多个基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有方法在视觉语言推理任务中难以建模视觉状态转换或生成连贯的视觉轨迹，因此需要一种更高效且统一的解决方案。

Method: Uni-CoT采用两级推理范式（宏观任务规划和微观子任务执行），结合图像文本监督和多任务目标训练，降低计算开销。
...
TL;DR: Hi3DEval是一个针对3D生成内容的分层评估框架，结合对象级和部分级评估，支持多维度分析和细粒度质量评估，同时扩展了纹理评估以包括材料真实性。
...
Method: 提出Hi3DEval框架，结合对象级和部分级评估，并引入材料真实性评估；构建Hi3DBench数据集和多代理标注流程；提出基于混合3D表示的自动评分系统。

Result: 实验表明，该方法在建模3D特性和与人类偏好对齐方面优于现有基于图像的指标。

Conclusion: Hi3DEval为3D生成内容提供了一种可扩展的评估方法，替代了手动评估。
...
<details>
  <summary>Details</summary>
Motivation: 现有GUI定位方法依赖昂贵标注或强化学习奖励，限制了数据效率和泛化能力。

Method: GUI-RC通过空间投票网格识别预测一致性区域；GUI-RCPO将一致性转化为奖励，实现无监督强化学习优化。

Result: GUI-RC提升Qwen2.5-VL-3B-Instruct在ScreenSpot-v2上3.46%；GUI-RCPO进一步优化至85.14%。
...
TL;DR: MOSEv2是一个更具挑战性的视频对象分割数据集，旨在推动真实场景下的研究，包含复杂场景、多样挑战和性能基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有数据集主要包含显著、主导和孤立的对象，限制了真实场景的泛化能力。MOSEv2旨在填补这一空白，推动视频对象分割在复杂环境中的发展。

Method: MOSEv2包含5,024个视频和701,976个高质量掩码，覆盖200个类别的10,074个对象，引入了更复杂的场景和挑战。

Result: 在MOSEv2上，20种代表性VOS方法的性能显著下降（如SAM2从76.4%降至50.9%），表明现有方法在真实场景中仍有不足。

Conclusion: MOSEv2为视频对象分割提供了更真实的测试平台，揭示了当前方法的局限性，并推动了进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 填补无色3D点云直接生成高斯分布的空白，提升3D高斯渲染的质量和效率。

Method: 采用多视角优化框架，结合深度感知图像扩散模型和表面锚定机制，确保几何精度和一致性。

Result: 在合成点云、真实扫描和大规模场景中均表现优异，生成高保真3D高斯分布。

Conclusion: GAP成功解决了无色点云到高斯分布的转换问题，为3D渲染提供了高效且高质量的解决方案。
...
TL;DR: FaceAnonyMixer 是一种可取消的人脸生成框架，通过混合真实人脸图像的潜在代码与可撤销密钥生成的合成代码，实现隐私保护，同时保持识别性能。
...
Result: 实验表明，FaceAnonyMixer 在商业 API 上比现有方法提升了 11% 的识别准确率，同时提供更强的隐私保护。
...
### [102] [BTPG-max: Achieving Local Maximal Bidirectional Pairs for Bidirectional Temporal Plan Graphs](https://arxiv.org/abs/2508.04849)
*Yifan Su,Rishi Veerapaneni,Jiaoyang Li*

Main category: cs.MA

TL;DR: 提出了一种名为BTPG-max的算法，用于在多智能体路径规划（MAPF）中优化双向时间计划图（BTPG），增加双向依赖对，提高延迟下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有MAPF规划器假设智能体在特定时间到达特定位置，但现实中延迟会导致偏离计划，引发碰撞。BTPG通过时间独立依赖关系解决这一问题，但仍有优化空间。

Method: 设计了BTPG-max算法，通过局部最优的方式增加BTPG中的双向依赖对，提升执行效率和鲁棒性。

Result: BTPG-max生成的BTPG具有更多双向边，表现出更好的实时行为和延迟鲁棒性。

Conclusion: BTPG-max算法在理论和实践中均优于现有方法，为MAPF问题提供了更高效的解决方案。
...
### [103] [Congestion Mitigation Path Planning for Large-Scale Multi-Agent Navigation in Dense Environments](https://arxiv.org/abs/2508.05253)
*Takuro Kato,Keisuke Okumura,Yoko Sasaki,Naoya Yokomachi*

Main category: cs.MA

TL;DR: 论文提出了一种新的路径规划问题CMPP，通过将拥堵直接嵌入成本函数来优化多智能体系统的全局流动，减少局部拥堵。


<details>
  <summary>Details</summary>
Motivation: 在高密度环境中，多智能体同时移动时，局部拥堵会影响整体导航效率，因此需要一种方法来优化全局流动。

Method: CMPP通过定义基于路径使用情况的成本函数，并开发了两种求解器：精确的混合整数非线性规划求解器和可扩展的两层搜索算法A-CMTS。

Result: 实验表明，CMPP能显著减少局部拥堵并提高系统吞吐量。

Conclusion: CMPP在多智能体系统（如物流和自动驾驶）中具有实际应用价值。
...
### [104] [NAEx: A Plug-and-Play Framework for Explaining Network Alignment](https://arxiv.org/abs/2508.04731)
*Shruti Saxena,Arijit Khan,Joydeep Chandra*

Main category: cs.LG

TL;DR: NAEx是一个模型无关的框架，通过识别关键子图和特征来解释网络对齐模型的预测，提升其可解释性。


<details>
  <summary>Details</summary>
Motivation: 网络对齐模型的可解释性有限，难以理解其决策，尤其是在高风险领域。NAEx旨在解决这一问题。

Method: NAEx通过可学习的边和特征掩码联合参数化图结构和特征空间，并引入优化目标确保解释忠实于原始预测。

Result: NAEx在基准数据集上表现出高效性和有效性，并与四种代表性网络对齐模型集成。

Conclusion: NAEx为网络对齐模型提供了可解释性，同时保持跨网络依赖关系，适用于未见数据。
...
### [105] [LumiGen: An LVLM-Enhanced Iterative Framework for Fine-Grained Text-to-Image Generation](https://arxiv.org/abs/2508.04732)
...
<details>
  <summary>Details</summary>
Motivation: 现有T2I模型在复杂指令处理、细粒度内容控制和语义一致性方面存在不足，而LVLM在跨模态理解和指令跟随方面表现出色，因此提出LumiGen以结合两者优势。
...
### [106] [Echo State Networks for Bitcoin Time Series Prediction](https://arxiv.org/abs/2508.05416)
*Mansi Sharma,Enrico Sartor,Marc Cavazza,Helmut Prendinger*

Main category: cs.LG

TL;DR: 该论文探讨了使用回声状态网络（ESN）预测加密货币价格，特别是在极端波动期间，并表明其优于现有机器学习方法。


<details>
  <summary>Details</summary>
Motivation: 由于高波动性和非平稳性，股票和加密货币价格预测具有挑战性，而ESN能有效捕捉动态数据中的非线性模式。

Method: 研究采用ESN进行加密货币预测，并通过Lyapunov指数进行混沌分析。

Result: ESN在混沌期间表现稳健，显著优于Boosting和Naïve方法。

Conclusion: ESN在高混沌环境下表现出色，为加密货币预测提供了有效工具。
...
### [107] [MissMecha: An All-in-One Python Package for Studying Missing Data Mechanisms](https://arxiv.org/abs/2508.04740)
...
<details>
  <summary>Details</summary>
Motivation: 解决现有工具在模拟缺失数据时的局限性，如机制单一、仅支持数值变量，无法满足真实世界数据的多样性需求。

Method: 开发MissMecha工具包，支持多种缺失机制（MCAR、MAR、MNAR），涵盖数值和分类特征，并提供可视化诊断和评估功能。

Result: MissMecha为研究人员和从业者提供了一个统一的平台，支持数据质量研究、基准测试和教育。

Conclusion: MissMecha填补了现有工具的空白，为处理混合类型表格数据的缺失问题提供了全面解决方案。
...
### [108] [MoMA: A Mixture-of-Multimodal-Agents Architecture for Enhancing Clinical Prediction Modelling](https://arxiv.org/abs/2508.05492)
...
TL;DR: MoMA是一种新型架构，利用多模态EHR数据和多个LLM代理进行临床预测任务，通过将非文本模态转换为结构化文本摘要，并结合临床笔记生成统一的多模态摘要，最终实现更准确的预测。


<details>
  <summary>Details</summary>
Motivation: 多模态EHR数据能提供更丰富的患者健康信息，但如何有效整合这些数据以提升临床预测模型的性能仍是一个挑战。

Method: MoMA采用三个LLM代理：专家代理将非文本模态（如医学图像和实验室结果）转换为结构化文本摘要；聚合代理将这些摘要与临床笔记结合生成统一的多模态摘要；预测代理基于统一摘要进行临床预测。

Result: 在三个预测任务中，MoMA的表现优于当前最先进的方法，展示了其在准确性和灵活性上的优势。

Conclusion: MoMA通过多模态数据整合和LLM代理的协同工作，显著提升了临床预测的准确性和适应性。
...
### [109] [Discovering Interpretable Programmatic Policies via Multimodal LLM-assisted Evolutionary Search](https://arxiv.org/abs/2508.05433)
...
<details>
  <summary>Details</summary>
Motivation: 解决深度强化学习在控制策略设计中性能高但缺乏可解释性的问题，增强信任和实际部署能力。

Method: MLES利用多模态大语言模型生成策略，结合进化机制优化，并通过视觉反馈分析失败模式以改进策略。

Result: 在两项控制任务中，MLES性能与PPO相当，同时提供透明逻辑和可追溯设计过程。

Conclusion: MLES是一种可扩展、可解释的控制策略发现方法，有望成为下一代主流方案。
...
### [110] [Edge-Assisted Collaborative Fine-Tuning for Multi-User Personalized Artificial Intelligence Generated Content (AIGC)](https://arxiv.org/abs/2508.04745)
*Nan Li,Wanting Yang,Marie Siew,Zehui Xiong,Binbin Chen,Shiwen Mao,Kwok-Yan Lam*

Main category: cs.LG

TL;DR: 论文提出了一种基于集群感知的分层联邦聚合框架，用于解决边缘设备上扩散模型的计算和隐私问题。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在边缘设备上计算资源需求高，且云解决方案存在隐私、个性化和通信成本问题。

Method: 采用低秩适应（LoRA）进行本地微调，通过客户端聚类和集群内聚合增强个性化，并实现跨集群知识交互。

Result: 框架在边缘约束下实现了加速收敛，并支持可扩展的多用户个性化AIGC服务。

Conclusion: 该框架在效率和隐私保护方面表现优异，适用于边缘场景下的个性化内容生成。
...
### [111] [TrajEvo: Trajectory Prediction Heuristics Design via LLM-driven Evolution](https://arxiv.org/abs/2508.05616)
*Zhikai Zhao,Chuanbo Hua,Federico Berto,Kanghoon Lee,Zihan Ma,Jiachen Li,Jinkyoo Park*

Main category: cs.LG

TL;DR: TrajEvo利用大型语言模型（LLM）和进化算法自动设计轨迹预测启发式方法，显著优于传统启发式和深度学习方法，尤其在泛化到未见过的OOD场景时表现突出。
...
Result: TrajEvo在多个真实数据集上优于现有方法，尤其在OOD场景中表现显著。

Conclusion: TrajEvo为自动设计高效、可解释且泛化能力强的轨迹预测启发式方法提供了有前景的方向。
...
### [112] [A Foundational Multi-Modal Model for Few-Shot Learning](https://arxiv.org/abs/2508.04746)
...
<details>
  <summary>Details</summary>
Motivation: 在生物医学、环境科学等领域，样本稀缺且数据收集成本高，传统小样本学习方法泛化能力有限，亟需创新解决方案。

Method: 构建了包含多种模态数据的M3FD数据集，并开发了M3F框架，通过多模态模型训练和微调提升FSL性能。

Result: M3F框架在同类任务中优于传统元学习方法，为实际应用提供了可行方案。

Conclusion: M3FD和M3F为数据稀缺领域的FSL提供了统一、可扩展的解决方案，降低了应用门槛。
...
### [113] [AttriLens-Mol: Attribute Guided Reinforcement Learning for Molecular Property Prediction with Large Language Models](https://arxiv.org/abs/2508.04748)
*Xuan Lin,Long Chen,Yile Wang*

Main category: cs.LG

TL;DR: AttriLens-Mol是一种基于属性引导的强化学习框架，用于提升大语言模型在分子属性预测任务中的性能，通过结构化输出和验证机制提高相关性和预测效果。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在分子属性预测中依赖人工提示且推理冗长，缺乏相关性，需要更高效的引导方法。

Method: 提出AttriLens-Mol框架，通过格式奖励、计数奖励和合理性奖励引导模型生成相关分子属性。

Result: 在多个数据集上显著提升性能，优于监督微调模型和其他先进模型，提取的属性也提升了决策树模型的性能。

Conclusion: AttriLens-Mol有效提取相关分子属性，提升预测性能和可解释性，代码已开源。
...
### [114] [PA-RNet: Perturbation-Aware Reasoning Network for Multimodal Time Series Forecasting](https://arxiv.org/abs/2508.04750)
...
Method: 提出PA-RNet，包含扰动感知投影模块和跨模态注意力机制，分离噪声并保留语义信息，理论证明其稳定性和误差降低能力。

Result: 实验表明，PA-RNet在多种领域和时间设置下均优于现有基线方法。

Conclusion: PA-RNet通过扰动感知和跨模态注意力机制，显著提升了多模态时间序列预测的鲁棒性和性能。
...
### [115] [InfoQ: Mixed-Precision Quantization via Global Information Flow](https://arxiv.org/abs/2508.04753)
...
Method: InfoQ通过量化每层并测量后续层的互信息变化，将比特分配问题转化为整数线性规划问题。
...
### [116] [Are Large Language Models Dynamic Treatment Planners? An In Silico Study from a Prior Knowledge Injection Angle](https://arxiv.org/abs/2508.04755)
*Zhiyao Luo,Tingting Zhu*

Main category: cs.LG

TL;DR: 研究评估了开源大语言模型（LLMs）在动态胰岛素剂量调整中的表现，发现其零样本推理性能可与小型强化学习代理（SRAs）媲美，但也存在局限性，如过度激进的剂量调整。


<details>
  <summary>Details</summary>
Motivation: 解决基于强化学习的动态治疗方案（DTRs）在实际部署中面临的临床知识注入和患者安全问题，探索LLMs作为替代方案的潜力。

Method: 在模拟的1型糖尿病环境中，比较开源LLMs与小型神经网络RL代理（SRAs）的零样本推理性能。

Result: LLMs在稳定患者群体中表现优于SRAs，但在复杂推理中存在剂量调整过激等问题。

Conclusion: 建议谨慎乐观地将LLMs整合到临床工作流中，需结合提示工程、验证和混合方法以确保安全性。
...
### [117] [Uncertainty-aware Predict-Then-Optimize Framework for Equitable Post-Disaster Power Restoration](https://arxiv.org/abs/2508.04780)
*Lin Jiang,Dahai Yu,Rongchao Xu,Tian Tang,Guang Wang*

Main category: cs.LG

TL;DR: 论文提出了一种公平感知的电力恢复策略EPOPR，通过量化回归和强化学习减少停电时间和社区间的不公平。


<details>
  <summary>Details</summary>
Motivation: 极端天气事件频发，现有电力恢复策略因请求量差异导致不公平，需平衡效率与公平。

Method: 设计了EPOPR框架，包含公平量化回归和时空注意力强化学习，解决预测和决策中的不确定性。

Result: 实验显示EPOPR平均减少停电时间3.60%，降低社区间不公平14.19%。

Conclusion: EPOPR有效提升了电力恢复的公平性和效率，为未来研究提供了新方向。
...
### [118] [Federated Continual Recommendation](https://arxiv.org/abs/2508.04792)
...
<details>
  <summary>Details</summary>
Motivation: 现有联邦推荐方法难以处理动态数据流，而持续学习方法又假设数据集中存储，无法满足隐私保护需求。

Method: 提出F3CRec框架，包含客户端自适应重放记忆和服务器端项目时序均值两个关键组件。

Result: 实验表明F3CRec在联邦环境下能长期保持推荐质量。

Conclusion: F3CRec成功整合了联邦学习和持续学习，为隐私保护下的动态推荐提供了有效解决方案。
...
### [119] [HCRide: Harmonizing Passenger Fairness and Driver Preference for Human-Centered Ride-Hailing](https://arxiv.org/abs/2508.04811)
*Lin Jiang,Yu Yang,Guang Wang*

Main category: cs.LG

TL;DR: 论文提出了一种以人为中心的网约车系统HCRide，通过多智能体强化学习算法Habic，平衡乘客公平性和司机偏好，同时提升系统效率。实验表明其优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有网约车调度系统多关注运营商收益，可能损害乘客和司机的体验。本文旨在设计一个兼顾乘客公平性、司机偏好和系统效率的系统。

Method: 提出HCRide系统，基于Habic算法（多智能体竞争机制、动态Actor网络和Bi-Critic网络），优化效率、公平性和司机偏好。

Result: 在两个真实数据集（深圳和纽约）上测试，HCRide在系统效率、公平性和司机偏好上分别提升2.02%、5.39%和10.21%。

Conclusion: HCRide成功平衡了乘客公平性和司机偏好，同时提升了系统效率，为网约车调度提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 复杂系统的可靠建模在工程和科学应用中至关重要，但传统方法在非线性或高维情况下可能失效。

Method: 通过RC学习非线性系统动态，作为UKF预测步骤的替代模型，并结合实时传感器数据修正模型漂移。

Result: 在基准问题和实时车辆轨迹估计任务中验证了RCUKF的有效性。

Conclusion: RCUKF为复杂系统提供了一种高效且鲁棒的建模方法。
...
### [121] [Unified Flow Matching for Long Horizon Event Forecasting](https://arxiv.org/abs/2508.04843)
*Xiao Shou*

Main category: cs.LG

TL;DR: 提出了一种基于流匹配的统一框架，用于非自回归建模标记事件序列，显著提升了长范围预测的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有自回归模型在长范围事件序列预测中存在效率低和误差累积的问题。

Method: 采用连续和离散流匹配技术，联合建模事件间隔时间和事件类型，实现非自回归生成。
...
### [122] [Advanced Hybrid Transformer LSTM Technique with Attention and TS Mixer for Drilling Rate of Penetration Prediction](https://arxiv.org/abs/2508.05210)
*Saddam Hussain Khan*

Main category: cs.LG

TL;DR: 提出了一种新型混合深度学习架构，结合LSTM、Transformer、TS-Mixer和注意力机制，显著提升了钻井ROP预测的准确性和实时性。


<details>
  <summary>Details</summary>
Motivation: 传统方法难以捕捉钻井数据的高维动态特性，导致预测效果不佳，亟需更先进的模型。

Method: 采用LSTM、Transformer、TS-Mixer和注意力机制的混合架构，建模时间依赖性和特征交互。

Result: 模型在真实数据集上表现优异（R-squared 0.9988，MAPE 1.447%），优于基准方法。
...
### [123] [Multi-Stage Knowledge-Distilled VGAE and GAT for Robust Controller-Area-Network Intrusion Detection](https://arxiv.org/abs/2508.04845)
...
Method: 采用变分图自编码器（VGAE）进行结构异常检测，结合知识蒸馏图注意力网络（KD-GAT）进行分类，并处理类别不平衡问题。

Result: 在六个公开CAN数据集上测试，F1分数平均提升16.2%，参数减少96%。

Conclusion: 该框架在检测精度和效率上优于现有方法，尤其适用于类别不平衡的数据集。
...
### [124] [Provable Post-Training Quantization: Theoretical Analysis of OPTQ and Qronos](https://arxiv.org/abs/2508.04853)
*Haoyu Zhang,Shihao Zhang,Ian Colbert,Rayan Saab*

Main category: cs.LG

TL;DR: 本文首次为OPTQ（GPTQ）和Qronos这两种后训练量化算法提供了定量误差界限，分析了OPTQ的迭代过程如何引入量化误差，并提出了非渐进的2-范数误差界限。


<details>
  <summary>Details</summary>
Motivation: 尽管OPTQ因其计算效率和强实证性能成为主流后训练量化方法，但缺乏严格的理论保证。本文旨在填补这一空白，为OPTQ及其变体提供理论支持。

Method: 通过分析OPTQ的迭代过程，推导出非渐进的2-范数误差界限，并扩展到Qronos算法。同时研究了正则化参数的选择和特征排序的启发式方法。

Result: 提出了OPTQ和Qronos的确定性及随机变体的定量误差界限，为实际设计选择提供了理论依据，并解释了Qronos的实证优势。

Conclusion: 本文为OPTQ和Qronos提供了首个定量理论保证，为后训练量化的实践和算法设计提供了指导。
...
### [125] [Agnostics: Learning to Code in Any Programming Language via Reinforcement with a Universal Learning Environment](https://arxiv.org/abs/2508.04865)
*Aleksander Boruch-Gruszecki,Yangtian Zi,Zixuan Wu,Tejas Oberoi,Carolyn Jane Anderson,Joydeep Biswas,Arjun Guha*

Main category: cs.LG

TL;DR: Agnostics是一种语言无关的后训练流程，通过观察代码的外部行为来验证多语言代码，显著提升了低资源语言的性能。
...
Method: 将单元测试数据集转换为I/O格式，提供配置以编译和运行目标语言，使用RLVR在代码执行环境中训练。

Result: 在五种低资源语言中显著提升模型性能，达到与更大模型相当的水平，并创下新的SOTA结果。

Conclusion: Agnostics简化了多语言代码训练，为低资源语言提供了高效解决方案。
...
### [126] [Hilbert Neural Operator: Operator Learning in the Analytic Signal Domain](https://arxiv.org/abs/2508.04882)
*Saman Pordanesh,Pejman Shahsavari,Hossein Ghadjari*

Main category: cs.LG

TL;DR: HNO是一种新的神经算子架构，通过希尔伯特变换将输入信号映射到其解析表示，从而为学习过程提供瞬时振幅和相位信息。


<details>
  <summary>Details</summary>
Motivation: 解决FNO等现有方法在频域卷积中的局限性，如周期性假设，并通过信号处理的强归纳偏置提升性能。

Method: 使用希尔伯特变换将输入信号转换为解析表示，并在该表示上应用谱卷积。

Result: HNO能够更有效地建模因果、相位敏感和非平稳系统的算子。

Conclusion: HNO通过引入希尔伯特变换和解析信号理论，提供了一种更有效的神经算子架构。
...
### [127] [Gaussian mixture layers for neural networks](https://arxiv.org/abs/2508.04883)
*Sinho Chewi,Philippe Rigollet,Yuling Yan*

Main category: cs.LG

TL;DR: 该论文研究了在概率测度上直接实现动态的可能性，提出了一种新型的Gaussian mixture (GM)层，并通过实验验证其性能与传统全连接层相当。


<details>
  <summary>Details</summary>
Motivation: 探索是否可以直接在概率测度上实现动态，以进一步理解神经网络的非参数视角。
...
### [128] [Uncertainty Quantification for Surface Ozone Emulators using Deep Learning](https://arxiv.org/abs/2508.04885)
*Kelsey Doerksen,Yuliya Marchetti,Steven Lu,Kevin Bowman,James Montgomery,Kazuyuki Miyazaki,Yarin Gal,Freddie Kalaitzis*

Main category: cs.LG

TL;DR: 论文提出了一种基于不确定性感知的U-Net架构，结合贝叶斯和分位数回归方法，用于预测MOMO-Chem模型的地表臭氧偏差，并评估了其在美国和欧洲的应用效果。


<details>
  <summary>Details</summary>
Motivation: 空气污染是全球性问题，传统物理模型在模拟地表臭氧趋势时存在不足，而深度学习模型缺乏可解释性。因此，需要一种既能捕捉复杂气候模式又能支持决策的方法。

Method: 采用不确定性感知的U-Net架构，结合贝叶斯和分位数回归方法，预测MOMO-Chem模型的地表臭氧偏差，并评估其在美国和欧洲2019年6月的应用效果。

Result: 研究展示了该方法在区域偏差估计中的能力，比较了两种不确定性量化方法的得分，并确定了适合MOMO-Chem偏差修正的地面站点。

Conclusion: 该方法为地表臭氧偏差建模提供了新工具，支持政策制定和公共健康措施的决策。
...
### [129] [Leveraging Deep Learning for Physical Model Bias of Global Air Quality Estimates](https://arxiv.org/abs/2508.04886)
*Kelsey Doerksen,Yuliya Marchetti,Kevin Bowman,Steven Lu,James Montgomery,Yarin Gal,Freddie Kalaitzis,Kazuyuki Miyazaki*

Main category: cs.LG

TL;DR: 论文提出了一种基于2D卷积神经网络的方法，用于改进表面臭氧的建模偏差，并在北美和欧洲验证了其优于传统机器学习方法的性能。
...
Method: 采用2D卷积神经网络架构，估计表面臭氧MOMO-Chem模型的残差（模型偏差），并结合高分辨率卫星影像的土地利用信息改进模型估计。

Result: 该方法在北美和欧洲表现出优于传统机器学习方法的性能，能更好地捕捉物理模型残差。
...
### [130] [Retrieval-Augmented Water Level Forecasting for Everglades](https://arxiv.org/abs/2508.04888)
...
<details>
  <summary>Details</summary>
Motivation: 解决深度学习模型在水文领域泛化能力不足的问题，提升水位预测的准确性以支持生态系统管理。

Method: 引入RAF框架，通过检索历史相似水文事件丰富模型输入，比较基于相似性和互信息的检索方法。

Result: 在Everglades真实数据上验证，RAF显著提升了水位预测的准确性。

Conclusion: RAF在水文领域具有潜力，为自适应AI方法在生态系统管理中的广泛应用铺平了道路。
...
### [131] [Will You Be Aware? Eye Tracking-Based Modeling of Situational Awareness in Augmented Reality](https://arxiv.org/abs/2508.05025)
*Zhehan Qu,Tianyi Hu,Christian Fronk,Maria Gorlatova*

Main category: cs.LG

TL;DR: 论文研究了AR引导心肺复苏（CPR）中的情境意识（SA），开发了AR应用并通过眼动追踪分析SA，提出FixGraphPool模型预测SA，效果优于现有方法。


<details>
  <summary>Details</summary>
Motivation: AR系统可能引发认知隧道效应，降低情境意识，尤其在安全关键场景如CPR中。研究旨在评估AR引导CPR中的SA并提出改进方法。

Method: 开发Magic Leap 2上的AR应用，提供实时CPR反馈，并通过模拟意外事件（如出血）评估SA。使用眼动追踪分析SA指标，提出FixGraphPool模型预测SA。

Result: 眼动分析显示高SA与更大扫视幅度和速度相关，虚拟内容注视比例和频率降低。FixGraphPool模型预测SA准确率达83.0%（F1=81.0%），优于其他方法。

Conclusion: 眼动追踪可用于AR中的SA建模，为设计兼顾用户安全和情境意识的AR系统提供支持。
...
### [132] [Honest and Reliable Evaluation and Expert Equivalence Testing of Automated Neonatal Seizure Detection](https://arxiv.org/abs/2508.04899)
*Jovana Kljajic,John M. O'Toole,Robert Hogan,Tamara Skoric*

Main category: cs.LG

TL;DR: 该研究系统评估了新生儿癫痫检测中常用的性能指标，并提出了针对性的最佳实践，以解决当前评估中的不一致性和偏见问题。
...
Result: Matthews和Pearson相关系数在类别不平衡下表现优于AUC；共识类型对评分者数量和一致性敏感；多评分者图灵测试（Fleiss k）最能反映专家水平的AI性能。

Conclusion: 建议报告至少一个平衡指标、敏感性、特异性、PPV和NPV、多评分者图灵测试结果（Fleiss k），并在独立验证集上验证。该框架为临床验证提供了重要前提。
...
### [133] [ASkDAgger: Active Skill-level Data Aggregation for Interactive Imitation Learning](https://arxiv.org/abs/2508.05310)
*Jelle Luijkx,Zlatan Ajanović,Laura Ferranti,Jens Kober*

Main category: cs.LG

TL;DR: ASkDAgger框架通过利用新手计划中的信息，减少教师反馈需求，提高模仿学习的效率和适应性。


<details>
  <summary>Details</summary>
Motivation: 减少人类教师在交互式模仿学习中的教学负担，同时充分利用新手的计划信息。

Method: 提出ASkDAgger框架，包括SAG、FIER和PIER三个组件，分别调整阈值、重放演示和优先重放。

Result: 在模拟和真实环境中验证了ASkDAgger的有效性，减少了反馈需求并提高了性能。

Conclusion: ASkDAgger通过优化反馈机制，显著提升了交互式模仿学习的效率和适应性。
...
### [134] [Sensitivity of Stability: Theoretical & Empirical Analysis of Replicability for Adaptive Data Selection in Transfer Learning](https://arxiv.org/abs/2508.04901)
*Prabhav Singh,Jessica Sorrell*

Main category: cs.LG

TL;DR: 本文研究了迁移学习中自适应数据选择策略对模型可复现性的影响，提出了选择敏感性的量化框架，并揭示了性能与可复现性之间的权衡关系。


<details>
  <summary>Details</summary>
Motivation: 迁移学习的广泛应用使得预训练模型能够高效适应新领域，但其可靠性尚未充分理解，尤其是在使用动态优先选择训练数据的自适应策略时。

Method: 提出了选择敏感性（ΔQ）的数学框架，量化了适应效果与结果一致性之间的权衡。通过理论分析和MultiNLI语料库上的实验验证了该框架。

Result: 实验表明，高自适应策略（如基于梯度的选择）性能优越但可复现性差，而低自适应策略可复现性较好。源域预训练可显著降低失败率。

Conclusion: 研究为迁移学习中的性能与可复现性权衡提供了指导，并强调了可复现性设计的重要性。
...
### [135] [Advancing Hate Speech Detection with Transformers: Insights from the MetaHate](https://arxiv.org/abs/2508.04913)
...
<details>
  <summary>Details</summary>
Motivation: 仇恨言论在社交媒体上广泛存在且危害严重，需要开发高效的自动化检测方法。现有深度学习方法（如RNN、LSTM、CNN）存在长期依赖和并行化效率低等问题。

Method: 使用MetaHate数据集（包含36个数据集和120万社交媒体样本），评估了BERT、RoBERTa、GPT-2和ELECTRA等Transformer模型。

Result: 微调后的ELECTRA模型表现最佳，F1分数为0.8980。研究还发现分类错误主要源于讽刺、隐晦语言和标签噪声。

Conclusion: Transformer模型在仇恨言论检测中表现优异，尤其是ELECTRA，但需进一步解决讽刺和隐晦语言等挑战。
...
### [136] [ALScope: A Unified Toolkit for Deep Active Learning](https://arxiv.org/abs/2508.04937)
...
Result: 实验表明，算法性能因领域和任务设置而异，非标准场景下算法仍有改进空间，部分算法性能好但耗时较长。

Conclusion: ALScope为深度主动学习提供了全面评估工具，揭示了算法在不同场景下的表现和优化方向。
...
### [137] [REINA: Regularized Entropy Information-Based Loss for Efficient Simultaneous Speech Translation](https://arxiv.org/abs/2508.04946)
*Nameer Hirschkind,Joseph Liu,Mahesh Kumar Nandwana,Xiao Yu*

Main category: cs.LG

TL;DR: 论文提出了一种名为REINA的新策略，通过信息理论优化同步语音翻译（SimulST）的质量与延迟权衡，并在多语言任务中实现SOTA结果。
...
Result: 在多语言任务中，REINA显著提升了延迟与质量的权衡，最高提升21%，并达到SOTA结果。

Conclusion: REINA为同步语音翻译提供了一种高效的方法，显著优化了质量与延迟的权衡。
...
### [138] [Self-Error Adjustment: Theory and Practice of Balancing Individual Performance and Diversity in Ensemble Learning](https://arxiv.org/abs/2508.04948)
*Rui Zou*

Main category: cs.LG

TL;DR: 论文提出了一种名为SEA的新框架，通过分解集成误差为个体性能项和多样性项，并引入可调参数，实现对集成性能的精细调控。


<details>
  <summary>Details</summary>
Motivation: 传统集成学习方法（如Bagging和Boosting）通过随机性促进多样性，但缺乏对准确性与多样性权衡的精确控制。NCL方法虽有改进，但理论界限松散且调整范围有限。
...
### [139] [Compressed Decentralized Momentum Stochastic Gradient Methods for Nonconvex Optimization](https://arxiv.org/abs/2508.04950)
*Wei Liu,Anweshit Panda,Ujwal Pandey,Christopher Brissette,Yikang Shen,George M. Slota,Naigang Wang,Jie Chen,Yangyang Xu*

Main category: cs.LG

TL;DR: 论文提出了两种压缩去中心化算法，分别针对梯度有界和数据异构场景，结合动量和消息压缩技术，实现了快速收敛和低通信成本。
...
Method: 设计了两种算法：1）梯度有界时的压缩去中心化自适应方法；2）数据异构时的压缩去中心化heavy-ball方法，结合梯度跟踪技术。

Result: 两种方法均达到最优收敛速率，支持线性加速和拓扑无关参数设置，在DNN和Transformer训练中表现优于现有方法。

Conclusion: 算法在理论和实践中均表现出色，为去中心化随机优化提供了高效解决方案。
...
### [140] [MENDR: Manifold Explainable Neural Data Representations](https://arxiv.org/abs/2508.04956)
...
<details>
  <summary>Details</summary>
Motivation: 现有EEG基础模型在预训练动态和嵌入信息保留方面缺乏透明度，限制了临床应用的可靠性。

Method: MENDR采用离散小波包变换分解EEG信号，学习对称正定矩阵嵌入，并通过几何椭球可视化提升可解释性。
...
### [141] [Analyzing the Impact of Multimodal Perception on Sample Complexity and Optimization Landscapes in Imitation Learning](https://arxiv.org/abs/2508.05077)
*Luai Abuelsamen,Temitope Lukman Adebanjo*

Main category: cs.LG

TL;DR: 论文通过统计学习理论分析了多模态模仿学习的理论基础，探讨多模态感知如何影响样本复杂性和优化景观，并证明多模态策略在泛化边界和优化方面优于单模态策略。


<details>
  <summary>Details</summary>
Motivation: 研究多模态感知（如RGB-D、本体感觉、语言）对模仿学习的影响，以理解多模态策略为何在性能上优于单模态策略。

Method: 基于多模态学习理论的最新进展，分析多模态策略的泛化边界和优化景观，并连接Rademacher复杂性、PAC学习和信息论等理论框架。

Result: 研究表明，正确整合的多模态策略可以实现更紧的泛化边界和更优的优化景观。

Conclusion: 多模态架构（如PerAct和CLIPort）的优越性能可通过统计学习理论解释，为多模态模仿学习提供了理论基础。
...
### [142] [Disentangling Bias by Modeling Intra- and Inter-modal Causal Attention for Multimodal Sentiment Analysis](https://arxiv.org/abs/2508.04999)
*Menghua Jiang,Yuxia Lin,Baoliang Chen,Haifeng Hu,Yuncheng Jiang,Sijie Mai*

Main category: cs.LG

TL;DR: 提出了一种多模态因果干预模型（MMCI），通过因果理论中的后门调整解决多模态情感分析中的虚假相关性，提升模型的泛化能力。
...
Method: 将多模态输入建模为多关系图，捕捉模态内和模态间依赖；使用注意力机制分离因果特征和捷径特征；通过后门调整动态结合这些特征。

Result: 在多个标准数据集和分布外测试集上验证了MMCI能有效抑制偏差并提升性能。
...
### [143] [R-Zero: Self-Evolving Reasoning LLM from Zero Data](https://arxiv.org/abs/2508.05004)
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM训练方法依赖人工标注任务和标签，限制了AI系统超越人类智能的能力。

Method: R-Zero通过Challenger生成任务，Solver解决任务，两者交互优化，形成自我改进的课程。

Result: R-Zero显著提升了LLM的推理能力，例如在数学和通用领域推理基准上分别提升了6.49和7.54分。
...
### [144] [SPaRFT: Self-Paced Reinforcement Fine-Tuning for Large Language Models](https://arxiv.org/abs/2508.05015)
*Dai Do,Manh Nguyen,Svetha Venkatesh,Hung Le*

Main category: cs.LG

TL;DR: SPaRFT是一种自定进度学习框架，通过优化数据选择和时机，高效训练语言模型，减少样本需求。


<details>
  <summary>Details</summary>
Motivation: 当前基于强化学习的语言模型训练方法需要大量数据和计算资源，且现有课程学习或数据选择方法多为启发式或资源密集，难以扩展和泛化。

Method: SPaRFT结合聚类数据缩减（按语义和难度分区）和多臂老虎机（动态分配训练样本），优化训练过程。

Result: 在多个推理基准测试中，SPaRFT使用最多100倍更少的样本，达到或超过现有基准的准确率。

Conclusion: 精心设计的性能驱动训练课程能以最少资源解锁语言模型的强大推理能力。
...
### [145] [Learning from Oblivion: Predicting Knowledge Overflowed Weights via Retrodiction of Forgetting](https://arxiv.org/abs/2508.05059)
*Jinhyeok Jang,Jaehong Kim,Jung Uk Kim*

Main category: cs.LG

TL;DR: 论文提出了一种名为KNOW的新策略，通过结构化遗忘及其逆过程生成知识更丰富的预训练权重，提升下游任务性能。
...
Method: 利用结构化遗忘及其逆过程，通过逐步缩小数据集的微调模拟遗忘，并用元学习建模权重预测。

Result: KNOW策略在多种数据集和架构上表现优于传统微调和简单权重预测。
...
### [146] [TANGO: Graph Neural Dynamics via Learned Energy and Tangential Flows](https://arxiv.org/abs/2508.05070)
*Moshe Eliasof,Eldad Haber,Carola-Bibiane Schönlieb*

Main category: cs.LG

TL;DR: TANGO是一个基于动态系统的图表示学习框架，通过学习的能量景观和下降动态控制节点特征的演化，结合能量梯度下降和切向演化，提升图神经网络的性能。
...
Method: 通过学习节点嵌入的Lyapunov函数定义能量减少方向，并结合消息传递学习的切向分量，实现灵活的特征演化。

Result: 在多种节点和图分类、回归任务中表现优异。

Conclusion: 联合学习能量函数和切向流能有效提升图神经网络的性能。
...
### [147] [ULU: A Unified Activation Function](https://arxiv.org/abs/2508.05073)
*Simin Huo*
...
<details>
  <summary>Details</summary>
Motivation: 设计一种能够区分处理正负输入的激活函数，以提升模型性能。

Method: 定义ULU为分段函数，正负输入分别处理；AULU通过可学习参数自适应调整响应。

Result: 实验表明ULU和AULU显著优于ReLU和Mish。

Conclusion: ULU和AULU是有效的激活函数，并引入LIB指标量化模型的归纳偏差。
...
### [148] [Integrated Influence: Data Attribution with Baseline](https://arxiv.org/abs/2508.05089)
...
<details>
  <summary>Details</summary>
Motivation: 现有数据归因方法（如LOO策略）仅扰动单个训练样本，忽视了训练集的集体影响，且缺乏基线灵活性，无法提供反事实解释。

Method: 提出Integrated Influence方法，定义基线数据集，通过数据退化过程将当前数据集过渡到基线，并累积每个样本在此过程中的影响。

Result: 实验表明，Integrated Influence在数据归因任务和错误标注样本识别任务中比现有方法更可靠。

Conclusion: Integrated Influence为数据归因提供了更全面的理论框架，并能涵盖现有方法（如影响函数）作为特例。
...
### [149] [Cold Start Active Preference Learning in Socio-Economic Domains](https://arxiv.org/abs/2508.05090)
*Mojtaba Fayaz-Bakhsh,Danial Ataee,MohammadAmin Fazli*

Main category: cs.LG

TL;DR: 提出了一种解决冷启动问题的主动偏好学习框架，通过自监督预训练和主动学习优化模型性能。


<details>
  <summary>Details</summary>
Motivation: 冷启动问题在计算社交系统和经济分析中尤为突出，标记数据稀缺且昂贵，需要一种无需初始标记数据的方法。

Method: 结合PCA自监督预训练生成伪标签，再通过主动学习循环优化模型。

Result: 在多个领域的数据集上表现优于标准主动学习方法，准确性更高且所需标记数据更少。
...
### [150] [Learning from Similarity-Confidence and Confidence-Difference](https://arxiv.org/abs/2508.05108)
*Tomoya Tate,Kosuke Sugiyama,Masato Uchida*

Main category: cs.LG

TL;DR: 提出了一种新的弱监督学习框架，通过整合多种弱监督信号（相似性-置信度和置信度-差异）来提升分类性能，特别是在标记数据有限的情况下。


<details>
  <summary>Details</summary>
Motivation: 在机器学习应用中，准确标记数据困难且标记数据有限，现有弱监督学习方法多依赖单一弱监督信号，无法充分利用多种监督信息。

Method: 提出SconfConfDiff分类方法，整合两种弱标签形式，并设计两种无偏风险估计器（基于现有估计器的凸组合和新设计的交互模型）。

Result: 实验证明该方法在多种设置下优于现有基线，且理论分析表明其具有最优收敛率和鲁棒性。

Conclusion: 该方法通过整合互补弱监督信号，显著提升了分类性能，并解决了过拟合和噪声问题。
...
### [151] [Exploring Superior Function Calls via Reinforcement Learning](https://arxiv.org/abs/2508.05118)
*Bingguang Hao,Maolin Wang,Zengzhuang Xu,Yicheng Chen,Cunyin Peng,Jinjie GU,Chenyi Zhuang*

Main category: cs.LG

TL;DR: 提出了一种新的强化学习框架，通过基于熵的策略优化提升语言模型在函数调用任务中的表现，取得了86.02%的准确率。


<details>
  <summary>Details</summary>
Motivation: 当前训练方法在函数调用任务中缺乏鲁棒性，监督微调依赖模式匹配，强化学习难以处理复杂动作空间。

Method: 采用两阶段数据准备流程，结合LLM评估和抽象语法树验证，优化策略探索和结构化推理。

Result: 在Berkeley Function Calling Leaderboard上达到开源模型最佳性能，复杂场景中比标准GRPO提升6%。

Conclusion: 该方法显著提升了代码预训练模型的表现，表明结构化语言生成能力对强化学习任务有益。
...
### [152] [HFedATM: Hierarchical Federated Domain Generalization via Optimal Transport and Regularized Mean Aggregation](https://arxiv.org/abs/2508.05135)
*Thinh Nguyen,Trung Phan,Binh T. Nguyen,Khoa D Doan,Kok-Seng Wong*

Main category: cs.LG

TL;DR: 论文提出了一种名为HFedATM的分层联邦域泛化方法，通过卷积滤波器对齐和正则化聚合解决域偏移问题，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 传统分层联邦学习（HFL）在域偏移问题上的局限性促使研究者探索联邦域泛化（FedDG）方法，但其在HFL框架中的应用尚未充分研究。

Method: 提出HFedATM方法，通过滤波器最优传输对齐和收缩感知正则化均值聚合，解决域偏移问题。

Result: 实验表明HFedATM显著提升了FedDG基线的性能，同时保持了计算和通信效率。

Conclusion: HFedATM在理论分析和实验中均表现出优越性，为分层联邦学习中的域偏移问题提供了有效解决方案。
...
### [153] [Deep Neural Networks with General Activations: Super-Convergence in Sobolev Norms](https://arxiv.org/abs/2508.05141)
...
Result: 神经网络在逼近PDE弱解时表现出超收敛，精度超越有限元和谱方法。

Conclusion: 为神经网络在科学计算中的应用提供了理论基础，展示了其优于传统方法的潜力。
...
### [154] [PSEO: Optimizing Post-hoc Stacking Ensemble Through Hyperparameter Tuning](https://arxiv.org/abs/2508.05144)
*Beicheng Xu,Wei Liu,Keyao Ding,Yupeng Lu,Bin Cui*

Main category: cs.LG

TL;DR: PSEO框架通过结合多样性与性能的模型选择和优化多层堆叠机制，解决了AutoML中CASH问题在集成阶段的固定策略问题，并在80个数据集上表现最佳。


<details>
  <summary>Details</summary>
Motivation: 现有AutoML系统在集成阶段通常采用固定策略，无法适应任务特性，限制了性能提升。

Method: PSEO通过二元二次编程选择基础模型，并引入多层堆叠机制，构建超参数空间以优化集成策略。
...
### [155] [Domain-driven Metrics for Reinforcement Learning: A Case Study on Epidemic Control using Agent-based Simulation](https://arxiv.org/abs/2508.05154)
*Rishabh Gaur,Gaurav Deshkar,Jayanta Kshirsagar,Harshal Hayatnagarkar,Janani Venugopalan*

Main category: cs.LG

TL;DR: 该研究开发了基于领域的强化学习（RL）评估指标，用于优化基于代理的模型（ABMs）和理性代理模型（RABMs），并通过疾病建模案例展示了其有效性。


<details>
  <summary>Details</summary>
Motivation: 由于ABMs和RABMs的复杂性和随机性，以及缺乏标准化的RL算法评估指标，研究旨在开发领域驱动的RL评估指标。

Method: 研究结合了领域驱动的奖励和传统及最新评估指标，通过政策优化在理性ABM疾病建模案例中验证。

Result: 研究结果表明，领域驱动的奖励与多种评估指标结合，在不同模拟场景（如口罩供应差异）中表现良好。

Conclusion: 领域驱动的RL评估指标为ABMs和RABMs的性能评估提供了有效工具。
...
### [156] [pFedDSH: Enabling Knowledge Transfer in Personalized Federated Learning through Data-free Sub-Hypernetwork](https://arxiv.org/abs/2508.05157)
...
<details>
  <summary>Details</summary>
Motivation: 现有个性化联邦学习方法假设客户端静态参与，而现实中新客户端可能动态加入，导致性能下降和知识转移效率低的问题。

Method: 提出pFedDSH框架，利用中央超网络生成个性化模型，结合批特定掩码保持知识稳定性，并采用数据重放策略实现后向知识迁移。

Result: 在CIFAR-10、CIFAR-100和Tiny-ImageNet上的实验表明，pFedDSH优于现有方法，实现了对新客户端的适应性和对现有客户端的性能稳定性。

Conclusion: pFedDSH在动态客户端加入的联邦学习场景中表现出色，解决了知识稳定性和高效转移的挑战。
...
### [157] [S$^2$M-Former: Spiking Symmetric Mixing Branchformer for Brain Auditory Attention Detection](https://arxiv.org/abs/2508.05164)
*Jiaqi Wang,Zhengyu Ma,Xiongri Shen,Chenlin Zhou,Leilei Zhao,Han Zhang,Yi Zhong,Siqi Cai,Zhenxi Song,Zhiguo Zhang*

Main category: cs.LG

TL;DR: S$^2$M-Former是一种新型的尖峰对称混合框架，通过空间和频率分支的并行设计及轻量级1D令牌序列，显著减少参数和能耗，同时在听觉注意力检测任务中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前EEG-based AAD缺乏能充分利用互补EEG特征且满足能效限制的协同框架，限制了其在神经导向听力设备中的应用。

Method: 提出S$^2$M-Former框架，包括尖峰驱动的对称架构（空间和频率分支并行设计）和轻量级1D令牌序列，以减少参数和能耗。

Result: 在三个AAD基准测试中，S$^2$M-Former在解码精度上达到SOTA，同时参数减少14.7倍，能耗降低5.8倍。
...
### [158] [Aligning LLMs on a Budget: Inference-Time Alignment with Heuristic Reward Models](https://arxiv.org/abs/2508.05165)
...
Result: 在真实数据集上，HIA在相同推理预算下优于其他基线方法，且在低预算下仍有效。
...
### [159] [Near Optimal Inference for the Best-Performing Algorithm](https://arxiv.org/abs/2508.05173)
*Amichai Painsky*
...
<details>
  <summary>Details</summary>
Motivation: 在多个机器学习算法性能比较中，如何从基准数据集中识别出未来未见数据集上表现最优的算法。

Method: 将问题建模为多选分布的子集选择问题，提出渐进和有限样本方案，并提供匹配下界。

Result: 新框架显著优于现有方法，并通过理论下界验证了其优越性。

Conclusion: 提出的子集选择框架在识别最优算法方面具有高效性和理论保障。
...
### [160] [Human Activity Recognition from Smartphone Sensor Data for Clinical Trials](https://arxiv.org/abs/2508.05175)
...
TL;DR: 提出了一种基于ResNet的人类活动识别（HAR）模型，用于检测步态与非步态活动及日常活动，表现优于现有模型。


<details>
  <summary>Details</summary>
Motivation: 开发一种低开销的HAR模型，准确识别步态与非步态活动及日常活动，并验证其在多发性硬化症患者中的实用性。

Method: 使用ResNet架构，结合智能手机传感器数据（来自健康对照和多发性硬化症患者），训练和评估模型。

Result: 模型在步态检测中准确率达98.4%-99.6%，日常活动识别中优于现有模型（96.2% vs 91.9%），并在多种手机佩戴位置下表现稳健。

Conclusion: 该HAR模型准确且实用，适用于多种场景，尤其在多发性硬化症患者中具有潜在应用价值。
...
### [161] [Physics-Informed Time-Integrated DeepONet: Temporal Tangent Space Operator Learning for High-Accuracy Inference](https://arxiv.org/abs/2508.05190)
...
<details>
  <summary>Details</summary>
Motivation: 传统方法（如全滚动和自回归）在长期预测中存在误差累积和泛化能力差的问题，限制了其可靠性和准确性。

Method: PITI-DeepONet采用双输出架构，通过物理信息或混合目标训练，学习时间导数算子并结合经典时间步进方案。

Result: 在多个基准问题中，PITI-DeepONet显著降低了误差（如热方程误差减少84%），表现优于传统方法。
...
### [162] [FAITH: A Framework for Assessing Intrinsic Tabular Hallucinations in finance](https://arxiv.org/abs/2508.05201)
*Mengao Zhang,Jiayu Fu,Tanya Warrier,Yuwen Wang,Tianhui Tan,Ke-wei Huang*

Main category: cs.LG

TL;DR: 论文提出了一种评估金融领域大语言模型（LLMs）内在幻觉的框架，通过上下文感知的掩码跨度预测任务，使用真实金融文档生成数据集，并分析了主流LLMs的表现。


<details>
  <summary>Details</summary>
Motivation: 金融领域对LLMs的幻觉问题尤为敏感，因为即使是微小的数值错误也可能影响决策和合规性。现有基准难以捕捉金融数据的独特需求。

Method: 开发了一个自动化数据集创建范式，采用掩码策略，并从S&P 500年报中构建了新的幻觉评估数据集。

Result: 论文对主流LLMs在金融表格数据上的内在幻觉模式进行了全面评估。

Conclusion: 该框架为内部LLM评估提供了可靠方法，是构建更可信金融生成AI系统的重要一步。
...
TL;DR: 论文提出了一种名为BAR的竞价感知检索框架，通过将广告竞价纳入检索评分函数，解决了多阶段不一致性问题，提升了平台收入和广告效果。


<details>
  <summary>Details</summary>
Motivation: 在线广告系统的检索阶段无法访问实时竞价，导致与排名阶段的不一致，影响平台收入和广告效果。

Method: BAR框架通过竞价感知建模（包括单调性约束学习和多任务蒸馏）和异步近线推理实现实时更新，同时任务专注细化模块增强特征交互。

Result: 在阿里巴巴展示广告平台上的实验和部署显示，BAR使平台收入增加4.32%，正向操作广告的展示量提升22.2%。

Conclusion: BAR框架有效解决了多阶段不一致性问题，显著提升了广告系统的性能和经济效益。
...
### [164] [DFW: A Novel Weighting Scheme for Covariate Balancing and Treatment Effect Estimation](https://arxiv.org/abs/2508.05215)
...
Method: 提出Deconfounding Factor Weighting (DFW)，利用去混杂因子构建稳定权重，优先处理低混杂样本，生成更接近随机对照试验的伪群体。

Result: DFW在真实和合成数据集上表现优于IPW和CBPS，实现了更好的协变量平衡和处理效应估计。

Conclusion: DFW是一种有效的倾向得分加权方法，适用于二元和多处理场景，显著提升了选择偏差处理能力。
...
### [165] [ML-based Short Physical Performance Battery future score prediction based on questionnaire data](https://arxiv.org/abs/2508.05222)
*Marcin Kolakowski,Seif Ben Bader*

Main category: cs.LG

TL;DR: 研究通过问卷数据预测老年人四年后的SPPB评分，XGBoost表现最佳，平均绝对误差0.79。


<details>
  <summary>Details</summary>
Motivation: 早期干预可减缓老年人身体机能衰退，需预测SPPB评分以指导干预。

Method: 测试多种ML算法（随机森林、XGBoost、线性回归、密集和TabNet神经网络），XGBoost表现最优。

Result: XGBoost平均绝对误差0.79，特征子集优化后误差0.82。

Conclusion: XGBoost能有效预测SPPB评分，特征选择可进一步提升模型性能。
...
### [166] [Don't Reach for the Stars: Rethinking Topology for Resilient Federated Learning](https://arxiv.org/abs/2508.05224)
*Mirko Konstantin,Anirban Mukhopadhyay*

Main category: cs.LG

TL;DR: 提出了一种去中心化的联邦学习框架LIGHTYEAR，通过P2P拓扑和本地验证集计算的一致性评分，提升模型性能和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统集中式联邦学习存在单点故障、个性化不足和对数据分布变化的脆弱性，需要更灵活和可靠的解决方案。

Method: 采用P2P拓扑，每个客户端通过本地验证集计算一致性评分，选择并聚合个性化的更新子集，加入正则化项稳定训练。

Result: 在两个数据集上的实验表明，LIGHTYEAR在客户端性能和对抗性/异构条件下优于集中式和其他P2P方法。
...
### [167] [Cross-LoRA: A Data-Free LoRA Transfer Framework across Heterogeneous LLMs](https://arxiv.org/abs/2508.05232)
*Feifan Xia,Mingyang Liao,Yuyang Fang,Defang Li,Yantong Xie,Weikang Li,Yang Li,Deguo Xia,Jizhou Huang*

Main category: cs.LG

TL;DR: Cross-LoRA是一种无需数据的框架，用于在不同基础模型间迁移LoRA模块，无需额外训练数据，提升模型性能。
...
Result: 在多个基准测试中，Cross-LoRA相对基础模型性能提升高达5.26%，且性能接近直接训练的LoRA适配器。

Conclusion: Cross-LoRA提供了一种高效、轻量级的跨模型LoRA迁移方法，无需额外数据或训练。
...
### [168] [MoBE: Mixture-of-Basis-Experts for Compressing MoE-based LLMs](https://arxiv.org/abs/2508.05257)
...
<details>
  <summary>Details</summary>
Motivation: 尽管MoE架构在扩展大型语言模型方面表现出色，但其部署时的高内存需求成为严重挑战。现有压缩方法往往导致较大的准确率下降。

Method: MoBE方法通过将专家矩阵分解为共享基矩阵的线性组合，最小化重构误差，实现高效压缩。

Result: 实验表明，MoBE能将模型参数减少24%-30%，准确率仅下降1%-2%（相对下降约2%）。

Conclusion: MoBE是一种高效且低损失的MoE模型压缩方法，优于现有技术。
...
### [169] [Marine Chlorophyll Prediction and Driver Analysis based on LSTM-RF Hybrid Models](https://arxiv.org/abs/2508.05260)
...
TL;DR: 提出了一种结合LSTM和RF的混合模型（LSTM-RF），用于预测海洋叶绿素浓度，解决了单一模型在时间序列建模和非线性特征刻画上的不足。实验表明该模型优于单独使用LSTM或RF。
...
Result: LSTM-RF模型在测试集上的R²为0.5386，MSE为0.005806，MAE为0.057147，显著优于单独使用LSTM（R²=0.0208）和RF（R²=0.4934）。

Conclusion: LSTM-RF模型为高频预测海洋生态变量提供了创新解决方案，提升了预测精度。
...
### [170] [FlowState: Sampling Rate Invariant Time Series Forecasting](https://arxiv.org/abs/2508.05287)
*Lars Graf,Thomas Ortner,Stanisław Woźniak,Angeliki Pantazi*

Main category: cs.LG

TL;DR: FlowState是一种新型时间序列基础模型（TSFM），通过状态空间模型编码器和函数基解码器解决了现有TSFM在泛化性、适应性和计算效率上的不足。
...
### [171] [RLHF Fine-Tuning of LLMs for Alignment with Implicit User Feedback in Conversational Recommenders](https://arxiv.org/abs/2508.05289)
*Zhongheng Yang,Aijia Sun,Yushang Zhao,Yinuo Yang,Dannier Li,Chengrui Zhou*

Main category: cs.LG

TL;DR: 本文提出了一种基于人类反馈强化学习（RLHF）的对话推荐系统（CRS）优化方法，通过最大化隐含用户反馈（IUF）来提升推荐效果。


<details>
  <summary>Details</summary>
Motivation: 传统的监督微调方法无法捕捉隐含的用户反馈信号（如停留时间、情感极性等），因此需要一种更有效的方法来优化基于大语言模型（LLM）的CRS。

Method: 使用RLHF方法，通过奖励模型R_{\phi}学习弱标记的参与信息，并采用近端策略优化（PPO）方法优化基础LLM M_{\theta}。模型架构模拟对话状态转移（s_t \to a_t \to s_{t+1}），其中动作a_t仅在对话历史条件下生成推荐。

Result: 在合成和真实数据集（如REDIAL、OpenDialKG）上的评估表明，RLHF微调模型在top-k推荐准确性、连贯性和用户满意度方面表现更优。

Conclusion: 隐含信号对齐可以高效实现CRS的可扩展和用户自适应设计。
...
### [172] [Optimal Growth Schedules for Batch Size and Learning Rate in SGD that Reduce SFO Complexity](https://arxiv.org/abs/2508.05297)
*Hikaru Umeda,Hideaki Iiduka*

Main category: cs.LG

TL;DR: 论文研究了深度学习训练中批量大小和学习率的调度问题，提出了基于随机一阶预言机（SFO）复杂度的最优增长策略，以平衡效率和收敛性。
...
Result: 提出的调度策略显著降低了SFO复杂度，为大规模批量训练提供了理论指导和实用建议。

Conclusion: 研究为深度学习中的高效大规模训练提供了理论和实践支持。
...
### [173] [Adaptive Batch Size and Learning Rate Scheduler for Stochastic Gradient Descent Based on Minimization of Stochastic First-order Oracle Complexity](https://arxiv.org/abs/2508.05302)
*Hikaru Umeda,Hideaki Iiduka*
...
<details>
  <summary>Details</summary>
Motivation: 由于SGD的收敛行为对批大小和学习率高度敏感，研究如何通过自适应调整这些参数来优化训练效率。

Method: 引入了一种自适应调度策略，根据训练过程中观察到的全梯度范数衰减动态调整批大小和学习率。

Result: 实验表明，基于该策略的自适应联合调度器相比现有调度器具有更快的收敛速度。

Conclusion: 通过理论支持的临界批大小和自适应调度策略，可以有效提升SGD的训练效率。
...
### [174] [Divide-and-Conquer for Enhancing Unlabeled Learning, Stability, and Plasticity in Semi-supervised Continual Learning](https://arxiv.org/abs/2508.05316)
*Yue Duan,Taicai Chen,Lei Qi,Yinghuan Shi*

Main category: cs.LG

TL;DR: USP框架通过分而治之的方法，协同提升半监督持续学习中的学习可塑性、无监督学习和记忆稳定性，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 半监督持续学习（SSCL）旨在利用标记和未标记数据降低标注成本，但需平衡无监督学习、记忆稳定性和学习可塑性。

Method: 提出USP框架，包括特征空间保留策略（FSR）、分而治之伪标签（DCP）和类均值锚定无监督蒸馏（CUD）。

Result: USP在最终准确率上比现有方法提升高达5.94%。

Conclusion: USP有效解决了SSCL中的关键挑战，代码已开源。
...
### [175] [Optimal Corpus Aware Training for Neural Machine Translation](https://arxiv.org/abs/2508.05364)
*Yi-Hsiu Liao,Cheng Shen,Brenda,Yang*

Main category: cs.LG

TL;DR: OCAT通过冻结大部分参数并仅调整少量语料相关参数，优化了CAT模型的训练，提升了模型准确率，且在WMT23任务中表现优异。
...
Method: OCAT通过冻结大部分参数，仅微调少量语料相关参数，优化CAT预训练模型。

Result: 在WMT23英中和英德翻译任务中，OCAT分别提升了3.6和1.8 chrF，且对超参数设置不敏感。

Conclusion: OCAT轻量、抗过拟合且高效，性能与现有最优微调技术相当或更优。
...
### [176] [Latent Preference Bandits](https://arxiv.org/abs/2508.05367)
*Newton Mwai,Emil Carlsson,Fredrik D. Johansson*

Main category: cs.LG

TL;DR: 论文提出了一种放宽潜在老虎机假设的方法，仅需模型对每个潜在状态中动作的偏好排序，而不需完整的奖励分布。


<details>
  <summary>Details</summary>
Motivation: 个性化任务中，单个个体面临决策点较少，学习成本高。潜在老虎机虽能减少探索时间，但需已知准确的潜在状态与奖励分布模型，实践中难以满足。

Method: 放宽潜在老虎机的假设，仅要求模型对每个潜在状态中动作的偏好排序一致，允许奖励分布不同。提出了一种后验采样算法。

Result: 该算法在奖励分布已知时性能与潜在老虎机相当，在奖励尺度不同时表现更优。

Conclusion: 放宽假设的潜在老虎机方法在个性化任务中更具实用性和鲁棒性。
...
### [177] [Echo: Decoupling Inference and Training for Large-Scale RL Alignment on Heterogeneous Swarms](https://arxiv.org/abs/2508.05387)
*Jie Xiao,Shaoduo Gan,Changyuan Fan,Qingnan Ren,Alfred Long,Yuchen Zhang,Rymon Yu,Eric Yang,Lynn Ai*

Main category: cs.LG

TL;DR: Echo是一种RL系统，通过分离推理和训练阶段，利用异构资源实现高效训练，同时保持统计效率。


<details>
  <summary>Details</summary>
Motivation: 现代基于RL的LLM后训练将轨迹采样和策略优化放在同一GPU集群上，导致系统在推理和训练工作负载之间切换，违反了SPMD假设。

Method: Echo通过异构的“推理”和“训练”集群分离这两个阶段，并引入两种轻量级同步协议：顺序拉取模式和异步推送-拉取模式。

Result: 在分布式集群上训练三种RL工作负载时，Echo在收敛速度和最终奖励上与完全共置的Verl基线相当，同时将轨迹生成卸载到边缘硬件。

Conclusion: 结果表明，利用分散的异构资源，大规模RL可以为LLM实现数据中心级性能。
...
### [178] [NT-ML: Backdoor Defense via Non-target Label Training and Mutual Learning](https://arxiv.org/abs/2508.05404)
...
<details>
  <summary>Details</summary>
Motivation: 深度神经网络易受后门攻击，需要一种高效的防御机制来恢复被污染模型。

Method: 采用非目标标签训练（NT）和互学习（ML）两阶段方法，结合教师模型和学生模型的优势。
...
### [179] [Cumulative Learning Rate Adaptation: Revisiting Path-Based Schedules for SGD and Adam](https://arxiv.org/abs/2508.05408)
*Asma Atamna,Tom Maus,Fabian Kievelitz,Tobias Glasmachers*

Main category: cs.LG

TL;DR: 本文研究了自适应学习率机制在深度学习中的实用性，重新审视了一种基于累积路径的调整方法，并提出了针对Adam优化器的修正版本。


<details>
  <summary>Details</summary>
Motivation: 学习率是深度学习中的关键超参数，其理想值可能随训练过程变化。本文旨在探讨动态调整学习率的实际效果。

Method: 重新审视2017年提出的基于累积路径的自适应学习率方法，针对Adam优化器提出修正版本，并进行实验比较。

Result: 实验比较了SGD和Adam优化器在有/无累积路径自适应的情况下的表现，并与最新方法对比。

Conclusion: 研究旨在明确自适应学习率策略的实际优势及其适用场景。
...
### [180] [MolSnap: Snap-Fast Molecular Generation with Latent Variational Mean Flow](https://arxiv.org/abs/2508.05411)
*Md Atik Ahamed,Qiang Ye,Qiang Cheng*

Main category: cs.LG

TL;DR: 提出了一种新的因果关系感知框架，通过Causality-Aware Transformer和Variational Mean Flow，解决了分子生成任务中高质量、多样性和快速推理的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有方法在分子生成任务中难以同时保证高质量、多样性和快速推理。

Method: 1. 提出Causality-Aware Transformer（CAT），联合编码分子图标记和文本指令，并在生成过程中强制因果依赖。2. 开发Variational Mean Flow（VMF），通过将潜在空间建模为高斯混合，增强表达能力。

Result: 在四个标准分子基准测试中，模型表现优于现有方法，新颖性（74.5%）、多样性（70.3%）和有效性（100%）均显著提升，且计算效率高。

Conclusion: 该框架在分子生成任务中实现了高质量、多样性和高效推理的平衡，为计算化学和药物发现提供了新工具。
...
### [181] [Negative Binomial Variational Autoencoders for Overdispersed Latent Modeling](https://arxiv.org/abs/2508.05423)
*Yixuan Zhang,Wenxin Zhang,Hua Jiang,Quyu Kong,Feng Zhou*

Main category: cs.LG

TL;DR: NegBio-VAE扩展了VAE框架，使用负二项分布建模神经活动的尖峰计数，解决了传统Poisson-VAE中均值与方差相等的刚性约束问题，显著提升了重建精度。


<details>
  <summary>Details</summary>
Motivation: 传统VAE和Poisson-VAE无法准确模拟神经活动的变异性，尤其是尖峰计数的过分散现象。

Method: 提出NegBio-VAE，采用负二项分布建模尖峰计数，开发了两种ELBO优化方案和可微分重参数化策略。

Result: 实验表明，NegBio-VAE通过引入分散参数，显著提高了重建保真度。

Conclusion: 显式建模过分散对尖峰类激活的准确表示至关重要，NegBio-VAE为此提供了有效解决方案。
...
### [182] [Federated Multi-Objective Learning with Controlled Pareto Frontiers](https://arxiv.org/abs/2508.05424)
*Jiansheng Rao,Jiayi Li,Zhizhi Gong,Soummya Kar,Haoxuan Li*

Main category: cs.LG

TL;DR: CR-FMOL是一种新的联邦多目标学习框架，通过偏好锥约束实现客户端公平性，优于传统方法。
...
Result: 在非独立同分布数据上表现优于传统方法，早期性能略低于FedAvg，但长期可达到相近精度。

Conclusion: CR-FMOL有效提升客户端公平性，是联邦学习中多目标优化的可行方案。
...
### [183] [Group Causal Policy Optimization for Post-Training Large Language Models](https://arxiv.org/abs/2508.05428)
*Ziyin Gu,Jingyao Wang,Ran Zuo,Chuxiong Sun,Zeen Song,Changwen Zheng,Wenwen Qiang*

Main category: cs.LG

TL;DR: 论文提出了一种名为GCPO的新方法，通过引入因果结构优化语言模型在专业领域的表现，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型（LLMs）在广泛任务中表现优异，但专业领域仍需针对性优化。现有方法如GRPO虽高效，但忽略了候选回答间的语义交互。

Method: 引入结构因果模型（SCM）揭示候选回答间的隐藏依赖关系，并提出GCPO方法，结合因果奖励调整和KL正则化。

Result: 实验表明，GCPO在多推理基准测试中优于GRPO等现有方法。

Conclusion: GCPO通过整合因果结构，显著提升了语言模型在专业任务中的表现。
...
### [184] [Competing Risks: Impact on Risk Estimation and Algorithmic Fairness](https://arxiv.org/abs/2508.05435)
*Vincent Jeanselme,Brian Tom,Jessica Barrett*

Main category: cs.LG

TL;DR: 论文探讨了将竞争风险视为截尾数据会导致生存估计的显著偏差，并量化了这种错误及其对预测性能和算法公平性的影响。


<details>
  <summary>Details</summary>
Motivation: 准确的时间到事件预测对决策至关重要，但竞争风险常被误处理为截尾数据，导致偏差和加剧不公平。

Method: 开发了一个框架来量化将竞争风险误分类为截尾数据的错误，并分析其对预测性能和公平性的影响。

Result: 研究显示，忽视竞争风险会高估风险并加剧群体间的不公平，特别是在心血管管理的实证分析中。
...
### [185] [Tail-Risk-Safe Monte Carlo Tree Search under PAC-Level Guarantees](https://arxiv.org/abs/2508.05441)
*Zuyuan Zhang,Arnob Ghosh,Tian Lan*

Main category: cs.LG

TL;DR: 论文提出两种新方法（CVaR-MCTS和W-MCTS）来解决MCTS中尾部风险控制不足的问题，通过引入CVaR和Wasserstein模糊集，实现尾部风险保障。
...
Method: 1. CVaR-MCTS：嵌入CVaR实现尾部风险控制；2. W-MCTS：引入Wasserstein模糊集解决尾部风险估计偏差。

Result: 在模拟环境中，新方法优于基线，实现稳健的尾部风险保障和更高回报。

Conclusion: CVaR-MCTS和W-MCTS有效解决了MCTS尾部风险控制问题，提供理论保障和实际性能提升。
...
### [186] [EnergyPatchTST: Multi-scale Time Series Transformers with Uncertainty Estimation for Energy Forecasting](https://arxiv.org/abs/2508.05454)
*Wei Li,Zixin Wang,Qizheng Sun,Qixiang Gao,Fenglei Yang*

Main category: cs.LG

TL;DR: EnergyPatchTST是一种基于Patch Time Series Transformer的改进方法，用于能源时间序列预测，通过多尺度特征提取和概率预测框架提升准确性。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在多尺度时间动态和真实数据不规则性方面存在局限，需要更准确的预测方法以支持电力规划和分配。

Method: 提出EnergyPatchTST，包含多尺度特征提取、概率预测框架、未来已知变量整合路径及预训练微调策略。
...
### [187] [Task complexity shapes internal representations and robustness in neural networks](https://arxiv.org/abs/2508.05463)
...
<details>
  <summary>Details</summary>
Motivation: 神经网络虽然性能优异，但其内部表示如何受输入数据复杂性和任务难度影响仍不清楚。本文旨在量化任务难度对MLP表示的影响。

Method: 使用五种数据无关的探针方法（剪枝、二值化、噪声注入、符号翻转和二分图随机化），对比分析MNIST和Fashion-MNIST数据集上简单和复杂分类任务中MLP的表示特性。

Result: 发现二值化权重在复杂任务中导致准确率崩溃，而简单任务模型仍保持鲁棒性；剪枝揭示了性能的急剧相变；噪声注入可提升准确率；仅保留符号结构即可维持高准确率。

Conclusion: 符号二分图拓扑在表示学习中至关重要，研究结果为模型压缩和可解释性提供了实用策略，并提出了一种任务复杂度的通用度量方法。
...
### [188] [Let's Measure Information Step-by-Step: LLM-Based Evaluation Beyond Vibes](https://arxiv.org/abs/2508.05469)
*Zachary Robertson,Sanmi Koyejo*

Main category: cs.LG

TL;DR: 论文提出了一种无需真实标签评估AI系统的方法，利用游戏抵抗性与输出质量的关系，证明了f-互信息是唯一自然条件下游戏抵抗的机制，并在多个领域验证了其优越性。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统评估常依赖真实标签，但获取成本高且不可行。研究旨在开发无需真实标签的评估机制，抵抗策略性操纵。

Method: 通过数据处理的不可逆性，证明f-互信息是唯一游戏抵抗机制，并比较了不同信息度量（如总变差距离）的可行性。

Result: 在翻译和同行评审等十个领域，信息论机制能完美区分忠实与策略性代理，且比现有方法抗操纵性强10-100倍。

Conclusion: 该方法在压缩比为10:1时性能最佳，为AI系统评估提供了高效且抗操纵的解决方案。
...
### [189] [Prediction of Survival Outcomes under Clinical Presence Shift: A Joint Neural Network Architecture](https://arxiv.org/abs/2508.05472)
...
Result: 在MIMIC-III数据集上的死亡率预测任务中，该方法比现有模型表现更好，且更具可迁移性。
...
### [190] [Parameter-free entropy-regularized multi-view clustering with hierarchical feature selection](https://arxiv.org/abs/2508.05504)
*Kristina P. Sinaga,Sara Colantonio,Miin-Shen Yang*

Main category: cs.LG

TL;DR: 论文提出了两种互补的算法AMVFCM-U和AAMVFCM-U，用于多视图聚类，通过熵正则化和自适应特征加权，实现了无参数框架和高效计算。
...
Method: 引入熵正则化替代模糊化参数，采用信噪比正则化进行特征加权，并通过双层级熵自动平衡视图和特征的贡献。AAMVFCM-U进一步通过自适应阈值实现分层降维。

Result: 在五个基准测试中优于15种先进方法，AAMVFCM-U计算效率提升97%，维度降至原尺寸的0.45%，并能自动识别关键视图组合。

Conclusion: 提出的算法在多视图聚类中实现了高效、自动化的模式发现，显著提升了性能和计算效率。
...
### [191] [Tractable Sharpness-Aware Learning of Probabilistic Circuits](https://arxiv.org/abs/2508.05537)
...
<details>
  <summary>Details</summary>
Motivation: 随着PCs模型的深度和表达能力增强，数据有限时容易过拟合，作者从对数似然景观角度分析过拟合问题，发现其常由收敛到尖锐最优解引起。

Method: 受神经网络中锐度感知最小化的启发，提出了一种基于Hessian的正则化方法，通过高效计算对数似然的Hessian迹（尖锐度代理）来指导训练。

Result: 实验表明，该方法能有效引导PCs收敛到更平坦的最小值，提升泛化性能。

Conclusion: Hessian迹正则化方法简单高效，适用于EM和梯度学习，显著改善了PCs的泛化能力。
...
### [192] [Adapting Vision-Language Models Without Labels: A Comprehensive Survey](https://arxiv.org/abs/2508.05547)
*Hao Dong,Lijun Sheng,Jian Liang,Ran He,Eleni Chatzi,Olga Fink*

Main category: cs.LG

TL;DR: 该论文提出了一种针对无监督视觉语言模型（VLM）适应的统一分类法，总结了四种关键范式，并分析了相关方法和挑战。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在广泛任务中表现出色，但在特定下游任务中表现欠佳，缺乏无监督适应的系统性研究。

Method: 提出基于无标签视觉数据可用性和性质的分类法，分为四种范式：无数据迁移、无监督域迁移、批量数据适应和流数据适应。

Result: 系统分析了每种范式的核心方法和策略，并总结了代表性基准和未来研究方向。

Conclusion: 该研究填补了无监督VLM适应领域的空白，为未来研究提供了系统框架和资源。
...
### [193] [X-VFL: A New Vertical Federated Learning Framework with Cross Completion and Decision Subspace Alignment](https://arxiv.org/abs/2508.05568)
...
<details>
  <summary>Details</summary>
Motivation: 垂直联邦学习（VFL）面临数据样本必须对齐且不允许缺失特征，以及需要所有客户端联合推断的挑战。

Method: X-VFL通过两个新模块（XCom和DS-Align）完成缺失特征并实现本地独立推断。

Result: 实验显示X-VFL在CIFAR-10和MIMIC-III数据集上分别提升了15%和43%的准确率。

Conclusion: X-VFL在部分缺失特征和本地独立推断场景中表现出色。
...
### [194] [Fairy$\pm i$: the First 2-bit Complex LLM with All Parameters in $\{\pm1, \pm i\}$](https://arxiv.org/abs/2508.05571)
...
<details>
  <summary>Details</summary>
Motivation: 当前QAT研究仅关注最小化量化误差，未尝试超越全精度模型的性能上限。

Method: 利用复数域表示优势，将权重映射到四次单位根{±1, ±i}，实现无乘法推理。

Result: Fairy±i在PPL和下游任务中超越现有2位量化方法的性能上限。

Conclusion: 该工作为极低比特约束下构建高精度LLM开辟了新方向。
...
### [195] [Iterative Learning of Computable Phenotypes for Treatment Resistant Hypertension using Large Language Models](https://arxiv.org/abs/2508.05581)
*Guilherme Seidyo Imai Aldeia,Daniel S. Herman,William G. La Cava*

Main category: cs.LG

TL;DR: 研究探讨大型语言模型（LLMs）能否生成准确且简洁的可计算表型（CPs），并通过迭代学习提升性能，结果表明LLMs在少量训练样本下接近最先进ML方法的性能。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs在生成可计算表型方面的潜力，以支持高血压患者的临床决策。

Method: 提出并测试了“合成、执行、调试、指导”策略，利用LLMs生成并通过数据驱动反馈迭代优化CPs。

Result: LLMs结合迭代学习能生成可解释且准确性合理的程序，性能接近最先进ML方法，且所需训练样本显著减少。

Conclusion: LLMs在生成可计算表型方面具有潜力，尤其在样本有限的情况下表现优异。
...
### [196] [Enhancing PyKEEN with Multiple Negative Sampling Solutions for Knowledge Graph Embedding Models](https://arxiv.org/abs/2508.05587)
*Claudia d'Amato,Ivan Diliso,Nicola Fanizzi,Zafar Saeed*

Main category: cs.LG

TL;DR: 论文提出了一种扩展PyKEEN框架的方法，集成了多种先进的负采样策略，以生成更有意义的负样本，并验证了其对知识图谱嵌入模型性能的影响。


<details>
  <summary>Details</summary>
Motivation: 现有的知识图谱嵌入方法在缺乏负样本时通常依赖简单的负采样策略，而主流库缺乏对高级策略的支持，限制了性能提升。

Method: 开发了一个PyKEEN框架的扩展，整合了多种静态和动态负采样策略，并保持与现有工作流的兼容性。

Result: 通过实证研究验证了扩展对嵌入模型性能（如链接预测任务）的积极影响。

Conclusion: 该扩展不仅提升了PyKEEN的功能，还为嵌入方法的开发和定制提供了更灵活的工具。
...
### [197] [Optimizing IoT Threat Detection with Kolmogorov-Arnold Networks (KANs)](https://arxiv.org/abs/2508.05591)
*Natalia Emelianova,Carlos Kamienski,Ronaldo C. Prati*

Main category: cs.LG

TL;DR: Kolmogorov-Arnold Networks (KANs) 在物联网入侵检测中表现优于传统MLPs，并与先进模型如随机森林和XGBoost竞争，同时提供更好的可解释性。
...
Method: 研究比较了KANs与传统MLPs及先进模型（如随机森林和XGBoost）在物联网入侵检测中的性能。
...
### [198] [Non-omniscient backdoor injection with a single poison sample: Proving the one-poison hypothesis for linear regression and linear classification](https://arxiv.org/abs/2508.05600)
*Thorsten Peinemann,Paula Arnold,Sebastian Berndt,Thomas Eisenbarth,Esfandiar Mohammadi*

Main category: cs.LG

TL;DR: 论文探讨了后门注入攻击，提出“一毒假设”：仅需一个毒样本即可成功注入后门且不影响正常任务性能，并在线性回归和分类中验证。


<details>
  <summary>Details</summary>
Motivation: 研究后门攻击中所需毒数据的最小量，解决现有攻击方法需大量毒样本或过多背景信息的局限性。

Method: 提出“一毒假设”，理论证明其在线性回归和分类中的可行性，并通过实验验证。

Result: 证明仅需一个毒样本即可实现零误差后门注入，且对正常任务影响有限。

Conclusion: “一毒假设”为后门攻击提供新视角，表明攻击可能更隐蔽且高效。
...
### [199] [Shuffle-R1: Efficient RL framework for Multimodal Large Language Models via Data-centric Dynamic Shuffle](https://arxiv.org/abs/2508.05612)
*Linghao Zhu,Yiran Guan,Dingkang Liang,Jianzhong Ju,Zhenbo Luo,Bin Qin,Jian Luan,Yuliang Liu,Xiang Bai*

Main category: cs.LG

TL;DR: Shuffle-R1框架通过动态重组轨迹采样和批次组成，解决了强化学习中的优势崩溃和滚动沉默问题，提升了多模态大语言模型（MLLM）的训练效率。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习（RL）训练中存在优势崩溃（Advantage Collapsing）和滚动沉默（Rollout Silencing）问题，导致梯度更新不理想，影响长期学习效率。

Method: 提出Shuffle-R1框架，包括成对轨迹采样（Pairwise Trajectory Sampling）和基于优势的轨迹重组（Advantage-based Trajectory Shuffle），以优化梯度信号和批次组成。

Result: 实验表明，Shuffle-R1在多个推理基准测试中优于现有RL基线方法，且计算开销低。

Conclusion: 数据为中心的调整对提升MLLM中RL训练效率至关重要。
...
### [200] [On the Generalization of SFT: A Reinforcement Learning Perspective with Reward Rectification](https://arxiv.org/abs/2508.05629)
...
Result: DFT在多个基准测试和基础模型上显著优于标准SFT，并展示了更好的泛化能力。

Conclusion: DFT通过理论指导的简单改进，显著提升了SFT的性能，并为离线强化学习提供了有效替代方案。
...
### [201] [Voost: A Unified and Scalable Diffusion Transformer for Bidirectional Virtual Try-On and Try-Off](https://arxiv.org/abs/2508.04825)
...
<details>
  <summary>Details</summary>
Motivation: 虚拟试穿中服装与身体的准确对应在姿态和外观变化下仍具挑战性，需更高效的建模方法。

Method: Voost采用扩散变换器联合学习试穿和试脱，支持灵活生成方向与服装类别条件，并引入注意力温度缩放和自校正采样技术。

Result: Voost在试穿和试脱基准测试中达到最优性能，对齐精度、视觉保真度和泛化能力均优于基线。

Conclusion: Voost通过统一框架和推理技术有效解决了虚拟试穿中的服装-身体对应问题，具有广泛适用性。
...
### [202] [Perceive-Sample-Compress: Towards Real-Time 3D Gaussian Splatting](https://arxiv.org/abs/2508.04965)
*Zijian Wang,Beizhen Zhao,Hao Wang*

Main category: cs.GR

TL;DR: 提出了一种感知-采样-压缩框架，优化3D高斯泼溅技术，提升大规模场景管理和存储效率。


<details>
  <summary>Details</summary>
Motivation: 传统3D高斯泼溅技术在大规模场景管理和存储效率上存在不足。

Method: 提出场景感知补偿算法、金字塔采样表示和广义高斯混合模型压缩算法。

Result: 显著提升内存效率和视觉质量，同时保持实时渲染速度。

Conclusion: 新方法有效解决了传统3DGS的局限性，适用于复杂环境和有限计算资源。
...
### [203] [Laplacian Analysis Meets Dynamics Modelling: Gaussian Splatting for 4D Reconstruction](https://arxiv.org/abs/2508.04966)
*Yifan Zhou,Beizhen Zhao,Pengcheng Wu,Hao Wang*

Main category: cs.GR

TL;DR: 本文提出了一种新的动态3D高斯泼溅（3DGS）框架，通过混合显式-隐式函数解决了现有方法在动态场景建模中的过平滑和特征碰撞问题。
...
Method: 提出了一种混合显式-隐式函数框架，包括频谱感知的拉普拉斯编码架构、增强的高斯动态属性以及基于KDTree的自适应高斯分裂策略。

Result: 实验表明，该方法在复杂动态场景重建中表现出色，具有更高的重建保真度。

Conclusion: 该方法通过创新的频谱控制和动态优化策略，显著提升了动态3DGS的性能。
...
### [204] [A Study of the Framework and Real-World Applications of Language Embedding for 3D Scene Understanding](https://arxiv.org/abs/2508.05064)
...
<details>
  <summary>Details</summary>
Motivation: 填补语言引导与3D高斯泼溅技术结合研究的空白，推动语言指导的3D场景理解发展。

Method: 结构化回顾当前研究，包括理论基础、整合策略和实际用例。

Result: 总结了计算瓶颈、泛化能力及语义标注数据不足等关键限制。
...
### [205] [RAP: Real-time Audio-driven Portrait Animation with Video Diffusion Transformer](https://arxiv.org/abs/2508.05115)
*Fangyu Du,Taiqing Li,Ziwei Zhang,Qian Qiao,Tan Yu,Dingcheng Zhen,Xu Jia,Yang Yang,Shunshun Yin,Siyuan Liu*

Main category: cs.GR

TL;DR: RAP提出了一种实时音频驱动肖像动画框架，通过混合注意力机制和静态-动态训练范式，实现了高质量且实时的动画生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法因计算复杂无法实时运行，而压缩表示又难以保留细节。RAP旨在解决这一矛盾。

Method: 引入混合注意力机制和静态-动态训练范式，避免显式运动监督。

Result: RAP在实时约束下实现了高质量动画生成，性能优于现有方法。

Conclusion: RAP为实时音频驱动肖像动画提供了高效且高质量的解决方案。
...
### [206] [Refining Gaussian Splatting: A Volumetric Densification Approach](https://arxiv.org/abs/2508.05187)
...
### [207] [GASP: A Gradient-Aware Shortest Path Algorithm for Boundary-Confined Visualization of 2-Manifold Reeb Graphs](https://arxiv.org/abs/2508.05524)
...
<details>
  <summary>Details</summary>
Motivation: 现有Reeb图绘制算法忽视或违反边界约束、紧凑性和梯度对齐等关键属性，导致可视化结果不能准确反映数据。

Method: 提出GASP算法，确保Reeb图满足边界约束、紧凑性和梯度对齐。

Result: 通过定性和定量评估，GASP生成的Reeb图优于几何重心算法（TTK实现）。

Conclusion: GASP算法能更准确地表示数据拓扑结构，提升可视化效果。
...
### [208] [Point cloud segmentation for 3D Clothed Human Layering](https://arxiv.org/abs/2508.05531)
...
<details>
  <summary>Details</summary>
Motivation: 3D衣物建模和模拟在多个领域至关重要，但现有方法难以处理衣物与身体的重叠部分，缺乏语义信息。

Method: 提出了一种新的3D点云分割范式，支持每个点同时关联不同层次，并创建了一个合成数据集用于训练和验证。

Result: 实验证明该方法在合成和真实扫描数据集上均有效，能够准确分割衣物层次。

Conclusion: 该研究为衣物建模提供了一种有效的语义分割方法，解决了重叠区域的识别问题。
...
### [209] [Physically Controllable Relighting of Photographs](https://arxiv.org/abs/2508.05626)
*Chris Careaga,Yağız Aksoy*

Main category: cs.GR

TL;DR: 提出了一种自监督的图像重光照方法，结合传统渲染的物理精确性和神经渲染的逼真效果，实现完全可控的光照编辑。
...
Method: 通过单目几何和固有组件估计推断场景的彩色网格表示，结合路径追踪引擎和神经渲染器实现逼真重光照。

Result: 实现了对真实场景的高质量、可控重光照，且通过自监督训练在原始图像集合上有效工作。

Conclusion: 该方法为真实场景重光照提供了物理精确且逼真的解决方案，显著提升了光照编辑的灵活性和质量。
...
### [210] [Optimality Principles and Neural Ordinary Differential Equations-based Process Modeling for Distributed Control](https://arxiv.org/abs/2508.04799)
*Michael R. Wartmann,B. Erik Ydstie*

Main category: cs.NE

TL;DR: 提出了一种过程建模框架，将数据驱动算法与经典过程模型和控制自然集成，通过拓扑属性和守恒量实现一致性。
...
Method: 使用连通性矩阵和网络图表示过程网络单元的互连，推导系统的自然目标函数（非平衡熵产生），并通过稀疏深度神经网络集成拓扑与数据驱动的动态关系。

Result: 在库存控制系统的实例中，成功将过程拓扑与神经网络ODE模型集成，形成状态空间模型用于模型预测控制。

Conclusion: 该框架能够将拓扑的基本守恒性质与数据驱动的动态关系结合，为过程控制提供新的建模方法。
...
### [211] [Modelling the emergence of open-ended technological evolution](https://arxiv.org/abs/2508.04828)
*James Winters,Mathieu Charbonneau*

Main category: cs.NE

TL;DR: 论文探讨了人类社会如何通过技术与需求空间的协同演化实现开放式的技术进步，指出这种增长罕见且依赖特定条件。


<details>
  <summary>Details</summary>
Motivation: 研究人类社会如何通过技术与需求的动态互动实现资源的持续增长和信息的无限扩展。

Method: 提出宏观模型，分析技术系统和需求空间在文化进化中的动态关系，探讨随机与选择过程的影响。

Result: 开放式增长极为罕见，需技术与需求协同演化，随机扰动与选择机制共同作用。

Conclusion: 开放式技术进化需技术系统与需求空间有效协同演化，维持资源持续增长。
...
### [212] [AI Should Be More Human, Not More Complex](https://arxiv.org/abs/2508.04713)
*Carlo Esposito*

Main category: cs.HC

TL;DR: 研究发现，用户更喜欢简洁且有来源标注的AI回答，而非冗长复杂的解释。当前AI追求“人工复杂化”反而降低用户信任和满意度。


<details>
  <summary>Details</summary>
Motivation: 探讨AI搜索应用中冗长复杂回答对用户体验的负面影响，挑战“复杂回答等于高性能”的假设。

Method: 通过对10,000名参与者比较五大AI搜索系统的回答，分析用户偏好。

Result: 用户明显偏好简洁、有来源标注的回答，复杂回答导致认知负担和信任下降。

Conclusion: AI应模仿人类有效沟通方式：直接、透明且有来源标注，而非追求复杂化。
...
### [213] [Evaluating the Impact of LLM-guided Reflection on Learning Outcomes with Interactive AI-Generated Educational Podcasts](https://arxiv.org/abs/2508.04787)
*Vishnu Menon,Andy Cherney,Elizabeth B. Cloude,Li Zhang,Tiffany D. Do*

Main category: cs.HC

TL;DR: 研究发现，在AI生成的播客中加入LLM引导的反思提示并未显著提升学习效果，但降低了用户的吸引力感知。


<details>
  <summary>Details</summary>
Motivation: 探讨反思提示在AI生成内容中对学习和用户体验的影响。

Method: 36名本科生参与实验，对比有反思提示和无提示版本的AI播客。

Result: 学习效果相似，但反思提示降低了用户的吸引力感知。
...
### [214] [At a Glance to Your Fingertips: Enabling Direct Manipulation of Distant Objects Through SightWarp](https://arxiv.org/abs/2508.04821)
*Yang Liu,Thorbjørn Mikkelsen,Zehai Liu,Gengchen Tian,Diako Mardanbegi,Qiushi Zhou,Hans Gellersen,Ken Pfeuffer*

Main category: cs.HC

TL;DR: SightWarp是一种利用眼手协调技术，将远处物体无缝召唤到用户指尖的交互技术，支持直接手势操作。


<details>
  <summary>Details</summary>
Motivation: 解决3D用户界面中远处物体难以直接操作的问题，提供更自然的交互体验。

Method: 通过用户注视远处物体后，将视线移至手部或移动手部触发创建近空间代理，支持直接手势操作。

Result: 用户研究表明，SightWarp比注视和捏合技术更高效，提升了6DOF操作等任务的性能。

Conclusion: SightWarp为远近空间物体交互提供了灵活且高效的解决方案。
...
### [215] [Charts-of-Thought: Enhancing LLM Visualization Literacy Through Structured Data Extraction](https://arxiv.org/abs/2508.04842)
*Amit Kumar Das,Mohammad Tarun,Klaus Mueller*

Main category: cs.HC

TL;DR: 论文评估了现代大型语言模型（LLM）的可视化素养，并提出了名为Charts-of-Thought的新提示技术。该方法显著提升了LLM在可视化测试中的表现，甚至超越人类基准。


<details>
  <summary>Details</summary>
Motivation: 研究动机是评估和改进LLM在可视化任务中的表现，探索结构化提示技术对复杂视觉解释任务的影响。

Method: 通过Charts-of-Thought方法，系统引导LLM完成数据提取、验证和分析，再回答可视化问题。测试了三种先进LLM（Claude-3.7-sonnet、GPT-4.5 preview和Gemini-2.0-pro）。

Result: Claude-3.7-sonnet得分50.17，远超人类基准28.82。所有模型性能均提升（GPT-4.5提高21.8%，Gemini-2.0提高9.4%，Claude-3.7提高13.5%）。

Conclusion: 研究表明，现代多模态LLM在适当分析框架下可超越人类可视化素养表现，结构化提示策略对复杂视觉任务至关重要，同时可能提升可视化可访问性。
...
### [216] [An Implementation of a Visual Stepper in the GRASP Programming System](https://arxiv.org/abs/2508.04859)
*Panicz Maciej Godek*
...
<details>
  <summary>Details</summary>
Motivation: 直接目的是展示视觉评估器扩展的实现，间接目的是提供GRASP设计和扩展机制的教程。尽管GRASP尚未完成，但解决类似问题的思路对Scheme社区可能有价值。

Method: 通过描述GRASP的视觉评估器扩展实现和扩展机制架构，提供具体的设计和实现细节。

Result: 尽管GRASP尚未最终完成，但提出的问题和解决方案对类似系统的开发具有参考意义。

Conclusion: GRASP的设计和扩展机制问题对Scheme社区具有潜在价值，尽管具体实现可能在未来发生变化。
...
### [217] [Learning AI Auditing: A Case Study of Teenagers Auditing a Generative AI Model](https://arxiv.org/abs/2508.04902)
*Luis Morales-Navarro,Michelle Gan,Evelyn Yu,Lauren Vogelstein,Yasmin B. Kafai,Danaé Metaxa*

Main category: cs.HC

TL;DR: 研究探讨高中生如何通过算法审计识别和理解日常AI/ML工具的偏见，通过工作坊发现青少年能独立探索新问题，如年龄偏见，并得出与专家相似的结论。


<details>
  <summary>Details</summary>
Motivation: 随着AI/ML技术融入青少年生活，亟需培养其AI素养，包括技术知识和社会影响意识。

Method: 为期两周的参与式设计工作坊，14名14-15岁青少年审计TikTok的生成AI模型。

Result: 青少年在审计中表现出创造性和独立性，发现与专家相似的偏见结论。

Conclusion: 算法审计可作为培养AI素养的学习活动，赋予青少年批判性思维，并为算法危害研究提供新视角。
...
### [218] [Root Cause Analysis Training for Healthcare Professionals With AI-Powered Virtual Simulation: A Proof-of-Concept](https://arxiv.org/abs/2508.04904)
*Yuqi Hu,Qiwen Xiong,Zhenzhen Qin,Brandon Watanabe,Yujing Wang,Mirjana Prpa,Ilmi Yoon*

Main category: cs.HC

TL;DR: AI驱动的3D模拟游戏用于医疗保健领域的根本原因分析（RCA）培训，提供互动沉浸式学习体验。


<details>
  <summary>Details</summary>
Motivation: 现有RCA培训资源需求高，导致培训不足和实施不一致，需一种成本低、可扩展的替代方案。

Method: 开发AI支持的3D模拟游戏，模拟ICU死亡事件的RCA调查，通过虚拟角色互动和LLM反馈提升技能。

Result: 系统结合LLM、情感语音和AI动画，提供自然互动和持续反馈，支持学习者改进。

Conclusion: 计划通过实证研究评估系统效果，验证其作为传统培训替代方案的潜力。
...
### [219] [Toward Supporting Narrative-Driven Data Exploration: Barriers and Design Opportunities](https://arxiv.org/abs/2508.04920)
*Oliver Huang,Carolina Nobre*

Main category: cs.HC

TL;DR: 论文探讨了数据分析中叙事驱动探索的挑战，并提出了设计机会以改进支持。


<details>
  <summary>Details</summary>
Motivation: 随着分析师的问题深化和变化，静态仪表板和预定义指标已无法满足需求，探索过程中见解分散，难以维护上下文或明确结论来源。

Method: 通过一项48名参与者的形成性研究，识别了叙事驱动探索的关键障碍。

Result: 研究发现包括跨视图维护上下文困难、追踪推理路径困难以及外部化演变解释困难。
...
### [220] [Situated Epistemic Infrastructures: A Diagnostic Framework for Post-Coherence Knowledge](https://arxiv.org/abs/2508.04995)
*Matthew Kelly*

Main category: cs.HC

TL;DR: 论文提出Situated Epistemic Infrastructures (SEI)框架，用于分析在后连贯性条件下知识如何在人机混合系统中获得权威性，强调协调而非分类。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（如ChatGPT）暴露了当代知识基础设施的脆弱性，模拟连贯性但绕过传统引用、权威和验证模式。

Method: 结合基础设施研究、平台理论和认识论，SEI框架通过追踪可信度在制度、计算和时间安排中的中介作用，提出诊断工具。

Result: SEI框架为AI治理、知识生产和信息系统伦理设计提供了非表征主义的替代方案。

Conclusion: SEI框架强调协调和适应性，为后连贯性条件下的知识权威性提供了新视角。
...
### [221] [Human-AI Schema Discovery and Application for Creative Problem Solving](https://arxiv.org/abs/2508.05045)
*Sitong Wang*

Main category: cs.HC

TL;DR: 该研究开发了一个人机协作框架，用于发现和应用模式（schemas）以支持创造性问题解决。


<details>
  <summary>Details</summary>
Motivation: 人类在复杂或陌生领域中难以发现和应用模式，限制了创造性问题解决的能力。

Method: 设计系统支持用户通过示例抽象出模式，并将其应用于人机协作的工作流程中。

Result: 研究揭示了模式引导的交互如何使隐性知识更易获取和操作，推动了更透明和协作的人机系统。

Conclusion: 该框架为创造性问题解决提供了新方法，增强了人机协作的透明性和效率。
...
### [222] [Accessibility Beyond Accommodations: A Systematic Redesign of Introduction to Computer Science for Students with Visual Impairments](https://arxiv.org/abs/2508.05056)
...
<details>
  <summary>Details</summary>
Motivation: 尽管计算机科学教育发展迅速，但视觉障碍学生仍面临系统性障碍，现有解决方案分散且难以实施。

Method: 框架包含五个关键组件：可访问的学习资源、课堂学习工具包、结构化支持系统、在线工具库和心理社会支持。

Result: 该框架基于通用设计原则，并通过专家咨询验证，为未来实证评估奠定了基础。

Conclusion: 该框架不仅解决了技术和教学问题，还强调在标准大学环境中的实际实施，为更广泛的机构采用提供了模板。
...
### [223] [A Desktop-Centric Design Space for Direct Object Examination and Visualization in Mixed-Reality Environments](https://arxiv.org/abs/2508.05088)
*Sam Johnson-Lacoss,Santiago V. Lombeyda,S. George Djorgovski*

Main category: cs.HC

TL;DR: 论文探讨了混合现实（MR）技术在科学研究和临床中的应用，提出了一个设计空间以优化MR环境下的交互方式。


<details>
  <summary>Details</summary>
Motivation: 随着MR技术的普及，研究科学家和临床医生需要从2D界面转向3D沉浸式环境，以更好地理解和分析3D现象及患者病理。

Method: 提出了一个设计空间，将不同的交互区域和模式分解，以支持在桌面物理空间内进行精确的对象中心数据分析。

Result: MR技术能够提供沉浸式3D表示，增强对物理结构、空间关系和2D数据的3D上下文的理解。

Conclusion: MR技术为科学研究和临床工作提供了更高效的工具，设计空间的提出有助于优化MR应用的用户体验。
...
### [224] [SparseEMG: Computational Design of Sparse EMG Layouts for Sensing Gestures](https://arxiv.org/abs/2508.05098)
*Anand Kumar,Antony Albert Raj Irudayaraj,Ishita Chandra,Adwait Sharma,Aditya Shekhar Nittala*

Main category: cs.HC

TL;DR: 该论文提出了一种数据驱动的方法，分析电极选择和分类器对EMG手势识别准确性和稀疏性的影响，并开发了SparseEMG工具，用于生成稀疏电极布局。


<details>
  <summary>Details</summary>
Motivation: 现有工具大多忽略电极选择对分类准确性的影响，本文旨在填补这一空白。

Method: 通过系统评估28种组合（4种选择方案，7种分类器）在六个数据集上的表现，确定了减少电极数量而不影响准确性的方法。

Result: 结果表明，使用Permutation Importance和Random Forest可将电极数量减少53.5%。SparseEMG工具验证了布局的跨用户可转移性。

Conclusion: SparseEMG工具能够高效生成稀疏电极布局，并在多种硬件设置中验证了其有效性。
...
### [225] [Metacognition and self-regulated learning in manipulative robotic problem-solving task](https://arxiv.org/abs/2508.05112)
*Margarida Romero,George Kalmpourtzis*

Main category: cs.HC

TL;DR: 本章探讨了元认知在创造性问题解决（CPS）中的作用，重点分析了元推理如何监控学习者的推理和CPS活动，并通过教育机器人案例研究展示了元认知调节探索与利用知识的过程。


<details>
  <summary>Details</summary>
Motivation: 研究元认知在CPS中的调节作用，特别是在解决未定义问题时如何通过探索和利用知识推动问题解决。

Method: 采用案例研究方法，分析学习者在解决机器人积木问题时的元认知调节过程。

Result: 研究发现，元认知通过调节探索和利用知识的过程，帮助学习者逐步构建系统级行为并找到解决方案。

Conclusion: 元认知在CPS中起到关键作用，特别是在未定义问题的解决中，通过调节探索与利用知识推动学习者的认知发展。
...
### [226] [AI Conversational Tutors in Foreign Language Learning: A Mixed-Methods Evaluation Study](https://arxiv.org/abs/2508.05156)
*Nikolaos Avouris*

Main category: cs.HC

TL;DR: 本文探讨了AI导师在外语学习中的应用，通过混合方法实证研究评估了多种先进AI导师的用户体验和对话功能质量，为未来工具设计提供参考。


<details>
  <summary>Details</summary>
Motivation: 近年来，自然语言理解和实时处理技术的进步推动了AI导师在外语学习中的发展，本研究旨在评估这些工具的实际效果和质量。

Method: 采用混合方法实证研究，包括用户体验评估和基于聊天记录的对话功能质量分析。

Result: 研究结果为评估AI导师系统质量提供了标准，并指出了数据隐私和学习者信息安全的重要性。

Conclusion: 本研究为未来AI导师工具的设计提供了实用建议，强调了对话功能质量和数据安全的双重关注。
...
### [227] [CWEFS: Brain volume conduction effects inspired channel-wise EEG feature selection for multi-dimensional emotion recognition](https://arxiv.org/abs/2508.05228)
...
<details>
  <summary>Details</summary>
Motivation: 高维多通道EEG特征中存在大量冗余信息，影响情绪表征的提取和实时性能，现有方法忽视了潜在EEG特征结构对情绪标签相关性的影响。

Method: CWEFS结合共享潜在结构模型和自适应通道权重学习，构建共识潜在空间并保留局部几何结构。

Result: 在三个流行的EEG数据集上验证，CWEFS选择的特征子集在六项评估指标中表现最优。

Conclusion: CWEFS方法有效解决了EEG特征选择中的问题，显著提升了多维情绪识别的性能。
...
### [228] [ADSEL: Adaptive dual self-expression learning for EEG feature selection via incomplete multi-dimensional emotional tagging](https://arxiv.org/abs/2508.05229)
*Tianze Yu,Junming Zhang,Wenjia Dong,Xueyuan Xu,Li Zhuo*

Main category: cs.HC

TL;DR: 提出了一种基于EEG的多维情绪识别特征选择算法，通过自适应双重自表达学习（ADSEL）解决标签不完整问题，提升模型泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有EEG特征选择方法假设标签完整，但实际中标签常不完整，影响模型泛化。现有方法忽略样本间及维度间的相关性。

Method: 结合自适应双重自表达学习（ADSEL）与最小二乘回归，通过双向信息共享实现标签重建和特征选择。

Result: ADSEL提高了标签恢复精度，并有效识别最优EEG特征子集。

Conclusion: 该方法在多维情绪识别中有效解决了标签不完整问题，提升了特征选择性能。
...
### [229] [FDC-Net: Rethinking the association between EEG artifact removal and multi-dimensional affective computing](https://arxiv.org/abs/2508.05231)
*Wenjia Dong,Xueyuan Xu,Tianze Yu,Junming Zhang,Li Zhuo*

Main category: cs.HC

TL;DR: 提出了一种名为FDC-Net的新框架，通过深度耦合去噪和情感识别任务，实现了端到端的噪声鲁棒情感识别。


<details>
  <summary>Details</summary>
Motivation: 解决当前EEG情感识别中因独立处理去噪和情感识别任务导致的误差累积和协同效应缺失问题。

Method: 采用双向梯度传播和联合优化策略，结合门控注意力机制与频率自适应Transformer。

Result: 在DEAP和DREAMER数据集上，FDC-Net在去噪和情感识别任务中均表现优异。

Conclusion: FDC-Net通过动态协作机制显著提升了噪声鲁棒性和情感识别性能。
...
### [230] [Driver Assistant: Persuading Drivers to Adjust Secondary Tasks Using Large Language Models](https://arxiv.org/abs/2508.05238)
...
<details>
  <summary>Details</summary>
Motivation: Level 3自动驾驶系统允许驾驶员进行次要任务，但在紧急情况下需要快速反应，增加了认知负担。

Method: 利用LLM根据路况触发，通过视觉和听觉途径提供说服性建议。
...
### [231] [A Methodological Framework and Questionnaire for Investigating Perceived Algorithmic Fairness](https://arxiv.org/abs/2508.05281)
*Ahmed Abdal Shafi Rasel,Ahmed Mustafa Amlan,Tasmim Shajahan Mim,Tanvir Hasan*

Main category: cs.HC

TL;DR: 研究通过混合方法探讨孟加拉国用户对算法决策公平性的看法，强调文化和社会因素对公平性理解的影响。


<details>
  <summary>Details</summary>
Motivation: 探索非西方文化背景下用户对算法公平性的感知，以丰富全球关于伦理AI的讨论。

Method: 结合定量调查和定性访谈，分析文化、社会和情境因素对公平性、透明度和问责制的影响。

Result: 发现用户对人机协作、解释机制和可争议性的复杂态度，强调文化敏感设计的重要性。

Conclusion: 研究为非西方视角的算法公平性讨论提供了新见解，推动全球伦理AI的发展。
...
### [232] [Critical Design Strategy: a Method for Heuristically Evaluating Visualisation Designs](https://arxiv.org/abs/2508.05325)
*Jonathan C. Roberts,Hanan Alnjar,Aron E. Owen,Panagiotis D. Ritsos*

Main category: cs.HC

TL;DR: 论文提出了一种名为Critical Design Strategy (CDS)的结构化方法，通过反思和批判性思维帮助设计师评估和改进可视化设计。该方法包含三个阶段，涵盖六个视角，并在教学实践中不断优化。


<details>
  <summary>Details</summary>
Motivation: 可视化设计领域缺乏系统化的批判性思考工具，尤其是对新手设计师。CDS旨在填补这一空白，帮助设计师识别改进点并做出更明智的设计决策。

Method: CDS分为三个阶段：1) 初步印象（标题和形容词选择）；2) 深入批判（30个启发式问题，涵盖六个视角）；3) 综合反思和下一步行动。该方法在教学模块中应用并持续优化。

Result: CDS在本科和研究生课程中得到应用，并通过多年实践不断改进，包括启发式问题的优化和支持材料的扩展。

Conclusion: CDS为可视化设计提供了一种实用的批判性思考框架，适用于教学和实际设计工作，未来可进一步推广和应用。
...
### [233] [Implementation and Application of Multi-Format 3D Data Integration in a Cross-Device Commercial Metaverse Platform](https://arxiv.org/abs/2508.05332)
*Masanori Ibara,Yuichi Hiroi,Takushi Kamegai,Takefumi Hiraki*

Main category: cs.HC

TL;DR: 本文探讨了如何通过工业元宇宙平台Cluster，利用3D数据（如BIM和CAD）实现跨领域协作决策，打破传统专家依赖的壁垒。


<details>
  <summary>Details</summary>
Motivation: 传统3D设计数据（如BIM和CAD）仅限专家使用，阻碍了普通用户参与决策。本文旨在通过工业元宇宙平台，降低使用门槛，促进民主化决策。

Method: 分析工业与建筑领域主要数据格式的特点与限制，整合元宇宙工作流，并通过多领域3D数据应用案例展示协作决策支持。

Result: 通过Cluster平台的多设备访问和多用户同时参与功能，实现了传统专家依赖系统难以达到的民主化工业元宇宙环境。

Conclusion: 工业元宇宙与数字孪生技术的结合，为跨领域协作决策提供了实用解决方案，推动了更广泛的用户参与。
...
### [234] [Towards Human-Centric Evaluation of Interaction-Aware Automated Vehicle Controllers: A Framework and Case Study](https://arxiv.org/abs/2508.05497)
*Federico Scarì,Olger Siebinga,Arkady Zgonnikov*

Main category: cs.HC

TL;DR: 论文提出了一种结构化评估框架，用于评估自动驾驶车辆（AV）与人类驾驶车辆（HDV）的交互，弥补了现有研究中忽视人类中心维度的不足。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注AV控制算法的技术性能（如避撞或车道保持效率），而忽略了与HDV交互的人类中心维度。

Method: 提出一个包含四个关键领域（交互效果、交互感知、交互努力和交互能力）的评估框架，并结合人机交互文献中的指标。

Result: 案例研究表明，该框架能揭示AV与HDV交互中驾驶员体验的关键差异，强调了全面评估的必要性。

Conclusion: 该框架为研究者、开发者和政策制定者提供了一种系统方法，以评估AV行为的技术性能之外的人类中心维度，促进AV的开发和推广。
...
### [235] [Discrepancy-Aware Contrastive Adaptation in Medical Time Series Analysis](https://arxiv.org/abs/2508.05572)
*Yifan Wang,Hongfeng Ai,Ruiqi Li,Maowei Jiang,Ruiyuan Kang,Jiahua Dong,Cheng Jiang,Chenzhong Li*

Main category: cs.HC

TL;DR: 论文提出了一种结合AE-GAN和LMCF的方法，解决医学时间序列诊断中的标注成本高和对比学习泛化性差的问题，并在多个疾病诊断任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 医学数据标注成本高，单中心数据集易导致过拟合；现有对比学习方法复杂且缺乏适应性。

Method: 使用AE-GAN提取先验知识，提出LMCF框架，结合多头注意力和自适应对比学习策略。

Result: 在三个目标数据集上优于七种基线方法，显著提升心肌梗死、阿尔茨海默病和帕金森病的诊断效果。

Conclusion: 该方法有效解决了医学时间序列诊断中的关键问题，具有重要的医疗应用价值。
...
### [236] [On the causality between affective impact and coordinated human-robot reactions](https://arxiv.org/abs/2508.04834)
*Morten Roed Frederiksen,Kasper Støy*

Main category: cs.RO

TL;DR: 研究机器人主动与人类共享事件反应是否影响人类对其情感影响的感知，发现共享反应和特定延迟时间能显著改变感知。
...
Method: 设计两个实验：一个隔离机器人反应元素，另一个测试不同延迟时间对物理互动的影响。

Result: 共享事件反应显著改变人类感知（p<0.05），物理互动中接近人类的反应时间（200ms）效果最佳。
...
### [237] [INTENTION: Inferring Tendencies of Humanoid Robot Motion Through Interactive Intuition and Grounded VLM](https://arxiv.org/abs/2508.04931)
...
<details>
  <summary>Details</summary>
Motivation: 传统机器人控制方法依赖精确物理模型和预定义动作序列，在真实世界中因建模不准确和泛化能力差而受限。人类则通过直觉交互表现出强大适应性，论文旨在让机器人模仿这种能力。

Method: 提出INTENTION框架，包括Memory Graph记录任务交互场景和Intuitive Perceptor提取视觉场景中的物理关系和功能属性。

Result: 机器人能够在无需重复指令的情况下，在新场景中推断出合适的交互行为。

Conclusion: INTENTION框架为机器人提供了类似人类的直觉理解和决策能力，适用于多样化场景。
...
### [238] [Optimal Planning for Multi-Robot Simultaneous Area and Line Coverage Using Hierarchical Cyclic Merging Regulation](https://arxiv.org/abs/2508.04981)
...
Method: 采用HCMR算法，结合莫尔斯理论分析图遍历中的流形附着过程，并通过循环合并搜索和边序列反向传播生成最优路径。
...
### [239] [Hierarchical Deep Deterministic Policy Gradient for Autonomous Maze Navigation of Mobile Robots](https://arxiv.org/abs/2508.04994)
*Wenjie Hu,Ye Zhou,Hann Woei Ho*

Main category: cs.RO

TL;DR: 本文提出了一种高效的层次化DDPG（HDDPG）算法，通过高低级策略结合和改进的奖励函数，显著提升了迷宫导航的成功率和平均奖励。


<details>
  <summary>Details</summary>
Motivation: 解决DDPG在迷宫导航中因稀疏奖励、低效探索和长时规划问题导致的低成功率和低奖励问题。

Method: 采用高低级策略：高级策略生成子目标，低级策略执行动作；结合离策略校正、自适应参数噪声和改进的奖励函数。

Result: HDDPG在迷宫导航任务中比基线算法成功率提升至少56.59%，平均奖励提高至少519.03。
...
### [240] [Mixed-Initiative Dialog for Human-Robot Collaborative Manipulation](https://arxiv.org/abs/2508.05535)
*Albert Yu,Chengshu Li,Luca Macesanu,Arnav Balaji,Ruchira Ray,Raymond Mooney,Roberto Martín-Martín*

Main category: cs.RO

TL;DR: MICoBot系统通过混合主动对话范式优化人机协作，结合多层级决策（元规划、任务分配、执行）提升任务成功率和用户体验。


<details>
  <summary>Details</summary>
Motivation: 解决人机协作中因人类行为多样性和动态性导致的沟通与任务分配挑战。

Method: 采用三层决策框架：元规划制定策略、任务分配优化步骤、执行层决定具体行动或对话。

Result: 在仿真和真实实验中显著优于纯LLM基线和其他任务分配模型。
...
### [241] [MAG-Nav: Language-Driven Object Navigation Leveraging Memory-Reserved Active Grounding](https://arxiv.org/abs/2508.05021)
...
<details>
  <summary>Details</summary>
Motivation: 解决智能机器人在未知环境中仅依靠自然语言描述进行视觉导航的挑战，提升视觉语言模型的泛化能力。

Method: 结合视角主动调整和历史记忆回溯机制，优化视觉感知并利用记忆解决歧义，无需标注数据或微调模型。

Result: 在HM3D数据集上表现优于现有方法，并在四足机器人上验证了实际导航效果。

Conclusion: 该框架为零样本导航提供了高效解决方案，适用于复杂开放环境。
...
### [242] [Benchmarking Shortcutting Techniques for Multi-Robot-Arm Motion Planning](https://arxiv.org/abs/2508.05027)
*Philip Huang,Yorai Shaoul,Jiaoyang Li*

Main category: cs.RO

TL;DR: 本文研究了多机械臂运动规划中的捷径方法，比较了现有方法并提出两种组合策略以实现最佳性能与运行时间的平衡。


<details>
  <summary>Details</summary>
Motivation: 多机械臂系统的高维度和潜在的碰撞风险使得生成高质量运动规划具有挑战性，传统方法常产生次优运动，现有捷径方法缺乏详细描述和性能分析。

Method: 通过定量比较现有捷径方法，分析其优缺点，并提出两种组合策略。

Result: 在多种模拟场景中验证了不同捷径方法的性能，并展示了组合策略的最佳性能与运行时间平衡。

Conclusion: 提出的组合策略在多机械臂运动规划中实现了高效且平滑的执行，为实际应用提供了实用解决方案。
...
### [243] [A Vision-Based Collision Sensing Method for Stable Circular Object Grasping with A Soft Gripper System](https://arxiv.org/abs/2508.05040)
...
<details>
  <summary>Details</summary>
Motivation: 外部碰撞对机器人执行器抓取圆形物体构成风险，需确保抓取过程的稳定性和安全性。

Method: 采用眼在手相机监测手指和抓取物体运动，开发碰撞丰富的抓取策略。

Result: 实验验证系统能即时响应碰撞，精确检测碰撞方向和大小。

Conclusion: 系统有效提升软夹持器在动态抓取中的稳定性和安全性。
...
### [244] [Examining the legibility of humanoid robot arm movements in a pointing task](https://arxiv.org/abs/2508.05104)
*Andrej Lúčny,Matilde Antonj,Carlo Mazzola,Hana Hornáčková,Ana Farić,Kristína Malinovská,Michal Vavrecka,Igor Farkaš*

Main category: cs.RO

TL;DR: 研究探讨人形机器人手臂动作的可读性，通过实验验证多模态优越性和视觉主导假设。
...
Method: 使用NICO人形机器人进行实验，观察不同线索（注视、指向、视线一致性）下人类对截断动作的预测。

Result: 实验支持多模态优越性和视觉主导假设。

Conclusion: 机器人动作的可读性可通过多模态线索提升，视觉线索尤为重要。
...
### [245] [From Canada to Japan: How 10,000 km Affect User Perception in Robot Teleoperation](https://arxiv.org/abs/2508.05143)
...
Method: 设计了包含问卷和专用软件架构（ROS与Unity）的协议，评估非专家用户对远程RTo的感知变化。
...
### [246] [Chemist Eye: A Visual Language Model-Powered System for Safety Monitoring and Robot Decision-Making in Self-Driving Laboratories](https://arxiv.org/abs/2508.05148)
...
<details>
  <summary>Details</summary>
Motivation: 自驱动实验室中机器人使用易燃锂电池，增加了火灾等安全风险，需要高效的安全监控系统。
...
### [247] [FCBV-Net: Category-Level Robotic Garment Smoothing via Feature-Conditioned Bimanual Value Prediction](https://arxiv.org/abs/2508.05153)
*Mohammed Daba,Jing Qiu*

Main category: cs.RO

TL;DR: FCBV-Net通过预训练的几何特征增强类别级策略泛化能力，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决机器人服装操作中类别级泛化的挑战，如高维度和类别内变化。

Method: 提出FCBV-Net，基于3D点云，利用预训练的几何特征预测双动作价值。
...
### [248] [A General Control Method for Human-Robot Integration](https://arxiv.org/abs/2412.14762)
*Maddalena Feder,Giorgio Grioli,Manuel G. Catalano,Antonio Bicchi*

Main category: cs.RO

TL;DR: 提出了一种新的广义控制方法，用于多自由度设备，帮助运动能力有限的人完成日常活动。


<details>
  <summary>Details</summary>
Motivation: 解决如何将用户低维空间的动作有效映射到复杂机器人辅助设备（如假肢、额外肢体或远程机器人化身）的挑战，目标是实现人与机器人无缝集成，减少用户负担。

Method: 提出一个框架，将用户补偿动作转化为机器人指令，同时减少补偿动作，适用于从假肢到全身机器人化身的多自由度系统。

Result: 通过模拟场景和真实世界试验验证了该控制策略的有效性，包括虚拟双胞胎和物理人形化身。

Conclusion: 该框架为多自由度辅助系统提供了一种通用控制方法，显著减少了用户的不适和努力。
...
### [249] [Learning to See and Act: Task-Aware View Planning for Robotic Manipulation](https://arxiv.org/abs/2508.05186)
...
Method: TAVP结合主动视图规划和任务特定表示学习，采用高效探索策略（通过伪环境加速）和混合专家（MoE）视觉编码器，以解耦不同任务的特征。

Result: 在RLBench任务上的实验表明，TAVP显著优于现有固定视角方法，生成了更完整和区分性的视觉表示。

Conclusion: TAVP通过任务感知的视觉表示学习，显著提升了动作预测能力，为多任务机器人操作提供了更鲁棒和通用的解决方案。
...
### [250] [Dancing with a Robot: An Experimental Study of Child-Robot Interaction in a Performative Art Setting](https://arxiv.org/abs/2508.05208)
...
<details>
  <summary>Details</summary>
Motivation: 研究儿童与机器人艺术表演者的互动，以优化人机交互系统。

Method: 通过观察分析18名儿童与机器人NED的互动。

Result: 儿童对机器人表现出好奇心，但互动中存在三大挑战。

Conclusion: 需优化人机交互设计，考虑观众能力和期望，以提升儿童体验。
...
### [251] [Towards Embodied Agentic AI: Review and Classification of LLM- and VLM-Driven Robot Autonomy and Interaction](https://arxiv.org/abs/2508.05294)
*Sahar Salimpour,Lei Fu,Farhad Keramat,Leonardo Militano,Giovanni Toffetti,Harry Edelman,Jorge Peña Queralta*

Main category: cs.RO

TL;DR: 本文综述了基础模型（如LLMs和VLMs）在机器人自主性和人机交互中的应用，探讨了VLAs和BLMs如何提升机器人能力，并提出了一个分类法来分析模型集成方法。


<details>
  <summary>Details</summary>
Motivation: 研究基础模型在机器人领域的应用潜力，推动机器人自主性和人机交互的发展。

Method: 通过综述现有研究和社区项目，提出分类法并比较不同解决方案中代理的作用。

Result: 展示了基础模型在机器人任务中的多样化应用，如自然语言指令理解、API调用和任务规划。

Conclusion: 基础模型为机器人领域带来了新的可能性，未来需进一步探索其在实际应用中的潜力。
...
### [252] [GhostShell: Streaming LLM Function Calls for Concurrent Embodied Programming](https://arxiv.org/abs/2508.05298)
...
TL;DR: GhostShell利用LLM实现实时行为编程，通过流式XML解析器和动态接口映射器，支持多通道调度，显著提升响应速度和任务正确率。
...
Result: 在34个真实任务中，GhostShell的Behavioral Correctness Metric达到0.85，响应速度提升66倍。

Conclusion: GhostShell在长时多模态任务中表现出色，具有强鲁棒性和泛化能力。
...
### [253] [A Multi-view Landmark Representation Approach with Application to GNSS-Visual-Inertial Odometry](https://arxiv.org/abs/2508.05368)
*Tong Hua,Jiale Han,Wei Ouyang*

Main category: cs.RO

TL;DR: 提出了一种多视角仅位姿估计方法，用于提升GNSS-视觉-惯性里程计（GVIO）的效率，通过直接关联地标表示与多相机位姿和观测，减少了计算负担。
...
Method: 推导了一种视觉测量模型，直接关联地标表示与多相机位姿和观测，形成紧密耦合的仅位姿测量，并应用于基于滤波的GVIO。

Result: 仿真和实际实验表明，该方法在效率和精度上均优于传统方法。

Conclusion: 提出的方法显著提升了GVIO的效率，同时保持了高精度，适用于多传感器融合场景。
...
### [254] [Information-Theoretic Graph Fusion with Vision-Language-Action Model for Policy Reasoning and Dual Robotic Control](https://arxiv.org/abs/2508.05342)
*Shunlei Li,Longsen Gao,Jin Wang,Chang Che,Xi Xiao,Jiuwen Cao,Yingbai Hu,Hamid Reza Karimi*

Main category: cs.RO

TL;DR: GF-VLA框架通过信息论和语言条件转换器，使双臂机器人直接从人类演示中学习任务级推理和执行，显著提升了泛化能力和任务成功率。
...
Method: 提取任务相关的手和物体信息，编码为时序场景图，结合语言条件转换器生成行为树和运动指令，并引入跨手选择策略优化执行效率。

Result: 在双臂块组装任务中，图准确率超过95%，任务成功率90%，表现出强泛化能力。

Conclusion: GF-VLA框架在任务级推理和执行上表现出色，适用于复杂场景和语义变化。
...
### [255] [Affecta-Context: The Context-Guided Behavior Adaptation Framework](https://arxiv.org/abs/2508.05359)
*Morten Roed Frederiksen,Kasper Støy*

Main category: cs.RO

TL;DR: Affecta-context框架通过物理上下文信息指导社交机器人行为优化，包括上下文表示和行为优先级学习，并在实验中验证了其泛化能力。
...
Result: 在72次交互实验中，机器人成功学习并泛化到未访问的物理上下文。

Conclusion: Affecta-context框架能有效指导社交机器人行为适应，并具有泛化能力。
...
### [256] [Towards Generalizable Safety in Crowd Navigation via Conformal Uncertainty Handling](https://arxiv.org/abs/2508.05634)
*Jianpeng Yao,Xiaopan Zhang,Yu Xia,Zejin Wang,Amit K. Roy-Chowdhury,Jiachen Li*

Main category: cs.RO

TL;DR: 论文提出了一种通过考虑行人预测不确定性来增强移动机器人在人群中导航鲁棒性的方法，使用自适应共形推理和约束强化学习。
...
Method: 通过自适应共形推理生成预测不确定性估计，并利用这些估计通过约束强化学习指导机器人行为。

Result: 在分布内场景中，方法实现了96.93%的成功率，碰撞和侵入人类轨迹的次数显著减少；在分布外场景中表现出更强的鲁棒性。

Conclusion: 该方法在真实机器人上验证了其安全性和鲁棒性，适用于稀疏和密集人群的交互。
...
### [257] [Robots can defuse high-intensity conflict situations](https://arxiv.org/abs/2508.05373)
*Morten Roed Frederiksen,Kasper Støy*

Main category: cs.RO

TL;DR: 研究探讨了机器人在高强度对抗中如何通过五种情感表达方式缓解冲突，发现所有方式均有效，但运动方式表现不同。
...
Result: 所有表达方式均能成功缓解冲突，运动方式表现显著不同，参与者对机器人受影响程度的感知相似。

Conclusion: 缓解高强度对抗更需机器人具备社会情境意识，而非特定表达方式。
...
### [258] [Real-Time Iteration Scheme for Diffusion Policy](https://arxiv.org/abs/2508.05396)
*Yufei Duan,Hang Yin,Danica Kragic*

Main category: cs.RO

TL;DR: 本文提出了一种基于实时迭代方案（RTI）的新方法，显著降低了扩散策略的推理时间，无需额外训练或策略重新设计。
...
Method: 采用实时迭代方案（RTI），利用前一时间步的解作为后续迭代的初始猜测，并提出基于缩放的方法处理离散动作。

Result: 实验表明，该方法大幅降低推理时间，同时保持与全步去噪扩散策略相当的总体性能。

Conclusion: 该方法为预训练扩散模型的集成提供了高效解决方案，尤其适用于资源密集型大模型。
...
### [259] [DistillDrive: End-to-End Multi-Mode Autonomous Driving Distillation by Isomorphic Hetero-Source Planning Model](https://arxiv.org/abs/2508.05402)
...
### [260] [Computational Design and Fabrication of Modular Robots with Untethered Control](https://arxiv.org/abs/2508.05410)
*Manas Bhargava,Takefumi Hiraki,Malina Strugaru,Michal Piovarci,Chiara Daraio,Daisuke Iwai,Bernd Bickel*

Main category: cs.RO

TL;DR: 提出了一种基于分布式驱动的机器人设计框架，结合3D打印骨骼和液晶弹性体肌肉，实现模块化组装和远程控制，并通过计算工具优化设计和运动。
...
Method: 使用3D打印骨骼和液晶弹性体（LCE）肌肉作为轻量级驱动器，开发红外响应的LCE杆实现分布式控制，并设计计算工具优化骨骼图和运动步态。

Result: 构建了多个机器人，展示复杂形态变化、多样化控制方案和环境适应性。

Conclusion: 通过模块化材料、分布式控制和计算设计的结合，推动了机器人向生物能力的迈进。
...
### [261] [Do Robots Really Need Anthropomorphic Hands?](https://arxiv.org/abs/2508.05415)
*Alexander Fabisch,Wadhah Zai El Amri,Chandandeep Singh,Nicolás Navarro-Guerrero*

Main category: cs.RO

TL;DR: 探讨人类手部是否为机器人手的理想模型，分析其复杂性、功能及替代设计的可能性。


<details>
  <summary>Details</summary>
Motivation: 研究人类手部的高自由度协调与高维传感器处理能力，是否应为机器人手的终极目标。

Method: 综述人类手部特性、商业机器人手对比，系统评估手部机制与技能。

Result: 发现手腕灵活性和手指外展/内收对功能更重要，三指设计是简单与灵巧的平衡，非人形设计可能更优。

Conclusion: 人类手部并非机器人手的唯一理想模型，非人形设计可能提供更高灵巧性。
...
### [262] [CleanUpBench: Embodied Sweeping and Grasping Benchmark](https://arxiv.org/abs/2508.05543)
...
<details>
  <summary>Details</summary>
Motivation: 现有的基准测试多针对复杂人形代理或大规模模拟，而移动清洁机器人作为现实可行的平台缺乏系统性评估工具。

Method: 基于NVIDIA Isaac Sim构建CleanUpBench，模拟配备清扫机制和六自由度机械臂的移动服务机器人，评估任务完成度、空间效率等。

Result: 提供了手动设计和程序生成的环境，以及基于启发式策略和地图规划的基线代理，支持比较研究。

Conclusion: CleanUpBench为日常场景中的具身智能提供了可扩展的测试平台，连接了低层技能评估与全场景测试。
...
### [263] [Robust adaptive fuzzy sliding mode control for trajectory tracking for of cylindrical manipulator](https://arxiv.org/abs/2508.05584)
*Van Cuong Pham,Minh Hai Tran,Phuc Anh Nguyen,Ngoc Son Vu,Nga Nguyen Thi*

Main category: cs.RO

TL;DR: 提出了一种鲁棒的自适应模糊滑模控制（AFSMC）方法，用于提升圆柱形机器人操纵器的轨迹跟踪性能。


<details>
  <summary>Details</summary>
Motivation: 圆柱形机器人操纵器在CNC和3D打印等应用中广泛使用，但传统方法在轨迹跟踪精度和抗干扰能力上存在不足。

Method: 结合模糊逻辑和滑模控制（SMC），模糊逻辑用于近似系统的不确定性动态，SMC确保鲁棒性。

Result: MATLAB/Simulink仿真显示，AFSMC显著提升了轨迹跟踪精度、稳定性和抗干扰能力。
...
### [264] [Genie Envisioner: A Unified World Foundation Platform for Robotic Manipulation](https://arxiv.org/abs/2508.05635)
...
Method: GE-Base 是一个大规模、指令条件的视频扩散模型，GE-Act 通过轻量级解码器将潜在表示映射为可执行动作，GE-Sim 作为神经仿真器生成高保真模拟。

Result: 平台支持跨多样化的机器人实现精确且通用的策略推断，并提供标准化评估基准 EWMBench。
...
### [265] [Linear Program-Based Stability Conditions for Nonlinear Autonomous Systems](https://arxiv.org/abs/2508.04871)
*Sadredin Hokmi,Mohammad Khajenejad*

Main category: eess.SY

TL;DR: 提出了一种基于线性规划（LP）的新方法，用于评估连续和离散非线性自治系统的渐近稳定性，替代传统的半定规划（SDP），显著降低了计算负担。


<details>
  <summary>Details</summary>
Motivation: 传统SDP方法在高维系统中计算负担重，需要更高效的方法来评估稳定性。

Method: 利用间接Lyapunov方法和Jacobian矩阵线性化系统动力学，通过矩阵变换和系统结构特性开发稳定性条件。

Result: 新方法在计算时间和内存使用上显著优于SDP方法，尤其适用于高维系统。

Conclusion: 提出的LP方法在计算效率和可扩展性上优于传统SDP方法，适用于高维非线性系统。
...
### [266] [Sequence Aware SAC Control for Engine Fuel Consumption Optimization in Electrified Powertrain](https://arxiv.org/abs/2508.04874)
*Wafeeq Jaleel,Md Ragib Rownak,Athar Hanif,Sidra Ghayour Bhatti,Qadeer Ahmed*

Main category: eess.SY

TL;DR: 提出了一种基于强化学习（RL）的框架，结合Soft Actor-Critic（SAC）算法、GRU和决策变换器（DT），优化混合动力重型卡车的能量管理，显著降低油耗并提升适应性。


<details>
  <summary>Details</summary>
Motivation: 混合动力重型卡车需要高效的能量管理以减少油耗并保持电池电量，传统方法难以满足复杂工况的需求。

Method: 将控制任务建模为序列决策问题，改进SAC算法，在Actor和Critic网络中引入GRU和DT以捕捉时间依赖性和提升规划能力。

Result: 实验表明，基于DT和GRU的SAC代理在HFET循环中油耗接近动态规划（DP）的1.8%，在未见过的驾驶循环中表现优于传统FFN代理。

Conclusion: 提出的序列感知代理在真实场景中表现出更强的适应性和鲁棒性，为混合动力卡车的能量管理提供了有效解决方案。
...
### [267] [Uncovering the Influence Flow Model of Transistor Amplifiers, Its Reconstruction and Application](https://arxiv.org/abs/2508.04977)
*Mohammed Tuhin Rana,Mishfad Shaikh Veedu,Murti V. Salapaka*

Main category: eess.SY

TL;DR: 论文提出了一种基于线性动态影响模型（LDIM）的多级晶体管放大器建模方法，利用数据驱动的网络重构技术分析级间交互，并通过电压时间序列测量重建网络结构。


<details>
  <summary>Details</summary>
Motivation: 多级晶体管放大器的动态耦合特性需要高效建模和分析方法，以优化设计和故障诊断。

Method: 采用电路分析技术将放大器建模为LDIM，结合图形建模和维纳滤波，仅通过电压时间序列测量重建网络结构。

Result: 通过仿真和实验验证了网络重构方法的有效性，并展示了其在故障诊断中的应用。

Conclusion: 该方法为放大器电路的设计、分析和调试提供了高效工具，尤其适用于故障诊断。
...
### [268] [Probabilistic Alternating Simulations for Policy Synthesis in Uncertain Stochastic Dynamical Systems](https://arxiv.org/abs/2508.05062)
*Thom Badings,Alessandro Abate*

Main category: eess.SY

TL;DR: 扩展概率模拟关系以处理随机和非确定性扰动，适用于政策合成。


<details>
  <summary>Details</summary>
Motivation: 现有概率模拟关系无法处理同时具有随机和非确定性扰动的系统。

Method: 提出一种受交替模拟启发的扩展概率模拟关系，结合概率性和鲁棒性推理。

Result: 实验验证了该方法在4D Dubins车辆政策合成中的适用性。

Conclusion: 扩展的关系为具有混合扰动的系统提供了有效的政策合成框架。
...
### [269] [Preparing for the worst: Long-term and short-term weather extremes in resource adequacy assessment](https://arxiv.org/abs/2508.05163)
*Aleksander Grochowicz,Hannah C. Bloomfield,Marta Victoria*

Main category: eess.SY

TL;DR: 本文通过影子价格方法识别电力系统中的高压力事件（系统定义事件），分析其对系统的影响，并分类挑战，提出短期和长期韧性指标与测试方法。


<details>
  <summary>Details</summary>
Motivation: 研究可再生能源整合到净零电力系统时供应安全的重要性，尤其是极端天气对供需的影响及其跨区域的传播。

Method: 基于影子价格的方法识别系统定义事件，分类挑战，并提出短期和长期韧性指标与测试方法。

Result: 发现需要足够的备用容量以应对天气变化，但其财务可行性不稳定，并提出了区分短期和长期韧性的方法。

Conclusion: 提出的方法和开源模型PyPSA-Eur可帮助构建更具韧性和充足性的能源系统。
...
### [270] [Overview of Controllability Definitions in Supervisory Control Theory](https://arxiv.org/abs/2508.05177)
*Jeroen J. A. Keiren,Michel A. Reniers*

Main category: eess.SY

TL;DR: 论文总结了监督控制理论中可控性的不同定义，并研究了它们在确定性和非确定性自动机中的关系。


<details>
  <summary>Details</summary>
Motivation: 由于文献中对同一概念（如可控性）的定义多样，导致理解困难，本文旨在厘清这些定义之间的关系。

Method: 列举文献中的可控性定义，分析其在确定性和非确定性自动机中的关系。

Result: 在非确定性自动机的一般背景下，Flordal和Malik的可控性定义与Kushi和Takai的不可控事件可接受性定义等价，且均隐含传统的语言可控性。在监督植物与植物的可控性背景下，Zhou等人的状态可控性也隐含语言可控性。

Conclusion: 本文澄清了不同可控性定义之间的关系，为监督控制理论提供了更清晰的理论基础。
...
### [271] [Passive nonlinear FIR filters for data-driven control](https://arxiv.org/abs/2508.05279)
*Zixing Wang,Fulvio Forni*

Main category: eess.SY

TL;DR: 提出一类新的被动非线性有限脉冲响应算子，通过提升空间中的有限脉冲响应滤波器构造，支持高效控制合成。


<details>
  <summary>Details</summary>
Motivation: 为物理系统（如机电系统）的控制提供高效且被动的解决方案。

Method: 在提升空间中构造有限脉冲响应滤波器，通过约束优化和最小二乘拟合实现控制合成。

Result: 通过频域采样建立高效的线性约束，确保被动性。

Conclusion: 该类算子特别适合物理系统的控制，因其被动性和高效性。
...
### [272] [A 20-Year Retrospective on Power and Thermal Modeling and Management](https://arxiv.org/abs/2508.05495)
*David Atienza,Kai Zhu,Darong Huang,Luis Costero*

Main category: eess.SY

TL;DR: 综述了现代处理器中超过二十年的功率和热建模与管理研究，比较了不同建模技术，并探讨了动态运行时管理策略及未来挑战。


<details>
  <summary>Details</summary>
Motivation: 随着处理器性能提升，功率密度和复杂热行为威胁能效和系统可靠性，需研究建模与管理方法。

Method: 比较分析功率估计技术（解析、回归、神经网络），回顾热建模方法（有限元、有限差分、数据驱动），分类动态运行时管理策略。

Result: 总结了多种建模和管理技术的优缺点，为未来研究提供了参考。

Conclusion: 提出新兴挑战和研究方向，强调动态管理和高效建模的重要性。
...
### [273] [Research on integrated intelligent energy management system based on big data analysis and machine learning](https://arxiv.org/abs/2508.05583)
*Jinzhou Xu,Yadan Zhang,Paola Tapia*

Main category: eess.SY

TL;DR: 论文探讨了大数据在综合智慧能源项目文件管理中的应用，提出了基于机器学习的优化方法，模型准确率达95%以上。


<details>
  <summary>Details</summary>
Motivation: 提高综合智慧能源项目文件管理的效率和管控能力。

Method: 开发了大数据分析框架，并利用三种机器学习方法优化文件管理效率。

Result: 惩罚线性回归模型在足够数据训练下准确率超过95%。

Conclusion: 大数据分析和机器学习可优化综合智慧能源项目文件管理流程，提升项目效率。
...
### [274] [Error Bounds for Radial Network Topology Learning from Quantized Measurements](https://arxiv.org/abs/2508.05620)
*Samuel Talkington,Aditya Rangarajan,Pedro A. de Alcântara,Line Roald,Daniel K. Molzahn,Daniel R. Fuhrmann*

Main category: eess.SY

TL;DR: 论文研究了径向网络拓扑学习问题的误差概率界限，考虑了传感器量化引入的非线性测量模型。


<details>
  <summary>Details</summary>
Motivation: 传统电力系统估计算法通常采用加性噪声模型，而本文扩展了模型，考虑了传感器通信网络的量化误差。

Method: 通过非线性测量模型分析量化误差，并推导拓扑学习误差的概率界限。

Result: 学习到的径向网络拓扑误差与量化区间宽度成正比，且节点数增加时误差呈次线性增长。

Conclusion: 在节点样本数对数增长条件下，量化误差对拓扑学习的影响可控。
...
### [275] [Toward Low-Latency End-to-End Voice Agents for Telecommunications Using Streaming ASR, Quantized LLMs, and Real-Time TTS](https://arxiv.org/abs/2508.04721)
*Vignesh Ethiraj,Ashwath David,Sidhanth Menon,Divya Vijay*

Main category: cs.SD

TL;DR: 论文介绍了一种低延迟的电信AI语音代理管道，用于实时交互式电信应用，结合了四种专用模型，实现了高性能的语音AI代理。


<details>
  <summary>Details</summary>
Motivation: 解决电信领域对低延迟、高响应性语音AI的需求，支持自动化客户服务、智能IVR和AI驱动的客户支持。

Method: 结合四种专用模型（TSLAM、T-VEC、TTE、T-Synth），通过流式ASR、对话智能、检索增强生成和实时TTS构建管道。

Result: 模型在延迟、领域相关性和实时性能上表现优异，RTF低于1.0，适用于企业级低延迟部署。
...
### [276] [Wearable Music2Emotion : Assessing Emotions Induced by AI-Generated Music through Portable EEG-fNIRS Fusion](https://arxiv.org/abs/2508.04723)
*Sha Zhao,Song Yi,Yangxuan Zhou,Jiadong Pan,Jiquan Wang,Jie Xia,Shijian Li,Shurong Dong,Gang Pan*

Main category: cs.SD

TL;DR: MEEtBrain是一个便携式多模态框架，用于情绪分析，结合AI生成音乐和无线EEG-fNIRS采集，解决了现有研究的三个主要限制。


<details>
  <summary>Details</summary>
Motivation: 音乐在情感计算中具有潜力，但现有研究存在音乐刺激受限、模态单一和便携性差的问题。

Method: 提出MEEtBrain框架，使用AI生成音乐刺激，并通过轻便头带设备同步采集EEG和fNIRS信号。

Result: 收集了14小时的多模态数据，验证了框架的有效性，并公开数据集以促进研究。

Conclusion: MEEtBrain为情感分析提供了更高效、便携的解决方案，并推动了多模态数据集的共享。
...
### [277] [Towards Hallucination-Free Music: A Reinforcement Learning Preference Optimization Framework for Reliable Song Generation](https://arxiv.org/abs/2508.05011)
...
<details>
  <summary>Details</summary>
Motivation: 现有的音频生成语言模型在歌词到歌曲生成中常出现内容幻觉，导致输出与输入歌词不一致，影响音乐连贯性。传统监督微调方法效果有限。

Method: 提出了一种强化学习框架，包括构建幻觉偏好数据集（基于音素错误率和规则过滤），并评估了三种偏好优化策略：DPO、PPO和GRPO。

Result: DPO显著降低音素错误率7.4%，PPO和GRPO分别降低4.9%和4.7%，同时保持音乐质量。
...
### [278] [SpectroStream: A Versatile Neural Codec for General Audio](https://arxiv.org/abs/2508.05207)
...
Result: 在48 kHz立体声音乐中实现高质量重建，比特率为4-16 kbps。

Conclusion: SpectroStream在更高采样率和多通道音频中表现优异，平衡了音质和相位一致性。
...
### [279] [Estimating Musical Surprisal from Audio in Autoregressive Diffusion Model Noise Spaces](https://arxiv.org/abs/2508.05306)
*Mathias Rose Bjare,Stefan Lattner,Gerhard Widmer*

Main category: cs.SD

TL;DR: 研究比较了基于扩散ODE的模型与GIVT在音乐期望和意外性建模中的表现，发现扩散模型在多项任务中表现更优。


<details>
  <summary>Details</summary>
Motivation: 探讨扩散模型在音乐信息内容（IC）建模中的有效性，尤其是对音乐意外性和音频特征的捕捉能力。

Method: 使用基于两种不同扩散ODE的模型计算IC，并与GIVT进行比较，通过负对数似然评估性能。

Result: 扩散模型在单音高意外性和多轨音频片段边界检测任务中表现优于或等同于GIVT。

Conclusion: 扩散模型在不同噪声水平下能有效捕捉音乐和音频特征，提升任务表现。
...
### [280] [A Scalable Pipeline for Enabling Non-Verbal Speech Generation and Understanding](https://arxiv.org/abs/2508.05385)
*Runchuan Ye,Yixuan Zhou,Renjie Yu,Zijian Lin,Kehan Li,Xiang Li,Xin Liu,Guoyang Zeng,Zhiyong Wu*

Main category: cs.SD

TL;DR: 论文提出了一个大规模的非言语语音数据集NonVerbalSpeech-38K，用于提升语音系统的情感智能和交互丰富性。


<details>
  <summary>Details</summary>
Motivation: 现有语音系统主要关注言语内容，缺乏对非言语线索（如笑声、叹息）的理解和生成能力，限制了交互的情感表达。

Method: 通过自动标注流程从真实媒体中收集并标注了38,718个样本（约131小时），涵盖10类非言语线索。

Result: 使用F5-TTS和Qwen2-Audio等模型验证了数据集在非言语语音生成和理解任务中的有效性。

Conclusion: 该数据集和标注流程为提升人机交互的丰富性提供了实用工具，推动了非言语语音研究的发展。
...
### [281] [SPGISpeech 2.0: Transcribed multi-speaker financial audio for speaker-tagged transcription](https://arxiv.org/abs/2508.05554)
...
TL;DR: SPGISpeech 2.0是一个金融领域的语音数据集，新增3,780小时的专业转录内容，支持多说话者ASR任务，并验证了其在说话者标记ASR性能上的提升。


<details>
  <summary>Details</summary>
Motivation: 提升语音识别任务的多样性，同时保持原始数据集的核心特性，适用于端到端ASR和多说话者场景。

Method: 扩展数据集，新增专业转录的财报电话会议音频，并提供说话者和通话信息。
...
### [282] [Online EFX Allocations with Predictions](https://arxiv.org/abs/2508.04779)
*Themistoklis Melissourgos,Nicos Protopapas*

Main category: cs.GT

TL;DR: 研究在线公平分配问题，目标是实现近似EFX分配，但发现即使有预测辅助也难以达成，仅在两代理同质估值下提出一种算法。


<details>
  <summary>Details</summary>
Motivation: 解决在线公平分配中实现EFX的挑战，探索预测辅助算法的潜力。

Method: 利用预测向量辅助分配，结合真实估值，提出两代理同质估值下的算法。

Result: 证明近似EFX分配在一般情况下不可行，预测准确性要求高，仅在两代理同质估值下算法有效。

Conclusion: 预测辅助在特定条件下可改善EFX分配，但普遍适用性受限。
...
### [283] [Toward Energy and Location-Aware Resource Allocation in Next Generation Networks](https://arxiv.org/abs/2508.05109)
*Mandar Datar,Mattia Merluzzi*

Main category: cs.GT

TL;DR: 论文提出了一种低复杂度的资源分配方法，通过Fisher市场模型在能源约束下平衡通信和计算资源，提升服务提供商的效用和公平性。


<details>
  <summary>Details</summary>
Motivation: 无线网络从单纯的资源提供者演变为涉及边缘和云计算资源的复杂系统，优化目标从性能导向转向价值导向，需平衡多方利益和能源约束。

Method: 采用Fisher市场模型，服务提供商通过虚拟货币预算竞争资源包，提出低复杂度解决方案。

Result: 数学证明了市场均衡，数值结果展示了不同场景下效用与能源的多维权衡。

Conclusion: 该方法在满足能源约束的同时，实现了高效用和公平性，优于社会最优解。
...
### [284] [A New Three-Players Auction Bridge with Dynamic Opponents and Team Members](https://arxiv.org/abs/2508.05582)
...
<details>
  <summary>Details</summary>
Motivation: 传统桥牌中的固定搭档关系限制了游戏的动态性和灵活性，本文旨在通过重新设计游戏规则和计分系统，提升游戏的公平性和策略性。

Method: 设计了动态队伍组成机制和新型计分系统，以减少传统规则中的偏见，并通过奖励机制强化战术决策和风险评估。

Result: 模拟结果显示，多样化的策略在游戏中表现高效，证明了新设计的有效性。

Conclusion: 这种三人桥牌架构适合竞赛，并可能扩大锦标赛卡牌游戏的参与范围。
