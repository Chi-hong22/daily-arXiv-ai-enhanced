...
Motivation: 现有方法未能解决多模态实体链接中的提及歧义和模态内容动态选择问题，限制了性能。

Method: MMoE模型包含四个模块：描述感知提及增强模块、多模态特征提取模块、层内混合专家模块和层间混合专家模块，通过动态选择特征提升匹配效果。

Result: 实验表明MMoE在性能上优于现有最先进方法。

Conclusion: MMoE通过动态选择和增强机制有效解决了多模态实体链接中的关键问题，具有显著优势。
...
<details>
  <summary>Details</summary>
Motivation: 早期识别人类活动对安全性和响应性至关重要，事件视觉传感器因其高时间分辨率和低延迟特性适合此需求，但现有方法在处理事件时存在局限性。

Method: 采用高速率双流脉冲神经网络（SNN），避免传统低速率帧或时空体素累积的限制，实现早期预测。

Result: 在THU EACT-50数据集上，准确率比之前工作提升2%，并通过早期事件识别框架验证了其性能。

Conclusion: 该方法在早期活动识别中表现出色，并在实际运动捕捉任务中展示了应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 尽管图像生成模型（如Stable Diffusion）在生成内容的质量和多样性上取得了显著进展，但其可能生成有害或侵权内容的问题引发了伦理和法律担忧。去学习技术（MU）被认为是一种解决方案，但其鲁棒性和有效性尚未充分验证。

Method: 提出Recall框架，通过优化对抗性图像提示（而非仅依赖文本提示）来测试去学习模型的鲁棒性。该方法利用多模态条件扩散模型的能力，从单一参考图像生成对抗性输入。

Result: 在十种先进去学习方法上的实验表明，Recall在对抗效果、计算效率和语义保真度上均优于现有基线，揭示了当前去学习机制的漏洞。

Conclusion: 研究揭示了现有去学习技术的不足，强调了开发更鲁棒解决方案的必要性，以确保生成模型的安全性和可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 促进深度学习模型在生物医学图像分析中的透明性、信任度和临床采用率，填补现有综述在模态感知和多模态学习方面的不足。

Method: 系统分类XAI方法，分析其原理、优势和局限性，并提出基于模态的分类法。

Result: 总结了常用评估指标和开源框架，探讨了多模态学习和视觉语言模型的应用前景。

Conclusion: 为生物医学图像分析中可解释深度学习的发展提供了深入的基础和未来方向。
...
Method: 使用YOLOv8检测视野内的船只，通过特征匹配和色调直方图距离分析确认目标船只，并利用几何原理定位目标。
...
Method: 开发了SpatialViz-Bench，一个多模态基准测试，包含12个任务和1,180个自动生成的问题，用于评估33种先进MLLMs的空间可视化能力。

Result: 模型表现差异大，存在与人类直觉不符的困难感知、2D到3D性能骤降，以及过度依赖公式推导而非可视化能力等问题。

Conclusion: SpatialViz-Bench揭示了当前MLLMs在空间可视化任务中的不足，填补了该领域的空白，并公开了基准测试。
...
Result: 在五个基准数据集上表现优异，IoU指标分别提升0.011和0.020。
...
<details>
  <summary>Details</summary>
Motivation: EEG信号在空间细节上的局限性阻碍了图像重建，因此需要一种间接方法来解码视觉体验。

Method: 通过对比学习将EEG信号映射到由大型语言模型生成的多级语义描述，再通过预训练的潜在扩散模型生成图像。

Result: 在EEGCVPR数据集上实现了最先进的视觉解码，揭示了语义与EEG信号的关联。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有视频生成模型在长视频生成中角色一致性和场景布局的不足，尤其是多主题长视频的挑战。

Method: 全面研究了32篇视频生成论文，识别关键架构组件和训练策略，并提出新的分类法。

Result: 构建了现有方法的分类法，并通过比较表格按架构设计和性能特征对论文进行分类。

Conclusion: 通过系统分析，提出了改进长视频生成中角色一致性和场景布局的关键方向。
...
Conclusion: 颜色是解决衣物变化重识别中外观偏差的有效且经济的代理。
...
<details>
  <summary>Details</summary>
Motivation: 传统人物识别在多样化活动中面临身份线索与运动动态和外观变化纠缠的挑战，现有视觉数据（如姿态和轮廓）提取不准确，需要更鲁棒的方法。

Method: 提出DisenQ框架，利用结构化语言指导解耦生物特征、运动和非生物特征，避免依赖额外视觉数据。

Result: 在三个基于活动的视频基准测试中取得最优性能，并在传统视频识别基准测试中表现出强泛化能力。

Conclusion: DisenQ框架通过语言指导有效解耦特征，显著提升活动生物识别的准确性和泛化能力。
...
Method: 构建包含6,875对图像-文本的数据集，评估指标包括偏差、答案相关性和忠实度。

Result: 闭源模型表现最佳，开源模型如Qwen2.5在多语言泛化上表现优异。

Conclusion: 发布基准和代码以促进可复现性和进一步研究。
...
Method: 引入MagiC基准，包含约5,500个弱监督QA示例和900个人工标注示例，评估15个模型的四个维度。
...
Result: 评分模型在多个基准测试中表现优异，显著提升了与人类评分的相关性及比较准确性。

Conclusion: ADIEE方法有效解决了指令引导图像编辑的自动化评估问题，并可作为奖励模型优化编辑效果。
...
Conclusion: 该方法在保持真实性的同时提高了虚拟试妆的可扩展性和效率。
...
Result: 模型在实体识别和连贯性方面显著提升，如grounding mAP提高14.8%，F1提高17.1%，跨帧实体持续性增加13.7%。

Conclusion: 提出的方法有效提升了视觉叙事系统中实体识别的准确性和连贯性，显著改善了故事生成质量。
...
Method: 利用2D到3D投影中的模糊性和一致性，合成多样化的伪几何数据，并通过深度基础模型和插值/重定位策略扩展数据。

Result: PacGDC在多个基准测试中表现出卓越的泛化能力，适应不同场景和深度模式。

Conclusion: PacGDC为深度补全提供了一种高效且泛化能力强的解决方案。
...
Conclusion: 提出的机制显著提升了PSM的自适应性，为复杂解剖结构的表示提供了更优解决方案。
...
Method: 提出MSAGSM，结合多尺度时间扩张和多头空间注意力，提升时空建模能力，并引入TTA数据集。

Result: MSAGSM在五个PES基准测试中表现优异，性能显著提升，计算开销小。

Conclusion: MSAGSM是一种轻量级即插即用模块，能有效提升PES任务性能，并在新数据集上验证了其优越性。
...
<details>
  <summary>Details</summary>
Motivation: 通过关键点引导，提升行人重识别的时空表示能力，解决现有方法的局限性。

Method: 全局分支使用Transformer进行时间聚合，局部分支根据关键点动态分割身体区域生成细粒度特征。

Result: 在MARS和iLIDS-VID基准测试中表现优异，MARS上mAP达91.73%，Rank-1准确率97.32%；iLIDS-VID上Rank-1准确率96.00%，Rank-5准确率100.0%。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注人类运动迁移，忽略了动物特有的习惯行为，导致跨类别运动迁移效果不佳。

Method: 基于生成框架，引入习惯保留模块和类别特定习惯编码器，结合LLM扩展迁移能力。

Result: 在DeformingThings4D-skl数据集上验证了模型的有效性，实验表明其优于现有方法。

Conclusion: 该框架成功保留了动物习惯行为，为跨类别运动迁移提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 无约束照片集合易获取但存在光照不一致和瞬态遮挡，传统方法难以处理。

Method: 结合多维特征嵌入和3D高斯溅射，引入Spiky 3D Gaussian Cutter平滑异常高斯，利用SAM掩模计算切割比例。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法需多阶段处理，忽略跨视角依赖且计算冗余，EscherNet++旨在通过掩码微调实现高效端到端新视角合成与模态补全。

Method: 采用输入级和特征级掩码微调，实现端到端模型；与其他前馈图像到网格模型结合，无需额外训练。

Result: 在遮挡任务中PSNR提升3.9，Volume IoU提升0.28；重建时间减少95%，支持快速3D重建。

Conclusion: EscherNet++在小数据集和批量下实现SOTA性能，泛化能力强，适用于实际遮挡重建场景。
...
Result: 计算资源消耗和可训练参数显著减少（约基础模型的1%），在UPMC-Food101和SNLI-VE数据集上表现优异，MM-IMDB数据集上表现相当。
...
Conclusion: Corvid通过增强链式思维推理能力和优化训练方法，显著提升了多模态大语言模型的推理性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前3D视觉语言研究中空间推理能力未被充分探索，现有数据集存在语义与空间关系混淆的问题，导致模型依赖表面捷径而非真正理解空间关系。

Method: 提出S\textsc{urprise}3D数据集，包含200k+视觉语言对和89k+人工标注的空间查询，避免使用对象名称以减少偏见。

Result: 初步测试显示当前先进的3D视觉定位方法和3D-LLM在该数据集上表现不佳，凸显了其挑战性。
...
<details>
  <summary>Details</summary>
Motivation: 工业点云分析中，现有基准数据集多关注低分辨率数据，而高分辨率数据对检测细微异常至关重要。为解决这一差距，研究提出了生成高分辨率3D异常数据的方法。

Method: 提出了一个可扩展的流程生成高分辨率3D异常数据，并开发了数据集MiniShift。同时，设计了Simple3D框架，结合MSND和LFSA技术，高效捕捉几何细节。

Result: Simple3D在MiniShift和现有基准测试中表现优异，精度和速度均超越现有方法，实现了实时推理（超过20 fps）。
...
<details>
  <summary>Details</summary>
Motivation: 超声成像因噪声问题对自动分割任务构成挑战，需要一种能有效抑制噪声并提升分割性能的方法。

Method: 提出DSANet框架，包含AFSA模块（相邻帧语义感知）和LGSA模块（局部与全局语义感知），通过特征融合增强噪声鲁棒性。

Result: 在四个基准数据集上，DSANet的分割精度显著优于现有方法，且推理速度更快。

Conclusion: DSANet通过双语义感知设计有效解决了超声视频分割中的噪声问题，同时提升了性能和效率。
...
Result: 模型在不同数据集上表现优异，测试准确率最高达95.05%。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有OoD分割方法在边界精度、异常评分一致性和背景噪声方面的不足，提升自动驾驶等安全敏感应用中的分割效果。

Method: 1. 粗粒度异常评分（CAS）；2. 对象感知分数校准（OASC）；3. 精细边界优化（MBP）。

Result: 在SMIYC和RoadAnomaly等基准测试中，像素级和组件级指标均显著提升（如AuPRC达96.99）。
...
<details>
  <summary>Details</summary>
Motivation: 恶劣天气条件下的人脸图像质量下降，导致识别准确率降低，现有方法缺乏专门针对天气退化的模块，导致纹理和结构失真。

Method: 提出了一种GAN框架，包含SFFT和DAFE模块。SFFT通过局部统计分布对齐提升结构保真度，DAFE通过特征嵌入实现退化无关的特征提取。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常将人体姿态表示为抽象图结构，忽略了关节间的物理依赖，导致学习困难且易生成不真实运动。GGMotion旨在更好地利用动态和运动学先验。

Method: 提出分组图网络，通过空间和时间边聚合关节特征；引入组内和组间交互模块捕捉多尺度依赖；结合等变多层感知机更新关节位置特征。

Result: 在Human3.6M、CMU-Mocap和3DPW基准测试中表现优异，短期运动预测性能显著提升。

Conclusion: GGMotion通过分组建模和动态-运动学传播，显著提升了运动预测的物理合理性和性能。
...
Method: 提出MUVOD数据集，包含17个场景的多视角视频和分割掩码，并设计评估指标和基线方法。

Result: MUVOD数据集包含7830张RGB图像和459个实例，覆盖73个类别，为4D分割任务提供了基准。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在密集点轨迹建模中，隐式变形场或线性混合蒙皮技术存在空间一致性不足或依赖启发式初始化的问题，且稀疏时间信号插值潜力未充分探索。

Method: 采用基于样条的轨迹表示，引入低秩时变空间编码来建模节点特性，替代传统的时空耦合技术。
...
Method: 使用模态条件令牌路由机制预训练多模态数据，并通过模态感知剪枝技术优化任务模型。

Result: 在多种遥感数据集上表现优于全监督训练和现有基础模型。
...
Method: 引入LSC概念，使用线性分类器评估VLMs视觉嵌入的性能，并通过任务依赖的干预（如激活现有路径或调整核心权重）进行实验。

Result: 发现VLMs存在强大的潜在推理路径，但复杂关系推理需要更深层次的调整。同时，提升表示质量可能导致模型在新提示格式下失败。
...
Method: 提出了DGKD-WLSS框架，结合扩散引导的知识蒸馏（DGKD）和深度引导的特征融合（DGF2），前者通过扩散去噪和知识蒸馏对齐正常光和低光特征，后者利用深度图作为光照不变的几何先验增强结构特征学习。

Result: 实验证明DGKD-WLSS在低光条件下的弱监督语义分割任务中达到了最先进的性能。

Conclusion: DGKD-WLSS通过结合扩散和深度信息，有效解决了低光环境下弱监督语义分割的挑战，并显著提升了模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 洪水是频繁的自然灾害，造成重大社会经济损失。现有洪水测绘产品往往在空间细节和覆盖范围之间权衡，或忽略洪水深度。HOTA旨在填补这一空白。

Method: HOTA采用分层重叠平铺聚合策略，结合SegFormer和双约束深度估计模块，利用多尺度推理和DEM差分方法生成3D洪水地图。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在多视角头发转移任务中生成一致且高质量输出的能力不足，影响数字人和虚拟形象等实际应用。

Method: 设计了多视角训练数据生成流程，包括Bald Converter、数据增强修复模型和面部微调多视角扩散模型；提出多阶段训练策略，整合极坐标嵌入和时间注意力层。

Result: 实验表明，该方法能准确转移发型并实现视角间无缝一致，显著优于现有方法。

Conclusion: Stable-Hair v2在多视角头发转移任务中设定了新基准，代码已开源。
...
Method: ViLU训练一个二元分类器作为不确定性预测器，使用加权二元交叉熵损失区分正确和错误预测，不依赖损失预测。

Result: 在多样化数据集上的实验表明，ViLU显著优于现有失败预测方法。
...
TL;DR: 该论文研究了深度学习面部表情识别（FER）模型中的年龄偏见，尤其是对老年人的影响，并提出了三种缓解策略，显著提高了老年人表情识别的准确性。
...
Method: 通过可解释AI技术分析年龄偏见，提出并评估三种策略：多任务学习、多模态输入和年龄加权损失，使用AffectNet数据集训练并在平衡数据集上验证。

Result: 策略显著提高了老年人表情识别的准确性，尤其是易错表情，且模型注意力更集中于相关面部区域。

Conclusion: 年龄偏见可通过简单训练调整有效缓解，近似人口统计标签有助于提升大规模情感计算系统的公平性。
...
TL;DR: MolCLIP结合细胞视频和分子模态，通过视觉语言模型改进药物识别和MoA识别性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法依赖细胞图像但忽略时间动态，而分子模态可补充图像信息。

Method: 提出MolCLIP，结合分子辅助CLIP框架和度量学习策略，优化视频特征学习。

Result: 在MitoDataset上，药物识别和MoA识别的mAP分别提升51.2%和20.5%。

Conclusion: MolCLIP通过多模态融合显著提升药物MoA识别性能。
...
TL;DR: 研究提出了一种基于深度学习的用户交互模型ARNet，用于简化儿科正畸中颈椎成熟度评估的关键点标注过程，提高生长潜力预测的效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 儿科正畸中准确评估生长潜力对制定有效治疗策略至关重要，但传统关键点标注方法费时费力。

Method: 通过侧位头颅X光片分析颈椎成熟度特征，提出ARNet模型，结合交互引导的重新校准网络和形态感知损失函数，优化关键点标注。

Result: ARNet在多数据集验证中表现出色，显著减少人工标注工作量，提升效率和准确性。

Conclusion: 该研究为儿科正畸生长潜力评估提供了一种高效的AI辅助诊断工具，推动了领域发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有动态面部表情识别研究多从深度学习角度出发，缺乏对AU-expression知识的利用，因此提出AU-DFER以提升模型效果。

Method: 量化AU对表情的贡献，设计权重矩阵，引入AU损失函数，并将其整合到现有最优模型中。

Result: 在多个主流数据集上验证，AU-DFER优于当前最优方法，且无需额外计算资源。

Conclusion: AU-DFER首次将量化AU知识整合到DFER模型中，并解决了数据标签不平衡问题，为未来研究提供了新思路。
...
Method: 重新定义多模态CoT推理为KL约束的奖励最大化问题，并提出RED解码策略，通过结合图像和推理条件分布来优化推理。
...
<details>
  <summary>Details</summary>
Motivation: 现有Mamba方法在水下深度估计中表现不佳，且现有数据集标签不可靠，导致深度估计不准确。
...
TL;DR: 提出了一种新的去模糊方法，通过可训练掩码预测器和帧内运动分析器，有效减少计算资源并提升模糊恢复效果。
...
<details>
  <summary>Details</summary>
Motivation: 统一视觉语言模型在多任务指令下的安全性问题尚未充分研究，需要系统评估其对抗性攻击的跨任务有效性。

Method: 构建CrossVLAD数据集，提出CRAFT攻击框架，通过区域中心化和令牌对齐实现高效攻击。

Result: CRAFT在Florence-2等模型上表现优于现有方法，显著提高了跨任务攻击和目标对象更改的成功率。

Conclusion: CrossVLAD和CRAFT为统一视觉语言模型的对抗性安全性提供了新的评估工具和攻击方法。
...
TL;DR: 研究重新审视了胸部X光数据集中的数据集偏见问题，通过简单变换数据集来识别偏见，并测试了多种网络架构。
...
<details>
  <summary>Details</summary>
Motivation: SAM模型在医学影像中具有潜力，但其2D架构不适用于3D数据，且现有3D方法复杂且资源密集。

Method: 受SegVol启发，提出了一种简化的3D可提示分割方法，避免滑动窗口和复杂的提示管理。
...
TL;DR: 论文提出了一种基于能量的解码方法，用于减少大型视觉语言模型中的物体幻觉问题，显著提升了三个VQA数据集的性能。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法在解码、视觉输入修改或依赖外部模型知识方面的局限性，并揭示VLMs在VQA数据集中的“Yes”比例不平衡现象。

Method: 提出基于能量的解码方法，动态选择能量分数最低的隐藏状态层，以减少对“Yes”答案的偏见。

Result: 在三个基准测试（POPE、MME、MMVP）中，方法显著提升了准确率和F1分数，平均准确率提升4.82%，Yes比例差距减少8.81%。

Conclusion: 该方法简单有效，能显著减少物体幻觉问题，提升模型性能并降低偏见。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖预训练模型的最终层特征，适应性有限；而全微调不实用，参数高效的侧调谐（ST）成为替代方案，但现有ST方法忽略了MR的稀疏性。

Method: 提出SDST，首个无锚点的ST架构，引入参考可变形自注意力机制，并首次将InternVideo2主干集成到ST框架中。

Result: 在QVHighlights、TACoS和Charades-STA上取得竞争性或SOTA结果，参数数量减少73%。

Conclusion: SDST显著提升了ST方法的性能，同时大幅降低了参数需求，为VTG任务提供了高效解决方案。
...
TL;DR: 论文提出X-RAFT模型，用于在荧光引导神经手术中通过跨模态光流方法实现高光谱图像的密集对应，显著减少误差。


<details>
  <summary>Details</summary>
Motivation: 通过高光谱成像技术提升荧光引导神经手术的实时定量荧光测量能力，以改善手术决策。

Method: 引入X-RAFT模型，采用跨模态输入的光流方法，通过自监督学习和流循环一致性优化图像编码器。

Result: 相比基线方法误差减少36.6%，优于现有跨模态光流方法（误差减少27.83%）。

Conclusion: X-RAFT模型在跨模态图像对应中表现优异，为荧光定量提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 增材制造中的几何误差会影响部件性能，传统DVC方法因数据量大和缺乏基准变形场而难以准确配准。
...
Conclusion: SCOOTER填补了无限制对抗攻击评估的空白，强调了人类感知的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 探讨在数据集偏移下如何选择校准方法，为实践者提供稳健校准的指导。

Method: 比较后处理校准方法和训练中校准策略（如标签平滑），在多种自然偏移和分类任务中测试。

Result: 发现熵正则化和标签平滑结合效果最佳，后处理校准器在少量OOD数据下更稳健，新校准方法改进有限，且校准偏移常以牺牲分布内校准为代价。

Conclusion: 基础模型微调后校准效果更好，集成方法结合微调能显著提升校准稳健性。
...
TL;DR: 提出了一种基于3D卷积神经网络的端到端rPPG提取网络，通过差分帧融合模块和自注意力机制提升信号重建精度，动态混合损失函数减少过拟合，实验验证了其优越性。
...
Result: 在PURE、UBFC-rPPG和MMPD数据集上表现优异，MAE为7.58，优于现有模型。

Conclusion: 所提网络在复杂场景下具有更高的鲁棒性和泛化能力，性能显著提升。
...
<details>
  <summary>Details</summary>
Motivation: 现实应用中，多模态输入的缺失或不完整会导致性能显著下降，现有静态提示方法缺乏灵活性且无法在关键模态缺失时保证可靠性。

Method: SyP框架包含动态适配器（动态生成提示）和协同提示策略（结合静态与动态提示），以灵活适应不同缺失条件并平衡模态信息。

Result: SyP在三个广泛使用的视觉识别数据集上显著优于现有方法，并在不同缺失率和条件下表现出鲁棒性。

Conclusion: SyP通过动态和协同提示策略有效解决了缺失模态问题，展示了卓越的适应性和可靠性。
...
TL;DR: 该研究首次将Vision Transformers（ViT）应用于无标记的肺部肿瘤运动预测，比较了患者特异性（PS）和多患者（MP）训练策略，发现PS模型在精度上更优，而MP模型在临床时间受限的情况下表现更稳健。


<details>
  <summary>Details</summary>
Motivation: 当前无标记方法主要依赖深度学习，而基于Transformer的架构在轨迹预测中表现优异，但在肺部肿瘤运动预测领域尚未探索。

Method: 使用31名患者的4DCT扫描生成的DRR训练MP模型，PS模型仅使用目标患者的规划数据。两种模型均预测1秒内的肿瘤运动，并通过ADE和FDE评估性能。

Result: PS模型在T1数据上表现更优，但MP模型对分次间的解剖变异更具鲁棒性，且在T2数据上无需重新训练即可达到可比性能。

Conclusion: ViT架构首次应用于无标记肿瘤运动预测，PS模型精度高，而MP模型适合时间受限的临床场景。
...
TL;DR: 研究了内容拼图求解器在现实挑战（如缺失、边缘和内容腐蚀）中的鲁棒性，发现标准求解器性能下降明显，但深度学习模型通过数据增强可显著提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有内容拼图求解器的评估缺乏现实挑战，如文物碎片或文件碎片的重新组装，因此需要研究其鲁棒性。

Method: 引入三种拼图损坏类型（缺失、边缘腐蚀、内容腐蚀），评估启发式和深度学习求解器的表现。

Result: 标准求解器在损坏增加时性能下降快，但深度学习模型通过微调和数据增强显著提升鲁棒性，其中Positional Diffusion模型表现最佳。

Conclusion: 研究为提升现实文物自动重建提供了方向，深度学习模型尤其是Positional Diffusion模型展现了潜力。
...
TL;DR: SimCIS提出了一种简单但强大的类增量图像分割方法，通过直接选择图像特征进行查询分配，解决了现有方法中的塑性损失和数据顺序依赖问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在解耦掩码生成与持续学习过程时，存在塑性损失和输入数据顺序依赖问题，SimCIS旨在解决这些问题。

Method: SimCIS通过直接选择图像特征进行查询分配，确保完美对齐以保留对象性，同时引入跨阶段一致性选择和基于视觉查询的重放机制。

Result: 实验表明，SimCIS在各种分割任务、设置、分割和输入数据顺序下均优于现有方法。

Conclusion: SimCIS是一种有效的类增量图像分割方法，解决了塑性损失和数据顺序依赖问题，性能优于现有技术。
...
<details>
  <summary>Details</summary>
Motivation: 工业表面缺陷是导致制造低效的主要原因之一，现有数据集无法反映真实工业场景，限制了自动化缺陷检测技术的发展。
...
Method: THUNDER通过多样化的数据集和下游任务，比较23种基础模型，分析特征空间并评估预测的鲁棒性和不确定性。

Result: THUNDER支持多种先进模型，并在16个数据集上进行了全面比较，提供了代码公开。

Conclusion: THUNDER为数字病理学领域提供了一个高效、可靠的基准测试工具，有助于推动研究进展。
...
TL;DR: 论文提出了一种名为SLURPP的新网络架构，结合预训练的潜在扩散模型和显式场景分解，用于水下图像恢复，显著提升了速度和性能。
...
Method: SLURPP结合预训练潜在扩散模型和显式场景分解，并设计了一个基于物理的水下图像合成流程生成多样化训练数据。
...
Method: 提出MIRA框架，包含校准的重新思考和重排模块，以及整合图像嵌入和医学知识库的医疗RAG框架。

Result: 在公开医学VQA和报告生成基准测试中，MIRA显著提升了事实准确性和整体性能，达到最新技术水平。

Conclusion: MIRA通过优化检索和推理机制，有效提升了多模态大语言模型在医学应用中的事实准确性。
...
Result: 在AMD/Xilinx Zynq UltraScale+平台上实现640x480像素图像处理，速度达54fps，优于现有解决方案。

Conclusion: 量化技术和硬件优化显著提升了模型在视觉里程计任务中的性能和效率，适用于资源受限的嵌入式系统。
...
TL;DR: 提出了一种名为CiCi的新型完全测试时间适应（TTA）策略，用于远程光电容积描记（rPPG）任务，通过结合时空一致性和不一致性先验知识，提升模型在推理时的适应能力。


<details>
  <summary>Details</summary>
Motivation: 现有深度rPPG模型在未见部署环境中的适应性受限于隐私和实时适应问题，需要一种无需源数据的实时自监督适应方法。

Method: 利用生理学先验知识，提出基于一致性和不一致性先验的CiCi框架，并结合梯度动态控制机制以稳定适应过程。

Result: 在五个不同数据集上的实验表明，该方法在TTA协议下优于现有技术，实现了最先进的实时自监督适应性能。

Conclusion: CiCi框架通过结合一致性和不一致性先验，显著提升了rPPG模型在未见环境中的适应能力，且无需源数据支持。
...
Conclusion: 该算法显著提高了小鼠跟踪效率和识别准确性，适用于长期监测。
...
Result: 在14个公共HAR数据集上，TinierHAR参数量减少2.7倍（相比TinyHAR）和43.3倍（相比DeepConvLSTM），计算量减少6.4倍和58.6倍，同时保持F1分数。
...
TL;DR: 提出了一个全栈框架，通过强化学习扩展视觉语言模型（VLM）在长视频中的推理能力，包含数据集、训练管道和基础设施，并在实验中表现出色。
...
Method: 提出三部分：1）大规模数据集LongVideo-Reason；2）两阶段训练管道（CoT-SFT和RL）；3）训练基础设施MR-SP。

Result: LongVILA-R1-7B在长视频QA基准测试中表现优异，MR-SP系统提速2.1倍。
...
TL;DR: 提出了一种合成火星景观视频的解决方案，包括数据重建和视频生成两部分，解决了火星数据稀缺和域差异问题。


<details>
  <summary>Details</summary>
Motivation: 为任务演练和机器人模拟合成逼真的火星景观视频，但面临火星数据稀缺和与地球图像的域差异挑战。
...
TL;DR: 论文提出了一种名为Geometry Forcing的方法，通过几何对齐目标改进视频扩散模型，使其学习更具3D一致性的表示。


<details>
  <summary>Details</summary>
Motivation: 视频扩散模型仅基于原始视频数据训练时，难以捕捉几何感知的结构，因此需要一种方法弥补这一差距。

Method: 提出Geometry Forcing，通过Angular Alignment和Scale Alignment两种目标，将模型中间表示与预训练的几何基础模型特征对齐。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要评估离线场景下的固定输入，而现实世界中的感知任务需要模型处理增量观测和时空推理，因此设计了OST-Bench以填补这一空白。

Method: 通过高效的数据收集流程，构建了包含1.4k场景和10k问答对的数据集，基于ScanNet、Matterport3D和ARKitScenes。评估了多个领先MLLMs的性能。

Result: 现有MLLMs在复杂时空推理任务中表现不佳，随着探索时间和记忆增长，准确性下降。实验揭示了模型在空间推理和长期记忆检索方面的核心挑战。

Conclusion: OST-Bench揭示了MLLMs在在线时空推理中的局限性，为未来研究提供了方向和资源，数据集和代码已开源。
...
TL;DR: 研究发现CLIP模型的数据属性（如低属性密度、不完整描述和显著性偏差）对绑定学习能力有负面影响，而增加批次大小或硬负样本无效。


<details>
  <summary>Details</summary>
Motivation: CLIP模型在表示学习上存在局限性，如无法区分颜色和物体的组合，需探究数据属性对其绑定能力的影响。

Method: 使用合成数据集系统分析数据属性对CLIP绑定学习的影响。

Result: 发现自然数据的常见属性（如低属性密度、显著性偏差）损害绑定性能，仅当数据具备特定属性时CLIP才能学习完美绑定。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型（LLMs）因处理大量时空令牌导致计算复杂度呈二次方增长，亟需一种高效的方法减少令牌数量而不显著影响性能。

Method: STTM采用多粒度空间令牌转换和定向时间维度合并，通过四叉树结构进行粗到细的搜索，实现令牌的高效合并。

Result: 在六个视频问答基准测试中，STTM表现优于现有令牌减少方法，50%令牌预算下速度提升2倍，准确率仅下降0.5%；30%预算下速度提升3倍，准确率下降2%。

Conclusion: STTM是一种高效、查询无关的视频令牌合并方法，适用于多种视频理解任务，显著提升了计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有脑视觉解码评估协议依赖粗糙指标，掩盖模型差异，缺乏神经科学基础，无法捕捉细粒度视觉差异。

Method: 引入BASIC框架，包括基于分割的分层指标（前景、语义、实例和组件掩码）和基于多模态大语言模型的结构化场景表示。

Result: 在统一框架下对多种视觉解码方法进行基准测试，提供更区分性、可解释性和全面性的评估。
...
<details>
  <summary>Details</summary>
Motivation: 解决少样本学习中源数据与查询数据分布不一致的问题，利用草图作为无源替代。

Method: 采用原型设置，结合基于网格的定位器和原型域适应。

Result: 通过实验验证了在新关键点和类别上的少样本收敛效果。

Conclusion: 框架成功克服了跨模态和用户风格挑战，适用于少样本关键点检测。
...
TL;DR: KARL是一种单次自适应标记器，基于Kolmogorov复杂性原理，通过单次前向传递预测图像标记数量，达到近似KC后停止。


<details>
  <summary>Details</summary>
Motivation: 现有视觉表示学习系统使用固定长度表示，忽略了复杂性或熟悉度的变化，而自适应标记方法通常需要多次编码搜索。

Method: KARL通过单次前向传递预测标记数量，训练过程类似于倒置强化学习，根据重建质量条件预测标记停止。

Result: KARL性能与现有自适应标记器相当，但仅需单次操作，并分析了编码器/解码器大小、连续与离散标记化等因素。
...
TL;DR: 论文提出了一种名为\NickName的新方法，通过增强离散码本的表示能力，优化量化策略，显著提升了VQ-VAEs的重建质量，并在多个基准测试中达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有VQ-VAEs在量化策略上仍有改进空间，与VAEs存在较大差距。论文旨在缩小这一差距，提升重建质量。

Method: 保留潜在维度以保留编码特征，并引入一组子码本进行量化。同时构建了512p和2k分辨率的零样本基准测试。

Result: \NickName在ImageNet和8个零样本基准测试中均达到最优性能，显著优于SD-VAE，rFID为0.49 vs. 0.91，PSNR表现优异。

Conclusion: \NickName在重建任务中表现出色，为高保真度图像处理任务提供了新思路，代码已公开。
...
TL;DR: TreeBench是一个新的视觉推理基准测试，专注于复杂场景中的细微目标感知、可追踪证据评估和二阶推理。TreeVGR是一种联合监督定位和推理的训练范式，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有模型如OpenAI-o3在视觉推理方面表现出色，但缺乏全面评估其能力的基准测试。

Method: 提出TreeBench基准测试，基于三个原则构建，并开发TreeVGR训练范式，结合强化学习监督定位和推理。

Result: TreeBench包含405个高难度视觉问答对，现有模型表现不佳（如OpenAI-o3仅54.87%准确率）。TreeVGR显著提升了多个基准测试的性能。
...
Method: 使用点互信息（PMI）衡量词共现频率，通过合成图像和自然图像编辑实验验证PMI与模型性能的相关性。

Result: PMI与CLIP零样本准确率高度相关（r=0.97），且在自然图像中重现（r=0.75），LMMs也表现出类似行为。
...
TL;DR: 本文通过混合主题分析方法，研究了数据新闻中的数据准备工作，扩展了数据科学工作模型，并提出了一个新的分类法来表征脏数据问题。


<details>
  <summary>Details</summary>
Motivation: 数据准备工作在数据科学中耗时且繁琐，但在数据新闻领域的研究较少，本文旨在填补这一空白。

Method: 采用混合主题分析方法，结合演绎和归纳编码，对36名专业数据记者进行访谈研究。

Result: 扩展了数据科学工作模型，提出了一个基于心智模型差异的脏数据分类法，并识别了记者面临的四大挑战。

Conclusion: 本文为数据新闻中的数据准备工作提供了新的理论和实践见解，有助于改进相关工作流程。
...
### [94] [FLoRA: An Advanced AI-Powered Engine to Facilitate Hybrid Human-AI Regulated Learning](https://arxiv.org/abs/2507.07362)
*Xinyu Li,Tongguang Li,Lixiang Yan,Yuheng Li,Linxuan Zhao,Mladen Raković,Inge Molenaar,Dragan Gašević,Yizhou Fan*

Main category: cs.HC

TL;DR: 论文介绍了增强版的\flora引擎，结合生成式AI和学习分析技术，支持自主学习（SRL）和混合人机调节学习（HHAIRL），并提供多种工具以实时适应学习者需求。


<details>
  <summary>Details</summary>
Motivation: 现有数字工具在支持自主学习时存在不足，如缺乏适应性和对SRL各阶段的全面支持。论文旨在通过\flora引擎解决这些问题。

Method: \flora引擎整合生成式AI和学习分析技术，提供协作写作、多代理聊天机器人等工具，实现动态、适应性支持。

Result: 研究表明，\flora引擎能有效促进SRL和HHAIRL，为AI增强学习提供理论和实践解决方案。

Conclusion: \flora引擎为未来AI增强学习环境提供了创新工具和理论支持，展示了其在教育中的潜力。
...
### [95] [Pluri-perspectivism in Human-robot Co-creativity with Older Adults](https://arxiv.org/abs/2507.07550)
*Marianne Bossema,Rob Saunders,Aske Plaat,Somaya Ben Allouch*

Main category: cs.HC

TL;DR: 本文探讨了多视角主义作为人类创造性体验的核心元素及其在人机共创中的重要性，提出了一个五维分层模型，用于指导共创行为设计和交互动态分析。


<details>
  <summary>Details</summary>
Motivation: 研究多视角主义如何支持创造性实践，并探讨机器人如何通过适应性行为增强人类创造力。

Method: 基于文献和10位视觉艺术家及8位艺术教育工作者的访谈研究，提出五维分层模型。

Result: 研究发现多视角主义能通过情境敏感行为提升人类创造力，展示了其潜力。
...
### [96] [ArchiveGPT: A human-centered evaluation of using a vision language model for image cataloguing](https://arxiv.org/abs/2507.07551)
*Line Abele,Gerrit Anders,Tolgahan Aydın,Jürgen Buder,Helen Fischer,Dominik Kimmel,Markus Huff*

Main category: cs.HC

TL;DR: 研究探讨了AI生成的目录描述是否能接近人类写作质量，并如何整合到档案和博物馆编目流程中。实验表明，AI生成描述在专家和非专家评估中表现尚可，但需人工审核以确保准确性，尤其是在考古等专业领域。


<details>
  <summary>Details</summary>
Motivation: 随着摄影收藏的快速增长，手动编目已无法满足需求，促使研究使用视觉语言模型（VLMs）自动化生成元数据。

Method: 使用VLM（InternVL2）为考古照片生成目录描述，由专家和非专家评估其质量、分类能力及对AI工具的信任度。

Result: AI生成描述的分类表现高于随机水平，但存在OCR错误和幻觉问题。专家对AI工具的接受度较低，更关注保存责任而非技术性能。

Conclusion: 建议采用协作方式，AI生成草稿后由人工审核，确保符合策展价值观。成功整合需技术改进（如领域微调）和建立专业信任。
...
### [97] [Conjugated Capabilities: Interrelations of Elementary Human Capabilities and Their Implication on Human-Machine Task Allocation and Capability Testing Procedures](https://arxiv.org/abs/2507.07560)
...
<details>
  <summary>Details</summary>
Motivation: 人类与自动化交互的基础是理解彼此的能力和表现，机器需根据人类能力调整行为。共轭能力的概念旨在通过能力间的努力分配克服人类限制。

Method: 分析IMBA标准中基本能力的相互关系，揭示共轭潜力，并在康复患者数据中验证。构建能力共轭网络，应用于制造业优化测试设计。

Result: 展示了共轭能力在优化IMBA测试设计和加速数据记录中的应用，并探讨了其对任务分配的影响。

Conclusion: 共轭能力为人类与自动化交互提供了新视角，优化了任务分配和测试设计，具有广泛的应用潜力。
...
TL;DR: 研究探讨了专家对商业AI公共演讲训练工具的看法，提出了改进建议。
...
Method: 通过16次半结构化访谈和2个焦点小组，收集专家意见。

Result: 专家认可AI工具的价值，但指出需改进反馈个性化和教学设计。
...
TL;DR: 提出了一种基于扩散模型的生成式全景图像拼接方法，解决了传统方法在视差和光照变化下的失败问题。


<details>
  <summary>Details</summary>
Motivation: 传统图像拼接方法在处理视差和光照变化时会产生伪影，现有生成模型无法合成大范围连贯的全景图。

Method: 通过微调扩散修复模型，基于多张参考图像保留场景内容和布局，生成无缝全景图。

Result: 在图像质量和场景一致性上显著优于基线方法。

Conclusion: 该方法能生成高质量、连贯的全景图，解决了现有技术的局限性。
...
TL;DR: LangSplatV2通过稀疏编码和高斯溅射技术，显著提升了3D语言场的速度和查询精度，实现了476.2 FPS的高维特征溅射和384.6 FPS的3D开放词汇查询。


<details>
  <summary>Details</summary>
Motivation: LangSplat在复杂场景中的语言交互应用受限，因其解码器计算量大，无法实现实时性能（8.2 FPS）。LangSplatV2旨在解决这一瓶颈。

Method: LangSplatV2将每个高斯视为全局字典中的稀疏编码，学习3D稀疏系数场，避免使用重型解码器，并通过CUDA优化实现高效溅射。

Result: LangSplatV2在保持或提升查询精度的同时，速度显著提升（42倍和47倍），实现了实时性能。

Conclusion: LangSplatV2通过稀疏编码和优化溅射方法，为3D语言场的高效实时应用提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有3D头发建模方法计算量大或需要网络训练的问题，提供更直观、高效的解决方案。

Method: 系统包含四个关键阶段：文本引导的头发检索、实时头发模拟、交互式头发优化和头发条件图像生成。

Result: 用户研究表明，该系统在快速原型设计上优于传统工作流程。

Conclusion: Digital Salon为数字媒体中的头发建模提供了直观、多功能且高效的解决方案，未来有望应用于真实沙龙环境。
...
TL;DR: 提出了一种基于神经潜在空间积分器的新型子空间模拟框架，显著提高了便携设备上的计算效率。
...
Result: 在棒、壳和固体等复杂示例中验证了方法的有效性和通用性。
...
TL;DR: 论文探讨了高端捕获阶段内容中图像抠图的挑战，提出了改进工作流程的指南，并展示了无需大量标注的高效管线。


<details>
  <summary>Details</summary>
Motivation: 高端捕获阶段的图像抠图在现有算法中表现不佳，需要针对性解决方案。

Method: 提出改进工作流程指南，并展示离线与实时的高效管线，无需大量标注。

Result: 通过基于扩散模型的验证方法，证明了所提方法的优势。

Conclusion: 论文为捕获阶段内容提供了实用的解决方案和验证方法。
...
Result: 实验结果表明，该方法在新视角合成、法线估计、分解和重光照方面表现优异，同时保持高效的训练推理过程。
...
TL;DR: Hi-D maps是一种新颖的多维分类数据可视化方法，通过2D多边形区域和多种视觉线索展示多维信息，支持交互和层次浏览。
...
Method: 将数据空间映射到2D多边形区域，通过平行切割和多种视觉线索（如方向、颜色、可计数符号等）展示信息，支持交互和层次浏览。

Result: Hi-D maps在多维数据可视化中提供清晰展示，但在维度过多时效果受限。

Conclusion: Hi-D maps是一种有效的多维数据可视化工具，尤其适合中等维度数据，并支持灵活探索。
...
### [107] [A Robust, Open-Source Framework for Spiking Neural Networks on Low-End FPGAs](https://arxiv.org/abs/2507.07284)
*Andrew Fan,Simon D. Levy*

Main category: cs.NE

TL;DR: 本文提出了一种基于FPGA的SNN加速框架，适用于低端FPGA，资源占用少，性能优越。


<details>
  <summary>Details</summary>
Motivation: 传统神经网络计算需求激增，SNN因其高效性成为解决方案，但现有硬件难以普及。FPGA作为中间方案，但高成本或单一拓扑限制了应用。

Method: 开发了一个包含SNN加速架构和PyTorch模型编译器的框架，支持任意拓扑SNN，资源占用低。

Result: 在低端FPGA上实现MNIST识别（0.52 ms/图），并准确模拟任意拓扑SNN。

Conclusion: 该框架为低端FPGA提供了高效、通用的SNN加速方案，代码开源。
...
### [108] [Homeostatic Adaptation of Optimal Population Codes under Metabolic Stress](https://arxiv.org/abs/2507.07874)
*Yi-Chun Hung,Gregory Schwartz,Emily A. Cooper,Emma Alexander*

Main category: cs.NE

TL;DR: 论文提出了一种新的理论框架，用于描述神经元在代谢资源限制和噪声特性下的信息处理行为，特别是低功耗模式下的动态变化。


<details>
  <summary>Details</summary>
Motivation: 现有数学模型无法准确描述神经元在代谢压力下的动态行为，如低功耗模式下的噪声增加和调谐曲线平坦化。

Method: 开发了一个基于能量预算和噪声水平的理论框架，结合生物物理模拟，推导出最优编码策略。

Result: 提出了一个能量依赖的分散泊松噪声模型，能够解释神经元在不同能量预算下的动态行为。

Conclusion: 该框架为理解神经元在代谢限制下的信息处理提供了新的理论基础，并得到了实验验证。
...
### [109] [Multilayer GNN for Predictive Maintenance and Clustering in Power Grids](https://arxiv.org/abs/2507.07298)
*Muhammad Kazim,Harun Pirim,Chau Le,Trung Le,Om Prakash Yadav*

Main category: eess.SY

TL;DR: 本文提出了一种多层图神经网络框架，用于提升电力系统的预测性维护和基于韧性的变电站聚类，显著优于传统方法。
...
Method: 采用多层图神经网络（GNN）框架，结合图注意力网络（空间）、图卷积网络（时间）和图同构网络（因果关系），通过注意力加权嵌入融合。

Result: 模型在30天F1分数上达到0.8935，优于XGBoost和随机森林3.2%和2.7%，比单层GNN高10-15%。聚类分析识别出8个风险组，最高风险组每年388.4次故障。
...
### [110] [Probability-Raising Causality for Uncertain Parametric Markov Decision Processes with PAC Guarantees](https://arxiv.org/abs/2507.07319)
*Ryohei Oura,yuji Ito*

Main category: eess.SY

TL;DR: 提出了一种基于参数采样、模型检查和样本覆盖的方法，用于识别不确定参数MDP中不良行为的潜在原因。


<details>
  <summary>Details</summary>
Motivation: 验证和理解复杂决策系统的行为至关重要，尤其是在MDP的转移概率不确定时，缺乏可靠的因果分析方法。

Method: 结合参数采样、模型检查和样本覆盖，定义基于概率提升原则的状态子集作为原因，并计算其概率阈值。

Result: 证明了所识别子集作为原因的概率超过指定阈值，并通过采样获得概率近似正确的下界。

Conclusion: 方法在路径规划场景中验证有效，为不确定MDP中的因果分析提供了可靠工具。
...
### [111] [Distributed and adaptive model predictive control for vehicle platoon systems under non-ideal communication](https://arxiv.org/abs/2507.07429)
*Qiaoni Han,Chengfei Xu,Zhiqiang Zuo*

Main category: eess.SY

TL;DR: 本文提出了一种分布式自适应模型预测控制方法，以应对无线通信不确定性对车队控制性能的影响。


<details>
  <summary>Details</summary>
Motivation: 无线通信的不确定性对车队控制性能提出了重大挑战，本文旨在减轻非理想通信对车队系统的影响。

Method: 通过定制补偿数据包处理通信不确定性，提出自适应MPC方法平衡系统响应速度和跟踪精度，并引入预测时间域更新策略以减少计算需求。

Result: 仿真结果表明，该方法在保证系统性能的同时显著降低了优化问题的计算资源需求。

Conclusion: 本文提出的方法有效解决了非理想通信下的车队控制问题，并提供了理论分析和仿真验证。
...
### [112] [Optimization of Probabilistic Constellation Shaping for Optical OFDM Systems with Clipping Distortion](https://arxiv.org/abs/2507.07507)
*Thanh V. Pham,Susumu Ishihara*

Main category: eess.SY

TL;DR: 该论文研究了概率星座成形（PCS）与光OFDM结合时对信号峰均功率比（PAPR）的影响，并提出了一种优化PCS的方法以最大化信道容量。


<details>
  <summary>Details</summary>
Motivation: PCS虽然能提高频谱效率和适应性，但与光OFDM结合时会增加PAPR，导致信号削波失真加剧。

Method: 提出了一种基于投影梯度下降的次优但高效的优化方法，以解决非凸优化问题。

Result: 仿真结果表明，该方法在严重削波失真条件下优于传统的均匀信号传输。

Conclusion: 优化后的PCS方法能有效提升光无线通信系统的性能，尤其是在高失真环境中。
...
### [113] [Perspective Chapter: Insights from Kalman Filtering with Correlated Noises Recursive Least-Square Algorithm for State and Parameter Estimation](https://arxiv.org/abs/2507.07588)
*Abd El Mageed Hag Elamin Khalid*

Main category: eess.SY

TL;DR: 提出了一种新的卡尔曼滤波方法KF-CN-RGELS，利用过程噪声与测量噪声的互相关性联合估计参数和系统状态，并通过理论分析和数值案例验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 研究线性随机系统中参数和状态的估计问题，特别是在存在确定性控制输入的情况下，以提高估计精度。

Method: 引入KF-CN-RGELS算法，利用噪声互相关性进行联合估计，并通过性能分析探讨相关系数对估计精度的影响。

Result: 研究发现，参数和状态的估计精度与正相关系数成正比，并通过与其他算法的比较验证了该方法的优越性。

Conclusion: 该算法显著提高了线性随机系统的估计精度，为控制系统设计和状态空间建模提供了重要参考。
...
### [114] [PhysioEdge: Multimodal Compressive Sensing Platform for Wearable Health Monitoring](https://arxiv.org/abs/2507.07645)
*Rens Baeyens,Dennis Laurijssen,Jan Steckel,Walter Daems*

Main category: eess.SY

TL;DR: 该论文提出了一种基于RP2350微控制器的定制硬件平台，用于同步多模态生物医学监测，结合压缩感知技术实现高效、低功耗的信号采集。


<details>
  <summary>Details</summary>
Motivation: 探索压缩感知与实时嵌入式系统的结合，以提升生物医学信号采集的效率和低功耗性能。

Method: 开发基于RP2350微控制器的硬件平台，支持同步采集多种生物医学信号（如PCG、ECG、EMG、PPG和IMU数据），并利用Sub-1GHz无线电系统实现多节点同步。

Result: 实验结果显示，压缩感知显著降低了功耗，多节点同步高效，系统具有可扩展性，适用于无线生物医学监测。

Conclusion: 该系统的紧凑设计和低成本特性使其适用于远程医疗和长期监测等医学应用。
...
### [115] [Remote Renewable Energy Hubs: a Taxonomy](https://arxiv.org/abs/2507.07659)
*Victor Dachet,Antoine Dubois,Bardhyl Miftari,Raphaël Fonteneau,Damien Ernst*

Main category: eess.SY

TL;DR: 论文提出了一种远程可再生能源枢纽（RREH）的分类法，以解决可再生能源在负荷中心附近有限可用性的问题。


<details>
  <summary>Details</summary>
Motivation: 可再生能源在负荷中心附近可用性有限，RREH作为解决方案，但缺乏统一分类法。

Method: 提出一种分类法，用于定义和比较RREH的设计，涵盖技术选择和本地集成。

Result: 分类法有助于描述、比较和识别新的RREH设计，指导政策制定和工程设计。

Conclusion: 分类法为RREH的设计提供了系统化工具，提升成本效率和本地集成。
...
### [116] [Ammonia, Methane, Hydrogen and Methanol Produced in Remote Renewable Energy Hubs: a Comparative Quantitative Analysis](https://arxiv.org/abs/2507.07681)
*Antoine Larbanois,Victor Dachet,Antoine Dubois,Raphaël Fonteneau,Damien Ernst*

Main category: eess.SY

TL;DR: 本文评估了四种远程可再生能源中心（RREHs）生产合成燃料的成本，发现氨（NH3）的成本效益最高。


<details>
  <summary>Details</summary>
Motivation: 研究旨在比较不同能源载体（甲烷、氨、氢和甲醇）在远程可再生能源中心中的生产成本和技术性能优劣。

Method: 扩展了Berger等人的研究，引入三种新载体，使用GBOML建模语言对系统进行建模和优化。

Result: 氨、氢和甲醇系统的成本均低于甲烷系统，其中氨的成本效益最佳。

Conclusion: 氨是远程可再生能源中心中最具成本效益的能源载体。
...
### [117] [Set-Based Control Barrier Functions and Safety Filters](https://arxiv.org/abs/2507.07805)
...
<details>
  <summary>Details</summary>
Motivation: 工业控制应用中高性能和形式化安全保证是常见需求，但传统CBF设计复杂，难以适用于大规模或数据驱动系统。

Method: 利用可达性分析和预测控制中的控制不变集，通过最小化集合缩放隐式定义CBF，支持数据驱动和高维表示。

Result: 通过高维质量-弹簧-阻尼系统和运动控制任务的仿真，以及电动驱动应用的实验验证了方法的有效性。

Conclusion: 基于集合的CBF方法简化了设计，适用于实时实现和学习近似，在安全关键控制中具有实际优势。
...
### [118] [Identifying the Smallest Adversarial Load Perturbations that Render DC-OPF Infeasible](https://arxiv.org/abs/2507.07850)
*Samuel Chevalier,William A. Wheeler*

Main category: eess.SY

TL;DR: 论文研究了使DC-OPF问题不可行的最小负载扰动，提出了基于Farkas引理的非凸优化方法，并结合生成控制策略提供上下界，最终通过优化问题逼近全局解。


<details>
  <summary>Details</summary>
Motivation: 识别最小负载扰动对电网机器学习验证、网络安全和可再生能源主导的电网运行鲁棒性具有重要意义。

Method: 应用参数化的Farkas引理处理DC-OPF方程，提出生成控制策略保证可解性，并通过优化问题逼近全局解。

Result: 在PGLib测试案例中验证方法，并与Gurobi 12.0的空间分支定界求解器结果对比。

Conclusion: 提出的方法能有效逼近全局解，为电网安全提供理论支持。
...
### [119] [Semi-supervised learning and integration of multi-sequence MR-images for carotid vessel wall and plaque segmentation](https://arxiv.org/abs/2507.07496)
...
### [120] [D-CNN and VQ-VAE Autoencoders for Compression and Denoising of Industrial X-ray Computed Tomography Images](https://arxiv.org/abs/2507.07704)
...
Method: 采用深度卷积神经网络（D-CNN）和向量量化变分自动编码器（VQ-VAE）两种架构，对砂岩样品的XCT数据进行压缩，并评估解码图像质量。
...
### [121] [Compressive Imaging Reconstruction via Tensor Decomposed Multi-Resolution Grid Encoding](https://arxiv.org/abs/2507.07707)
...
Conclusion: GridTD是一种高效、通用的压缩成像重建方法，具有理论和实际优势。
...
### [122] [Breast Ultrasound Tumor Generation via Mask Generator and Text-Guided Network:A Clinically Controllable Framework with Downstream Evaluation](https://arxiv.org/abs/2507.07721)
*Haoyu Pan,Hongxin Lin,Zetian Feng,Chuxuan Lin,Junyang Mo,Chu Zhang,Zijian Wu,Yi Wang,Qingqing Zheng*

Main category: eess.IV

TL;DR: 提出了一种基于临床可控生成框架的乳腺超声图像合成方法，解决专家标注数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 由于专家标注的乳腺超声图像数据稀缺，开发鲁棒的深度学习模型受到限制。

Method: 结合临床描述和结构掩模生成肿瘤图像，设计了语义-曲率掩模生成器，生成多样化肿瘤掩模。

Result: 在六个公共乳腺超声数据集上的定量评估显示合成图像显著提升下游诊断任务效果，视觉图灵测试证实图像真实性。
...
### [123] [MeD-3D: A Multimodal Deep Learning Framework for Precise Recurrence Prediction in Clear Cell Renal Cell Carcinoma (ccRCC)](https://arxiv.org/abs/2507.07839)
*Hasaan Maqsood,Saif Ur Rehman Khan*

Main category: eess.IV

TL;DR: 提出一种多模态深度学习框架，整合CT、MRI、病理图像、临床数据和基因组数据，以提高透明细胞肾细胞癌（ccRCC）复发的预测准确性。


<details>
  <summary>Details</summary>
Motivation: 传统单模态预测模型无法全面捕捉ccRCC的复杂性，导致预测准确性不足。本研究旨在通过多模态数据整合提升预测能力。

Method: 使用领域特定模型处理不同模态数据（如CLAM处理病理图像，MeD-3D处理CT/MRI，MLP处理临床和基因组数据），并通过早期和晚期融合策略整合特征。
...
### [124] [ArteryX: Advancing Brain Artery Feature Extraction with Vessel-Fused Networks and a Robust Validation Framework](https://arxiv.org/abs/2507.07920)
...
Conclusion: ArteryX有望成为临床工作流程中的标准化工具，促进脑血管病变的早期检测和跨队列研究。
...
### [125] [Optimal Auction Design in the Joint Advertising](https://arxiv.org/abs/2507.07418)
*Yang Li,Yuchao Ma,Qi Qi*

Main category: cs.GT

TL;DR: 本文提出了一种针对联合广告的最优机制，并在多广告位场景中提出了BundleNet方法，显著提升了平台收入。


<details>
  <summary>Details</summary>
Motivation: 现有联合广告机制未能实现最优性，因其过于关注单个广告主而忽视了广告捆绑结构。

Method: 在单广告位场景中识别最优机制；在多广告位场景中提出BundleNet，一种基于神经网络的捆绑方法。

Result: BundleNet在单广告位场景中接近理论最优，在多广告位场景中达到最先进性能，显著提升收入。

Conclusion: BundleNet方法在联合广告中实现了高效分配和收入提升，同时满足激励兼容性和个体理性。
...
### [126] [Incentive Mechanism for Mobile Crowd Sensing with Assumed Bid Cost Reverse Auction](https://arxiv.org/abs/2507.07688)
*Jowa Yangchin,Ningrinla Marchang*

Main category: cs.GT

TL;DR: 论文提出了一种名为RA-ABC的新方法，允许用户在收集数据前出价，以减少资源消耗。进一步扩展为RA-ABCDR，支持动态招募用户。实验表明，该方法在用户保留率和成本效率上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 优化移动群智感知（MCS）系统，通过改进反向拍卖机制提升用户、任务提供者和平台的效用。

Method: 提出RA-ABC方法，用户在收集数据前出价；扩展为RA-ABCDR，支持动态用户招募。通过计算用户投资回报率（ROI）调整参与行为。

Result: RA-ABCDR在用户保留率上提升54.6%，拍卖成本降低22.2%，系统性能更高效和可持续。

Conclusion: 动态用户招募显著提升系统稳定性、公平性和成本效率，RA-ABCDR是一种高效的MCS优化方法。
...
### [127] [Hybrid Advertising in the Sponsored Search](https://arxiv.org/abs/2507.07711)
*Zhen Zhang,Weian Li,Yuhan Wang,Qi Qi,Kun Huang*

Main category: cs.GT

TL;DR: 提出了一种名为“混合广告”的新模型，结合独立商店和捆绑广告，通过Hybrid Regret Network（HRegNet）优化拍卖机制，显著提升平台收入。


<details>
  <summary>Details</summary>
Motivation: 传统广告模型和联合广告模型各自吸引不同用户群体，导致点击率低。为解决这一问题并提高通用性，提出混合广告模型。

Method: 引入Hybrid Regret Network（HRegNet），设计用于优化混合广告中的拍卖机制，确保激励兼容性和个体合理性。

Result: 在合成和真实数据上的实验表明，HRegNet生成的机制显著提高了平台收入。

Conclusion: 混合广告模型和HRegNet为平台提供了一种更灵活且高效的广告解决方案。
...
### [128] [Improving the Price of Anarchy via Predictions in Parallel-Link Networks](https://arxiv.org/abs/2507.07915)
*George Christodoulou,Vasilis Christoforidis,Alkmini Sgouritsa,Ioannis Vlachos*

Main category: cs.GT

TL;DR: 研究非原子拥塞游戏中基于机器学习的预测如何优化协调机制，以最小化自私行为的影响。


<details>
  <summary>Details</summary>
Motivation: 探讨在并行链路网络中，如何利用预测信息设计协调机制，以平衡准确性和鲁棒性。

Method: 提出一种简单的输入率建议机制，分析其在准确预测和错误预测下的表现。

Result: 机制在预测准确时优化社会成本，预测错误时损失最小，且具有最优鲁棒性。

Conclusion: 该机制在单调成本函数下表现最优，并能平滑处理预测误差，达到有界鲁棒性。
...
### [129] [g2o vs. Ceres: Optimizing Scan Matching in Cartographer SLAM](https://arxiv.org/abs/2507.07142)
*Quanjie Qiu,MengCheng Lau*

Main category: cs.RO

TL;DR: 比较了g2o和Ceres在Cartographer框架中的扫描匹配性能，Ceres在速度、收敛效率和地图清晰度上表现更优，而g2o在局部障碍检测中表现更好。


<details>
  <summary>Details</summary>
Motivation: 评估g2o与Ceres在SLAM框架Cartographer中的性能差异，以优化姿态估计和地图精度。

Method: 在Cartographer中对比g2o和Ceres的性能，包括速度、收敛效率和地图清晰度。

Result: Ceres在速度、收敛效率和地图清晰度上优于g2o，但g2o在局部障碍检测中表现更好。

Conclusion: Ceres更适合整体优化任务，而g2o在特定场景（如障碍检测）中有优势。
...
### [130] [Self-Wearing Adaptive Garments via Soft Robotic Unfurling](https://arxiv.org/abs/2507.07221)
...
Result: SWAG在不同衣物配置下表现出高效的穿衣能力，为传统机器人穿衣提供了有前景的替代方案。

Conclusion: SWAG系统通过软机器人技术，显著提升了穿衣辅助的实用性和适应性。
...
### [131] [3D Steering and Localization in Pipes and Burrows using an Externally Steered Soft Growing Robot](https://arxiv.org/abs/2507.07225)
*Yimeng Qin,Jared Grinberg,William Heap,Allison M. Okamura*

Main category: cs.RO

TL;DR: 提出了一种新型可转向藤蔓机器人，用于在狭窄管道和洞穴中导航，具有高机动性和适应性。


<details>
  <summary>Details</summary>
Motivation: 现有藤蔓机器人在复杂路径和分支环境中导航能力有限，需要改进以适应更复杂的环境。

Method: 设计了一种具有外部尖端安装的藤蔓机器人，通过改变生长方向和支撑管道壁实现三维转向。

Result: 机器人实现了51.7°的最大转向角，可在半径小至2.5厘米的管道中导航，并具备实时三维定位能力。

Conclusion: 该机器人设计在复杂管道和洞穴环境中表现出色，为狭窄空间导航提供了新解决方案。
...
### [132] [LangNavBench: Evaluation of Natural Language Understanding in Semantic Navigation](https://arxiv.org/abs/2507.07299)
*Sonia Raychaudhuri,Enrico Cancelli,Tommaso Campari,Lamberto Ballan,Manolis Savva,Angel X. Chang*

Main category: cs.RO

TL;DR: LangNav是一个专注于语言理解的语义导航数据集，用于测试代理在不同详细程度的语言描述下定位目标对象的能力。LangNavBench是基于LangNav的基准测试，用于系统评估模型在属性、空间和关系线索以及类别层次结构上的表现。提出的MLFM方法在LangNav数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有语义导航数据集缺乏对语言理解的系统评估，因此需要LangNav和LangNavBench来填补这一空白。

Method: 提出了Multi-Layered Feature Map (MLFM)方法，构建可查询的多层语义地图，特别适用于小对象或涉及空间关系的指令。

Result: MLFM在LangNav数据集上优于现有的基于地图的导航基线方法。

Conclusion: LangNav和LangNavBench为语言驱动的语义导航提供了系统评估工具，MLFM方法在复杂语言描述下表现优异。
...
### [133] [Effects of Wrist-Worn Haptic Feedback on Force Accuracy and Task Speed during a Teleoperated Robotic Surgery Task](https://arxiv.org/abs/2507.07327)
*Brian B. Vuong,Josie Davidson,Sangheui Cheon,Kyujin Cho,Allison M. Okamura*

Main category: cs.RO

TL;DR: 将触觉反馈从手部转移到手腕的穿戴设备，显著降低了力应用误差，但增加了操作时间。


<details>
  <summary>Details</summary>
Motivation: 手部触觉反馈会干扰手术机器人操作杆的直接交互，因此探索手腕触觉反馈的有效性。
...
TL;DR: 论文探讨了“涌现”和“群体”缺乏共识定义的问题，提出了一个框架以区分可观测和不可观测状态，强调这些概念的主观性。
...
Method: 提出一个框架，分离可观测和不可观测状态，比较现有定义。

Result: 认为“群体”不仅由群体行为定义，还由行为生成过程决定，强调概念的主观性。

Conclusion: 支持机器人群体系统的设计和部署，区分多机器人系统与真正的群体。
...
### [135] [UniTracker: Learning Universal Whole-Body Motion Tracker for Humanoid Robots](https://arxiv.org/abs/2507.07356)
*Kangning Yin,Weishuai Zeng,Ke Fan,Zirui Wang,Qiang Zhang,Zheng Tian,Jingbo Wang,Jiangmiao Pang,Weinan Zhang*

Main category: cs.RO

TL;DR: UniTracker是一个基于CVAE的框架，用于提升人形机器人的全身控制多样性、鲁棒性和泛化能力。
...
Method: 将CVAE集成到学生策略中，显式建模人类运动的潜在多样性，利用CVAE先验保留运动特征。

Result: UniTracker在仿真和实际部署中显著优于MLP-based DAgger基线，运动质量、泛化能力和鲁棒性更优。
...
### [136] [FiDTouch: A 3D Wearable Haptic Display for the Finger Pad](https://arxiv.org/abs/2507.07661)
...
<details>
  <summary>Details</summary>
Motivation: 指尖触觉设备在虚拟现实、医疗培训和远程机器人操作等领域具有广泛应用潜力，但现有设备在提供精确触觉反馈方面存在不足。

Method: 采用微型倒置Delta机器人设计，提供接触、压力、皮肤拉伸和振动等触觉反馈，并通过两阶段用户研究评估性能。
...
TL;DR: 论文提出了一种基于共形预测的软体机器人运动学建模框架，用于量化预测不确定性，提高建模精度。
...
Method: 研究多种线性和非线性机器学习模型，提出基于分块共形预测的框架，量化预测位置不确定性。
...
Method: 提出PILOC框架，利用局部感知和通信，引入信息素逆向引导机制，并将其嵌入深度强化学习（DRL）的观察空间。
...
TL;DR: 该论文提出了一种用于自动驾驶车辆运动规划模块的时间保护机制，扩展了现有方法，确保实时性和安全性。


<details>
  <summary>Details</summary>
Motivation: 解决复杂或基于学习的软件中运动规划模块的功能安全问题，特别是在嵌入式实时环境中的在线验证需求。

Method: 引入时间保护机制，监控规划输出的时间一致性，结合约束可行性检查和基于成本的合理性评估。

Result: 原型实现表明保护模块能在实时范围内运行，并有效检测不安全轨迹。

Conclusion: 研究提供了一个模块化和可扩展的运行时轨迹验证框架，未来工作包括完善保护逻辑和硬件验证。
...
<details>
  <summary>Details</summary>
Motivation: 解决GPS拒绝环境下视觉惯性里程计（VIO）随时间累积漂移的问题，利用SCR提供无漂移的高精度绝对姿态估计。

Method: 结合基于证据学习的SCR姿态估计器和后退水平轨迹优化器，通过优化器引导相机朝向不确定性低的像素，同时使用固定滞后平滑器融合SCR和IMU数据。

Result: 仿真中，相对于固定偏航和前视基线，平移和旋转平均误差分别减少了54%/15%和40%/31%。硬件在环实验验证了框架的可行性。

Conclusion: 提出的框架在GPS拒绝环境中有效降低了定位误差，并通过实验验证了其可行性。
...
### [141] [Adaptive Gaussian Mixture Models-based Anomaly Detection for under-constrained Cable-Driven Parallel Robots](https://arxiv.org/abs/2507.07714)
...
<details>
  <summary>Details</summary>
Motivation: 在CDPRs的负载操作任务中，需在固定姿态下检测异常（如风或电缆碰撞），但传统方法依赖额外传感器。本文旨在仅用扭矩数据实现高效检测。

Method: 通过短时间校准拟合GMM模型，实时计算马氏距离并设定统计阈值触发异常标志，定期更新模型参数以适应环境变化。

Result: 在14次长时间测试中，方法实现了100%真阳性率和95.4%平均真阴性率，检测延迟1秒，优于非自适应方法。

Conclusion: 该方法无需额外传感器，对漂移和环境变化具有高鲁棒性，适用于CDPRs的实时异常检测。
...
### [142] [Implementation and Assessment of an Augmented Training Curriculum for Surgical Robotics](https://arxiv.org/abs/2507.07718)
*Alberto Rota,Ke Fan,Elena De Momi*

Main category: cs.RO

TL;DR: 开发并验证了一种触觉增强的虚拟现实手术机器人训练模拟器，通过高水平的触觉辅助算法提升学员技能，实验表明其能显著提高训练效果并促进技能迁移到无辅助的临床场景。
...
Method: 开发了一种触觉增强的虚拟现实模拟器，包含8个手术任务，通过物理引擎和触觉接口实现运动引导和障碍避免，并提供定量评分。

Result: 实验证明，引入增强的机器人辅助显著提高了训练效果，并促进了技能向无辅助临床场景的迁移。

Conclusion: 触觉增强的虚拟现实模拟器是提升手术机器人培训效果的有效工具，尤其在技能迁移方面表现突出。
...
### [143] [Distributed Surface Inspection via Operational Modal Analysis by a Swarm of Miniaturized Vibration-Sensing Robots](https://arxiv.org/abs/2507.07724)
*Thiemen Siemensma,Niels de Boer,Bahar Haghighat*

Main category: cs.RO

TL;DR: 本文研究了利用微型振动传感机器人群体在仿真环境中检测和定位结构损伤的方法，结合高斯过程估计器和模态分析，验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 传统静态传感器网络在结构监测中覆盖能力有限，机器人群体因其动态性有望显著提升结构监测效果。

Method: 使用有限元分析（Abaqus）生成结构振动数据，导入Webots仿真机器人群体动态；采用高斯过程估计器引导探索，模态分析检测损伤。
...
### [144] [On the capabilities of LLMs for classifying and segmenting time series of fruit picking motions into primitive actions](https://arxiv.org/abs/2507.07745)
*Eleni Konstantinidou,Nikolaos Kounalakis,Nikolaos Efstathopoulos,Dimitrios Papageorgiou*

Main category: cs.RO

TL;DR: 研究探讨了大型语言模型（LLMs）在水果采摘任务中分类和分割复杂动作为基本动作（如推、拉、扭等）的能力，通过三种微调方法进行比较。


<details>
  <summary>Details</summary>
Motivation: LLMs（如ChatGPT）已显著减轻认知负荷，本研究旨在探索其在学习示范（LbD）中分类和分割动作的潜力，以简化任务编码。

Method: 使用UR10e机器人采集数据集，比较三种不同的LLM微调方法，替代传统监督学习或分析方法。

Result: 未明确提及具体结果，但目标是使方法更易应用于实际场景。

Conclusion: LLMs在动作分类和分割任务中具有潜力，可能简化实际部署。
...
### [145] [IRAF-SLAM: An Illumination-Robust and Adaptive Feature-Culling Front-End for Visual SLAM in Challenging Environments](https://arxiv.org/abs/2507.07752)
...
<details>
  <summary>Details</summary>
Motivation: 现有SLAM系统在动态物体、低纹理和光照变化下性能下降，需要一种更鲁棒的前端解决方案。
...
### [146] [Collaborative Human-Robot Surgery for Mandibular Angle Split Osteotomy: Optical Tracking based Approach](https://arxiv.org/abs/2507.07794)
...
Method: 将手术任务分解为位置控制、方向控制和力控制三个子任务，利用光学跟踪系统（OTS）和机器人协作完成手术。
...
### [147] [Beyond Robustness: Learning Unknown Dynamic Load Adaptation for Quadruped Locomotion on Rough Terrain](https://arxiv.org/abs/2507.07825)
*Leixin Chang,Yuxuan Nai,Hua Chen,Liangjing Yang*

Main category: cs.RO

TL;DR: 论文提出了一种通用的负载特性建模方法，结合强化学习控制，使四足机器人能够推断负载动态并稳定负载，验证了其在真实场景中的有效性。
...
Method: 提出负载特性建模方法，结合强化学习控制技术，实现负载动态推断和稳定交互。

Result: 仿真实验表明，该方法在突然负载抵抗、负载稳定和复杂地形下的重载运动方面优于其他方法。

Conclusion: 该方法为四足机器人在未知动态负载下的应用提供了一种有效的解决方案。
...
### [148] [Perceptual Distortions and Autonomous Representation Learning in a Minimal Robotic System](https://arxiv.org/abs/2507.07845)
...
<details>
  <summary>Details</summary>
Motivation: 探索自主机器人感知失真的本质及其对表示学习的影响，以理解感知在自主导航中的作用。

Method: 使用配备距离传感器和指南针的模拟两轮机器人，在方形环境中随机探索，分析其传感器数据。

Result: 尽管感知空间存在失真，但机器人仍能学习到与物理环境相关的结构化表示。

Conclusion: 研究揭示了感知在自主导航中的重要性，为具身认知和最小代理提供了新见解。
...
### [149] [ROS Help Desk: GenAI Powered, User-Centric Framework for ROS Error Diagnosis and Debugging](https://arxiv.org/abs/2507.07846)
...
<details>
  <summary>Details</summary>
Motivation: 随着机器人系统融入日常生活，ROS的复杂架构和技术消息系统导致用户难以理解和诊断问题，增加了维护停机时间。
...
TL;DR: 提出了一种基于预测差异原则（PDP）的规则分类方法，用于区分自动紧急制动系统（AEBS）的误报和真报，以提高验证的透明度和一致性。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法中因场景参数不确定性和驾驶员干预导致的分析复杂性，以及人工标注的主观性和潜在偏差问题。

Method: 采用规则分类方法，基于预测差异原则（PDP），并结合人工标注以提高分类的透明度和一致性。

Result: 揭示了该方法的关键优势、局限性和系统要求，表明其能有效提升AEBS验证的可靠性和可重复性。

Conclusion: 该方法可补充现有实践，为更可靠的AEBS验证框架铺平道路，未来需进一步优化和扩展适用性。
...
Method: 通过数据驱动方法，仅利用关节传感器实现全身触觉感知。

Result: 在Franka机器人手臂上定位精度为8.0厘米，在Spot四足机器人上为7.2厘米，频率约2000Hz。

Conclusion: UniTac提供了一种无需额外传感器的触觉感知解决方案，适用于多种机器人平台。
...
### [152] [Audio-Visual Speech Separation via Bottleneck Iterative Network](https://arxiv.org/abs/2507.07270)
*Sidong Zhang,Shiv Shankar,Trang Nguyen,Andrea Fanelli,Madalina Fiterau*

Main category: cs.SD

TL;DR: 提出了一种名为Bottleneck Iterative Network (BIN)的轻量级迭代表示细化方法，通过融合令牌限制融合表示，提升模型性能同时避免模型规模大幅增加。


<details>
  <summary>Details</summary>
Motivation: 现有语音分离模型通常使用深度模态特定网络提取单模态特征，但存在成本过高或性能不足的问题。

Method: 采用轻量级融合块的迭代表示细化方法，通过融合令牌限制融合表示，平衡模型性能与训练成本。

Result: 在NTCD-TIMIT和LRS3+WHAM!数据集上，BIN在SI-SDRi指标上优于现有基准模型，同时训练和GPU推理时间减少50%以上。

Conclusion: BIN方法在提升语音分离性能的同时，显著降低了计算成本，具有实际应用潜力。
...
### [153] [SonicMotion: Dynamic Spatial Audio Soundscapes with Latent Diffusion Models](https://arxiv.org/abs/2507.07318)
...
Method: 提出了SonicMotion模型，包含两种变体，分别针对用户输入和声源定位精度。同时创建了一个新的模拟空间音频-字幕对数据集。

Result: 模型在语义对齐和音频质量上达到了先进水平，并能捕捉所需的空间属性。

Conclusion: SonicMotion模型在生成动态声源的空间音频方面表现出色，为未来研究提供了新工具和数据支持。
...
### [154] [VP-SelDoA: Visual-prompted Selective DoA Estimation of Target Sound via Semantic-Spatial Matching](https://arxiv.org/abs/2507.07384)
*Yu Chen,Xinyuan Qian,Hongxu Zhu,Jiadong Wang,Kainan Chen,Haizhou Li*

Main category: cs.SD

TL;DR: 论文提出了一种名为CI-AVL的新任务，通过跨实例音频-视觉定位解决现有AV-SSL方法的三大挑战，并提出了VP-SelDoA方法和VGG-SSL数据集。
...
Method: 提出VP-SelDoA方法，结合语义级模态融合和Frequency-Temporal ConMamba架构生成目标选择性掩码，并开发语义-空间匹配机制。

Result: 实验表明，该方法优于现有技术，MAE为12.04，ACC为78.23%。

Conclusion: CI-AVL任务和方法有效解决了现有挑战，提升了定位性能和泛化能力。
...
### [155] [DMF2Mel: A Dynamic Multiscale Fusion Network for EEG-Driven Mel Spectrogram Reconstruction](https://arxiv.org/abs/2507.07526)
*Cunhang Fan,Sheng Zhang,Jingjing Zhang,Enrui Liu,Xinhui Li,Minggang Zhao,Zhao Lv*

Main category: cs.SD

TL;DR: 论文提出DMF2Mel网络，通过动态多尺度融合解决脑信号解码中长序列建模的挑战，显著提升mel频谱图重建效果。


<details>
  <summary>Details</summary>
Motivation: 现有技术在脑信号解码中难以平衡长序列建模的效率与信息保留，尤其是连续想象语音的精确重建。

Method: 提出DMF2Mel网络，包含DC-FAM、HAMS-Net、SplineMap注意力和convMamba模块，分别处理特征分离、多尺度融合、全局与局部建模及长时依赖。

Result: 在SparrKULee数据集上，已知和未知受试者的mel频谱图重建Pearson系数分别提升48%和35%。

Conclusion: DMF2Mel有效解决了长序列脑信号解码的挑战，为连续想象语音重建提供了新方法。
...
### [156] [Assessing the Alignment of Audio Representations with Timbre Similarity Ratings](https://arxiv.org/abs/2507.07764)
...
<details>
  <summary>Details</summary>
Motivation: 传统音色空间方法存在可扩展性和泛化性问题，而深度学习在音频质量评估和图像相似性中表现优异，但缺乏足够的人类评分数据训练深度模型。

Method: 引入指标评估音频表示与人类音色相似性判断的对齐性，比较嵌入距离的绝对值和排名。评估了多种音频表示，包括信号处理方法和预训练模型。

Result: 基于CLAP模型和声音匹配模型的风格嵌入表现最佳，优于其他表示方法。

Conclusion: 风格嵌入在建模音色相似性方面具有潜力，为未来研究提供了方向。
...
### [157] [SecureSpeech: Prompt-based Speaker and Content Protection](https://arxiv.org/abs/2507.07799)
*Belinda Soh Hui Hui,Xiaoxiao Miao,Xin Wang*

Main category: cs.SD

TL;DR: 提出了一种基于提示的语音生成管道，实现说话者身份和内容的双重匿名化，通过生成不可链接的说话者身份和替换敏感内容，同时保持语音质量和内容保留。


<details>
  <summary>Details</summary>
Motivation: 解决语音领域因身份盗用和说话者重新识别引发的隐私问题。

Method: 1) 生成与源说话者不可链接的身份，通过描述符控制；2) 使用命名实体识别模型和大语言模型替换敏感内容；3) 通过文本到语音合成模型生成高保真隐私友好语音。

Result: 实验结果显示显著隐私保护，同时保持内容保留和音频质量。
...
### [158] [End-to-end Acoustic-linguistic Emotion and Intent Recognition Enhanced by Semi-supervised Learning](https://arxiv.org/abs/2507.07806)
*Zhao Ren,Rathi Adarshi Rammohan,Kevin Scheck,Sheng Li,Tanja Schultz*

Main category: cs.SD

TL;DR: 论文提出了一种半监督学习方法，用于语音情感和意图识别，结合了未标记和标记数据，通过多任务学习和两种半监督学习方法提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 由于手动标注语音数据成本高昂，难以训练机器学习模型，因此需要利用大量未标记数据提升识别效果。

Method: 采用端到端的声学和语言模型，结合多任务学习，比较了两种半监督学习方法（fix-match和full-match）。

Result: 实验表明，半监督学习方法显著提升了模型性能，最佳模型的后期融合在联合识别平衡指标上分别优于声学和文本基线12.3%和10.4%。

Conclusion: 半监督学习能有效利用未标记数据，提升语音情感和意图识别的性能。
...
### [159] [Re-Bottleneck: Latent Re-Structuring for Neural Audio Autoencoders](https://arxiv.org/abs/2507.07867)
...
Method: 通过引入“Re-Bottleneck”，在预训练模型的基础上，仅通过潜在空间损失训练内部瓶颈层，以注入特定结构。

Result: 实验表明，该方法能在不牺牲重建质量的情况下实现潜在通道排序、语义嵌入对齐和等变性。

Conclusion: Re-Bottleneck框架为神经音频模型提供了一种灵活高效的方式，使其能适应多样化应用需求。
...
### [160] [Edge-ASR: Towards Low-Bit Quantization of Automatic Speech Recognition Models](https://arxiv.org/abs/2507.07877)
*Chen Feng,Yicheng Lin,Shaojie Zhuo,Chenzheng Su,Ramchalam Kinattinkara Ramakrishnan,Zhaocong Yuan,Xiaopeng Zhang*

Main category: cs.SD

TL;DR: 论文研究了在资源受限的边缘设备上部署自动语音识别（ASR）模型的挑战，通过后训练量化（PTQ）方法优化模型性能和效率。


<details>
  <summary>Details</summary>
Motivation: 尽管ASR模型在准确性上取得了显著进展，但在边缘设备上部署仍面临内存、计算和功耗的限制，量化技术是解决这一问题的关键。

Method: 研究对两种领先的边缘ASR模型（Whisper和Moonshine）应用了八种最先进的PTQ方法，并在七个数据集上系统评估了量化对模型性能的影响。

Result: 结果显示，即使使用3位量化，高性能模型仍能保持准确性，尤其是在采用先进的PTQ技术时。

Conclusion: 研究为在低功耗边缘设备上优化ASR模型提供了重要见解，量化技术是实现高效部署的有效途径。
...
### [161] [LISTEN: Lightweight Industrial Sound-representable Transformer for Edge Notification](https://arxiv.org/abs/2507.07879)
*Changheon Han,Yun Seok Kang,Yuseop Sim,Martin Byung-Guk Jun,Hyung Wook Park*

Main category: cs.SD

TL;DR: LISTEN是一个轻量级的工业声音基础模型，通过知识蒸馏实现低成本边缘设备的实时运行，性能接近大型模型。


<details>
  <summary>Details</summary>
Motivation: 解决深度学习在工业声音分析中对大型标注数据和昂贵硬件的依赖问题。

Method: 采用知识蒸馏技术，开发轻量级模型LISTEN，适用于边缘设备。

Result: LISTEN在基准任务中表现接近大型模型，且能在实时环境中运行。
...
### [162] [Input Conditioned Layer Dropping in Speech Foundation Models](https://arxiv.org/abs/2507.07954)
...
Method: 通过实验和理论证明，研究标准神经网络在组合任务上的泛化能力，包括数据规模、模型规模的影响。

Result: 发现扩大数据和模型规模可实现组合泛化，且多层感知器能以线性神经元数量近似组合任务族。

Conclusion: 成功的组合泛化与隐藏激活中任务成分的线性解码相关，此指标可用于解释文本到图像生成模型的组合失败。
...
### [164] [Attentions Under the Microscope: A Comparative Study of Resource Utilization for Variants of Self-Attention](https://arxiv.org/abs/2507.07247)
...
### [165] [Growing Transformers: Modular Composition and Layer-wise Expansion on a Frozen Substrate](https://arxiv.org/abs/2507.07129)
*A. Bochkov*

Main category: cs.LG

TL;DR: 论文提出了一种基于非可训练确定性输入嵌入的模块化语言模型构建方法，支持模型的无缝组合和逐层增长，提高了资源效率和灵活性。


<details>
  <summary>Details</summary>
Motivation: 传统的大语言模型（LLM）训练方法资源密集且缺乏灵活性，本文探索了一种替代的、构造性的模型开发方法。

Method: 利用固定的输入嵌入作为通用接口，实现专家模型的模块化组合（通过平均输出logits）和逐层增长的训练方法。

Result: 组合后的MoE模型在MMLU等推理基准上表现优于单个专家模型；逐层训练方法显示出稳定的收敛性和深度与复杂推理能力的相关性。

Conclusion: 研究提出了一种从整体优化转向模块化和增量构建的AI开发新范式，为资源高效扩展和持续学习开辟了新途径。
...
### [166] [FACap: A Large-scale Fashion Dataset for Fine-grained Composed Image Retrieval](https://arxiv.org/abs/2507.07135)
*François Gardères,Shizhe Chen,Camille-Sovanneary Gauthier,Jean Ponce*

Main category: cs.LG

TL;DR: 论文提出FACap数据集和FashionBLIP-2模型，用于解决时尚领域的组合图像检索任务，通过自动标注和多头查询匹配提升性能。


<details>
  <summary>Details</summary>
Motivation: 时尚领域的组合图像检索任务面临词汇丰富多样和缺乏大规模标注数据的挑战，需要针对性的解决方案。

Method: 利用网络来源的时尚图像和两阶段标注流程生成修改文本，并基于BLIP-2模型微调，加入轻量级适配器和多头查询匹配。

Result: FashionBLIP-2在Fashion IQ和enhFashionIQ基准测试中表现显著提升，尤其在细粒度修改文本检索上。

Conclusion: FACap数据集和FashionBLIP-2模型为时尚领域的组合图像检索提供了高效解决方案，适用于电商等高需求场景。
...
### [167] [Automating Evaluation of Diffusion Model Unlearning with (Vision-) Language Model World Knowledge](https://arxiv.org/abs/2507.07137)
...
Method: 开发了autoeval-dmun工具，利用（视觉）语言模型评估遗忘效果，提取结构化知识以识别可能受损的邻近概念，并通过对抗提示绕过遗忘。

Result: 语言模型能够有效排序邻近概念（与遗忘损害相关），并通过合成对抗提示成功绕过遗忘。

Conclusion: autoeval-dmun为评估扩散模型遗忘效果提供了自动化解决方案，并展示了语言模型在语义分析和对抗提示中的潜力。
...
### [168] [Bias-Aware Mislabeling Detection via Decoupled Confident Learning](https://arxiv.org/abs/2507.07216)
...
### [169] [GNNs Meet Sequence Models Along the Shortest-Path: an Expressive Method for Link Prediction](https://arxiv.org/abs/2507.07138)
*Francesco Ferrini,Veronica Lachi,Antonio Longa,Bruno Lepri,Andrea Passerini*

Main category: cs.LG

TL;DR: SP4LP是一种结合GNN和最短路径序列建模的新框架，用于高效捕捉多跳依赖关系，提升链接预测性能。
...
Method: SP4LP先通过GNN计算节点表示，再提取节点对的最短路径并用序列模型处理，以捕捉多跳关系。

Result: SP4LP在链接预测基准测试中达到最先进性能，理论证明其表达能力优于标准GNN和现有结构特征方法。
...
### [170] [Exploring Sparse Adapters for Scalable Merging of Parameter Efficient Experts](https://arxiv.org/abs/2507.07140)
...
Result: 稀疏适配器在多任务合并后表现出优于LoRA和全微调的分布内性能，但保持强泛化性能仍具挑战。
...
### [171] [Str-GCL: Structural Commonsense Driven Graph Contrastive Learning](https://arxiv.org/abs/2507.07141)
...
<details>
  <summary>Details</summary>
Motivation: 当前图对比学习方法主要关注隐式语义关系，忽略了图结构和属性中的结构常识，导致表示学习效果受限。

Method: Str-GCL利用一阶逻辑规则表示结构常识，并通过拓扑和属性规则在不改变原图的情况下，结合表示对齐机制指导编码器。
...
### [172] [Understanding Malware Propagation Dynamics through Scientific Machine Learning](https://arxiv.org/abs/2507.07143)
*Karthik Pappu,Prathamesh Dinesh Joshi,Raj Abhijit Dandekar,Rajat Dandekar,Sreedath Panat*

Main category: cs.LG

TL;DR: 论文提出了一种结合科学机器学习的混合方法，用于更准确地建模恶意软件传播，并通过实验验证其优于传统和纯神经方法。


<details>
  <summary>Details</summary>
Motivation: 准确建模恶意软件传播对设计有效的网络安全防御至关重要，尤其是针对实时演化的自适应威胁。传统模型和纯神经方法难以捕捉真实网络中的非线性反馈机制。

Method: 评估了三种方法：经典ODE、UDE和神经ODE，并引入符号恢复方法将学习的神经反馈转化为显式数学表达式。
...
### [173] [CCQ: Convolutional Code for Extreme Low-bit Quantization in LLMs](https://arxiv.org/abs/2507.07145)
...
TL;DR: CCQ是一种针对大语言模型（LLMs）的2.0-2.75位量化方法，通过硬件感知的位移编码和解码技术，显著降低模型大小和推理成本，同时保持高精度。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型规模的快速扩大，推理成本急剧上升，部署难度增加。现有量化方法在低于3位时面临精度和效率的严重下降，亟需一种高效的极低位量化方案。

Method: CCQ结合卷积码、混合编码和代码簇技术，构建无查找表的编码空间，实现代码本与权重向量的线性映射，优化推理性能。同时借鉴向量量化的数据映射概念，减少极低位条件下的性能损失。

Result: 实验表明，CCQ在多个基准测试中表现优异。例如，将DeepSeek-V3压缩至184GB，ERNIE-4.5-300B-A47B压缩至89GB，实现单GPU部署。

Conclusion: CCQ通过创新的量化方法，成功解决了极低位量化中的精度和效率问题，为大语言模型的低成本部署提供了可行方案。
...
### [174] [An attention-aware GNN-based input defender against multi-turn jailbreak on LLMs](https://arxiv.org/abs/2507.07146)
*Zixuan Huang,Kecheng Huang,Lihao Yin,Bowei He,Huiling Zhen,Mingxuan Yuan,Zili Shao*

Main category: cs.LG

TL;DR: G-Guard是一种基于注意力感知的图神经网络输入分类器，用于防御大语言模型（LLM）的多轮越狱攻击。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM经过严格的安全训练和微调，但仍易受多轮越狱攻击的威胁，这些攻击逐渐升级对话，难以检测和缓解。

Method: G-Guard通过构建实体图捕获多轮查询中的有害关键词关系，并引入注意力感知增强机制，检索最相似的单轮查询作为标记节点。

Result: 评估结果显示，G-Guard在所有数据集和评估指标上均优于基线方法。

Conclusion: G-Guard为防御多轮越狱攻击提供了有效的解决方案，显著提升了LLM的安全性。
...
### [175] [Weighted Multi-Prompt Learning with Description-free Large Language Model Distillation](https://arxiv.org/abs/2507.07147)
*Sua Lee,Kyubum Shin,Jung Ho Park*

Main category: cs.LG

TL;DR: 论文提出了一种名为DeMul的新方法，通过直接从LLM中提取知识到提示中，避免了传统方法中提取描述的不可靠性问题，并在多提示设置中展示了提示加权的重要性。


<details>
  <summary>Details</summary>
Motivation: 现有方法通过从LLM提取文本描述来增强VLM的提示，但存在高变异性和低可靠性的问题。

Method: 提出Description-free Multi-prompt Learning (DeMul)，直接从LLM中蒸馏知识到提示中，避免提取描述，并采用多提示加权机制。

Result: 在11个识别数据集上取得了优越性能。

Conclusion: DeMul方法通过消除描述提取过程，提高了提示的语义丰富性和可靠性，同时多提示加权进一步优化了性能。
...
### [176] [Bridging the Last Mile of Prediction: Enhancing Time Series Forecasting with Conditional Guided Flow Matching](https://arxiv.org/abs/2507.07192)
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在时间序列预测中存在局限性，如固定的源分布和有限的采样路径，而流匹配技术具有更快生成和更高灵活性的优势。

Method: CGFM扩展了流匹配，通过整合辅助模型的输出，利用其预测误差，并结合历史数据作为条件和指导，构建双面条件概率路径。

Result: 实验表明，CGFM在时间序列预测任务中表现优于现有最先进模型。

Conclusion: CGFM通过利用辅助模型的误差和扩展概率路径空间，显著提升了预测效果。
...
### [177] [Combining Pre-Trained Models for Enhanced Feature Representation in Reinforcement Learning](https://arxiv.org/abs/2507.07197)
*Elia Piccoli,Malio Li,Giacomo Carfì,Vincenzo Lomonaco,Davide Bacciu*

Main category: cs.LG

TL;DR: 本文提出了一种名为Weight Sharing Attention (WSA)的新架构，用于结合多个预训练模型的嵌入信息，以丰富强化学习中的状态表示，平衡效率与性能。


<details>
  <summary>Details</summary>
Motivation: 强化学习（RL）通常缺乏对世界的先验知识，而预训练模型能提供丰富的潜在表示。如何有效结合这些信息以提升RL性能是一个未充分研究的问题。
...
### [178] [Real-Time Decorrelation-Based Anomaly Detection for Multivariate Time Series](https://arxiv.org/abs/2507.07559)
*Amirhossein Sadough,Mahyar Shahsavari,Mark Wijtvliet,Marcel van Gerven*

Main category: cs.LG

TL;DR: DAD是一种基于在线去相关学习的实时异常检测方法，适用于高维多变量时间序列，具有高效性和低内存需求。


<details>
  <summary>Details</summary>
Motivation: 实时异常检测在工业物联网等领域需求激增，需要处理高维流数据且无需存储历史数据的方法。

Method: DAD通过单次动态学习数据的相关结构，实现高效检测，并提出超参数调优策略。

Result: 实验表明，DAD在多种异常类型上表现优于现有方法，尤其适合高维实时数据流。

Conclusion: DAD在检测效果与计算效率间取得平衡，为实时内存受限的异常检测设定了新标准。
...
### [179] [BEAVER: Building Environments with Assessable Variation for Evaluating Multi-Objective Reinforcement Learning](https://arxiv.org/abs/2507.07769)
*Ruohong Liu,Jack Umenberger,Yize Chen*

Main category: cs.LG

TL;DR: 论文探讨了强化学习在建筑能源管理中的泛化能力，提出了多目标上下文强化学习问题，并构建了评估基准。


<details>
  <summary>Details</summary>
Motivation: 解决强化学习方法在建筑能源管理中跨环境和多目标场景下的泛化能力不足问题。

Method: 形式化定义了跨环境多目标任务的泛化空间，提出多目标上下文强化学习问题，并构建了评估基准。

Result: 现有方法能在冲突目标间取得平衡，但在某些环境变化下性能下降，需引入动态相关上下文信息。

Conclusion: 强化学习在建筑能源管理中需进一步结合动态上下文信息以提高泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 状态轨迹在训练中可能显著变形，导致状态空间数据覆盖不足，影响训练算法性能和模型鲁棒性。

Method: 引入基于数据分布的正则化惩罚，确保状态空间数据分布均匀，适用于局部仿射状态空间模型。
...
### [181] [Efficient Parametric SVD of Koopman Operator for Stochastic Dynamical Systems](https://arxiv.org/abs/2507.07222)
...
Method: 基于低秩近似思想，提出了一种可扩展且概念简单的方法，用于学习Koopman算子的前k个奇异函数。
...
TL;DR: MUSE提出了一种基于信息论的方法，通过Jensen-Shannon Divergence聚合多个LLM的输出，以提升不确定性估计的可靠性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在输入不一致时表现出不确定性，需在高风险场景中量化这种不确定性。现有研究多关注单一模型，忽视了模型多样性的潜力。
...
### [183] [Towards Robust Surrogate Models: Benchmarking Machine Learning Approaches to Expediting Phase Field Simulations of Brittle Fracture](https://arxiv.org/abs/2507.07237)
...
<details>
  <summary>Details</summary>
Motivation: 当前机器学习在断裂建模中的研究多基于过于简单的基准，未能反映实际断裂过程的复杂性，因此需要一个更具挑战性的数据集。

Method: 构建了一个包含6,000次模拟的数据集，涵盖三种能量分解方法、两种边界条件和1,000种随机初始裂纹配置。同时评估了PINN、FNO和UNet等基线模型。
...
<details>
  <summary>Details</summary>
Motivation: 随着机器学习模型在物联网边缘环境中的广泛部署，分布式深度学习引入了新的安全风险。中间特征的泄露可能被攻击者利用，威胁整个系统。

Method: 提出一种利用中间特征的攻击策略，包括通过统计分析重构张量形状，并设计代理模型架构以有效蒸馏特征。
...
### [185] [Robust Multimodal Learning Framework For Intake Gesture Detection Using Contactless Radar and Wearable IMU Sensors](https://arxiv.org/abs/2507.07261)
...
<details>
  <summary>Details</summary>
Motivation: 通过多模态学习提升食物摄入手势检测性能，并解决多模态学习中缺失模态时的鲁棒性问题。

Method: 提出MM-TCN-CMA框架，整合IMU和雷达数据，采用跨模态注意力机制，并在新数据集上进行实验。

Result: 框架在完整模态下比单模态模型提升4.3%（雷达）和5.2%（IMU），在缺失模态下仍保持性能提升。

Conclusion: 首次证明了IMU和雷达数据融合的鲁棒多模态学习框架在食物摄入手势检测中的有效性。
...
### [186] [Beyond the ATE: Interpretable Modelling of Treatment Effects over Dose and Time](https://arxiv.org/abs/2507.07271)
*Julianna Piskorz,Krzysztof Kacprzyk,Mihaela van der Schaar*

Main category: cs.LG

TL;DR: 提出了一种动态建模治疗效果的框架，解决了ATE在剂量和时间变化下的局限性，支持临床决策。


<details>
  <summary>Details</summary>
Motivation: ATE在评估治疗效果时无法捕捉剂量和时间变化的动态效应，尤其在医疗领域需要更细致的分析。

Method: 采用SemanticODE框架，将治疗效果建模为剂量和时间的平滑曲面，支持临床相关属性的提取和编辑。

Result: 方法能够准确、可解释且可编辑地建模治疗动态，支持因果分析和实际决策。

Conclusion: 该框架为高风险领域提供了透明、稳健且可验证的治疗效果动态分析工具。
...
### [187] [TRIP: A Nonparametric Test to Diagnose Biased Feature Importance Scores](https://arxiv.org/abs/2507.07276)
...
<details>
  <summary>Details</summary>
Motivation: 理解特征对预测的贡献是机器学习模型的重要组成部分，但置换特征重要性在依赖特征存在时可能误导。

Method: 开发了TRIP测试，通过最小假设检测不可靠的置换特征重要性分数，并扩展至高维设置。

Result: 在模拟数据和应用中，TRIP能可靠检测置换特征重要性的不可靠性。

Conclusion: TRIP为依赖特征下的特征重要性评估提供了可靠工具。
...
### [188] [Natural Evolutionary Search meets Probabilistic Numerics](https://arxiv.org/abs/2507.07288)
*Pierre Osselin,Masaki Adachi,Xiaowen Dong,Michael A. Osborne*

Main category: cs.LG

TL;DR: ProbNES算法通过贝叶斯积分增强NES框架，显著提升样本效率，在多种任务中表现优于非概率方法和全局高效方法。
...
### [189] [Estimating Dataset Dimension via Singular Metrics under the Manifold Hypothesis: Application to Inverse Problems](https://arxiv.org/abs/2507.07291)
*Paola Causin,Alessio Marta*

Main category: cs.LG

TL;DR: 提出了一种基于变分自编码器（VAE）和黎曼几何的框架，用于估计高维数据的本征维度（ID）、构建局部坐标以及学习空间映射，并应用于生物医学成像中的逆问题。


<details>
  <summary>Details</summary>
Motivation: 高维数据通常具有低维几何结构（流形假设），但完全利用这一特性需要解决本征维度估计、局部坐标构建和空间映射学习三个关键任务。

Method: 使用混合变分自编码器（VAE）和黎曼几何工具，通过分析VAE解码器拉回度量的数值秩来估计本征维度，并构建局部坐标图集。

Result: 该方法能准确参数化流形并提升逆问题的解决效果，特别是在生物医学成像中。同时，本征维度可作为模型容量的有效代理指标。

Conclusion: 提出的框架为高维数据的低维结构建模提供了有效工具，并在逆问题和模型修剪中展示了实用价值。
...
### [190] [Discretization-independent multifidelity operator learning for partial differential equations](https://arxiv.org/abs/2507.07292)
...
<details>
  <summary>Details</summary>
Motivation: 研究算子学习模型中理论公式与实际实现之间的关系，特别是离散独立性如何支持多保真度学习。

Method: 开发了一种离散独立的编码-近似-重构模型，通过神经表示学习基函数，并验证其在局部和非局部PDE问题中的表现。

Result: 多保真度训练显著提高了准确性和计算效率，并增强了离散独立性。

Conclusion: 该模型首次全面研究了离散独立性如何实现稳健高效的多保真度算子学习，并通过实验验证了其优势。
...
### [191] [AdeptHEQ-FL: Adaptive Homomorphic Encryption for Federated Learning of Hybrid Classical-Quantum Models with Dynamic Layer Sparing](https://arxiv.org/abs/2507.07316)
*Md Abrar Jahin,Taufikur Rahman Fuad,M. F. Mridha,Nafiz Fahad,Md. Jakir Hossen*

Main category: cs.LG

TL;DR: AdeptHEQ-FL是一个结合经典与量子计算的联邦学习框架，通过混合架构、隐私保护聚合和动态层冻结，提升性能并降低通信开销。


<details>
  <summary>Details</summary>
Motivation: 解决联邦学习在非独立同分布环境下的性能、隐私和通信效率的平衡问题。

Method: 采用混合CNN-PQC架构、差分隐私验证精度加权聚合、选择性同态加密和动态层冻结。

Result: 在CIFAR-10数据集上，比Standard-FedQNN和FHE-FedQNN分别提升约25.43%和14.17%的准确率，同时减少通信开销。

Conclusion: AdeptHEQ-FL在隐私保护、性能和资源效率方面表现出色，适用于实际联邦学习场景。
...
### [192] [Optimizing Communication and Device Clustering for Clustered Federated Learning with Differential Privacy](https://arxiv.org/abs/2507.07320)
...
<details>
  <summary>Details</summary>
Motivation: 解决在异构基站和用户非独立同分布数据下，联邦学习的资源分配和隐私保护问题，同时减少通信开销。

Method: 采用差分隐私技术，提出DPVD-MARL算法，动态优化资源块分配、用户调度和隐私噪声，以最小化训练损失。

Result: 仿真结果显示，DPVD-MARL比独立Q学习的收敛速度提升20%，累计奖励提高15%。

Conclusion: DPVD-MARL算法在集群联邦学习中有效提升了性能和收敛速度，同时兼顾了隐私保护。
...
TL;DR: Q-chunking是一种通过动作分块技术改进离线到在线强化学习的方法，解决了稀疏奖励任务中的探索和样本效率问题。
...
Method: Q-chunking将动作分块技术应用于基于时间差分（TD）的强化学习方法，直接在分块动作空间中运行RL，利用离线数据中的时间一致性行为进行更有效的探索。

Result: 实验表明，Q-chunking在稀疏奖励的长期任务中表现优异，优于现有最佳方法。
...
### [194] [Optimizing Model Splitting and Device Task Assignment for Deceptive Signal Assisted Private Multi-hop Split Learning](https://arxiv.org/abs/2507.07323)
*Dongyu Wei,Xiaoren Xu,Yuchen Liu,H. Vincent Poor,Mingzhe Chen*

Main category: cs.LG

TL;DR: 论文研究了欺骗信号辅助的私有分割学习，提出了一种优化模型以减少信息泄露，并采用深度强化学习框架解决设备选择和模型分配问题。
...
Method: 提出基于软行动者-批评家深度强化学习的框架（ICM-CA），结合内在好奇心模块和交叉注意力，优化设备选择和模型分配。

Result: 仿真结果显示，与传统SAC算法相比，收敛速度提升3倍，信息泄露减少13%。

Conclusion: 该方法有效减少信息泄露并提升训练效率。
...
### [195] [Bridging the Plausibility-Validity Gap by Fine-Tuning a Reasoning-Enhanced LLM for Chemical Synthesis and Discovery](https://arxiv.org/abs/2507.07328)
*Malikussaid,Hilal Hudan Nuha*

Main category: cs.LG

TL;DR: 论文提出了一种方法，通过微调LLMs（如Magistral Small模型）和创建双领域数据集，解决LLMs在化学领域生成科学上合理但事实错误信息的问题。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs在专业领域（如化学）中存在的“合理性-有效性差距”问题，提升其生成内容的科学准确性。

Method: 使用LoRA微调Magistral Small模型，并构建标准化的双领域数据集（涵盖分子性质和化学反应）。

Result: 微调后的模型在格式、化学有效性及合成路线可行性上显著优于基线模型，但在立体化学等方面仍有局限。
...
### [196] [Leveraging Manifold Embeddings for Enhanced Graph Transformer Representations and Learning](https://arxiv.org/abs/2507.07335)
*Ankit Jyothish,Ali Jannesari*

Main category: cs.LG

TL;DR: 论文提出了一种轻量级的黎曼混合专家层，将节点投影到多种流形（球形、平坦、双曲）以匹配局部结构，提升了图变换器的准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统图变换器将所有节点嵌入单一欧几里得空间，忽略了异质拓扑结构，限制了模型的表达能力。

Method: 在现有图变换器前添加黎曼混合专家层，动态路由节点到最适合的流形空间（球形、平坦或双曲）。
...
### [197] [Zero-Shot Context Generalization in Reinforcement Learning from Few Training Contexts](https://arxiv.org/abs/2507.07348)
...
Method: 通过引入CEBE和CSE方法，利用上下文参数中的规律性，提升单一上下文训练下的泛化能力。

Result: 理论和实验证明，CEBE能近似多上下文训练的Q函数，CSE在确定性控制环境中表现优异。

Conclusion: CEBE和CSE方法有效提升了DRL的泛化能力，尤其在数据受限的实际应用中具有潜力。
...
### [198] [Learning from positive and unlabeled examples -Finite size sample bounds](https://arxiv.org/abs/2507.07354)
...
<details>
  <summary>Details</summary>
Motivation: PU学习在许多实际应用中很重要，但现有工作通常依赖于类先验已知或正标记数据分布受限的简化假设。本文旨在放宽这些假设，提供更通用的理论分析。

Method: 通过理论分析，研究了PU学习在不同设置下的统计复杂性，不假设类先验已知。

Result: 证明了正标记和未标记样本所需样本量的上下界。

Conclusion: 本文为PU学习提供了更通用的理论框架，放宽了类先验已知的限制，为实际应用提供了理论支持。
...
### [199] [Goal-Oriented Sequential Bayesian Experimental Design for Causal Learning](https://arxiv.org/abs/2507.07359)
...
### [200] [Atherosclerosis through Hierarchical Explainable Neural Network Analysis](https://arxiv.org/abs/2507.07373)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在疾病分类中缺乏对队列特征的全面理解，且未整合患者间的共享致病依赖性，因此需要一种新方法来提升分类性能。

Method: 开发了ATHENA框架，通过分层网络表示和集成模态学习，优化患者特异性分子指纹，并与队列模式保持一致。

Result: 在391名患者的数据集上，ATHENA显著提升了分类性能（AUC提升13%，F1分数提升20%）。
...
### [201] [Bradley-Terry and Multi-Objective Reward Modeling Are Complementary](https://arxiv.org/abs/2507.07375)
...
### [202] [GRIT: Graph Transformer For Internal Ice Layer Thickness Prediction](https://arxiv.org/abs/2507.07388)
*Zesheng Liu,Maryam Rahnemoonfar*

Main category: cs.LG

TL;DR: GRIT是一种基于图变换器的冰层厚度预测方法，通过结合几何图学习和注意力机制，显著降低了预测误差。


<details>
  <summary>Details</summary>
Motivation: 提高对雷达图像中冰层厚度和变化的理解，以监测积雪、评估冰动态过程并减少气候模型的不确定性。
...
### [203] [ST-GRIT: Spatio-Temporal Graph Transformer For Internal Ice Layer Thickness Prediction](https://arxiv.org/abs/2507.07389)
*Zesheng Liu,Maryam Rahnemoonfar*

Main category: cs.LG

TL;DR: ST-GRIT是一种时空图变换器，用于从雷达图像中提取冰层厚度信息，通过时空注意力机制显著优于现有方法。
...
Result: 在格陵兰冰盖雷达数据上，ST-GRIT实现了更低的均方根误差，优于现有方法。
...
### [204] [Learning Collective Variables from Time-lagged Generation](https://arxiv.org/abs/2507.07390)
...
<details>
  <summary>Details</summary>
Motivation: 由于分子动力学模拟难以直接观测罕见事件（如状态转换），现有机器学习方法在CV发现中未能充分编码动态细节。

Method: TLC通过建模时间滞后的条件分布而非静态Boltzmann分布，学习CVs以捕捉慢动态行为。

Result: 在Alanine Dipeptide系统中，TLC在SMD和OPES任务中表现优于现有MLCV方法。

Conclusion: TLC框架能更准确地捕捉动态行为，为增强采样提供了更有效的CVs。
...
### [205] [Generalized Tree Edit Distance (GTED): A Faithful Evaluation Metric for Statement Autoformalization](https://arxiv.org/abs/2507.07399)
*Yuntian Liu,Tao Zhu,Xiaoyang Liu,Yu Chen,Zhaoxuan Liu,Qingfeng Guo,Jiashuo Zhang,Kangjie Bao,Tao Luo*

Main category: cs.LG

TL;DR: 论文提出了一种名为GTED的新型评估框架，用于自动将自然语言语句转化为形式化语言，并通过标准化和树形结构转换提升语义相似度评估的准确性。
...
### [206] [HGMP:Heterogeneous Graph Multi-Task Prompt Learning](https://arxiv.org/abs/2507.07405)
...
Conclusion: HGMP通过多任务提示和对比预训练策略，有效解决了预训练与下游任务的不匹配问题，提升了异构图任务的性能。
...
### [207] [Neural networks leverage nominally quantum and post-quantum representations](https://arxiv.org/abs/2507.07432)
*Paul M. Riechers,Thomas J. Elliott,Adam S. Shai*

Main category: cs.LG

TL;DR: 深度神经网络通过预测下一个标记的训练，能够发现并表示对训练数据的低维生成模型的信念，类似于在推理过程中进行贝叶斯更新。


<details>
  <summary>Details</summary>
Motivation: 探索神经网络如何通过训练自然地表示对数据的潜在生成模型的信念，并揭示其与经典电路的差异。

Method: 使用深度神经网络（如Transformer和RNN）进行下一个标记的预测训练，分析其内在的表示能力。

Result: 神经网络能够发现并表示低维生成模型的信念，且这种表示能力不受架构限制，而经典电路无法实现。

Conclusion: 神经网络在训练中自然地形成对数据的潜在模型的信念，这种能力是经典电路所不具备的。
...
### [208] [General purpose models for the chemical sciences](https://arxiv.org/abs/2507.07456)
...
TL;DR: 综述讨论了通用模型（GPMs）在化学科学中的应用潜力，尽管数据多样且模糊，但这些模型能灵活处理小数据集。


<details>
  <summary>Details</summary>
Motivation: 化学科学面临数据多样、模糊且规模小的挑战，传统机器学习难以完全解决，GPMs提供了新的可能性。
...
### [209] [Resolving Token-Space Gradient Conflicts: Token Space Manipulation for Transformer-Based Multi-Task Learning](https://arxiv.org/abs/2507.07485)
...
Method: DTME-MTL在令牌空间中动态识别梯度冲突，并基于冲突类型应用自适应解决方案，避免直接转换共享参数。
...
### [210] [Uncertainty Quantification for Motor Imagery BCI -- Machine Learning vs. Deep Learning](https://arxiv.org/abs/2507.07511)
...
<details>
  <summary>Details</summary>
Motivation: 研究BCI分类器在不确定性量化方面的能力，以提高分类的可靠性和准确性。

Method: 比较了CSP-LDA、MDRM、深度集成、直接不确定性量化及标准CNN在不确定性量化方面的表现。

Result: CSP-LDA和MDRM-T提供最佳不确定性估计，深度集成和标准CNN分类效果最好。所有模型能区分难易样本。

Conclusion: 通过拒绝模糊样本，可以提高BCI分类的准确性，CSP-LDA和MDRM-T在不确定性量化方面表现优异。
...
### [211] [Neural Concept Verifier: Scaling Prover-Verifier Games via Concept Encodings](https://arxiv.org/abs/2507.07532)
*Berkant Turan,Suhrab Asadulla,David Steinmann,Wolfgang Stammer,Sebastian Pokutta*

Main category: cs.LG

TL;DR: NCV结合PVGs和CBMs，提出了一种可解释的非线性分类框架，适用于高维数据，性能优于现有基线。


<details>
  <summary>Details</summary>
Motivation: PVGs在非线性分类模型中具有验证潜力，但未应用于高维图像；CBMs能解释高维数据，但依赖低容量线性预测器。

Method: NCV通过概念编码将PVGs与CBMs结合，使用非线性预测器进行决策。

Result: NCV在高维复杂数据集上优于CBM和像素基PVG分类器，并能减少捷径行为。
...
<details>
  <summary>Details</summary>
Motivation: 现有上下文感知低秩近似方法因依赖经典公式（涉及显式Gram矩阵计算和逆运算）而存在数值不稳定性，影响近似质量或导致数值奇异矩阵。

Method: 提出了一种无逆正则化框架，完全基于稳定分解，解决了GPU内存不足、输入激活矩阵接近奇异或数据不足时的挑战。

Result: 方法在三种挑战性场景下均表现稳定，证明了在数据不足时收敛到期望近似并推导了显式误差界。

Conclusion: 新方法克服了现有技术的数值缺陷，适用于多种复杂场景，提升了上下文感知低秩近似的稳定性和可靠性。
...
### [213] [CHOMET: Conditional Handovers via Meta-Learning](https://arxiv.org/abs/2507.07581)
...
Method: 采用O-RAN范式结合元学习，优化CHO的资源分配和信令开销管理。

Result: 在动态信号条件下，性能比3GPP基准至少提升180%。

Conclusion: 该框架为CHO优化提供了高效解决方案，显著提升了网络性能。
...
### [215] [Stress Monitoring in Healthcare: An Ensemble Machine Learning Framework Using Wearable Sensor Data](https://arxiv.org/abs/2507.07589)
*Arpana Sinhal,Anay Sinhal,Amit Sinhal*

Main category: cs.LG

TL;DR: 本研究通过多模态数据集和机器学习方法，解决了医护人员压力监测中的数据集不足和方法局限性问题，提出了可部署的压力监测系统。


<details>
  <summary>Details</summary>
Motivation: 医护人员（尤其是护士）面临较高的职业压力，COVID-19疫情加剧了这一问题。现有研究缺乏全面数据集和鲁棒的分析框架。

Method: 引入多模态数据集（生理信号、皮肤电活动、心率和皮肤温度），使用SMOTE处理类别不平衡，评估并组合多种机器学习模型（如随机森林、XGBoost和MLP）为堆叠分类器。

Result: 通过公开数据集和可重复分析流程，提出了可部署的压力监测系统，为医护人员心理健康提供支持。
...
### [216] [Synthetic MC via Biological Transmitters: Therapeutic Modulation of the Gut-Brain Axis](https://arxiv.org/abs/2507.07604)
*Sebastian Lotter,Elisabeth Mohr,Andrina Rutsch,Lukas Brand,Francesca Ronchi,Laura Díaz-Marugán*

Main category: cs.LG

TL;DR: 本文提出了一种基于肠道-脑轴（GBA）间接调制分子信号的合成分子通信（SMC）系统，利用个人健康数据和机器学习模型设计更有效的治疗方法。


<details>
  <summary>Details</summary>
Motivation: 现有GBA调制治疗方法标准化且效果有限，缺乏对分子信号通路的了解，因此需要更精准和个性化的方法。

Method: 通过机器学习模型验证GBA调制的理论要求，并利用有限数据识别有效的调制通路。

Result: 模型在多个数据集上表现出色，能准确识别GBA的不同调制因子。

Conclusion: 该方法为个性化GBA调制治疗提供了可行方案，并识别了关键调制通路。
...
### [217] [Sparse Self-Federated Learning for Energy Efficient Cooperative Intelligence in Society 5.0](https://arxiv.org/abs/2507.07613)
...
<details>
  <summary>Details</summary>
Motivation: 传统联邦学习在满足Society 5.0的可持续性需求上存在不足，尤其是在资源受限的物联网设备中，其高能耗和带宽需求与绿色AI原则冲突。

Method: 提出SParSeFuL方法，结合聚合计算实现自组织，并通过神经网络稀疏化降低资源消耗。

Result: SParSeFuL显著减少了能源和带宽需求，提升了联邦学习的可持续性。

Conclusion: SParSeFuL为资源受限的物联网设备提供了一种高效、可持续的联邦学习解决方案。
...
### [218] [Sparse Causal Discovery with Generative Intervention for Unsupervised Graph Domain Adaptation](https://arxiv.org/abs/2507.07621)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法因因果-虚假特征纠缠和全局对齐策略失败导致性能不佳，需要更有效的方法。

Method: SLOGAN通过稀疏因果图结构和生成干预机制分离因果特征，并通过动态校准策略优化伪标签。

Result: 在多个真实数据集上，SLOGAN显著优于现有基线方法。

Conclusion: SLOGAN通过因果特征分离和动态干预，有效解决了无监督图域自适应中的关键问题。
...
TL;DR: TransformEEG是一种混合卷积-Transformer模型，用于通过EEG数据检测帕金森病，具有更高的泛化能力和准确性。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习模型在EEG数据上泛化能力差，受高个体间变异性影响，需要开发更适合EEG数据的新架构。

Method: 提出TransformEEG，结合深度卷积分词器和Transformer编码器，生成通道特定特征并优化自注意力层。

Result: 在四个公开数据集上，TransformEEG的中位数平衡准确率为78.45%，结合数据增强后提升至80.10%，变异性显著降低。

Conclusion: TransformEEG在帕金森病检测中表现更一致、可靠，优于其他模型。
...
### [220] [HLF-FSL. A Decentralized Federated Split Learning Solution for IoT on Hyperledger Fabric](https://arxiv.org/abs/2507.07637)
...
Method: 采用FSL与HLF结合的去中心化架构，利用HLF的链码协调模型分割执行和点对点聚合，无需中心协调器，并通过HLF的瞬态字段和私有数据集合保护数据和模型激活。

Result: 在CIFAR-10和MNIST基准测试中，HLF-FSL达到与集中式FSL相当的精度，同时减少每轮训练时间，性能和扩展性测试显示区块链开销最小且精度保持。

Conclusion: HLF-FSL展示了企业级可行性，为敏感领域的协作机器学习提供了隐私保护和可扩展的解决方案。
...
### [221] [Some Theoretical Results on Layerwise Effective Dimension Oscillations in Finite Width ReLU Networks](https://arxiv.org/abs/2507.07675)
*Darshan Makwana*

Main category: cs.LG

TL;DR: 论文分析了有限宽度全连接ReLU网络中每层的有效维度（特征矩阵的秩），推导了随机高斯权重下隐藏激活矩阵的期望秩的闭式表达式，并揭示了秩随深度几何衰减及局部峰值现象。


<details>
  <summary>Details</summary>
Motivation: 研究ReLU网络中每层激活矩阵的秩行为，以更精确地理解深度网络的表达能力。

Method: 固定输入批次和随机高斯权重，推导隐藏激活矩阵的期望秩闭式表达式，并分析其几何衰减和局部峰值现象。

Result: 秩随深度几何衰减，衰减比为1-2/π≈0.3634，并在特定深度出现局部峰值；正交初始化或强负斜率Leaky-ReLU可避免秩下降。

Conclusion: 研究揭示了ReLU网络中秩的动态变化，为深度网络表达能力提供了更细致的理解。
...
### [222] [Balancing the Past and Present: A Coordinated Replay Framework for Federated Class-Incremental Learning](https://arxiv.org/abs/2507.07712)
...
<details>
  <summary>Details</summary>
Motivation: 联邦类增量学习（FCIL）在处理多客户端持续增加的任务时，数据重放方法因类不平衡问题性能受限。

Method: FedCBDR包含全局视角数据重放模块和任务感知温度缩放模块，分别解决类内和类间不平衡问题。

Result: 实验表明，FedCBDR在异构数据分布下实现了类平衡采样，并在任务不平衡情况下提升了泛化能力，Top-1准确率提高了2%-15%。

Conclusion: FedCBDR通过全局协调和动态调整策略，有效解决了FCIL中的类不平衡问题，性能优于现有方法。
...
### [223] [GuardVal: Dynamic Large Language Model Jailbreak Evaluation for Comprehensive Safety Testing](https://arxiv.org/abs/2507.07735)
*Peiyan Zhang,Haibo Jin,Liying Kang,Haohan Wang*

Main category: cs.LG

TL;DR: 本文提出GuardVal协议，动态生成和优化越狱提示，评估LLM的安全性，并揭示不同模型的脆弱性。
...
Result: 应用于多种模型（如Mistral-7b、GPT-4），揭示其行为模式和脆弱性。

Conclusion: GuardVal提供更准确的评估，为未来研究和安全模型开发提供依据。
...
### [224] [Efficient and Scalable Estimation of Distributional Treatment Effects with Multi-Task Neural Networks](https://arxiv.org/abs/2507.07738)
...
<details>
  <summary>Details</summary>
Motivation: 传统方法主要关注平均处理效应（ATE），而DTE能提供更细粒度的实验结果洞察，但现有回归调整方法在估计DTE时面临数据不平衡和计算效率低的挑战。
...
### [225] [OPC: One-Point-Contraction Unlearning Toward Deep Feature Forgetting](https://arxiv.org/abs/2507.07754)
...
<details>
  <summary>Details</summary>
Motivation: 现有机器遗忘方法仅调整模型响应，内部表征仍保留被遗忘数据信息，无法满足隐私、法律或伦理要求。

Method: 提出基于特征表示单点收缩的深度遗忘理论准则，并设计高效近似算法OPC。

Result: OPC在图像分类遗忘任务中表现优异，能有效抵抗性能恢复和梯度反转攻击。

Conclusion: OPC的深度特征遗忘机制显著提升了机器遗忘的鲁棒性，为未来研究提供了新方向。
...
### [226] [TRIX- Trading Adversarial Fairness via Mixed Adversarial Training](https://arxiv.org/abs/2507.07768)
*Tejaswini Medi,Steffen Jung,Margret Keuper*

Main category: cs.LG

TL;DR: TRIX是一种特征感知的对抗训练框架，通过自适应分配不同强度的对抗样本，减少类别间鲁棒性差异，提升整体防御效果。
...
Method: TRIX为强类别分配弱目标对抗样本以促进特征多样性，为弱类别分配强非目标对抗样本以增强鲁棒性，并结合每类损失权重和扰动强度调整。

Result: 在标准图像分类基准测试中，TRIX显著提高了最差类别准确率，减少了类别间鲁棒性差异，并保持了整体准确率。
...
### [227] [Synchronizing Task Behavior: Aligning Multiple Tasks during Test-Time Training](https://arxiv.org/abs/2507.07778)
*Wooseong Jeong,Jegyeong Cho,Youngho Yoon,Kuk-Jin Yoon*

Main category: cs.LG

TL;DR: S4T是一种新的测试时训练方法，通过同步多任务行为来解决传统TTT在多任务域适应中的问题。


<details>
  <summary>Details</summary>
Motivation: 解决传统测试时训练（TTT）在多任务域适应中任务行为不同步的问题。

Method: 提出S4T方法，通过预测任务关系来同步多任务行为，结合传统TTT协议。

Result: S4T在多个基准测试中优于现有TTT方法。

Conclusion: S4T为多任务域适应提供了一种有效的同步解决方案。
...
### [228] [Deep Survival Analysis in Multimodal Medical Data: A Parametric and Probabilistic Approach with Competing Risks](https://arxiv.org/abs/2507.07804)
*Alba Garrido,Alejandro Almodóvar,Patricia A. Apellániz,Juan Parras,Santiago Zazo*

Main category: cs.LG

TL;DR: 提出了一种名为SAMVAE的多模态深度学习框架，用于生存分析，整合六种数据模态，提升肿瘤生存预测的准确性。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖单一数据模态，难以捕捉肿瘤生物学的复杂性，因此需要多模态整合方法。

Method: SAMVAE通过模态特定编码器将输入映射到共享潜在空间，结合临床变量、分子特征和病理图像，支持竞争风险和连续时间建模。

Result: 在两个癌症队列中验证了SAMVAE的优越性能，尤其在竞争风险场景下表现突出。
...
### [229] [Pay Attention to Attention Distribution: A New Local Lipschitz Bound for Transformers](https://arxiv.org/abs/2507.07814)
...
Method: 通过改进softmax函数的谱范数闭式表达式，推导更准确的Lipschitz界，并设计JaSMin正则化项。

Result: 新Lipschitz界更准确，揭示了注意力图分布对鲁棒性的影响，JaSMin有效提升网络鲁棒性。

Conclusion: 新方法为Transformer的鲁棒性提供了理论解释和实用工具，JaSMin是一种有效的轻量级正则化方法。
...
### [230] [An Empirical Bernstein Inequality for Dependent Data in Hilbert Spaces and Applications](https://arxiv.org/abs/2507.07826)
...
### [231] [Towards Benchmarking Foundation Models for Tabular Data With Text](https://arxiv.org/abs/2507.07829)
...
Method: 提出了一系列简单有效的消融策略，将文本整合到传统表格流程中，并手动整理了一组具有文本特征的真实数据集。
...
### [232] ["So, Tell Me About Your Policy...": Distillation of interpretable policies from Deep Reinforcement Learning agents](https://arxiv.org/abs/2507.07848)
...
Method: 通过考虑优势函数（包含动作优越性信息），训练可解释策略，并利用先前收集的经验进行训练。

Result: 算法在经典控制环境和金融交易场景中验证了其从复杂专家策略中提取有意义信息的能力。

Conclusion: 该算法为可解释策略的提取提供了有效方法，平衡了性能与可解释性。
...
### [233] [Pre-Trained AI Model Assisted Online Decision-Making under Missing Covariates: A Theoretical Perspective](https://arxiv.org/abs/2507.07852)
*Haichen Hu,David Simchi-Levi*

Main category: cs.LG

TL;DR: 论文研究了在缺失协变量但可通过预训练AI模型填补的序列上下文决策问题，提出了“模型弹性”概念，量化了填补误差对决策的影响，并展示了在MAR设置下通过校准预训练模型显著提升决策效果。


<details>
  <summary>Details</summary>
Motivation: 探讨预训练AI模型在填补缺失协变量时对序列决策问题的影响，并提出理论框架以量化这种影响。

Method: 引入“模型弹性”概念，结合正交统计学习和双重稳健回归方法，校准预训练模型以减少填补误差。

Result: 在校准预训练模型后，填补协变量的质量显著提升，决策的遗憾界限大幅改善。

Conclusion: 预训练模型的准确性对序列决策任务至关重要，模型弹性可作为评估和改进预训练模型整合效果的基础指标。
...
### [234] [Optimization Guarantees for Square-Root Natural-Gradient Variational Inference](https://arxiv.org/abs/2507.07853)
...
<details>
  <summary>Details</summary>
Motivation: 尽管自然梯度下降在变分推断中表现出快速收敛，但其理论收敛保证一直难以建立，尤其是在简单的高斯近似和凹对数似然情况下。

Method: 采用平方根参数化高斯协方差的方法，解决了收敛保证的挑战。

Result: 实验证明了自然梯度方法的有效性，并展示了其优于欧几里得或Wasserstein几何算法的优势。

Conclusion: 通过平方根参数化，论文为自然梯度变分高斯推断提供了新的理论支持，并验证了其实际效果。
...
### [235] [Credit Risk Analysis for SMEs Using Graph Neural Networks in Supply Chain](https://arxiv.org/abs/2507.07854)
...
Method: 采用图神经网络（GNN）框架，通过分析中小企业的交易和社交数据，映射空间依赖关系以预测贷款违约风险。
...
### [236] [Principled Foundations for Preference Optimization](https://arxiv.org/abs/2507.07855)
*Wenxuan Zhou,Shujian Zhang,Brice Magdalou,John Lambert,Ehsan Amid,Richard Nock,Andrew Hard*

Main category: cs.LG

TL;DR: 本文揭示了直接偏好优化（DPO）是两种主要理论（Savage的损失函数和随机选择理论）在机器学习偏好学习中的特定联系形式，并扩展了其应用范围。
...
Method: 通过建立Savage损失函数与随机选择理论的联系，支持了选择理论中的弃权和机器学习中的非凸目标。

Result: DPO框架可以免费扩展，包括边际和长度校正等设置。

Conclusion: 本文为DPO提供了理论基础，帮助理解其操作原理和潜在问题。
...
### [237] [Predicting and generating antibiotics against future pathogens with ApexOracle](https://arxiv.org/abs/2507.07862)
*Tianang Leng,Fangping Wan,Marcelo Der Torossian Torres,Cesar de la Fuente-Nunez*

Main category: cs.LG

TL;DR: ApexOracle是一种AI模型，能预测现有化合物的抗菌效力并设计新型分子，以对抗新型病原体或耐药菌株。


<details>
  <summary>Details</summary>
Motivation: 抗菌素耐药性（AMR）日益严重，现有抗生素开发速度跟不上，亟需快速发现有效抗生素。
...
### [238] [Can AI-predicted complexes teach machine learning to compute drug binding affinity?](https://arxiv.org/abs/2507.07882)
...
Method: 通过评估共折叠模型生成的合成数据对机器学习评分函数性能的影响，并开发启发式方法识别高质量预测。
...
### [239] [SAMO: A Lightweight Sharpness-Aware Approach for Multi-Task Optimization with Joint Global-Local Perturbation](https://arxiv.org/abs/2507.07883)
...
<details>
  <summary>Details</summary>
Motivation: 多任务学习（MTL）中任务冲突限制了模型性能，而SAM（锐度感知最小化）能缓解冲突，但如何结合全局和局部信息以及减少计算开销是挑战。

Method: 提出SAMO方法，利用联合全局-局部扰动，通过前向传递近似局部扰动并进行层归一化以提高效率。

Result: 在多个多任务基准测试中验证了SAMO的有效性和高效性。

Conclusion: SAMO通过轻量级设计成功解决了MTL中的任务冲突问题，并显著降低了计算成本。
...
### [240] [UnIT: Scalable Unstructured Inference-Time Pruning for MAC-efficient Neural Inference on MCUs](https://arxiv.org/abs/2507.07885)
*Ashe Neth,Sawinder kaur,Mohammad Nur Hossain Khan,Subrata Biswas,Asif Salekin,Bashima Islam*

Main category: cs.LG

TL;DR: UnIT是一种轻量级方法，通过动态跳过不必要的乘加操作，实现高效的无结构剪枝，适用于微控制器，无需重新训练或硬件定制。
...
Result: 在MSP430微控制器上，UnIT显著减少了计算量、加速了推理并降低了能耗，同时保持准确性。

Conclusion: 无结构推理时剪枝是一种可行且实用的解决方案，适用于在微控制器上高效部署深度神经网络。
...
### [241] [Efficient Causal Discovery for Autoregressive Time Series](https://arxiv.org/abs/2507.07898)
*Mohammad Fesanghary,Achintya Gopal*

Main category: cs.LG

TL;DR: 提出一种针对非线性自回归时间序列的新型约束算法，显著降低计算复杂度，提升效率和可扩展性。
...
Conclusion: 该算法在非线性时间序列因果推断中具有高效性和实用性潜力。
...
### [242] [Agentic Retrieval of Topics and Insights from Earnings Calls](https://arxiv.org/abs/2507.07906)
*Anant Gupta,Rajarshi Bhowmik,Geoffrey Gunow*

Main category: cs.LG

TL;DR: 提出了一种基于LLM-agent的方法，动态发现和提取季度财报电话会议中的新兴主题，并构建层次化本体以捕捉主题关系。


<details>
  <summary>Details</summary>
Motivation: 传统主题建模技术难以动态捕捉行业演变中的新兴主题及其关系，而金融分析需要实时跟踪公司战略焦点。

Method: 使用LLM-agent从文档中提取主题，构建层次化本体，并通过主题本体建立新旧主题间的关系。

Result: 通过评估本体一致性、主题演化准确性和捕捉新兴金融趋势的能力，验证了方法的有效性。

Conclusion: 该方法能有效推断公司层面的见解和随时间变化的新兴趋势，为金融分析提供了动态工具。
...
### [243] [Plausible Counterfactual Explanations of Recommendations](https://arxiv.org/abs/2507.07919)
...
<details>
  <summary>Details</summary>
Motivation: 反事实解释在推荐系统中具有重要作用，从法律要求到用户体验和说服力。
...
### [244] [Low Resource Reconstruction Attacks Through Benign Prompts](https://arxiv.org/abs/2507.07947)
...
<details>
  <summary>Details</summary>
Motivation: 生成模型（如扩散模型）的进步引发了隐私、版权和数据管理的风险，现有技术依赖高资源和精心设计的提示，本研究旨在开发更简单的方法。

Method: 基于领域知识，利用电商平台抓取数据的模板化特性，设计了一种无需训练集访问的攻击方法。

Result: 发现某些看似无害的提示（如“蓝色Unisex T恤”）可重建真实人类面部图像，揭示了潜在风险。

Conclusion: 研究强调了生成模型中数据重建的普遍风险，即使是普通用户也可能无意中触发此类问题。
...
### [245] [Dynamic Chunking for End-to-End Hierarchical Sequence Modeling](https://arxiv.org/abs/2507.07955)
...
<details>
  <summary>Details</summary>
Motivation: 当前语言模型仍依赖分词预处理，限制了端到端模型的潜力，H-Net旨在解决这一问题。
...
### [246] [Prospective Learning in Retrospect](https://arxiv.org/abs/2507.07965)
*Yuxin Bai,Cecelia Shuai,Ashwin De Silva,Siyu Yu,Pratik Chaudhari,Joshua T. Vogelstein*

Main category: cs.LG

TL;DR: 论文提出了一种基于前瞻学习框架的改进算法，扩展了其在动态数据分布和演化目标中的应用，特别是在顺序决策场景（如觅食）中。


<details>
  <summary>Details</summary>
Motivation: 传统PAC学习框架无法适应动态数据分布和变化的学习目标，导致性能不佳。前瞻学习框架能部分解决这一问题，但仍有改进空间。

Method: 在前瞻学习框架基础上改进算法和数值结果，并将其扩展到顺序决策场景（如觅食）。

Result: 初步结果显示改进后的算法在动态环境中表现更优。

Conclusion: 前瞻学习框架的扩展和改进为动态环境下的机器学习提供了更有效的解决方案。
...
### [247] [EXPO: Stable Reinforcement Learning with Expressive Policies](https://arxiv.org/abs/2507.07986)
*Perry Dong,Qiyang Li,Dorsa Sadigh,Chelsea Finn*

Main category: cs.LG

TL;DR: 论文提出EXPO算法，通过结合稳定模仿学习和轻量级高斯编辑策略，解决在线强化学习中表达性策略训练的稳定性问题，显著提升样本效率。


<details>
  <summary>Details</summary>
Motivation: 在线强化学习中，表达性策略（如扩散和流匹配策略）因参数化复杂导致梯度传播不稳定，影响价值最大化。

Method: 提出EXPO算法，使用两个参数化策略：一个稳定的模仿学习基础策略和一个轻量级高斯编辑策略，动态优化动作以最大化Q值。

Result: 在离线数据微调和在线训练中，EXPO比现有方法样本效率提升2-3倍。

Conclusion: EXPO通过分离稳定学习和动态优化，有效解决了表达性策略训练的稳定性问题，显著提升了样本效率。
...
### [248] [Skip a Layer or Loop it? Test-Time Depth Adaptation of Pretrained LLMs](https://arxiv.org/abs/2507.07996)
