...
TL;DR: VL4Gaze是首个大规模基准测试，用于评估和训练视觉语言模型在视线理解方面的能力，包含48.9万个自动生成的问答对，涵盖四个互补任务，结果显示现有模型在无特定监督时表现不佳，但通过VL4Gaze训练能显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 人类视线为理解视觉场景中的注意力、意图和社交互动提供了重要线索，但当前视觉语言模型在视线理解方面尚未得到系统研究。缺乏专门的基准测试来评估或训练模型在这方面的能力，不清楚通用视觉语言预训练是否能自然产生视线理解能力。

Method: 提出VL4Gaze基准测试，包含489K自动生成的问答对，覆盖124K张图像，将视线理解统一为视觉问答问题，包含四个互补任务：视线对象描述、视线方向描述、视线点定位、模糊问题识别。评估商业和开源VLMs在上下文学习和微调设置下的表现。

Result: 即使是大规模视觉语言模型，在没有任务特定监督的情况下也难以可靠推断视线语义和空间定位。相反，在VL4Gaze上进行训练能带来所有任务上显著且一致的改进，突出了针对性多任务监督对于开发视线理解能力的重要性。

Conclusion: 视线理解能力不会从通用视觉语言预训练中自然涌现，需要专门的基准测试和针对性训练。VL4Gaze填补了这一空白，为开发具有视线理解能力的视觉语言模型提供了重要资源和方向。
...
TL;DR: 本文介绍了OccuFly——首个基于相机的真实世界空中语义场景补全基准数据集，解决了无人机场景中LiDAR数据获取困难的问题，并提出了基于传统3D重建的自动标注框架。


<details>
  <summary>Details</summary>
Motivation: 语义场景补全在移动机器人3D感知中至关重要，但现有研究主要集中于自动驾驶等地面场景，空中场景（如自主飞行）尚未充分探索。同时，LiDAR作为主要数据采集方式对无人机存在诸多限制（法规、重量、能耗、稀疏性等），而相机在无人机上普遍存在却缺乏相应基准。

Method: 提出了OccuFly基准数据集，在50m、40m、30m高度采集春夏秋冬四季数据，覆盖城市、工业、农村场景，提供22个语义类别。关键创新是提出了基于相机模态的LiDAR-free数据生成框架，利用传统3D重建技术，通过将标注的2D掩码提升到重建点云中实现自动标签传递，大幅减少人工3D标注工作量。

Result: 建立了首个真实世界基于相机的空中SSC基准数据集，数据格式遵循现有研究规范便于集成。在OccuFly上对现有最先进方法进行了基准测试，突出了高空视角特有的挑战，为全面的空中3D场景理解提供了综合视觉基准。

Conclusion: OccuFly填补了空中语义场景补全研究的空白，通过相机模态解决了无人机LiDAR数据获取的难题，提出的自动标注框架显著降低了标注成本，为空中3D感知研究提供了重要基础设施，将推动下游应用的发展。
...
<details>
  <summary>Details</summary>
Motivation: 乳腺超声分割对计算机辅助诊断和治疗规划至关重要。虽然基于提示的方法在有文本或空间提示时能提升分割性能，但许多公开BUS数据集缺乏可靠的元数据或报告，限制了训练数据规模并降低了模型鲁棒性。

Method: 提出NullBUS多模态混合监督框架，引入可空提示机制，通过可学习的空嵌入和存在掩码处理缺失文本，使模型能够在元数据缺失时回退到仅图像证据，在有文本时利用文本信息。

Result: 在三个公开BUS数据集统一池上的评估显示，NullBUS实现了0.8568的平均IoU和0.9103的平均Dice，在混合提示可用性下达到了最先进的性能。

Conclusion: NullBUS通过可空提示机制有效解决了BUS分割中文本提示缺失的问题，实现了在混合提示可用性下的鲁棒分割性能，为缺乏可靠元数据的公开数据集提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D生成方法在几何质量、细节保留和训练资源效率方面存在局限，需要开发一个既能生成高保真3D几何，又能有效利用公开数据集和有限计算资源的框架。

Method: 采用两阶段生成流程：1）先生成粗略全局结构；2）通过空间解耦的扩散过程进行细化，使用体素查询和RoPE编码提供空间锚点，让模型专注于局部几何细节合成。同时开发了包含水密处理和高质量过滤的数据处理流程。

Result: 模型在公开3D数据集上训练，尽管训练资源有限，但实现了强大的几何质量。在数据处理质量和几何生成方面与现有开源方法相比具有竞争力。

Conclusion: UltraShape 1.0提供了一个有效的3D扩散框架，通过创新的数据处理和空间解耦方法，在有限资源下实现了高质量的3D几何生成，为未来研究提供了有价值的工具和模型。
...
<details>
  <summary>Details</summary>
Motivation: 当前用于量化细胞形态的模型通常只针对单一显微镜成像类型进行训练，导致模型无法在不同生物学研究中重复使用，因为技术规格不匹配或目标实验条件超出分布范围。

Method: 从公开来源整理和构建CHAMMI-75数据集，包含75个不同生物学研究的异质多通道显微镜图像，用于训练通道自适应的细胞形态模型。

Result: 使用CHAMMI-75进行训练可以改善多通道生物成像任务的性能，主要原因是其显微镜模态的高度多样性。
...
TL;DR: 提出自适应视觉预处理方法，动态调整输入分辨率和空间覆盖范围，显著降低视觉冗余，在FastVLM上实现超过50%的推理时间减少和55%的视觉token减少。
...
Result: 在DocVQA数据集上评估，自适应预处理使每张图像推理时间减少超过50%，降低平均完整生成时间，视觉token数量相比基线流水线持续减少超过55%。
...
TL;DR: ALIVE是一个本地部署的交互式视频学习系统，通过神经虚拟化身、内容感知检索和多模态交互，将被动讲座视频观看转变为动态实时学习体验。


<details>
  <summary>Details</summary>
Motivation: 传统讲座视频缺乏实时澄清机制，学习者遇到困惑时需外部搜索。现有交互学习系统通常缺乏讲座意识、依赖云服务，或未能将检索与虚拟化身解释整合到统一的隐私保护流程中。

Method: ALIVE系统包含三个核心组件：1) 通过ASR转录、LLM精炼和神经说话头合成生成的虚拟化身讲座；2) 结合语义相似性和时间戳对齐的内容感知检索机制；3) 支持文本或语音提问、接收文本或虚拟化身响应的实时多模态交互。系统采用轻量级嵌入模型、FAISS检索和分段虚拟化身合成。
...
TL;DR: 本文提出了一种用于地下管道检测的3D GPR智能检测框架，通过三视图联合分析、改进的YOLO算法和3D空间特征匹配，显著提升了复杂场景下小尺度目标的识别精度。


<details>
  <summary>Details</summary>
Motivation: 解决3D GPR地下管道检测中存在的多视图特征相关性弱、小尺度目标识别精度低、复杂场景下鲁棒性不足等问题。

Method: 1) 基于B/C/D-Scan三视图联合分析策略，建立三维管道三视图特征评估方法；2) 提出DCO-YOLO框架，集成DySample、CGLU和OutlookAttention跨维度相关机制；3) 提出3D-DIoU空间特征匹配算法，集成三维几何约束和中心距离惩罚项。

Result: 在真实城市地下管道数据实验中，该方法在复杂多管道场景下的准确率、召回率和平均精度分别为96.2%、93.3%和96.7%，比基线模型分别提高了2.0%、2.1%和0.9%。
...
TL;DR: DexAvatar是一个从单目手语视频中重建生物力学精确的细粒度手部关节和身体运动的新框架，相比现有方法在身体和手部姿态估计上提升了35.11%。


<details>
  <summary>Details</summary>
Motivation: 当前手语生成主要依赖数据驱动的生成方法，需要大量精确的2D和3D人体姿态数据。但大多数手语数据集基于视频，仅有自动重建的2D关键点，缺乏准确的3D信息。现有从手语视频进行3D人体姿态估计的方法容易受到自遮挡、噪声和运动模糊的影响，导致重建质量差。

Method: 提出DexAvatar框架，从野外单目手语视频中重建生物力学精确的细粒度手部关节和身体运动，通过学习的3D手部和身体先验进行指导。

Result: 在SGNify运动捕捉数据集（该任务唯一可用的基准测试）上表现优异，相比现有最先进方法，在身体和手部姿态估计上提升了35.11%。
...
TL;DR: 提出一种新的参数高效微调策略DRI，通过特征空间优化而非权重空间优化，在保持视觉基础模型冻结的同时，通过轻量级偏移编码器和调制器注入领域特定表示，显著提升跨模态船舶重识别性能。
...
Method: 提出领域表示注入(DRI)策略：保持视觉基础模型完全冻结，设计轻量级可学习的偏移编码器提取富含模态和身份属性的领域特定表示，通过调制器根据中间特征上下文信息自适应转换这些表示，然后通过加法融合注入中间层，动态重塑特征分布而不改变预训练权重。

Result: 在HOSS-ReID数据集上仅使用1.54M和7.05M参数分别达到57.9%和60.5%的mAP，实现了最先进的性能，且参数量极小。

Conclusion: DRI方法通过特征空间优化有效桥接模态差异，在保持视觉基础模型通用知识的同时，以极少的可训练参数实现了跨模态船舶重识别的最先进性能，为参数高效微调提供了新思路。
...
Method: 1) 创建首个综合基准测试，包含超过100万张压缩图像，涵盖多种图像编解码器和任务类型；2) 分析性能差距来源：压缩过程中的信息损失和VLM泛化失败；3) 提出通用VLM适配器来增强模型在压缩图像上的性能。

Result: 1) 建立了全面的压缩图像VLM评估基准；2) 识别出性能差距主要来自VLM泛化失败而非信息损失；3) 提出的通用适配器能提升VLM在各种编解码器和比特率压缩图像上的性能10%-30%。
...
Method: 提出PanoGrounder框架，使用增强了3D语义和几何特征的全景渲染作为2D和3D之间的中间表示。采用三阶段流程：1) 基于场景布局和几何放置紧凑的全景点；2) 使用VLM在每个全景渲染上对文本查询进行定位；3) 通过提升将每个视图的预测融合为单个3D边界框。

Result: 在ScanRefer和Nr3D数据集上取得了最先进的结果，并在未见过的3D数据集和文本重述方面表现出优越的泛化能力。

Conclusion: PanoGrounder通过将多模态全景表示与预训练2D VLM相结合，为3D视觉定位提供了有效的解决方案，克服了传统方法的泛化限制。
...
### [14] [Reasoning-Driven Amodal Completion: Collaborative Agents and Perceptual Evaluation](https://arxiv.org/abs/2512.20936)
*Hongxing Fan,Shuyu Zhao,Jiayang Ao,Lu Sheng*

Main category: cs.CV

TL;DR: 提出协作多智能体推理框架，通过语义规划与视觉合成的解耦，结合自校正验证智能体和多样化假设生成器，实现语义一致的单次合成，并引入MAC-Score评估指标。


<details>
  <summary>Details</summary>
Motivation: 现有渐进式方法存在推理不稳定和误差累积问题，难以保持语义一致性和结构完整性，需要更稳健的不可见区域推理方法。

Method: 协作多智能体推理框架，将语义规划与视觉合成解耦，包含自校正验证智能体（使用思维链推理修正分割）和多样化假设生成器（提供多种语义解释），并引入MAC-Score评估指标。

Result: 在多个数据集上显著优于现有最先进方法，验证了框架的有效性和MAC-Score与人类判断的一致性。

Conclusion: 通过语义规划优先的协作多智能体方法，实现了更稳健、语义一致的模态补全，为不可见区域推理提供了新框架和评估标准。
...
### [15] [Beyond Artifacts: Real-Centric Envelope Modeling for Reliable AI-Generated Image Detection](https://arxiv.org/abs/2512.20937)
...
<details>
  <summary>Details</summary>
Motivation: 现有检测器过度依赖生成器特定的伪影特征，对真实世界图像退化（如多轮跨平台共享和后处理）高度敏感。随着生成架构演进和图像链式退化，这些伪影线索变得过时且难以检测。

Method: 提出REM新范式：1）通过特征级扰动自重建生成近真实样本；2）使用包络估计器与跨域一致性学习真实图像流形的边界；3）构建RealChain基准测试集，涵盖开源和商业生成器及模拟真实世界退化。

Result: 在八个基准测试中，REM比最先进方法平均提升7.5%，在严重退化的RealChain基准上保持优异泛化能力，为真实世界条件下的合成图像检测奠定坚实基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有神经激光雷达建图方法大多依赖密集隐式表示且未充分利用几何结构，而现有的体素引导方法难以实现实时性能，需要一种既能利用几何结构又能高效运行的大规模增量建图方法
...
TL;DR: 提出一种利用大量正常X光片合成罕见肺部异常数据的新方法，通过扩散模型和语言模型知识引导，显著提升长尾病变的诊断精度


<details>
  <summary>Details</summary>
Motivation: 胸部X光片中长尾肺部异常（罕见病变）诊断困难，现有基于扩散的方法因罕见病变样本稀缺而生成能力有限，导致诊断精度不足

Method: 1) 收集大量正常X光片训练扩散模型生成正常图像；2) 用预训练扩散模型对病变X光片中的常见病变进行修复，保留罕见病变作为增强数据；3) 引入大型语言模型知识引导模块和渐进增量学习策略稳定微调过程

Result: 在公开肺部数据集MIMIC和CheXpert上的综合评估表明，该方法在性能上达到了新的基准水平

Conclusion: 提出的数据合成流程有效解决了长尾肺部异常诊断中数据稀缺问题，通过利用大量正常X光片和智能修复技术，显著提升了罕见病变的诊断性能
...
TL;DR: 提出前馈多视角逆渲染框架，通过注意力机制实现跨视角一致性，并利用无标签真实视频进行一致性微调，提升泛化能力
...
Method: 1) 前馈多视角逆渲染框架，直接从RGB图像序列预测空间变化的反照率、金属度、粗糙度、漫反射着色和表面法线；2) 通过跨视角交替注意力机制捕捉视角内长距离光照交互和视角间材质一致性；3) 提出基于一致性的微调策略，利用无标签真实世界视频增强多视角一致性和野外条件下的鲁棒性

Result: 在基准数据集上的广泛实验表明，该方法在多视角一致性、材质和法线估计质量以及向真实世界图像的泛化能力方面达到最先进性能

Conclusion: 提出的前馈多视角逆渲染框架通过注意力机制和一致性微调策略，有效解决了现有方法在多视角一致性、计算效率和真实世界泛化方面的局限性
...
<details>
  <summary>Details</summary>
Motivation: 虽然自回归生成模型在NLP领域取得了革命性进展，但大多数视觉生成预训练方法仍依赖BERT风格的掩码建模，忽略了视频分析所需的时间信息。现有的自回归视觉预训练方法存在语义定位不准确、生成质量差等问题。
...
Method: 提出粒度球引导掩码(GBGM)方法，基于粒度球计算(GBC)进行结构感知增强，通过粗到细的分层掩码过程自适应保留语义丰富、结构重要的区域，抑制冗余区域
...
<details>
  <summary>Details</summary>
Motivation: 大规模文本到图像扩散模型在图像生成和编辑方面取得了巨大成功，但扩展到视频编辑仍然具有挑战性。现有方法虽然通过添加时间注意力机制来适应视频任务，但仍存在时序不一致性和高计算开销的问题。

Method: 提出FluencyVE方法，将线性时序模块Mamba集成到基于预训练Stable Diffusion模型的视频编辑框架中，替换时间注意力层。同时采用低秩近似矩阵替换因果注意力中的查询和键权重矩阵，并在训练时使用加权平均技术更新注意力分数。

Result: 实验和分析表明，该方法在编辑真实世界视频的各种属性、主体和位置方面取得了有希望的结果，有效保持了文本到图像模型的生成能力，同时显著降低了计算负担。

Conclusion: FluencyVE是一种简单而有效的单次视频编辑方法，通过集成Mamba时序模块和优化注意力机制，成功解决了视频编辑中的时序不一致性和计算效率问题。
...
TL;DR: 提出针对3D场说话人脸生成方法的视频防御框架，通过扰动3D信息获取过程来保护肖像视频，同时保持高质量视频保真度


<details>
  <summary>Details</summary>
Motivation: 现有的3D场说话人脸生成方法能够实时合成高保真个性化说话人脸视频，这带来了严重的隐私问题，因为恶意使用者可能滥用个人肖像。目前缺乏有效的防御框架来保护视频免受这类3D场TFG方法的攻击，而基于图像的逐帧2D扰动方法计算成本过高且会严重降低视频质量。
...
Conclusion: 该研究提出了首个针对3D场说话人脸生成方法的有效视频防御框架，能够在保护肖像视频隐私的同时保持高质量的视频保真度，为解决3D TFG方法带来的隐私威胁提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有大多数人脸识别模型基于可见光数据集训练，在红外图像上性能显著下降。早期基于特征的红外人脸识别方法效果不佳，生成式方法成为主流但面临模型和模态差异导致的图像失真和特征丢失问题。

Method: 提出基于潜在扩散的模型，从热红外输入生成高质量可见光人脸图像。引入多属性分类器从可见光图像提取关键面部属性，减少红外到可见光恢复过程中的特征损失。设计Self-attn Mamba模块增强跨模态特征的全局建模能力并显著提升推理速度。

Result: 在两个基准数据集上的实验结果表明，该方法在图像质量和身份特征保留方面均达到最先进水平，验证了所提方法的有效性。

Conclusion: 提出的潜在扩散模型结合多属性分类器和Self-attn Mamba模块，有效解决了红外-可见光人脸识别中的图像失真和特征丢失问题，为异构人脸识别提供了高质量解决方案。
...
TL;DR: KOREATECH-CGH数据集包含6,000对RGB-D图像和复杂全息图对，分辨率从256×256到2048×2048，深度范围覆盖宽3D场景，并引入振幅投影技术提升大深度范围的全息图质量。


<details>
  <summary>Details</summary>
Motivation: 机器学习计算机生成全息术（ML-CGH）发展迅速，但受限于高质量、大规模全息图数据集的缺乏，因此需要构建公开可用的高质量数据集来推动该领域发展。

Method: 提出KOREATECH-CGH公开数据集，包含6,000对RGB-D图像和复杂全息图对，覆盖多种分辨率；引入振幅投影后处理技术，在保持相位不变的情况下替换各深度层的振幅分量以提升重建质量。

Result: 振幅投影技术显著提升重建保真度，达到27.01 dB PSNR和0.87 SSIM，比最近优化的轮廓掩模层方法分别高出2.03 dB和0.04 SSIM；数据集在先进ML模型的全息图生成和超分辨率任务中验证了实用性。

Conclusion: KOREATECH-CGH数据集解决了ML-CGH领域数据稀缺问题，振幅投影技术有效提升全息图质量，该数据集适用于训练和评估下一代ML-CGH系统，推动该领域发展。
...
<details>
  <summary>Details</summary>
Motivation: 低秩矩阵补全在众多应用中表现出色，但秩最小化问题是NP难的。常用的核范数作为秩函数的凸替代往往产生次优解，因为对奇异值的过度收缩。需要寻找更有效的非凸替代函数来更好地近似秩函数。

Method: 提出了一种新的重加权对数范数作为秩函数的非凸替代，该替代比许多现有替代提供了更接近的近似。采用交替方向乘子法（ADMM）高效求解由此产生的优化问题。

Result: 在图像修复实验中的结果表明，所提出的方法在视觉质量和定量指标上都优于最先进的低秩矩阵补全方法。

Conclusion: 重加权对数范数作为秩函数的非凸替代比核范数等凸替代更有效，能够获得更好的低秩矩阵补全性能，特别是在图像修复应用中表现出优越性。
...
TL;DR: 提出一种基于事件相机的光流引导6自由度物体姿态跟踪方法，通过2D-3D混合特征提取和光流关联优化，在精度和鲁棒性上优于现有事件相机方法。
...
Method: 采用2D-3D混合特征提取策略检测事件和物体模型的角点和边缘；通过最大化时空窗口内事件关联概率搜索角点的光流；建立光流引导的角点-边缘关联；通过最小化角点与边缘距离迭代优化6自由度物体姿态。
...
<details>
  <summary>Details</summary>
Motivation: 当前病理计算领域存在三个关键问题：1）缺乏大规模高质量图像-文本语料库；2）缺乏精确的细粒度语义控制，导致依赖非语义线索；3）术语异质性，同一诊断概念有多种表述方式，影响文本条件化的可靠性。

Method: UniPath采用多流控制机制：原始文本流、高层语义流（使用可学习查询从冻结的病理MLLM中提取诊断语义标记）和原型流（通过原型库实现组件级形态控制）。同时构建了265万图像-文本语料库和6.8万高质量标注子集。

Result: UniPath在病理图像生成中达到SOTA性能，Patho-FID为80.9（比第二名提升51%），细粒度语义控制达到真实图像的98.7%。建立了专门的四级评估体系。

Conclusion: UniPath通过利用成熟的诊断理解能力实现可控生成，解决了病理图像生成中的关键挑战。研究将公开数据集、源代码和预训练模型权重，促进领域发展。
...
TL;DR: 提出一种利用非因果注意力捕捉完整表格结构的新型多任务模型，以及用于更快单元格内容推理的并行推理算法，在两大公共数据集上取得视觉和统计上的优越性


<details>
  <summary>Details</summary>
Motivation: 从大量文档中提取和使用多样化知识是智能信息检索的迫切挑战。文档包含需要不同识别方法的元素。表格识别通常包含三个子任务：表格结构、单元格位置和单元格内容识别。现有模型虽然结合多任务学习、局部注意力和相互学习取得了优秀识别效果，但其有效性未得到充分解释，且推理时间较长。

Method: 提出一种新型多任务模型，利用非因果注意力捕捉完整表格结构，并设计并行推理算法以加速单元格内容推理。

Result: 在两个大型公共数据集上，该模型在视觉和统计上都展示了优越性。

Conclusion: 提出的方法有效解决了表格识别中多任务学习的效率和解释性问题，通过非因果注意力和并行推理实现了更快速、更准确的表格识别。
...
TL;DR: T2AV-Compass是一个统一的文本到音视频生成系统评估基准，包含500个多样化的复杂提示和双层次评估框架，用于全面评估视频质量、音频质量、跨模态对齐、指令跟随和感知真实性。
...
Method: 通过分类学驱动的流程构建500个多样化和复杂的提示，确保语义丰富性和物理合理性；引入双层次评估框架，结合客观信号级指标（视频质量、音频质量、跨模态对齐）和主观的MLLM-as-a-Judge协议（指令跟随和真实性评估）。

Result: 对11个代表性T2AV系统的广泛评估显示，即使是最强的模型也远未达到人类水平的真实性和跨模态一致性，在音频真实性、细粒度同步、指令跟随等方面存在持续失败。

Conclusion: 结果表明未来模型有显著的改进空间，T2AV-Compass作为一个具有挑战性和诊断性的测试平台，对推进文本到音视频生成技术具有重要价值。
...
TL;DR: UniRec-0.1B是一个仅0.1B参数的轻量级统一识别模型，能够识别文本和公式的多层次内容，在保持高性能的同时实现2-9倍加速。
...
Method: 1) 构建包含4000万文本、公式及其混合样本的UniRec40M数据集；2) 针对层次结构可变性和文本-公式语义纠缠问题，提出分层监督训练和语义解耦分词器；3) 建立涵盖中英文多领域多层次的综合评估基准。

Result: UniRec-0.1B在综合基准和公开基准测试中，性能优于通用视觉语言模型和领先的文档解析专家模型，同时实现2-9倍的速度提升。

Conclusion: UniRec-0.1B是一个高效且有效的轻量级统一识别模型，成功解决了现有模型参数量大、计算成本高的问题，在文档解析领域具有重要应用价值。
...
TL;DR: FreeInpaint是一个即插即用的免调优文本引导图像修复方法，通过推理时直接优化扩散潜变量来提升生成图像的忠实度
...
Method: 提出先验引导的噪声优化方法引导模型注意力到有效修复区域，并设计针对修复任务的复合引导目标，在推理时优化中间潜变量

Result: 通过在不同修复扩散模型和评估指标上的广泛实验，证明了FreeInpaint的有效性和鲁棒性

Conclusion: FreeInpaint是一个有效的免调优方法，能同时提升文本引导图像修复的提示对齐和视觉合理性
...
TL;DR: 本文提出了首个大规模海洋视觉语言模型数据集和基准测试MarineEval，包含2000个基于图像的问答对，用于评估现有VLMs在海洋专业领域的表现，发现现有模型在回答需要专业知识的海洋问题时效果不佳。
...
Method: 构建了首个大规模海洋VLM数据集和基准测试MarineEval，包含2000个基于图像的问答对。确保数据的多样性和覆盖范围：7个任务维度和20个能力维度。将领域要求特别整合到数据构建中，并由相应的海洋领域专家验证。

Result: 对17个现有VLMs在MarineEval上进行了全面基准测试，实验结果显示现有VLMs无法有效回答领域特定问题，性能仍有很大提升空间。调查了现有模型在回答海洋研究问题时的局限性。

Conclusion: 现有视觉语言模型在海洋专业领域表现不足，需要进一步改进。作者希望新的基准测试和观察结果能促进未来研究，推动VLMs在专业领域的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本引导医学分割方法通常依赖未对齐的图像和文本编码器，需要复杂的多模态融合模块。虽然CLIP提供了预对齐的多模态特征空间，但直接应用于医学图像存在三个主要问题：1）细粒度解剖结构保留不足；2）复杂临床描述建模不足；3）领域特定的语义不对齐。
...
Conclusion: TGC-Net通过参数高效的任务特定适配，有效解决了CLIP在医学图像分割中的局限性，为文本引导的医学分割提供了一种更有效的方法。
...
<details>
  <summary>Details</summary>
Motivation: 海洋视觉理解对于监测和保护海洋生态系统至关重要，但目前进展受到训练数据有限和缺乏系统化任务制定的阻碍，限制了模型在海洋领域的有效应用。

Method: 构建了ORCA多模态基准数据集，包含丰富的视觉和文本标注，涵盖形态学特征。评估了18个最先进模型在三个任务上的表现：目标检测（封闭集和开放词汇）、实例描述和视觉定位。

Result: 评估结果突显了海洋理解的关键挑战，包括物种多样性、形态重叠和特定领域需求，表明海洋视觉理解任务的难度。

Conclusion: ORCA建立了一个全面的基准，为海洋领域研究提供了系统化的评估框架，将推动海洋视觉理解方法的发展。
...
TL;DR: 提出一种基于隐式神经表示(INRs)的连续人体运动序列表示框架NAME，能够以任意帧率插值、补间甚至外推输入运动序列
...
Method: 提出参数化激活诱导的分层隐式表示框架(NAME)：1)分层时间编码机制从多个时间尺度提取特征；2)集成基于傅里叶变换的自定义参数化激活函数到MLP解码器中
...
TL;DR: VisRes Bench是一个专门设计用于在无语言监督的自然场景中评估视觉语言模型视觉推理能力的基准测试，揭示了当前SOTA模型在感知和关系推理方面的明显局限性。


<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在视觉问答和图像描述等任务上取得了显著进展，但这些模型究竟是在进行视觉推理还是主要依赖语言先验仍不清楚。需要创建一个专门的基准来研究模型在自然场景中的纯视觉推理能力。

Method: 研究者设计了VisRes Bench基准测试，包含三个复杂度层级：Level 1测试感知完整性和全局图像匹配（在模糊、纹理变化、遮挡和旋转等扰动下）；Level 2测试基于单一属性（如颜色、数量、方向）的规则推理；Level 3测试需要整合多个视觉属性的组合推理。该基准包含超过19,000张受控任务图像。

Result: 研究发现，在细微的感知扰动下，最先进的视觉语言模型表现接近随机水平，显示出模型在模式识别之外的抽象能力有限。VisRes Bench能够有效隔离不同层级的推理能力，揭示了当前模型在感知和关系视觉推理能力上的明显局限性。

Conclusion: VisRes Bench为多模态研究提供了一个统一的框架，可用于推进抽象视觉推理能力的发展。该基准揭示了当前视觉语言模型在纯视觉推理方面的不足，为未来模型改进提供了明确的方向。
...
TL;DR: EveryWear是一个基于日常可穿戴设备的轻量级人体运动捕捉系统，使用智能手机、智能手表、耳机和智能眼镜，无需显式校准，通过多模态师生框架整合视觉和惯性信号，在真实世界数据上训练，消除仿真到现实的差距。


<details>
  <summary>Details</summary>
Motivation: 现有基于设备的运动估计方法存在佩戴不便、硬件昂贵、校准繁琐等问题，阻碍了日常生活中的应用。需要一种轻量级、实用性强且基于日常可穿戴设备的解决方案。

Method: 使用智能手机、智能手表、耳机和智能眼镜（配备一个前向和两个下向摄像头）作为硬件基础。提出多模态师生框架，整合智能眼镜的视觉线索和其他设备的惯性信号。在Ego-Elec数据集（9小时真实世界数据，包含56种日常活动，17种室内外环境）上直接训练，避免使用合成数据。

Result: 实验表明该方法优于基线模型，验证了其在实际全身运动估计中的有效性。通过直接在真实世界数据上训练，有效消除了先前工作中存在的仿真到现实差距。

Conclusion: EveryWear提供了一种实用、轻量级且无需校准的人体运动捕捉方法，基于日常可穿戴设备，通过多模态融合和真实世界数据训练，解决了现有方法的局限性，为XR交互等应用提供了可行的解决方案。
...
Result: 该方法在直接微调基础上表现更优，在多种视觉中心任务上取得最先进结果，包括那些中间抽象难以指定的任务，同时也能泛化到多任务指令调优。

Conclusion: 提出的任务无关机制能够有效解决LMMs在视觉推理任务上的局限性，通过让模型自主发现视觉推理标记，避免了手工监督的限制，实现了更好的泛化能力和任务适应性。
...
TL;DR: 本文提出了一种轻量级的两阶段图像检索方法，通过事件中心实体提取结合时间上下文信号，使用BM25进行候选过滤，再用BEiT-3模型进行重排序，在OpenEvents v1基准上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现实世界的图像文本检索面临挑战，包括模糊或依赖上下文的查询、语言多样性以及需要可扩展的解决方案。现有的方法在处理复杂真实场景时效果有限。

Method: 提出两阶段检索流程：第一阶段基于事件中心实体提取，使用BM25进行高效候选过滤；第二阶段应用BEiT-3模型捕捉深度多模态语义并对结果进行重排序。

Result: 在OpenEvents v1基准测试中，该方法获得了0.559的平均精度均值，显著优于先前基线方法。

Conclusion: 结合事件引导过滤与长文本视觉语言建模的方法，在复杂真实场景中实现了准确高效的图像检索，证明了该方法的有效性。
...
TL;DR: SegMo：基于片段对齐的文本到3D人体动作生成框架，通过将文本和动作分解为语义连贯的片段实现细粒度对齐，提升生成质量并支持检索任务。


<details>
  <summary>Details</summary>
Motivation: 现有方法在序列级别对齐文本描述与人体动作，忽略了模态内部语义结构。文本描述和动作序列都可以自然分解为更小、语义连贯的片段，这些片段可以作为原子对齐单元实现更细粒度的对应关系。

Method: 提出SegMo框架，包含三个模块：(1)文本片段提取：将复杂文本描述分解为时序有序的短语，每个短语表示简单原子动作；(2)动作片段提取：将完整动作序列划分为对应动作片段；(3)细粒度文本-动作对齐：通过对比学习对齐文本和动作片段。

Result: 在两个广泛使用的数据集上改进强基线，在HumanML3D测试集上TOP 1分数达到0.553。由于学习了文本和动作片段的共享嵌入空间，SegMo还可应用于动作定位和动作到文本检索等检索式任务。

Conclusion: SegMo通过将文本和动作分解为语义片段并实现细粒度对齐，显著提升了文本到3D人体动作生成的质量，同时扩展了框架的应用范围到检索任务。
...
<details>
  <summary>Details</summary>
Motivation: 传统"一镜到底"拍摄技术成本高昂且受现实条件限制，而现有视频生成模型通常采用简单的片段拼接方法，无法保证视觉平滑性和时序一致性，需要更先进的解决方案。

Method: 1) 在DiT架构中集成轻量级中间条件机制，采用自适应调优策略利用基础训练数据；2) 构建高质量数据集并进行视觉表达微调，针对主体运动合理性和过渡平滑性应用定制DPO方案；3) 设计内存高效的分段自回归推理策略以生成长序列。

Result: 实验表明该方法能够生成视觉震撼、无缝连贯的"一镜到底"效果，同时保持计算效率，成功将零散的视觉材料转化为生动连贯的"一镜到底"电影体验。

Conclusion: DreaMontage框架通过创新的技术方案有效解决了"一镜到底"视频生成的挑战，为用户提供了将碎片化视觉素材转化为高质量长镜头视频的强大工具。
...
Method: 提出统一Any-Modality AD框架：1) 双路径DINOv2编码器结合特征分布对齐机制，统计对齐不完整模态特征与完整模态表示；2) 内在正常原型提取器和INP引导解码器，仅重建正常解剖模式并放大异常偏差；3) 训练时采用随机模态掩码和间接特征补全，使模型适应所有模态配置而无需重新训练。

Result: 在BraTS2018、MU-Glioma-Post和Pretreat-MetsToBrain-Masks数据集上的广泛实验表明，该方法在7种模态组合下持续超越最先进的工业和医学AD基线，实现了优越的泛化性能。

Conclusion: 本研究为现实世界中不完美模态条件下的多模态医学异常检测建立了可扩展的范式，解决了临床工作流中模态缺失的关键问题。
...
TL;DR: 本文提出ACD框架，通过注意力监督实现视频扩散模型的直接条件控制，解决了现有方法条件控制能力有限的问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频合成方法在可控性方面存在局限：无分类器引导方法通过建模数据和条件的联合分布间接实现条件控制，效果有限；基于分类器的引导方法可能产生对抗性伪影，无法真正满足条件要求。
...
<details>
  <summary>Details</summary>
Motivation: 当前最先进的图像序列生成方法通常将图像序列视为堆叠帧的大型张量，这种表示方式存在效率瓶颈。作者旨在为生成模型设计更有效的图像序列建模方法。

Method: 方法分为两个阶段：1）使用仅训练于网格图像（包含下采样帧）的生成模型，利用扩散变换器（DiT）的自注意力机制捕获帧间相关性，将2D图像生成器扩展为低分辨率3D图像序列生成器；2）对每个帧单独进行超分辨率处理，添加序列无关的高分辨率细节。

Result: 相比现有图像序列生成模型，该方法在合成质量、序列一致性方面表现更优，能够生成任意长度的高保真序列，在推理时间和训练数据使用上效率更高。在多个数据集上，质量和推理速度（至少快两倍）均优于最先进方法。

Conclusion: 这种分解式生成方法克服了当前最先进方法的关键限制，提供了更有效的图像序列生成方案，能够跨不同数据域有效泛化，而无需额外的先验知识和监督。
...
TL;DR: 提出一种基于多尺度信号处理的方法，从表格掩码中检测表格边缘，通过高斯卷积和统计阈值处理一维信号来识别行列边界，提高低分辨率或噪声图像中的表格结构提取准确性。


<details>
  <summary>Details</summary>
Motivation: 表格结构提取在文档图像分析中至关重要，但在低分辨率或噪声图像中准确识别表格行列边界仍然具有挑战性。现有方法如基于掩码的边缘检测虽然对噪声有一定鲁棒性，但通常直接在图像上应用掩码，导致噪声敏感、分辨率损失或计算成本高。

Method: 提出一种多尺度信号处理方法：1) 将行列转换建模为一维信号；2) 使用逐渐增加方差的高斯卷积处理信号；3) 采用统计阈值抑制噪声同时保留稳定的结构边缘；4) 将检测到的信号峰值映射回图像坐标以获得准确的边界分割。

Result: 在PubLayNet-1M基准测试中，使用TableNet和PyTesseract OCR时，提出的列边缘检测方法将Cell-Aware Segmentation Accuracy (CASA)从67%提高到76%。该方法通过零填充和缩放策略对分辨率变化具有鲁棒性，并产生适合下游分析的优化结构化表格输出。

Conclusion: 该方法通过将表格边缘检测转化为信号处理问题，提供了一种鲁棒且高效的解决方案，能够有效处理低分辨率或噪声图像中的表格结构提取，显著提高了表格分割的准确性。
...
<details>
  <summary>Details</summary>
Motivation: 在大型全切片图像中，小切片的解释通常需要更大的图像上下文信息。标准的切片编码器管道提取脱离上下文的切片嵌入，无法建模对局部和全局任务都至关重要的丰富切片级信息。此外，不同的切片编码器在不同下游任务中表现各异，因此需要一个统一的模型来为任何切片级基础模型生成的嵌入提供上下文。
...
Conclusion: TICON提供了一个统一的框架，能够为任何切片级基础模型生成的嵌入提供丰富的上下文信息，显著提升计算病理学中各种任务的性能，同时通过高效的数据利用实现了超越现有方法的切片级表示能力。
...
<details>
  <summary>Details</summary>
Motivation: SAM2等视觉基础模型在处理视频对象分割时，由于需要传播所有视觉token而面临高计算和内存成本，限制了实际部署的可行性。

Method: 提出文本引导的token剪枝框架，在视觉编码后、基于内存的传播前，通过轻量级路由机制对token进行排序，综合考虑局部视觉上下文、语义相关性（来自对象中心文本描述）和不确定性线索，仅保留最具信息量的token进行下游处理。

Result: 在多个具有挑战性的视频分割基准测试中，该方法相比未剪枝的SAM2基线实现了高达42.50%的推理速度提升和37.41%的GPU内存使用降低，同时保持了竞争力的J和F性能。

Conclusion: 编码后token剪枝为高效、提示感知的视频分割提供了实用有效的途径，展示了早期token选择在提高基于transformer的视频分割系统可扩展性方面的潜力，适用于实时和资源受限的应用场景。
...
Method: 构建了Streamo-Instruct-465K大规模指令跟随数据集，涵盖多样时间上下文和多任务监督。通过端到端训练和简化流程，实现统一训练跨异构流任务。

Result: Streamo在多种流视频基准测试中表现出强大的时间推理能力、响应式交互和广泛泛化能力，在流视频理解任务上取得显著效果。
...
TL;DR: 研究发现视觉语言模型存在显著流行度偏见，在著名建筑上的准确率比普通建筑高34%，表明模型依赖记忆而非泛化理解。为此构建了包含55,546张建筑图像的YearGuessr数据集，并提出流行度感知评估指标来量化这种偏见。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的视觉语言模型在建筑年代预测任务中存在明显的流行度偏见，模型对著名建筑的识别准确率远高于普通建筑，这表明模型可能过度依赖记忆而非真正的理解能力。需要系统性地研究和量化这种偏见。

Method: 构建了YearGuessr数据集，包含55,546张来自157个国家的建筑图像，标注了建造年份（1001-2024）、GPS数据和页面浏览量作为流行度指标。将建造年份预测任务构建为序数回归问题，并引入流行度感知区间准确度指标来量化偏见。评估了30多个模型，包括提出的YearCLIP模型。

Result: 视觉语言模型在流行、记忆的项目上表现出色，但在未被识别的主题上表现显著较差，证实了模型存在严重的流行度偏见。最先进模型在著名建筑上的准确率比普通建筑高出34%，暴露了其推理能力的根本缺陷。

Conclusion: 视觉语言模型存在严重的流行度偏见，过度依赖记忆而非泛化理解能力。YearGuessr数据集和流行度感知评估指标为系统研究这种偏见提供了基准，揭示了当前模型在推理能力上的局限性。
...
TL;DR: HiStream提出高效自回归框架，通过空间、时间和时间步压缩三轴优化，显著加速高分辨率视频生成，在1080p基准上实现76.2倍加速且质量损失可忽略。


<details>
  <summary>Details</summary>
Motivation: 高分辨率视频生成面临扩散模型二次复杂度的计算瓶颈，导致实际推理不可行，需要解决这一效率问题。

Method: HiStream采用三轴压缩策略：1) 空间压缩：先在低分辨率去噪，再利用缓存特征在高分辨率细化；2) 时间压缩：分块处理策略配合固定大小锚点缓存确保稳定推理速度；3) 时间步压缩：对后续缓存条件块应用更少去噪步骤。

Result: 在1080p基准测试中，主要模型HiStream(i+ii)达到最先进视觉质量，相比Wan2.1基线去噪加速76.2倍且质量损失可忽略。更快变体HiStream+(i+ii+iii)实现107.5倍加速，在速度与质量间提供良好权衡。

Conclusion: HiStream框架通过系统性冗余减少，使高分辨率视频生成变得实用且可扩展，解决了扩散模型的计算瓶颈问题。
...
### [51] [Towards Optimal Performance and Action Consistency Guarantees in Dec-POMDPs with Inconsistent Beliefs and Limited Communication](https://arxiv.org/abs/2512.20778)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中多智能体通常因通信限制而持有不一致的信念，现有方法假设所有智能体在规划时具有相同信念（基于相同数据）不切实际，导致协调不良和次优甚至不安全的表现。

Method: 引入新颖的去中心化框架，明确考虑信念不一致性，为行动一致性和性能提供概率保证（相对于开环多智能体POMDP），选择性触发通信，并解决在选定联合行动后是否应共享数据以改进推理性能的问题。

Result: 仿真结果表明该方法优于最先进的算法。

Conclusion: 该框架有效解决了多智能体在信念不一致情况下的决策挑战，通过选择性通信平衡了性能与通信成本，为实际应用提供了实用解决方案。
...
### [52] [DAO-Agent: Zero Knowledge-Verified Incentives for Decentralized Multi-Agent Coordination](https://arxiv.org/abs/2512.20973)
*Yihan Xia,Taotao Wang,Wenxin Xu,Shengli Zhang*

Main category: cs.MA

TL;DR: DAO-Agent：一个结合DAO治理、零知识证明和混合架构的框架，用于在去中心化环境中实现可审计的LLM多智能体协作，同时保护策略隐私并大幅降低链上成本。


<details>
  <summary>Details</summary>
Motivation: 自主LLM多智能体系统在无信任环境中面临挑战：集中式协调难以确保透明的贡献度量和公平的激励分配，而区块链作为去中心化平台又会带来高计算成本和策略隐私泄露风险。

Method: 提出DAO-Agent框架，包含三个关键技术：1）链上DAO治理机制实现透明协调和不可变日志；2）零知识证明机制支持链下基于Shapley值的贡献度量；3）混合链上/链下架构以最小计算开销验证ZKP验证的贡献度量。

Result: 实验表明，DAO-Agent相比原生链上方案可减少99.9%的验证gas成本，验证复杂度为常数时间且随联盟规模增加保持稳定，为去中心化环境中的智能体协调提供了可扩展基础。

Conclusion: DAO-Agent框架成功解决了自主LLM智能体在无信任环境中的协调挑战，在保持策略隐私的同时实现了可审计的任务执行和公平的激励分配，显著降低了链上成本。
...
### [53] [A Plan Reuse Mechanism for LLM-Driven Agent](https://arxiv.org/abs/2512.21309)
*Guopeng Li,Ruiqi Wu,Haisheng Tan*

Main category: cs.MA

TL;DR: AgentReuse：一种LLM驱动智能体的计划重用机制，通过语义相似度评估和意图分类实现计划重用，显著降低延迟


<details>
  <summary>Details</summary>
Motivation: LLM驱动智能体在生成计划时存在数十秒的延迟问题，严重影响用户体验。研究发现约30%的用户请求相同或相似，这为计划重用提供了机会，但直接评估原始请求文本的相似性很困难，且自然语言表达多样性和计划文本的非结构化格式使得计划重用具有挑战性。

Method: 提出并实现了AgentReuse机制，利用请求语义的相似性和差异性，通过意图分类来评估请求之间的相似性，从而实现计划的重用。

Result: 基于真实世界数据集的实验结果显示，AgentReuse实现了93%的有效计划重用率，在评估请求相似性方面F1分数达到0.9718，准确率达到0.9459，相比不使用重用机制的基线方法，延迟降低了93.12%。

Conclusion: AgentReuse通过有效的计划重用机制，显著降低了LLM驱动智能体的响应延迟，提升了用户体验，为解决智能体系统中的延迟问题提供了有效方案。
...
### [54] [Fixed-time control with prescribed performance for path following of underwater gliders](https://arxiv.org/abs/2512.20748)
*Hanzhi Yang,Nina Mahmoudian*

Main category: eess.SY

TL;DR: 本文提出了一种用于水下滑翔机三维路径跟踪的固定时间规定性能控制方案，结合有限时间性能函数和固定时间控制框架，确保跟踪误差在规定性能边界内并在固定时间内收敛。


<details>
  <summary>Details</summary>
Motivation: 水下滑翔机在飓风季节观测和长期环境监测等挑战性任务中面临强流和湍流的导航安全风险，需要解决模型不确定性和环境扰动下的鲁棒控制问题。

Method: 采用固定时间规定性能控制方案，集成有限时间性能函数和固定时间控制框架；开发固定时间滑模扰动观测器进行扰动估计；结合iLOS制导律实现精确路径跟踪。

Result: 数值仿真表明，该方法在跟踪精度、收敛速度和控制平滑性方面优于传统滑模和规定性能控制器，验证了其在水下导航中的有效性。

Conclusion: 提出的控制方案能够确保水下滑翔机在模型不确定性和环境扰动下的安全导航，实现精确的路径跟踪，具有实际应用价值。
...
### [55] [Decentralized water-level balancing for irrigation channels in storage critical operations](https://arxiv.org/abs/2512.20966)
*Timm Strecker,Michael Cantoni*

Main category: eess.SY

TL;DR: 提出了一种用于开放水道水位控制的反馈控制系统，通过分散控制器调节闸门流量，平衡相邻水池水位误差，确保在不确定供需扰动下的稳定运行。


<details>
  <summary>Details</summary>
Motivation: 开放水道系统面临不确定的供需扰动，这些扰动可能超过单个水池的容量，导致水位偏离设定值。需要设计有效的控制策略来平衡相邻水池的水位差异，确保系统稳定运行。

Method: 采用分散式控制器调节水池间闸门流量，将相邻水位误差的加权差调节为零。开发了基于水道分布参数传递函数模型的顺序SISO环路整形设计方法，用于每个局部流量控制器设计。

Result: 通过支持性分析验证了相关性能规范下设计过程的递归可行性，以及所得MIMO闭环系统的稳定性。提供了数值模拟和现场试验结果作为验证。

Conclusion: 提出的反馈控制系统能有效处理开放水道中的不确定供需扰动，通过分散控制策略实现水位稳定控制，理论和实验验证了方法的可行性和有效性。
...
### [56] [Universal Transient Stability Analysis: A Large Language Model-Enabled Dynamics Prediction Framework](https://arxiv.org/abs/2512.20970)
*Chao Shen,Ke Zuo,Mingyang Sun*

Main category: eess.SY

TL;DR: TSA-LLM：基于大语言模型的通用暂态稳定分析框架，通过通道独立分解、参数高效微调和两阶段训练策略，实现多场景下的零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有暂态稳定分析预测框架缺乏"通用性"——单一预训练模型无法泛化到不同运行条件、未知故障和异构系统，需要为每个场景重新训练。

Method: 1) 数据预处理：通道独立分解解决维度异质性，样本归一化消除稳定/不稳定管道分离，时间分块实现高效长序列建模；2) 参数高效微调策略：冻结核心Transformer块，仅微调专用输入嵌入和输出投影层；3) 两阶段微调：结合教师强制（使用真实数据）和计划采样（逐步转向模型预测），减少长时域迭代预测的累积误差。

Result: 在仅使用新英格兰39节点系统训练的情况下，TSA-LLM实现了对混合稳定条件和未知故障的零样本泛化；在冰岛189节点系统上，仅用5%微调数据即可达到专家性能水平。

Conclusion: TSA-LLM验证了通用暂态稳定分析框架的可行性，通过大规模参数和跨场景训练数据实现可扩展性，消除了场景特定重新训练的需求。
...
### [57] [Dyna-Style Reinforcement Learning Modeling and Control of Non-linear Dynamics](https://arxiv.org/abs/2512.21081)
*Karim Abdelsalam,Zeyad Gamal,Ayman El-Badawy*

Main category: eess.SY

TL;DR: 提出一种结合SINDy数据驱动建模与TD3强化学习的Dyna风格控制框架，用于高效控制非线性系统，并在双旋翼系统上验证了其优越性能。


<details>
  <summary>Details</summary>
Motivation: 控制具有复杂非线性动力学的系统面临重大挑战，传统模型无关强化学习方法样本效率低，需要大量数据。需要开发既能高效学习又能准确控制非线性系统的框架。

Method: 提出SINDy-TD3混合框架：1) 使用SINDy从数据中识别系统非线性动力学模型；2) 利用该模型生成合成数据rollouts；3) 在真实环境训练期间，定期将合成数据注入TD3强化学习的回放缓冲区；4) 在双旋翼系统上进行案例研究，评估稳定性和轨迹跟踪性能。

Result: SINDy-TD3方法在双旋翼系统的稳定性和轨迹跟踪任务中表现出色，相比直接强化学习技术，具有更高的准确性和鲁棒性，有效解决了样本效率问题。

Conclusion: 结合数据驱动建模与强化学习的混合方法为复杂动力系统控制提供了有效解决方案，SINDy-TD3框架在有限数据条件下实现了高效、鲁棒的控制性能，展示了该方法的潜力。
...
### [58] [Safe Navigation with Zonotopic Tubes: An Elastic Tube-based MPC Framework](https://arxiv.org/abs/2512.21198)
*Niyousha Ghiasi,Bahare Kiumarsi,Hamidreza Modares*

Main category: eess.SY

TL;DR: 提出了一种基于弹性管的模型预测控制框架，用于处理未知离散时间线性系统，无需精确系统模型或扰动边界知识，通过数据驱动和先验知识迭代优化控制性能。


<details>
  <summary>Details</summary>
Motivation: 现有弹性管MPC方法通常假设系统模型和扰动边界已知，但在实际应用中这些信息往往不完整。本文旨在开发一种不依赖精确模型和扰动边界知识的自适应控制框架，提高MPC在未知系统中的可行性和性能。

Method: 采用数据驱动的矩阵zonotope模型集识别系统动态，结合先验物理知识剔除不一致的模型和扰动。通过分离开环模型失配和闭环效应，减少保守性。自适应协同设计管和辅助反馈，确保λ-收缩zonotopic管，保证鲁棒正不变性。

Result: 建立了递归可行性条件，引入多面体Lyapunov候选函数证明闭环误差动态的指数稳定性。仿真结果表明该方法提高了鲁棒性、扩大了可行域，仅需少量在线数据即可实现安全的闭环性能。

Conclusion: 提出的自适应弹性管MPC框架成功解决了未知线性系统的控制问题，通过数据驱动和先验知识的融合，在不需要精确模型和扰动边界的情况下实现了鲁棒、可行的控制性能。
...
### [59] [A Multimodal Human-Centered Framework for Assessing Pedestrian Well-Being in the Wild](https://arxiv.org/abs/2512.21200)
*Yasaman Hakiminejad,Arash Tavakoli*

Main category: eess.SY

TL;DR: 本文提出了一种多模态、以人为中心的框架，通过整合生理传感、地理空间追踪和瞬时自我报告三种数据流，在真实环境中评估行人幸福感。


<details>
  <summary>Details</summary>
Motivation: 现有评估行人环境的方法主要依赖静态基础设施指标或回顾性调查，忽视了日常步行体验的动态性、主观性和心理生理维度。行人幸福感作为可持续城市交通和宜居城市设计的关键组成部分，目前缺乏有效的测量方法。

Method: 开发了一个多模态框架，整合三种互补数据流：1）连续生理传感（心率变异性和皮肤电活动）；2）地理空间追踪（GPS）；3）使用经验取样法的瞬时自我报告（压力、情感和基础设施感知）。在费城地区进行自然案例研究，参与者佩戴研究级可穿戴传感器和GPS智能手机进行日常活动。

Result: 结果显示主观体验和生理反应存在显著的个体间和个体内变异性。发现了与交通暴露、行人基础设施质量和环境围合度相关的环境依赖模式。常用步行性指数可能无法完全捕捉行人幸福感的体验维度。

Conclusion: 该框架通过实现真实世界中行人体验的多模态测量，为推进以人为中心的城市分析提供了可扩展和可转移的方法，有助于更全面地理解城市环境对行人幸福感的影响。
...
### [60] [ARX-Implementation of encrypted nonlinear dynamic controllers using observer form](https://arxiv.org/abs/2512.21244)
*Deuksun Hong,Donghyeon Song,Mingyu Jeong,Junsoo Kim*

Main category: eess.SY

TL;DR: 提出一种将状态空间系统模型转换为ARX模型的方法，以解决加密控制系统中递归操作次数有限的问题，使非线性动态控制能够在有限次加密运算下实现。


<details>
  <summary>Details</summary>
Motivation: 计算密码系统应用于控制系统虽然提高了安全性和隐私性，但大多数情况下加密数据的递归操作次数有限，特别是在需要快速计算的场景中。这限制了非线性动态控制的实现。

Method: 提出将状态空间系统模型表示为带外生输入的自回归模型（ARX模型）的方法。通过将工厂的输入和输出加密传输到控制器，重新表述的ARX形式仅使用有限次操作计算每个输出。该方法将控制器替换为观测器形式，并应用类似于有限脉冲响应逼近的方法。

Result: 验证了通过适当选择参数，在观测器和闭环系统稳定的条件下，逼近误差及其影响可以任意小。仿真结果证明了所提方法的有效性。
...
### [61] [A Lyapunov-Based Small-Gain Theorem for Fixed-Time ISS: Theory, Optimization, and Games](https://arxiv.org/abs/2512.21314)
*Michael Tang,Miroslav Krstic,Jorge Poveda*

Main category: eess.SY

TL;DR: 本文提出了一个基于Lyapunov的小增益定理，用于建立互联非线性动力系统的固定时间输入到状态稳定性（FxT-ISS）保证。


<details>
  <summary>Details</summary>
Motivation: 现有Lyapunov小增益定理主要针对渐近稳定性和有限时间稳定性，缺乏对固定时间稳定性的系统分析框架。本文旨在填补这一空白，为具有固定时间稳定性的互联系统提供理论分析工具。

Method: 开发了一个基于Lyapunov的小增益定理框架，考虑每个子系统都允许FxT-ISS Lyapunov函数的情况。在适当的非线性小增益条件下，证明整个互联系统继承FxT-ISS特性。

Result: 提出的理论框架能够系统分析具有固定时间稳定性的互联结构。通过两个应用实例验证了理论的有效性：具有时变成本函数的反馈优化问题，以及具有非线性动力系统的非合作博弈纳什均衡寻求问题。

Conclusion: 本文提出的Lyapunov小增益定理为固定时间输入到状态稳定性提供了系统分析框架，补充了现有渐近和有限时间稳定性的相关理论，并在优化和博弈控制问题中展示了实际应用价值。
...
### [62] [MegaRAG: Multimodal Knowledge Graph-Based Retrieval Augmented Generation](https://arxiv.org/abs/2512.20626)
*Chi-Hsiang Hsiao,Yi-Cheng Wang,Tzung-Sheng Lin,Yi-Ren Yeh,Chu-Song Chen*

Main category: cs.AI

TL;DR: 本文提出了一种多模态知识图谱增强的检索增强生成方法，通过整合视觉线索到知识图谱构建、检索和答案生成过程，提升了跨模态推理能力，在文本和多模态语料库的问答任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统检索增强生成方法在处理长篇领域特定内容（如整本书）时存在局限性，主要受限于上下文窗口大小，难以进行深度推理。现有的知识图谱增强方法仅支持文本输入，未能利用视觉等其他模态的互补信息，而视觉文档的理解需要整合文本、视觉和空间线索。

Method: 提出多模态知识图谱增强的检索增强生成框架，将视觉线索整合到知识图谱构建、检索阶段和答案生成过程中，实现跨模态推理以提升内容理解能力。

Result: 实验结果表明，该方法在全局和细粒度问答任务中，在文本和多模态语料库上均一致优于现有的检索增强生成方法。

Conclusion: 通过整合视觉线索到知识图谱增强的检索增强生成框架，能够有效提升跨模态推理能力，解决传统方法在处理长篇领域特定内容和多模态文档时的局限性。
...
### [63] [Proceedings of the 20th International Conference on Knowledge, Information and Creativity Support Systems (KICSS 2025)](https://arxiv.org/abs/2512.20628)
...
Result: 成功举办了第20届国际知识、信息与创造力支持系统会议，收录了经过严格评审的学术论文

Conclusion: KICSS 2025会议为相关领域研究者提供了高质量的学术交流平台，论文集记录了当前研究进展，部分优秀成果将扩展发表至期刊
...
### [64] [MAR:Multi-Agent Reflexion Improves Reasoning Abilities in LLMs](https://arxiv.org/abs/2512.20845)
*Onat Ozer,Grace Wu,Yuchen Wang,Daniel Dosti,Honghao Zhang,Vivi De La Rue*

Main category: cs.AI

TL;DR: 论文提出使用多智能体多角色辩论方法来生成反思，相比单一LLM自我反思，能提高反思多样性并避免思维退化，在HotPot QA和HumanEval任务上取得更好性能


<details>
  <summary>Details</summary>
Motivation: LLMs通过反思错误可以提高推理任务性能，但单一LLM的持续自我反思会导致思维退化，重复相同错误。需要解决反思多样性和避免思维固化的问题

Method: 引入多智能体多角色辩论方法生成反思，通过不同角色和视角的智能体进行辩论，产生更丰富的反思内容

Result: 在HotPot QA上达到47% EM准确率，在HumanEval上达到82.7%准确率，均超过单一LLM反思方法

Conclusion: 多智能体多角色辩论方法能有效提高反思多样性，避免思维退化，在复杂推理和编程任务上表现优于单一LLM自我反思
...
### [65] [MicroProbe: Efficient Reliability Assessment for Foundation Models with Minimal Data](https://arxiv.org/abs/2512.20630)
...
<details>
  <summary>Details</summary>
Motivation: 基础模型可靠性评估通常需要数千个评估样本，计算成本高且耗时，难以满足实际部署需求，需要一种更高效的评估方法。

Method: 结合五个关键可靠性维度的战略提示多样性、先进的不确定性量化以及自适应加权，通过战略选择的100个探测样本高效检测潜在故障模式。

Result: 在多个语言模型（GPT-2变体）和跨领域验证（医疗、金融、法律）中，microprobe相比随机采样基线实现了23.5%更高的综合可靠性分数，具有显著的统计显著性（p < 0.001，Cohen's d = 1.21）。专家验证评分4.14/5.0（随机选择为3.14/5.0），评估成本降低90%，覆盖率达到传统方法的95%。

Conclusion: Microprobe填补了负责任AI部署中高效模型评估的关键空白，能够以极低的成本和时间完成具有统计显著性的可靠性评估。
...
### [66] [Erkang-Diagnosis-1.1 Technical Report](https://arxiv.org/abs/2512.20632)
...
Conclusion: Erkang-Diagnosis-1.1是一个安全、可靠、专业的AI健康顾问，能够作为用户的智能健康伴侣，有效赋能基层医疗和健康管理。
...
### [67] [A Blockchain-Monitored Agentic AI Architecture for Trusted Perception-Reasoning-Action Pipelines](https://arxiv.org/abs/2512.20985)
*Salman Jan,Hassan Ali Razzaqi,Ali Akarma,Mohammad Riyaz Belgaum*

Main category: cs.AI

TL;DR: 提出一个结合LangChain多智能体系统和许可区块链的架构，用于确保自主AI系统的监控、策略执行和不可变审计，在智能库存管理、交通信号控制和医疗监测等场景中验证了有效性。
...
Result: 实验涵盖智能库存管理、交通信号控制和医疗监测场景。结果表明，区块链安全验证能有效防止未经授权的操作，提供整个决策过程的可追溯性，并将操作延迟保持在合理范围内。
...
### [68] [Reasoning Relay: Evaluating Stability and Interchangeability of Large Language Models in Mathematical Reasoning](https://arxiv.org/abs/2512.20647)
*Leo Lu,Jonathan Zhang,Sean Chua,Spencer Kim,Kevin Zhu,Sean O'Brien,Vasu Sharma*

Main category: cs.AI

TL;DR: 本文探索不同大语言模型间的推理链可互换性，研究一个模型的部分推理链能否被另一个模型可靠地继续完成，并评估这种互换性对推理可信度和准确性的影响。


<details>
  <summary>Details</summary>
Motivation: 虽然CoT提示显著提升了LLMs的推理能力，但先前研究主要关注通过内部推理策略提升模型性能，对于不同模型间推理的可互换性知之甚少。本文旨在探索一个模型的部分推理链能否被另一个模型可靠地继续完成，无论是同一模型家族内还是跨家族，以此作为检验推理时可信度的手段。

Method: 使用Gemma-3-4B-IT和LLaMA-3.1-70B-Instruct作为基线模型，通过词元级对数概率阈值在推理的早期、中期和晚期阶段截断推理链。然后用Gemma-3-1B-IT和LLaMA-3.1-8B-Instruct进行继续实验，测试家族内和跨家族行为。评估流程结合截断阈值和过程奖励模型(PRM)，提供可复现的框架来评估推理稳定性。

Result: 使用PRM评估发现，混合推理链通常能够保持，在某些情况下甚至能提高最终准确性和逻辑结构。这表明推理链的可互换性成为推理模型的一种新兴行为特性。

Conclusion: 推理可互换性是推理模型的一个新兴行为特性，为协作AI系统中可靠的模块化推理提供了新范式，展示了不同模型间推理链可以相互衔接并保持甚至提升推理质量。
...
### [69] [From Pilots to Practices: A Scoping Review of GenAI-Enabled Personalization in Computer Science Education](https://arxiv.org/abs/2512.20714)
...
<details>
  <summary>Details</summary>
Motivation: 生成式AI能够实现大规模个性化计算机科学教育，但需要研究这种个性化是支持还是削弱学习效果。本文旨在通过综述现有研究，绘制个性化机制和有效性信号，为教育实践提供指导。

Method: 采用范围综述方法，从259条记录中有目的地抽样32项研究（2023-2025年），分析高等教育计算机科学背景下的个性化机制和有效性信号，识别应用领域和设计选择如何影响学习成果。

Result: 识别了五个应用领域：智能辅导、个性化材料、形成性反馈、AI增强评估和代码审查。发现采用解释优先指导、解决方案保留、分级提示阶梯和基于学生作品（代码、测试、评分标准）的设计比无约束聊天界面显示出更积极的学习过程。成功实施共享四种模式：基于学生作品的上下文感知辅导、需要反思的多级提示结构、与传统CS基础设施（自动评分器和评分标准）结合、以及人在环路的质保。

Conclusion: 生成式AI可以作为精确脚手架机制，但需要嵌入可审计的工作流程中，在扩展个性化支持的同时保留学生的有效努力。提出了探索优先的采用框架，强调试点、工具化、保护学习的默认设置和基于证据的扩展。同时识别了学术诚信、隐私、偏见与公平、过度依赖等风险及相应缓解措施。
...
### [70] [FinAgent: An Agentic AI Framework Integrating Personal Finance and Nutrition Planning](https://arxiv.org/abs/2512.20991)
*Toqeer Ali Syed,Abdulaziz Alshahrani,Ali Ullah,Ali Akarma,Sohail Khan,Muhammad Nauman,Salman Jan*

Main category: cs.AI

TL;DR: 该论文提出了一种价格感知的智能AI系统，结合个人财务管理与饮食优化，为中等收入家庭在食品价格波动环境下提供营养充足且成本合理的膳食计划。


<details>
  <summary>Details</summary>
Motivation: 中等收入家庭面临预算有限与营养需求的双重挑战，特别是在食品价格波动的环境中。现有膳食规划方法往往无法同时兼顾成本控制、营养充足性和对市场价格变化的适应性。

Method: 采用模块化多智能体架构，包含预算、营养、价格监控和健康个性化等特定智能体。这些智能体共享知识库，并使用替代图来确保以最低成本维持营养质量。系统整合家庭收入、固定支出、医疗健康状况以及实时食品成本数据。

Result: 基于沙特家庭案例的模拟显示：相对于静态每周菜单，成本持续降低12-18%；营养充足性超过95%；在20-30%的价格变化下仍保持高性能。

Conclusion: 该框架能够在本地将经济性与营养充足性相结合，为实现可持续和公平的膳食规划提供可行途径，符合联合国可持续发展目标中的零饥饿和良好健康目标。
...
### [71] [AIAuditTrack: A Framework for AI Security system](https://arxiv.org/abs/2512.20649)
*Zixun Luo,Yuhang Fan,Yufei Li,Youzhi Zhang,Hengyu Lin,Ziqi Wang*

Main category: cs.AI

TL;DR: AiAuditTrack (AAT) 是一个基于区块链的框架，用于记录AI使用流量和治理，通过去中心化身份和可验证凭证建立可信的AI实体身份，并在链上记录实体间交互轨迹，实现跨系统监督和审计。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型驱动的AI应用快速扩张，AI交互数据激增，引发了安全、问责和风险追溯方面的紧迫挑战。需要一种能够确保AI使用可追溯、可审计的解决方案。

Method: AAT采用去中心化身份(DID)和可验证凭证(VC)建立可信的AI实体身份，将AI实体建模为动态交互图中的节点，边表示时间特定的行为轨迹。提出风险扩散算法来追踪风险行为源头，并在相关实体间传播早期预警。

Result: 系统性能通过区块链TPS指标评估，证明AAT在大规模交互记录下的可行性和稳定性。该框架为复杂多智能体环境中的AI审计、风险管理和责任归属提供了可扩展且可验证的解决方案。

Conclusion: AAT提供了一个基于区块链的框架，能够有效解决AI交互数据的审计、风险管理和责任追溯问题，为AI治理提供了可扩展且可验证的技术方案。
...
### [72] [Mixture of Attention Schemes (MoAS): Learning to Route Between MHA, GQA, and MQA](https://arxiv.org/abs/2512.20650)
*Esmail Gumaan*
...
Method: 提出了混合注意力方案（MoAS）架构，通过学习的路由器为每个token动态选择最优的注意力方案（MHA、GQA或MQA），动态路由比静态方案平均效果更好。

Result: 在WikiText-2上的实验结果显示，动态路由（验证损失2.3074）优于静态混合（2.3093），验证了该方法的有效性，同时保持了与MHA基线竞争的性能并提供了条件计算效率的潜力。

Conclusion: MoAS通过动态路由机制成功解决了注意力机制选择中的质量-效率权衡问题，为Transformer模型提供了更灵活的架构设计，在保持性能的同时优化推理效率。
...
### [73] [Memory Bear AI A Breakthrough from Memory to Cognition Toward Artificial General Intelligence](https://arxiv.org/abs/2512.20651)
*Deliang Wen,Ke Sun*

Main category: cs.AI

TL;DR: Memory Bear系统通过构建类人记忆架构，解决LLM在内存限制、长期知识遗忘、信息冗余和幻觉生成等问题，在医疗、企业运营和教育等领域实现性能突破。
...
Result: 在医疗、企业运营和教育等多个领域展示显著工程创新和性能突破，相比现有解决方案（如Mem0、MemGPT、Graphiti），在准确性、token效率和响应延迟等关键指标上表现更优。

Conclusion: Memory Bear系统显著提升了长期对话中的知识保真度和检索效率，降低了幻觉率，并通过记忆-认知集成增强了上下文适应性和推理能力，标志着AI从"记忆"向"认知"迈进的关键一步。
...
### [74] [From Fake Focus to Real Precision: Confusion-Driven Adversarial Attention Learning in Transformers](https://arxiv.org/abs/2512.20661)
*Yawei Liu*

Main category: cs.AI

TL;DR: 提出AFA对抗性反馈注意力训练机制，通过动态掩码策略和策略梯度优化，使Transformer模型自动将注意力重新分配到任务相关的关键词上，提升情感分析性能。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer模型在情感分析任务中常将注意力过度集中在常见词汇上，而忽略了不常见但高度任务相关的词汇，这显著影响了模型性能。需要一种无需人工标注就能自动优化注意力分布的方法。

Method: 提出对抗性反馈注意力(AFA)训练机制：1) 动态掩码策略：尝试掩码不同词汇来欺骗判别器；2) 判别器检测掩码引起的显著差异；3) 利用Transformer对token级扰动的敏感性，采用策略梯度方法优化注意力分布，实现高效快速收敛。

Result: 在三个公开数据集上实现了最先进的结果。将该训练机制应用于增强大型语言模型的注意力，进一步获得了12.6%的性能提升。

Conclusion: AFA机制能够有效解决Transformer模型注意力分配不均衡的问题，通过对抗性训练自动优化注意力分布，显著提升情感分析任务的性能，且可扩展到大型语言模型中。
...
### [75] [Quantifying Laziness, Decoding Suboptimality, and Context Degradation in Large Language Models](https://arxiv.org/abs/2512.20662)
*Yiqing Ma,Jung-Hua Liu*

Main category: cs.AI

TL;DR: 研究发现大型语言模型存在懒惰行为（不完整执行多部分指令），但在解码次优性和上下文退化方面表现优于预期，建议采用自优化和动态提示策略改善指令遵循
...
Method: 通过三个受控实验（A、B、C）量化这些现象，测试多个先进LLM（OpenAI GPT-4变体、DeepSeek）。实验A评估多部分指令遵循，实验B测试简单推理任务中的解码质量，实验C进行200轮混乱对话测试上下文保持能力

Result: 发现普遍存在的懒惰行为：模型经常省略必要部分或未能满足长度要求。但在简单推理任务中解码次优性证据有限（贪婪答案与最高置信度解决方案一致），且在200轮混乱对话测试中表现出意外的上下文保持鲁棒性，能比预期更好地维持关键事实和指令

Conclusion: 详细指令遵循仍是开放挑战，但现代LLM在内部缓解了一些假设的失效模式（如上下文遗忘）。建议采用自优化和动态提示等策略减少懒惰性并增强多指令遵循能力
...
### [76] [Bridging the AI Trustworthiness Gap between Functions and Norms](https://arxiv.org/abs/2512.20671)
*Daan Di Scala,Sophie Lathouwers,Michael van Bekkum*

Main category: cs.AI

TL;DR: 该立场论文指出可信人工智能（TAI）中功能性TAI（FTAI）与规范性TAI（NTAI）之间存在鸿沟，提出需要开发一种语义语言来桥接两者，帮助开发者评估AI系统的可信度，并将规范转化为具体实施步骤。


<details>
  <summary>Details</summary>
Motivation: 由于法规和功能优势，可信人工智能（TAI）正受到关注。然而，功能性TAI（关注如何实施可信系统）与规范性TAI（关注需要执行的法规）之间存在鸿沟，使得评估AI系统的可信度变得困难。

Method: 作者提出开发一种概念性语义语言来桥接FTAI和NTAI。这种语言可以作为框架帮助开发者评估AI系统的可信度，并协助利益相关者将规范和法规转化为系统的具体实施步骤。

Result: 论文描述了当前最先进的研究现状，识别了FTAI和NTAI之间的鸿沟，讨论了开发语义语言的起点及其预期效果，并提供了关键考虑因素。

Conclusion: 需要开发一种语义语言来桥接功能性TAI和规范性TAI之间的鸿沟，以促进可信人工智能的评估和实施。论文提供了未来行动的关键考虑因素和方向。
...
### [77] [AgentMath: Empowering Mathematical Reasoning for Large Language Models via Tool-Augmented Agent](https://arxiv.org/abs/2512.20745)
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型在自然语言推理方面取得了显著进展，但在处理需要复杂数学运算的问题时仍然计算效率低下且准确性不足，需要更有效的解决方案。

Method: 提出了三个关键创新：1) 将自然语言思维链自动转换为结构化工具增强轨迹，生成高质量SFT数据；2) 新颖的智能体强化学习范式，动态交织自然语言生成与实时代码执行；3) 高效训练系统，包含异步调度、部分回滚等技术，实现4-5倍加速。

Result: AgentMath在AIME24、AIME25和HMMT25等数学竞赛基准上达到最先进性能，AgentMath-30B-A3B分别获得90.6%、86.4%和73.8%的准确率。
...
### [78] [Context-Sensitive Abstractions for Reinforcement Learning with Parameterized Actions](https://arxiv.org/abs/2512.20831)
*Rashmeet Kaur Nayyar,Naman Shah,Siddharth Srivastava*

Main category: cs.AI

TL;DR: 本文提出了一种在参数化动作空间中学习状态和动作抽象的方法，通过在线渐进细化抽象来提高稀疏奖励、长视野RL任务的样本效率。


<details>
  <summary>Details</summary>
Motivation: 现实世界的顺序决策通常涉及参数化动作空间，需要同时处理离散动作选择和连续动作参数决策。现有方法存在严重限制：规划方法需要手动构建动作模型，标准RL算法只适用于离散或连续动作，而少数处理参数化动作的RL方法依赖领域特定工程且未能利用这些空间的潜在结构。

Method: 提出算法使智能体能够在线自主学习状态和动作抽象，并在学习过程中渐进细化这些抽象，在状态-动作空间的关键区域增加细粒度细节，从而提高性能分辨率。

Result: 在多个连续状态、参数化动作领域中，这种抽象驱动的方法使TD(λ)算法实现了比最先进基线方法显著更高的样本效率。

Conclusion: 通过扩展RL算法到参数化动作空间的长视野、稀疏奖励设置，使智能体能够自主学习状态和动作抽象，显著提高了样本效率，解决了现有方法在参数化动作空间中的局限性。
...
TL;DR: 提出一个概率框架解决LLM智能体知识交换中的认知不对称问题，通过Beta-Bernoulli分布建模信念，引入遗忘因子γ，将公开贡献重构为最优主动学习


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM和RAG的自主智能体存在认知不对称问题，它们是数字内容的单向消费者，缺乏双向知识交换机制。这种隔离导致冗余推理和集体智能停滞。现有的自我反思框架大多是启发式和私有的，缺乏量化确定性或证明外部交互合理性的概率基础。

Method: 提出一个形式化的概率框架，使用带遗忘因子γ的Beta-Bernoulli分布对智能体在命题上的信念进行建模。通过信念方差分离认知不确定性，建立交互的双重驱动力：稳态动机（维持确定性对抗时间衰减）和最优学习策略（针对最大模糊点）。引入认知缓存机制，利用遗忘因子动态优先处理非平稳知识分布的活跃头部资源。

Result: 模拟结果表明，这种不确定性驱动策略在异构（Zipfian）环境中显著优于随机基线，保持对概念漂移的高适应性。积累的信念状态可作为RLHF的可验证奖励信号和SFT的高质量数据过滤器。

Conclusion: 该框架为智能体提供了非利他主义的双向知识交换动机，将公开贡献重构为最优主动学习：分享解决方案以获取反馈是智能体减少自身不确定性的最有效方法。认知缓存确保了系统的可扩展性。
...
### [80] [RoboSafe: Safeguarding Embodied Agents via Executable Safety Logic](https://arxiv.org/abs/2512.21220)
*Le Wang,Zonghao Ying,Xiao Yang,Quanchen Zou,Zhenfei Yin,Tianlin Li,Jian Yang,Yaodong Yang,Aishan Liu,Xianglong Liu*

Main category: cs.AI

TL;DR: RoboSafe：一种通过可执行谓词安全逻辑为具身智能体提供混合推理运行时安全防护的系统，结合后向反思推理和前向预测推理，显著减少危险行为


<details>
  <summary>Details</summary>
Motivation: 基于视觉语言模型的具身智能体在执行复杂现实任务时容易受到危险指令的影响，现有基于静态规则过滤或提示级控制的防御方法难以处理动态、时间依赖和上下文丰富的环境中的隐含风险

Method: 提出RoboSafe系统，包含两个互补推理模块：1）后向反思推理模块，持续回顾短期记忆中的轨迹以推断时间安全谓词，检测到违规时主动触发重新规划；2）前向预测推理模块，通过从长期安全记忆和多模态观察中生成上下文感知的安全谓词来预测即将到来的风险。这些组件在混合长短安全记忆上运行，形成可验证、可解释且可执行的安全逻辑

Result: 在多个智能体上的广泛实验表明，RoboSafe相比领先基线显著减少了危险行为（风险发生率降低36.8%），同时保持接近原始的任务性能。物理机械臂的真实世界评估进一步证实了其实用性

Conclusion: RoboSafe通过混合推理和可执行谓词安全逻辑为具身智能体提供了有效的运行时安全防护，能够适应动态环境中的隐含风险，在保证安全的同时维持任务性能
...
### [81] [TrafficSimAgent: A Hierarchical Agent Framework for Autonomous Traffic Simulation with MCP Control](https://arxiv.org/abs/2512.20996)
...
<details>
  <summary>Details</summary>
Motivation: 现有交通仿真平台如SUMO和MATSim功能全面，但非专业用户难以从头开始实验并将其应用于日常工作，需要简化使用门槛。
...
Conclusion: TrafficSimAgent成功解决了非专业用户使用复杂交通仿真平台的挑战，通过LLM驱动的专家智能体协作实现了灵活高效的实验设计和决策优化。
...
### [82] [LLM Personas as a Substitute for Field Experiments in Method Benchmarking](https://arxiv.org/abs/2512.21080)
*Enoch Hyunwook Kang*

Main category: cs.AI

TL;DR: 论文证明了在特定条件下（仅观察聚合结果、算法盲评估），用LLM角色模拟替代人类进行A/B测试是有效的基准接口替代方案，并给出了信息论框架来量化所需样本量。


<details>
  <summary>Details</summary>
Motivation: A/B测试虽然是最可信的社会系统方法基准，但成本高、延迟长，限制了迭代方法开发。LLM角色模拟提供了廉价替代方案，但需要验证这种替代是否保持了基准接口的有效性。

Method: 提出理论证明：在仅观察聚合结果和算法盲评估条件下，角色模拟与人类测试在方法视角下等价。定义信息论可区分性概念，量化角色基准与实地实验的决策相关性，推导出可靠区分不同方法所需独立角色评估数量的明确界限。

Result: 证明了当且仅当条件成立时，角色模拟与人类测试在方法优化视角下无法区分。建立了信息论框架，表明使角色基准与实地实验同等决策相关本质上是一个样本量问题，给出了具体样本量界限。

Conclusion: 在特定约束条件下，LLM角色模拟可以作为A/B测试的有效替代基准，其有效性取决于样本量。这为廉价、快速的方法开发提供了理论依据，同时明确了角色基准的适用范围和局限性。
...
### [83] [Beyond Context: Large Language Models Failure to Grasp Users Intent](https://arxiv.org/abs/2512.21110)
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM安全方法存在根本性缺陷，只关注显性有害内容而忽略了上下文理解和意图识别这一关键漏洞，这为恶意用户提供了可系统利用的规避途径。

Method: 对多个最先进的LLM（包括ChatGPT、Claude、Gemini和DeepSeek）进行实证评估，分析通过情感框架、渐进式揭示和学术论证等技术绕过可靠安全机制的情况。

Result: 推理增强配置反而放大了利用效果，提高了事实精确性但未能质疑潜在意图。唯一例外是Claude Opus 4.1，在某些用例中优先考虑意图检测而非信息提供。这些模式显示当前架构设计存在系统性漏洞。

Conclusion: 当前局限性需要范式转变，将上下文理解和意图识别作为核心安全能力，而非事后保护机制。
...
### [84] [A Real-World Evaluation of LLM Medication Safety Reviews in NHS Primary Care](https://arxiv.org/abs/2512.21127)
...
TL;DR: 该研究首次在真实NHS初级保健数据上评估基于大语言模型的用药安全审查系统，发现虽然系统在识别临床问题方面表现良好（敏感性100%），但仅在46.9%的患者中正确识别所有问题和干预措施，主要失败机制是情境推理而非药物知识缺失。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在医学基准测试中常达到或超过临床医生水平，但很少在真实临床数据上评估或深入分析超越表面指标的表现。本研究旨在填补这一空白，评估LLM在真实NHS初级保健数据上的用药安全审查能力。

Method: 在回顾性研究中，使用覆盖NHS Cheshire和Merseyside地区2,125,549名成人的大规模电子健康记录，战略性地抽样患者以捕捉广泛的临床复杂性和用药安全风险，经过数据质量排除后得到277名患者。专家临床医生审查这些患者，并对系统识别的问题和提出的干预措施进行分级。

Result: 主要LLM系统在识别临床问题存在方面表现强劲（敏感性100%，特异性83.1%），但仅在46.9%的患者中正确识别所有问题和干预措施。失败分析显示主要失败机制是情境推理而非药物知识缺失，存在五种主要失败模式：对不确定性的过度自信、未根据患者情境调整标准指南、误解医疗实践方式、事实错误和流程盲点。

Conclusion: 这项研究揭示了基于LLM的临床AI安全部署前必须解决的缺陷，强调了需要进行更大规模的前瞻性评估，并深入研究LLM在临床环境中的行为模式。研究提供了45个详细案例，全面覆盖所有识别出的失败情况。
...
### [85] [SACodec: Asymmetric Quantization with Semantic Anchoring for Low-Bitrate High-Fidelity Neural Speech Codecs](https://arxiv.org/abs/2512.20944)
*Zhongren Dong,Bin Wang,Jing Han,Haotian Guo,Xiaojun Mo,Yimin Cao,Zixing Zhang*

Main category: cs.SD

TL;DR: SACodec是一种新型神经语音编解码器，通过语义锚定机制和双量化器设计，在低比特率（1.5kbps）下同时保持声学保真度和语义丰富性。


<details>
  <summary>Details</summary>
Motivation: 传统神经语音编解码器在低比特率下存在基本权衡：保持声学保真度往往会损害语义丰富性，需要一种能够同时优化这两个方面的解决方案。

Method: 提出SACodec，基于非对称双量化器架构，采用语义锚定机制。通过轻量级投影器将声学特征与冻结的大规模mHuBERT码本对齐，注入语言先验；然后使用带SimVQ的残差激活模块的单层量化器恢复细粒度声学细节。

Result: 在1.5kbps比特率下，SACodec在保真度和语义丰富性方面均达到新的最先进水平：主观听力测试显示其重建质量在感知上与原始音频高度可比，同时其token在下游任务中表现出显著改善的语义丰富性。

Conclusion: SACodec通过语义锚定机制成功解决了低比特率下声学保真度与语义丰富性之间的权衡问题，为低比特率神经语音编解码提供了有效解决方案。
...
### [86] [Towards Practical Automatic Piano Reduction using BERT with Semi-supervised Learning](https://arxiv.org/abs/2512.21324)
*Wan Ki Wong,Ka Ho To,Chuck-jee Chau,Lucas Wong,Kevin Y. Yip,Irwin King*

Main category: cs.SD

TL;DR: 提出了一种基于半监督学习的自动钢琴缩编方法，通过音乐简化和和声化两步流程，利用MidiBERT框架实现，能在少量标注数据下生成实用的钢琴缩编结果。


<details>
  <summary>Details</summary>
Motivation: 钢琴缩编是重要的音乐转换过程，但手动制作耗时耗力。虽然监督学习可用于学习输入输出映射，但难以获得大量标注数据。本研究旨在利用半监督学习，充分利用古典音乐中丰富的无标注数据，减少标注工作量。

Method: 采用两步法：音乐简化后和声化。提出并实现了两种基于MidiBERT机器学习框架的解决方案，利用半监督学习方法处理钢琴缩编任务。

Result: 提出的方法能够输出实用且真实的钢琴缩编样本，生成的缩编结果准确，仅需少量后处理调整。为半监督学习在自动钢琴缩编中的应用奠定了基础。

Conclusion: 本研究为自动钢琴缩编中的半监督学习应用建立了基础框架，未来研究者可参考此工作产生更先进的结果，为解决标注数据稀缺问题提供了有效途径。
...
### [87] [Efficient Computation of Integer-constrained Cones for Conformal Parameterizations](https://arxiv.org/abs/2512.20904)
...
<details>
  <summary>Details</summary>
Motivation: 需要高效生成旋转无缝的共形参数化，同时控制锥奇异点的数量和参数化失真，现有方法计算成本高

Method: 交替优化三种离散变量（顶点约束位置、整数约束角度、锥数量）；采用显式构造算法减少优化规模；推导新导数公式移动锥点；结合重新定位、添加锥点、自适应选择约束变量、配对锥点等策略

Result: 在大规模测试数据集上成功生成旋转无缝且低失真的参数化；相比最先进方法实现平均30倍的加速，同时保持相当的锥数量和参数化失真

Conclusion: 该方法在锥数量和参数化失真之间实现了有利的权衡，证明了所提出锥奇异点计算方法的有效性和实用性
...
### [88] [AirGS: Real-Time 4D Gaussian Streaming for Free-Viewpoint Video Experiences](https://arxiv.org/abs/2512.20943)
*Zhe Wang,Jinghang Li,Yifei Zhu*

Main category: cs.GR

TL;DR: AirGS是一个针对4D高斯泼溅的流媒体优化框架，通过重新设计训练和传输流程，解决了现有方法在长序列中的质量下降、带宽和存储开销大的问题，实现了高质量、低延迟的自由视点视频体验。


<details>
  <summary>Details</summary>
Motivation: 现有4D高斯泼溅方法在长序列中会出现质量下降，同时需要大量带宽和存储开销，限制了其在实时和大规模部署中的应用。需要一种流媒体优化的解决方案来支持高质量、低延迟的自由视点视频体验。

Method: 1. 将高斯视频流转换为多通道2D格式，智能识别关键帧以提升帧重建质量
2. 结合时间一致性和膨胀损失来减少训练时间和表示大小
3. 将4DGS传输建模为整数线性规划问题，设计轻量级剪枝级别选择算法，自适应地剪枝要传输的高斯更新，平衡重建质量和带宽消耗

Result: 1. 当场景变化时，PSNR质量偏差减少超过20%
2. 帧级PSNR始终保持在30以上
3. 训练速度提升6倍
4. 与最先进的4DGS方法相比，每帧传输大小减少近50%

Conclusion: AirGS通过重新设计4D高斯泼溅的训练和传输流程，显著提升了自由视点视频的质量、训练效率和传输效率，为实时和大规模部署提供了可行的解决方案。
...
### [89] [Mechanism-Based Intelligence (MBI): Differentiable Incentives for Rational Coordination and Guaranteed Alignment in Multi-Agent Systems](https://arxiv.org/abs/2512.20688)
*Stefano Grassi*

Main category: cs.GT

TL;DR: 提出机制智能（MBI）框架，通过可微价格机制（DPM）解决多智能体系统中的信息分散和激励对齐问题，实现线性可扩展的协调优化。


<details>
  <summary>Details</summary>
Motivation: 自主多智能体系统存在根本性脆弱：难以解决哈耶克信息问题（获取分散的私有知识）和赫维茨激励问题（使局部行动与全局目标对齐），导致协调计算不可行。

Method: 引入机制智能（MBI）范式，将智能重新概念化为多个"大脑"协调的涌现现象。核心是可微价格机制（DPM），计算精确损失梯度作为动态的VCG等价激励信号，保证主导策略激励相容（DSIC）并收敛到全局最优。贝叶斯扩展确保在非对称信息下的激励相容（BIC）。

Result: 框架随智能体数量线性扩展（O(N)），绕过了Dec-POMDPs的组合复杂性，经验上比无模型强化学习快50倍。通过结构性地将智能体自利与集体目标对齐，提供了可证明高效、可审计且可泛化的协调方法。

Conclusion: 基于经济原理的机制智能框架为可信赖、可扩展的多智能体智能提供了理论基础，解决了传统方法中的协调难题，实现了激励相容和全局最优的协调。
...
### [90] [(Im)possibility of Incentive Design for Challenge-based Blockchain Protocols](https://arxiv.org/abs/2512.20864)
*Suhyeon Lee,Dieu-Huyen Nguyen,Donghwan Lee*

Main category: cs.GT

TL;DR: 区块链挑战协议激励分析：单赢家设计难以同时实现诚实无损失和欺诈威慑，多赢家设计可达成双目标


<details>
  <summary>Details</summary>
Motivation: 区块链上链计算成本高昂，Truebit和乐观汇总等系统使用挑战协议将计算移出链外。但现有研究未明确诚实挑战者是否真正有激励，以及不诚实提议者在最坏情况下是否受到适当惩罚。

Method: 建立包含合谋少数派、异质成本和三种排序模式的模型，分析能否同时实现两个目标：诚实无损失和欺诈威慑。比较单赢家设计和多赢家设计的激励兼容性。

Result: 单赢家设计中激励设计不可能或规模有限，无法同时满足两个目标。多赢家设计可获得简单明确的条件，使诚实无损失和欺诈威慑同时成立。

Conclusion: 区块链挑战协议的激励设计需要采用多赢家模式，单赢家设计存在根本性限制。多赢家设计为构建安全高效的链下计算系统提供了可行的激励框架。
...
### [91] [Policy-Conditioned Policies for Multi-Agent Task Solving](https://arxiv.org/abs/2512.21024)
*Yue Lin,Shuhui Zhu,Wenhao Li,Ang Li,Dan Qiao,Pascal Poupart,Hongyuan Zha,Baoxiang Wang*

Main category: cs.GT

TL;DR: 提出了一种新范式，将策略表示为人类可读的源代码，利用大语言模型作为近似解释器，通过程序化迭代最佳响应算法在多智能体任务中实现动态策略适应。


<details>
  <summary>Details</summary>
Motivation: 在多智能体任务中，由于深度强化学习策略是难以理解的高维参数向量，导致智能体无法直接理解对手策略，存在"表示瓶颈"问题，阻碍了动态策略适应。

Method: 将策略表示为人类可解释的源代码，利用大语言模型作为近似解释器，提出程序化迭代最佳响应算法，通过文本梯度优化策略代码，使用游戏效用和运行时单元测试的结构化反馈。

Result: 该方法有效解决了多个标准协调矩阵游戏和合作性基于等级的觅食环境，证明了程序化表示能够桥接策略理解与适应之间的鸿沟。

Conclusion: 通过将策略表示为源代码并利用大语言模型，成功克服了深度强化学习中的表示瓶颈，实现了程序均衡概念的操作化，为多智能体学习提供了新的范式。
...
### [92] [Pioneering Multimodal Emotion Recognition in the Era of Large Models: From Closed Sets to Open Vocabularies](https://arxiv.org/abs/2512.20938)
*Jing Han,Zhiqiang Gao,Shihao Gao,Jialing Liu,Hongyu Chen,Zixing Zhang,Björn W. Schuller*

Main category: cs.HC

TL;DR: 该研究首次对19种主流多模态大语言模型在开放词汇多模态情感识别任务上进行大规模基准测试，发现两阶段三模态融合方法效果最佳，视频是最关键模态，开源与闭源模型性能差距很小。


<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型在多模态整合方面表现出色，但其在细粒度情感理解方面的潜力尚未得到系统探索。开放词汇多模态情感识别是克服封闭情感集合限制的有前景方向，但目前缺乏对MLLMs在此领域的全面评估。

Method: 在OV-MERD数据集上进行大规模基准测试，评估19种主流MLLMs，包括通用模型、模态专业化模型和推理增强架构。系统分析模型推理能力、融合策略、上下文利用和提示设计。

Result: 两阶段三模态（音频、视频、文本）融合方法在开放词汇多模态情感识别中表现最优，视频是最关键模态。开源与闭源大语言模型之间的性能差距出人意料地小。

Conclusion: 该研究为开放词汇和细粒度情感计算建立了重要基准，提供了实用指南，为更细致和可解释的情感AI系统铺平了道路。代码将在接受后公开。
...
### [93] [A Design Study Process Model for Medical Visualization](https://arxiv.org/abs/2512.21034)
*Mengjie Fan,Liang Zhou*

Main category: cs.HC

TL;DR: 提出基于医学可视化研究特点的设计研究过程模型，强调利益相关者区分、认知习惯驱动的设计阶段划分，以及任务类型分类，提升医学可视化设计的针对性、可推广性和可操作性。


<details>
  <summary>Details</summary>
Motivation: 现有设计研究过程模型未能充分考虑医学可视化研究的特殊性，需要针对医学问题的复杂性和多样性，建立更系统化的理论框架和实践指导。

Method: 通过文献综述分析现有医学可视化与视觉分析工作，结合跨学科研究经验，识别医学可视化研究特征，构建新的设计研究过程模型，并应用于视觉分析方法设计和三个医学可视化工作的重新分析。

Result: 新模型强调：1) 设计前区分不同利益相关者和目标用户；2) 根据分析逻辑或认知习惯划分设计阶段；3) 将任务类型分为推断性或描述性，并进一步分为基于假设或无假设。模型为每个步骤提供参考指导，使可视化设计更具针对性、可推广性和可操作性。

Conclusion: 新过程模型为跨学科医学可视化研究提供了系统的理论框架和实践指导，能够适应医学问题的复杂性和多样性，为未来研究者提供可参考的建议。
...
### [94] [When LLMs fall short in Deductive Coding: Model Comparison and Human AI Collaboration Workflow Design](https://arxiv.org/abs/2512.21041)
*Zijian Li,Luzhen Tang,Mengyu Xia,Xinyu Li,Naping Chen,Dragan Gašević,Yizhou Fan*

Main category: cs.HC

TL;DR: 本研究探讨了大型语言模型在理论驱动编码任务中的表现，发现LLMs在语义相似度和理论解释方面存在局限，但人机协作工作流能提高编码效率并保持可靠性。


<details>
  <summary>Details</summary>
Motivation: 随着生成式人工智能推动教育对话数据增长，自动化编码成为学习分析提高效率的有前景方向。然而，自动化编码可能因数据不平衡而难以捕捉罕见但关键的代码，而人工编码又耗时耗力。需要了解LLMs在理论驱动编码中的潜力以及人机协作的可能性。

Method: 在两个数据集上比较了小型Transformer分类器（如BERT）和大型语言模型的编码性能，特别关注对话代码中的头尾不平衡分布。设计并评估了人机协作工作流。

Result: LLMs在演绎编码任务中并未超越BERT模型，表现出系统性错误和偏见。人机协作工作流在保持编码可靠性的同时提高了编码效率。

Conclusion: 研究揭示了LLMs的局限性（特别是在语义相似度和理论解释方面）以及人类判断的不可或缺作用，同时证明了人机协作工作流在编码任务中的实际应用前景。
...
### [95] [Learning Factors in AI-Augmented Education: A Comparative Study of Middle and High School Students](https://arxiv.org/abs/2512.21246)
*Gaia Ebli,Bianca Raimondi,Maurizio Gabbrielli*

Main category: cs.HC

TL;DR: 研究探讨AI工具在中小学教育中不同年龄段学生对学习因素的感知差异，发现初中生和高中生对AI辅助学习的体验、清晰度、舒适度和动机等四个关键因素的关联模式存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 现有AI教育研究多集中于高等教育和传统教学环境，缺乏对K-12阶段AI辅助学习中关键学习因素关系的研究，特别是不同年龄段学生感知结构的差异。

Method: 在真实课堂环境中，让学生使用AI工具进行编程学习活动，收集四个学习因素数据和学生感知数据，采用多方法定量分析（相关分析和文本挖掘）比较初中和高中学生的感知结构差异。

Result: 初中生在所有维度间表现出强正相关，显示整体性评价模式；高中生则在关键维度间表现出弱相关或接近零相关，显示更分化的独立评估过程。

Conclusion: 感知维度在AI增强学习中起主动中介作用，发展阶段调节其相互依赖性，为制定适应学习者发展水平和年龄特定维度的AI整合策略奠定基础。
...
### [96] [Parameter-Efficient Neural CDEs via Implicit Function Jacobians](https://arxiv.org/abs/2512.20625)
*Ilya Kuleshov,Alexey Zaytsev*

Main category: cs.LG

TL;DR: 本文提出了一种参数高效的Neural CDE方法，作为传统Neural CDE的替代方案，大幅减少参数数量，同时将其类比为"连续RNN"。


<details>
  <summary>Details</summary>
Motivation: Neural CDE方法虽然适用于时间序列分析，但存在参数数量过多的主要缺点，需要更高效的替代方案。

Method: 提出参数高效的Neural CDE方法，通过减少所需参数数量来实现更高效的时序分析，同时建立与"连续RNN"的逻辑类比关系。

Result: 所提方法在保持Neural CDE功能的同时，显著减少了参数数量，实现了参数效率的提升。

Conclusion: 参数高效的Neural CDE方法为时序分析提供了更实用的解决方案，既减少了计算负担，又保持了方法的理论优势。
...
### [97] [Learning Evolving Latent Strategies for Multi-Agent Language Systems without Model Fine-Tuning](https://arxiv.org/abs/2512.20629)
*Wenlong Tang*

Main category: cs.LG

TL;DR: 提出无需微调语言模型参数的多智能体语言框架，通过外部潜在向量实现持续策略演化，采用双循环架构更新抽象概念表示。


<details>
  <summary>Details</summary>
Motivation: 传统静态语义表示限制了抽象概念的演化能力，需要一种无需修改模型参数就能让语言智能体持续适应环境变化的低成本方法。

Method: 构建双循环架构：行为循环基于环境奖励调整动作偏好；语言循环通过反思生成文本的语义嵌入来更新外部潜在向量，实现抽象概念的持续演化。

Result: 智能体的潜在空间在反思驱动更新下显示清晰的收敛轨迹和关键时刻的结构性转变；系统展现出无需共享奖励就能隐式推断和持续适应情感智能体的涌现能力。
...
### [98] [Zero-Training Temporal Drift Detection for Transformer Sentiment Models: A Comprehensive Analysis on Authentic Social Media Streams](https://arxiv.org/abs/2512.20631)
*Aayam Bansal,Ishaan Gangwani*

Main category: cs.LG

TL;DR: 该研究对基于Transformer的情感模型进行了零训练时间漂移分析，发现在真实社交媒体事件中模型性能显著下降，最大准确率下降达23.4%，并提出四种优于基线的新漂移度量方法。


<details>
  <summary>Details</summary>
Motivation: 研究动机是分析Transformer情感模型在真实社交媒体事件中的时间漂移问题，特别是在动态内容期间模型性能的不稳定性，这对于实时情感监控系统的部署至关重要。

Method: 采用零训练方法，对三种Transformer架构进行系统评估，使用12,279个真实社交媒体帖子进行统计验证，引入四种新的漂移度量指标，并与基于嵌入的基线方法进行比较。

Result: 结果显示模型在事件驱动期间存在显著不稳定性，准确率下降最高达23.4%，置信度最大下降13.0%（Bootstrap 95% CI: [9.1%, 16.5%]），新提出的度量方法在计算效率上优于基线，适合生产部署。

Conclusion: 该零训练方法可直接部署于实时情感监控系统，为Transformer模型在动态内容期间的行为提供了新见解，其检测能力超过行业监控阈值，具有实际应用价值。
...
### [99] [Enhancing Lung Cancer Treatment Outcome Prediction through Semantic Feature Engineering Using Large Language Models](https://arxiv.org/abs/2512.20633)
...
TL;DR: 使用大型语言模型作为目标导向知识策展器，将多模态临床数据转换为任务对齐的特征表示，在肺癌治疗结果预测中优于传统方法
...
Method: 提出一个框架，使用大型语言模型作为目标导向知识策展器，将实验室、基因组和药物数据转换为高保真、任务对齐的特征表示。该方法作为离线预处理步骤，自然地集成到医院信息管道中。

Result: 在肺癌队列（N=184）中，该方法平均AUROC为0.803（95% CI: 0.799-0.807），优于专家工程特征、直接文本嵌入和端到端Transformer等基线方法。消融研究进一步证实了结合所有三种模态的互补价值。
...
### [100] [Real Time Detection and Quantitative Analysis of Spurious Forgetting in Continual Learning](https://arxiv.org/abs/2512.20634)
*Weiwei Wang*

Main category: cs.LG

TL;DR: 论文提出浅层与深层对齐框架，首次量化表征对齐深度，揭示当前任务对齐方法仅在前3-5个输出token保持浅层对齐，导致模型易受遗忘影响，并提出包含量化指标、实时检测、可视化工具和自适应缓解策略的完整解决方案。


<details>
  <summary>Details</summary>
Motivation: 持续学习中的灾难性遗忘是大型语言模型面临的根本挑战。最近研究发现性能下降可能源于任务对齐破坏引起的虚假遗忘而非真实知识丢失，但现有工作仅定性描述对齐、依赖事后分析且缺乏自动区分机制。

Method: 提出浅层与深层对齐框架：1) 在0-1尺度上量化测量不同token位置的对齐深度；2) 训练期间实时检测浅层对齐的方法；3) 可视化与恢复预测的专门分析工具；4) 自动区分遗忘类型并促进深层对齐的自适应缓解策略。

Result: 在多个数据集和模型架构(Qwen2.5-3B到Qwen2.5-32B)上的广泛实验表明：识别准确率达到86.2-90.6%；促进深层对齐可将抗遗忘鲁棒性比基线提高3.3-7.1%。

Conclusion: 该研究首次量化表征对齐深度，解释了虚假遗忘的发生机制、可逆性以及微调攻击的有效性，提出的完整框架能有效区分遗忘类型并促进深层对齐，显著提升模型在持续学习中的抗遗忘能力。
...
### [101] [Data-Free Pruning of Self-Attention Layers in LLMs](https://arxiv.org/abs/2512.20636)
*Dhananjay Saikumar,Blesson Varghese*

Main category: cs.LG

TL;DR: 提出Gate-Norm方法，通过查询-键耦合度一次性评估注意力子层重要性，无需数据、前向传播或微调即可快速剪枝LLM中的冗余注意力层，提升推理速度同时保持精度。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型中许多注意力子层可以被移除而几乎不影响性能，作者将此归因于"注意力抑制假设"：在预训练过程中，一些深层注意力层学会了抑制自身贡献，让残差流和MLP承载表示。

Method: 提出Gate-Norm方法，这是一种一次性、仅权重的标准，通过查询-键耦合度对注意力子层进行排序，移除耦合度最低的子层。该方法无需校准数据、前向传播、微调或专用内核。

Result: 在40层、130亿参数的LLaMA模型上，Gate-Norm在1秒内完成剪枝。移除8-16个注意力子层可使推理吞吐量提升至1.30倍，同时在多个基准测试中保持平均零样本精度在未剪枝基线的2%以内。

Conclusion: Gate-Norm在精度上与数据驱动的剪枝方法相当，但评分层速度快约1000倍，实现了实用、无需数据的LLM压缩，为高效推理提供了有效解决方案。
...
### [102] [Forecasting N-Body Dynamics: A Comparative Study of Neural Ordinary Differential Equations and Universal Differential Equations](https://arxiv.org/abs/2512.20643)
...
<details>
  <summary>Details</summary>
Motivation: 传统机器学习模型用于预测天体轨迹时通常是数据密集型的黑盒模型，忽略了物理定律，缺乏可解释性。科学机器学习可以直接将已知物理定律嵌入到机器学习框架中，提高模型的物理一致性和数据效率。

Method: 使用Julia编程语言，采用科学机器学习框架：神经常微分方程和通用微分方程来预测系统动力学。使用合成噪声数据模拟现实观测限制，并确定预测崩溃点（模型准确预测未来未见数据所需的最小训练数据量）。

Result: UDE模型的数据效率远高于神经ODE模型，仅需要20%的数据即可进行正确预测，而神经ODE模型需要90%的数据。这表明UDE模型在处理n体问题轨迹预测时具有显著的数据效率优势。

Conclusion: 科学机器学习方法，特别是通用微分方程，能够有效结合物理定律和机器学习，在n体问题预测中实现更高的数据效率和准确性，为天体物理学中的轨迹预测提供了更优的解决方案。
...
### [103] [MaskOpt: A Large-Scale Mask Optimization Dataset to Advance AI in Integrated Circuit Manufacturing](https://arxiv.org/abs/2512.20655)
*Yuting Hu,Lei Zhuang,Hua Xiang,Jinjun Xiong,Gi-Joon Nam*

Main category: cs.LG

TL;DR: MaskOpt是一个用于IC掩模优化的大规模基准数据集，基于45nm节点的真实IC设计构建，包含超过22万个金属层和通孔层图块，支持不同上下文窗口大小，用于评估深度学习模型在单元感知和上下文感知掩模优化中的性能。


<details>
  <summary>Details</summary>
Motivation: 随着IC尺寸缩小到低于光刻波长，光学光刻面临衍射和工艺可变性的挑战。基于模型的OPC和ILT计算成本高，限制了可扩展性。现有的深度学习掩模优化数据集通常依赖合成布局，忽略标准单元层次结构和周围上下文，限制了在实际掩模优化中的应用。

Method: 从45nm节点的真实IC设计中构建MaskOpt数据集，包含104,714个金属层图块和121,952个通孔层图块。每个图块在标准单元放置处裁剪以保留单元信息，利用重复逻辑门出现。支持不同上下文窗口大小以捕捉光学邻近效应中邻近形状的影响。

Result: 评估了最先进的深度学习模型用于IC掩模优化，建立了基准测试结果。评估结果揭示了基线模型之间的不同权衡。进一步的上下文大小分析和输入消融研究证实了周围几何形状和单元感知输入在实现准确掩模生成中的重要性。

Conclusion: MaskOpt数据集为单元感知和上下文感知掩模优化提供了大规模基准，支持深度学习模型在实际IC设计中的应用，解决了现有数据集的局限性，并为掩模优化研究提供了重要的评估平台。
...
### [104] [Dominating vs. Dominated: Generative Collapse in Diffusion Models](https://arxiv.org/abs/2512.20666)
*Hayeon Jeong,Jong-Seok Lee*

Main category: cs.LG

TL;DR: 本文研究了文本到图像扩散模型中的"主导vs被主导"不平衡现象，即多概念提示中一个概念主导生成而抑制其他概念的问题。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型在生成多概念提示时，经常出现一个概念主导而抑制其他概念的现象，这种"主导vs被主导"不平衡影响了生成结果的多样性和可控性。

Method: 引入DominanceBench进行系统性分析，从数据和架构两个角度研究不平衡原因，包括训练数据实例多样性分析、交叉注意力动态分析以及头部消融研究。

Result: 发现训练数据实例多样性有限加剧了概念间干扰；主导token会快速饱和注意力并在扩散时间步中逐步抑制其他token；DvD行为源于多个注意力头的分布式机制。

Conclusion: 研究结果为理解生成崩溃提供了关键见解，有助于推进更可靠和可控的文本到图像生成。
...
### [105] [Improving Cardiac Risk Prediction Using Data Generation Techniques](https://arxiv.org/abs/2512.20669)
*Alexandre Cabodevila,Pedro Gamallo-Fernandez,Juan C. Vidal,Manuel Lama*

Main category: cs.LG

TL;DR: 提出基于条件变分自编码器的架构，用于生成真实的心脏康复临床记录，以解决医疗数据稀缺、不适用和缺失值问题，提升心脏风险预测模型性能


<details>
  <summary>Details</summary>
Motivation: 心脏康复过程涉及多阶段、个性化医疗决策和多方专业人员协调，可建模为业务流程进行分析。但现实医疗数据库存在数据稀缺（收集成本高、时间长）、现有记录不适用于特定分析目的、以及缺失值普遍（患者接受的诊断测试不同）等显著限制

Method: 提出基于条件变分自编码器的架构，用于合成与现实观察一致的逼真临床记录。主要目标是增加可用数据集的大小和多样性，以提升心脏风险预测模型性能，并减少潜在危险的诊断程序（如运动负荷测试）的需求

Result: 所提出的架构能够生成一致且逼真的合成数据，其使用提高了用于心脏风险检测的各种分类器的准确性，在合成数据生成方面优于最先进的深度学习方法

Conclusion: 基于CVAE的架构能有效解决心脏康复数据分析中的现实限制，通过生成高质量合成数据提升风险预测性能，减少对危险诊断程序的依赖
...
### [106] [Disentangling Fact from Sentiment: A Dynamic Conflict-Consensus Framework for Multimodal Fake News Detection](https://arxiv.org/abs/2512.20670)
*Weilin Zhou,Zonghao Ying,Junjie Mu,Shengwei Tian,Quanchen Zou,Deyue Zhang,Dongdong Yang,Xiangzheng Zhang*

Main category: cs.LG

TL;DR: DCCF框架通过主动寻找和放大跨模态矛盾来检测假新闻，而非传统的一致性融合方法，平均准确率提升3.52%


<details>
  <summary>Details</summary>
Motivation: 现有基于一致性的多模态假新闻检测方法错误地将关键跨模态差异视为噪声，导致过度平滑并稀释了伪造证据。主流的一致性融合方法通过最小化特征差异来对齐模态，但这种方法无意中平滑了作为伪造主要证据的细微跨模态矛盾。

Method: 提出动态冲突-共识框架(DCCF)，采用不一致性寻求范式：1) 将输入解耦为独立的事实空间和情感空间，区分客观不匹配与情感不协调；2) 采用物理启发的特征动力学迭代极化这些表示，主动提取最大信息量的冲突；3) 冲突-共识机制将局部差异标准化到全局上下文中进行稳健的审议判断。

Result: 在三个真实世界数据集上的广泛实验表明，DCCF始终优于最先进的基线方法，平均准确率提升3.52%。

Conclusion: 通过主动寻求和放大跨模态矛盾而非抑制它们，DCCF能够更有效地检测假新闻，突破了传统一致性融合方法的局限性。
...
### [107] [HyDRA: Hierarchical and Dynamic Rank Adaptation for Mobile Vision Language Model](https://arxiv.org/abs/2512.20674)
*Yuanhao Xi,Xiaohuan Bing,Ramin Yahyapour*

Main category: cs.LG

TL;DR: HyDRA是一个用于移动视觉语言模型的高效微调框架，通过分层动态秩调度在保持可训练参数不变的情况下提升性能4.7%，在某些任务中甚至超越全参数微调。


<details>
  <summary>Details</summary>
Motivation: 移动视觉语言模型虽然应用场景广泛，但训练计算需求巨大，标准LoRA固定秩方法不足以有效训练处理文本和图像双模态的移动VLMs。

Method: 提出HyDRA框架，包含两种优化策略：1）分层优化：粗粒度为不同层分配不同秩，细粒度调整单个层内秩；2）动态调整：使用轻量性能模型进行端到端自动优化，在微调过程中确定和调整秩。

Result: 在流行基准测试中，HyDRA始终优于基线，在各种模型尺寸下实现4.7%的性能提升，且不增加可训练参数数量。在某些任务中甚至超越全参数微调。

Conclusion: HyDRA通过分层动态秩调度有效解决了移动视觉语言模型的高效微调问题，在保持参数效率的同时显著提升性能，为移动VLMs的实际应用提供了可行的解决方案。
...
### [108] [FEM-Bench: A Structured Scientific Reasoning Benchmark for Evaluating Code-Generating LLMs](https://arxiv.org/abs/2512.20732)
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在物理世界推理能力方面的进步，缺乏评估其生成科学有效物理模型能力的严格基准已成为关键缺口。计算力学提供了结构化科学推理评估的理想基础，因为它遵循清晰的数学结构、强制执行严格的物理和数值约束、支持客观验证。

Method: 引入FEM-Bench计算力学基准，包含与计算力学研究生入门课程材料对齐的33个入门但非平凡任务。这些任务捕捉了基本的数值和物理建模挑战，同时只代表了该学科复杂性的很小一部分。通过让大语言模型生成有限元方法及相关代码来评估其能力。

Result: 在5次尝试运行中，函数编写表现最佳的Gemini 3 Pro模型至少完成30/33个任务一次，26/33个任务全部五次完成。单元测试编写表现最佳的GPT-5模型平均联合成功率为73.8%。其他流行模型表现出广泛的性能差异。

Conclusion: FEM-Bench为评估AI生成科学代码建立了结构化基础，尽管任务相对简单，但最先进的大语言模型仍不能可靠解决所有问题。未来迭代将纳入更复杂的任务以跟踪模型进展。
...
### [109] [Stabilizing Multimodal Autoencoders: A Theoretical and Empirical Analysis of Fusion Strategies](https://arxiv.org/abs/2512.20749)
*Diyar Altinses,Andreas Schwung*

Main category: cs.LG

TL;DR: 该论文分析了多模态自编码器的Lipschitz性质，提出了基于理论分析的注意力融合方法，通过实验验证了该方法在训练稳定性、收敛速度和准确性方面的优越性。


<details>
  <summary>Details</summary>
Motivation: 随着多模态自编码器的发展，理解其稳定性和鲁棒性对于优化训练、架构设计和实际应用至关重要。本文旨在分析多模态自编码器的Lipschitz性质，为提升模型训练稳定性提供理论基础。

Method: 首先推导多模态自编码器中聚合方法的理论Lipschitz常数，然后基于理论分析提出正则化注意力融合方法，最后通过实验验证理论发现，估计不同融合策略的Lipschitz常数。

Result: 实验结果表明，提出的融合函数不仅符合理论预测，而且在一致性、收敛速度和准确性方面优于现有策略，为多模态自编码器的融合提供了理论依据。

Conclusion: 本文为理解多模态自编码器的融合机制提供了坚实的理论基础，并提出了一种提升模型性能的解决方案，有助于增强多模态自编码器的训练稳定性和实际应用价值。
...
TL;DR: RLVR在因果推理任务中的泛化效果取决于模型规模和训练查询级别，仅当模型具备足够初始推理能力时才能提升特定因果推理子技能
...
Method: 构建因果图和查询数据集，涵盖概率查询级别（关联、干预、反事实）和结构复杂度两个维度，使用RLVR和监督微调（SFT）对Qwen-2.5-Instruct模型（3B-32B）进行微调

Result: RLVR在特定模型规模和训练查询级别组合下比SFT有更好的同级别和跨级别泛化能力；RLVR的有效性取决于模型的初始推理能力，足够初始能力下能改进边缘化策略并减少中间概率计算错误

Conclusion: RLVR能改进特定因果推理子技能，但其益处仅在模型具备足够初始推理能力时才会显现，为RLVR在复杂推理任务中的应用提供了实证指导
...
TL;DR: TS-Arena平台解决时间序列基础模型评估中的信息泄露危机，通过实时数据流预注册机制确保测试数据在推理时物理上不存在，建立移动时间边界防止历史污染


<details>
  <summary>Details</summary>
Motivation: 时间序列基础模型虽然具有变革性预测能力，但存在根本性评估危机：训练集和测试集重叠导致信息泄露，以及全局模式非法转移到测试数据。现有评估方法允许利用观察到的全局冲击，违反了有效基准测试所需的独立性要求

Method: 引入TS-Arena平台，通过实时数据流预注册机制，将真正未知的未来作为确定的测试环境。平台确保评估目标在推理时物理上不存在，强制执行严格的全局时间分割，建立移动时间边界防止历史污染

Result: TS-Arena为能源领域提供了可持续的基础设施，能够在真实世界约束下比较基础模型。平台原型已在Hugging Face上提供

Conclusion: TS-Arena恢复了预测的操作完整性，通过防止历史污染提供了模型泛化能力的真实评估，为解决时间序列基础模型评估危机提供了有效解决方案
...
TL;DR: 该研究首次针对Cox生存分析模型提出了子群发现问题，开发了新的评估指标和算法，在理论和实验上验证了方法的有效性，并在NASA喷气发动机数据上进行了案例研究。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解决生存分析中的子群发现问题，即寻找数据中Cox模型表现特别好的可解释子集。现有质量函数无法有效解决Cox模型的子群发现问题，需要新的技术方法。

Method: 提出了两个技术创新：1）期望预测熵（EPE），用于评估预测风险函数的生存模型；2）条件秩统计量（CRS），用于量化个体点与现有子群生存时间分布的偏差。基于这些指标开发了八种算法，主要算法能同时利用EPE和CRS，在设定良好的情况下提供理论正确性保证。

Result: 实验验证了理论，在设定良好的情况下能够恢复真实子群，在实际应用中相比在整个数据集上朴素拟合Cox模型能获得更好的模型拟合效果。NASA喷气发动机数据的案例研究发现已知的非线性/同质性，这些发现与实践中采用的设计选择相符。

Conclusion: 该研究首次系统解决了Cox模型的子群发现问题，提出的EPE和CRS指标以及相关算法在理论和实践上都表现出色，能够发现数据中有意义的子结构，为生存分析提供了新的工具。
...
TL;DR: 提出了一种针对生成式AI流程优化的矩阵指数泰勒算法，通过动态选择泰勒阶数和缩放因子，在保证精度的同时最小化计算量，相比现有方法显著加速且保持数值稳定性。


<details>
  <summary>Details</summary>
Motivation: 矩阵指数是科学计算和系统仿真的基础算子，在控制理论、量子力学和现代生成式机器学习中都有应用。传统Padé近似结合缩放平方方法虽为标准，但近期基于泰勒的方法利用超越经典Paterson-Stockmeyer技术的多项式求值方案，提供了更优的精度和更低的计算复杂度。本文针对生成式AI流程的高吞吐量需求，提出优化的泰勒算法。

Method: 开发了优化的泰勒基矩阵指数算法，专门针对生成式AI流程的高吞吐量需求。提供了严格的误差分析，并开发了动态选择策略来自动确定泰勒阶数和缩放因子，以在规定的误差容限下最小化计算工作量。

Result: 广泛的数值实验表明，与现有最先进的实现相比，该方法提供了显著的加速效果，并保持了高数值稳定性。这些结果确立了所提方法作为大规模生成建模的高效工具。

Conclusion: 提出的优化泰勒算法在生成式AI流程中表现出色，通过动态参数选择实现了计算效率与数值精度的平衡，成为大规模生成建模的高效工具。
...
### [114] [FedMPDD: Communication-Efficient Federated Learning with Privacy Preservation Attributes via Projected Directional Derivative](https://arxiv.org/abs/2512.20814)
*Mohammadreza Rostami,Solmaz S. Kia*

Main category: cs.LG

TL;DR: FedMPDD是一种联邦学习算法，通过多投影方向导数压缩梯度，将通信成本从O(d)降至O(m)，同时提供隐私保护，收敛速度与FedSGD相当。
...
Method: 使用多投影方向导数编码：客户端计算梯度在多个随机向量上的方向导数，将高维梯度压缩为小消息；服务器通过投影回相同随机向量解码聚合信息。

Result: 通信成本从O(d)降至O(m)，m≪d；收敛速度为O(1/√K)，与FedSGD相当；提供可调隐私-效用权衡，对抗梯度反演攻击。

Conclusion: FedMPDD在显著降低通信成本的同时保持收敛性能，并提供内置隐私保护，通过投影数量实现隐私-效用权衡，实验验证了理论分析。
...
### [115] [Defending against adversarial attacks using mixture of experts](https://arxiv.org/abs/2512.20821)
*Mohammad Meymani,Roozbeh Razavi-Far*

Main category: cs.LG

TL;DR: 本文提出了一种基于专家混合架构的对抗训练防御系统，通过联合训练9个预训练专家模型和门控机制，显著提升了模型对抗攻击的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型在面对对抗性威胁时表现出脆弱性，这些威胁包括对抗性扰动、训练数据投毒和模型查询攻击等，可能导致模型误分类或性能下降。现有的防御系统需要进一步提升对抗攻击的鲁棒性。

Method: 提出了一种基于专家混合架构的防御系统，使用9个以ResNet-18为骨干的预训练专家模型。在端到端训练过程中，联合更新专家模型参数和门控机制，实现专家模型的进一步优化。

Result: 提出的防御系统在对抗攻击防御方面优于现有的最先进防御系统和普通分类器，即使这些对比模型使用了比ResNet-18更复杂的架构。

Conclusion: 基于专家混合架构的对抗训练防御系统能有效提升机器学习模型对抗对抗性威胁的鲁棒性，为构建更安全的机器学习系统提供了有效解决方案。
...
### [116] [Memory-Efficient Acceleration of Block Low-Rank Foundation Models on Resource Constrained GPUs](https://arxiv.org/abs/2512.20861)
*Pierre Abillama,Changwoo Lee,Juechu Dong,David Blaauw,Dennis Sylvester,Hun-Seok Kim*

Main category: cs.LG

TL;DR: 该研究针对基于Transformer的基础模型在单GPU上部署时面临的计算和内存挑战，通过引入自定义Triton内核优化BLR压缩方法，在内存受限的GPU上实现了显著的加速和模型压缩。


<details>
  <summary>Details</summary>
Motivation: 随着Transformer基础模型规模的快速增长，在单GPU上部署完整模型变得越来越困难，计算成本高昂。虽然BLR压缩技术能学习紧凑的权重矩阵表示，但在多令牌推理时仍面临内存瓶颈问题，导致延迟增加。

Method: 使用屋顶线分析揭示BLR方法在多令牌推理中的内存瓶颈问题，然后为Monarch和BLAST两种BLR方法开发自定义Triton内核，采用部分融合和内存布局优化技术来提升性能。

Result: 在NVIDIA Jetson Orin Nano和A40等内存受限的GPU上，相比PyTorch密集基线和CUDA后端优化，新内核实现了最高3.76倍的加速和3倍的模型压缩，支持Llama-7/1B、GPT2-S、DiT-XL/2和ViT-B等多种模型。

Conclusion: 通过针对BLR压缩方法的定制化内核优化，可以有效解决多令牌推理中的内存瓶颈问题，在保持模型精度的同时显著提升推理效率并减少内存占用，为在资源受限设备上部署大型基础模型提供了实用解决方案。
...
### [117] [Towards a General Framework for Predicting and Explaining the Hardness of Graph-based Combinatorial Optimization Problems using Machine Learning and Association Rule Mining](https://arxiv.org/abs/2512.20915)
...
<details>
  <summary>Details</summary>
Motivation: 开发一个通用的机器学习框架来预测组合优化问题的计算难度，并提供可解释的预测结果，帮助理解问题实例的难易特性。

Method: 两阶段框架：第一阶段创建包含图特征和难度分类的数据集，训练分类算法；第二阶段使用关联规则挖掘解释预测，并训练回归模型预测计算时间。

Result: 在3287个最大团问题实例上测试，仅用3个图特征就实现了加权F1分数0.9921、少数类F1分数0.878、ROC-AUC分数0.9083；最佳关联规则支持度0.8829，准确率87.64%；最佳回归模型百分比RMSE为5.12，R2值为0.991。

Conclusion: GCO-HPIF框架在预测组合优化问题计算难度方面表现出色，同时提供了良好的可解释性，为算法选择和问题理解提供了有力工具。
...
### [118] [RevFFN: Memory-Efficient Full-Parameter Fine-Tuning of Mixture-of-Experts LLMs with Reversible Blocks](https://arxiv.org/abs/2512.20920)
*Ningyuan Liu,Jing Yang,Kaitong Cai,Keze Wang*

Main category: cs.LG

TL;DR: RevFFN是一种用于MoE大语言模型的内存高效全参数微调方法，通过可逆Transformer块重构激活值，显著减少内存占用，使单GPU微调成为可能。
...
Method: 引入RevFFN，采用精心设计的可逆Transformer块，在反向传播期间可以从输出重构层输入激活值，从而消除存储大多数中间激活值的需求，同时保持MoE架构的表达能力。

Result: 该方法显著降低了全参数微调的峰值内存消耗，使得在单个消费级或服务器级GPU上实现高效全参数微调成为可能。

Conclusion: RevFFN为MoE大语言模型提供了一种内存高效的全参数微调范式，解决了传统方法的内存瓶颈问题，使单GPU微调变得可行。
...
### [119] [Guardrailed Elasticity Pricing: A Churn-Aware Forecasting Playbook for Subscription Strategy](https://arxiv.org/abs/2512.20932)
*Deepit Sapru*

Main category: cs.LG

TL;DR: 本文提出了一个营销分析框架，将订阅定价操作化为动态、有护栏的决策系统，结合多变量需求预测、细分价格弹性和流失倾向来优化收入、利润和留存率。


<details>
  <summary>Details</summary>
Motivation: 传统静态定价和统一提价策略无法适应不同客户群体的支付意愿差异，可能导致收入优化不足、客户流失或信任侵蚀。需要一种既能最大化收入又能保护价格敏感客户、维持客户信任的动态定价方法。

Method: 结合季节性时间序列模型和基于树的学习器进行多变量需求预测，使用蒙特卡洛场景测试映射风险范围，通过约束优化强制执行客户体验、利润底线和允许流失率的业务护栏。

Result: 在异构SaaS产品组合中验证，该方法持续优于静态定价层级和统一提价策略，通过将价格调整重新分配到支付意愿较高的细分市场，同时保护价格敏感群体。

Conclusion: 该框架作为战略手册，明确了何时从固定定价转向动态定价、如何将定价与客户终身价值和月度经常性收入目标对齐，以及如何嵌入道德护栏，实现可持续增长而不侵蚀客户信任。
...
### [120] [Solving Functional PDEs with Gaussian Processes and Applications to Functional Renormalization Group Equations](https://arxiv.org/abs/2512.20956)
...
<details>
  <summary>Details</summary>
Motivation: 传统方法如局域势近似在处理非微扰泛函重整化群方程时存在局限性，特别是在处理非恒定场时。需要一种更灵活、更通用的方法来求解这类泛函微分方程。

Method: 使用高斯过程算子学习框架，直接在函数空间上构建泛函表示。该方法允许通过先验均值或核设计融入物理先验知识，独立于特定方程或离散化。

Result: 该方法在Wetterich和Wilson-Polchinski方程上表现出色，达到或超越了现有近似方法（如局域势近似）的性能，同时显著提高了灵活性。特别地，能够处理非恒定场，为研究更复杂的场构型（如瞬子）提供了可能。

Conclusion: 提出的高斯过程算子学习框架为求解非微扰泛函重整化群方程提供了一种灵活、通用的方法，能够处理复杂场构型，有望在量子场论研究中发挥重要作用。
...
### [121] [Can Agentic AI Match the Performance of Human Data Scientists?](https://arxiv.org/abs/2512.20959)
...
TL;DR: 当前基于大语言模型的智能AI在数据科学中无法匹敌具备领域知识的人类专家，特别是在需要识别隐藏变量（如图像数据中的关键信息）的预测任务中。


<details>
  <summary>Details</summary>
Motivation: 探索智能AI系统是否能真正匹配人类数据科学家的性能，特别是在需要领域专业知识的情况下。当前AI虽然自动化了数据科学工作流程，但缺乏有效利用领域知识的能力。

Method: 设计了一个预测任务，其中关键的潜在变量隐藏在相关图像数据而非表格特征中。使用财产保险的合成数据集进行实验，比较仅依赖通用分析流程的智能AI与具备领域知识的方法。

Result: 实验表明，依赖通用分析工作流程的智能AI表现不如使用领域特定洞察的方法。人类专家能够利用领域知识识别重要隐藏变量，而仅生成通用表格数据建模代码的AI无法取得良好性能。

Conclusion: 当前智能AI在数据科学中存在关键局限性，无法有效识别和整合领域知识。未来研究需要开发能够更好识别和融入领域知识的智能AI系统。
...
### [122] [Generalization of Diffusion Models Arises with a Balanced Representation Space](https://arxiv.org/abs/2512.20963)
*Zekai Zhang,Xiao Li,Xiang Li,Lianghe Shi,Meng Wu,Molei Tao,Qing Qu*

Main category: cs.LG

TL;DR: 该论文分析了扩散模型中记忆化与泛化的区别，从表示学习的角度证明：记忆化对应模型在权重中存储原始训练样本，产生"尖峰"表示；泛化则对应模型捕捉局部数据统计，产生"平衡"表示。基于这些发现，提出了检测记忆化的表示方法和无需训练的编辑技术。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在生成高质量多样化样本方面表现出色，但过度拟合训练目标时存在记忆训练数据的风险。需要理解记忆化与泛化的本质区别，以促进更好的生成建模。

Method: 通过分析两层ReLU去噪自编码器(DAE)，从表示学习角度理论分析记忆化与泛化的区别。在真实世界的无条件扩散模型和文本到图像扩散模型上验证理论发现，并提出基于表示的检测方法和无需训练的编辑技术。

Result: 理论证明：记忆化对应模型在权重中存储原始训练样本，产生"尖峰"表示；泛化对应模型捕捉局部数据统计，产生"平衡"表示。在真实扩散模型上验证了这些表示结构，提出的方法能够有效检测记忆化并实现精确控制。

Conclusion: 学习良好的表示是新颖且有意义的生成建模的核心。记忆化与泛化的区别体现在表示结构上，基于表示的方法能够检测记忆化并实现精确控制，对实际应用有重要意义。
...
### [123] [CoSeNet: A Novel Approach for Optimal Segmentation of Correlation Matrices](https://arxiv.org/abs/2512.21000)
*Alberto. Palomo-Alonso,David Casillas-Perez,Silvia Jimenez-Fernandez,Antonio Portilla-Figueras,Sancho Salcedo-Sanz*

Main category: cs.LG

TL;DR: CoSeNet是一种用于噪声相关矩阵中相关片段最优识别的新型四层算法架构，通过重叠技术和预训练机器学习算法实现鲁棒性，使用启发式算法优化参数，输出无噪声二进制矩阵表示最优分割。


<details>
  <summary>Details</summary>
Motivation: 现有方法在噪声相关矩阵中识别相关片段的效果有限，需要一种更有效、鲁棒且可泛化的方法来准确识别相关片段并优化分割结果。

Method: 提出CoSeNet四层算法架构：输入层、格式化层、重缩放层和分割层。采用重叠技术增强鲁棒性，使用预训练机器学习算法，通过基于窗口差异度量的启发式算法优化重缩放层参数。

Result: CoSeNet能有效识别相关矩阵中的相关片段，性能优于先前类似问题的方法，输出无噪声二进制矩阵表示最优分割及其分割点。

Conclusion: CoSeNet为噪声相关矩阵中的相关片段识别提供了一种新颖有效的解决方案，在效率、内存和速度之间取得平衡，适用于多种应用场景。
...
### [124] [LLM Swiss Round: Aggregating Multi-Benchmark Performance via Competitive Swiss-System Dynamics](https://arxiv.org/abs/2512.21010)
*Jiashuo Liu,Jiayun Wu,Chunjie Wu,Jingkai Liu,Zaiyuan Wang,Huan Zhou,Wenhao Huang,Hongseok Namkoong*

Main category: cs.LG

TL;DR: 提出CSD框架，通过瑞士制动态配对和蒙特卡洛模拟，为LLM提供比传统静态评分更全面的竞争性评估系统


<details>
  <summary>Details</summary>
Motivation: 当前LLM评估方法存在局限性：使用静态评分，难以确定不同基准测试的适当混合比例，无法捕捉模型在连续高风险任务中的动态竞争适应性和脆弱性

Method: 提出竞争性瑞士制动态框架(CSD)，模拟多轮顺序竞赛，根据累积胜负记录在基准测试序列中动态配对模型；使用蒙特卡洛模拟(N=100,000次迭代)计算统计稳健的期望获胜分数；实施失败敏感性分析，通过参数化每轮淘汰数量来区分稳健通用型和激进专用型模型

Result: CSD提供了比传统聚合评分和静态配对模型更细致、上下文感知的排名，代表了向风险感知的下一代LLM评估迈出的重要一步

Conclusion: CSD框架解决了当前LLM评估方法的局限性，通过动态竞争性评估提供更全面的模型性能排名，能够区分不同风险偏好的模型类型
...
### [125] [Shared Representation Learning for High-Dimensional Multi-Task Forecasting under Resource Contention in Cloud-Native Backends](https://arxiv.org/abs/2512.21102)
*Zixiao Huang,Jixiao Yang,Sijia Li,Chi Zhang,Jinyu Chen,Chengda Xu*

Main category: cs.LG

TL;DR: 提出一个用于云原生后端系统高维多任务时间序列的统一预测框架，通过共享编码、状态融合、跨任务结构传播和动态调整机制，在高度动态负载、耦合指标和平行任务环境下实现稳定预测。


<details>
  <summary>Details</summary>
Motivation: 云原生后端系统在高度动态负载、耦合指标和平行任务环境下，需要满足对高维多任务时间序列的预测需求，以支持智能后端管理。

Method: 构建共享编码结构统一表示多样化监控指标；采用状态融合机制捕捉不同时间尺度的趋势变化和局部扰动；引入跨任务结构传播模块建模节点间潜在依赖关系；集成动态调整机制根据系统状态变化自动调节内部特征流。
...
### [126] [A Mechanistic Analysis of Transformers for Dynamical Systems](https://arxiv.org/abs/2512.21113)
*Gregory Duthé,Nikolaos Evangelou,Wei Liu,Ioannis G. Kevrekidis,Eleni Chatzi*

Main category: cs.LG

TL;DR: 本文从动力系统角度分析单层Transformer在时序数据建模中的表示能力和局限性，揭示注意力机制在不同动态机制下的工作原理和约束条件。


<details>
  <summary>Details</summary>
Motivation: Transformer在时序建模中应用日益广泛，但其内部机制从动力系统角度理解不足。与传统自回归和状态空间模型相比，Transformer通常被视为黑箱，这限制了其在跨不同动态机制下的通用或零样本预测应用。

Method: 从动力系统视角将因果自注意力解释为线性、历史依赖的递归，通过线性和非线性案例研究分析其处理时序信息的方式。研究单层Transformer在动力数据上的表示能力。

Result: 在线性系统中，softmax注意力的凸性约束限制了可表示的动态类别，导致振荡设置中的过度平滑。在非线性部分可观测系统中，注意力作为自适应延迟嵌入机制，在足够时间上下文和潜在维度可用时实现有效状态重构。

Conclusion: 研究结果有助于将经验观察与经典动力系统理论联系起来，提供了关于Transformer作为动力系统模型何时成功或失败以及原因的见解，为理解注意力机制在时序建模中的工作原理提供了理论框架。
...
### [127] [MODE: Multi-Objective Adaptive Coreset Selection](https://arxiv.org/abs/2512.21152)
...
<details>
  <summary>Details</summary>
Motivation: 现有核心集选择方法通常是静态的，无法适应不同训练阶段的数据需求。需要一种能够根据训练动态调整选择策略的框架，以更有效地利用数据并减少内存需求。
...
Conclusion: MODE框架通过动态适应训练阶段的核心集选择策略，实现了高效的数据利用和内存优化，为数据效用演化提供了新的理解视角。
...
### [128] [A Unified Framework for EEG Seizure Detection Using Universum-Integrated Generalized Eigenvalues Proximal Support Vector Machine](https://arxiv.org/abs/2512.21170)
...
<details>
  <summary>Details</summary>
Motivation: 解决EEG分析中的关键挑战：非平稳性、低信噪比和标记数据有限，需要开发更有效的分类方法。

Method: 提出U-GEPSVM通过基于比率的优化函数将Universum约束纳入GEPSVM框架；IU-GEPSVM通过加权差异公式增强稳定性，提供对类别分离和Universum对齐的独立控制。

Result: 在Bonn大学EEG数据集的两个二分类任务上，IU-GEPSVM达到峰值准确率85%（O vs S）和80%（Z vs S），平均准确率分别为81.29%和77.57%，优于基线方法。

Conclusion: 提出的Universum增强分类器能有效处理EEG信号分类问题，IU-GEPSVM在准确率和稳定性方面表现优异，为EEG分析提供了有前景的解决方案。
...
### [129] [Model Merging via Multi-Teacher Knowledge Distillation](https://arxiv.org/abs/2512.21288)
*Seyed Arshan Dalili,Mehrdad Mahdavi*

Main category: cs.LG

TL;DR: 该论文提出了SAMerging方法，通过建立模型合并的PAC-Bayes泛化边界，将模型合并形式化为多教师知识蒸馏，并使用锐度感知最小化寻找平坦最小值，在视觉和NLP基准测试中达到新的SOTA。
...
Method: 1) 建立模型合并的平坦感知PAC-Bayes泛化边界，引入"跨任务异质性"项；2) 将模型合并形式化为稀缺无标签数据上的多教师知识蒸馏；3) 提出SAMerging方法，使用锐度感知最小化寻找平坦最小值。

Result: SAMerging在视觉和NLP基准测试中建立了新的最先进性能，代码已开源。

Conclusion: 通过理论分析和SAMerging方法，为模型合并提供了原则性框架，解决了系数缩放权重选择的脆弱性问题，实现了优越的泛化性能。
...
### [130] [Learning to Solve PDEs on Neural Shape Representations](https://arxiv.org/abs/2512.21311)
*Lilian Welschinger,Yilin Liu,Zican Wang,Niloy Mitra*

Main category: cs.LG

TL;DR: 提出一种无需网格、基于神经表示的表面偏微分方程求解方法，可直接在神经域中求解表面PDE，避免显式网格提取或逐实例优化


<details>
  <summary>Details</summary>
Motivation: 传统PDE求解器基于多边形/三角网格，而现代3D资产越来越多采用神经表示，这种不匹配导致无法直接在神经域中求解表面PDE，需要显式网格提取或逐实例残差训练，阻碍端到端工作流程

Method: 提出一种无网格公式，学习基于神经（局部）形状属性的局部更新算子，使表面PDE能够直接在神经数据所在位置求解。该算子与主流神经表面表示自然集成，在单个代表性形状上训练一次，可泛化到形状和拓扑变化

Result: 在解析基准测试（球体上的热方程和泊松求解）和不同表示的真实神经资产上，该方法略微优于CPM，同时与FEM保持合理接近，提供了首个在神经和经典表面表示上求解表面PDE的端到端流程

Conclusion: 该方法实现了无需显式网格化或逐实例优化的准确快速推理，同时保持可微性，为神经表示上的表面PDE求解提供了首个端到端解决方案
...
### [131] [Does the Data Processing Inequality Reflect Practice? On the Utility of Low-Level Tasks](https://arxiv.org/abs/2512.21315)
*Roy Turgeman,Tom Tirer*

Main category: cs.LG

TL;DR: 本文挑战了数据处理不等式在分类任务中的普遍适用性，证明在有限训练样本下，预处理（如去噪、编码）可以提升分类准确率，即使对于接近最优贝叶斯分类器的模型也是如此。


<details>
  <summary>Details</summary>
Motivation: 尽管数据处理不等式表明信号处理不会增加信息量，但实践中常先进行"低层"处理再执行分类任务。本文旨在理解低层处理何时及为何对分类有益，挑战传统信息论原则在实际机器学习中的适用性。

Method: 1) 对二元分类设置进行理论分析，考虑与最优贝叶斯分类器紧密相关且随训练样本增加而收敛的分类器；2) 证明对任意有限训练样本，存在能提高分类准确率的预处理方法；3) 研究类别分离度、训练集大小和类别平衡对预处理增益的影响；4) 通过理论设置的实证研究验证理论；5) 在实际基准数据集上研究去噪和编码对深度分类器性能的影响。

Result: 理论证明：对于任何有限训练样本，都存在能提高分类准确率的预处理方法。实证研究显示：预处理增益受类别分离度、训练集大小和类别平衡影响，小训练集、低类别分离度、类别不平衡时增益更大。实际深度分类器在基准数据集上的表现与理论结果一致。

Conclusion: 数据处理不等式在无限训练样本的理想情况下成立，但在有限样本的实际场景中，预处理（如去噪、编码）可以显著提升分类性能，特别是在训练数据有限、类别分离度低或类别不平衡的情况下。这为实际机器学习中预处理步骤的使用提供了理论依据。
...
### [132] [Measuring all the noises of LLM Evals](https://arxiv.org/abs/2512.21326)
*Sida Wang*

Main category: cs.LG

TL;DR: 论文提出针对LLM评估的统计噪声分析方法，定义了三种噪声类型，开发了全对配对方法，发现评估具有可预测的总噪声水平，预测噪声通常大于数据噪声，通过平均可显著提升统计功效。


<details>
  <summary>Details</summary>
Motivation: 将成熟的统计方法有效应用于LLM评估需要考虑其独特的噪声特征。LLM评估中存在不同类型的噪声，需要明确定义和测量这些噪声，以便更好地进行模型比较和显著性检验。

Method: 提出全对配对方法，将配对分析应用于所有LLM对，基于数百万个问题级预测测量三种噪声：预测噪声（同一问题生成不同答案）、数据噪声（问题抽样）、以及根据全方差定律的总噪声。

Result: 测量揭示了明确模式：1）每个评估在所有模型对中表现出特征性且高度可预测的总噪声水平；2）配对预测噪声通常超过配对数据噪声，意味着通过平均减少预测噪声可以显著增加统计功效。

Conclusion: 这些发现使实践者能够无需定制测试即可评估显著性，并在控制实验中检测更小的效应。该方法为LLM评估提供了更有效的统计工具，提高了比较实验的统计功效。
...
### [133] [Anytime Metaheuristic Framework for Global Route Optimization in Expected-Time Mobile Search](https://arxiv.org/abs/2512.20711)
...
<details>
  <summary>Details</summary>
Motivation: 静态2D连续环境中的全局路径优化问题由于目标评估的难处理性（环境连续性和运动/可见性约束的相互作用）而未被充分探索。现有方法要么采用部分离散化导致离散感知公式，要么通过固定图上的最小延迟问题启发式近似目标。

Method: 提出Milaps框架，整合新颖的辅助目标，并采用旅行送货员问题的最新随时元启发式算法。最佳策略快速生成初步解，为感知配置分配静态权重，并通过元启发式优化全局成本。

Result: 在新型大规模数据集上的评估显示，相比现有最优基线，在解质量和运行时间之间实现了更优越的权衡。定性研究展示了框架在不同场景下的灵活性。

Conclusion: Milaps框架通过基于最小延迟问题的建模方法，有效解决了连续环境中的预期时间移动搜索问题，在有限运行时间约束下表现出色，为全局路径优化提供了实用解决方案。
...
### [134] [A General Purpose Method for Robotic Interception of Non-Cooperative Dynamic Targets](https://arxiv.org/abs/2512.20769)
*Tanmay P. Patel,Erica L. Tevere,Erik H. Kramer,Rudranarayan M. Mukherjee*

Main category: cs.RO

TL;DR: 该论文提出了一个通用的自主视觉拦截框架，可在无人机、地面漫游车和太空船测试平台三种不同移动平台上实现对动态非合作目标的拦截，仅使用单目摄像头和基准标记，无需全局信息。


<details>
  <summary>Details</summary>
Motivation: 研究动机是开发一个通用的自主拦截框架，能够在有限可观测性（部分视野、传感器丢失、目标遮挡）和没有全局定位的情况下，适应不同动态特性的机器人平台，实现对动态非合作目标的可靠拦截。

Method: 方法包括：(1) 使用扩展卡尔曼滤波器在间歇性测量中进行相对姿态估计；(2) 基于历史条件的运动预测器用于动态目标轨迹传播；(3) 实时求解约束凸规划的滚动时域规划器，确保时间高效且运动学可行的拦截路径。

Result: 实验结果显示：在模拟和物理实验中表现出鲁棒性能，拦截误差低（在定点保持和场景完成时），在确定性和随机目标运动模式下成功率高，能够在Jetson Orin、VOXL2和Raspberry Pi 5等嵌入式处理器上实时执行。

Conclusion: 该框架具有通用性、鲁棒性和计算效率，能够适应不同动态特性的机器人平台，在有限可观测性和无全局定位条件下实现对动态非合作目标的可靠拦截，为异构移动系统的自主拦截问题提供了全面解决方案。
...
### [135] [YCB-Handovers Dataset: Analyzing Object Weight Impact on Human Handovers to Adapt Robotic Handover Motion](https://arxiv.org/abs/2512.20847)
*Parag Khanna,Karen Jane Dsouza,Chunyu Wang,Mårten Björkman,Christian Smith*

Main category: cs.RO

TL;DR: YCB-Handovers数据集收集了2771个人类间物体传递的运动数据，包含不同重量物体，旨在填补人机协作研究空白，为机器人运动规划提供重量敏感的行为参考。


<details>
  <summary>Details</summary>
Motivation: 当前人机协作研究中缺乏考虑物体重量影响的传递数据集，需要建立包含重量变化的人类传递行为数据，以支持更直观、适应性的机器人运动规划。

Method: 基于YCB标准数据集，收集2771个人类间物体传递的运动数据，涵盖广泛重量范围，分析重量对传递行为的影响，特别关注需要小心传递的物体。

Result: 创建了包含重量变化的YCB-Handovers数据集，提供了物体重量对人类传递行为影响的详细分析，特别识别了需要小心传递的物体与标准传递的差异。

Conclusion: YCB-Handovers数据集填补了人机协作研究的重要空白，为开发重量敏感、人类启发的机器人运动规划模型提供了宝贵资源，支持更自然的人机交互。
...
### [136] [Early warning signals for loss of control](https://arxiv.org/abs/2512.20868)
*Jasper J. van Beers,Marten Scheffer,Prashant Solanki,Ingrid A. van de Leemput,Egbert H. van Nes,Coen C. de Visser*

Main category: cs.RO

TL;DR: 论文提出了一种不依赖系统模型的通用稳定性监测方法，基于临界减速现象来预测反馈系统的失稳风险，并在无人机上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 传统工程方法依赖精确系统模型来确保稳定性，但当系统受损或偏离模型时，这些方法失效。需要一种不依赖模型的通用方法来监测系统接近失稳的状态。

Method: 利用临界减速这一非线性系统接近临界点的通用现象作为动态弹性指标，开发了不依赖系统模型的整体系统安全监测方法。

Result: 在无人机上验证了该方法能有效监测系统接近失稳的状态，为实时预警系统提供了基础。

Conclusion: 这种基于通用原理的方法可广泛应用于反应堆、飞机、自动驾驶汽车等各类受控系统，为弹性系统设计和实时预警开辟了新途径。
...
### [137] [Proprioception Enhances Vision Language Model in Generating Captions and Subtask Segmentations for Robot Task](https://arxiv.org/abs/2512.20876)
*Kanata Suzuki,Shota Shimizu,Tetsuya Ogata*

Main category: cs.RO

TL;DR: 本文提出了一种通过视频描述任务评估视觉语言模型理解机器人低级运动能力的方法，利用图像描述和轨迹数据生成任务描述并进行子任务分割。
...
Method: 提出方法通过机器人任务的图像描述和轨迹数据生成多个"场景"描述，然后总结这些描述生成完整任务描述。同时，通过比较图像描述文本嵌入的相似性进行子任务分割。在两种描述任务中，都通过向VLM提供机器人运动数据（关节和末端执行器状态）来提高性能。

Result: 通过模拟器实验验证了所提方法的有效性。

Conclusion: 该方法能够评估视觉语言模型理解机器人运动的能力，通过连接语言和运动来提高机器人模仿学习的效率，并作为基础模型性能的衡量标准。
...
### [138] [Stretchable and High-Precision Optical Tactile Sensor for Trajectory Tracking of Parallel Mechanisms](https://arxiv.org/abs/2512.20888)
*Yiding Nie,Dongliang Fan,Jiatai Huang,Chunyu Liu,Jian S. Dai*

Main category: cs.RO

TL;DR: 提出基于连续光谱滤波原理的可拉伸触觉传感器，实现超高空间分辨率，具有自解耦能力和抗拉伸弯曲干扰特性


<details>
  <summary>Details</summary>
Motivation: 可拉伸传感器在软体机器人、医疗设备和人机交互中应用前景广阔，但现有离散传感策略（如传感器阵列和分布式传感器）难以同时实现高空间分辨率、自解耦能力和对离轴刺激的不敏感性

Method: 开发基于连续光谱滤波原理的可拉伸触觉传感器，通过连续光谱响应实现超高分辨率，传感器具有设计可扩展性和交互鲁棒性，能承受穿刺和切割

Result: 传感器在拉伸和弯曲状态下仍保持高线性空间响应（0.996），实现高连续空间分辨率（7 μm）和力分辨率（5 mN），集成到平面并联机构中实现实时精确轨迹跟踪（旋转分辨率：0.02°）

Conclusion: 基于连续光谱滤波原理的可拉伸触觉传感器成功解决了传统离散传感策略的局限性，为软体机器人、医疗设备和人机交互应用提供了高性能的传感解决方案
...
### [139] [Certifiable Alignment of GNSS and Local Frames via Lagrangian Duality](https://arxiv.org/abs/2512.20931)
*Baoshan Song,Matthew Giamou,Penggao Yan,Chunxi Xia,Li-Ta Hsu*

Main category: cs.RO

TL;DR: 提出一种全局最优的GNSS帧对齐求解器，通过凸松弛处理原始伪距或多普勒测量，提供可验证的最优解，即使在仅有2颗卫星和2D运动场景下也能保证最优性。


<details>
  <summary>Details</summary>
Motivation: 现有GNSS帧对齐方法在卫星信号受限环境中表现不佳，依赖大量卫星或使用局部优化方法无法保证解的最优性，且无法验证解的正确性。

Method: 将帧对齐问题建模为非凸QCQP问题，通过拉格朗日对偶松弛为凹问题，提供原问题的下界成本，进行松弛紧致性和可观测性分析以推导可验证最优性准则。

Result: 实验表明该方法在仅有2颗卫星多普勒测量和2D车辆运动场景下仍能提供可验证最优解，而传统VOBA方法和先进GVINS技术可能失败或收敛到局部最优解。
...
### [140] [ETP-R1: Evolving Topological Planning with Reinforcement Fine-tuning for Vision-Language Navigation in Continuous Environments](https://arxiv.org/abs/2512.20940)
*Shuhao Ye,Sitong Mao,Yuxiang Cui,Xuan Yu,Shichao Zhai,Wen Chen,Shunbo Zhou,Rong Xiong,Yue Wang*

Main category: cs.RO

TL;DR: ETP-R1框架通过大规模数据预训练和强化微调，将图基VLN-CE模型的性能提升到新高度，在R2R-CE和RxR-CE基准测试中达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 当前基于图的VLN-CE方法虽然高效，但在利用大规模数据和先进训练范式方面落后于基于大视觉语言模型的方法，需要弥合这一差距。

Method: 1. 使用Gemini API构建高质量、大规模预训练数据集；2. 统一R2R和RxR数据进行联合预训练；3. 采用三阶段训练范式，首次将闭环在线强化微调应用于图基VLN-CE模型，使用GRPO算法。

Result: 在R2R-CE和RxR-CE基准测试的所有主要指标上都达到了新的最先进性能。

Conclusion: ETP-R1框架成功地将大规模数据预训练和强化微调范式应用于图基VLN-CE模型，显著提升了性能，证明了该方法的有效性。
...
### [141] [Multimodal Sensing for Robot-Assisted Sub-Tissue Feature Detection in Physiotherapy Palpation](https://arxiv.org/abs/2512.20992)
*Tian-Ao Ren,Jorge Garcia,Seongheon Hong,Jared Grinberg,Hojung Choi,Julia Di,Hao Li,Dmitry Grinberg,Mark R. Cutkosky*

Main category: cs.RO

TL;DR: 开发了一种集成了视觉触觉成像和6轴力扭矩传感器的多模态传感器，用于机器人触诊，能更可靠地检测软组织下的细微特征。


<details>
  <summary>Details</summary>
Motivation: 机器人触诊依赖力传感，但在软组织环境中力信号变化大，无法可靠地揭示细微的皮下特征，需要更有效的传感方法。

Method: 开发了一种紧凑的多模态传感器，集成了高分辨率视觉触觉成像和6轴力扭矩传感器，在具有不同皮下肌腱几何形状的硅胶模型上进行实验。

Result: 单独使用力信号经常产生模糊响应，而触觉图像能清晰揭示存在、直径、深度、交叉和多重性等结构差异；结合触觉和力模态可实现稳健的皮下特征检测和受控机器人触诊。

Conclusion: 结合触觉和力传感的多模态方法比单独使用力传感更有效，能实现更可靠的皮下特征检测和安全的机器人触诊交互。
...
### [142] [Tracing Energy Flow: Learning Tactile-based Grasping Force Control to Prevent Slippage in Dynamic Object Interaction](https://arxiv.org/abs/2512.21043)
...
<details>
  <summary>Details</summary>
Motivation: 机器人在动态物体交互中调节抓握力以防止滑动是一个基本挑战，特别是当物体有未知属性、多个滚动接触点且外部传感不可靠时。人类仅凭触觉就能快速调节抓握力，这启发了本研究

Method: 提出物理信息能量抽象方法，将物体建模为虚拟能量容器。通过手指施加功率与物体保留能量之间的不一致性来推断滑动感知的稳定性。基于此抽象，采用基于模型的学习和规划从触觉传感中建模能量动态，并进行实时抓握力优化

Result: 在仿真和硬件实验中，该方法能在几分钟内从零开始学习抓握力控制，有效减少滑动，延长各种运动-物体对的抓握持续时间，且不依赖外部传感或物体先验知识

Conclusion: 基于物理能量抽象的方法使机器人能够仅通过触觉快速学习抓握力控制，解决了动态物体交互中的滑动问题，为机器人灵巧操作提供了新思路
...
### [143] [Language-Guided Grasp Detection with Coarse-to-Fine Learning for Robotic Manipulation](https://arxiv.org/abs/2512.21065)
*Zebin Jiang,Tianle Jin,Xiangtong Yao,Alois Knoll,Hu Cao*

Main category: cs.RO

TL;DR: 提出LGGD方法，通过粗到细的学习范式实现语言引导的机器人抓取，利用CLIP嵌入和层次化跨模态融合，在复杂场景中实现语义对齐和指令自适应抓取预测。


<details>
  <summary>Details</summary>
Motivation: 当前语言引导的抓取方法通常采用浅层融合策略，导致语义基础有限，语言意图与视觉抓取推理之间的对齐较弱。在非结构化、杂乱和语义多样的环境中，机器人需要更好地理解任务相关的自然语言指令。

Method: 提出LGGD方法，采用粗到细的学习范式：1) 利用CLIP视觉和文本嵌入构建层次化跨模态融合管道，逐步将语言线索注入视觉特征重建过程；2) 引入语言条件动态卷积头(LDCH)，基于句子级特征混合多个卷积专家，实现指令自适应的粗掩码和抓取预测；3) 最终细化模块增强复杂场景中的抓取一致性和鲁棒性。

Result: 在OCID-VLG和Grasp-Anything++数据集上的实验表明，LGGD超越了现有的语言引导抓取方法，对未见过的物体和多样化语言查询表现出强大的泛化能力。在实际机器人平台上的部署证明了该方法在执行准确、指令条件抓取动作方面的实际有效性。

Conclusion: LGGD方法通过精细的视觉-语义对齐和指令自适应预测，显著提升了语言引导机器人抓取的性能，在复杂环境中表现出优异的泛化能力和实际应用价值。
...
### [144] [Global End-Effector Pose Control of an Underactuated Aerial Manipulator via Reinforcement Learning](https://arxiv.org/abs/2512.21085)
*Shlok Deshmukh,Javier Alonso-Mora,Sihao Sun*

Main category: cs.RO

TL;DR: 本文研究了一种轻量级2自由度机械臂与四旋翼无人机结合的空中机械臂系统，通过强化学习实现六自由度末端执行器姿态控制，在外部扰动下保持厘米级位置精度和度级方向精度。


<details>
  <summary>Details</summary>
Motivation: 空中机械臂面临严格的重量和机械复杂度限制。轻量级2自由度机械臂设计虽然简化了结构并减少了负载，但也带来了欠驱动和对外部扰动敏感等挑战，特别是在操纵重物和推动任务中。

Method: 采用强化学习方法，在仿真环境中训练PPO（近端策略优化）智能体，生成四旋翼加速度和机体角速度的前馈命令以及关节角度目标。这些命令分别由增量非线性动态反演姿态控制器和PID关节控制器进行跟踪。

Result: 飞行实验表明，系统能够实现厘米级的位置精度和度级的方向精度，在外部力扰动下表现出鲁棒性能。

Conclusion: 研究结果突显了基于学习的控制策略在实现简单、轻量级平台进行接触丰富的空中操纵方面的潜力。
...
### [145] [Robust and Efficient MuJoCo-based Model Predictive Control via Web of Affine Spaces Derivatives](https://arxiv.org/abs/2512.21109)
*Chen Liang,Daniel Rakita*

Main category: cs.RO

TL;DR: WASP导数作为有限差分法的替代方案集成到MuJoCo MPC中，在多个机器人任务中实现了高达2倍的速度提升和更稳定的性能。


<details>
  <summary>Details</summary>
Motivation: MJPC（MuJoCo MPC）库虽然提供了现成的MPC算法，但其依赖有限差分法计算导数，这在高自由度系统或复杂场景中成为性能瓶颈，限制了实时控制应用。

Method: 将Web of Affine Spaces（WASP）导数作为即插即用替代方案集成到MJPC中，WASP通过重用先前相关导数计算的信息，高效计算准确的导数近似序列。

Result: WASP在多个MJPC任务中表现优异：无缝集成、性能稳定，与基于导数的规划器（如iLQG）结合时相比FD后端实现高达2倍加速，且优于MJPC的随机采样规划器。

Conclusion: WASP导数为MJPC提供了高效可靠的导数计算方案，特别适合MPC的迭代细粒度更新，作者开源了集成WASP的MJPC实现以支持采用和未来研究。
...
### [146] [Flocking phase transition and threat responses in bio-inspired autonomous drone swarms](https://arxiv.org/abs/2512.21196)
*Matthieu Verdoucq,Dari Trendafilov,Clément Sire,Ramón Escobedo,Guy Theraulaz,Gautier Hattenberger*

Main category: cs.RO

TL;DR: 该论文提出了一种受动物群体启发的无人机三维集群算法，通过调节局部对齐和吸引两种相互作用增益，实现了集群行为相变，并在临界区域展现出最佳的响应性和恢复能力。


<details>
  <summary>Details</summary>
Motivation: 受动物群体集体运动的启发，为自主空中集群提供强大的设计原则。动物群体通过简单的局部交互规则展现出复杂的集体行为，这为无人机集群设计提供了生物灵感。

Method: 提出生物启发的三维集群算法，每个无人机仅与最小数量的有影响力邻居交互，依赖局部对齐和吸引两种线索。通过系统调节这两种交互增益，绘制相图揭示集群行为相变。使用十架无人机进行户外实验，并结合校准的飞行动力学模型进行仿真验证。

Result: 相图显示集群行为存在尖锐相变，包括集群和编队两种状态，临界区域中敏感性、极化波动和重组能力达到峰值。实验表明在相变附近操作能增强对外部干扰的响应性。当面对入侵者时，集群能快速执行集体转向、瞬态扩展，并在几秒内可靠恢复高对齐度。

Conclusion: 最小局部交互规则足以产生多种集体行为相，简单的增益调节为无人机集群提供了调整稳定性、灵活性和恢复力的有效机制。生物启发的设计原则能实现具有响应性和适应性的自主空中集群。
...
### [147] [Quadrupped-Legged Robot Movement Plan Generation using Large Language Model](https://arxiv.org/abs/2512.21293)
*Muhtadin,Vincentius Gusti Putu A. B. M.,Ahmad Zaini,Mauridhi Hery Purnomo,I Ketut Eddy Purnama,Chastine Fatichah*

Main category: cs.RO

TL;DR: 提出基于大语言模型的四足机器人自然语言控制框架，通过分布式架构将高层指令处理卸载到外部服务器，在结构化室内环境中验证了超过90%的成功率。


<details>
  <summary>Details</summary>
Motivation: 传统四足机器人控制界面需要专业知识，使用门槛高。为了解决这个问题，本文旨在开发一个直观的自然语言控制框架，降低操作难度。

Method: 采用分布式架构，将大语言模型的高层指令处理卸载到外部服务器，以克服机器人本地的计算限制。系统通过实时传感器融合（LiDAR、IMU和里程计）将LLM生成的计划转化为可执行的ROS导航命令。

Result: 在结构化室内环境中进行了四个不同场景的实验验证，从单房间任务到复杂的跨区域导航。结果显示系统具有鲁棒性，在所有场景中实现了超过90%的总体成功率。

Conclusion: 验证了基于卸载的LLM规划方法在现实世界四足机器人自主部署中的可行性，为降低机器人操作门槛提供了有效解决方案。
...
### [148] [Schrödinger's Navigator: Imagining an Ensemble of Futures for Zero-Shot Object Navigation](https://arxiv.org/abs/2512.21201)
...
<details>
  <summary>Details</summary>
Motivation: 现有零样本物体导航方法在真实杂乱环境中表现不佳，特别是在存在严重遮挡、未知风险和动态移动目标的情况下。需要一种能够处理不确定性和预见未来的导航框架。

Method: 受薛定谔思想实验启发，将未观测空间视为一组可能的未来世界。基于自中心视觉输入和三条候选轨迹，轨迹条件3D世界模型沿每条路径想象未来观测。将想象的3D观测融合到导航地图中，更新价值地图，指导策略选择能避免遮挡、减少不确定空间暴露并更好跟踪移动目标的轨迹。

Result: 在Go2四足机器人上的实验表明，在严重静态遮挡、未知风险和动态移动目标三种挑战性场景中，Schrödinger's Navigator在自定位、物体定位和总体成功率方面均优于现有零样本物体导航基线方法。

Conclusion: 轨迹条件3D想象能有效实现鲁棒的零样本物体导航，特别是在遮挡严重的环境中。该方法使机器人能够超越遮挡预见未来，无需额外绕行或密集全局建图。
...
### [149] [Wireless Center of Pressure Feedback System for Humanoid Robot Balance Control using ESP32-C3](https://arxiv.org/abs/2512.21219)
*Muhtadin,Faris Rafi Pramana,Dion Hayu Fandiantoro,Moh Ismarintan Zazuli,Atar Fuady Babgei*

Main category: cs.RO

TL;DR: 本文提出了一种用于人形机器人（特别是舞蹈机器人）的无线嵌入式平衡系统，通过脚部集成的四个称重传感器和ESP32-C3微控制器实时估计压力中心，采用PID控制策略调整躯干、髋部和踝关节，在3度倾斜面上实现了100%的单腿抬升平衡成功率。


<details>
  <summary>Details</summary>
Motivation: 人形机器人在单支撑阶段保持稳定性是一个基本挑战，特别是对于需要复杂动作和高机械自由度的舞蹈机器人。传统的系留传感器配置通常会限制关节运动并引入机械噪声。

Method: 设计了一个无线嵌入式平衡系统，包括集成了四个称重传感器的定制脚部单元和ESP32-C3微控制器，用于实时估计压力中心。CoP数据无线传输到主控制器，减少29自由度VI-ROSE人形机器人的布线复杂性。采用PID控制策略，基于CoP反馈调整躯干、髋部和踝关节的滚动关节。

Result: 实验表征显示传感器具有高精度，平均测量误差为14.8克。在3度倾斜面上进行单腿抬升任务时，优化PID参数（Kp=0.10，Kd=0.005）后，提出的控制系统实现了100%的平衡保持成功率。

Conclusion: 无线CoP反馈能有效增强人形机器人的姿态稳定性，同时不损害其机械灵活性。该系统为需要高动态性能的机器人应用提供了有前景的解决方案。
...
### [150] [Relative Localization System Design for SnailBot: A Modular Self-reconfigurable Robot](https://arxiv.org/abs/2512.21226)
*Shuhan Zhang,Tin Lun Lam*

Main category: cs.RO

TL;DR: 本文为SnailBot模块化自重构机器人设计并实现了一种相对定位系统，该系统集成了ArUco标记识别、光流分析和IMU数据处理，通过统一的融合框架实现鲁棒准确的相对定位。


<details>
  <summary>Details</summary>
Motivation: 为模块化自重构机器人SnailBot开发一个可靠的相对定位系统，以支持协作机器人任务，解决在动态环境中实现准确相对定位的挑战。

Method: 系统集成了三种技术：ArUco标记识别、光流分析和IMU数据处理，通过基于规则的融合策略将这些数据源统一到一个框架中，实现鲁棒的相对定位。

Result: 实验验证表明系统在实时操作中有效，基于规则的融合策略确保了在动态场景中的可靠性，系统表现出良好的定位精度和鲁棒性。

Conclusion: 该相对定位系统为模块化机器人系统提供了可扩展部署的潜力，统一的融合框架能够有效支持协作机器人任务中的相对定位需求。
...
### [151] [UniTacHand: Unified Spatio-Tactile Representation for Human to Robotic Hand Skill Transfer](https://arxiv.org/abs/2512.21233)
...
<details>
  <summary>Details</summary>
Motivation: 机器人触觉感知对于灵巧操作至关重要，但大规模真实世界机器人触觉数据收集困难。人类操作数据收集成本低，但人类与机器人触觉数据存在不对齐问题，难以将人类数据学习的策略迁移到机器人。

Method: 1) 将人类手和机器人手的触觉信号投影到MANO手模型的形态一致的2D表面空间，统一异构数据结构；2) 引入对比学习方法在统一潜在空间中对齐人类和机器人触觉数据，仅需10分钟配对数据训练。

Result: 实现了零样本触觉策略从人类到真实机器人的迁移，能泛化到预训练未见过的物体。混合人类和机器人演示数据的协同训练相比仅使用机器人数据，获得更好的性能和数据效率。

Conclusion: UniTacHand为触觉灵巧手提供了通用、可扩展且数据高效的学习路径，解决了人类-机器人触觉数据对齐问题，促进了触觉策略的迁移学习。
...
### [152] [RoboCade: Gamifying Robot Data Collection](https://arxiv.org/abs/2512.21235)
*Suvir Mirchandani,Mia Tang,Jiafei Duan,Jubayer Ibn Hamid,Michael Cho,Dorsa Sadigh*

Main category: cs.RO

TL;DR: 开发了RoboCade游戏化远程遥操作平台，通过游戏化设计让普通用户参与机器人演示数据收集，解决了传统数据收集成本高、规模有限的问题，并验证了该数据能有效提升下游策略训练效果。


<details>
  <summary>Details</summary>
Motivation: 模仿学习需要大量人类演示数据，但传统数据收集方法成本高昂、过程繁琐，限制了训练数据的规模。为了解决这一可扩展性挑战，研究者希望通过游戏化设计吸引更广泛的用户参与数据收集。

Method: 开发了RoboCade游戏化远程遥操作平台，在系统界面中嵌入视觉反馈、音效、目标可视化、进度条、排行榜和徽章等游戏化元素。提出了构建游戏化任务的原则，使任务结构与下游目标任务重叠。在三个操作任务（空间排列、扫描和插入）上实例化了该平台。

Result: 通过平台收集演示数据集，与游戏化数据共同训练机器人策略可将非游戏化目标任务的成功率提高16-56%。用户研究表明，新手用户认为游戏化平台比标准非游戏化平台更有趣（+24%）。

Conclusion: 游戏化数据收集是一种有前景的可扩展、易访问且引人入胜的方法，能够有效收集机器人演示数据，解决传统数据收集的规模限制问题。
...
TL;DR: 提出IPIM原语计算，使基于采样的检测规划器能完全使用神经SDF隐式模型，在保持轨迹质量的同时大幅减少内存使用（最高达70倍）


<details>
  <summary>Details</summary>
Motivation: 基础设施老化复杂化使检测规划更关键，现有基于采样的检测规划器虽快但内存消耗大，特别是对于大型复杂结构。隐式模型（如神经SDF）能高效表示复杂结构，但现有规划原语主要针对显式模型设计，需要在隐式和显式模型间频繁转换

Method: 提出IPIM（Inspection Planning Primitives with Implicit Models）原语计算集合，使基于采样的检测规划器能够完全使用神经SDF隐式表示进行规划，无需转换为显式模型

Result: 在三个场景（包括超过9200万个三角网格面的复杂真实结构）的评估表明，即使使用基本的基于采样规划器配合IPIM，也能生成与最先进规划器质量相当的检测轨迹，同时内存使用减少高达70倍

Conclusion: IPIM原语使基于采样的检测规划器能够有效利用神经SDF隐式模型，在保持轨迹质量的同时显著降低内存需求，特别适用于大型复杂结构的检测规划
