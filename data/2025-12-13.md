...
TL;DR: Any4D是一个可扩展的多视角变换器，用于度量尺度、密集前馈4D重建，能够直接生成N帧的逐像素运动和几何预测，支持多种模态输入，在精度和计算效率上均有显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注2视角密集场景流或稀疏3D点跟踪，而4D重建方法通常局限于单目RGB视频输入。需要开发一个能够处理多种传感器模态、更灵活高效的4D重建框架。

Method: 采用模块化的4D场景表示：将每视角的4D预测编码为以局部相机坐标表示的自我中心因子（深度图和相机内参）和以全局世界坐标表示的他者中心因子（相机外参和场景流）。使用可扩展的多视角变换器架构。

Result: 在多种设置下均取得优越性能：精度方面误差降低2-3倍，计算效率提升15倍，为多个下游应用开辟了新途径。

Conclusion: Any4D提供了一个灵活高效的4D重建框架，能够处理多种传感器模态，在精度和速度上均显著优于现有方法，具有广泛的应用前景。
...
TL;DR: MMSI-Video-Bench是一个全面评估多模态大语言模型视频空间智能的基准测试，包含1106个问题，涵盖感知、规划、预测和跨视频推理四个层次，揭示了当前模型与人类在空间理解上的巨大差距。
...
Method: 构建了一个完全人工标注的视频空间智能基准测试，基于四层框架（感知、规划、预测、跨视频推理），包含1106个问题，源自25个数据集和内部视频的1278个片段，每个项目由3DV专家精心设计和审查。

Result: 评估了25个开源和专有MLLM，发现显著的人机差距：许多模型表现接近随机，最佳推理模型落后人类近60%。空间微调模型在基准测试上泛化能力仍然不足，存在几何推理、运动基础、长时预测和跨视频对应等系统性失败。

Conclusion: MMSI-Video-Bench为推进视频空间智能研究建立了坚实的测试平台，揭示了当前模型的局限性，并发现典型帧采样策略、3D空间线索和思维链提示在推理密集型基准测试中效果有限。
...
TL;DR: BabyVLM-V2是一个基于婴儿发展轨迹的视觉语言模型框架，通过纵向多模态预训练集和认知评估工具箱，在紧凑模型上实现了与GPT-4o竞争的性能。
...
Method: 1) 构建纵向、多方面的婴儿中心视听语料预训练集；2) 开发DevCV工具箱，将NIH Baby Toolbox的视觉相关测量转化为10个多模态任务基准；3) 使用紧凑模型从头开始预训练。

Result: 紧凑模型在DevCV工具箱上表现出色，在某些任务上超越了GPT-4o，证明了发展性预训练的有效性。

Conclusion: BabyVLM-V2框架为发展性合理的视觉基础模型预训练研究提供了原则性统一方法，有望加速该领域进展。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法针对不同的4D一致性任务（如新颖视图合成、文本到视频等）采用分散的特定任务模型，这些方法在不相交的数据集上训练，缺乏统一性。需要一种能够泛化到多种4D任务的一般性框架。

Method: OmniView将空间、时间和视图条件分开表示，允许这些输入的灵活组合。这种分离表示使得模型能够处理静态、动态和多视图输入，进行时间前后外推，以及从文本或图像提示生成具有完整相机控制的视频。

Result: 在多个基准测试中，OmniView与特定任务模型竞争激烈：在LLFF多视图NVS数据集上图像质量得分提升33%，在Neural 3D Video动态NVS基准上提升60%，在RE-10K静态相机控制上提升20%，在文本条件视频生成中相机轨迹误差减少4倍。

Conclusion: OmniView展示了单一模型在多种4D任务上的强大泛化能力，证明了通用4D视频模型的可行性，为统一的4D一致性处理提供了有效框架。
...
TL;DR: Mull-Tokens是一种模态无关的潜在令牌方法，允许模型在图像和文本模态之间自由思考，以解决多模态推理问题，在空间推理基准上比基线方法平均提升3%。


<details>
  <summary>Details</summary>
Motivation: 现有多模态模型在空间、时间、可用性等现实世界推理方面存在局限性，它们依赖专业工具、昂贵的图像生成或手工制作的数据来在文本和图像思维之间切换，需要更简单、可扩展的解决方案。
...
TL;DR: AlcheMinT是一个统一框架，通过引入显式时间戳条件实现主题驱动视频生成中的细粒度时序控制，解决了现有方法在主题出现和消失时间控制上的不足。
...
Method: 提出AlcheMinT框架：1) 引入新颖的位置编码机制，支持时间间隔编码并与预训练视频生成模型的位置嵌入无缝集成；2) 加入主题描述性文本标记以加强视觉身份与视频标题的绑定；3) 通过标记级连接避免额外交叉注意力模块，参数开销极小。

Result: 实验结果表明，AlcheMinT在视觉质量上匹配最先进的视频个性化方法，同时首次实现了视频内多主题生成的精确时序控制，在主题身份保持、视频保真度和时序遵循方面表现优异。

Conclusion: AlcheMinT通过显式时间戳条件实现了主题驱动视频生成中的精确时序控制，为组合视频合成、故事板制作等应用提供了有效解决方案，且保持了高质量生成效果。
...
TL;DR: 首次系统研究RL在文本到3D自回归生成中的应用，提出Hi-GRPO层次优化方法，开发了首个RL增强的文本到3D模型AR3D-R1


<details>
  <summary>Details</summary>
Motivation: 虽然强化学习在2D图像生成中已证明有效，但由于3D对象具有更高的空间复杂性，需要全局一致的几何结构和细粒度局部纹理，使得RL在3D生成中的应用仍未被充分探索。3D生成对奖励设计和RL算法特别敏感。

Method: 1. 奖励设计：评估奖励维度和模型选择，发现与人类偏好对齐至关重要，通用多模态模型为3D属性提供稳健信号
2. RL算法：研究GRPO变体，强调令牌级优化的有效性，并进一步研究训练数据和迭代的扩展
3. 文本到3D基准：引入MME-3DR基准来测量3D生成模型的隐式推理能力
4. 高级RL范式：基于3D生成的天然层次结构，提出Hi-GRPO方法，通过专用奖励集成优化全局到局部的层次3D生成

Result: 基于这些洞察开发了AR3D-R1，这是首个RL增强的文本到3D模型，能够从粗略形状到纹理细化进行专业生成。代码已开源。

Conclusion: 这项研究为RL驱动的3D生成推理提供了重要见解，首次系统探索了RL在文本到3D自回归生成中的应用，提出了有效的层次优化方法，并开发了实用的RL增强模型。
...
TL;DR: SceneMaker是一个解耦的3D场景生成框架，通过分离去遮挡模型和3D物体生成，并整合全局-局部注意力机制进行姿态估计，在严重遮挡和开放集场景中实现高质量几何和准确姿态。
...
Method: 1. 将去遮挡模型与3D物体生成解耦，利用图像数据集和收集的去遮挡数据集增强开放集遮挡模式多样性；2. 提出统一姿态估计模型，整合全局和局部机制的自注意力和交叉注意力；3. 构建开放集3D场景数据集扩展姿态估计模型的泛化能力。

Result: 综合实验表明，该解耦框架在室内和开放集场景中均表现出优越性。
...
TL;DR: LLMs可用于大型用户设施的提案选择，提供比传统人工评审更可扩展、一致且经济高效的替代方案，在排名和识别高潜力提案方面表现不逊于人类评审员，成本降低两个数量级。


<details>
  <summary>Details</summary>
Motivation: 传统人工评审存在提案间相关性弱、评审员偏见和不一致性问题。基于成对偏好的方法在逻辑上更优越，但二次方工作量使其对人类评审员不切实际。LLMs可以解决这一限制。

Method: 利用LLMs进行提案排名，基于Spallation Neutron Source三个光束线的精心策划提案和发表记录。使用成对偏好方法，并通过嵌入模型进行提案相似性的定量评估。

Result: LLM排名与人类排名强相关（Spearman ρ≈0.2-0.8，去除10%异常值后≥0.5）。在识别具有高发表潜力的提案方面，LLM表现不逊于人类评审员，成本降低两个数量级以上。

Conclusion: LLMs为大型用户设施的提案选择提供了可扩展、一致且经济高效的替代方案，不仅能够有效排名提案，还能进行人类难以完成的先进分析，如提案相似性评估。
...
Conclusion: 提出的节点级剪枝框架在电路发现中实现了更好的可扩展性和粒度控制，能够识别更小、更精确的电路，同时大幅降低计算资源需求。
...
<details>
  <summary>Details</summary>
Motivation: 水下环境自主导航面临GPS缺失、能见度差、水下障碍物等挑战，需要开发鲁棒的导航方法来解决这些问题。

Method: 采用基于PPO算法的深度强化学习方法，结合目标导向导航信息、虚拟占用网格和操作区域边界射线投射的观测空间，并与DWA确定性运动规划器进行对比。

Result: PPO策略在高度杂乱环境中持续优于DWA方法，具有更好的局部适应性和更少的碰撞，同时验证了从仿真到真实世界的可迁移性。

Conclusion: 深度强化学习在水下机器人自主导航中具有实际应用价值，PPO算法在复杂环境中表现出色，且仿真训练的策略可以成功迁移到真实环境。
...
<details>
  <summary>Details</summary>
Motivation: 时间差分方法通过自举机制高效学习状态和动作价值，但容易产生自举偏差。块评论家方法通过估计短动作序列的价值来加速价值备份，但从中提取策略存在挑战：策略必须以开环方式输出整个动作块，这在需要策略反应性的环境中可能次优，且随着块长度增长建模困难。

Method: 提出解耦评论家与策略的块长度，让策略在较短动作块上操作。通过优化策略对抗一个蒸馏评论家，该评论家通过乐观地从原始块评论家回溯来近似部分动作块扩展为完整块时可实现的最大价值。

Result: 在具有挑战性的长时域离线目标条件任务上评估，该方法可靠地优于先前方法。

Conclusion: 该方法保留了多步价值传播的优势，同时避免了开环次优性和学习长动作块策略的困难，在长时域任务中表现出色。
...
TL;DR: 线性模型在仅使用历史室内温度数据进行长期预测的任务中，表现优于更复杂的Transformer架构，其中DLinear模型在所有数据集分割上获得最佳精度。


<details>
  <summary>Details</summary>
Motivation: 研究仅使用历史室内温度数据进行长期预测的挑战性单变量设置，比较线性模型和Transformer家族模型在这种外生变量限制条件下的表现。

Method: 使用Linear、NLinear、DLinear、Transformer、Informer和Autoformer模型，在标准化的训练、验证和测试分割下进行长期外生变量温度预测评估。

Result: 线性基线模型（Linear、NLinear、DLinear）在性能上一致优于更复杂的Transformer家族架构，其中DLinear在所有分割上实现了最佳的整体精度。

Conclusion: 精心设计的线性模型在仅使用外生变量的挑战性时间序列预测场景中仍然是强大的基线方法，复杂模型不一定带来性能优势。
...
Method: 基于连续扩散的比率迁移学习，提出离散扩散模型的引导迁移学习(GTL)。该方法不修改预训练去噪器，通过引导采样从目标分布生成样本。支持离散时间和连续时间两种扩散模型，并提供高效引导采样器，通过规划器选择位置和候选词来减少计算量。

Result: 在序列数据（包括合成马尔可夫链和语言建模）上评估GTL，证明其有效性。高效采样器使大规模语言建模在大型词汇和长序列上变得实用。

Conclusion: GTL为离散扩散模型提供了一种无需微调的迁移学习方法，通过引导采样和高效采样器解决了计算效率问题，使大规模语言建模应用成为可能。
...
Method: 提出1-swap算法：1) 通过强制每行相同稀疏度解耦行约束；2) 使用校准数据的Gram矩阵高效计算最优权重交换；3) 从任意剪枝掩码开始，在GPU上高效运行，几乎无超参数

Result: 相比Wanda方法，单层剪枝误差降低高达60%；在GPT架构上持续改善困惑度和零样本准确率

Conclusion: 通过解耦行约束和使用Gram矩阵，将掩码选择问题变得可处理，提出的1-swap算法在大语言模型规模上高效且有效
...
TL;DR: 提出一种让LLM能够同时思考、倾听和生成输出的方法，无需额外训练即可实现实时推理


<details>
  <summary>Details</summary>
Motivation: 现有LLM推理时需要先完成思考再给出答案，这种顺序交互方式不适合需要实时响应的应用场景（如语音助手），而人类可以异步地听、想、说

Method: 利用旋转嵌入的特性，使原本设计用于顺序交互的LLM能够同时进行思考、倾听和生成输出，无需额外训练

Result: 在数学、常识和安全推理任务上评估，方法能够生成准确的思考增强答案，将首个非思考token的生成时间从几分钟减少到≤5秒，整体实时延迟降低6-11倍

Conclusion: 通过利用旋转嵌入的特性，成功实现了LLM的异步推理能力，使其能够在实时交互场景中同时思考、倾听和生成响应
...
TL;DR: 该论文提出了一种名为Derf的新型逐点函数，作为归一化层的替代方案，在多个领域超越了LayerNorm、RMSNorm和Dynamic Tanh的性能
...
Method: 首先研究逐点函数的内在特性如何影响训练和性能，基于这些发现进行大规模函数设计搜索，最终引入Derf(x) = erf(αx + s)，其中erf(x)是重新缩放的高斯累积分布函数
...
Result: 在两个公共基准测试（Digit-Five和DomainNet）上，DaSH比最先进的数据选择基线方法在准确率上提升了高达26.2%，同时需要显著更少的探索步骤。消融实验表明DaSH对低资源设置和缺乏相关数据集的情况具有鲁棒性

Conclusion: DaSH方法适用于实际多源学习工作流中的可扩展和自适应数据集选择，能够有效解决数据集选择问题，在资源约束下提升下游性能
...
<details>
  <summary>Details</summary>
Motivation: 传统归一化流需要精确的可逆变换，导致因果解码成为主要瓶颈。TARFlow等变体虽然结合了Transformer和自回归流，但因果解码限制了效率和灵活性。

Method: 提出BiFlow框架，放弃精确解析逆的要求，学习一个近似噪声到数据的逆映射模型。这使得可以使用更灵活的损失函数和架构设计。

Result: 在ImageNet上，相比因果解码方法，BiFlow提高了生成质量，同时将采样速度加速了高达两个数量级（100倍）。在基于NF的方法中达到最先进水平，在单次评估方法中具有竞争力。

Conclusion: BiFlow通过近似逆映射解决了NF的因果解码瓶颈，为归一化流这一经典范式提供了更灵活高效的框架，有望推动该领域的进一步发展。
