...
TL;DR: 这篇综述论文探讨了如何利用无动作标注的视频数据来学习机器人操控的控制接口，提出了一个以接口为中心的分类法，并分析了不同方法的控制集成特性。


<details>
  <summary>Details</summary>
Motivation: 视频数据包含了丰富的物理动态信息，能够捕捉物体运动、接触展开和场景演化等物理过程，且不需要机器人动作标签。然而，将视频中的时间结构转化为可靠的机器人控制仍然是一个开放挑战，因为视频缺乏动作监督，并且在体现、视角和物理约束方面与机器人经验存在差异。

Method: 论文提出了一个以接口为中心的分类法，将方法分为三个家族：1）直接视频-动作策略（保持接口隐式）；2）潜在动作方法（通过紧凑的学习中间层路由时间结构）；3）显式视觉接口（预测可解释的目标供下游控制使用）。论文分析了每种方法的控制集成特性，包括闭环方式、执行前可验证性以及故障引入点。

Result: 通过对不同家族方法的综合分析，论文发现最紧迫的开放挑战集中在机器人集成层——将视频衍生预测连接到可靠机器人行为的机制。论文为填补这一差距指出了研究方向。

Conclusion: 视频数据为机器人操控学习提供了丰富的物理动态信息，但需要有效的接口设计来弥合视频观察与机器人控制之间的差距。未来的研究应重点关注机器人集成层的开发，以确保视频衍生预测能够可靠地转化为实际的机器人行为。
...
TL;DR: COMB是一个紧凑、开源、模块化的机电平台，专门设计用于在标准观察蜂箱框架内进行蜂巢内实验，集成了XY定位平台、可移动访问窗口、可互换负载模块和嵌入式控制架构。
...
Method: 设计了一个紧凑的开源模块化机电平台，包含XY定位平台、可移动访问窗口（用于通过蜂巢边界进行密封工具访问）、可互换负载模块和嵌入式控制架构，支持可重复轨迹执行和信号生成。

Result: 通过三个代表性模块展示了平台能力：仿生舞蹈和信号负载、近距离蜂巢扫描仪、用于局部振荡刺激的电磁翅膀驱动器；通过工程特性验证了平台性能，包括摇摆轨迹执行跟踪、重复蜂巢马赛克的多图像拼接以及翅膀驱动器的视频光谱分析。

Conclusion: COMB作为可重复使用的实验机器人平台，用于受控的蜂巢内传感和驱动，是早期特定任务蜜蜂机器人系统的紧凑通用后继者。
...
TL;DR: 该综述系统分析了无人水下航行器（UUV）的传感器驱动规划与控制方法，重点关注在复杂水下环境中基于实时传感器输入（如声纳和IMU）的局部规划层，比较了解耦与耦合架构，并评估了不同控制器的性能特点。


<details>
  <summary>Details</summary>
Motivation: UUV在复杂、不确定的水下环境中面临导航挑战：传感器测量噪声大、缺乏GNSS信号、水下通信带宽低延迟高。需要先进的规划与控制策略来提高定位精度和自主性，实现动态避障和实时重规划。

Method: 综述采用分类分析方法，将现有文献分为解耦和耦合两种架构。解耦架构顺序处理规划和控制阶段，而耦合架构提供更紧密的反馈回路。重点分析基于实时传感器输入（SONAR、IMU）的局部反应式规划层。

Result: 比较分析显示：PID控制器简单但缺乏复杂机动的预测能力；MPC提供优越的路径优化但计算密集；不变集控制器提供强安全性保证但可能在受限环境中牺牲敏捷性。综述提出了规划与控制架构的分类法。

Conclusion: 该综述为UUV自主导航提供了传感器驱动规划与控制方法的系统性分析框架，强调自适应局部规划的重要性，并分析了不同控制器在集成规划框架中的作用，为未来研究提供了理论基础和方向指导。
...
TL;DR: StarVLA是一个开源的视觉-语言-动作（VLA）研究代码库，提供模块化架构、可复用训练策略和统一评估接口，旨在解决当前VLA方法碎片化问题，促进比较和复现。


<details>
  <summary>Details</summary>
Motivation: 当前VLA方法存在架构不兼容、代码库分散、评估协议不一致的问题，阻碍了方法的系统性比较和可复现性，需要统一的框架来整合这些能力。

Method: StarVLA采用模块化的骨干-动作头架构，支持VLM骨干（如Qwen-VL）和世界模型骨干（如Cosmos）以及多种动作解码范式；提供跨具身学习和多模态协同训练等可复用训练策略；集成多个主要基准测试，通过统一接口支持仿真和真实机器人部署。

Result: StarVLA提供了简单、完全可复现的单基准训练方案，在最小数据工程的情况下，已在多个基准测试中匹配或超越了先前方法，支持VLM和世界模型骨干。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于单目视觉的端到端学习方法存在样本效率低和仿真到真实差距大的问题，主要原因是高维RGB观测直接映射到动作命令，以及仿真与真实域之间的视觉差异。

Method: 采用真实-仿真-真实范式，将表示学习与策略优化解耦：1）使用3D高斯重建技术增强几何约束，实现高保真实到仿真转换；2）在仿真环境中通过对比学习提取紧凑、抗噪声的潜在特征；3）使用预训练编码器提供低维特征输入，降低策略计算负担并增强抗噪能力。

Result: 在仿真和真实环境中的大量实验表明，GaussFly相比基线方法具有更优的样本效率和渐进性能，能够实现零样本策略迁移到未见过的真实环境，有效弥合了仿真到真实的差距。

Conclusion: GaussFly通过解耦表示学习和策略优化，结合3D高斯重建和对比学习，成功解决了自主飞行器视觉运动策略学习中的样本效率低和仿真到真实差距大的长期挑战。
...
TL;DR: 本文提出了一个用于肌腱驱动可转向针芯的双线性模型预测控制框架，该针芯与标准近距离放射治疗针兼容，通过虚拟输入映射到物理可实现的插入速度和肌腱张力，实现了固定目标定位和移动目标轨迹跟踪。


<details>
  <summary>Details</summary>
Motivation: 可转向针能够通过弯曲轨迹避开敏感解剖结构，从而改善组织间近距离放射治疗。然而，现有的建模和控制方法主要针对定制针设计开发，不适用于与商用临床针兼容的针芯。

Method: 提出了一个双线性模型预测控制框架，采用几何双线性模型，包含三个虚拟输入（插入速度和两个弯曲率），这些输入被映射到物理可实现的输入（插入速度和相应的肌腱张力）。通过仿真和物理插入实验验证，使用基于图像的尖端跟踪。

Result: 开环模型验证的估计误差低于2毫米（插入针长度的3%），闭环固定目标跟踪误差低至1.45毫米（插入长度的1.7%）。但在某些弯曲方向上实验显示较大位置误差，达到8.3毫米（插入长度的7.8%）。

Conclusion: 结果证明了临床兼容可转向近距离放射治疗系统实现固定目标定位和移动目标轨迹跟踪的可行性，同时突出了未来在标定和传感方面需要改进的领域。
...
TL;DR: RoboPlayground：一个语言驱动的机器人操作评估框架，允许用户通过自然语言定义可执行的任务，解决了传统固定基准测试的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统机器人操作系统的评估主要依赖少数专家设计的固定基准测试，任务实例、约束和成功标准预定义且难以扩展。这种范式限制了评估的参与范围，且无法揭示策略如何响应用户定义的任务意图、约束和成功概念的变化。

Method: 提出RoboPlayground框架，允许用户在结构化物理域中使用自然语言编写可执行的机器人操作任务。自然语言指令被编译为可复现的任务规范，包含明确的资产定义、初始化分布和成功谓词。每个指令定义了一个结构化的相关任务族，在保持可执行性和可比性的同时，支持受控的语义和行为变化。

Result: 1. 用户研究表明，语言驱动界面比基于编程和代码辅助的基线更易用且认知负荷更低；2. 在语言定义的任务族上评估学习策略，揭示了固定基准测试中不明显的泛化失败；3. 任务多样性随贡献者多样性而非任务数量扩展，支持通过众包贡献持续增长评估空间。

Conclusion: RoboPlayground通过语言驱动的评估框架，使机器人操作系统的评估更加灵活、可扩展和包容，能够更好地揭示策略的泛化能力，并支持通过社区贡献持续丰富评估空间。
...
TL;DR: 该论文提出了一种紧凑、易制造的折叠式致动器，通过3D打印塑料条的复合折叠和拉链式连接实现可逆的尺寸和刚度变换，用于构建可适应不同任务和环境的形状变化机器人系统。


<details>
  <summary>Details</summary>
Motivation: 当前形状变化系统通常采用特定定制的机制，难以扩展、重新配置或跨应用移植。需要开发一种通用、可扩展的形状变化致动器，使机器人能够适应不断变化的任务和环境。

Method: 通过将柔性3D打印塑料条复合折叠并拉链式连接成方形截面的可展开梁，实现可逆的尺寸和刚度变换。该方法允许在紧凑（柔性）和展开（准刚性）状态之间平滑连续过渡，多个模块组合后可实现多样化的形状和刚度变换。

Result: 对致动器的机械性能进行了表征，并展示了一个包含四个模块的自适应行走机器人集成系统，验证了该致动器在实际机器人应用中的可行性。

Conclusion: 该折叠式致动器提供了一种紧凑、易制造且可扩展的解决方案，能够实现可逆的尺寸和刚度变换，为构建适应性强的形状变化机器人系统提供了新的可能性。
...
TL;DR: 该论文提出将语义嵌入应用于分子群体行为分析，通过DNA功能化增强控制，提取语义原子匹配预期行为，提高模拟实验的可解释性。


<details>
  <summary>Details</summary>
Motivation: 分子群体系统（如细胞骨架丝）的行为优化需要更丰富的特征集，当前方法在解释性和可靠性方面存在局限，需要更好的工具来设计和优化体外系统。

Method: 扩展标准微管模型，加入DNA功能化相互作用，应用语义嵌入技术学习分子群体行为范围，从模拟结果中提取语义原子，并分解模拟帧以分析外部控制值的影响。

Result: 提取的语义原子与预期行为匹配，模拟帧的分解准确描述了外部控制值的预期影响，为模拟实验提供了可解释性线索。

Conclusion: 语义嵌入方法提高了分子群体系统模拟实验的可解释性，使其在体外系统设计和优化中更加可靠，为分子群体行为分析提供了新工具。
...
TL;DR: 提出集成规划与控制框架，利用实时传感器数据动态引导AUV轨迹，在未知水下环境中实现鲁棒避障和增强机动性


<details>
  <summary>Details</summary>
Motivation: 水下自主航行面临能见度差、信号传输弱、水流动态变化等挑战，导致全局定位不准确、通信不可靠、避障困难。传感器噪声引入不确定性，使规划与控制复杂化

Method: 基于预设计反馈控制器的运动规划方法，利用实时传感器数据动态引导闭环AUV轨迹，减少在线优化计算复杂度，提高复杂水下空间操作安全性

Result: 通过ROS Gazebo仿真在RexRov AUV上验证方法有效性，与PID跟踪方法对比评估性能，量化进入目标通信范围时的航位推算定位误差

Conclusion: 提出的集成规划与控制框架能够有效应对水下环境挑战，实现鲁棒避障和增强机动性，为未知水下环境中的AUV导航提供可行解决方案
...
TL;DR: ExpressMM框架整合高层语言引导规划与底层视觉-语言-动作策略，为移动机械臂在协作任务中生成可表达意图的行为，支持用户中断和指令修改。


<details>
  <summary>Details</summary>
Motivation: 现有表达性机器人行为方法多采用预编程或模仿学习，未充分考虑协作任务中用户可能中断、修改或重定向机器人动作的人机交互场景。

Method: 开发ExpressMM框架，整合基于视觉语言模型的高层语言引导规划器（用于感知和对话推理）与底层视觉-语言-动作策略，生成表达性机器人行为，并支持可中断交互。

Result: 在移动机械臂协助人类完成协作装配场景的演示中，观众评估问卷显示：ExpressMM的表达性行为帮助观察者清晰理解机器人动作意图，支持社交适当且可理解的交互；参与者认为机器人对协作任务有用，行为可预测且安全。

Conclusion: ExpressMM框架通过整合高层规划与底层策略，成功实现了在协作人机交互中生成表达性机器人行为，支持可中断交互，提升了机器人有用性、安全性和可预测性的感知。
...
TL;DR: 提出统一框架，将效率与可靠性协同用于连续移动操作，通过可靠性感知轨迹规划和相位依赖切换控制器，在不确定环境中高效可靠地完成连续任务。


<details>
  <summary>Details</summary>
Motivation: 人类能够无缝融合预期规划和即时反馈，在连续移动操作中实现高效可靠。但机器人复制这种行为面临根本挑战：长时域规划与实时反应性冲突，且过度追求效率会损害不确定环境中的可靠性，影响稳定感知和补偿能力。

Method: 提出统一框架，包含：1) 可靠性感知轨迹规划器，将可靠执行要素嵌入时空优化，生成高效且可靠性有保障的全局轨迹；2) 相位依赖切换控制器，在全局轨迹跟踪（效率）和任务误差补偿（可靠性）间无缝切换；3) 分层初始化方法，促进复杂长时域规划问题的在线重规划。

Result: 真实世界评估表明，该方法能在不确定环境（动态扰动、感知和控制误差）下高效可靠地完成连续任务，且能泛化到具有不同末端执行器约束的任务。相比最先进基线方法，始终实现最高效率，同时将任务成功率提高26.67%--81.67%。消融研究验证了各组件贡献。

Conclusion: 提出的统一框架成功协同了效率与可靠性，使机器人能够在不确定环境中像人类一样高效可靠地执行连续移动操作任务，显著提升了任务成功率和效率表现。
...
TL;DR: CoEnv框架通过组合式环境（真实+仿真）解决多智能体协作中的空间协调、时间推理和共享工作空间感知问题，实现安全策略探索和可靠真实部署


<details>
  <summary>Details</summary>
Motivation: 多智能体具身系统在复杂协作操作中面临空间协调、时间推理和共享工作空间感知等关键挑战，需要一种能够分离认知规划和物理执行的协作范式

Method: 提出组合式环境概念，整合真实世界和仿真组件；CoEnv框架包含三个阶段：真实到仿真场景重建、VLM驱动的动作合成（支持实时规划和迭代规划）、以及经过验证的仿真到真实转移

Result: 在具有挑战性的多臂操作基准测试中，CoEnv实现了高任务成功率和执行效率，为多智能体具身AI建立了新范式

Conclusion: CoEnv通过组合式环境框架有效解决了多智能体协作中的关键挑战，实现了安全策略探索和可靠真实部署，为多智能体具身AI提供了新的研究范式
...
TL;DR: 本文提出JailWAM框架，首次针对世界动作模型(WAM)的越狱攻击漏洞进行系统研究，建立了三层次安全分类框架和评估基准，在仿真环境中实现了84.2%的攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 世界动作模型(WAM)具有强大的物理交互能力，但如果忽视安全性，将直接威胁人身、财产和环境安全。现有研究对WAM的越狱攻击漏洞关注极为有限，存在关键的安全缺口。

Method: 提出JailWAM框架，包含三个核心组件：1)视觉轨迹映射，统一异构动作空间；2)风险判别器，作为高召回率筛选工具；3)双路径验证策略，先快速粗筛再通过闭环物理仿真全面验证。同时构建JailWAM-Bench评估基准。

Result: 在RoboTwin仿真环境中，该框架能高效暴露物理漏洞，在最先进的LingBot-VA模型上实现了84.2%的攻击成功率。基于JailWAM可以构建鲁棒的防御机制。

Conclusion: JailWAM框架填补了WAM安全研究的关键空白，为设计安全可靠的机器人控制系统提供了有效的技术解决方案，能够系统评估和防御越狱攻击。
...
TL;DR: 该论文提出了一种模块化空中机器人系统（MARS）的新型控制框架，通过机械设计改进和虚拟四旋翼抽象，解决了现有控制算法在对接、分离和跟踪时的振荡问题，实现了敏捷飞行和稳定运输。


<details>
  <summary>Details</summary>
Motivation: 现有模块化空中机器人系统的控制算法依赖简化的准静态模型和基于规则的分配，导致不连续和无界的电机指令，随着无人机单元数量增加会产生姿态误差累积，在对接、分离和航点跟踪时引起严重振荡。

Method: 1. 设计紧凑机械系统，实现被动对接、无检测被动锁定和磁辅助分离；2. 引入力-力矩等效和多面体约束的虚拟四旋翼模型，显式建模可行力矩集；3. 基于此抽象设计两阶段预测分配管道：约束预测跟踪器计算虚拟输入，动态分配器映射到各模块产生平滑电机指令；4. 优化最大化控制权限的偏航角。

Result: 在10多种配置的仿真和真实实验中，实现了稳定对接、锁定和分离，以及有效的控制性能。首次在真实世界中演示了MARS实现敏捷飞行和运输，峰值俯仰角达40度，平均位置误差为0.0896米。

Conclusion: 该研究提出的机械设计和控制框架成功解决了模块化空中机器人系统的控制挑战，实现了稳定对接分离和敏捷飞行，为MARS的实际应用提供了可行的解决方案。
...
TL;DR: 本文开发了一种基于物理的软锥形机器人手仿真模型，结合进化策略框架自动优化舀取轨迹，解决了软体机器人控制复杂性的问题。
...
Method: 提出基于物理的仿真方法，准确建模软工具从平面到自适应锥形结构的变形行为，结合进化策略框架自动优化舀取轨迹，无需手动参数调整。

Result: 通过仿真和真实机器人实验验证优化轨迹，结果显示出强大的泛化能力，成功解决了现有方法难以应对的一系列挑战性任务。

Conclusion: 该方法有效解决了软体机器人舀取任务中的控制复杂性挑战，为软体机器人的实际应用提供了可行的解决方案。
...
TL;DR: ReV是一个视觉运动策略框架，通过耦合扩散头和轨迹引导策略，在仅使用专家演示训练的情况下，能够实时适应意外情况并重新规划轨迹。


<details>
  <summary>Details</summary>
Motivation: 解决机器人操作中视觉运动策略学习的根本问题：如何在仅依赖原始专家演示训练的情况下，增强对分布外执行错误或动态重规划轨迹的鲁棒性。

Method: 提出Referring-Aware Visuomotor Policy (ReV)闭环框架，使用耦合扩散头：全局扩散头生成全局一致但时间稀疏的动作锚点，并确定参考点的时间位置；局部扩散头基于当前时间位置自适应插值相邻锚点。通过向专家演示应用针对性扰动进行训练。

Result: 无需额外数据或微调方案，ReV在具有挑战性的模拟和现实世界任务中实现了更高的成功率。

Conclusion: ReV框架能够实时适应意外情况并重新规划轨迹，仅通过专家演示训练就能在动态变化场景中实现鲁棒操作。
...
TL;DR: 提出DAERT框架，通过多样性感知的强化学习红队测试，暴露视觉-语言-动作模型对语言变化的脆弱性，显著降低任务成功率。


<details>
  <summary>Details</summary>
Motivation: VLA模型在机器人操作中表现出色，但对语言细微变化的鲁棒性不足，这是实际部署中的关键安全隐患。现有的基于RL的红队测试方法存在模式崩溃问题，只能发现有限的失败模式，无法全面揭示风险。

Method: 提出DAERT框架，通过评估均匀策略来生成多样化的挑战性指令，同时确保攻击有效性（在物理模拟器中测量执行失败率）。该方法平衡了指令多样性和攻击效果。

Result: 在多个机器人基准测试中，针对π₀和OpenVLA等最先进的VLA模型进行实验。DAERT方法发现更广泛、更有效的对抗指令，将平均任务成功率从93.33%降至5.85%。

Conclusion: DAERT提供了一个可扩展的方法来压力测试VLA智能体，在实际部署前暴露关键的安全盲点，解决了传统RL红队测试中的模式崩溃问题。
...
TL;DR: 提出一种4自由度柔性腹腔镜器械的控制架构，集成远端弯曲、独立远端头部旋转、轴旋转和夹持器，直径10mm兼容标准套管，通过Raspberry Pi 5和Motoron控制器实现驱动单元与SpaceMouse遥操作，验证了剪刀连杆模型精度并与ATHENA并联机器人集成进行胰腺手术模拟验证。


<details>
  <summary>Details</summary>
Motivation: 微创手术（MIS）能减少患者创伤并缩短恢复时间，但传统腹腔镜器械受限于运动范围有限，需要开发更灵活的器械来改善手术操作性能。

Method: 开发4自由度柔性腹腔镜器械控制架构，包括远端弯曲、独立远端头部旋转、轴旋转和夹持器功能；采用Raspberry Pi 5和Motoron控制器实现驱动单元；集成SpaceMouse进行遥操作；建立剪刀连杆分析模型并进行参数化；通过CAD测量和OptiTrack运动捕捉验证模型精度；与ATHENA并联机器人集成进行模拟胰腺手术程序验证。

Result: 器械直径10mm兼容标准套管；剪刀连杆模型预测的钳口开度与CAD测量结果吻合（MAE 0.13°），与OptiTrack运动捕捉结果吻合（MAE 1.43°）；成功与ATHENA并联机器人集成并通过模拟胰腺手术程序验证。

Conclusion: 成功开发并验证了4自由度柔性腹腔镜器械的控制架构，该器械在保持标准套管兼容性的同时提供了更大的运动灵活性，为改善微创手术操作性能提供了有效解决方案。
...
TL;DR: 提出一个新颖的训练框架，通过对比模型显式地对齐视觉-语言-动作层次模型中的语言描述与动作轨迹，实现机器人透明度


<details>
  <summary>Details</summary>
Motivation: 现有层次化视觉-语言-动作模型虽然能生成语言和低级动作，但在训练过程中没有考虑这些模态之间的显式对齐，这阻碍了机器人透明度的实现

Method: 提出一个训练框架，使用对比模型评估生成语言与相应动作轨迹之间的对齐程度，通过离线偏好学习来细化层次化视觉-语言-动作模型的接地

Result: 在LanguageTable数据集上应用该框架，建立了强大的基线，性能与完全监督微调相当，同时最小化了昂贵数据标注的需求

Conclusion: 该框架通过显式地对齐语言和动作轨迹，为机器人透明度提供了关键解决方案，同时为多模态接地表示提供了重要见解
...
TL;DR: 提出了一种通过修改拉普拉斯矩阵权重来实现平面仿射编队集体机动的方法，无需领导者即可实现更广泛的集体运动


<details>
  <summary>Details</summary>
Motivation: 传统领导者-跟随者策略限制了机器人集群的集体运动能力，需要一种无领导者方案来实现更广泛的仿射变换运动

Method: 通过修改拉普拉斯矩阵的原始权重，使用复数表示二维信息，设计特征向量和特征值来定义集体运动

Result: 实现了包括平移、旋转、缩放和剪切在内的所有线性组合的集体运动，通过20个智能体的仿真验证了理论结果

Conclusion: 提出的无领导者方案能够实现更广泛的仿射编队集体机动，为机器人集群的协同控制提供了新方法
...
TL;DR: A1是一个开源VLA框架，通过自适应推理方案和层间截断流匹配技术，在保持高成功率的同时显著降低推理成本，实现低成本高性能的机器人操作。


<details>
  <summary>Details</summary>
Motivation: 当前VLA模型在实际部署中存在成本问题：十亿级参数的VLM主干和迭代扩散/流式动作头导致高延迟和高计算成本，使得在普通硬件上难以实现实时控制。

Method: 1. 利用预训练VLM提供隐式动作先验；2. 引入预算感知的自适应推理方案，通过监控中间层动作一致性实现早期终止；3. 提出层间截断流匹配技术，跨层预热去噪过程，减少有效去噪迭代次数。

Result: 在仿真基准（LIBERO、VLABench）和真实机器人（Franka、AgiBot）上达到最先进成功率，同时显著降低推理成本：流匹配推理延迟降低72%，主干计算减少76.6%且性能损失小；在RoboChallenge上平均成功率29.00%，优于多个基线模型。

Conclusion: A1框架通过优化整个推理管道，在保持高性能的同时大幅降低计算成本，为低成本、高吞吐量的机器人操作提供了可行的开源解决方案。
...
TL;DR: RSBM框架通过共享速度场结构和调节熵正则化参数，在3步积分内实现高效视觉导航，平衡多模态覆盖和路径平直性


<details>
  <summary>Details</summary>
Motivation: 基于扩散模型和薛定谔桥的生成策略需要大量积分步骤，无法满足实时机器人控制需求，需要开发更高效的导航策略

Method: 提出Rectified Schrödinger Bridge Matching (RSBM)，利用标准薛定谔桥和确定性最优传输之间的共享速度场结构，通过熵正则化参数ε控制平衡，使用学习到的条件先验缩短传输距离

Result: 标准方法需要≥10步收敛，RSBM仅需3步积分就达到94%余弦相似度和92%成功率，无需蒸馏或多阶段训练

Conclusion: RSBM显著缩小了高保真生成策略与具身AI低延迟需求之间的差距，为实时视觉导航提供了高效解决方案
...
TL;DR: 本文研究六旋翼无人机动态控制分配，采用双倾斜旋翼设计，提出分层控制结构，考虑执行器饱和效应，并通过数值仿真验证


<details>
  <summary>Details</summary>
Motivation: 研究六旋翼无人机在轨迹跟踪任务中的动态控制分配问题，特别关注双倾斜旋翼平台的设计，旨在解决执行器饱和效应和系统冗余优化问题

Method: 采用分层控制结构：高层控制器生成跟踪任务所需力矩，控制分配律确保执行器产生相应力矩；分配器为执行器集施加一阶期望动力学，利用系统冗余优化目标函数；明确建模执行器饱和并分析其对控制性能的影响

Result: 通过数值仿真验证了控制分配策略的有效性，分析了执行器饱和对控制性能的理论影响，并研究了通过目标函数中施加不对称形状来优化螺旋桨倾斜角的作用

Conclusion: 提出的动态控制分配方法能有效处理六旋翼无人机的轨迹跟踪任务，特别考虑了执行器饱和效应和系统冗余优化，为双倾斜旋翼平台的控制提供了理论框架和实用方案
...
TL;DR: 该研究提出了一种考虑物体局部机械特性的机器人抓取规划与执行方法，通过构建表面可承受接触力地图，在几何有效抓取的基础上筛选机械安全的接触区域，并基于局部可承受力调节手指刚度，实现安全可靠的抓取操作。


<details>
  <summary>Details</summary>
Motivation: 现有抓取规划器通常将物体表面视为结构均匀的，忽略了不同区域机械特性的差异。在薄弱区域接触可能导致物体损坏，即使几何上是完美抓取。因此需要开发能够考虑物体局部机械特性的抓取规划方法。

Method: 1. 识别目标物体并使用SAM3D重建3D几何模型；2. 通过物理信息几何分析计算表面各位置可承受的最大侧向接触力地图；3. 基于几何有效性和任务目标一致性筛选抓取候选；4. 使用力地图感知标准对候选抓取重新排序，优先选择接触点在机械可承受区域的抓取；5. 基于接触点局部可承受力调节各手指的阻抗控制器刚度。

Result: 在纸杯、塑料杯和玻璃杯上的验证表明，该方法能够一致地选择结构更强的接触区域，并将抓握力保持在安全范围内。相比传统仅考虑几何的方法，显著提高了抓取的安全性。

Conclusion: 该工作将灵巧操作从纯几何问题重新定义为物理基础的联合规划问题，结合抓取选择和抓握执行，为未来人形系统提供了更安全可靠的抓取解决方案。
...
TL;DR: 该论文提出了一种针对MammoBot辅助机器人系统的危险管理方法，结合SHARD和STPA分析技术，识别临床环境中人机交互的安全风险，并将结果转化为安全需求。


<details>
  <summary>Details</summary>
Motivation: 临床环境中机器人系统与脆弱患者近距离接触存在显著安全风险，需要从早期开发阶段确保安全性，以改善医疗护理的可及性和质量。

Method: 采用利益相关者引导的过程建模，结合软件危险分析与设计解决方案(SHARD)和系统理论过程分析(STPA)。与临床医生、机器人专家和患者代表协作定义机器人辅助工作流程，识别技术和程序偏差，分析用户交互中的不安全控制行为。

Result: 分析显示许多危险并非来自组件故障，而是源于时间不匹配、过早行动和系统状态误解。这些危险被转化为细化和额外的安全需求，约束系统行为并减少对正确人类计时或解释的依赖。

Conclusion: 该工作展示了一种结构化、可追溯的安全驱动设计方法，对临床环境中的辅助机器人系统具有潜在适用性。
...
TL;DR: 使用强化学习训练端到端传感器-电机策略，让四旋翼无人机仅凭机载视觉和本体感知就能完成高难度狭窄缝隙穿越，包括5厘米间隙、90度倾斜等极端情况，无需预先知道缝隙位置和方向。


<details>
  <summary>Details</summary>
Motivation: 无人机在狭窄环境中执行精确的激进机动仍然是一个关键瓶颈，特别是需要利用倾斜姿态和机身不对称性穿越狭窄缝隙的SE(3)约束问题。现有方法通常依赖精确的环境模型和规划，限制了系统的自主性和适应性。

Method: 开发传感器-电机策略，直接将机载视觉和本体感知映射到低级控制命令。使用强化学习在仿真中进行端到端策略蒸馏，通过基于模型规划器生成的轨迹进行初始化来缓解探索困难。采用精心设计的仿真到真实转换方法。

Result: 该方法使四旋翼无人机能够在5厘米间隙、倾斜达90度的情况下穿越矩形缝隙，无需知道缝隙位置或方向。即使未经动态缝隙训练，策略也能反应式地穿越移动缝隙。在密集排列的狭窄缝隙轨道上验证了方法的有效性。

Conclusion: 提出的策略学习方法具有灵活性，能够为几何多样的缝隙开发策略，无需依赖手动定义的穿越姿态和视觉特征，显著提升了无人机在受限环境中的自主机动能力。
...
TL;DR: BiCoord是一个用于长时程、紧密协调的双臂操作基准测试，包含需要连续臂间依赖和动态角色交换的多样化任务，并提出了从时空角度评估协调性的量化指标。


<details>
  <summary>Details</summary>
Motivation: 现有的双臂操作基准测试（如RoboTwin和RLBench2）主要关注短时程和松散协调的任务，无法捕捉真实世界中双臂行为的时空耦合特性，需要一个新的基准来研究长时程紧密协调的双臂操作。

Method: 提出了BiCoord基准测试，包含多样化的长时程任务，这些任务需要连续的双臂间依赖和动态角色交换；同时开发了一套从时间、空间和时空三个维度评估协调性的量化指标。

Result: 实验结果显示，代表性的操作策略（如DP、RDT、Pi0和OpenVLA-OFT）在长时程和高度耦合的任务上表现不佳，揭示了实现长时程紧密协调任务的基本挑战。

Conclusion: BiCoord基准测试为研究长时程协作操作提供了基础，能够激发未来关于协调感知机器人学习的研究，所有数据集、代码和补充材料均已公开。
...
<details>
  <summary>Details</summary>
Motivation: 随着机器人在共享安全关键环境中运行，仅仅安全行动已不足够，还需要让人类合作者理解其安全决策。当前人机协作中的安全约束触发行为（如停止或切换模式）对附近工作人员来说是不透明的。

Method: 开发了一个基于对话的交互式安全决策解释框架，将解释与基于约束的安全评估紧密结合，使用相同的状态和约束表示来支撑行为选择和对话。解释直接从记录的决策轨迹中推导，支持因果、对比和反事实查询。反事实推理在固定的认证安全参数下有界评估，确保交互探索不会放松操作保证。

Result: 在建筑机器人场景中实例化了该框架，提供了结构化的操作轨迹，展示了约束感知对话如何澄清安全干预并支持协调的任务恢复。

Conclusion: 通过将解释视为安全控制的操作接口，这项工作为人机协作中交互式、安全感知的自主性设计提供了新的视角。
...
TL;DR: 研究探索在训练阶段使用仪器化（物体传感器化）来改善接触丰富操作策略的性能，而不增加部署时的依赖。以按钮按压为测试平台，使用指尖麦克风捕捉接触相关音频，通过仪器化的按钮状态信号作为特权监督来微调音频编码器成为接触事件检测器，最终策略仅使用视觉和音频进行推理。


<details>
  <summary>Details</summary>
Motivation: 仅从摄像头和本体感觉学习接触丰富的操作很困难，因为接触事件只能被部分观察到。研究旨在测试训练阶段的仪器化是否能提高策略性能，同时避免在部署时产生依赖。

Method: 以按钮按压为测试平台，使用指尖麦克风捕捉接触相关音频。利用仪器化的按钮状态信号作为特权监督，微调音频编码器成为接触事件检测器。结合三种策略将生成的表示与模仿学习相结合，确保策略在推理时仅使用视觉和音频。

Result: 按钮按压成功率在不同方法间相似，但仪器化引导的音频表示能持续减少接触力。这表明仪器化作为训练时的辅助目标对学习接触丰富的操作策略具有实际价值。

Conclusion: 仪器化可以作为训练时的实用辅助目标，用于学习接触丰富的操作策略，提高性能而不增加部署时的依赖。
...
TL;DR: 提出基于自然语言和手势的机器人编程框架，利用大语言模型和计算机视觉技术，通过系统反馈确保安全性和用户接受度


<details>
  <summary>Details</summary>
Motivation: 受人类自然交互启发，旨在为终端用户提供直观的机器人编程方式，解决传统编程复杂性问题，提升用户体验和接受度

Method: 结合自然语言和手势输入，使用大语言模型进行语言理解，计算机视觉识别手势，通过澄清问题和视觉反馈让用户审查调整生成的程序

Result: 开发了一个能够将自然交互转化为机器人程序的系统，通过反馈机制确保程序的安全性和透明度

Conclusion: 该框架为终端用户提供了直观的机器人编程方法，通过自然交互和反馈机制实现了安全、透明且用户友好的编程体验
...
TL;DR: 提出了一种轻量级控制框架，用于星形相互依赖倾转六旋翼无人机执行交互任务，通过离线计算零力矩力多面体查找表优化倾角选择，在计算时间、姿态跟踪性能和驱动效率方面均有改进。


<details>
  <summary>Details</summary>
Motivation: 从机动性角度看，倾转多旋翼无人机的主要优势在于可执行力矩的动态可变性，这对于物理交互任务至关重要。倾角选择需要优化以确保高性能，同时避免突变并保持实际可行性。

Method: 提出轻量级控制框架，使用离线计算的零力矩力多面体查找表来识别期望控制力的可行倾角，并通过平衡效率和平滑性选择最优倾角。该框架与几何全姿态控制器集成。

Result: 蒙特卡洛仿真显示计算时间显著减少，姿态跟踪性能改善，驱动效率具有竞争力。在Simscape中进行的完整墙面检查任务物理仿真进一步证实了该方法在交互场景中的可行性。

Conclusion: 所提出的控制框架为倾转六旋翼无人机在交互任务中提供了有效的倾角优化策略，在计算效率、跟踪性能和实际可行性方面均表现出色。
...
TL;DR: 本文提出了一种用于平面五杆闭链单足机器人的协同设计框架，联合优化机械设计、电机与变速箱参数以及控制参数，以实现动态跳跃性能最大化。


<details>
  <summary>Details</summary>
Motivation: 现有协同设计研究大多关注连杆尺寸和传动比优化，忽略了详细的执行器设计（特别是电机和变速箱参数优化），且主要局限于串联开链机构。本文旨在填补这一空白。
...
Conclusion: 所提出的协同设计框架能够有效优化闭链机构的机械设计、执行器参数和控制策略，显著提升跳跃性能并降低能耗，为高性能腿式机器人设计提供了新方法。
...
TL;DR: HiPolicy：一种分层多频率动作分块框架，通过联合预测不同频率的动作序列来平衡长期依赖建模与精细闭环控制，解决机器人模仿学习中的基本权衡问题。


<details>
  <summary>Details</summary>
Motivation: 机器人模仿学习面临一个基本权衡：建模长期依赖与实现精细闭环控制之间的矛盾。现有的固定频率动作分块方法难以同时实现这两个目标。

Method: 提出HiPolicy分层多频率动作分块框架：1）联合预测不同频率的动作序列以捕捉粗粒度高级规划和精确反应动作；2）从历史观测中提取并融合与每个频率对齐的分层特征；3）引入基于熵的引导执行机制，根据动作不确定性自适应平衡长期规划与精细控制。

Result: 在多样化模拟基准和真实世界操作任务上的实验表明，HiPolicy可以无缝集成到现有的2D和3D生成策略中，在显著提升执行效率的同时带来一致的性能改进。

Conclusion: HiPolicy通过分层多频率动作分块有效解决了机器人模仿学习中长期依赖建模与精细控制之间的权衡问题，为现有策略提供了可扩展的改进方案。
...
TL;DR: 提出基于手势的用户界面，通过指向和点击手势实现物体选择，实验验证了准确性和选择时间


<details>
  <summary>Details</summary>
Motivation: 手势是人类自然的交流方式，可以用于人机交互，特别是人机协作中的物体选择任务

Method: 开发基于手势的用户界面，使用指向和点击手势进行物体选择，通过实验评估系统性能

Result: 20名参与者的实验结果显示，该系统在准确性和选择时间方面表现良好，证明了高效协作的潜力

Conclusion: 基于手势的物体选择界面具有实用价值，能够支持高效的人机协作交互
...
TL;DR: 将力反馈MPC与扩散运动先验结合，解决工业去毛刺任务中的接触力控制、避障和运动规划问题


<details>
  <summary>Details</summary>
Motivation: 传统MPC在扭矩控制机器人中常忽略实时力反馈，难以处理碰撞约束下的接触密集型工业任务。去毛刺任务需要精确工具插入、稳定力调节和无碰撞圆周运动，超出了标准MPC的能力范围。

Method: 提出一个集成力反馈MPC与扩散运动先验的框架。扩散模型作为运动策略的记忆库，提供跨多个任务实例的鲁棒初始化和适应，而MPC确保具有显式力跟踪、扭矩可行性和碰撞避免的安全执行。

Result: 在扭矩控制机械臂上进行工业去毛刺任务验证。实验展示了可靠的工具插入、精确的法向力跟踪，即使在难以到达的配置和障碍约束下也能实现圆周去毛刺运动。

Conclusion: 这是首次将扩散运动先验与力反馈MPC集成，用于碰撞感知、接触密集型的工业任务，为解决复杂工业操作提供了新方法。
...
### [37] [Delta6: A Low-Cost, 6-DOF Force-Sensing Flexible End-Effector](https://arxiv.org/abs/2604.06150)
*Yue Feng,Weicheng Huang,Chen Qiu,Huixu Dong,I-Ming Chen*

Main category: cs.RO

TL;DR: Delta6是一款低成本六自由度力/力矩末端执行器，采用拮抗弹簧和磁编码器设计，无需校准即可实现7%满量程误差，经序列模型优化后降至3.8%，适用于接触密集型任务。


<details>
  <summary>Details</summary>
Motivation: 现有六自由度力传感解决方案成本高且复杂，需要开发一种低成本、易组装、高精度的替代方案，使机器人能够执行接触密集型任务。

Method: 结合拮抗弹簧和磁编码器设计，采用全3D打印原型和现成零件组装，通过参数化分析模型扩展性能极限，使用轻量级序列模型进行误差补偿。

Result: 原型机承受±14.4N力和±0.33N.m力矩，无校准时99%百分位误差为7%满量程，经序列模型优化后降至3.8%，在多种计算平台上验证了带宽可调性，成功完成曲面抛光和紧密装配任务。

Conclusion: Delta6是一种稳健、低成本的六维力传感替代方案，通过简单设计和开源实现，为机器人接触任务提供了实用解决方案。
