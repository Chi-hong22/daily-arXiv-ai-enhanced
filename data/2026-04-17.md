...
TL;DR: CART是一个基于上下文感知地形适应的高层控制器，通过整合本体感知和外感知实现稳健的地形理解，在仿真和真实世界中均优于现有多模态感知基线方法。


<details>
  <summary>Details</summary>
Motivation: 动物通过结合视觉和触觉等多种模态来感知地形并稳定行走。当前腿部机器人的地形适应方法主要依赖先验经验（特别是视觉传感器观测），在复杂越野地形上容易失败，存在"视觉-纹理悖论"问题。

Method: 提出CART高层控制器，采用上下文感知地形适应方法，整合来自机载传感器的本体感知和外感知，实现对地形的稳健理解。使用机器人基座的振动稳定性作为评估学习到的上下文地形特性的指标。

Result: 在IsaacSim仿真器中使用ANYmal-C机器人，在真实世界中使用Boston Dynamics SPOT机器人进行实验。CART在仿真中平均成功率比所有基线提高5%，在真实世界中总体稳定性分别提高45%和24%，且不增加机器人完成运动任务的时间。

Conclusion: CART通过整合多模态感知实现上下文感知地形适应，在复杂地形上显著提高了腿部机器人的稳定性和成功率，解决了传统经验学习方法中的视觉-纹理悖论问题。
...
TL;DR: 本文提出了一种基于交替优化的鲁棒同步定位与校准方法，用于解决在GPS信号弱或无信号环境中自主移动机器人的定位问题，通过同时估计平台姿态和磁力计校准参数来提高定位精度。


<details>
  <summary>Details</summary>
Motivation: 自主移动机器人在室内、地下等GPS信号弱或无信号环境中的定位仍然是一个主要障碍。基础设施依赖方法成本高且灵活性有限，而基于车载传感器的方法受几何特征模糊和动态障碍物遮挡影响。基于环境磁场的定位方法虽然不依赖外部基础设施，但磁力计测量常受传感器平台上铁磁材料引起的失真影响，需要准确的校准。

Method: 提出了一种基于交替优化的鲁棒同步定位与校准方法，迭代且高效地同时估计平台姿态和磁力计校准参数。该方法避免了传统依赖旋转磁力计的校准方法对大型重型平台的不实用性。

Result: 在高保真仿真和真实环境中的广泛评估表明，所提出的RoSLAC方法实现了高定位精度，同时与最先进的磁力计校准技术相比保持了较低的计算成本。

Conclusion: RoSLAC方法为自主移动机器人在GPS信号弱或无信号环境中的定位提供了一种有效的解决方案，通过同时进行定位和校准，克服了传统校准方法的局限性，提高了定位可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 移动机器人在挑战性环境中运行时，常常缺乏足够的几何信息来约束点云配准，导致LiDAR-惯性里程计精度下降甚至发散。需要一种能够利用环境中细微几何变化来提高鲁棒性的方法。

Method: 1. 提出高分辨率地图表示，将表面存储为紧凑的体素级定向高度图像，可直接用于配准而无需计算中间几何基元；2. 提出地图信息点采样策略，专注于几何信息丰富的区域，在信息稀疏环境中提高鲁棒性并降低计算成本。

Result: 在多种传感器、平台和环境下的实验表明，该方法在约束良好的场景中达到最先进性能，在基线方法发散的挑战性场景中表现显著改善。此外，BIEVR-LIO捕获的细粒度几何可用于下游任务，如机器人运动的高程映射。

Conclusion: BIEVR-LIO通过创新的地图表示和采样策略，有效解决了LiDAR-惯性里程计在几何信息稀疏环境中的鲁棒性问题，为移动机器人在挑战性环境中的可靠定位提供了有效解决方案。
...
TL;DR: CooperDrive是一个协同感知框架，通过在车辆间共享物体级信息来增强遮挡和非视距场景下的感知能力，提高驾驶安全性和反应时间


<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆在遮挡和非视距场景下感知受限，延迟反应会增加碰撞风险，需要增强情境感知能力

Method: 利用检测器的鸟瞰图特征估计车辆姿态，重建BEV表示，通过轻量级物体级共享和融合策略连接感知与规划，扩展物体集以提前预测潜在冲突

Result: 在遮挡严重的非视距交叉路口测试中，CooperDrive提高了反应提前时间、最小碰撞时间和停车余量，仅需90kbps带宽，平均端到端延迟89毫秒

Conclusion: CooperDrive通过协同感知将反应性行为转变为预测性安全驾驶决策，有效解决了遮挡和非视距场景下的自动驾驶挑战
...
TL;DR: 提出了一种基于样条的连续时间状态估计方法，用于视觉-惯性-测距融合定位，通过虚拟锚点构建和异常值剔除提高定位精度和可靠性。


<details>
  <summary>Details</summary>
Motivation: 视觉惯性里程计在长期定位中精度会下降，需要全局约束。超宽带测距可以缓解漂移，但需要良好部署的锚点，这在狭窄或低功耗环境中难以保证。现有方法多为离散时间滤波或优化，难以在异步多传感器采样下平衡定位精度、轨迹一致性和融合效率。

Method: 1. 预处理阶段：使用VIO运动先验和UWB测距测量构建虚拟锚点并剔除异常值，缓解几何退化并提高测距可靠性。2. 估计阶段：使用B样条在连续时间内参数化位姿轨迹，将惯性、视觉和测距约束表示为滑动窗口图中的因子。3. 联合优化样条控制点和少量辅助参数，获得连续时间轨迹估计。

Result: 在公共数据集和真实世界实验中的评估证明了该方法的有效性和实际潜力。

Conclusion: 提出的基于样条的连续时间状态估计方法能够有效解决视觉-惯性-测距融合定位中的问题，在异步多传感器采样下实现了定位精度、轨迹一致性和融合效率的良好平衡。
...
TL;DR: 利用基于模型的深度强化学习，研究被动身体特性对双足机器人行走和跑步运动生成的影响，发现具有被动元件的机器人能通过稳定极限环获得更鲁棒、节能的运动。


<details>
  <summary>Details</summary>
Motivation: 探索具身智能中身体被动特性对运动学习的重要性，研究被动元件（如弹簧）如何影响双足机器人的行走和跑步运动生成。

Method: 在模拟器中构建两个模型：一个带有被动元件（如弹簧），另一个类似通用人形机器人没有被动元件。使用基于模型的深度强化学习方法进行训练，分析系统吸引子对学习过程的影响。

Result: 具有被动元件的模型训练受系统吸引子影响显著，轨迹快速收敛到极限环但需要较长时间获得高奖励。最终获得的运动更鲁棒、节能。被动元件机器人通过身体与地面的动态交互产生的稳定极限环，能高效获得高性能运动。

Conclusion: 身体被动特性的实现对未来具身AI至关重要，机器人通过利用被动元件产生的稳定极限环，能够获得更高效、鲁棒的运动能力。
...
TL;DR: 该研究提供了一个多平台LiDAR参考数据集，整合了无人机、地面和背包式激光扫描，用于森林结构测量和生态模型校准。


<details>
  <summary>Details</summary>
Motivation: 为支持3D结构数据的校准、基准测试和整合，特别是将激光扫描数据与生态观测和标准异速生长模型相结合，需要一个标准化的多平台LiDAR数据集。

Method: 整合了无人机激光扫描(ULS)测量冠层覆盖、地面激光扫描(TLS)详细测绘树干、以及背包移动激光扫描(MLS)实时SLAM进行高效亚冠层采集。采用无标记、SLAM感知协议减少野外和处理时间，结合手动和自动化方法。

Result: 创建了一个包含约3.33亿TLS点云的多平台LiDAR数据集，补充了ULS和MLS数据，覆盖冠层和下层植被。数据以LAZ和E57格式提供，带有UTM坐标和配准报告，确保可重复性。

Conclusion: 该数据集为测试配准方法、评估扫描效率以及将点云与分割、定量结构模型和生物量估算联系起来提供了基准。通过在长期ICOS站点采集数据，将3D结构与数十年的生态和通量测量联系起来，展示了如何结合TLS、MLS和ULS进行重复调查和森林生态系统数字孪生。
...
TL;DR: DigiForest是一个大规模精准林业系统，利用数字技术和自主机器人进行树木级数据采集、森林清单构建、决策支持以及低影响选择性采伐。


<details>
  <summary>Details</summary>
Motivation: 森林覆盖地球陆地面积三分之一，对全球生物多样性、气候调节和人类福祉至关重要。欧洲森林覆盖约40%土地面积，林业部门对实现欧盟气候中和与生物多样性目标具有核心作用。当前实践需要进一步创新来应对这些挑战。

Method: DigiForest包含四个主要组件：(1)自主异构移动机器人（空中、腿式和袋式）进行树木级数据采集；(2)自动提取树木特征构建森林清单；(3)决策支持系统预测森林生长并支持决策；(4)使用专用自主采伐机进行低影响选择性采伐。

Result: 这些技术已在芬兰、英国和瑞士等多个地点的真实森林环境中进行了广泛验证。

Conclusion: DigiForest为可持续森林管理、增加长寿命木制品使用和建设韧性森林生态系统提供了创新的数字解决方案，有助于实现欧盟的气候和生物多样性目标。
...
TL;DR: 提出World-Value-Action (WAV)模型，一种通过隐式规划解决VLA模型在长时域决策中局限性的统一框架，相比直接动作预测方法显著提升任务成功率。


<details>
  <summary>Details</summary>
Motivation: 现有Vision-Language-Action (VLA)模型大多依赖直接动作预测，缺乏对长时域轨迹的推理和后果评估能力，限制了在复杂决策任务中的性能。

Method: WAV模型学习基于视觉观察和语言指令的未来轨迹结构化潜在表示。包含学习的世界模型预测未来状态，轨迹价值函数评估长时域效用，动作生成通过潜在空间推理实现，逐步将概率质量集中在高价值和动态可行的轨迹上。

Result: WAV模型在广泛模拟和真实世界实验中一致优于最先进方法，在任务成功率、泛化能力和鲁棒性方面取得显著改进，特别是在长时域和组合场景中。

Conclusion: WAV模型通过隐式规划和潜在空间推理有效解决了VLA模型在长时域决策中的局限性，为构建更智能的具身智能体提供了有前景的框架。
...
<details>
  <summary>Details</summary>
Motivation: 当单次抓取-放置无法将物体从初始姿态转移到目标姿态同时保持抓取可行性时，需要重抓取规划。主要挑战在于推理中间姿态间的共享抓取连通性，而离散搜索方法在此问题上表现脆弱。

Method: 1) 使用能量基模型(EBM)建模物体姿态下的抓取可行性；2) 利用能量可加性构建测量姿态对连通性的连续能量景观；3) 基于梯度优化中间物体姿态；4) 引入自适应迭代深化策略自动确定最小中间步骤数。

Result: 实验表明：1) 提出的成本公式提供平滑且信息丰富的梯度，相比其他方案提高了规划鲁棒性；2) 能够泛化到未见过的抓取姿态和跨末端执行器转移(用吸盘约束训练的模型可指导平行夹爪抓取操作)；3) 多步规划结果进一步证明了自适应深化和最小步数搜索的有效性。

Conclusion: 提出的基于可微分姿态序列连通性度量的隐式多步重抓取规划框架，通过连续能量景观建模和梯度优化，有效解决了重抓取规划中的连通性推理问题，提高了规划鲁棒性和泛化能力。
...
TL;DR: CAL2M是一个无需校准的辅助眼大尺度定位与建图框架，通过非线性变换解决VGFM在千米级SLAM中的几何失真问题，消除尺度模糊并确保全局一致性重建。
...
Method: 1. 引入"辅助眼"利用恒定物理间距先验，无需时空预校准即可消除尺度模糊；2. 提出基于对极几何约束的本征参数和位姿校正模型，通过基础矩阵分解校正由不准确本征参数引起的旋转和平移误差；3. 提出基于锚点传播的全局一致建图策略，通过构建和融合轨迹上的锚点建立局部到全局的直接映射关系，应用非线性变换弹性对齐子地图。

Result: CAL2M是一个即插即用框架，兼容任意VGFM，能够有效消除几何错位，确保全局一致的重建。源代码将在GitHub上公开。

Conclusion: CAL2M通过引入辅助眼、对极几何校正和锚点传播策略，解决了VGFM在大尺度SLAM中的几何失真问题，实现了无需校准的千米级定位与建图，显著提升了重建的全局一致性。
...
TL;DR: 提出Switch分层多技能系统，实现人形机器人任意时刻无缝技能切换，解决现有方法在技能间灵活过渡的困难。


<details>
  <summary>Details</summary>
Motivation: 现有基于深度强化学习的全身控制方法在实现不同技能间的灵活过渡方面存在困难，存在安全隐患和实际限制。

Method: 采用三层架构：1)基于多技能运动数据构建技能图，建立基于运动学相似性的跨技能转换；2)通过深度强化学习训练全身跟踪策略；3)在线技能调度器驱动跟踪策略，实现鲁棒执行和平滑过渡。

Result: 实验表明Switch系统使人形机器人能够以高成功率执行敏捷的技能切换，同时保持强大的运动模仿性能。

Conclusion: 提出的分层多技能系统Switch有效解决了人形机器人技能间无缝过渡的挑战，实现了高效、稳定、实时的多样化运动技能执行。
...
TL;DR: 提出一种用于4D成像雷达扫描配准的改进方法，通过图基的成对一致性最大化（PCM）进行异常值剔除，结合雷达测量模型的不确定性感知评分函数，在特征贫乏的露天矿环境中显著提升配准精度。


<details>
  <summary>Details</summary>
Motivation: 在灰尘和低能见度环境中，4D成像雷达虽然适用，但由于扫描稀疏性、噪声和多径反射导致的虚假检测，扫描配准仍然具有挑战性。在特征贫乏的露天矿环境中，缺乏显著地标进一步降低了对应关系的可靠性。

Method: 将图基的成对一致性最大化（PCM）作为异常值剔除步骤集成到迭代最近点（ICP）循环中。提出雷达适应的成对距离不变评分函数，该函数结合了从雷达测量模型导出的各向异性、每个检测的不确定性。使用贪心启发式算法近似解决一致性最大化问题，在成对一致性图中寻找大团。

Result: 在露天矿数据集上评估，相比没有PCM的广义ICP（GICP）基线，该方法在1米段上减少相对位置误差（RPE）29.6%，在100米段上最多减少55%。

Conclusion: 所提出的方法适用于在线使用（由于图基PCM中的贪心启发式），旨在集成到定位管道中，显著提高了在特征贫乏环境中雷达扫描配准的鲁棒性。
...
TL;DR: 该论文提出在4D毫米波雷达的3D高斯建模和扫描匹配中融入雷达截面积(RCS)的物理特性，以丰富场景信息并改进扫描匹配精度


<details>
  <summary>Details</summary>
Motivation: 4D毫米波雷达在机器人应用中日益重要，但通常只使用XYZ位置和速度信息，而忽略了雷达截面积(RCS)信息。虽然多普勒速度常用于过滤动态点，但RCS信息往往被忽视，未在建模和扫描匹配过程中充分利用

Method: 基于先前的3D高斯建模和扫描匹配工作，提出将RCS的物理行为特性融入模型中，利用RCS信息来丰富场景的汇总信息并改进扫描匹配过程

Result: 论文未在摘要中提供具体实验结果，但预期通过融入RCS物理特性能够进一步丰富场景信息并改善扫描匹配性能

Conclusion: 通过将RCS的物理行为特性整合到4D毫米波雷达的3D高斯建模和扫描匹配中，可以更充分地利用雷达数据，提高场景理解能力和扫描匹配精度
...
TL;DR: 本文提出了一种集成废物管理框架，结合机器人废物分拣模块和优化生物消化器，实现智能自动化废物处理，分类准确率达98%，生物转化效率高。
...
Method: 采用集成废物管理框架，包含：1）机器人废物分拣系统（使用MyCobot 280 Jetson Nano机械臂、YOLOv8目标检测和ROS路径规划）；2）优化生物消化器系统（配备多传感器监测温度、pH、压力、电机转速等参数，使用粒子群优化算法结合回归模型动态调整参数）。

Result: 动态条件下测试显示：分拣准确率达98%，生物转化效率高，系统运行稳定，消化效率在各种环境条件下最大化。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集缺乏对人类和多种机器人手灵巧抓取的全面覆盖，需要高质量的多模态数据来促进物理交互研究和跨域灵巧操作学习。

Method: 利用先进的视觉方法和专门的多摄像头系统，采集人类和多种机器人手在100种不同物体上的抓取轨迹，同步记录高分辨率触觉信号、多视角视频和第一人称视角视频流。

Result: 构建了包含1.4K个抓取试验的数据集，涵盖成功和失败案例，提供高精度时空3D运动真值、视觉、运动学和触觉多模态数据。

Conclusion: HRDexDB作为基础性基准数据集，通过提供人类灵巧性和机器人执行在相同目标物体上的对齐捕获，支持多模态策略学习和跨域灵巧操作研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有视障辅助规划方法在多目标优化、泛化能力和可解释性方面存在不足，难以平衡舒适性、安全性等多重目标，需要更有效的解决方案。

Method: 提出MHHTOF框架，包含：1）启发式轨迹采样集群（HTSC）与动量约束轨迹优化（MTO）平衡舒适性和安全性；2）残差增强深度强化学习模块优化候选轨迹；3）双阶段成本建模机制（DCMM）在Frenet空间确保一致性，在Cartesian空间集成用户偏好。

Result: 实验表明，该框架收敛速度比基线快近一倍，获得更低更稳定的成本。在复杂动态场景中，MHHTOF展现出更稳定的速度和加速度曲线，风险降低，在鲁棒性、安全性和效率方面具有优势。

Conclusion: MHHTOF框架有效解决了视障辅助规划中的多目标优化挑战，通过动量约束和混合启发式方法实现了更好的舒适性、安全性和可解释性，为实际应用提供了可靠解决方案。
...
TL;DR: DEX-Mouse是一个便携、免校准的手持遥操作接口，具有集成动觉力反馈，成本低于150美元，支持机器人手直接安装在操作者前臂上，实现机器人对齐的数据采集。


<details>
  <summary>Details</summary>
Motivation: 当前数据驱动的灵巧手操作需要大规模、物理一致的演示数据。现有方法存在仿真到现实的差距、重定向问题、需要校准、缺乏便携性和跨平台兼容性等问题。

Method: 开发了DEX-Mouse系统，使用商用现成组件构建便携式免校准手持遥操作接口，集成动觉力反馈，支持机器人手直接安装在操作者前臂的配置。

Result: 在比较性用户研究中，使用该系统的操作者在附着配置下实现了86.67%的任务完成率，并且感知工作量比空间分离的遥操作设置显著降低。

Conclusion: DEX-Mouse提供了一个低成本、便携、免校准的遥操作解决方案，能够产生机器人对齐的数据，降低了操作者工作量，并通过开源硬件和软件促进复制和采用。
...
Method: 提出DockAnywhere框架：1) 解耦依赖于对接点的基座运动与跨视角不变的接触式操作技能；2) 在可行性约束下采样可行对接点；3) 通过结构保持增强生成对应轨迹；4) 将机器人和物体表示为点云，在3D空间中进行点级空间编辑合成视觉观察，确保跨视角的观察与动作一致性。

Result: 在ManiSkill和真实机器人平台上的大量实验表明，DockAnywhere显著提高了策略成功率，能够轻松泛化到训练中未见对接点的新视角，大幅增强了移动操作策略在真实世界部署中的泛化能力。

Conclusion: DockAnywhere通过低成本演示生成框架有效解决了移动操作中的视角泛化问题，为机器人在家庭和工厂等广阔环境中的交互提供了更强大的泛化能力。
...
TL;DR: CAVERS是一个在天然喀斯特洞穴中采集的多模态数据集，包含RGB-D-I、热红外和LiDAR数据，提供毫米级精度的地面真实位姿，用于评估SLAM和建图算法。


<details>
  <summary>Details</summary>
Motivation: 天然喀斯特洞穴环境（不规则几何、湿滑反射表面、无环境光、复杂分支通道）对机器人感知和导航提出独特挑战，但现有公开数据集稀缺且模态有限。

Method: 在西班牙马拉加的Cueva de la Victoria洞穴两个结构不同的房间中采集数据，使用Intel RealSense D435i RGB-D-I相机、Optris PI640i近红外热相机和Velodyne VLP-16 LiDAR，以手持和轮式机器人两种方式在完全黑暗和人工照明条件下采集。使用Optirack运动捕捉系统提供毫米级精度6自由度位姿和速度地面真实数据。

Result: 采集了24个序列约335GB数据，对7种最先进的SLAM和里程计算法（视觉、视觉-惯性、热-惯性、LiDAR）以及3D重建流程进行了基准测试，验证了数据集的可用性。

Conclusion: CAVERS数据集填补了天然喀斯特洞穴环境多模态数据集的空白，为机器人感知和导航研究提供了有价值的基准测试平台，数据集已公开可用。
...
<details>
  <summary>Details</summary>
Motivation: 解决多无人机刚性负载运输系统在密集障碍环境中的轨迹规划问题，特别是减少电缆引起的振荡和确保安全避障

Method: 第一阶段：开发增强型Tube-RRT*算法，集成主动混合采样和自适应扩展策略，生成安全可行的虚拟管道；第二阶段：构建凸二次规划，考虑负载平移和旋转动力学、电缆张力约束和碰撞安全约束

Result: 增强型Tube-RRT*相比STube-RRT*和AETube-RRT*具有更高的成功率和有效采样率，产生更短的最优路径和更小的累积转弯角度；凸二次规划生成平滑无碰撞的负载轨迹
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉SLAM系统在无人机竞速的极端条件下（高速飞行、剧烈机动、运动模糊）表现不佳，且未充分利用竞速环境的结构化特性，需要更鲁棒的实时定位方案。

Method: 采用双位姿图架构：临时图累积多个门框观测并在关键帧间优化为单个精炼约束，然后提升到持久主图中。这种设计保留了频繁检测的信息丰富性，同时防止图增长影响实时性能。系统设计为传感器无关，本文使用单目视觉惯性里程计和视觉门框检测进行验证。

Result: 在TII-RATM数据集上，相比独立VIO，ATE减少了56%到74%；消融实验显示双图架构在相同计算成本下比单图基线精度提高10%到12%；在A2RL竞赛中实现实时机载定位，每圈减少里程计漂移达4.2米。

Conclusion: 双位姿图架构有效解决了自主无人机竞速中的定位挑战，在保持实时性能的同时显著提高了定位精度和鲁棒性，为高速动态环境下的视觉定位提供了实用解决方案。
...
TL;DR: 本文提出了一个在非规则六边形网格上的确定性单车辆覆盖路径规划启发式算法基准测试，包含10,000个哈密顿可行实例、17种启发式算法和标准评估协议，发现显式最短路径重连算法能可靠解决松弛覆盖任务但几乎无法产生零重访路径。
...
Method: 创建了基于合成但具有海事动机的兴趣区域生成的10,000个哈密顿可行非规则六边形图实例，评估了来自7个家族的17种启发式算法，采用包含哈密顿成功率、完全覆盖率、重访次数、路径长度、航向变化和CPU延迟的统一评估协议。

Result: 显式最短路径重连算法能可靠解决松弛覆盖任务但几乎无法产生零重访路径；精确深度优先搜索确认所有实例都是哈密顿可行的；最强的经典哈密顿基线是使用基于索引的平局决胜和终端包含残差度策略的Warnsdorff变体，达到79.0%的哈密顿成功率。

Conclusion: 关键设计选择不是单独的平局决胜，而是在端点保留到最后一步时如何定义残差度，这表明未充分报告的实现细节会显著影响在具有瓶颈的稀疏几何图上的性能。该基准旨在为启发式算法分析提供受控测试平台，而非声称在舰队规模上的操作最优性。
...
TL;DR: 提出了一种分层时空动作标记器HiST-AT，用于上下文模仿学习，通过分层向量量化和时空信息利用，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有的动作标记方法在上下文模仿学习中可能未能充分利用时空信息，需要更有效的表示学习方法来提升模仿学习的性能。

Method: 提出分层方法：1) 两层向量量化，下层将输入动作分配到细粒度子簇，上层将子簇映射到簇；2) 扩展为分层时空动作标记器HiST-AT，同时利用空间和时间线索，进行多级聚类并恢复输入动作及其时间戳。

Result: 分层方法优于非分层对应方法，主要利用空间信息重建输入动作；HiST-AT在多个模拟和真实机器人操作基准测试中建立了新的最先进性能。

Conclusion: 分层时空动作标记器HiST-AT通过有效利用时空信息，显著提升了上下文模仿学习的性能，为机器人操作任务提供了强大的表示学习方法。
...
TL;DR: 提出一个基于视觉的人体姿态估计和运动预测框架，提供符合性预测保证，确保人机协作的安全认证
...
Method: 结合偶然不确定性估计与异常检测，提出符合性预测集用于人体运动预测
...
TL;DR: 该论文研究抽象模拟器到真实世界的强化学习迁移问题，提出通过真实世界数据修正抽象模拟器动力学的方法，实现成功的策略迁移。


<details>
  <summary>Details</summary>
Motivation: 随着机器人在复杂和大规模领域部署，模拟器难以建模所有任务细节。当模拟器遗漏关键任务细节时，需要研究如何从抽象模拟器训练策略并成功迁移到真实世界。

Method: 使用强化学习中的状态抽象语言形式化抽象sim2real问题，提出利用真实世界任务数据修正抽象模拟器动力学的方法，使策略能够成功迁移。

Result: 该方法在sim2sim和sim2real评估中都实现了成功的策略迁移，验证了方法的有效性。

Conclusion: 通过形式化抽象sim2real问题并引入基于真实世界数据修正模拟器动力学的方法，能够解决抽象模拟器遗漏任务细节时的策略迁移问题，为复杂环境中的机器人强化学习提供了解决方案。
