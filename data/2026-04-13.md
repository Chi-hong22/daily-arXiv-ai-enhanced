...
Method: 采用螺旋理论关节轴表示和等距流形学习，构建紧凑的几何保持潜在空间；从现有机械设计中学习设计搜索空间；通过运动重定向和Procrustes分析从人类运动数据直接定义损失函数；在潜在空间中使用无梯度优化进行设计优化。

Result: 建立了一个数据驱动机器人设计的原理性框架，证明利用现有设计和人类运动可以有效指导新型机器人设计的自动化发现。

Conclusion: 提出的方法最小化了人类参与，通过从现有设计中学习搜索空间和从人类运动数据定义损失，为机器人设计自动化提供了新的可行路径。
...
TL;DR: 提出了一种零样本的"文本到仿真到现实"生成框架，利用大语言模型和视觉语言模型自动从自然语言提示生成多样化的人机物理交互场景，用于训练视觉模仿学习策略并实现零样本仿真到现实迁移。


<details>
  <summary>Details</summary>
Motivation: 自主人机物理交互系统的发展受到大规模训练数据稀缺的限制，难以学习适用于现实应用的鲁棒机器人行为。

Method: 开发了零样本"text2sim2real"生成仿真框架，利用LLMs和VLMs从高级自然语言提示自动生成软体人体模型、场景布局和机器人运动轨迹。使用该框架自主收集大规模合成演示数据集，然后训练基于分割点云的视觉模仿学习策略。

Result: 在抓挠和洗澡两个物理辅助任务上进行了用户研究评估，学习到的策略成功实现了零样本仿真到现实迁移，成功率超过80%，并对非脚本化人体运动表现出鲁棒性。

Conclusion: 提出了首个用于人机物理交互应用的生成仿真流水线，实现了仿真环境合成、数据收集和策略学习的自动化，为解决大规模训练数据稀缺问题提供了创新方案。
...
TL;DR: 提出一个分层框架，通过视觉语言模型进行任务感知的双手机器人操作，联合推理接触区域和手臂分配，在真实世界任务中优于几何和语义基线方法。


<details>
  <summary>Details</summary>
Motivation: 双手机器人操作需要同时推理与物体的交互位置和手臂分配，这是一个几何规划器无法单独解决的联合问题。现有方法要么将可供性预测视为粗略的部分分割，要么依赖几何启发式进行手臂分配，无法联合推理任务相关的接触区域和手臂分配。

Method: 提出分层框架：1）将多视角RGB-D观测融合为一致的3D场景表示；2）生成全局6自由度抓取候选；3）通过查询视觉语言模型对每个物体进行任务相关可供性区域的空间和语义过滤；4）为各个物体分配手臂，确保几何有效性同时尊重任务语义。

Result: 在双手机器人平台上评估了9个真实世界操作任务，涵盖4个类别：并行操作、协调稳定、工具使用和人类交接。相比几何和语义基线方法，在任务导向抓取方面实现了更高的任务成功率。

Conclusion: 显式的语义推理（可供性和手臂分配）有助于在非结构化环境中实现可靠的双手机器人操作。视觉语言模型能够泛化到不同物体类别和任务描述，无需类别特定的训练。
...
TL;DR: 提出了一种可泛化的四足机器人世界模型(QWM)，通过显式编码机器人形态学参数，实现跨不同形态机器人的零样本控制，解决了现有世界模型对特定硬件过拟合的问题。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型通常针对特定机器人硬件训练，当机器人形态（如肢体长度、执行器动力学）发生变化时，模型会完全失效，需要重新训练。这限制了世界模型的通用性和实用性，特别是在四足机器人领域，不同机器人形态差异显著。

Method: 提出QWM框架，通过显式条件化生成动力学于机器人工程规格，而不是将静态物理属性作为隐变量从运动历史中推断。方法包括：1）物理形态编码器，2）奖励归一化器，使模型能够作为神经模拟器在不同形态间泛化。

Result: 首次实现了能够在不同形态四足机器人间进行零样本泛化的世界模型。模型能够作为分布有界插值器在四足机器人形态家族内工作，支持零样本控制，但并非通用物理引擎。

Conclusion: 该工作代表了向形态条件化世界模型的重要一步，为腿式机器人的通用世界模型提供了框架。虽然QWM在四足机器人形态家族内工作，而非通用物理引擎，但为跨形态泛化提供了可行方案。
...
TL;DR: 提出用于协作机械臂实时低级运动规划的通用接口，通过多项式插值和二次规划求解器生成平滑轨迹，在离线绘图、动态抓取和遥操作三个场景中验证了方法的可靠性、通用性和易用性。


<details>
  <summary>Details</summary>
Motivation: 为协作机械臂的实时低级运动规划提供通用接口，解决异构硬件平台间的可移植性问题，使机器人应用具有更广泛的适用性。

Method: 基于WinGs Operating Studio中间件，提供最小化但灵活的运动控制接口，采用n次多项式插值器和二次规划求解器，生成具有精确位置、速度和加速度轮廓的平滑连续可微分轨迹。

Result: 在三个场景中成功验证：1）离线演示中机械臂准确绘制几何图形；2）中断式低频重规划场景中机械臂抓取移动机器人上的动态物体；3）遥操作实验中一个机械臂控制另一个执行灵巧操作。

Conclusion: 提出的方法在协作机械臂实时运动规划中表现出可靠性、通用性和易用性，为异构机器人平台的标准化控制提供了有效解决方案。
...
TL;DR: 该研究提出了一种基于强化学习的单侧胫骨截肢者跑步运动仿真方法，使用混合连杆系统模拟叶片弹簧型运动假肢的柔性变形，通过仿真分析不同假肢刚度对跑步性能的影响。


<details>
  <summary>Details</summary>
Motivation: 运动假肢的设计和选择通常依赖试错法，缺乏考虑人体运动与假肢变形相互作用的综合分析。需要一种能够模拟真实条件与虚拟条件差异的仿真方法，为用户特定的假肢设计和选择提供有价值的见解。

Method: 采用混合连杆系统，结合分段恒定应变模型表示假肢的柔性变形。基于强化学习的方法生成全身动态运动，结合运动捕捉数据的模仿学习和精确的假肢动力学计算。在不同虚拟假肢刚度条件下进行跑步运动仿真。

Result: 成功模拟了不同假肢刚度条件下的跑步运动，分析了从仿真中获得的运输代谢成本。结果表明刚度变化会影响跑步性能，验证了该方法在虚拟条件下进行仿真和分析的潜力。

Conclusion: 该方法展示了在不同于真实条件的虚拟环境下进行仿真和分析的潜力，为运动假肢的用户特定设计和选择提供了有价值的工具，能够减少对试错法的依赖。
...
TL;DR: HTNav是一个新的协作导航框架，结合模仿学习和强化学习，用于解决复杂城市环境中的空中视觉语言导航问题，在CityNav基准测试中取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有空中视觉语言导航方法在复杂城市环境中面临多个挑战：对未见场景的泛化能力不足、长距离路径规划性能欠佳、以及对空间连续性理解不够充分。

Method: 提出HTNav框架，集成模仿学习和强化学习的混合IL-RL框架，采用分阶段训练机制确保基础导航策略稳定性，同时增强环境探索能力；引入分层决策机制实现宏观路径规划与细粒度动作控制的协作交互；增加地图表示学习模块以加深对开放域空间连续性的理解。

Result: 在CityNav基准测试中，该方法在所有场景级别和任务难度上都取得了最先进的性能，实验结果表明该框架显著提高了复杂城市环境中的导航精度和鲁棒性。

Conclusion: HTNav框架通过混合IL-RL方法、分层决策机制和地图表示学习，有效解决了复杂城市环境中空中视觉语言导航的关键挑战，为物流配送和城市巡检等应用提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型主要依赖粗粒度的2D感知，缺乏对3D几何的精确推理能力，而这对于机器人装配等精细操作任务至关重要。

Method: 提出AssemLM模型，采用专门的点云编码器捕捉细粒度的几何和旋转特征，然后将这些特征集成到多模态语言模型中，支持装配任务的准确3D空间推理。同时构建了包含90万+多模态样本的AssemBench数据集。

Result: AssemLM在多种装配场景中实现了6D姿态推理的最先进性能。真实机器人评估表明，该模型能够支持现实环境中的细粒度和多步骤装配执行。

Conclusion: AssemLM填补了现有具身AI基准在3D几何推理方面的空白，展示了在机器人装配应用中的潜力，为空间推理提供了从2D到3D的完整解决方案。
...
TL;DR: V-CAGE是一个用于自主机器人数据合成的智能体框架，通过上下文感知的场景生成和视觉语言模型闭环验证，实现高质量机器人操作数据集的规模化合成。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言-动作模型需要大量语义连贯且物理可行的数据集，但现有场景生成方法缺乏上下文感知能力，难以合成嵌入丰富语义信息的高保真环境，经常导致目标位置不可达，使任务过早失败。

Method: V-CAGE作为一个具身智能体系统，利用基础模型桥接高级语义推理与低级物理交互。具体包括：1）修复引导的场景构建，系统安排上下文感知布局；2）集成功能元数据与基于视觉语言模型的闭环验证机制，作为视觉批评家过滤无声故障；3）实现感知驱动的压缩算法，减少90%以上文件大小。

Result: 通过集中语义布局规划和视觉自验证，V-CAGE自动化端到端流程，实现了多样化、高质量机器人操作数据集的高度可扩展合成。

Conclusion: V-CAGE框架通过智能体化的数据合成方法，解决了现有场景生成方法的局限性，为视觉-语言-动作模型的规模化训练提供了高效的数据生成解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 空中视觉地点识别在变化环境条件下的鲁棒性对长期自主飞行至关重要。现有持续学习方法在处理地理特征严重类内变化时常常失败，需要解决灾难性遗忘问题。

Method: 将空中VPR建模为基于任务的域增量学习问题，提出异构记忆框架，包含"学习与处置"管道，将地理知识解耦为静态卫星锚点（保留全局几何先验）和动态经验回放缓冲区（保留域特定特征），引入空间约束分配策略优化缓冲区选择。

Result: 在21个多样化任务序列的基准测试中，多样性驱动的缓冲区选择比随机基线在知识保留方面提升7.8%，最大化结构多样性实现了更好的可塑性-稳定性平衡，在随机序列中保持顺序无关的鲁棒性。

Conclusion: 保持结构特征覆盖比样本难度对解决空中自主系统的灾难性遗忘更为关键，异构记忆框架显著提升了空间泛化能力。
...
TL;DR: 本文提出了一种用于一般冗余并联机器人（PKM）的统一术语体系，区分了运动学冗余和驱动冗余，引入了基于构型空间的运动学模型，并建立了非线性控制系统来分类不同的驱动方案。


<details>
  <summary>Details</summary>
Motivation: 并联机器人（PKM）同时存在运动学冗余和驱动冗余，但文献中对驱动冗余的分类不一致。本文旨在为一般冗余PKM建立统一的术语体系，澄清这两种冗余概念的区别。

Method: 引入以构型空间（c-space）为核心的PKM运动学模型，区分构型空间、输出和输入奇异性。建立基于构型空间的非线性控制系统，引入驱动度（DOA）作为独立控制向量场的数量，将PKM分为全驱动和欠驱动系统，并通过DOA与自由度（DOF）的关系来分类驱动冗余。

Result: 提出了统一的冗余PKM术语体系，明确了运动学冗余和驱动冗余的区别。从几何角度证明了通过冗余驱动方案可以避免输入奇异性。建立了基于构型空间的控制系统框架，为PKM的驱动方案分类提供了理论基础。

Conclusion: 本文为并联机器人的冗余概念提供了系统化的分类框架，强调了构型空间几何的重要性，证明了冗余驱动在避免奇异性方面的作用，为PKM的设计和控制提供了理论指导。
...
TL;DR: 该研究开发了用于微型飞行机器人栖息的柔软可拉伸电粘附垫，通过实验比较了不同电极设计的粘附性能，并在Crazyflie四旋翼上成功演示了在平滑表面的可重复栖息能力。


<details>
  <summary>Details</summary>
Motivation: 电粘附技术为微型飞行机器人提供了电控开关的粘附能力，但实际应用中柔软可拉伸电粘附垫的实现仍然有限。本研究旨在开发适用于空中栖息应用的柔软可拉伸电粘附垫。

Method: 1) 开发了高效制造工艺，制备具有正弦波和同心圆电极的多种尺寸柔软可拉伸电粘附垫；2) 使用Instron测试装置在非激活(0 kV)和激活(4.8 kV)条件下对比法向和剪切粘附力；3) 在配备电粘附脚的Crazyflie四旋翼上进行栖息演示，测试在平面和曲面基板上的性能。

Result: 剪切粘附力占主导地位，在部分垫接触情况下可达约3N；法向粘附力相对较小且强烈依赖于基板特性。Crazyflie原型机在光滑塑料表面（包括曲面几何形状）上展示了可重复的附着能力，并在电压移除时能够快速分离。

Conclusion: 柔软电粘附脚为微型飞行器提供了轻量可靠的栖息解决方案，剪切粘附力是实现有效栖息的关键因素，该技术展示了在微型空中机器人应用中的潜力。
...
TL;DR: 本文分析了激光雷达在道路环境中可能产生概率性多回波的情况，通过分析数据集展示了两种机械旋转激光雷达的累积分布函数，并提出了评估这些概率性多回波对激光雷达定位影响的定性方法。


<details>
  <summary>Details</summary>
Motivation: 激光雷达在自动驾驶车辆姿态估计中应用日益广泛，但对其在道路环境中的性能理解不足。现有激光雷达定位算法通常假设激光雷达沿给定射线路径总能识别唯一表面，然而实际中当多个散射表面出现在激光雷达锥形光束内时，可能产生概率性测量结果。

Method: 分析激光雷达数据集以识别特定射线路径上的概率性回波情况，为两种不同机械旋转激光雷达单元（固定基座）提供代表性的累积分布函数，并提出定性方法来评估概率性多回波情况对激光雷达定位的影响。

Result: 展示了两种机械旋转激光雷达单元在特定射线路径上的概率性回波累积分布函数，证实了激光雷达确实会在多个散射表面存在时产生概率性测量结果。

Conclusion: 激光雷达在道路环境中确实存在概率性多回波现象，这对激光雷达定位算法提出了挑战。提出的定性评估方法有助于理解这些概率性情况对定位性能的影响，为改进激光雷达定位算法提供了基础。
...
TL;DR: POMDAR是一个基于任务性能的机器人手灵巧性基准测试，通过结构化操作和抓取动作集合来形式化定义灵巧性，提供客观、可重复的评估框架。


<details>
  <summary>Details</summary>
Motivation: 当前机器人手灵巧性定义模糊且不一致，不同系统使用不同标准评估，导致设计间难以进行有意义的比较，需要基于可测量结果的统一性能定义。

Method: 从人类运动控制分类学系统推导出基准测试，包含四种操作配置（垂直、水平、连续旋转、纯抓取），通过机械支架约束任务运动、抑制补偿策略，定义结合任务正确性和执行速度的量化评分指标。

Result: 开发了POMDAR基准测试，在真实世界和仿真中实现，提供开源标准化评估框架，能够客观、可重复地评估不同手设计，促进灵巧操作平台的系统发展。

Conclusion: POMDAR为拟人机器人手提供了基于分类学的标准化基准测试，实现了灵巧性的统一、可测量定义，有助于系统比较和评估不同设计，推动灵巧操作技术的进步。
...
<details>
  <summary>Details</summary>
Motivation: 自主系统在复杂环境中需要实时理解其他智能体的意图，但现有方法难以处理时变意图和未知参数的问题。
...
Conclusion: 提出的在线意图预测框架能够有效处理时变意图和未知参数，为自主系统在动态环境中的实时决策提供了可靠支持。
...
TL;DR: 提出了一种鲁棒自适应反步阻抗控制策略，用于接触丰富和不确定环境中的机器人操作，无需机器人动态参数即可实现阻抗跟踪和不确定性处理。


<details>
  <summary>Details</summary>
Motivation: 针对机器人在接触丰富和不确定环境中操作时面临的挑战，需要一种能够处理系统耦合动态、外部扰动和未建模动态的控制策略，同时不依赖精确的机器人动态参数。

Method: 采用基于反步法的自适应阻抗控制方案作为内环跟踪参考阻抗模型；使用泰勒级数估计器处理系统动态不确定性，自适应估计器确定外力的上界；通过稳定性分析证明系统的半全局实用有限时间稳定性。
...
TL;DR: MADRI框架通过将视频流转换为特征向量，并结合机器人内部传感器数据和场景图进行重建式异常检测，提高了人机协作中的异常检测性能。


<details>
  <summary>Details</summary>
Motivation: 在人机交互中确保安全和可靠性需要及时检测可能导致系统故障或不安全行为的意外事件。虽然重建模型在人机交互中已被积极探索，但直接在特征向量上操作的方法仍未被充分研究。

Method: 提出MADRI框架：1）将视频流转换为语义上有意义的特征向量；2）进行基于重建的异常检测；3）用机器人内部传感器读数和场景图增强视觉特征向量，以捕捉视觉环境中的外部异常和机器人内部的故障。

Result: 实验结果表明：仅基于视觉特征向量的重建对异常检测有效，而结合其他模态进一步提高了检测性能，突显了多模态特征重建在人机协作中稳健异常检测的优势。

Conclusion: 多模态特征重建方法能够有效检测人机协作中的异常，结合视觉特征、内部传感器和场景图信息可以更全面地捕捉外部和内部异常，提高系统的安全性和可靠性。
...
TL;DR: VAG是一个统一的流匹配双流框架，能联合生成视频和动作，解决现有世界模型在策略学习中的局限性


<details>
  <summary>Details</summary>
Motivation: 机器人基础模型需要大量人类遥操作数据，但收集任务特定演示成本高昂。合成数据（特别是生成视频）是可行方向，但现有世界模型不适合策略学习，因为它们不提供配对的动作轨迹。世界-动作模型部分解决了这个问题，但缺乏视频-动作对齐，而两阶段流水线效率低且误差累积

Method: 提出VAG，一个基于流匹配的统一双流框架，在视觉和语言条件下联合生成视频和动作。通过同步两个分支的去噪过程，并使用自适应3D池化机制将紧凑的全局视频上下文传输到动作分支，提高生成过程中的跨模态一致性

Result: 在模拟和真实世界设置中，VAG能生成对齐的视频-动作对，具有竞争力的预测质量，支持可执行轨迹回放，并提供有用的合成预训练数据，改善下游策略泛化能力

Conclusion: VAG作为一个实用的世界-动作模型，在具身数据合成方面具有潜力，能够生成高质量的对齐视频-动作对，支持机器人策略学习
...
TL;DR: 使用肌肉骨骼模拟与强化学习结合的设备无关框架，为下肢外骨骼开发可泛化的个性化控制策略，无需大量数据收集或迭代优化。
...
Method: 结合生理合理的肌肉骨骼模拟与强化学习，开发设备无关框架，生成生理合理的运动动力学并捕捉临床观察到的代偿策略。

Result: 控制策略产生的髋关节和踝关节辅助扭矩与人类实验中验证的最先进配置一致，在不同步行速度下持续降低代谢成本；对于模拟的步态障碍模型，策略提供不对称、缺陷特定的外骨骼辅助，改善能量效率和双侧运动对称性。

Conclusion: 通过强化学习的生理合理肌肉骨骼模拟可以作为可扩展的基础，为健全人群和临床人群提供个性化的外骨骼控制，无需大量物理试验。
...
TL;DR: Adaptor是一个少样本框架，通过两阶段处理解决辅助遥操作中因操作者习惯差异导致的轨迹分布异质性问题，实现稳健的跨操作者意图识别。


<details>
  <summary>Details</summary>
Motivation: 辅助遥操作通过共享控制提高效率，但不同操作者的习惯和专业知识差异导致高度异质的轨迹分布，这会削弱意图识别的稳定性。

Method: Adaptor采用两阶段方法：1)预处理阶段通过噪声注入合成轨迹扰动来建模意图不确定性，并进行几何感知的关键帧提取；2)策略学习阶段使用意图专家编码处理后的轨迹，并将其与预训练视觉语言模型上下文融合，以条件化动作专家生成动作。

Result: 在真实世界和模拟基准测试中，Adaptor实现了最先进的性能，在成功率和效率上均优于基线方法。此外，该方法在不同专业水平的操作者之间表现出低方差，展示了稳健的跨操作者泛化能力。

Conclusion: Adaptor框架通过有效处理操作者间的轨迹分布异质性，实现了稳健的跨操作者意图识别，为辅助遥操作系统提供了实用的解决方案。
...
TL;DR: SafeMind是一个可微分的随机安全控制框架，将概率控制屏障函数与语义上下文理解和元自适应风险校准相结合，为四足机器人在不确定环境下提供形式化安全保证。


<details>
  <summary>Details</summary>
Motivation: 基于学习的四足控制器虽然表现出色，但通常缺乏在模型不确定性、感知噪声和非结构化接触条件下的形式化安全保证。现有方法难以同时处理随机性和语义上下文，需要一种能统一概率安全保证与语义理解的控制框架。

Method: SafeMind框架包含：1）通过方差感知屏障约束在可微二次规划中显式建模认知和偶然不确定性；2）语义到约束编码器利用感知或语言线索调节安全边界；3）元自适应学习器在不同环境中连续调整风险敏感性。框架提供概率前向不变性、可行性和稳定性的理论条件。

Result: 在Unitree A1和ANYmal C机器人上以200Hz频率部署，验证了12种地形类型、动态障碍物、形态扰动和语义定义任务。相比最先进的CBF、MPC和混合RL基线，安全违规减少3-10倍，能耗降低10-15%，同时保持实时控制性能。

Conclusion: SafeMind成功将概率安全保证与语义上下文理解统一起来，为四足机器人在不确定和非结构化环境中提供了形式化安全保证，显著提高了安全性和能效，同时保持实时控制性能。
